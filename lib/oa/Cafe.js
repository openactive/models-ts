...
exports.validateCafe = exports.CafeOrSubClassJoiSchema = exports.CafeJoiSchema = void 0;
 ...
 */
exports.CafeJoiSchema = Joi.object({
    '@type': Joi.string().valid('Cafe').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.CafeOrSubClassJoiSchema = Joi.alternatives().try([
    exports.CafeJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeCafe = Cafe.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateCafe(maybeCafe) {
    const { value, error } = exports.CafeJoiSchema.validate(maybeCafe);
    if (error) {
        return new oaValidationError_1.OaValidationError('Cafe', maybeCafe, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateCafe = validateCafe;
