...
exports.validateApartmentComplex = exports.ApartmentComplexOrSubClassJoiSchema = exports.ApartmentComplexJoiSchema = void 0;
 ...
 */
exports.ApartmentComplexJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.ApartmentComplexOrSubClassJoiSchema = Joi.alternatives().try([
    exports.ApartmentComplexJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeApartmentComplex = ApartmentComplex.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateApartmentComplex(maybeApartmentComplex) {
    const { value, error } = exports.ApartmentComplexJoiSchema.validate(maybeApartmentComplex);
    if (error) {
        return new oaValidationError_1.OaValidationError('ApartmentComplex', maybeApartmentComplex, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateApartmentComplex = validateApartmentComplex;
