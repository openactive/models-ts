...
exports.validateAtlas = exports.AtlasOrSubClassJoiSchema = exports.AtlasJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:Atlas - Validation schema (w/ JOI)
 */
exports.AtlasJoiSchema = Joi.object({
    '@type': Joi.string().valid('Atlas').required(),
 ...
 * schema:Atlas - Validation schema (w/ JOI)
 *
 * This differs from AtlasJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.AtlasOrSubClassJoiSchema = Joi.alternatives().try([
    exports.AtlasJoiSchema,
]);
/**
 * Runtime validator for schema:Atlas.
 *
 * If some data has a structure which matches a schema:Atlas, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAtlas = Atlas.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAtlas instanceof OaValidationError) {
 *   // From this point on, `maybeAtlas` will have type `OaValidationError`
 *   const error = maybeAtlas;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAtlas` will have type `Atlas`
 * const atlas = maybeAtlas;
 * ```
 */
function validateAtlas(maybeAtlas) {
    const { value, error } = exports.AtlasJoiSchema.validate(maybeAtlas);
    if (error) {
        return new oaValidationError_1.OaValidationError('Atlas', maybeAtlas, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateAtlas = validateAtlas;
