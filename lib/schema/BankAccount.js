...
exports.validateBankAccount = exports.BankAccountOrSubClassJoiSchema = exports.BankAccountJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:BankAccount - Validation schema (w/ JOI)
 */
exports.BankAccountJoiSchema = Joi.object({
    '@type': Joi.string().valid('BankAccount').required(),
    '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
    identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
    name: Joi.string(),
    description: Joi.string(),
    '@id': Joi.string().uri(),
    accountMinimumInflow: Joi.alternatives().try(Joi.lazy(() => schema.MonetaryAmountOrSubClassJoiSchema), Joi.string().uri()),
    accountOverdraftLimit: Joi.alternatives().try(Joi.lazy(() => schema.MonetaryAmountOrSubClassJoiSchema), Joi.string().uri()),
    bankAccountType: Joi.alternatives().try(Joi.string().uri(), Joi.string()),
 ...
 * schema:BankAccount - Validation schema (w/ JOI)
 *
 * This differs from BankAccountJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.BankAccountOrSubClassJoiSchema = Joi.alternatives().try([
    exports.BankAccountJoiSchema,
    Joi.lazy(() => schema.DepositAccountOrSubClassJoiSchema),
]);
/**
 * Runtime validator for schema:BankAccount.
 *
 * If some data has a structure which matches a schema:BankAccount, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBankAccount = BankAccount.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBankAccount instanceof OaValidationError) {
 *   // From this point on, `maybeBankAccount` will have type `OaValidationError`
 *   const error = maybeBankAccount;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBankAccount` will have type `BankAccount`
 * const bankAccount = maybeBankAccount;
 * ```
 */
function validateBankAccount(maybeBankAccount) {
    const { value, error } = exports.BankAccountJoiSchema.validate(maybeBankAccount);
    if (error) {
        return new oaValidationError_1.OaValidationError('BankAccount', maybeBankAccount, error);
    }
