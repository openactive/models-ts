...
exports.validateBroadcastService = exports.BroadcastServiceOrSubClassJoiSchema = exports.BroadcastServiceJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:BroadcastService - Validation schema (w/ JOI)
 */
exports.BroadcastServiceJoiSchema = Joi.object({
    '@type': Joi.string().valid('BroadcastService').required(),
 ...
 * schema:BroadcastService - Validation schema (w/ JOI)
 *
 * This differs from BroadcastServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.BroadcastServiceOrSubClassJoiSchema = Joi.alternatives().try([
    exports.BroadcastServiceJoiSchema,
    Joi.lazy(() => schema.RadioBroadcastServiceOrSubClassJoiSchema),
]);
/**
 * Runtime validator for schema:BroadcastService.
 *
 * If some data has a structure which matches a schema:BroadcastService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBroadcastService = BroadcastService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBroadcastService instanceof OaValidationError) {
 *   // From this point on, `maybeBroadcastService` will have type `OaValidationError`
 *   const error = maybeBroadcastService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBroadcastService` will have type `BroadcastService`
 * const broadcastService = maybeBroadcastService;
 * ```
 */
function validateBroadcastService(maybeBroadcastService) {
    const { value, error } = exports.BroadcastServiceJoiSchema.validate(maybeBroadcastService);
    if (error) {
        return new oaValidationError_1.OaValidationError('BroadcastService', maybeBroadcastService, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateBroadcastService = validateBroadcastService;
