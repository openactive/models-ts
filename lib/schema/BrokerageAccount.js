...
exports.validateBrokerageAccount = exports.BrokerageAccountOrSubClassJoiSchema = exports.BrokerageAccountJoiSchema = void 0;
 ...
 */
exports.BrokerageAccountJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.BrokerageAccountOrSubClassJoiSchema = Joi.alternatives().try([
    exports.BrokerageAccountJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeBrokerageAccount = validateBrokerageAccount(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateBrokerageAccount(maybeBrokerageAccount) {
    const { value, error } = exports.BrokerageAccountJoiSchema.validate(maybeBrokerageAccount);
    if (error) {
        return new oaValidationError_1.OaValidationError('BrokerageAccount', maybeBrokerageAccount, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateBrokerageAccount = validateBrokerageAccount;
