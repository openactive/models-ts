...
exports.validateBusStation = exports.BusStationOrSubClassJoiSchema = exports.BusStationJoiSchema = void 0;
 ...
 */
exports.BusStationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.BusStationOrSubClassJoiSchema = Joi.alternatives().try([
    exports.BusStationJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeBusStation = validateBusStation(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateBusStation(maybeBusStation) {
    const { value, error } = exports.BusStationJoiSchema.validate(maybeBusStation);
    if (error) {
        return new oaValidationError_1.OaValidationError('BusStation', maybeBusStation, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateBusStation = validateBusStation;
