...
exports.validateCableOrSatelliteService = exports.CableOrSatelliteServiceOrSubClassJoiSchema = exports.CableOrSatelliteServiceJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:CableOrSatelliteService - Validation schema (w/ JOI)
 */
exports.CableOrSatelliteServiceJoiSchema = Joi.object({
    '@type': Joi.string().valid('CableOrSatelliteService').required(),
 ...
 * schema:CableOrSatelliteService - Validation schema (w/ JOI)
 *
 * This differs from CableOrSatelliteServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.CableOrSatelliteServiceOrSubClassJoiSchema = Joi.alternatives().try([
    exports.CableOrSatelliteServiceJoiSchema,
]);
/**
 * Runtime validator for schema:CableOrSatelliteService.
 *
 * If some data has a structure which matches a schema:CableOrSatelliteService, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCableOrSatelliteService` will have type `CableOrSatelliteService`
 * const cableOrSatelliteService = maybeCableOrSatelliteService;
 * ```
 */
function validateCableOrSatelliteService(maybeCableOrSatelliteService) {
    const { value, error } = exports.CableOrSatelliteServiceJoiSchema.validate(maybeCableOrSatelliteService);
    if (error) {
        return new oaValidationError_1.OaValidationError('CableOrSatelliteService', maybeCableOrSatelliteService, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateCableOrSatelliteService = validateCableOrSatelliteService;
