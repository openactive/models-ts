...
exports.validateCheckoutPage = exports.CheckoutPageOrSubClassJoiSchema = exports.CheckoutPageJoiSchema = void 0;
 ...
 */
exports.CheckoutPageJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.CheckoutPageOrSubClassJoiSchema = Joi.alternatives().try([
    exports.CheckoutPageJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeCheckoutPage = CheckoutPage.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateCheckoutPage(maybeCheckoutPage) {
    const { value, error } = exports.CheckoutPageJoiSchema.validate(maybeCheckoutPage);
    if (error) {
        return new oaValidationError_1.OaValidationError('CheckoutPage', maybeCheckoutPage, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateCheckoutPage = validateCheckoutPage;
