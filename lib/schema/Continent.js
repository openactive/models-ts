...
exports.validateContinent = exports.ContinentOrSubClassJoiSchema = exports.ContinentJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:Continent - Validation schema (w/ JOI)
 */
exports.ContinentJoiSchema = Joi.object({
    '@type': Joi.string().valid('Continent').required(),
 ...
 * schema:Continent - Validation schema (w/ JOI)
 *
 * This differs from ContinentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.ContinentOrSubClassJoiSchema = Joi.alternatives().try([
    exports.ContinentJoiSchema,
]);
/**
 * Runtime validator for schema:Continent.
 *
 * If some data has a structure which matches a schema:Continent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeContinent = Continent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeContinent instanceof OaValidationError) {
 *   // From this point on, `maybeContinent` will have type `OaValidationError`
 *   const error = maybeContinent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeContinent` will have type `Continent`
 * const continent = maybeContinent;
 * ```
 */
function validateContinent(maybeContinent) {
    const { value, error } = exports.ContinentJoiSchema.validate(maybeContinent);
    if (error) {
        return new oaValidationError_1.OaValidationError('Continent', maybeContinent, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateContinent = validateContinent;
