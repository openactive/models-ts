...
exports.validateCurrencyConversionService = exports.CurrencyConversionServiceOrSubClassJoiSchema = exports.CurrencyConversionServiceJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:CurrencyConversionService - Validation schema (w/ JOI)
 */
exports.CurrencyConversionServiceJoiSchema = Joi.object({
    '@type': Joi.string().valid('CurrencyConversionService').required(),
 ...
 * schema:CurrencyConversionService - Validation schema (w/ JOI)
 *
 * This differs from CurrencyConversionServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.CurrencyConversionServiceOrSubClassJoiSchema = Joi.alternatives().try([
    exports.CurrencyConversionServiceJoiSchema,
]);
/**
 * Runtime validator for schema:CurrencyConversionService.
 *
 * If some data has a structure which matches a schema:CurrencyConversionService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCurrencyConversionService = CurrencyConversionService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCurrencyConversionService instanceof OaValidationError) {
 *   // From this point on, `maybeCurrencyConversionService` will have type `OaValidationError`
 *   const error = maybeCurrencyConversionService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCurrencyConversionService` will have type `CurrencyConversionService`
 * const currencyConversionService = maybeCurrencyConversionService;
 * ```
 */
function validateCurrencyConversionService(maybeCurrencyConversionService) {
    const { value, error } = exports.CurrencyConversionServiceJoiSchema.validate(maybeCurrencyConversionService);
    if (error) {
        return new oaValidationError_1.OaValidationError('CurrencyConversionService', maybeCurrencyConversionService, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateCurrencyConversionService = validateCurrencyConversionService;
