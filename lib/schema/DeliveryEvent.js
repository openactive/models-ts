...
exports.validateDeliveryEvent = exports.DeliveryEventOrSubClassJoiSchema = exports.DeliveryEventJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:DeliveryEvent - Validation schema (w/ JOI)
 */
exports.DeliveryEventJoiSchema = Joi.object({
    '@type': Joi.string().valid('DeliveryEvent').required(),
    ...
    availableFrom: Joi.string().isoDate(),
    availableThrough: Joi.string().isoDate(),
 ...
 * schema:DeliveryEvent - Validation schema (w/ JOI)
 *
 * This differs from DeliveryEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.DeliveryEventOrSubClassJoiSchema = Joi.alternatives().try([
    exports.DeliveryEventJoiSchema,
]);
/**
 * Runtime validator for schema:DeliveryEvent.
 *
 * If some data has a structure which matches a schema:DeliveryEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDeliveryEvent = DeliveryEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDeliveryEvent instanceof OaValidationError) {
 *   // From this point on, `maybeDeliveryEvent` will have type `OaValidationError`
 *   const error = maybeDeliveryEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDeliveryEvent` will have type `DeliveryEvent`
 * const deliveryEvent = maybeDeliveryEvent;
 * ```
 */
function validateDeliveryEvent(maybeDeliveryEvent) {
    const { value, error } = exports.DeliveryEventJoiSchema.validate(maybeDeliveryEvent);
    if (error) {
        return new oaValidationError_1.OaValidationError('DeliveryEvent', maybeDeliveryEvent, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateDeliveryEvent = validateDeliveryEvent;
