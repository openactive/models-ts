...
exports.validateDiagnosticLab = exports.DiagnosticLabOrSubClassJoiSchema = exports.DiagnosticLabJoiSchema = void 0;
 ...
 */
exports.DiagnosticLabJoiSchema = Joi.object({
    '@type': Joi.string().valid('DiagnosticLab').required(),
    '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
    identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
    name: Joi.string(),
    description: Joi.string(),
    '@id': Joi.string().uri(),
    availableTest: Joi.alternatives().try(Joi.lazy(() => schema.MedicalTestOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
exports.DiagnosticLabOrSubClassJoiSchema = Joi.alternatives().try([
    exports.DiagnosticLabJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeDiagnosticLab = validateDiagnosticLab(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateDiagnosticLab(maybeDiagnosticLab) {
    const { value, error } = exports.DiagnosticLabJoiSchema.validate(maybeDiagnosticLab);
    if (error) {
        return new oaValidationError_1.OaValidationError('DiagnosticLab', maybeDiagnosticLab, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateDiagnosticLab = validateDiagnosticLab;
