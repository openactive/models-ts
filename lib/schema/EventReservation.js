...
exports.validateEventReservation = exports.EventReservationOrSubClassJoiSchema = exports.EventReservationJoiSchema = void 0;
 ...
 */
exports.EventReservationJoiSchema = Joi.object({
    '@type': Joi.string().valid('EventReservation').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.EventReservationOrSubClassJoiSchema = Joi.alternatives().try([
    exports.EventReservationJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeEventReservation = EventReservation.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateEventReservation(maybeEventReservation) {
    const { value, error } = exports.EventReservationJoiSchema.validate(maybeEventReservation);
    if (error) {
        return new oaValidationError_1.OaValidationError('EventReservation', maybeEventReservation, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateEventReservation = validateEventReservation;
