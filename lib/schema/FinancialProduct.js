...
exports.validateFinancialProduct = exports.FinancialProductOrSubClassJoiSchema = exports.FinancialProductJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:FinancialProduct - Validation schema (w/ JOI)
 */
exports.FinancialProductJoiSchema = Joi.object({
    '@type': Joi.string().valid('FinancialProduct').required(),
 ...
 * schema:FinancialProduct - Validation schema (w/ JOI)
 *
 * This differs from FinancialProductJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.FinancialProductOrSubClassJoiSchema = Joi.alternatives().try([
    exports.FinancialProductJoiSchema,
    Joi.lazy(() => schema.BankAccountOrSubClassJoiSchema),
    Joi.lazy(() => schema.CurrencyConversionServiceOrSubClassJoiSchema),
    Joi.lazy(() => schema.InvestmentOrDepositOrSubClassJoiSchema),
    Joi.lazy(() => schema.LoanOrCreditOrSubClassJoiSchema),
    Joi.lazy(() => schema.PaymentCardOrSubClassJoiSchema),
    Joi.lazy(() => schema.PaymentServiceOrSubClassJoiSchema),
]);
/**
 * Runtime validator for schema:FinancialProduct.
 *
 * If some data has a structure which matches a schema:FinancialProduct, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeFinancialProduct` will have type `FinancialProduct`
 * const financialProduct = maybeFinancialProduct;
 * ```
 */
function validateFinancialProduct(maybeFinancialProduct) {
    const { value, error } = exports.FinancialProductJoiSchema.validate(maybeFinancialProduct);
    if (error) {
        return new oaValidationError_1.OaValidationError('FinancialProduct', maybeFinancialProduct, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateFinancialProduct = validateFinancialProduct;
