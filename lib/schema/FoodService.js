...
exports.validateFoodService = exports.FoodServiceOrSubClassJoiSchema = exports.FoodServiceJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:FoodService - Validation schema (w/ JOI)
 */
exports.FoodServiceJoiSchema = Joi.object({
    '@type': Joi.string().valid('FoodService').required(),
 ...
 * schema:FoodService - Validation schema (w/ JOI)
 *
 * This differs from FoodServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.FoodServiceOrSubClassJoiSchema = Joi.alternatives().try([
    exports.FoodServiceJoiSchema,
]);
/**
 * Runtime validator for schema:FoodService.
 *
 * If some data has a structure which matches a schema:FoodService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFoodService = FoodService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFoodService instanceof OaValidationError) {
 *   // From this point on, `maybeFoodService` will have type `OaValidationError`
 *   const error = maybeFoodService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFoodService` will have type `FoodService`
 * const foodService = maybeFoodService;
 * ```
 */
function validateFoodService(maybeFoodService) {
    const { value, error } = exports.FoodServiceJoiSchema.validate(maybeFoodService);
    if (error) {
        return new oaValidationError_1.OaValidationError('FoodService', maybeFoodService, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateFoodService = validateFoodService;
