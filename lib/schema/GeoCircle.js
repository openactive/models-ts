...
exports.validateGeoCircle = exports.GeoCircleOrSubClassJoiSchema = exports.GeoCircleJoiSchema = void 0;
 ...
 */
exports.GeoCircleJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.GeoCircleOrSubClassJoiSchema = Joi.alternatives().try([
    exports.GeoCircleJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeGeoCircle = validateGeoCircle(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateGeoCircle(maybeGeoCircle) {
    const { value, error } = exports.GeoCircleJoiSchema.validate(maybeGeoCircle);
    if (error) {
        return new oaValidationError_1.OaValidationError('GeoCircle', maybeGeoCircle, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateGeoCircle = validateGeoCircle;
