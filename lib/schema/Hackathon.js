...
exports.validateHackathon = exports.HackathonOrSubClassJoiSchema = exports.HackathonJoiSchema = void 0;
 ...
 */
exports.HackathonJoiSchema = Joi.object({
    '@type': Joi.string().valid('Hackathon').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.HackathonOrSubClassJoiSchema = Joi.alternatives().try([
    exports.HackathonJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeHackathon = Hackathon.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateHackathon(maybeHackathon) {
    const { value, error } = exports.HackathonJoiSchema.validate(maybeHackathon);
    if (error) {
        return new oaValidationError_1.OaValidationError('Hackathon', maybeHackathon, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateHackathon = validateHackathon;
