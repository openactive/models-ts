...
exports.validateHealthPlanNetwork = exports.HealthPlanNetworkOrSubClassJoiSchema = exports.HealthPlanNetworkJoiSchema = void 0;
 ...
 */
exports.HealthPlanNetworkJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.HealthPlanNetworkOrSubClassJoiSchema = Joi.alternatives().try([
    exports.HealthPlanNetworkJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeHealthPlanNetwork = HealthPlanNetwork.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateHealthPlanNetwork(maybeHealthPlanNetwork) {
    const { value, error } = exports.HealthPlanNetworkJoiSchema.validate(maybeHealthPlanNetwork);
    if (error) {
        return new oaValidationError_1.OaValidationError('HealthPlanNetwork', maybeHealthPlanNetwork, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateHealthPlanNetwork = validateHealthPlanNetwork;
