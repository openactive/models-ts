...
exports.validateInvestmentOrDeposit = exports.InvestmentOrDepositOrSubClassJoiSchema = exports.InvestmentOrDepositJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:InvestmentOrDeposit - Validation schema (w/ JOI)
 */
exports.InvestmentOrDepositJoiSchema = Joi.object({
    '@type': Joi.string().valid('InvestmentOrDeposit').required(),
 ...
 * schema:InvestmentOrDeposit - Validation schema (w/ JOI)
 *
 * This differs from InvestmentOrDepositJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.InvestmentOrDepositOrSubClassJoiSchema = Joi.alternatives().try([
    exports.InvestmentOrDepositJoiSchema,
    Joi.lazy(() => schema.DepositAccountOrSubClassJoiSchema),
    Joi.lazy(() => schema.InvestmentFundOrSubClassJoiSchema),
    Joi.lazy(() => schema.BrokerageAccountOrSubClassJoiSchema),
]);
/**
 * Runtime validator for schema:InvestmentOrDeposit.
 *
 * If some data has a structure which matches a schema:InvestmentOrDeposit, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeInvestmentOrDeposit = InvestmentOrDeposit.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeInvestmentOrDeposit instanceof OaValidationError) {
 *   // From this point on, `maybeInvestmentOrDeposit` will have type `OaValidationError`
 *   const error = maybeInvestmentOrDeposit;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeInvestmentOrDeposit` will have type `InvestmentOrDeposit`
 * const investmentOrDeposit = maybeInvestmentOrDeposit;
 * ```
 */
function validateInvestmentOrDeposit(maybeInvestmentOrDeposit) {
    const { value, error } = exports.InvestmentOrDepositJoiSchema.validate(maybeInvestmentOrDeposit);
    if (error) {
        return new oaValidationError_1.OaValidationError('InvestmentOrDeposit', maybeInvestmentOrDeposit, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateInvestmentOrDeposit = validateInvestmentOrDeposit;
