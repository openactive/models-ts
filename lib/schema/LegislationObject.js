...
exports.validateLegislationObject = exports.LegislationObjectOrSubClassJoiSchema = exports.LegislationObjectJoiSchema = void 0;
 ...
 */
exports.LegislationObjectJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.LegislationObjectOrSubClassJoiSchema = Joi.alternatives().try([
    exports.LegislationObjectJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeLegislationObject = LegislationObject.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateLegislationObject(maybeLegislationObject) {
    const { value, error } = exports.LegislationObjectJoiSchema.validate(maybeLegislationObject);
    if (error) {
        return new oaValidationError_1.OaValidationError('LegislationObject', maybeLegislationObject, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateLegislationObject = validateLegislationObject;
