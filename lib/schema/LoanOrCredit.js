...
exports.validateLoanOrCredit = exports.LoanOrCreditOrSubClassJoiSchema = exports.LoanOrCreditJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:LoanOrCredit - Validation schema (w/ JOI)
 */
exports.LoanOrCreditJoiSchema = Joi.object({
    '@type': Joi.string().valid('LoanOrCredit').required(),
 ...
 * schema:LoanOrCredit - Validation schema (w/ JOI)
 *
 * This differs from LoanOrCreditJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.LoanOrCreditOrSubClassJoiSchema = Joi.alternatives().try([
    exports.LoanOrCreditJoiSchema,
    Joi.lazy(() => schema.CreditCardOrSubClassJoiSchema),
    Joi.lazy(() => schema.MortgageLoanOrSubClassJoiSchema),
]);
/**
 * Runtime validator for schema:LoanOrCredit.
 *
 * If some data has a structure which matches a schema:LoanOrCredit, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLoanOrCredit = LoanOrCredit.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLoanOrCredit instanceof OaValidationError) {
 *   // From this point on, `maybeLoanOrCredit` will have type `OaValidationError`
 *   const error = maybeLoanOrCredit;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLoanOrCredit` will have type `LoanOrCredit`
 * const loanOrCredit = maybeLoanOrCredit;
 * ```
 */
function validateLoanOrCredit(maybeLoanOrCredit) {
    const { value, error } = exports.LoanOrCreditJoiSchema.validate(maybeLoanOrCredit);
    if (error) {
        return new oaValidationError_1.OaValidationError('LoanOrCredit', maybeLoanOrCredit, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateLoanOrCredit = validateLoanOrCredit;
