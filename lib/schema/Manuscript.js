...
exports.validateManuscript = exports.ManuscriptOrSubClassJoiSchema = exports.ManuscriptJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:Manuscript - Validation schema (w/ JOI)
 */
exports.ManuscriptJoiSchema = Joi.object({
    '@type': Joi.string().valid('Manuscript').required(),
 ...
 * schema:Manuscript - Validation schema (w/ JOI)
 *
 * This differs from ManuscriptJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.ManuscriptOrSubClassJoiSchema = Joi.alternatives().try([
    exports.ManuscriptJoiSchema,
]);
/**
 * Runtime validator for schema:Manuscript.
 *
 * If some data has a structure which matches a schema:Manuscript, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeManuscript = Manuscript.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeManuscript instanceof OaValidationError) {
 *   // From this point on, `maybeManuscript` will have type `OaValidationError`
 *   const error = maybeManuscript;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeManuscript` will have type `Manuscript`
 * const manuscript = maybeManuscript;
 * ```
 */
function validateManuscript(maybeManuscript) {
    const { value, error } = exports.ManuscriptJoiSchema.validate(maybeManuscript);
    if (error) {
        return new oaValidationError_1.OaValidationError('Manuscript', maybeManuscript, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateManuscript = validateManuscript;
