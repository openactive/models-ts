...
exports.validateMedicalSignOrSymptom = exports.MedicalSignOrSymptomOrSubClassJoiSchema = exports.MedicalSignOrSymptomJoiSchema = void 0;
 ...
 */
exports.MedicalSignOrSymptomJoiSchema = Joi.object({
    '@type': Joi.string().valid('MedicalSignOrSymptom').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.MedicalSignOrSymptomOrSubClassJoiSchema = Joi.alternatives().try([
    exports.MedicalSignOrSymptomJoiSchema,
    Joi.lazy(() => schema.MedicalSymptomOrSubClassJoiSchema),
    Joi.lazy(() => schema.MedicalSignOrSubClassJoiSchema),
]);
/**
 ...
 *
 * ```ts
 * const maybeMedicalSignOrSymptom = validateMedicalSignOrSymptom(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateMedicalSignOrSymptom(maybeMedicalSignOrSymptom) {
    const { value, error } = exports.MedicalSignOrSymptomJoiSchema.validate(maybeMedicalSignOrSymptom);
    if (error) {
        return new oaValidationError_1.OaValidationError('MedicalSignOrSymptom', maybeMedicalSignOrSymptom, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateMedicalSignOrSymptom = validateMedicalSignOrSymptom;
