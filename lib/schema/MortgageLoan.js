...
exports.validateMortgageLoan = exports.MortgageLoanOrSubClassJoiSchema = exports.MortgageLoanJoiSchema = void 0;
 ...
 */
exports.MortgageLoanJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.MortgageLoanOrSubClassJoiSchema = Joi.alternatives().try([
    exports.MortgageLoanJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeMortgageLoan = MortgageLoan.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateMortgageLoan(maybeMortgageLoan) {
    const { value, error } = exports.MortgageLoanJoiSchema.validate(maybeMortgageLoan);
    if (error) {
        return new oaValidationError_1.OaValidationError('MortgageLoan', maybeMortgageLoan, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateMortgageLoan = validateMortgageLoan;
