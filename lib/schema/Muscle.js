...
exports.validateMuscle = exports.MuscleOrSubClassJoiSchema = exports.MuscleJoiSchema = void 0;
 ...
 */
exports.MuscleJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.MuscleOrSubClassJoiSchema = Joi.alternatives().try([
    exports.MuscleJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeMuscle = Muscle.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateMuscle(maybeMuscle) {
    const { value, error } = exports.MuscleJoiSchema.validate(maybeMuscle);
    if (error) {
        return new oaValidationError_1.OaValidationError('Muscle', maybeMuscle, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateMuscle = validateMuscle;
