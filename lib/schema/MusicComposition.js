...
exports.validateMusicComposition = exports.MusicCompositionOrSubClassJoiSchema = exports.MusicCompositionJoiSchema = void 0;
 ...
 */
exports.MusicCompositionJoiSchema = Joi.object({
    '@type': Joi.string().valid('MusicComposition').required(),
    '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
    identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
    name: Joi.string(),
    description: Joi.string(),
    '@id': Joi.string().uri(),
    musicCompositionForm: Joi.string(),
    firstPerformance: Joi.alternatives().try(Joi.lazy(() => schema.Event_OrSubClassJoiSchema), Joi.string().uri()),
    iswcCode: Joi.string(),
    lyrics: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
    musicArrangement: Joi.alternatives().try(Joi.lazy(() => schema.MusicCompositionOrSubClassJoiSchema), Joi.string().uri()),
    recordedAs: Joi.alternatives().try(Joi.lazy(() => schema.MusicRecordingOrSubClassJoiSchema), Joi.string().uri()),
    musicalKey: Joi.string(),
    includedComposition: Joi.alternatives().try(Joi.lazy(() => schema.MusicCompositionOrSubClassJoiSchema), Joi.string().uri()),
    lyricist: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
exports.MusicCompositionOrSubClassJoiSchema = Joi.alternatives().try([
    exports.MusicCompositionJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeMusicComposition = MusicComposition.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateMusicComposition(maybeMusicComposition) {
    const { value, error } = exports.MusicCompositionJoiSchema.validate(maybeMusicComposition);
    if (error) {
        return new oaValidationError_1.OaValidationError('MusicComposition', maybeMusicComposition, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateMusicComposition = validateMusicComposition;
