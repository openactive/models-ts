...
exports.validateOfferForLease = exports.OfferForLeaseOrSubClassJoiSchema = exports.OfferForLeaseJoiSchema = void 0;
 ...
 */
exports.OfferForLeaseJoiSchema = Joi.object({
    '@type': Joi.string().valid('OfferForLease').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.OfferForLeaseOrSubClassJoiSchema = Joi.alternatives().try([
    exports.OfferForLeaseJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeOfferForLease = OfferForLease.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateOfferForLease(maybeOfferForLease) {
    const { value, error } = exports.OfferForLeaseJoiSchema.validate(maybeOfferForLease);
    if (error) {
        return new oaValidationError_1.OaValidationError('OfferForLease', maybeOfferForLease, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateOfferForLease = validateOfferForLease;
