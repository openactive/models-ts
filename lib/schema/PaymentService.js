...
exports.validatePaymentService = exports.PaymentServiceOrSubClassJoiSchema = exports.PaymentServiceJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:PaymentService - Validation schema (w/ JOI)
 */
exports.PaymentServiceJoiSchema = Joi.object({
    '@type': Joi.string().valid('PaymentService').required(),
 ...
 * schema:PaymentService - Validation schema (w/ JOI)
 *
 * This differs from PaymentServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.PaymentServiceOrSubClassJoiSchema = Joi.alternatives().try([
    exports.PaymentServiceJoiSchema,
]);
/**
 * Runtime validator for schema:PaymentService.
 *
 * If some data has a structure which matches a schema:PaymentService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePaymentService = PaymentService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePaymentService instanceof OaValidationError) {
 *   // From this point on, `maybePaymentService` will have type `OaValidationError`
 *   const error = maybePaymentService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePaymentService` will have type `PaymentService`
 * const paymentService = maybePaymentService;
 * ```
 */
function validatePaymentService(maybePaymentService) {
    const { value, error } = exports.PaymentServiceJoiSchema.validate(maybePaymentService);
    if (error) {
        return new oaValidationError_1.OaValidationError('PaymentService', maybePaymentService, error);
    }
