...
exports.validatePerformingGroup = exports.PerformingGroupOrSubClassJoiSchema = exports.PerformingGroupJoiSchema = void 0;
 ...
 */
exports.PerformingGroupJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.PerformingGroupOrSubClassJoiSchema = Joi.alternatives().try([
    exports.PerformingGroupJoiSchema,
 ...
 *
 * ```ts
 * const maybePerformingGroup = PerformingGroup.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validatePerformingGroup(maybePerformingGroup) {
    const { value, error } = exports.PerformingGroupJoiSchema.validate(maybePerformingGroup);
    if (error) {
        return new oaValidationError_1.OaValidationError('PerformingGroup', maybePerformingGroup, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validatePerformingGroup = validatePerformingGroup;
