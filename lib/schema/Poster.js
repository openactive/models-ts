...
exports.validatePoster = exports.PosterOrSubClassJoiSchema = exports.PosterJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:Poster - Validation schema (w/ JOI)
 */
exports.PosterJoiSchema = Joi.object({
 ...
 * schema:Poster - Validation schema (w/ JOI)
 *
 * This differs from PosterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.PosterOrSubClassJoiSchema = Joi.alternatives().try([
    exports.PosterJoiSchema,
]);
/**
 * Runtime validator for schema:Poster.
 *
 * If some data has a structure which matches a schema:Poster, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePoster` will have type `Poster`
 * const poster = maybePoster;
 * ```
 */
function validatePoster(maybePoster) {
    const { value, error } = exports.PosterJoiSchema.validate(maybePoster);
    if (error) {
        return new oaValidationError_1.OaValidationError('Poster', maybePoster, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validatePoster = validatePoster;
