...
exports.validatePreOrderAction = exports.PreOrderActionOrSubClassJoiSchema = exports.PreOrderActionJoiSchema = void 0;
 ...
 */
exports.PreOrderActionJoiSchema = Joi.object({
    '@type': Joi.string().valid('PreOrderAction').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.PreOrderActionOrSubClassJoiSchema = Joi.alternatives().try([
    exports.PreOrderActionJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybePreOrderAction = validatePreOrderAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validatePreOrderAction(maybePreOrderAction) {
    const { value, error } = exports.PreOrderActionJoiSchema.validate(maybePreOrderAction);
    if (error) {
        return new oaValidationError_1.OaValidationError('PreOrderAction', maybePreOrderAction, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validatePreOrderAction = validatePreOrderAction;
