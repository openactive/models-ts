...
exports.validatePriceTypeEnumeration = exports.PriceTypeEnumerationJoiSchema = void 0;
 ...
 */
exports.PriceTypeEnumerationJoiSchema = Joi.string().valid('https://schema.org/SalePrice', 'https://schema.org/InvoicePrice', 'https://schema.org/MinimumAdvertisedPrice', 'https://schema.org/ListPrice', 'https://schema.org/SRP', 'https://schema.org/MSRP');
/**
 ...
 * ```
 */
function validatePriceTypeEnumeration(maybePriceTypeEnumeration) {
    const { value, error } = exports.PriceTypeEnumerationJoiSchema.validate(maybePriceTypeEnumeration);
    if (error) {
        return new oaValidationError_1.OaValidationError('PriceTypeEnumeration', maybePriceTypeEnumeration, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validatePriceTypeEnumeration = validatePriceTypeEnumeration;
