...
exports.validateRadioChannel = exports.RadioChannelOrSubClassJoiSchema = exports.RadioChannelJoiSchema = void 0;
 ...
 */
exports.RadioChannelJoiSchema = Joi.object({
    '@type': Joi.string().valid('RadioChannel').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.RadioChannelOrSubClassJoiSchema = Joi.alternatives().try([
    exports.RadioChannelJoiSchema,
    Joi.lazy(() => schema.FMRadioChannelOrSubClassJoiSchema),
    Joi.lazy(() => schema.AMRadioChannelOrSubClassJoiSchema),
]);
/**
 ...
 * ```
 */
function validateRadioChannel(maybeRadioChannel) {
    const { value, error } = exports.RadioChannelJoiSchema.validate(maybeRadioChannel);
    if (error) {
        return new oaValidationError_1.OaValidationError('RadioChannel', maybeRadioChannel, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateRadioChannel = validateRadioChannel;
