...
exports.validateSchool = exports.SchoolOrSubClassJoiSchema = exports.SchoolJoiSchema = void 0;
 ...
 */
exports.SchoolJoiSchema = Joi.object({
    '@type': Joi.string().valid('School').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.SchoolOrSubClassJoiSchema = Joi.alternatives().try([
    exports.SchoolJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeSchool = validateSchool(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateSchool(maybeSchool) {
    const { value, error } = exports.SchoolJoiSchema.validate(maybeSchool);
    if (error) {
        return new oaValidationError_1.OaValidationError('School', maybeSchool, error);
    }
