...
exports.validateSendAction = exports.SendActionOrSubClassJoiSchema = exports.SendActionJoiSchema = void 0;
 ...
 */
exports.SendActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.SendActionOrSubClassJoiSchema = Joi.alternatives().try([
    exports.SendActionJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeSendAction = validateSendAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateSendAction(maybeSendAction) {
    const { value, error } = exports.SendActionJoiSchema.validate(maybeSendAction);
    if (error) {
        return new oaValidationError_1.OaValidationError('SendAction', maybeSendAction, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateSendAction = validateSendAction;
