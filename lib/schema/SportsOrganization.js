...
exports.validateSportsOrganization = exports.SportsOrganizationOrSubClassJoiSchema = exports.SportsOrganizationJoiSchema = void 0;
 ...
 */
exports.SportsOrganizationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.SportsOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
    exports.SportsOrganizationJoiSchema,
 ...
 *
 * ```ts
 * const maybeSportsOrganization = SportsOrganization.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateSportsOrganization(maybeSportsOrganization) {
    const { value, error } = exports.SportsOrganizationJoiSchema.validate(maybeSportsOrganization);
    if (error) {
        return new oaValidationError_1.OaValidationError('SportsOrganization', maybeSportsOrganization, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateSportsOrganization = validateSportsOrganization;
