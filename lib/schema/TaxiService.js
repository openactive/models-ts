...
exports.validateTaxiService = exports.TaxiServiceOrSubClassJoiSchema = exports.TaxiServiceJoiSchema = void 0;
 ...
 */
exports.TaxiServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.TaxiServiceOrSubClassJoiSchema = Joi.alternatives().try([
    exports.TaxiServiceJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeTaxiService = validateTaxiService(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateTaxiService(maybeTaxiService) {
    const { value, error } = exports.TaxiServiceJoiSchema.validate(maybeTaxiService);
    if (error) {
        return new oaValidationError_1.OaValidationError('TaxiService', maybeTaxiService, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateTaxiService = validateTaxiService;
