...
exports.validateTravelAgency = exports.TravelAgencyOrSubClassJoiSchema = exports.TravelAgencyJoiSchema = void 0;
 ...
 */
exports.TravelAgencyJoiSchema = Joi.object({
    '@type': Joi.string().valid('TravelAgency').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.TravelAgencyOrSubClassJoiSchema = Joi.alternatives().try([
    exports.TravelAgencyJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeTravelAgency = TravelAgency.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateTravelAgency(maybeTravelAgency) {
    const { value, error } = exports.TravelAgencyJoiSchema.validate(maybeTravelAgency);
    if (error) {
        return new oaValidationError_1.OaValidationError('TravelAgency', maybeTravelAgency, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateTravelAgency = validateTravelAgency;
