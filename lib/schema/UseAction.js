...
exports.validateUseAction = exports.UseActionOrSubClassJoiSchema = exports.UseActionJoiSchema = void 0;
 ...
 */
exports.UseActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
exports.UseActionOrSubClassJoiSchema = Joi.alternatives().try([
    exports.UseActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeUseAction = validateUseAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateUseAction(maybeUseAction) {
    const { value, error } = exports.UseActionJoiSchema.validate(maybeUseAction);
    if (error) {
        return new oaValidationError_1.OaValidationError('UseAction', maybeUseAction, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateUseAction = validateUseAction;
