...
exports.validateUserPlays = exports.UserPlaysOrSubClassJoiSchema = exports.UserPlaysJoiSchema = void 0;
 ...
 */
exports.UserPlaysJoiSchema = Joi.object({
    '@type': Joi.string().valid('UserPlays').required(),
 ...
 * `SessionSeries`, etc.
 */
exports.UserPlaysOrSubClassJoiSchema = Joi.alternatives().try([
    exports.UserPlaysJoiSchema,
]);
/**
 ...
 *
 * ```ts
 * const maybeUserPlays = UserPlays.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
function validateUserPlays(maybeUserPlays) {
    const { value, error } = exports.UserPlaysJoiSchema.validate(maybeUserPlays);
    if (error) {
        return new oaValidationError_1.OaValidationError('UserPlays', maybeUserPlays, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateUserPlays = validateUserPlays;
