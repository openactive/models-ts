...
exports.validateVoteAction = exports.VoteActionOrSubClassJoiSchema = exports.VoteActionJoiSchema = void 0;
 ...
 */
exports.VoteActionJoiSchema = Joi.object({
    '@type': Joi.string().valid('VoteAction').required(),
    '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
    identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
    name: Joi.string(),
    description: Joi.string(),
    '@id': Joi.string().uri(),
    candidate: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
    option: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
exports.VoteActionOrSubClassJoiSchema = Joi.alternatives().try([
    exports.VoteActionJoiSchema,
]);
/**
 ...
 * ```
 */
function validateVoteAction(maybeVoteAction) {
    const { value, error } = exports.VoteActionJoiSchema.validate(maybeVoteAction);
    if (error) {
        return new oaValidationError_1.OaValidationError('VoteAction', maybeVoteAction, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateVoteAction = validateVoteAction;
