...
exports.validateWaterfall = exports.WaterfallOrSubClassJoiSchema = exports.WaterfallJoiSchema = void 0;
// This file was generated
const Joi = require("joi");
const oaValidationError_1 = require("../oaValidationError");
const schema = require("../schema");
/**
 * schema:Waterfall - Validation schema (w/ JOI)
 */
exports.WaterfallJoiSchema = Joi.object({
    '@type': Joi.string().valid('Waterfall').required(),
 ...
 * schema:Waterfall - Validation schema (w/ JOI)
 *
 * This differs from WaterfallJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
exports.WaterfallOrSubClassJoiSchema = Joi.alternatives().try([
    exports.WaterfallJoiSchema,
]);
/**
 * Runtime validator for schema:Waterfall.
 *
 * If some data has a structure which matches a schema:Waterfall, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeWaterfall` will have type `Waterfall`
 * const waterfall = maybeWaterfall;
 * ```
 */
function validateWaterfall(maybeWaterfall) {
    const { value, error } = exports.WaterfallJoiSchema.validate(maybeWaterfall);
    if (error) {
        return new oaValidationError_1.OaValidationError('Waterfall', maybeWaterfall, error);
    }
    /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
    to have the right type. Therefore, we just cast it to the right type. */
    return value;
}
exports.validateWaterfall = validateWaterfall;
