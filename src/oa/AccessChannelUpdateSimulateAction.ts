 ...
 */
export type AccessChannelUpdateSimulateAction = {
  '@type': 'AccessChannelUpdateSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from AccessChannelUpdateSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AccessChannelUpdateSimulateActionOrSubClass =
  | AccessChannelUpdateSimulateAction
  ;
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const AccessChannelUpdateSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AccessChannelUpdateSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  AccessChannelUpdateSimulateActionJoiSchema,
]);
 ...
 * ```
 */
export function validateAccessChannelUpdateSimulateAction(maybeAccessChannelUpdateSimulateAction: unknown): AccessChannelUpdateSimulateAction | OaValidationError {
  const { value, error } = AccessChannelUpdateSimulateActionJoiSchema.validate(maybeAccessChannelUpdateSimulateAction);
  if (error) {
    return new OaValidationError('AccessChannelUpdateSimulateAction', maybeAccessChannelUpdateSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AccessChannelUpdateSimulateAction;
}
