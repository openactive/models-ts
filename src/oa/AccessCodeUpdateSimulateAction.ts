 ...
 */
export type AccessCodeUpdateSimulateAction = {
  '@type': 'AccessCodeUpdateSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from AccessCodeUpdateSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AccessCodeUpdateSimulateActionOrSubClass =
  | AccessCodeUpdateSimulateAction
  ;
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const AccessCodeUpdateSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AccessCodeUpdateSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  AccessCodeUpdateSimulateActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeAccessCodeUpdateSimulateAction = validateAccessCodeUpdateSimulateAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateAccessCodeUpdateSimulateAction(maybeAccessCodeUpdateSimulateAction: unknown): AccessCodeUpdateSimulateAction | OaValidationError {
  const { value, error } = AccessCodeUpdateSimulateActionJoiSchema.validate(maybeAccessCodeUpdateSimulateAction);
  if (error) {
    return new OaValidationError('AccessCodeUpdateSimulateAction', maybeAccessCodeUpdateSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AccessCodeUpdateSimulateAction;
}
