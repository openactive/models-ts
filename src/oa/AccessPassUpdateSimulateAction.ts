 ...
 */
export type AccessPassUpdateSimulateAction = {
  '@type': 'AccessPassUpdateSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from AccessPassUpdateSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AccessPassUpdateSimulateActionOrSubClass =
  | AccessPassUpdateSimulateAction
  ;
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const AccessPassUpdateSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AccessPassUpdateSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  AccessPassUpdateSimulateActionJoiSchema,
]);
 ...
 * ```
 */
export function validateAccessPassUpdateSimulateAction(maybeAccessPassUpdateSimulateAction: unknown): AccessPassUpdateSimulateAction | OaValidationError {
  const { value, error } = AccessPassUpdateSimulateActionJoiSchema.validate(maybeAccessPassUpdateSimulateAction);
  if (error) {
    return new OaValidationError('AccessPassUpdateSimulateAction', maybeAccessPassUpdateSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AccessPassUpdateSimulateAction;
}
