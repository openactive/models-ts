...
export type ActionOrSubClass =
  | Action
  | oa.OpenBookingSimulateActionOrSubClass
  ;

/**
 * Action - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
...
export const ActionOrSubClassJoiSchema = Joi.alternatives().try([
  ActionJoiSchema,
  Joi.lazy(() => oa.OpenBookingSimulateActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for Action.
 *
 * If some data has a structure which matches a Action, it will be returned with the correct type.
