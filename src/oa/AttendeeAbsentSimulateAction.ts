 ...
 */
export type AttendeeAbsentSimulateAction = {
 ...
 * etc.
 */
export type AttendeeAbsentSimulateActionOrSubClass =
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const AttendeeAbsentSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AttendeeAbsentSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  AttendeeAbsentSimulateActionJoiSchema,
]);
 ...
 * ```
 */
export function validateAttendeeAbsentSimulateAction(maybeAttendeeAbsentSimulateAction: unknown): AttendeeAbsentSimulateAction | OaValidationError {
  const { value, error } = AttendeeAbsentSimulateActionJoiSchema.validate(maybeAttendeeAbsentSimulateAction);
  if (error) {
    return new OaValidationError('AttendeeAbsentSimulateAction', maybeAttendeeAbsentSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AttendeeAbsentSimulateAction;
}
