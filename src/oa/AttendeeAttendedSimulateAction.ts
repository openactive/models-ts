 ...
 */
export type AttendeeAttendedSimulateAction = {
  '@type': 'AttendeeAttendedSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from AttendeeAttendedSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AttendeeAttendedSimulateActionOrSubClass =
  | AttendeeAttendedSimulateAction
  ;
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const AttendeeAttendedSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AttendeeAttendedSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  AttendeeAttendedSimulateActionJoiSchema,
]);
 ...
 * ```
 */
export function validateAttendeeAttendedSimulateAction(maybeAttendeeAttendedSimulateAction: unknown): AttendeeAttendedSimulateAction | OaValidationError {
  const { value, error } = AttendeeAttendedSimulateActionJoiSchema.validate(maybeAttendeeAttendedSimulateAction);
  if (error) {
    return new OaValidationError('AttendeeAttendedSimulateAction', maybeAttendeeAttendedSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AttendeeAttendedSimulateAction;
}
