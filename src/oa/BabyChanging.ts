 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from BabyChanging because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BabyChangingOrSubClass =
  | BabyChanging
  ;
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const BabyChangingJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BabyChangingOrSubClassJoiSchema = Joi.alternatives().try([
  BabyChangingJoiSchema,
]);
 ...
 * ```
 */
export function validateBabyChanging(maybeBabyChanging: unknown): BabyChanging | OaValidationError {
  const { value, error } = BabyChangingJoiSchema.validate(maybeBabyChanging);
  if (error) {
    return new OaValidationError('BabyChanging', maybeBabyChanging, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BabyChanging;
}
