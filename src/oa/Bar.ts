 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Bar because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BarOrSubClass =
  | Bar
  ;
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const BarJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BarOrSubClassJoiSchema = Joi.alternatives().try([
  BarJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeBar = Bar.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBar(maybeBar: unknown): Bar | OaValidationError {
  const { value, error } = BarJoiSchema.validate(maybeBar);
  if (error) {
    return new OaValidationError('Bar', maybeBar, error);
  }
