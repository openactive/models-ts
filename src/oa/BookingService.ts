 ...
 * etc.
 */
export type BookingServiceOrSubClass =
  | BookingService
  ;
 ...
 * This type is derived from https://schema.org/Service, which means that any of this type's properties within schema.org may also be used.
 */
export const BookingServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BookingServiceOrSubClassJoiSchema = Joi.alternatives().try([
  BookingServiceJoiSchema,
]);
 ...
 * ```
 */
export function validateBookingService(maybeBookingService: unknown): BookingService | OaValidationError {
  const { value, error } = BookingServiceJoiSchema.validate(maybeBookingService);
  if (error) {
    return new OaValidationError('BookingService', maybeBookingService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BookingService;
}
