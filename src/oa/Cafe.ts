 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const CafeJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CafeOrSubClassJoiSchema = Joi.alternatives().try([
  CafeJoiSchema,
]);
 ...
 * ```
 */
export function validateCafe(maybeCafe: unknown): Cafe | OaValidationError {
  const { value, error } = CafeJoiSchema.validate(maybeCafe);
  if (error) {
    return new OaValidationError('Cafe', maybeCafe, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Cafe;
}
