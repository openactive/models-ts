 ...
 */
export type CancellationNotPermittedError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from CancellationNotPermittedError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CancellationNotPermittedErrorOrSubClass =
  | CancellationNotPermittedError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const CancellationNotPermittedErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CancellationNotPermittedErrorOrSubClassJoiSchema = Joi.alternatives().try([
  CancellationNotPermittedErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeCancellationNotPermittedError = validateCancellationNotPermittedError(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateCancellationNotPermittedError(maybeCancellationNotPermittedError: unknown): CancellationNotPermittedError | OaValidationError {
  const { value, error } = CancellationNotPermittedErrorJoiSchema.validate(maybeCancellationNotPermittedError);
  if (error) {
    return new OaValidationError('CancellationNotPermittedError', maybeCancellationNotPermittedError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CancellationNotPermittedError;
}
