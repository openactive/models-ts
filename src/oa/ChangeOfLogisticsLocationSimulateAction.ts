 ...
 */
export type ChangeOfLogisticsLocationSimulateAction = {
  '@type': 'ChangeOfLogisticsLocationSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ChangeOfLogisticsLocationSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChangeOfLogisticsLocationSimulateActionOrSubClass =
  | ChangeOfLogisticsLocationSimulateAction
  ;
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const ChangeOfLogisticsLocationSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ChangeOfLogisticsLocationSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  ChangeOfLogisticsLocationSimulateActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeChangeOfLogisticsLocationSimulateAction = ChangeOfLogisticsLocationSimulateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateChangeOfLogisticsLocationSimulateAction(maybeChangeOfLogisticsLocationSimulateAction: unknown): ChangeOfLogisticsLocationSimulateAction | OaValidationError {
  const { value, error } = ChangeOfLogisticsLocationSimulateActionJoiSchema.validate(maybeChangeOfLogisticsLocationSimulateAction);
  if (error) {
    return new OaValidationError('ChangeOfLogisticsLocationSimulateAction', maybeChangeOfLogisticsLocationSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ChangeOfLogisticsLocationSimulateAction;
}
