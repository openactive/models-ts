 ...
 */
export type ChangeOfLogisticsTimeSimulateAction = {
  '@type': 'ChangeOfLogisticsTimeSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ChangeOfLogisticsTimeSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChangeOfLogisticsTimeSimulateActionOrSubClass =
  | ChangeOfLogisticsTimeSimulateAction
  ;

/**
 * test:ChangeOfLogisticsTimeSimulateAction - Validation schema (w/ JOI)
 *
 * [NOTICE: This class is part of the Open Booking API Test Interface, and MUST NOT be used in production.] 
 *
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const ChangeOfLogisticsTimeSimulateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ChangeOfLogisticsTimeSimulateAction').required(),
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ChangeOfLogisticsTimeSimulateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ChangeOfLogisticsTimeSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  ChangeOfLogisticsTimeSimulateActionJoiSchema,
]);

/**
 * Runtime validator for test:ChangeOfLogisticsTimeSimulateAction.
 *
 * If some data has a structure which matches a test:ChangeOfLogisticsTimeSimulateAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeChangeOfLogisticsTimeSimulateAction = ChangeOfLogisticsTimeSimulateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeChangeOfLogisticsTimeSimulateAction instanceof OaValidationError) {
 *   // From this point on, `maybeChangeOfLogisticsTimeSimulateAction` will have type `OaValidationError`
 *   const error = maybeChangeOfLogisticsTimeSimulateAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeChangeOfLogisticsTimeSimulateAction` will have type `ChangeOfLogisticsTimeSimulateAction`
 * const changeOfLogisticsTimeSimulateAction = maybeChangeOfLogisticsTimeSimulateAction;
 * ```
 */
export function validateChangeOfLogisticsTimeSimulateAction(maybeChangeOfLogisticsTimeSimulateAction: unknown): ChangeOfLogisticsTimeSimulateAction | OaValidationError {
  const { value, error } = ChangeOfLogisticsTimeSimulateActionJoiSchema.validate(maybeChangeOfLogisticsTimeSimulateAction);
  if (error) {
    return new OaValidationError('ChangeOfLogisticsTimeSimulateAction', maybeChangeOfLogisticsTimeSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ChangeOfLogisticsTimeSimulateAction;
}
