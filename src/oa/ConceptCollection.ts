 ...
 * This type is derived from https://schema.org/CreativeWork, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ConceptCollection because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ConceptCollectionOrSubClass =
  | ConceptCollection
  ;

/**
 * beta:ConceptCollection - Validation schema (w/ JOI)
 *
 * [NOTICE: This is a beta class, and is highly likely to change in future versions of this library.] 
 *
 * This type is derived from https://schema.org/CreativeWork, which means that any of this type's properties within schema.org may also be used.
 */
export const ConceptCollectionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ConceptCollection').required(),
 ...
 * This type is derived from https://schema.org/CreativeWork, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ConceptCollectionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ConceptCollectionOrSubClassJoiSchema = Joi.alternatives().try([
  ConceptCollectionJoiSchema,
]);

/**
 * Runtime validator for beta:ConceptCollection.
 *
 * If some data has a structure which matches a beta:ConceptCollection, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeConceptCollection = ConceptCollection.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeConceptCollection instanceof OaValidationError) {
 *   // From this point on, `maybeConceptCollection` will have type `OaValidationError`
 *   const error = maybeConceptCollection;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeConceptCollection` will have type `ConceptCollection`
 * const conceptCollection = maybeConceptCollection;
 * ```
 */
export function validateConceptCollection(maybeConceptCollection: unknown): ConceptCollection | OaValidationError {
  const { value, error } = ConceptCollectionJoiSchema.validate(maybeConceptCollection);
  if (error) {
    return new OaValidationError('ConceptCollection', maybeConceptCollection, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ConceptCollection;
}
