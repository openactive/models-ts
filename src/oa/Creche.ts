 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Creche because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CrecheOrSubClass =
  | Creche
  ;

/**
 * Creche - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const CrecheJoiSchema = Joi.object({
  '@type': Joi.string().valid('Creche').required(),
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from CrecheJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CrecheOrSubClassJoiSchema = Joi.alternatives().try([
  CrecheJoiSchema,
]);

/**
 * Runtime validator for Creche.
 *
 * If some data has a structure which matches a Creche, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCreche = Creche.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCreche instanceof OaValidationError) {
 *   // From this point on, `maybeCreche` will have type `OaValidationError`
 *   const error = maybeCreche;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCreche` will have type `Creche`
 * const creche = maybeCreche;
 * ```
 */
export function validateCreche(maybeCreche: unknown): Creche | OaValidationError {
  const { value, error } = CrecheJoiSchema.validate(maybeCreche);
  if (error) {
    return new OaValidationError('Creche', maybeCreche, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Creche;
}
