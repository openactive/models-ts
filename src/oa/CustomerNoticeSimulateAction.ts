 ...
 */
export type CustomerNoticeSimulateAction = {
  '@type': 'CustomerNoticeSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from CustomerNoticeSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CustomerNoticeSimulateActionOrSubClass =
  | CustomerNoticeSimulateAction
  ;

/**
 * test:CustomerNoticeSimulateAction - Validation schema (w/ JOI)
 *
 * [NOTICE: This class is part of the Open Booking API Test Interface, and MUST NOT be used in production.] 
 *
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const CustomerNoticeSimulateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('CustomerNoticeSimulateAction').required(),
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from CustomerNoticeSimulateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CustomerNoticeSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  CustomerNoticeSimulateActionJoiSchema,
]);

/**
 * Runtime validator for test:CustomerNoticeSimulateAction.
 *
 * If some data has a structure which matches a test:CustomerNoticeSimulateAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCustomerNoticeSimulateAction` will have type `CustomerNoticeSimulateAction`
 * const customerNoticeSimulateAction = maybeCustomerNoticeSimulateAction;
 * ```
 */
export function validateCustomerNoticeSimulateAction(maybeCustomerNoticeSimulateAction: unknown): CustomerNoticeSimulateAction | OaValidationError {
  const { value, error } = CustomerNoticeSimulateActionJoiSchema.validate(maybeCustomerNoticeSimulateAction);
  if (error) {
    return new OaValidationError('CustomerNoticeSimulateAction', maybeCustomerNoticeSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CustomerNoticeSimulateAction;
}
