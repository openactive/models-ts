...
export type DataCatalog = {
   ...
   * ```
   */
  publisher?: oa.OrganizationOrSubClass;
  /**
...
/**
 * DataCatalog - Validation schema (w/ JOI)
 *
 * 
 *
 * EARLY RELEASE NOTICE: This class represents a draft that is designed to inform the OpenActive specification work with implementation feedback. It is mostly stable, as it based entirely on schema.org. HOWEVER, IT IS STILL SUBJECT TO CHANGE, as the [Dataset API Discovery specification](https://openactive.io/dataset-api-discovery/EditorsDraft/) evolves.
...
export const DataCatalogJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  dataset: Joi.array().items(Joi.string().uri()),
  dateModified: Joi.string().isoDate(),
  datePublished: Joi.string().isoDate(),
  hasPart: Joi.array().items(Joi.string().uri()),
...
/**
 * Runtime validator for DataCatalog.
 *
 * If some data has a structure which matches a DataCatalog, it will be returned with the correct type.
