 ...
 * This type is derived from https://schema.org/DataDownload, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from DataDownload because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DataDownloadOrSubClass =
  | DataDownload
  ;

/**
 * DataDownload - Validation schema (w/ JOI)
 *
 * 
 *
 * EARLY RELEASE NOTICE: This class represents a draft that is designed to inform the OpenActive specification work with implementation feedback. IT IS SUBJECT TO CHANGE, as the [Dataset API Discovery specification](https://openactive.io/dataset-api-discovery/EditorsDraft/) evolves.
 ...
 * This type is derived from https://schema.org/DataDownload, which means that any of this type's properties within schema.org may also be used.
 */
export const DataDownloadJoiSchema = Joi.object({
  '@type': Joi.string().valid('DataDownload').required(),
 ...
 * This type is derived from https://schema.org/DataDownload, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from DataDownloadJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DataDownloadOrSubClassJoiSchema = Joi.alternatives().try([
  DataDownloadJoiSchema,
]);

/**
 * Runtime validator for DataDownload.
 *
 * If some data has a structure which matches a DataDownload, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDataDownload = DataDownload.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDataDownload instanceof OaValidationError) {
 *   // From this point on, `maybeDataDownload` will have type `OaValidationError`
 *   const error = maybeDataDownload;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDataDownload` will have type `DataDownload`
 * const dataDownload = maybeDataDownload;
 * ```
 */
export function validateDataDownload(maybeDataDownload: unknown): DataDownload | OaValidationError {
  const { value, error } = DataDownloadJoiSchema.validate(maybeDataDownload);
  if (error) {
    return new OaValidationError('DataDownload', maybeDataDownload, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DataDownload;
}
