 ...
 * etc.
 */
export type FacilityUseOrSubClass =
  | FacilityUse
  | oa.IndividualFacilityUseOrSubClass
  ;
 ...
 * This type is derived from https://schema.org/Product, which means that any of this type's properties within schema.org may also be used.
 */
export const FacilityUseJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FacilityUseOrSubClassJoiSchema = Joi.alternatives().try([
  FacilityUseJoiSchema,
 ...
 * ```
 */
export function validateFacilityUse(maybeFacilityUse: unknown): FacilityUse | OaValidationError {
  const { value, error } = FacilityUseJoiSchema.validate(maybeFacilityUse);
  if (error) {
    return new OaValidationError('FacilityUse', maybeFacilityUse, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FacilityUse;
}
