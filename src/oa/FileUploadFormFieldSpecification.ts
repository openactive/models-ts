 ...
 */
export type FileUploadFormFieldSpecification = {
 ...
 * This type is derived from https://schema.org/PropertyValueSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from FileUploadFormFieldSpecification because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FileUploadFormFieldSpecificationOrSubClass =
  | FileUploadFormFieldSpecification
  ;
 ...
 * This type is derived from https://schema.org/PropertyValueSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const FileUploadFormFieldSpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FileUploadFormFieldSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  FileUploadFormFieldSpecificationJoiSchema,
]);
 ...
 * ```
 */
export function validateFileUploadFormFieldSpecification(maybeFileUploadFormFieldSpecification: unknown): FileUploadFormFieldSpecification | OaValidationError {
  const { value, error } = FileUploadFormFieldSpecificationJoiSchema.validate(maybeFileUploadFormFieldSpecification);
  if (error) {
    return new OaValidationError('FileUploadFormFieldSpecification', maybeFileUploadFormFieldSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FileUploadFormFieldSpecification;
}
