 ...
 * etc.
 */
export type HeadlineEventOrSubClass =
  | HeadlineEvent
  ;
 ...
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 */
export const HeadlineEventJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const HeadlineEventOrSubClassJoiSchema = Joi.alternatives().try([
  HeadlineEventJoiSchema,
]);
 ...
 * ```
 */
export function validateHeadlineEvent(maybeHeadlineEvent: unknown): HeadlineEvent | OaValidationError {
  const { value, error } = HeadlineEventJoiSchema.validate(maybeHeadlineEvent);
  if (error) {
    return new OaValidationError('HeadlineEvent', maybeHeadlineEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HeadlineEvent;
}
