 ...
 */
export type IncompleteAttendeeDetailsError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from IncompleteAttendeeDetailsError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type IncompleteAttendeeDetailsErrorOrSubClass =
  | IncompleteAttendeeDetailsError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const IncompleteAttendeeDetailsErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const IncompleteAttendeeDetailsErrorOrSubClassJoiSchema = Joi.alternatives().try([
  IncompleteAttendeeDetailsErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeIncompleteAttendeeDetailsError = validateIncompleteAttendeeDetailsError(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateIncompleteAttendeeDetailsError(maybeIncompleteAttendeeDetailsError: unknown): IncompleteAttendeeDetailsError | OaValidationError {
  const { value, error } = IncompleteAttendeeDetailsErrorJoiSchema.validate(maybeIncompleteAttendeeDetailsError);
  if (error) {
    return new OaValidationError('IncompleteAttendeeDetailsError', maybeIncompleteAttendeeDetailsError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as IncompleteAttendeeDetailsError;
}
