 ...
 */
export type IncompleteBrokerDetailsError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from IncompleteBrokerDetailsError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type IncompleteBrokerDetailsErrorOrSubClass =
  | IncompleteBrokerDetailsError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const IncompleteBrokerDetailsErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const IncompleteBrokerDetailsErrorOrSubClassJoiSchema = Joi.alternatives().try([
  IncompleteBrokerDetailsErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateIncompleteBrokerDetailsError(maybeIncompleteBrokerDetailsError: unknown): IncompleteBrokerDetailsError | OaValidationError {
  const { value, error } = IncompleteBrokerDetailsErrorJoiSchema.validate(maybeIncompleteBrokerDetailsError);
  if (error) {
    return new OaValidationError('IncompleteBrokerDetailsError', maybeIncompleteBrokerDetailsError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as IncompleteBrokerDetailsError;
}
