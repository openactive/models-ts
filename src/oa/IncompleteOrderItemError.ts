 ...
 */
export type IncompleteOrderItemError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from IncompleteOrderItemError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type IncompleteOrderItemErrorOrSubClass =
  | IncompleteOrderItemError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const IncompleteOrderItemErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const IncompleteOrderItemErrorOrSubClassJoiSchema = Joi.alternatives().try([
  IncompleteOrderItemErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeIncompleteOrderItemError = IncompleteOrderItemError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateIncompleteOrderItemError(maybeIncompleteOrderItemError: unknown): IncompleteOrderItemError | OaValidationError {
  const { value, error } = IncompleteOrderItemErrorJoiSchema.validate(maybeIncompleteOrderItemError);
  if (error) {
    return new OaValidationError('IncompleteOrderItemError', maybeIncompleteOrderItemError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as IncompleteOrderItemError;
}
