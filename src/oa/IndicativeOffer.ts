 ...
 * etc.
 */
export type IndicativeOfferOrSubClass =
  | IndicativeOffer
  ;
 ...
 * This type is derived from https://schema.org/Offer, which means that any of this type's properties within schema.org may also be used.
 */
export const IndicativeOfferJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const IndicativeOfferOrSubClassJoiSchema = Joi.alternatives().try([
  IndicativeOfferJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeIndicativeOffer = IndicativeOffer.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateIndicativeOffer(maybeIndicativeOffer: unknown): IndicativeOffer | OaValidationError {
  const { value, error } = IndicativeOfferJoiSchema.validate(maybeIndicativeOffer);
  if (error) {
    return new OaValidationError('IndicativeOffer', maybeIndicativeOffer, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as IndicativeOffer;
}
