 ...
 */
export type InternalApplicationError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InternalApplicationError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InternalApplicationErrorOrSubClass =
  | InternalApplicationError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InternalApplicationErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InternalApplicationErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InternalApplicationErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeInternalApplicationError = validateInternalApplicationError(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateInternalApplicationError(maybeInternalApplicationError: unknown): InternalApplicationError | OaValidationError {
  const { value, error } = InternalApplicationErrorJoiSchema.validate(maybeInternalApplicationError);
  if (error) {
    return new OaValidationError('InternalApplicationError', maybeInternalApplicationError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InternalApplicationError;
}
