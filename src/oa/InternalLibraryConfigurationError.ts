 ...
 */
export type InternalLibraryConfigurationError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InternalLibraryConfigurationError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InternalLibraryConfigurationErrorOrSubClass =
  | InternalLibraryConfigurationError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InternalLibraryConfigurationErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InternalLibraryConfigurationErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InternalLibraryConfigurationErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateInternalLibraryConfigurationError(maybeInternalLibraryConfigurationError: unknown): InternalLibraryConfigurationError | OaValidationError {
  const { value, error } = InternalLibraryConfigurationErrorJoiSchema.validate(maybeInternalLibraryConfigurationError);
  if (error) {
    return new OaValidationError('InternalLibraryConfigurationError', maybeInternalLibraryConfigurationError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InternalLibraryConfigurationError;
}
