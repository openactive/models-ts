 ...
 */
export type InternalLibraryError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InternalLibraryError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InternalLibraryErrorOrSubClass =
  | InternalLibraryError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InternalLibraryErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InternalLibraryErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InternalLibraryErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeInternalLibraryError = InternalLibraryError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateInternalLibraryError(maybeInternalLibraryError: unknown): InternalLibraryError | OaValidationError {
  const { value, error } = InternalLibraryErrorJoiSchema.validate(maybeInternalLibraryError);
  if (error) {
    return new OaValidationError('InternalLibraryError', maybeInternalLibraryError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InternalLibraryError;
}
