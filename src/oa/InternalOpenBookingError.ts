 ...
 */
export type InternalOpenBookingError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InternalOpenBookingError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InternalOpenBookingErrorOrSubClass =
  | InternalOpenBookingError
  | oa.InternalApplicationErrorOrSubClass
  | oa.InternalLibraryConfigurationErrorOrSubClass
  | oa.InternalLibraryErrorOrSubClass
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InternalOpenBookingErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InternalOpenBookingErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InternalOpenBookingErrorJoiSchema,
 ...
 * ```
 */
export function validateInternalOpenBookingError(maybeInternalOpenBookingError: unknown): InternalOpenBookingError | OaValidationError {
  const { value, error } = InternalOpenBookingErrorJoiSchema.validate(maybeInternalOpenBookingError);
  if (error) {
    return new OaValidationError('InternalOpenBookingError', maybeInternalOpenBookingError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InternalOpenBookingError;
}
