 ...
 */
export type InvalidAPITokenError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InvalidAPITokenError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InvalidAPITokenErrorOrSubClass =
  | InvalidAPITokenError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InvalidAPITokenErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InvalidAPITokenErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InvalidAPITokenErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeInvalidAPITokenError = InvalidAPITokenError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateInvalidAPITokenError(maybeInvalidAPITokenError: unknown): InvalidAPITokenError | OaValidationError {
  const { value, error } = InvalidAPITokenErrorJoiSchema.validate(maybeInvalidAPITokenError);
  if (error) {
    return new OaValidationError('InvalidAPITokenError', maybeInvalidAPITokenError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InvalidAPITokenError;
}
