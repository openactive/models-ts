 ...
 */
export type InvalidAuthorizationDetailsError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InvalidAuthorizationDetailsError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InvalidAuthorizationDetailsErrorOrSubClass =
  | InvalidAuthorizationDetailsError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InvalidAuthorizationDetailsErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InvalidAuthorizationDetailsErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InvalidAuthorizationDetailsErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateInvalidAuthorizationDetailsError(maybeInvalidAuthorizationDetailsError: unknown): InvalidAuthorizationDetailsError | OaValidationError {
  const { value, error } = InvalidAuthorizationDetailsErrorJoiSchema.validate(maybeInvalidAuthorizationDetailsError);
  if (error) {
    return new OaValidationError('InvalidAuthorizationDetailsError', maybeInvalidAuthorizationDetailsError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InvalidAuthorizationDetailsError;
}
