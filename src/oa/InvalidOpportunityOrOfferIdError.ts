 ...
 */
export type InvalidOpportunityOrOfferIdError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InvalidOpportunityOrOfferIdError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InvalidOpportunityOrOfferIdErrorOrSubClass =
  | InvalidOpportunityOrOfferIdError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InvalidOpportunityOrOfferIdErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InvalidOpportunityOrOfferIdErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InvalidOpportunityOrOfferIdErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeInvalidOpportunityOrOfferIdError = InvalidOpportunityOrOfferIdError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateInvalidOpportunityOrOfferIdError(maybeInvalidOpportunityOrOfferIdError: unknown): InvalidOpportunityOrOfferIdError | OaValidationError {
  const { value, error } = InvalidOpportunityOrOfferIdErrorJoiSchema.validate(maybeInvalidOpportunityOrOfferIdError);
  if (error) {
    return new OaValidationError('InvalidOpportunityOrOfferIdError', maybeInvalidOpportunityOrOfferIdError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InvalidOpportunityOrOfferIdError;
}
