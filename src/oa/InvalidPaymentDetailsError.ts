 ...
 */
export type InvalidPaymentDetailsError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from InvalidPaymentDetailsError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InvalidPaymentDetailsErrorOrSubClass =
  | InvalidPaymentDetailsError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const InvalidPaymentDetailsErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InvalidPaymentDetailsErrorOrSubClassJoiSchema = Joi.alternatives().try([
  InvalidPaymentDetailsErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateInvalidPaymentDetailsError(maybeInvalidPaymentDetailsError: unknown): InvalidPaymentDetailsError | OaValidationError {
  const { value, error } = InvalidPaymentDetailsErrorJoiSchema.validate(maybeInvalidPaymentDetailsError);
  if (error) {
    return new OaValidationError('InvalidPaymentDetailsError', maybeInvalidPaymentDetailsError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InvalidPaymentDetailsError;
}
