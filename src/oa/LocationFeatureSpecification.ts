 ...
 * etc.
 */
export type LocationFeatureSpecificationOrSubClass =
  | LocationFeatureSpecification
  | oa.BabyChangingOrSubClass
  | oa.ChangingFacilitiesOrSubClass
  | oa.CrecheOrSubClass
  | oa.LockersOrSubClass
  | oa.ParkingOrSubClass
  | oa.ShowersOrSubClass
  | oa.ToiletsOrSubClass
  | oa.TowelsOrSubClass
  | oa.CafeOrSubClass
  | oa.BarOrSubClass
  ;
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const LocationFeatureSpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LocationFeatureSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  LocationFeatureSpecificationJoiSchema,
 ...
 * ```
 */
export function validateLocationFeatureSpecification(maybeLocationFeatureSpecification: unknown): LocationFeatureSpecification | OaValidationError {
  const { value, error } = LocationFeatureSpecificationJoiSchema.validate(maybeLocationFeatureSpecification);
  if (error) {
    return new OaValidationError('LocationFeatureSpecification', maybeLocationFeatureSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LocationFeatureSpecification;
}
