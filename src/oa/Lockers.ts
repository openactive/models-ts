 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Lockers because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LockersOrSubClass =
  | Lockers
  ;
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const LockersJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LockersOrSubClassJoiSchema = Joi.alternatives().try([
  LockersJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeLockers = validateLockers(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateLockers(maybeLockers: unknown): Lockers | OaValidationError {
  const { value, error } = LockersJoiSchema.validate(maybeLockers);
  if (error) {
    return new OaValidationError('Lockers', maybeLockers, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Lockers;
}
