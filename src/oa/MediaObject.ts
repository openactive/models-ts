...
export type MediaObjectOrSubClass =
  | MediaObject
  | oa.AudioObjectOrSubClass
  | oa.ImageObjectOrSubClass
  | oa.VideoObjectOrSubClass
  ;

/**
 * MediaObject - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/MediaObject, which means that any of this type's properties within schema.org may also be used.
 */
...
export const MediaObjectOrSubClassJoiSchema = Joi.alternatives().try([
  ...
  Joi.lazy(() => oa.AudioObjectOrSubClassJoiSchema),
  Joi.lazy(() => oa.ImageObjectOrSubClassJoiSchema),
  Joi.lazy(() => oa.VideoObjectOrSubClassJoiSchema),
]);

/**
 * Runtime validator for MediaObject.
 *
 * If some data has a structure which matches a MediaObject, it will be returned with the correct type.
