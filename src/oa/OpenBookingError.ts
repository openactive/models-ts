 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from OpenBookingError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OpenBookingErrorOrSubClass =
  | OpenBookingError
  | oa.CancellationNotPermittedErrorOrSubClass
  | oa.GoneErrorOrSubClass
  | oa.IncompleteAttendeeDetailsErrorOrSubClass
  | oa.IncompleteBrokerDetailsErrorOrSubClass
  | oa.IncompleteCustomerDetailsErrorOrSubClass
  | oa.IncompleteIntakeFormErrorOrSubClass
  | oa.IncompleteOrderItemErrorOrSubClass
  | oa.IncompletePaymentDetailsErrorOrSubClass
  | oa.InternalOpenBookingErrorOrSubClass
  | oa.InvalidAPITokenErrorOrSubClass
  | oa.InvalidAuthorizationDetailsErrorOrSubClass
  | oa.InvalidIntakeFormErrorOrSubClass
  | oa.InvalidOpportunityOrOfferIdErrorOrSubClass
  | oa.InvalidPaymentDetailsErrorOrSubClass
  | oa.MethodNotAllowedErrorOrSubClass
  | oa.MissingPaymentDetailsErrorOrSubClass
  | oa.NoAPITokenErrorOrSubClass
  | oa.NotFoundErrorOrSubClass
  | oa.OpportunityCapacityIsReservedByLeaseErrorOrSubClass
  | oa.OpportunityHasInsufficientCapacityErrorOrSubClass
  | oa.OpportunityIntractableErrorOrSubClass
  | oa.OpportunityIsFullErrorOrSubClass
  | oa.OpportunityIsInConflictErrorOrSubClass
  | oa.OpportunityOfferPairNotBookableErrorOrSubClass
  | oa.OrderAlreadyExistsErrorOrSubClass
  | oa.OrderCreationFailedErrorOrSubClass
  | oa.OrderItemIdInvalidErrorOrSubClass
  | oa.OrderItemNotWithinOrderErrorOrSubClass
  | oa.OrderProposalVersionOutdatedErrorOrSubClass
  | oa.PatchContainsExcessivePropertiesErrorOrSubClass
  | oa.PatchNotAllowedOnPropertyErrorOrSubClass
  | oa.SellerMismatchErrorOrSubClass
  | oa.SellerNotFoundErrorOrSubClass
  | oa.TemporarilyUnableToCreateOrderErrorOrSubClass
  | oa.TemporarilyUnableToDeleteOrderErrorOrSubClass
  | oa.TemporarilyUnableToProduceOrderQuoteErrorOrSubClass
  | oa.TemporarilyUnableToUpdateOrderErrorOrSubClass
  | oa.TooManyRequestsErrorOrSubClass
  | oa.TotalPaymentDueMismatchErrorOrSubClass
  | oa.UnableToProcessOrderItemErrorOrSubClass
  | oa.UnacceptableOfferErrorOrSubClass
  | oa.UnauthenticatedErrorOrSubClass
  | oa.UnexpectedOrderTypeErrorOrSubClass
  | oa.UnknownOfferErrorOrSubClass
  | oa.UnknownOpportunityErrorOrSubClass
  | oa.UnknownOrIncorrectEndpointErrorOrSubClass
  | oa.UnknownOrderErrorOrSubClass
  | oa.UnnecessaryPaymentDetailsErrorOrSubClass
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OpenBookingErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OpenBookingErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OpenBookingErrorJoiSchema,
 ...
 *
 * ```ts
 * const maybeOpenBookingError = OpenBookingError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOpenBookingError(maybeOpenBookingError: unknown): OpenBookingError | OaValidationError {
  const { value, error } = OpenBookingErrorJoiSchema.validate(maybeOpenBookingError);
  if (error) {
    return new OaValidationError('OpenBookingError', maybeOpenBookingError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OpenBookingError;
}
