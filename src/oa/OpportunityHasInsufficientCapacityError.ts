 ...
 */
export type OpportunityHasInsufficientCapacityError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from OpportunityHasInsufficientCapacityError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OpportunityHasInsufficientCapacityErrorOrSubClass =
  | OpportunityHasInsufficientCapacityError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OpportunityHasInsufficientCapacityErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OpportunityHasInsufficientCapacityErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OpportunityHasInsufficientCapacityErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeOpportunityHasInsufficientCapacityError = validateOpportunityHasInsufficientCapacityError(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOpportunityHasInsufficientCapacityError(maybeOpportunityHasInsufficientCapacityError: unknown): OpportunityHasInsufficientCapacityError | OaValidationError {
  const { value, error } = OpportunityHasInsufficientCapacityErrorJoiSchema.validate(maybeOpportunityHasInsufficientCapacityError);
  if (error) {
    return new OaValidationError('OpportunityHasInsufficientCapacityError', maybeOpportunityHasInsufficientCapacityError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OpportunityHasInsufficientCapacityError;
}
