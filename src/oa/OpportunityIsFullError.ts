 ...
 */
export type OpportunityIsFullError = {
 ...
 * etc.
 */
export type OpportunityIsFullErrorOrSubClass =
  | OpportunityIsFullError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OpportunityIsFullErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OpportunityIsFullErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OpportunityIsFullErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateOpportunityIsFullError(maybeOpportunityIsFullError: unknown): OpportunityIsFullError | OaValidationError {
  const { value, error } = OpportunityIsFullErrorJoiSchema.validate(maybeOpportunityIsFullError);
  if (error) {
    return new OaValidationError('OpportunityIsFullError', maybeOpportunityIsFullError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OpportunityIsFullError;
}
