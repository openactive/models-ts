 ...
 */
export type OpportunityIsInConflictError = {
 ...
 * etc.
 */
export type OpportunityIsInConflictErrorOrSubClass =
  | OpportunityIsInConflictError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OpportunityIsInConflictErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OpportunityIsInConflictErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OpportunityIsInConflictErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateOpportunityIsInConflictError(maybeOpportunityIsInConflictError: unknown): OpportunityIsInConflictError | OaValidationError {
  const { value, error } = OpportunityIsInConflictErrorJoiSchema.validate(maybeOpportunityIsInConflictError);
  if (error) {
    return new OaValidationError('OpportunityIsInConflictError', maybeOpportunityIsInConflictError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OpportunityIsInConflictError;
}
