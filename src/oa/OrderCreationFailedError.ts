 ...
 */
export type OrderCreationFailedError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from OrderCreationFailedError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OrderCreationFailedErrorOrSubClass =
  | OrderCreationFailedError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OrderCreationFailedErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OrderCreationFailedErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OrderCreationFailedErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeOrderCreationFailedError = OrderCreationFailedError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOrderCreationFailedError(maybeOrderCreationFailedError: unknown): OrderCreationFailedError | OaValidationError {
  const { value, error } = OrderCreationFailedErrorJoiSchema.validate(maybeOrderCreationFailedError);
  if (error) {
    return new OaValidationError('OrderCreationFailedError', maybeOrderCreationFailedError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OrderCreationFailedError;
}
