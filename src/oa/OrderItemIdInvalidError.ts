 ...
 */
export type OrderItemIdInvalidError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from OrderItemIdInvalidError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OrderItemIdInvalidErrorOrSubClass =
  | OrderItemIdInvalidError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OrderItemIdInvalidErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OrderItemIdInvalidErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OrderItemIdInvalidErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeOrderItemIdInvalidError = validateOrderItemIdInvalidError(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOrderItemIdInvalidError(maybeOrderItemIdInvalidError: unknown): OrderItemIdInvalidError | OaValidationError {
  const { value, error } = OrderItemIdInvalidErrorJoiSchema.validate(maybeOrderItemIdInvalidError);
  if (error) {
    return new OaValidationError('OrderItemIdInvalidError', maybeOrderItemIdInvalidError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OrderItemIdInvalidError;
}
