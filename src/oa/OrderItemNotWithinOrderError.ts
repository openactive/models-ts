 ...
 */
export type OrderItemNotWithinOrderError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from OrderItemNotWithinOrderError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OrderItemNotWithinOrderErrorOrSubClass =
  | OrderItemNotWithinOrderError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const OrderItemNotWithinOrderErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OrderItemNotWithinOrderErrorOrSubClassJoiSchema = Joi.alternatives().try([
  OrderItemNotWithinOrderErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateOrderItemNotWithinOrderError(maybeOrderItemNotWithinOrderError: unknown): OrderItemNotWithinOrderError | OaValidationError {
  const { value, error } = OrderItemNotWithinOrderErrorJoiSchema.validate(maybeOrderItemNotWithinOrderError);
  if (error) {
    return new OaValidationError('OrderItemNotWithinOrderError', maybeOrderItemNotWithinOrderError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OrderItemNotWithinOrderError;
}
