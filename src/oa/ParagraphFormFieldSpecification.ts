 ...
 */
export type ParagraphFormFieldSpecification = {
   ...
   *
   * ```json
   * "@id": "https://example.com/question112345"
   * ```
   */
  '@id'?: string;
  /**
 ...
 * etc.
 */
export type ParagraphFormFieldSpecificationOrSubClass =
  | ParagraphFormFieldSpecification
  ;
 ...
 * This type is derived from https://schema.org/PropertyValueSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const ParagraphFormFieldSpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ParagraphFormFieldSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  ParagraphFormFieldSpecificationJoiSchema,
]);
 ...
 * ```
 */
export function validateParagraphFormFieldSpecification(maybeParagraphFormFieldSpecification: unknown): ParagraphFormFieldSpecification | OaValidationError {
  const { value, error } = ParagraphFormFieldSpecificationJoiSchema.validate(maybeParagraphFormFieldSpecification);
  if (error) {
    return new OaValidationError('ParagraphFormFieldSpecification', maybeParagraphFormFieldSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ParagraphFormFieldSpecification;
}
