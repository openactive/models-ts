 ...
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from PrivacyPolicy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PrivacyPolicyOrSubClass =
  | PrivacyPolicy
  ;

/**
 * PrivacyPolicy - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 */
export const PrivacyPolicyJoiSchema = Joi.object({
  '@type': Joi.string().valid('PrivacyPolicy').required(),
  ...
  requiresExplicitConsent: Joi.boolean(),
  url: Joi.string().uri(),
 ...
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from PrivacyPolicyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PrivacyPolicyOrSubClassJoiSchema = Joi.alternatives().try([
  PrivacyPolicyJoiSchema,
]);

/**
 * Runtime validator for PrivacyPolicy.
 *
 * If some data has a structure which matches a PrivacyPolicy, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePrivacyPolicy = PrivacyPolicy.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePrivacyPolicy instanceof OaValidationError) {
 *   // From this point on, `maybePrivacyPolicy` will have type `OaValidationError`
 *   const error = maybePrivacyPolicy;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePrivacyPolicy` will have type `PrivacyPolicy`
 * const privacyPolicy = maybePrivacyPolicy;
 * ```
 */
export function validatePrivacyPolicy(maybePrivacyPolicy: unknown): PrivacyPolicy | OaValidationError {
  const { value, error } = PrivacyPolicyJoiSchema.validate(maybePrivacyPolicy);
  if (error) {
    return new OaValidationError('PrivacyPolicy', maybePrivacyPolicy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PrivacyPolicy;
}
