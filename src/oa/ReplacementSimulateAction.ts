 ...
 */
export type ReplacementSimulateAction = {
  '@type': 'ReplacementSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ReplacementSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReplacementSimulateActionOrSubClass =
  | ReplacementSimulateAction
  ;

/**
 * test:ReplacementSimulateAction - Validation schema (w/ JOI)
 *
 * [NOTICE: This class is part of the Open Booking API Test Interface, and MUST NOT be used in production.] 
 *
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const ReplacementSimulateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReplacementSimulateAction').required(),
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ReplacementSimulateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReplacementSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReplacementSimulateActionJoiSchema,
]);

/**
 * Runtime validator for test:ReplacementSimulateAction.
 *
 * If some data has a structure which matches a test:ReplacementSimulateAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeReplacementSimulateAction` will have type `ReplacementSimulateAction`
 * const replacementSimulateAction = maybeReplacementSimulateAction;
 * ```
 */
export function validateReplacementSimulateAction(maybeReplacementSimulateAction: unknown): ReplacementSimulateAction | OaValidationError {
  const { value, error } = ReplacementSimulateActionJoiSchema.validate(maybeReplacementSimulateAction);
  if (error) {
    return new OaValidationError('ReplacementSimulateAction', maybeReplacementSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReplacementSimulateAction;
}
