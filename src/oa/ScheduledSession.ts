...
export type ScheduledSession = {
   ...
   */
  superEvent?: oa.Event_OrSubClass | string;
  /**
 ...
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ScheduledSession because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ScheduledSessionOrSubClass =
  | ScheduledSession
  ;

/**
 * ScheduledSession - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 */
export const ScheduledSessionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ScheduledSession').required(),
  ...
  superEvent: Joi.alternatives().try(Joi.lazy(() => oa.Event_OrSubClassJoiSchema), Joi.string().uri()),
  url: Joi.string().uri(),
 ...
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ScheduledSessionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ScheduledSessionOrSubClassJoiSchema = Joi.alternatives().try([
  ScheduledSessionJoiSchema,
]);

/**
 * Runtime validator for ScheduledSession.
 *
 * If some data has a structure which matches a ScheduledSession, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeScheduledSession = ScheduledSession.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeScheduledSession instanceof OaValidationError) {
 *   // From this point on, `maybeScheduledSession` will have type `OaValidationError`
 *   const error = maybeScheduledSession;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeScheduledSession` will have type `ScheduledSession`
 * const scheduledSession = maybeScheduledSession;
 * ```
 */
export function validateScheduledSession(maybeScheduledSession: unknown): ScheduledSession | OaValidationError {
  const { value, error } = ScheduledSessionJoiSchema.validate(maybeScheduledSession);
  if (error) {
    return new OaValidationError('ScheduledSession', maybeScheduledSession, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ScheduledSession;
}
