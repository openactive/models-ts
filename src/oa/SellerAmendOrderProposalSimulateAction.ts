 ...
 */
export type SellerAmendOrderProposalSimulateAction = {
  '@type': 'SellerAmendOrderProposalSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SellerAmendOrderProposalSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SellerAmendOrderProposalSimulateActionOrSubClass =
  | SellerAmendOrderProposalSimulateAction
  ;

/**
 * test:SellerAmendOrderProposalSimulateAction - Validation schema (w/ JOI)
 *
 * [NOTICE: This class is part of the Open Booking API Test Interface, and MUST NOT be used in production.] 
 *
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const SellerAmendOrderProposalSimulateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('SellerAmendOrderProposalSimulateAction').required(),
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SellerAmendOrderProposalSimulateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SellerAmendOrderProposalSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  SellerAmendOrderProposalSimulateActionJoiSchema,
]);

/**
 * Runtime validator for test:SellerAmendOrderProposalSimulateAction.
 *
 * If some data has a structure which matches a test:SellerAmendOrderProposalSimulateAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSellerAmendOrderProposalSimulateAction` will have type `SellerAmendOrderProposalSimulateAction`
 * const sellerAmendOrderProposalSimulateAction = maybeSellerAmendOrderProposalSimulateAction;
 * ```
 */
export function validateSellerAmendOrderProposalSimulateAction(maybeSellerAmendOrderProposalSimulateAction: unknown): SellerAmendOrderProposalSimulateAction | OaValidationError {
  const { value, error } = SellerAmendOrderProposalSimulateActionJoiSchema.validate(maybeSellerAmendOrderProposalSimulateAction);
  if (error) {
    return new OaValidationError('SellerAmendOrderProposalSimulateAction', maybeSellerAmendOrderProposalSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SellerAmendOrderProposalSimulateAction;
}
