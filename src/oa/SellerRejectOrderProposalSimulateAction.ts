 ...
 */
export type SellerRejectOrderProposalSimulateAction = {
  '@type': 'SellerRejectOrderProposalSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SellerRejectOrderProposalSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SellerRejectOrderProposalSimulateActionOrSubClass =
  | SellerRejectOrderProposalSimulateAction
  ;

/**
 * test:SellerRejectOrderProposalSimulateAction - Validation schema (w/ JOI)
 *
 * [NOTICE: This class is part of the Open Booking API Test Interface, and MUST NOT be used in production.] 
 *
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const SellerRejectOrderProposalSimulateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('SellerRejectOrderProposalSimulateAction').required(),
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SellerRejectOrderProposalSimulateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SellerRejectOrderProposalSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  SellerRejectOrderProposalSimulateActionJoiSchema,
]);

/**
 * Runtime validator for test:SellerRejectOrderProposalSimulateAction.
 *
 * If some data has a structure which matches a test:SellerRejectOrderProposalSimulateAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSellerRejectOrderProposalSimulateAction` will have type `SellerRejectOrderProposalSimulateAction`
 * const sellerRejectOrderProposalSimulateAction = maybeSellerRejectOrderProposalSimulateAction;
 * ```
 */
export function validateSellerRejectOrderProposalSimulateAction(maybeSellerRejectOrderProposalSimulateAction: unknown): SellerRejectOrderProposalSimulateAction | OaValidationError {
  const { value, error } = SellerRejectOrderProposalSimulateActionJoiSchema.validate(maybeSellerRejectOrderProposalSimulateAction);
  if (error) {
    return new OaValidationError('SellerRejectOrderProposalSimulateAction', maybeSellerRejectOrderProposalSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SellerRejectOrderProposalSimulateAction;
}
