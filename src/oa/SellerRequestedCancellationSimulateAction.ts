 ...
 */
export type SellerRequestedCancellationSimulateAction = {
  '@type': 'SellerRequestedCancellationSimulateAction';
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SellerRequestedCancellationSimulateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SellerRequestedCancellationSimulateActionOrSubClass =
 ...
 * This type is derived from https://schema.org/Action, which means that any of this type's properties within schema.org may also be used.
 */
export const SellerRequestedCancellationSimulateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SellerRequestedCancellationSimulateActionOrSubClassJoiSchema = Joi.alternatives().try([
  SellerRequestedCancellationSimulateActionJoiSchema,
 ...
 * ```
 */
export function validateSellerRequestedCancellationSimulateAction(maybeSellerRequestedCancellationSimulateAction: unknown): SellerRequestedCancellationSimulateAction | OaValidationError {
  const { value, error } = SellerRequestedCancellationSimulateActionJoiSchema.validate(maybeSellerRequestedCancellationSimulateAction);
  if (error) {
    return new OaValidationError('SellerRequestedCancellationSimulateAction', maybeSellerRequestedCancellationSimulateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SellerRequestedCancellationSimulateAction;
}
