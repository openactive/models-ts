...
export type SessionSeries = {
   ...
   * ```
   */
  eventSchedule?: oa.ScheduleOrSubClass[];
  /**
   ...
   */
  subEvent?: oa.ScheduledSessionOrSubClass[];
  /**
 ...
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SessionSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SessionSeriesOrSubClass =
  | SessionSeries
  ;

/**
 * SessionSeries - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 */
export const SessionSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('SessionSeries').required(),
  ...
  subEvent: Joi.array().items(Joi.lazy(() => oa.ScheduledSessionOrSubClassJoiSchema)),
 ...
 * This type is derived from https://schema.org/Event, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from SessionSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SessionSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  SessionSeriesJoiSchema,
]);

/**
 * Runtime validator for SessionSeries.
 *
 * If some data has a structure which matches a SessionSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSessionSeries = SessionSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSessionSeries instanceof OaValidationError) {
 *   // From this point on, `maybeSessionSeries` will have type `OaValidationError`
 *   const error = maybeSessionSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSessionSeries` will have type `SessionSeries`
 * const sessionSeries = maybeSessionSeries;
 * ```
 */
export function validateSessionSeries(maybeSessionSeries: unknown): SessionSeries | OaValidationError {
  const { value, error } = SessionSeriesJoiSchema.validate(maybeSessionSeries);
  if (error) {
    return new OaValidationError('SessionSeries', maybeSessionSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SessionSeries;
}
