 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Showers because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ShowersOrSubClass =
  | Showers
  ;
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const ShowersJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ShowersOrSubClassJoiSchema = Joi.alternatives().try([
  ShowersJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeShowers = Showers.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateShowers(maybeShowers: unknown): Showers | OaValidationError {
  const { value, error } = ShowersJoiSchema.validate(maybeShowers);
  if (error) {
    return new OaValidationError('Showers', maybeShowers, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Showers;
}
