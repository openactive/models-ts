 ...
 */
export type TemporarilyUnableToCreateOrderError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TemporarilyUnableToCreateOrderError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TemporarilyUnableToCreateOrderErrorOrSubClass =
  | TemporarilyUnableToCreateOrderError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const TemporarilyUnableToCreateOrderErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TemporarilyUnableToCreateOrderErrorOrSubClassJoiSchema = Joi.alternatives().try([
  TemporarilyUnableToCreateOrderErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeTemporarilyUnableToCreateOrderError = TemporarilyUnableToCreateOrderError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTemporarilyUnableToCreateOrderError(maybeTemporarilyUnableToCreateOrderError: unknown): TemporarilyUnableToCreateOrderError | OaValidationError {
  const { value, error } = TemporarilyUnableToCreateOrderErrorJoiSchema.validate(maybeTemporarilyUnableToCreateOrderError);
  if (error) {
    return new OaValidationError('TemporarilyUnableToCreateOrderError', maybeTemporarilyUnableToCreateOrderError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TemporarilyUnableToCreateOrderError;
}
