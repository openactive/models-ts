 ...
 */
export type TemporarilyUnableToDeleteOrderError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TemporarilyUnableToDeleteOrderError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TemporarilyUnableToDeleteOrderErrorOrSubClass =
  | TemporarilyUnableToDeleteOrderError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const TemporarilyUnableToDeleteOrderErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TemporarilyUnableToDeleteOrderErrorOrSubClassJoiSchema = Joi.alternatives().try([
  TemporarilyUnableToDeleteOrderErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateTemporarilyUnableToDeleteOrderError(maybeTemporarilyUnableToDeleteOrderError: unknown): TemporarilyUnableToDeleteOrderError | OaValidationError {
  const { value, error } = TemporarilyUnableToDeleteOrderErrorJoiSchema.validate(maybeTemporarilyUnableToDeleteOrderError);
  if (error) {
    return new OaValidationError('TemporarilyUnableToDeleteOrderError', maybeTemporarilyUnableToDeleteOrderError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TemporarilyUnableToDeleteOrderError;
}
