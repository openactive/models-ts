 ...
 */
export type TemporarilyUnableToUpdateOrderError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TemporarilyUnableToUpdateOrderError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TemporarilyUnableToUpdateOrderErrorOrSubClass =
  | TemporarilyUnableToUpdateOrderError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const TemporarilyUnableToUpdateOrderErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TemporarilyUnableToUpdateOrderErrorOrSubClassJoiSchema = Joi.alternatives().try([
  TemporarilyUnableToUpdateOrderErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateTemporarilyUnableToUpdateOrderError(maybeTemporarilyUnableToUpdateOrderError: unknown): TemporarilyUnableToUpdateOrderError | OaValidationError {
  const { value, error } = TemporarilyUnableToUpdateOrderErrorJoiSchema.validate(maybeTemporarilyUnableToUpdateOrderError);
  if (error) {
    return new OaValidationError('TemporarilyUnableToUpdateOrderError', maybeTemporarilyUnableToUpdateOrderError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TemporarilyUnableToUpdateOrderError;
}
