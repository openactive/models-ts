 ...
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Terms because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TermsOrSubClass =
  | Terms
  | oa.PrivacyPolicyOrSubClass
  | oa.TermsOfUseOrSubClass
  ;

/**
 * Terms - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 */
export const TermsJoiSchema = Joi.object({
 ...
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TermsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TermsOrSubClassJoiSchema = Joi.alternatives().try([
  TermsJoiSchema,
  Joi.lazy(() => oa.PrivacyPolicyOrSubClassJoiSchema),
  Joi.lazy(() => oa.TermsOfUseOrSubClassJoiSchema),
]);

/**
 * Runtime validator for Terms.
 *
 * If some data has a structure which matches a Terms, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTerms = Terms.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTerms instanceof OaValidationError) {
 *   // From this point on, `maybeTerms` will have type `OaValidationError`
 *   const error = maybeTerms;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTerms` will have type `Terms`
 * const terms = maybeTerms;
 * ```
 */
export function validateTerms(maybeTerms: unknown): Terms | OaValidationError {
  const { value, error } = TermsJoiSchema.validate(maybeTerms);
  if (error) {
    return new OaValidationError('Terms', maybeTerms, error);
  }
