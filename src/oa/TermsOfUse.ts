 ...
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TermsOfUse because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TermsOfUseOrSubClass =
  | TermsOfUse
  ;

/**
 * TermsOfUse - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 */
export const TermsOfUseJoiSchema = Joi.object({
  '@type': Joi.string().valid('TermsOfUse').required(),
 ...
 * This type is derived from https://schema.org/DigitalDocument, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TermsOfUseJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TermsOfUseOrSubClassJoiSchema = Joi.alternatives().try([
  TermsOfUseJoiSchema,
]);

/**
 * Runtime validator for TermsOfUse.
 *
 * If some data has a structure which matches a TermsOfUse, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTermsOfUse = TermsOfUse.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTermsOfUse instanceof OaValidationError) {
 *   // From this point on, `maybeTermsOfUse` will have type `OaValidationError`
 *   const error = maybeTermsOfUse;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTermsOfUse` will have type `TermsOfUse`
 * const termsOfUse = maybeTermsOfUse;
 * ```
 */
export function validateTermsOfUse(maybeTermsOfUse: unknown): TermsOfUse | OaValidationError {
  const { value, error } = TermsOfUseJoiSchema.validate(maybeTermsOfUse);
  if (error) {
    return new OaValidationError('TermsOfUse', maybeTermsOfUse, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TermsOfUse;
}
