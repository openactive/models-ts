 ...
 *
 * ```ts
 * const maybeTestOpportunityCriteriaEnumeration = TestOpportunityCriteriaEnumeration.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTestOpportunityCriteriaEnumeration(maybeTestOpportunityCriteriaEnumeration: unknown): TestOpportunityCriteriaEnumeration | OaValidationError {
  const { value, error } = TestOpportunityCriteriaEnumerationJoiSchema.validate(maybeTestOpportunityCriteriaEnumeration);
  if (error) {
    return new OaValidationError('TestOpportunityCriteriaEnumeration', maybeTestOpportunityCriteriaEnumeration, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TestOpportunityCriteriaEnumeration;
}
