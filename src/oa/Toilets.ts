 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Toilets because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ToiletsOrSubClass =
  | Toilets
  ;

/**
 * Toilets - Validation schema (w/ JOI)
 *
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const ToiletsJoiSchema = Joi.object({
  '@type': Joi.string().valid('Toilets').required(),
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from ToiletsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ToiletsOrSubClassJoiSchema = Joi.alternatives().try([
  ToiletsJoiSchema,
]);

/**
 * Runtime validator for Toilets.
 *
 * If some data has a structure which matches a Toilets, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeToilets = Toilets.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeToilets instanceof OaValidationError) {
 *   // From this point on, `maybeToilets` will have type `OaValidationError`
 *   const error = maybeToilets;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeToilets` will have type `Toilets`
 * const toilets = maybeToilets;
 * ```
 */
export function validateToilets(maybeToilets: unknown): Toilets | OaValidationError {
  const { value, error } = ToiletsJoiSchema.validate(maybeToilets);
  if (error) {
    return new OaValidationError('Toilets', maybeToilets, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Toilets;
}
