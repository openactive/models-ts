 ...
 */
export type TotalPaymentDueMismatchError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from TotalPaymentDueMismatchError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TotalPaymentDueMismatchErrorOrSubClass =
  | TotalPaymentDueMismatchError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const TotalPaymentDueMismatchErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TotalPaymentDueMismatchErrorOrSubClassJoiSchema = Joi.alternatives().try([
  TotalPaymentDueMismatchErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateTotalPaymentDueMismatchError(maybeTotalPaymentDueMismatchError: unknown): TotalPaymentDueMismatchError | OaValidationError {
  const { value, error } = TotalPaymentDueMismatchErrorJoiSchema.validate(maybeTotalPaymentDueMismatchError);
  if (error) {
    return new OaValidationError('TotalPaymentDueMismatchError', maybeTotalPaymentDueMismatchError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TotalPaymentDueMismatchError;
}
