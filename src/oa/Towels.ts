 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from Towels because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TowelsOrSubClass =
  | Towels
  ;
 ...
 * This type is derived from https://schema.org/LocationFeatureSpecification, which means that any of this type's properties within schema.org may also be used.
 */
export const TowelsJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TowelsOrSubClassJoiSchema = Joi.alternatives().try([
  TowelsJoiSchema,
]);
 ...
 * ```
 */
export function validateTowels(maybeTowels: unknown): Towels | OaValidationError {
  const { value, error } = TowelsJoiSchema.validate(maybeTowels);
  if (error) {
    return new OaValidationError('Towels', maybeTowels, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Towels;
}
