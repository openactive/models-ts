 ...
 */
export type UnableToProcessOrderItemError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from UnableToProcessOrderItemError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnableToProcessOrderItemErrorOrSubClass =
  | UnableToProcessOrderItemError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const UnableToProcessOrderItemErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UnableToProcessOrderItemErrorOrSubClassJoiSchema = Joi.alternatives().try([
  UnableToProcessOrderItemErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateUnableToProcessOrderItemError(maybeUnableToProcessOrderItemError: unknown): UnableToProcessOrderItemError | OaValidationError {
  const { value, error } = UnableToProcessOrderItemErrorJoiSchema.validate(maybeUnableToProcessOrderItemError);
  if (error) {
    return new OaValidationError('UnableToProcessOrderItemError', maybeUnableToProcessOrderItemError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnableToProcessOrderItemError;
}
