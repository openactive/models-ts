 ...
 */
export type UnacceptableOfferError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from UnacceptableOfferError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnacceptableOfferErrorOrSubClass =
  | UnacceptableOfferError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const UnacceptableOfferErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UnacceptableOfferErrorOrSubClassJoiSchema = Joi.alternatives().try([
  UnacceptableOfferErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeUnacceptableOfferError = UnacceptableOfferError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateUnacceptableOfferError(maybeUnacceptableOfferError: unknown): UnacceptableOfferError | OaValidationError {
  const { value, error } = UnacceptableOfferErrorJoiSchema.validate(maybeUnacceptableOfferError);
  if (error) {
    return new OaValidationError('UnacceptableOfferError', maybeUnacceptableOfferError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnacceptableOfferError;
}
