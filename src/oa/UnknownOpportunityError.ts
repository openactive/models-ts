 ...
 */
export type UnknownOpportunityError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from UnknownOpportunityError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnknownOpportunityErrorOrSubClass =
  | UnknownOpportunityError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const UnknownOpportunityErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UnknownOpportunityErrorOrSubClassJoiSchema = Joi.alternatives().try([
  UnknownOpportunityErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateUnknownOpportunityError(maybeUnknownOpportunityError: unknown): UnknownOpportunityError | OaValidationError {
  const { value, error } = UnknownOpportunityErrorJoiSchema.validate(maybeUnknownOpportunityError);
  if (error) {
    return new OaValidationError('UnknownOpportunityError', maybeUnknownOpportunityError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnknownOpportunityError;
}
