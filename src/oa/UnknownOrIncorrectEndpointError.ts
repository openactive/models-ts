 ...
 */
export type UnknownOrIncorrectEndpointError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from UnknownOrIncorrectEndpointError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnknownOrIncorrectEndpointErrorOrSubClass =
  | UnknownOrIncorrectEndpointError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const UnknownOrIncorrectEndpointErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UnknownOrIncorrectEndpointErrorOrSubClassJoiSchema = Joi.alternatives().try([
  UnknownOrIncorrectEndpointErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateUnknownOrIncorrectEndpointError(maybeUnknownOrIncorrectEndpointError: unknown): UnknownOrIncorrectEndpointError | OaValidationError {
  const { value, error } = UnknownOrIncorrectEndpointErrorJoiSchema.validate(maybeUnknownOrIncorrectEndpointError);
  if (error) {
    return new OaValidationError('UnknownOrIncorrectEndpointError', maybeUnknownOrIncorrectEndpointError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnknownOrIncorrectEndpointError;
}
