 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from UnknownOrderError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnknownOrderErrorOrSubClass =
  | UnknownOrderError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const UnknownOrderErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UnknownOrderErrorOrSubClassJoiSchema = Joi.alternatives().try([
  UnknownOrderErrorJoiSchema,
]);
 ...
 * ```
 */
export function validateUnknownOrderError(maybeUnknownOrderError: unknown): UnknownOrderError | OaValidationError {
  const { value, error } = UnknownOrderErrorJoiSchema.validate(maybeUnknownOrderError);
  if (error) {
    return new OaValidationError('UnknownOrderError', maybeUnknownOrderError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnknownOrderError;
}
