 ...
 */
export type UnnecessaryPaymentDetailsError = {
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 *
 * This differs from UnnecessaryPaymentDetailsError because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnnecessaryPaymentDetailsErrorOrSubClass =
  | UnnecessaryPaymentDetailsError
  ;
 ...
 * This type is derived from https://schema.org/Thing, which means that any of this type's properties within schema.org may also be used.
 */
export const UnnecessaryPaymentDetailsErrorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UnnecessaryPaymentDetailsErrorOrSubClassJoiSchema = Joi.alternatives().try([
  UnnecessaryPaymentDetailsErrorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeUnnecessaryPaymentDetailsError = UnnecessaryPaymentDetailsError.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateUnnecessaryPaymentDetailsError(maybeUnnecessaryPaymentDetailsError: unknown): UnnecessaryPaymentDetailsError | OaValidationError {
  const { value, error } = UnnecessaryPaymentDetailsErrorJoiSchema.validate(maybeUnnecessaryPaymentDetailsError);
  if (error) {
    return new OaValidationError('UnnecessaryPaymentDetailsError', maybeUnnecessaryPaymentDetailsError, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnnecessaryPaymentDetailsError;
}
