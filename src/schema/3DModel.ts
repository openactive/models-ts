 ...
 * schema:3DModel
 *
 * This differs from ThreeDModel because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ThreeDModelOrSubClass =
  | ThreeDModel
  ;

/**
 * schema:3DModel - Validation schema (w/ JOI)
 */
export const ThreeDModelJoiSchema = Joi.object({
  '@type': Joi.string().valid('3DModel').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  isResizable: Joi.boolean(),
 ...
 * schema:3DModel - Validation schema (w/ JOI)
 *
 * This differs from ThreeDModelJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ThreeDModelOrSubClassJoiSchema = Joi.alternatives().try([
  ThreeDModelJoiSchema,
]);

/**
 * Runtime validator for schema:3DModel.
 *
 * If some data has a structure which matches a schema:3DModel, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeThreeDModel = ThreeDModel.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeThreeDModel instanceof OaValidationError) {
 *   // From this point on, `maybeThreeDModel` will have type `OaValidationError`
 *   const error = maybeThreeDModel;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeThreeDModel` will have type `ThreeDModel`
 * const threeDModel = maybeThreeDModel;
 * ```
 */
export function validateThreeDModel(maybeThreeDModel: unknown): ThreeDModel | OaValidationError {
  const { value, error } = ThreeDModelJoiSchema.validate(maybeThreeDModel);
  if (error) {
    return new OaValidationError('ThreeDModel', maybeThreeDModel, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ThreeDModel;
}
