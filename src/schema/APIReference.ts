 ...
 * schema:APIReference
 *
 * This differs from APIReference because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type APIReferenceOrSubClass =
  | APIReference
  ;

/**
 * schema:APIReference - Validation schema (w/ JOI)
 */
export const APIReferenceJoiSchema = Joi.object({
  '@type': Joi.string().valid('APIReference').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  programmingModel: Joi.string(),
  targetPlatform: Joi.string(),
  assembly: Joi.string(),
  executableLibraryName: Joi.string(),
  assemblyVersion: Joi.string(),
 ...
 * schema:APIReference - Validation schema (w/ JOI)
 *
 * This differs from APIReferenceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const APIReferenceOrSubClassJoiSchema = Joi.alternatives().try([
  APIReferenceJoiSchema,
]);

/**
 * Runtime validator for schema:APIReference.
 *
 * If some data has a structure which matches a schema:APIReference, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAPIReference = APIReference.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAPIReference instanceof OaValidationError) {
 *   // From this point on, `maybeAPIReference` will have type `OaValidationError`
 *   const error = maybeAPIReference;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAPIReference` will have type `APIReference`
 * const aPIReference = maybeAPIReference;
 * ```
 */
export function validateAPIReference(maybeAPIReference: unknown): APIReference | OaValidationError {
  const { value, error } = APIReferenceJoiSchema.validate(maybeAPIReference);
  if (error) {
    return new OaValidationError('APIReference', maybeAPIReference, error);
  }
