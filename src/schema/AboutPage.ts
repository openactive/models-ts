 ...
 * schema:AboutPage
 *
 * This differs from AboutPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AboutPageOrSubClass =
  | AboutPage
  ;

/**
 * schema:AboutPage - Validation schema (w/ JOI)
 */
export const AboutPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('AboutPage').required(),
 ...
 * schema:AboutPage - Validation schema (w/ JOI)
 *
 * This differs from AboutPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AboutPageOrSubClassJoiSchema = Joi.alternatives().try([
  AboutPageJoiSchema,
]);

/**
 * Runtime validator for schema:AboutPage.
 *
 * If some data has a structure which matches a schema:AboutPage, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAboutPage` will have type `AboutPage`
 * const aboutPage = maybeAboutPage;
 * ```
 */
export function validateAboutPage(maybeAboutPage: unknown): AboutPage | OaValidationError {
  const { value, error } = AboutPageJoiSchema.validate(maybeAboutPage);
  if (error) {
    return new OaValidationError('AboutPage', maybeAboutPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AboutPage;
}
