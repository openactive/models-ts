 ...
 * schema:AcceptAction
 *
 * This differs from AcceptAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AcceptActionOrSubClass =
  | AcceptAction
  ;
 ...
 */
export const AcceptActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AcceptActionOrSubClassJoiSchema = Joi.alternatives().try([
  AcceptActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeAcceptAction = AcceptAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateAcceptAction(maybeAcceptAction: unknown): AcceptAction | OaValidationError {
  const { value, error } = AcceptActionJoiSchema.validate(maybeAcceptAction);
  if (error) {
    return new OaValidationError('AcceptAction', maybeAcceptAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AcceptAction;
}
