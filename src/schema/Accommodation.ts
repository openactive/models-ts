 ...
 * schema:Accommodation
 *
 * This differs from Accommodation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AccommodationOrSubClass =
  | Accommodation
  | schema.RoomOrSubClass
  | schema.ApartmentOrSubClass
  | schema.SuiteOrSubClass
  | schema.HouseOrSubClass
  | schema.CampingPitchOrSubClass
  ;

/**
 * schema:Accommodation - Validation schema (w/ JOI)
 */
export const AccommodationJoiSchema = Joi.object({
  '@type': Joi.string().valid('Accommodation').required(),
 ...
 * schema:Accommodation - Validation schema (w/ JOI)
 *
 * This differs from AccommodationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AccommodationOrSubClassJoiSchema = Joi.alternatives().try([
  AccommodationJoiSchema,
  Joi.lazy(() => schema.RoomOrSubClassJoiSchema),
  Joi.lazy(() => schema.ApartmentOrSubClassJoiSchema),
  Joi.lazy(() => schema.SuiteOrSubClassJoiSchema),
  Joi.lazy(() => schema.HouseOrSubClassJoiSchema),
  Joi.lazy(() => schema.CampingPitchOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Accommodation.
 *
 * If some data has a structure which matches a schema:Accommodation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAccommodation = validateAccommodation(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAccommodation instanceof OaValidationError) {
 *   // From this point on, `maybeAccommodation` will have type `OaValidationError`
 *   const error = maybeAccommodation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAccommodation` will have type `Accommodation`
 * const accommodation = maybeAccommodation;
 * ```
 */
export function validateAccommodation(maybeAccommodation: unknown): Accommodation | OaValidationError {
  const { value, error } = AccommodationJoiSchema.validate(maybeAccommodation);
  if (error) {
    return new OaValidationError('Accommodation', maybeAccommodation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Accommodation;
}
