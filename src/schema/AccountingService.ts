 ...
 * schema:AccountingService
 *
 * This differs from AccountingService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AccountingServiceOrSubClass =
  | AccountingService
  ;

/**
 * schema:AccountingService - Validation schema (w/ JOI)
 */
export const AccountingServiceJoiSchema = Joi.object({
  '@type': Joi.string().valid('AccountingService').required(),
 ...
 * schema:AccountingService - Validation schema (w/ JOI)
 *
 * This differs from AccountingServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AccountingServiceOrSubClassJoiSchema = Joi.alternatives().try([
  AccountingServiceJoiSchema,
]);

/**
 * Runtime validator for schema:AccountingService.
 *
 * If some data has a structure which matches a schema:AccountingService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAccountingService = AccountingService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAccountingService instanceof OaValidationError) {
 *   // From this point on, `maybeAccountingService` will have type `OaValidationError`
 *   const error = maybeAccountingService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAccountingService` will have type `AccountingService`
 * const accountingService = maybeAccountingService;
 * ```
 */
export function validateAccountingService(maybeAccountingService: unknown): AccountingService | OaValidationError {
  const { value, error } = AccountingServiceJoiSchema.validate(maybeAccountingService);
  if (error) {
    return new OaValidationError('AccountingService', maybeAccountingService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AccountingService;
}
