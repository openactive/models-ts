 ...
 * schema:AchieveAction
 *
 * This differs from AchieveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AchieveActionOrSubClass =
  | AchieveAction
  | schema.WinActionOrSubClass
  | schema.TieActionOrSubClass
  | schema.LoseActionOrSubClass
  ;

/**
 * schema:AchieveAction - Validation schema (w/ JOI)
 */
export const AchieveActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AchieveActionOrSubClassJoiSchema = Joi.alternatives().try([
  AchieveActionJoiSchema,
 ...
 * ```
 */
export function validateAchieveAction(maybeAchieveAction: unknown): AchieveAction | OaValidationError {
  const { value, error } = AchieveActionJoiSchema.validate(maybeAchieveAction);
  if (error) {
    return new OaValidationError('AchieveAction', maybeAchieveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AchieveAction;
}
