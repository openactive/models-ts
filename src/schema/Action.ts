 ...
 * schema:Action
 *
 * This differs from Action because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ActionOrSubClass =
  | Action
  | schema.ConsumeActionOrSubClass
  | schema.MoveActionOrSubClass
  | schema.AchieveActionOrSubClass
  | schema.ControlActionOrSubClass
  | schema.FindActionOrSubClass
  | schema.PlayActionOrSubClass
  | schema.AssessActionOrSubClass
  | schema.OrganizeActionOrSubClass
  | schema.SearchActionOrSubClass
  | schema.SeekToActionOrSubClass
  | schema.TradeActionOrSubClass
  | schema.SolveMathActionOrSubClass
  | schema.CreateActionOrSubClass
  | schema.InteractActionOrSubClass
  | schema.UpdateActionOrSubClass
  | schema.TransferActionOrSubClass
  ;

/**
 * schema:Action - Validation schema (w/ JOI)
 */
export const ActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ActionOrSubClassJoiSchema = Joi.alternatives().try([
  ActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeAction = Action.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateAction(maybeAction: unknown): Action | OaValidationError {
  const { value, error } = ActionJoiSchema.validate(maybeAction);
  if (error) {
    return new OaValidationError('Action', maybeAction, error);
  }
