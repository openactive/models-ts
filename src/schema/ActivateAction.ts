 ...
 * schema:ActivateAction
 *
 * This differs from ActivateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ActivateActionOrSubClass =
  | ActivateAction
  ;

/**
 * schema:ActivateAction - Validation schema (w/ JOI)
 */
export const ActivateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ActivateAction').required(),
 ...
 * schema:ActivateAction - Validation schema (w/ JOI)
 *
 * This differs from ActivateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ActivateActionOrSubClassJoiSchema = Joi.alternatives().try([
  ActivateActionJoiSchema,
]);

/**
 * Runtime validator for schema:ActivateAction.
 *
 * If some data has a structure which matches a schema:ActivateAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeActivateAction = ActivateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeActivateAction instanceof OaValidationError) {
 *   // From this point on, `maybeActivateAction` will have type `OaValidationError`
 *   const error = maybeActivateAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeActivateAction` will have type `ActivateAction`
 * const activateAction = maybeActivateAction;
 * ```
 */
export function validateActivateAction(maybeActivateAction: unknown): ActivateAction | OaValidationError {
  const { value, error } = ActivateActionJoiSchema.validate(maybeActivateAction);
  if (error) {
    return new OaValidationError('ActivateAction', maybeActivateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ActivateAction;
}
