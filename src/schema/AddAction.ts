 ...
 * schema:AddAction
 *
 * This differs from AddAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AddActionOrSubClass =
  | AddAction
  | schema.InsertActionOrSubClass
  ;

/**
 * schema:AddAction - Validation schema (w/ JOI)
 */
export const AddActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AddAction').required(),
 ...
 * schema:AddAction - Validation schema (w/ JOI)
 *
 * This differs from AddActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AddActionOrSubClassJoiSchema = Joi.alternatives().try([
  AddActionJoiSchema,
  Joi.lazy(() => schema.InsertActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:AddAction.
 *
 * If some data has a structure which matches a schema:AddAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAddAction = AddAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAddAction instanceof OaValidationError) {
 *   // From this point on, `maybeAddAction` will have type `OaValidationError`
 *   const error = maybeAddAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAddAction` will have type `AddAction`
 * const addAction = maybeAddAction;
 * ```
 */
export function validateAddAction(maybeAddAction: unknown): AddAction | OaValidationError {
  const { value, error } = AddActionJoiSchema.validate(maybeAddAction);
  if (error) {
    return new OaValidationError('AddAction', maybeAddAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AddAction;
}
