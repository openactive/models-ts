 ...
 * schema:AdministrativeArea
 *
 * This differs from AdministrativeArea because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AdministrativeAreaOrSubClass =
  | AdministrativeArea
  | schema.StateOrSubClass
  | schema.CountryOrSubClass
  | schema.CityOrSubClass
  | schema.SchoolDistrictOrSubClass
  ;

/**
 * schema:AdministrativeArea - Validation schema (w/ JOI)
 */
export const AdministrativeAreaJoiSchema = Joi.object({
  '@type': Joi.string().valid('AdministrativeArea').required(),
 ...
 * schema:AdministrativeArea - Validation schema (w/ JOI)
 *
 * This differs from AdministrativeAreaJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AdministrativeAreaOrSubClassJoiSchema = Joi.alternatives().try([
  AdministrativeAreaJoiSchema,
  Joi.lazy(() => schema.StateOrSubClassJoiSchema),
  Joi.lazy(() => schema.CountryOrSubClassJoiSchema),
  Joi.lazy(() => schema.CityOrSubClassJoiSchema),
  Joi.lazy(() => schema.SchoolDistrictOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:AdministrativeArea.
 *
 * If some data has a structure which matches a schema:AdministrativeArea, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAdministrativeArea` will have type `AdministrativeArea`
 * const administrativeArea = maybeAdministrativeArea;
 * ```
 */
export function validateAdministrativeArea(maybeAdministrativeArea: unknown): AdministrativeArea | OaValidationError {
  const { value, error } = AdministrativeAreaJoiSchema.validate(maybeAdministrativeArea);
  if (error) {
    return new OaValidationError('AdministrativeArea', maybeAdministrativeArea, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AdministrativeArea;
}
