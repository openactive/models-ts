 ...
 * schema:AdultEntertainment
 *
 * This differs from AdultEntertainment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AdultEntertainmentOrSubClass =
  | AdultEntertainment
  ;

/**
 * schema:AdultEntertainment - Validation schema (w/ JOI)
 */
export const AdultEntertainmentJoiSchema = Joi.object({
  '@type': Joi.string().valid('AdultEntertainment').required(),
 ...
 * schema:AdultEntertainment - Validation schema (w/ JOI)
 *
 * This differs from AdultEntertainmentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AdultEntertainmentOrSubClassJoiSchema = Joi.alternatives().try([
  AdultEntertainmentJoiSchema,
]);

/**
 * Runtime validator for schema:AdultEntertainment.
 *
 * If some data has a structure which matches a schema:AdultEntertainment, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAdultEntertainment = AdultEntertainment.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAdultEntertainment instanceof OaValidationError) {
 *   // From this point on, `maybeAdultEntertainment` will have type `OaValidationError`
 *   const error = maybeAdultEntertainment;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAdultEntertainment` will have type `AdultEntertainment`
 * const adultEntertainment = maybeAdultEntertainment;
 * ```
 */
export function validateAdultEntertainment(maybeAdultEntertainment: unknown): AdultEntertainment | OaValidationError {
  const { value, error } = AdultEntertainmentJoiSchema.validate(maybeAdultEntertainment);
  if (error) {
    return new OaValidationError('AdultEntertainment', maybeAdultEntertainment, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AdultEntertainment;
}
