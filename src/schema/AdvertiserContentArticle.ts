 ...
 * schema:AdvertiserContentArticle
 *
 * This differs from AdvertiserContentArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AdvertiserContentArticleOrSubClass =
  | AdvertiserContentArticle
  ;

/**
 * schema:AdvertiserContentArticle - Validation schema (w/ JOI)
 */
export const AdvertiserContentArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('AdvertiserContentArticle').required(),
 ...
 * schema:AdvertiserContentArticle - Validation schema (w/ JOI)
 *
 * This differs from AdvertiserContentArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AdvertiserContentArticleOrSubClassJoiSchema = Joi.alternatives().try([
  AdvertiserContentArticleJoiSchema,
]);

/**
 * Runtime validator for schema:AdvertiserContentArticle.
 *
 * If some data has a structure which matches a schema:AdvertiserContentArticle, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAdvertiserContentArticle` will have type `AdvertiserContentArticle`
 * const advertiserContentArticle = maybeAdvertiserContentArticle;
 * ```
 */
export function validateAdvertiserContentArticle(maybeAdvertiserContentArticle: unknown): AdvertiserContentArticle | OaValidationError {
  const { value, error } = AdvertiserContentArticleJoiSchema.validate(maybeAdvertiserContentArticle);
  if (error) {
    return new OaValidationError('AdvertiserContentArticle', maybeAdvertiserContentArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AdvertiserContentArticle;
}
