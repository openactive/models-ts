 ...
 * etc.
 */
export type AggregateOfferOrSubClass =
  | AggregateOffer
  ;
 ...
 */
export const AggregateOfferJoiSchema = Joi.object({
  ...
  highPrice: Joi.alternatives().try(Joi.string(), Joi.number()),
 ...
 * `SessionSeries`, etc.
 */
export const AggregateOfferOrSubClassJoiSchema = Joi.alternatives().try([
  AggregateOfferJoiSchema,
]);
 ...
 * ```
 */
export function validateAggregateOffer(maybeAggregateOffer: unknown): AggregateOffer | OaValidationError {
  const { value, error } = AggregateOfferJoiSchema.validate(maybeAggregateOffer);
  if (error) {
    return new OaValidationError('AggregateOffer', maybeAggregateOffer, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AggregateOffer;
}
