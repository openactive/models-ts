 ...
 * schema:AgreeAction
 *
 * This differs from AgreeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AgreeActionOrSubClass =
  | AgreeAction
  ;

/**
 * schema:AgreeAction - Validation schema (w/ JOI)
 */
export const AgreeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AgreeAction').required(),
 ...
 * schema:AgreeAction - Validation schema (w/ JOI)
 *
 * This differs from AgreeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AgreeActionOrSubClassJoiSchema = Joi.alternatives().try([
  AgreeActionJoiSchema,
]);

/**
 * Runtime validator for schema:AgreeAction.
 *
 * If some data has a structure which matches a schema:AgreeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAgreeAction = AgreeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAgreeAction instanceof OaValidationError) {
 *   // From this point on, `maybeAgreeAction` will have type `OaValidationError`
 *   const error = maybeAgreeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAgreeAction` will have type `AgreeAction`
 * const agreeAction = maybeAgreeAction;
 * ```
 */
export function validateAgreeAction(maybeAgreeAction: unknown): AgreeAction | OaValidationError {
  const { value, error } = AgreeActionJoiSchema.validate(maybeAgreeAction);
  if (error) {
    return new OaValidationError('AgreeAction', maybeAgreeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AgreeAction;
}
