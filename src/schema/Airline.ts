 ...
 * schema:Airline
 *
 * This differs from Airline because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AirlineOrSubClass =
  | Airline
  ;

/**
 * schema:Airline - Validation schema (w/ JOI)
 */
export const AirlineJoiSchema = Joi.object({
  '@type': Joi.string().valid('Airline').required(),
 ...
 * schema:Airline - Validation schema (w/ JOI)
 *
 * This differs from AirlineJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AirlineOrSubClassJoiSchema = Joi.alternatives().try([
  AirlineJoiSchema,
]);

/**
 * Runtime validator for schema:Airline.
 *
 * If some data has a structure which matches a schema:Airline, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAirline = Airline.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAirline instanceof OaValidationError) {
 *   // From this point on, `maybeAirline` will have type `OaValidationError`
 *   const error = maybeAirline;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAirline` will have type `Airline`
 * const airline = maybeAirline;
 * ```
 */
export function validateAirline(maybeAirline: unknown): Airline | OaValidationError {
  const { value, error } = AirlineJoiSchema.validate(maybeAirline);
  if (error) {
    return new OaValidationError('Airline', maybeAirline, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Airline;
}
