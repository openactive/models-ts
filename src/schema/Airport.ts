 ...
 * schema:Airport
 *
 * This differs from Airport because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AirportOrSubClass =
  | Airport
  ;

/**
 * schema:Airport - Validation schema (w/ JOI)
 */
export const AirportJoiSchema = Joi.object({
 ...
 * schema:Airport - Validation schema (w/ JOI)
 *
 * This differs from AirportJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AirportOrSubClassJoiSchema = Joi.alternatives().try([
  AirportJoiSchema,
]);

/**
 * Runtime validator for schema:Airport.
 *
 * If some data has a structure which matches a schema:Airport, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAirport = Airport.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAirport instanceof OaValidationError) {
 *   // From this point on, `maybeAirport` will have type `OaValidationError`
 *   const error = maybeAirport;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAirport` will have type `Airport`
 * const airport = maybeAirport;
 * ```
 */
export function validateAirport(maybeAirport: unknown): Airport | OaValidationError {
  const { value, error } = AirportJoiSchema.validate(maybeAirport);
  if (error) {
    return new OaValidationError('Airport', maybeAirport, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Airport;
}
