 ...
 * schema:AllocateAction
 *
 * This differs from AllocateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AllocateActionOrSubClass =
  | AllocateAction
  | schema.RejectActionOrSubClass
  | schema.AcceptActionOrSubClass
  | schema.AssignActionOrSubClass
  | schema.AuthorizeActionOrSubClass
  ;

/**
 * schema:AllocateAction - Validation schema (w/ JOI)
 */
export const AllocateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AllocateAction').required(),
 ...
 * schema:AllocateAction - Validation schema (w/ JOI)
 *
 * This differs from AllocateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AllocateActionOrSubClassJoiSchema = Joi.alternatives().try([
  AllocateActionJoiSchema,
  Joi.lazy(() => schema.RejectActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AcceptActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AssignActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AuthorizeActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:AllocateAction.
 *
 * If some data has a structure which matches a schema:AllocateAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAllocateAction = AllocateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAllocateAction instanceof OaValidationError) {
 *   // From this point on, `maybeAllocateAction` will have type `OaValidationError`
 *   const error = maybeAllocateAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAllocateAction` will have type `AllocateAction`
 * const allocateAction = maybeAllocateAction;
 * ```
 */
export function validateAllocateAction(maybeAllocateAction: unknown): AllocateAction | OaValidationError {
  const { value, error } = AllocateActionJoiSchema.validate(maybeAllocateAction);
  if (error) {
    return new OaValidationError('AllocateAction', maybeAllocateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AllocateAction;
}
