 ...
 * schema:AmpStory
 *
 * This differs from AmpStory because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AmpStoryOrSubClass =
  | AmpStory
  ;

/**
 * schema:AmpStory - Validation schema (w/ JOI)
 */
export const AmpStoryJoiSchema = Joi.object({
  '@type': Joi.string().valid('AmpStory').required(),
 ...
 * schema:AmpStory - Validation schema (w/ JOI)
 *
 * This differs from AmpStoryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AmpStoryOrSubClassJoiSchema = Joi.alternatives().try([
  AmpStoryJoiSchema,
]);

/**
 * Runtime validator for schema:AmpStory.
 *
 * If some data has a structure which matches a schema:AmpStory, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAmpStory` will have type `AmpStory`
 * const ampStory = maybeAmpStory;
 * ```
 */
export function validateAmpStory(maybeAmpStory: unknown): AmpStory | OaValidationError {
  const { value, error } = AmpStoryJoiSchema.validate(maybeAmpStory);
  if (error) {
    return new OaValidationError('AmpStory', maybeAmpStory, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AmpStory;
}
