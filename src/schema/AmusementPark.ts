 ...
 * schema:AmusementPark
 *
 * This differs from AmusementPark because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AmusementParkOrSubClass =
  | AmusementPark
  ;

/**
 * schema:AmusementPark - Validation schema (w/ JOI)
 */
export const AmusementParkJoiSchema = Joi.object({
  '@type': Joi.string().valid('AmusementPark').required(),
 ...
 * schema:AmusementPark - Validation schema (w/ JOI)
 *
 * This differs from AmusementParkJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AmusementParkOrSubClassJoiSchema = Joi.alternatives().try([
  AmusementParkJoiSchema,
]);

/**
 * Runtime validator for schema:AmusementPark.
 *
 * If some data has a structure which matches a schema:AmusementPark, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAmusementPark = AmusementPark.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAmusementPark instanceof OaValidationError) {
 *   // From this point on, `maybeAmusementPark` will have type `OaValidationError`
 *   const error = maybeAmusementPark;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAmusementPark` will have type `AmusementPark`
 * const amusementPark = maybeAmusementPark;
 * ```
 */
export function validateAmusementPark(maybeAmusementPark: unknown): AmusementPark | OaValidationError {
  const { value, error } = AmusementParkJoiSchema.validate(maybeAmusementPark);
  if (error) {
    return new OaValidationError('AmusementPark', maybeAmusementPark, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AmusementPark;
}
