 ...
 * schema:AnalysisNewsArticle
 *
 * This differs from AnalysisNewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AnalysisNewsArticleOrSubClass =
  | AnalysisNewsArticle
  ;

/**
 * schema:AnalysisNewsArticle - Validation schema (w/ JOI)
 */
export const AnalysisNewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('AnalysisNewsArticle').required(),
 ...
 * schema:AnalysisNewsArticle - Validation schema (w/ JOI)
 *
 * This differs from AnalysisNewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AnalysisNewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  AnalysisNewsArticleJoiSchema,
]);

/**
 * Runtime validator for schema:AnalysisNewsArticle.
 *
 * If some data has a structure which matches a schema:AnalysisNewsArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAnalysisNewsArticle = AnalysisNewsArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAnalysisNewsArticle instanceof OaValidationError) {
 *   // From this point on, `maybeAnalysisNewsArticle` will have type `OaValidationError`
 *   const error = maybeAnalysisNewsArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAnalysisNewsArticle` will have type `AnalysisNewsArticle`
 * const analysisNewsArticle = maybeAnalysisNewsArticle;
 * ```
 */
export function validateAnalysisNewsArticle(maybeAnalysisNewsArticle: unknown): AnalysisNewsArticle | OaValidationError {
  const { value, error } = AnalysisNewsArticleJoiSchema.validate(maybeAnalysisNewsArticle);
  if (error) {
    return new OaValidationError('AnalysisNewsArticle', maybeAnalysisNewsArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AnalysisNewsArticle;
}
