 ...
 * schema:AnimalShelter
 *
 * This differs from AnimalShelter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AnimalShelterOrSubClass =
  | AnimalShelter
  ;

/**
 * schema:AnimalShelter - Validation schema (w/ JOI)
 */
export const AnimalShelterJoiSchema = Joi.object({
  '@type': Joi.string().valid('AnimalShelter').required(),
 ...
 * schema:AnimalShelter - Validation schema (w/ JOI)
 *
 * This differs from AnimalShelterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AnimalShelterOrSubClassJoiSchema = Joi.alternatives().try([
  AnimalShelterJoiSchema,
]);

/**
 * Runtime validator for schema:AnimalShelter.
 *
 * If some data has a structure which matches a schema:AnimalShelter, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAnimalShelter = AnimalShelter.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAnimalShelter instanceof OaValidationError) {
 *   // From this point on, `maybeAnimalShelter` will have type `OaValidationError`
 *   const error = maybeAnimalShelter;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAnimalShelter` will have type `AnimalShelter`
 * const animalShelter = maybeAnimalShelter;
 * ```
 */
export function validateAnimalShelter(maybeAnimalShelter: unknown): AnimalShelter | OaValidationError {
  const { value, error } = AnimalShelterJoiSchema.validate(maybeAnimalShelter);
  if (error) {
    return new OaValidationError('AnimalShelter', maybeAnimalShelter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AnimalShelter;
}
