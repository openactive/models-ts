...
export type Answer = {
   ...
   */
  answerExplanation?: schema.CommentOrSubClass | schema.WebContentOrSubClass | string;
  /**
 ...
 * schema:Answer
 *
 * This differs from Answer because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AnswerOrSubClass =
  | Answer
  ;

/**
 * schema:Answer - Validation schema (w/ JOI)
 */
export const AnswerJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  answerExplanation: Joi.alternatives().try(Joi.lazy(() => schema.CommentOrSubClassJoiSchema), Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:Answer - Validation schema (w/ JOI)
 *
 * This differs from AnswerJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AnswerOrSubClassJoiSchema = Joi.alternatives().try([
  AnswerJoiSchema,
]);

/**
 * Runtime validator for schema:Answer.
 *
 * If some data has a structure which matches a schema:Answer, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAnswer = Answer.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAnswer instanceof OaValidationError) {
 *   // From this point on, `maybeAnswer` will have type `OaValidationError`
 *   const error = maybeAnswer;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAnswer` will have type `Answer`
 * const answer = maybeAnswer;
 * ```
 */
export function validateAnswer(maybeAnswer: unknown): Answer | OaValidationError {
  const { value, error } = AnswerJoiSchema.validate(maybeAnswer);
  if (error) {
    return new OaValidationError('Answer', maybeAnswer, error);
  }
