...
export type Answer = {
   ...
   */
  answerExplanation?: schema.WebContentOrSubClass | schema.CommentOrSubClass | string;
  /**
...
export const AnswerJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  answerExplanation: Joi.alternatives().try(Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.lazy(() => schema.CommentOrSubClassJoiSchema), Joi.string().uri()),
 ...
 *
 * ```ts
 * const maybeAnswer = validateAnswer(req.body); // `req.body` will have type `any` or `unknown`
