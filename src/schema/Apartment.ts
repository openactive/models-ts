 ...
 * schema:Apartment
 *
 * This differs from Apartment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ApartmentOrSubClass =
  | Apartment
  ;

/**
 * schema:Apartment - Validation schema (w/ JOI)
 */
export const ApartmentJoiSchema = Joi.object({
 ...
 * schema:Apartment - Validation schema (w/ JOI)
 *
 * This differs from ApartmentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ApartmentOrSubClassJoiSchema = Joi.alternatives().try([
  ApartmentJoiSchema,
]);

/**
 * Runtime validator for schema:Apartment.
 *
 * If some data has a structure which matches a schema:Apartment, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeApartment = validateApartment(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeApartment instanceof OaValidationError) {
 *   // From this point on, `maybeApartment` will have type `OaValidationError`
 *   const error = maybeApartment;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeApartment` will have type `Apartment`
 * const apartment = maybeApartment;
 * ```
 */
export function validateApartment(maybeApartment: unknown): Apartment | OaValidationError {
  const { value, error } = ApartmentJoiSchema.validate(maybeApartment);
  if (error) {
    return new OaValidationError('Apartment', maybeApartment, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Apartment;
}
