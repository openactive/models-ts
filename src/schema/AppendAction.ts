 ...
 * schema:AppendAction
 *
 * This differs from AppendAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AppendActionOrSubClass =
  | AppendAction
  ;

/**
 * schema:AppendAction - Validation schema (w/ JOI)
 */
export const AppendActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AppendAction').required(),
 ...
 * schema:AppendAction - Validation schema (w/ JOI)
 *
 * This differs from AppendActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AppendActionOrSubClassJoiSchema = Joi.alternatives().try([
  AppendActionJoiSchema,
]);

/**
 * Runtime validator for schema:AppendAction.
 *
 * If some data has a structure which matches a schema:AppendAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAppendAction` will have type `AppendAction`
 * const appendAction = maybeAppendAction;
 * ```
 */
export function validateAppendAction(maybeAppendAction: unknown): AppendAction | OaValidationError {
  const { value, error } = AppendActionJoiSchema.validate(maybeAppendAction);
  if (error) {
    return new OaValidationError('AppendAction', maybeAppendAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AppendAction;
}
