 ...
 * schema:ApplyAction
 *
 * This differs from ApplyAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ApplyActionOrSubClass =
  | ApplyAction
  ;

/**
 * schema:ApplyAction - Validation schema (w/ JOI)
 */
export const ApplyActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ApplyAction').required(),
 ...
 * schema:ApplyAction - Validation schema (w/ JOI)
 *
 * This differs from ApplyActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ApplyActionOrSubClassJoiSchema = Joi.alternatives().try([
  ApplyActionJoiSchema,
]);

/**
 * Runtime validator for schema:ApplyAction.
 *
 * If some data has a structure which matches a schema:ApplyAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeApplyAction = ApplyAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeApplyAction instanceof OaValidationError) {
 *   // From this point on, `maybeApplyAction` will have type `OaValidationError`
 *   const error = maybeApplyAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeApplyAction` will have type `ApplyAction`
 * const applyAction = maybeApplyAction;
 * ```
 */
export function validateApplyAction(maybeApplyAction: unknown): ApplyAction | OaValidationError {
  const { value, error } = ApplyActionJoiSchema.validate(maybeApplyAction);
  if (error) {
    return new OaValidationError('ApplyAction', maybeApplyAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ApplyAction;
}
