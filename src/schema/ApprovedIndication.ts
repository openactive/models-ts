 ...
 * schema:ApprovedIndication
 *
 * This differs from ApprovedIndication because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ApprovedIndicationOrSubClass =
  | ApprovedIndication
  ;

/**
 * schema:ApprovedIndication - Validation schema (w/ JOI)
 */
export const ApprovedIndicationJoiSchema = Joi.object({
  '@type': Joi.string().valid('ApprovedIndication').required(),
 ...
 * schema:ApprovedIndication - Validation schema (w/ JOI)
 *
 * This differs from ApprovedIndicationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ApprovedIndicationOrSubClassJoiSchema = Joi.alternatives().try([
  ApprovedIndicationJoiSchema,
]);

/**
 * Runtime validator for schema:ApprovedIndication.
 *
 * If some data has a structure which matches a schema:ApprovedIndication, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeApprovedIndication` will have type `ApprovedIndication`
 * const approvedIndication = maybeApprovedIndication;
 * ```
 */
export function validateApprovedIndication(maybeApprovedIndication: unknown): ApprovedIndication | OaValidationError {
  const { value, error } = ApprovedIndicationJoiSchema.validate(maybeApprovedIndication);
  if (error) {
    return new OaValidationError('ApprovedIndication', maybeApprovedIndication, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ApprovedIndication;
}
