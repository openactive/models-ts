 ...
 * schema:Aquarium
 *
 * This differs from Aquarium because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AquariumOrSubClass =
  | Aquarium
  ;

/**
 * schema:Aquarium - Validation schema (w/ JOI)
 */
export const AquariumJoiSchema = Joi.object({
  '@type': Joi.string().valid('Aquarium').required(),
 ...
 * schema:Aquarium - Validation schema (w/ JOI)
 *
 * This differs from AquariumJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AquariumOrSubClassJoiSchema = Joi.alternatives().try([
  AquariumJoiSchema,
]);

/**
 * Runtime validator for schema:Aquarium.
 *
 * If some data has a structure which matches a schema:Aquarium, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAquarium = Aquarium.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAquarium instanceof OaValidationError) {
 *   // From this point on, `maybeAquarium` will have type `OaValidationError`
 *   const error = maybeAquarium;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAquarium` will have type `Aquarium`
 * const aquarium = maybeAquarium;
 * ```
 */
export function validateAquarium(maybeAquarium: unknown): Aquarium | OaValidationError {
  const { value, error } = AquariumJoiSchema.validate(maybeAquarium);
  if (error) {
    return new OaValidationError('Aquarium', maybeAquarium, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Aquarium;
}
