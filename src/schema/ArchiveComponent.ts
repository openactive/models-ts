...
export type ArchiveComponent = {
   ...
   * [object Object]
   */
  holdingArchive?: schema.ArchiveOrganizationOrSubClass | string;
  /**
   * [object Object]
   */
  itemLocation?: schema.PostalAddressOrSubClass | schema.PlaceOrSubClass | string;
  /**
 ...
 * schema:ArchiveComponent
 *
 * This differs from ArchiveComponent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ArchiveComponentOrSubClass =
  | ArchiveComponent
  ;

/**
 * schema:ArchiveComponent - Validation schema (w/ JOI)
 */
export const ArchiveComponentJoiSchema = Joi.object({
  '@type': Joi.string().valid('ArchiveComponent').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  holdingArchive: Joi.alternatives().try(Joi.lazy(() => schema.ArchiveOrganizationOrSubClassJoiSchema), Joi.string().uri()),
  itemLocation: Joi.alternatives().try(Joi.lazy(() => schema.PostalAddressOrSubClassJoiSchema), Joi.lazy(() => schema.PlaceOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:ArchiveComponent - Validation schema (w/ JOI)
 *
 * This differs from ArchiveComponentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ArchiveComponentOrSubClassJoiSchema = Joi.alternatives().try([
  ArchiveComponentJoiSchema,
]);

/**
 * Runtime validator for schema:ArchiveComponent.
 *
 * If some data has a structure which matches a schema:ArchiveComponent, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeArchiveComponent` will have type `ArchiveComponent`
 * const archiveComponent = maybeArchiveComponent;
 * ```
 */
export function validateArchiveComponent(maybeArchiveComponent: unknown): ArchiveComponent | OaValidationError {
  const { value, error } = ArchiveComponentJoiSchema.validate(maybeArchiveComponent);
  if (error) {
    return new OaValidationError('ArchiveComponent', maybeArchiveComponent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ArchiveComponent;
}
