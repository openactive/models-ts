...
export type ArchiveOrganization = {
   ...
   * [object Object]
   */
  archiveHeld?: schema.ArchiveComponentOrSubClass | string;
  /**
 ...
 * schema:ArchiveOrganization
 *
 * This differs from ArchiveOrganization because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ArchiveOrganizationOrSubClass =
  | ArchiveOrganization
  ;

/**
 * schema:ArchiveOrganization - Validation schema (w/ JOI)
 */
export const ArchiveOrganizationJoiSchema = Joi.object({
  '@type': Joi.string().valid('ArchiveOrganization').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  archiveHeld: Joi.alternatives().try(Joi.lazy(() => schema.ArchiveComponentOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ArchiveOrganization - Validation schema (w/ JOI)
 *
 * This differs from ArchiveOrganizationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ArchiveOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
  ArchiveOrganizationJoiSchema,
]);

/**
 * Runtime validator for schema:ArchiveOrganization.
 *
 * If some data has a structure which matches a schema:ArchiveOrganization, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeArchiveOrganization` will have type `ArchiveOrganization`
 * const archiveOrganization = maybeArchiveOrganization;
 * ```
 */
export function validateArchiveOrganization(maybeArchiveOrganization: unknown): ArchiveOrganization | OaValidationError {
  const { value, error } = ArchiveOrganizationJoiSchema.validate(maybeArchiveOrganization);
  if (error) {
    return new OaValidationError('ArchiveOrganization', maybeArchiveOrganization, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ArchiveOrganization;
}
