 ...
 * schema:ArriveAction
 *
 * This differs from ArriveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ArriveActionOrSubClass =
  | ArriveAction
  ;

/**
 * schema:ArriveAction - Validation schema (w/ JOI)
 */
export const ArriveActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ArriveAction').required(),
 ...
 * schema:ArriveAction - Validation schema (w/ JOI)
 *
 * This differs from ArriveActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ArriveActionOrSubClassJoiSchema = Joi.alternatives().try([
  ArriveActionJoiSchema,
]);

/**
 * Runtime validator for schema:ArriveAction.
 *
 * If some data has a structure which matches a schema:ArriveAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeArriveAction = ArriveAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeArriveAction instanceof OaValidationError) {
 *   // From this point on, `maybeArriveAction` will have type `OaValidationError`
 *   const error = maybeArriveAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeArriveAction` will have type `ArriveAction`
 * const arriveAction = maybeArriveAction;
 * ```
 */
export function validateArriveAction(maybeArriveAction: unknown): ArriveAction | OaValidationError {
  const { value, error } = ArriveActionJoiSchema.validate(maybeArriveAction);
  if (error) {
    return new OaValidationError('ArriveAction', maybeArriveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ArriveAction;
}
