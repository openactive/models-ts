 ...
 * schema:ArtGallery
 *
 * This differs from ArtGallery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ArtGalleryOrSubClass =
  | ArtGallery
  ;

/**
 * schema:ArtGallery - Validation schema (w/ JOI)
 */
export const ArtGalleryJoiSchema = Joi.object({
  '@type': Joi.string().valid('ArtGallery').required(),
 ...
 * schema:ArtGallery - Validation schema (w/ JOI)
 *
 * This differs from ArtGalleryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ArtGalleryOrSubClassJoiSchema = Joi.alternatives().try([
  ArtGalleryJoiSchema,
]);

/**
 * Runtime validator for schema:ArtGallery.
 *
 * If some data has a structure which matches a schema:ArtGallery, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeArtGallery = ArtGallery.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeArtGallery instanceof OaValidationError) {
 *   // From this point on, `maybeArtGallery` will have type `OaValidationError`
 *   const error = maybeArtGallery;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeArtGallery` will have type `ArtGallery`
 * const artGallery = maybeArtGallery;
 * ```
 */
export function validateArtGallery(maybeArtGallery: unknown): ArtGallery | OaValidationError {
  const { value, error } = ArtGalleryJoiSchema.validate(maybeArtGallery);
  if (error) {
    return new OaValidationError('ArtGallery', maybeArtGallery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ArtGallery;
}
