...
export type Artery = {
   ...
   */
  arterialBranch?: schema.AnatomicalStructureOrSubClass | string;
  /**
   * The area to which the artery supplies blood.
   */
  supplyTo?: schema.AnatomicalStructureOrSubClass | string;
  /**
 ...
 * schema:Artery
 *
 * This differs from Artery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ArteryOrSubClass =
  | Artery
  ;
 ...
 */
export const ArteryJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ArteryOrSubClassJoiSchema = Joi.alternatives().try([
  ArteryJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeArtery = Artery.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateArtery(maybeArtery: unknown): Artery | OaValidationError {
  const { value, error } = ArteryJoiSchema.validate(maybeArtery);
  if (error) {
    return new OaValidationError('Artery', maybeArtery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Artery;
}
