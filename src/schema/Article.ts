 ...
 * schema:Article
 *
 * This differs from Article because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ArticleOrSubClass =
  | Article
  | schema.ReportOrSubClass
  | schema.ScholarlyArticleOrSubClass
  | schema.AdvertiserContentArticleOrSubClass
  | schema.NewsArticleOrSubClass
  | schema.SatiricalArticleOrSubClass
  | schema.SocialMediaPostingOrSubClass
  | schema.TechArticleOrSubClass
  ;

/**
 * schema:Article - Validation schema (w/ JOI)
 */
 ...
 * schema:Article - Validation schema (w/ JOI)
 *
 * This differs from ArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ArticleOrSubClassJoiSchema = Joi.alternatives().try([
  ArticleJoiSchema,
  Joi.lazy(() => schema.ReportOrSubClassJoiSchema),
  Joi.lazy(() => schema.ScholarlyArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.AdvertiserContentArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.NewsArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.SatiricalArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.SocialMediaPostingOrSubClassJoiSchema),
  Joi.lazy(() => schema.TechArticleOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Article.
 *
 * If some data has a structure which matches a schema:Article, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeArticle = Article.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeArticle instanceof OaValidationError) {
 *   // From this point on, `maybeArticle` will have type `OaValidationError`
 *   const error = maybeArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeArticle` will have type `Article`
 * const article = maybeArticle;
 * ```
 */
export function validateArticle(maybeArticle: unknown): Article | OaValidationError {
  const { value, error } = ArticleJoiSchema.validate(maybeArticle);
  if (error) {
    return new OaValidationError('Article', maybeArticle, error);
  }
