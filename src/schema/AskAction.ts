...
export type AskAction = {
   ...
   */
  question?: schema.QuestionOrSubClass | string;
  /**
 ...
 * schema:AskAction
 *
 * This differs from AskAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AskActionOrSubClass =
  | AskAction
  ;

/**
 * schema:AskAction - Validation schema (w/ JOI)
 */
export const AskActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AskAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  question: Joi.alternatives().try(Joi.lazy(() => schema.QuestionOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:AskAction - Validation schema (w/ JOI)
 *
 * This differs from AskActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AskActionOrSubClassJoiSchema = Joi.alternatives().try([
  AskActionJoiSchema,
]);

/**
 * Runtime validator for schema:AskAction.
 *
 * If some data has a structure which matches a schema:AskAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAskAction = AskAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAskAction instanceof OaValidationError) {
 *   // From this point on, `maybeAskAction` will have type `OaValidationError`
 *   const error = maybeAskAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAskAction` will have type `AskAction`
 * const askAction = maybeAskAction;
 * ```
 */
export function validateAskAction(maybeAskAction: unknown): AskAction | OaValidationError {
  const { value, error } = AskActionJoiSchema.validate(maybeAskAction);
  if (error) {
    return new OaValidationError('AskAction', maybeAskAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AskAction;
}
