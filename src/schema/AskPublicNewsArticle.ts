 ...
 */
export type AskPublicNewsArticle = {
 ...
 * schema:AskPublicNewsArticle
 *
 * This differs from AskPublicNewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AskPublicNewsArticleOrSubClass =
  | AskPublicNewsArticle
  ;

/**
 * schema:AskPublicNewsArticle - Validation schema (w/ JOI)
 */
export const AskPublicNewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('AskPublicNewsArticle').required(),
 ...
 * schema:AskPublicNewsArticle - Validation schema (w/ JOI)
 *
 * This differs from AskPublicNewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AskPublicNewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  AskPublicNewsArticleJoiSchema,
]);

/**
 * Runtime validator for schema:AskPublicNewsArticle.
 *
 * If some data has a structure which matches a schema:AskPublicNewsArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAskPublicNewsArticle = AskPublicNewsArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAskPublicNewsArticle instanceof OaValidationError) {
 *   // From this point on, `maybeAskPublicNewsArticle` will have type `OaValidationError`
 *   const error = maybeAskPublicNewsArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAskPublicNewsArticle` will have type `AskPublicNewsArticle`
 * const askPublicNewsArticle = maybeAskPublicNewsArticle;
 * ```
 */
export function validateAskPublicNewsArticle(maybeAskPublicNewsArticle: unknown): AskPublicNewsArticle | OaValidationError {
  const { value, error } = AskPublicNewsArticleJoiSchema.validate(maybeAskPublicNewsArticle);
  if (error) {
    return new OaValidationError('AskPublicNewsArticle', maybeAskPublicNewsArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AskPublicNewsArticle;
}
