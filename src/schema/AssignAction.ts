 ...
 * schema:AssignAction
 *
 * This differs from AssignAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AssignActionOrSubClass =
  | AssignAction
  ;

/**
 * schema:AssignAction - Validation schema (w/ JOI)
 */
export const AssignActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AssignAction').required(),
 ...
 * schema:AssignAction - Validation schema (w/ JOI)
 *
 * This differs from AssignActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AssignActionOrSubClassJoiSchema = Joi.alternatives().try([
  AssignActionJoiSchema,
]);

/**
 * Runtime validator for schema:AssignAction.
 *
 * If some data has a structure which matches a schema:AssignAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAssignAction = AssignAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAssignAction instanceof OaValidationError) {
 *   // From this point on, `maybeAssignAction` will have type `OaValidationError`
 *   const error = maybeAssignAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAssignAction` will have type `AssignAction`
 * const assignAction = maybeAssignAction;
 * ```
 */
export function validateAssignAction(maybeAssignAction: unknown): AssignAction | OaValidationError {
  const { value, error } = AssignActionJoiSchema.validate(maybeAssignAction);
  if (error) {
    return new OaValidationError('AssignAction', maybeAssignAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AssignAction;
}
