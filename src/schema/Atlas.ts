 ...
 * schema:Atlas
 *
 * This differs from Atlas because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AtlasOrSubClass =
  | Atlas
  ;
 ...
 */
export const AtlasJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const AtlasOrSubClassJoiSchema = Joi.alternatives().try([
  AtlasJoiSchema,
]);
 ...
 * ```
 */
export function validateAtlas(maybeAtlas: unknown): Atlas | OaValidationError {
  const { value, error } = AtlasJoiSchema.validate(maybeAtlas);
  if (error) {
    return new OaValidationError('Atlas', maybeAtlas, error);
  }
