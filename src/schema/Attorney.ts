 ...
 * schema:Attorney
 *
 * This differs from Attorney because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AttorneyOrSubClass =
  | Attorney
  ;

/**
 * schema:Attorney - Validation schema (w/ JOI)
 */
export const AttorneyJoiSchema = Joi.object({
  '@type': Joi.string().valid('Attorney').required(),
 ...
 * schema:Attorney - Validation schema (w/ JOI)
 *
 * This differs from AttorneyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AttorneyOrSubClassJoiSchema = Joi.alternatives().try([
  AttorneyJoiSchema,
]);

/**
 * Runtime validator for schema:Attorney.
 *
 * If some data has a structure which matches a schema:Attorney, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAttorney = Attorney.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAttorney instanceof OaValidationError) {
 *   // From this point on, `maybeAttorney` will have type `OaValidationError`
 *   const error = maybeAttorney;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAttorney` will have type `Attorney`
 * const attorney = maybeAttorney;
 * ```
 */
export function validateAttorney(maybeAttorney: unknown): Attorney | OaValidationError {
  const { value, error } = AttorneyJoiSchema.validate(maybeAttorney);
  if (error) {
    return new OaValidationError('Attorney', maybeAttorney, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Attorney;
}
