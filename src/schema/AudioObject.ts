 ...
 * schema:AudioObject
 *
 * This differs from AudioObject because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AudioObjectOrSubClass =
  | AudioObject
  | schema.AudiobookOrSubClass
  | schema.AudioObjectSnapshotOrSubClass
  ;

/**
 * schema:AudioObject - Validation schema (w/ JOI)
 */
export const AudioObjectJoiSchema = Joi.object({
  '@type': Joi.string().valid('AudioObject').required(),
 ...
 * schema:AudioObject - Validation schema (w/ JOI)
 *
 * This differs from AudioObjectJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AudioObjectOrSubClassJoiSchema = Joi.alternatives().try([
  AudioObjectJoiSchema,
  Joi.lazy(() => schema.AudiobookOrSubClassJoiSchema),
  Joi.lazy(() => schema.AudioObjectSnapshotOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:AudioObject.
 *
 * If some data has a structure which matches a schema:AudioObject, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAudioObject = validateAudioObject(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAudioObject instanceof OaValidationError) {
 *   // From this point on, `maybeAudioObject` will have type `OaValidationError`
 *   const error = maybeAudioObject;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAudioObject` will have type `AudioObject`
 * const audioObject = maybeAudioObject;
 * ```
 */
export function validateAudioObject(maybeAudioObject: unknown): AudioObject | OaValidationError {
  const { value, error } = AudioObjectJoiSchema.validate(maybeAudioObject);
  if (error) {
    return new OaValidationError('AudioObject', maybeAudioObject, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AudioObject;
}
