 ...
 */
export type AudioObjectSnapshot = {
 ...
 * schema:AudioObjectSnapshot
 *
 * This differs from AudioObjectSnapshot because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AudioObjectSnapshotOrSubClass =
  | AudioObjectSnapshot
  ;

/**
 * schema:AudioObjectSnapshot - Validation schema (w/ JOI)
 */
export const AudioObjectSnapshotJoiSchema = Joi.object({
  '@type': Joi.string().valid('AudioObjectSnapshot').required(),
 ...
 * schema:AudioObjectSnapshot - Validation schema (w/ JOI)
 *
 * This differs from AudioObjectSnapshotJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AudioObjectSnapshotOrSubClassJoiSchema = Joi.alternatives().try([
  AudioObjectSnapshotJoiSchema,
]);

/**
 * Runtime validator for schema:AudioObjectSnapshot.
 *
 * If some data has a structure which matches a schema:AudioObjectSnapshot, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAudioObjectSnapshot = validateAudioObjectSnapshot(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAudioObjectSnapshot instanceof OaValidationError) {
 *   // From this point on, `maybeAudioObjectSnapshot` will have type `OaValidationError`
 *   const error = maybeAudioObjectSnapshot;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAudioObjectSnapshot` will have type `AudioObjectSnapshot`
 * const audioObjectSnapshot = maybeAudioObjectSnapshot;
 * ```
 */
export function validateAudioObjectSnapshot(maybeAudioObjectSnapshot: unknown): AudioObjectSnapshot | OaValidationError {
  const { value, error } = AudioObjectSnapshotJoiSchema.validate(maybeAudioObjectSnapshot);
  if (error) {
    return new OaValidationError('AudioObjectSnapshot', maybeAudioObjectSnapshot, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AudioObjectSnapshot;
}
