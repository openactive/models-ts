...
export type Audiobook = {
   ...
   */
  readBy?: schema.PersonOrSubClass | string;
  /**
   ...
   */
  illustrator?: schema.PersonOrSubClass | string;
  /**
 ...
 * schema:Audiobook
 *
 * This differs from Audiobook because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AudiobookOrSubClass =
  | Audiobook
  ;

/**
 * schema:Audiobook - Validation schema (w/ JOI)
 */
export const AudiobookJoiSchema = Joi.object({
  '@type': Joi.string().valid('Audiobook').required(),
  ...
  readBy: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  ...
  illustrator: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  abridged: Joi.boolean(),
  isbn: Joi.string(),
  numberOfPages: Joi.number().integer(),
  bookFormat: Joi.lazy(() => schema.BookFormatTypeJoiSchema),
 ...
 * schema:Audiobook - Validation schema (w/ JOI)
 *
 * This differs from AudiobookJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AudiobookOrSubClassJoiSchema = Joi.alternatives().try([
  AudiobookJoiSchema,
]);

/**
 * Runtime validator for schema:Audiobook.
 *
 * If some data has a structure which matches a schema:Audiobook, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAudiobook = validateAudiobook(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAudiobook instanceof OaValidationError) {
 *   // From this point on, `maybeAudiobook` will have type `OaValidationError`
 *   const error = maybeAudiobook;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAudiobook` will have type `Audiobook`
 * const audiobook = maybeAudiobook;
 * ```
 */
export function validateAudiobook(maybeAudiobook: unknown): Audiobook | OaValidationError {
  const { value, error } = AudiobookJoiSchema.validate(maybeAudiobook);
  if (error) {
    return new OaValidationError('Audiobook', maybeAudiobook, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Audiobook;
}
