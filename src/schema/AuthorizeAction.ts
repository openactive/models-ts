 ...
 * schema:AuthorizeAction
 *
 * This differs from AuthorizeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AuthorizeActionOrSubClass =
  | AuthorizeAction
  ;

/**
 * schema:AuthorizeAction - Validation schema (w/ JOI)
 */
export const AuthorizeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('AuthorizeAction').required(),
 ...
 * schema:AuthorizeAction - Validation schema (w/ JOI)
 *
 * This differs from AuthorizeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AuthorizeActionOrSubClassJoiSchema = Joi.alternatives().try([
  AuthorizeActionJoiSchema,
]);

/**
 * Runtime validator for schema:AuthorizeAction.
 *
 * If some data has a structure which matches a schema:AuthorizeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAuthorizeAction = AuthorizeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAuthorizeAction instanceof OaValidationError) {
 *   // From this point on, `maybeAuthorizeAction` will have type `OaValidationError`
 *   const error = maybeAuthorizeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAuthorizeAction` will have type `AuthorizeAction`
 * const authorizeAction = maybeAuthorizeAction;
 * ```
 */
export function validateAuthorizeAction(maybeAuthorizeAction: unknown): AuthorizeAction | OaValidationError {
  const { value, error } = AuthorizeActionJoiSchema.validate(maybeAuthorizeAction);
  if (error) {
    return new OaValidationError('AuthorizeAction', maybeAuthorizeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AuthorizeAction;
}
