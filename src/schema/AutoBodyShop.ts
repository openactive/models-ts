 ...
 * schema:AutoBodyShop
 *
 * This differs from AutoBodyShop because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutoBodyShopOrSubClass =
  | AutoBodyShop
  ;

/**
 * schema:AutoBodyShop - Validation schema (w/ JOI)
 */
export const AutoBodyShopJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutoBodyShop').required(),
 ...
 * schema:AutoBodyShop - Validation schema (w/ JOI)
 *
 * This differs from AutoBodyShopJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutoBodyShopOrSubClassJoiSchema = Joi.alternatives().try([
  AutoBodyShopJoiSchema,
]);

/**
 * Runtime validator for schema:AutoBodyShop.
 *
 * If some data has a structure which matches a schema:AutoBodyShop, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAutoBodyShop = AutoBodyShop.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAutoBodyShop instanceof OaValidationError) {
 *   // From this point on, `maybeAutoBodyShop` will have type `OaValidationError`
 *   const error = maybeAutoBodyShop;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAutoBodyShop` will have type `AutoBodyShop`
 * const autoBodyShop = maybeAutoBodyShop;
 * ```
 */
export function validateAutoBodyShop(maybeAutoBodyShop: unknown): AutoBodyShop | OaValidationError {
  const { value, error } = AutoBodyShopJoiSchema.validate(maybeAutoBodyShop);
  if (error) {
    return new OaValidationError('AutoBodyShop', maybeAutoBodyShop, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutoBodyShop;
}
