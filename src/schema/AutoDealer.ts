 ...
 * schema:AutoDealer
 *
 * This differs from AutoDealer because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutoDealerOrSubClass =
  | AutoDealer
  ;

/**
 * schema:AutoDealer - Validation schema (w/ JOI)
 */
export const AutoDealerJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutoDealer').required(),
 ...
 * schema:AutoDealer - Validation schema (w/ JOI)
 *
 * This differs from AutoDealerJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutoDealerOrSubClassJoiSchema = Joi.alternatives().try([
  AutoDealerJoiSchema,
]);

/**
 * Runtime validator for schema:AutoDealer.
 *
 * If some data has a structure which matches a schema:AutoDealer, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAutoDealer = AutoDealer.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAutoDealer instanceof OaValidationError) {
 *   // From this point on, `maybeAutoDealer` will have type `OaValidationError`
 *   const error = maybeAutoDealer;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAutoDealer` will have type `AutoDealer`
 * const autoDealer = maybeAutoDealer;
 * ```
 */
export function validateAutoDealer(maybeAutoDealer: unknown): AutoDealer | OaValidationError {
  const { value, error } = AutoDealerJoiSchema.validate(maybeAutoDealer);
  if (error) {
    return new OaValidationError('AutoDealer', maybeAutoDealer, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutoDealer;
}
