 ...
 * schema:AutoPartsStore
 *
 * This differs from AutoPartsStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutoPartsStoreOrSubClass =
  | AutoPartsStore
  ;

/**
 * schema:AutoPartsStore - Validation schema (w/ JOI)
 */
export const AutoPartsStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutoPartsStore').required(),
 ...
 * schema:AutoPartsStore - Validation schema (w/ JOI)
 *
 * This differs from AutoPartsStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutoPartsStoreOrSubClassJoiSchema = Joi.alternatives().try([
  AutoPartsStoreJoiSchema,
]);

/**
 * Runtime validator for schema:AutoPartsStore.
 *
 * If some data has a structure which matches a schema:AutoPartsStore, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAutoPartsStore` will have type `AutoPartsStore`
 * const autoPartsStore = maybeAutoPartsStore;
 * ```
 */
export function validateAutoPartsStore(maybeAutoPartsStore: unknown): AutoPartsStore | OaValidationError {
  const { value, error } = AutoPartsStoreJoiSchema.validate(maybeAutoPartsStore);
  if (error) {
    return new OaValidationError('AutoPartsStore', maybeAutoPartsStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutoPartsStore;
}
