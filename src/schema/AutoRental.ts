 ...
 * schema:AutoRental
 *
 * This differs from AutoRental because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutoRentalOrSubClass =
  | AutoRental
  ;

/**
 * schema:AutoRental - Validation schema (w/ JOI)
 */
export const AutoRentalJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutoRental').required(),
 ...
 * schema:AutoRental - Validation schema (w/ JOI)
 *
 * This differs from AutoRentalJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutoRentalOrSubClassJoiSchema = Joi.alternatives().try([
  AutoRentalJoiSchema,
]);

/**
 * Runtime validator for schema:AutoRental.
 *
 * If some data has a structure which matches a schema:AutoRental, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAutoRental` will have type `AutoRental`
 * const autoRental = maybeAutoRental;
 * ```
 */
export function validateAutoRental(maybeAutoRental: unknown): AutoRental | OaValidationError {
  const { value, error } = AutoRentalJoiSchema.validate(maybeAutoRental);
  if (error) {
    return new OaValidationError('AutoRental', maybeAutoRental, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutoRental;
}
