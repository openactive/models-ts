 ...
 * schema:AutoRepair
 *
 * This differs from AutoRepair because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutoRepairOrSubClass =
  | AutoRepair
  ;

/**
 * schema:AutoRepair - Validation schema (w/ JOI)
 */
export const AutoRepairJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutoRepair').required(),
 ...
 * schema:AutoRepair - Validation schema (w/ JOI)
 *
 * This differs from AutoRepairJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutoRepairOrSubClassJoiSchema = Joi.alternatives().try([
  AutoRepairJoiSchema,
]);

/**
 * Runtime validator for schema:AutoRepair.
 *
 * If some data has a structure which matches a schema:AutoRepair, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAutoRepair = AutoRepair.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAutoRepair instanceof OaValidationError) {
 *   // From this point on, `maybeAutoRepair` will have type `OaValidationError`
 *   const error = maybeAutoRepair;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAutoRepair` will have type `AutoRepair`
 * const autoRepair = maybeAutoRepair;
 * ```
 */
export function validateAutoRepair(maybeAutoRepair: unknown): AutoRepair | OaValidationError {
  const { value, error } = AutoRepairJoiSchema.validate(maybeAutoRepair);
  if (error) {
    return new OaValidationError('AutoRepair', maybeAutoRepair, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutoRepair;
}
