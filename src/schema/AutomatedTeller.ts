 ...
 * schema:AutomatedTeller
 *
 * This differs from AutomatedTeller because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutomatedTellerOrSubClass =
  | AutomatedTeller
  ;

/**
 * schema:AutomatedTeller - Validation schema (w/ JOI)
 */
export const AutomatedTellerJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutomatedTeller').required(),
 ...
 * schema:AutomatedTeller - Validation schema (w/ JOI)
 *
 * This differs from AutomatedTellerJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutomatedTellerOrSubClassJoiSchema = Joi.alternatives().try([
  AutomatedTellerJoiSchema,
]);

/**
 * Runtime validator for schema:AutomatedTeller.
 *
 * If some data has a structure which matches a schema:AutomatedTeller, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeAutomatedTeller` will have type `AutomatedTeller`
 * const automatedTeller = maybeAutomatedTeller;
 * ```
 */
export function validateAutomatedTeller(maybeAutomatedTeller: unknown): AutomatedTeller | OaValidationError {
  const { value, error } = AutomatedTellerJoiSchema.validate(maybeAutomatedTeller);
  if (error) {
    return new OaValidationError('AutomatedTeller', maybeAutomatedTeller, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutomatedTeller;
}
