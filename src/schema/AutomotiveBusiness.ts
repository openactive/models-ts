 ...
 * schema:AutomotiveBusiness
 *
 * This differs from AutomotiveBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type AutomotiveBusinessOrSubClass =
  | AutomotiveBusiness
  | schema.AutoPartsStoreOrSubClass
  | schema.AutoRentalOrSubClass
  | schema.AutoDealerOrSubClass
  | schema.GasStationOrSubClass
  | schema.AutoWashOrSubClass
  | schema.MotorcycleRepairOrSubClass
  | schema.AutoRepairOrSubClass
  | schema.MotorcycleDealerOrSubClass
  | schema.AutoBodyShopOrSubClass
  ;

/**
 * schema:AutomotiveBusiness - Validation schema (w/ JOI)
 */
export const AutomotiveBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('AutomotiveBusiness').required(),
 ...
 * schema:AutomotiveBusiness - Validation schema (w/ JOI)
 *
 * This differs from AutomotiveBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const AutomotiveBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  AutomotiveBusinessJoiSchema,
  Joi.lazy(() => schema.AutoPartsStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutoRentalOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutoDealerOrSubClassJoiSchema),
  Joi.lazy(() => schema.GasStationOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutoWashOrSubClassJoiSchema),
  Joi.lazy(() => schema.MotorcycleRepairOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutoRepairOrSubClassJoiSchema),
  Joi.lazy(() => schema.MotorcycleDealerOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutoBodyShopOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:AutomotiveBusiness.
 *
 * If some data has a structure which matches a schema:AutomotiveBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeAutomotiveBusiness = AutomotiveBusiness.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeAutomotiveBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeAutomotiveBusiness` will have type `OaValidationError`
 *   const error = maybeAutomotiveBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeAutomotiveBusiness` will have type `AutomotiveBusiness`
 * const automotiveBusiness = maybeAutomotiveBusiness;
 * ```
 */
export function validateAutomotiveBusiness(maybeAutomotiveBusiness: unknown): AutomotiveBusiness | OaValidationError {
  const { value, error } = AutomotiveBusinessJoiSchema.validate(maybeAutomotiveBusiness);
  if (error) {
    return new OaValidationError('AutomotiveBusiness', maybeAutomotiveBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as AutomotiveBusiness;
}
