 ...
 */
export type BackgroundNewsArticle = {
 ...
 * schema:BackgroundNewsArticle
 *
 * This differs from BackgroundNewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BackgroundNewsArticleOrSubClass =
  | BackgroundNewsArticle
  ;

/**
 * schema:BackgroundNewsArticle - Validation schema (w/ JOI)
 */
export const BackgroundNewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('BackgroundNewsArticle').required(),
 ...
 * schema:BackgroundNewsArticle - Validation schema (w/ JOI)
 *
 * This differs from BackgroundNewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BackgroundNewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  BackgroundNewsArticleJoiSchema,
]);

/**
 * Runtime validator for schema:BackgroundNewsArticle.
 *
 * If some data has a structure which matches a schema:BackgroundNewsArticle, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeBackgroundNewsArticle` will have type `BackgroundNewsArticle`
 * const backgroundNewsArticle = maybeBackgroundNewsArticle;
 * ```
 */
export function validateBackgroundNewsArticle(maybeBackgroundNewsArticle: unknown): BackgroundNewsArticle | OaValidationError {
  const { value, error } = BackgroundNewsArticleJoiSchema.validate(maybeBackgroundNewsArticle);
  if (error) {
    return new OaValidationError('BackgroundNewsArticle', maybeBackgroundNewsArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BackgroundNewsArticle;
}
