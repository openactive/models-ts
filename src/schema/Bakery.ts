 ...
 * schema:Bakery
 *
 * This differs from Bakery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BakeryOrSubClass =
  | Bakery
  ;

/**
 * schema:Bakery - Validation schema (w/ JOI)
 */
export const BakeryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Bakery').required(),
 ...
 * schema:Bakery - Validation schema (w/ JOI)
 *
 * This differs from BakeryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BakeryOrSubClassJoiSchema = Joi.alternatives().try([
  BakeryJoiSchema,
]);

/**
 * Runtime validator for schema:Bakery.
 *
 * If some data has a structure which matches a schema:Bakery, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBakery = Bakery.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBakery instanceof OaValidationError) {
 *   // From this point on, `maybeBakery` will have type `OaValidationError`
 *   const error = maybeBakery;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBakery` will have type `Bakery`
 * const bakery = maybeBakery;
 * ```
 */
export function validateBakery(maybeBakery: unknown): Bakery | OaValidationError {
  const { value, error } = BakeryJoiSchema.validate(maybeBakery);
  if (error) {
    return new OaValidationError('Bakery', maybeBakery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Bakery;
}
