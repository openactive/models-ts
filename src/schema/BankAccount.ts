...
export type BankAccount = {
   ...
   */
  accountMinimumInflow?: schema.MonetaryAmountOrSubClass | string;
  /**
   * An overdraft is an extension of credit from a lending institution when an account reaches zero. An overdraft allows the individual to continue withdrawing money even if the account has no funds in it. Basically the bank allows people to borrow a set amount of money.
   */
  accountOverdraftLimit?: schema.MonetaryAmountOrSubClass | string;
  /**
 ...
 * schema:BankAccount
 *
 * This differs from BankAccount because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BankAccountOrSubClass =
  | BankAccount
 ...
 */
export const BankAccountJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BankAccountOrSubClassJoiSchema = Joi.alternatives().try([
  BankAccountJoiSchema,
 ...
 * ```
 */
export function validateBankAccount(maybeBankAccount: unknown): BankAccount | OaValidationError {
  const { value, error } = BankAccountJoiSchema.validate(maybeBankAccount);
  if (error) {
    return new OaValidationError('BankAccount', maybeBankAccount, error);
  }
