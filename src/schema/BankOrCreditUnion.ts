 ...
 * schema:BankOrCreditUnion
 *
 * This differs from BankOrCreditUnion because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BankOrCreditUnionOrSubClass =
  | BankOrCreditUnion
  ;

/**
 * schema:BankOrCreditUnion - Validation schema (w/ JOI)
 */
export const BankOrCreditUnionJoiSchema = Joi.object({
  '@type': Joi.string().valid('BankOrCreditUnion').required(),
 ...
 * schema:BankOrCreditUnion - Validation schema (w/ JOI)
 *
 * This differs from BankOrCreditUnionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BankOrCreditUnionOrSubClassJoiSchema = Joi.alternatives().try([
  BankOrCreditUnionJoiSchema,
]);

/**
 * Runtime validator for schema:BankOrCreditUnion.
 *
 * If some data has a structure which matches a schema:BankOrCreditUnion, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBankOrCreditUnion = BankOrCreditUnion.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBankOrCreditUnion instanceof OaValidationError) {
 *   // From this point on, `maybeBankOrCreditUnion` will have type `OaValidationError`
 *   const error = maybeBankOrCreditUnion;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBankOrCreditUnion` will have type `BankOrCreditUnion`
 * const bankOrCreditUnion = maybeBankOrCreditUnion;
 * ```
 */
export function validateBankOrCreditUnion(maybeBankOrCreditUnion: unknown): BankOrCreditUnion | OaValidationError {
  const { value, error } = BankOrCreditUnionJoiSchema.validate(maybeBankOrCreditUnion);
  if (error) {
    return new OaValidationError('BankOrCreditUnion', maybeBankOrCreditUnion, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BankOrCreditUnion;
}
