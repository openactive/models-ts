 ...
 * schema:BarOrPub
 *
 * This differs from BarOrPub because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BarOrPubOrSubClass =
  | BarOrPub
  ;

/**
 * schema:BarOrPub - Validation schema (w/ JOI)
 */
export const BarOrPubJoiSchema = Joi.object({
  '@type': Joi.string().valid('BarOrPub').required(),
 ...
 * schema:BarOrPub - Validation schema (w/ JOI)
 *
 * This differs from BarOrPubJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BarOrPubOrSubClassJoiSchema = Joi.alternatives().try([
  BarOrPubJoiSchema,
]);

/**
 * Runtime validator for schema:BarOrPub.
 *
 * If some data has a structure which matches a schema:BarOrPub, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBarOrPub = BarOrPub.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBarOrPub instanceof OaValidationError) {
 *   // From this point on, `maybeBarOrPub` will have type `OaValidationError`
 *   const error = maybeBarOrPub;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBarOrPub` will have type `BarOrPub`
 * const barOrPub = maybeBarOrPub;
 * ```
 */
export function validateBarOrPub(maybeBarOrPub: unknown): BarOrPub | OaValidationError {
  const { value, error } = BarOrPubJoiSchema.validate(maybeBarOrPub);
  if (error) {
    return new OaValidationError('BarOrPub', maybeBarOrPub, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BarOrPub;
}
