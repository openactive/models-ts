 ...
 * schema:Barcode
 *
 * This differs from Barcode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BarcodeOrSubClass =
  | Barcode
  ;

/**
 * schema:Barcode - Validation schema (w/ JOI)
 */
export const BarcodeJoiSchema = Joi.object({
 ...
 * schema:Barcode - Validation schema (w/ JOI)
 *
 * This differs from BarcodeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BarcodeOrSubClassJoiSchema = Joi.alternatives().try([
  BarcodeJoiSchema,
]);

/**
 * Runtime validator for schema:Barcode.
 *
 * If some data has a structure which matches a schema:Barcode, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBarcode = validateBarcode(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBarcode instanceof OaValidationError) {
 *   // From this point on, `maybeBarcode` will have type `OaValidationError`
 *   const error = maybeBarcode;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBarcode` will have type `Barcode`
 * const barcode = maybeBarcode;
 * ```
 */
export function validateBarcode(maybeBarcode: unknown): Barcode | OaValidationError {
  const { value, error } = BarcodeJoiSchema.validate(maybeBarcode);
  if (error) {
    return new OaValidationError('Barcode', maybeBarcode, error);
  }
