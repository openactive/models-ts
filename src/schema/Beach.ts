 ...
 * schema:Beach
 *
 * This differs from Beach because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BeachOrSubClass =
  | Beach
  ;

/**
 * schema:Beach - Validation schema (w/ JOI)
 */
export const BeachJoiSchema = Joi.object({
  '@type': Joi.string().valid('Beach').required(),
 ...
 * schema:Beach - Validation schema (w/ JOI)
 *
 * This differs from BeachJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BeachOrSubClassJoiSchema = Joi.alternatives().try([
  BeachJoiSchema,
]);

/**
 * Runtime validator for schema:Beach.
 *
 * If some data has a structure which matches a schema:Beach, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBeach = Beach.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBeach instanceof OaValidationError) {
 *   // From this point on, `maybeBeach` will have type `OaValidationError`
 *   const error = maybeBeach;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBeach` will have type `Beach`
 * const beach = maybeBeach;
 * ```
 */
export function validateBeach(maybeBeach: unknown): Beach | OaValidationError {
  const { value, error } = BeachJoiSchema.validate(maybeBeach);
  if (error) {
    return new OaValidationError('Beach', maybeBeach, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Beach;
}
