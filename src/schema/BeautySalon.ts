 ...
 * schema:BeautySalon
 *
 * This differs from BeautySalon because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BeautySalonOrSubClass =
  | BeautySalon
  ;

/**
 * schema:BeautySalon - Validation schema (w/ JOI)
 */
export const BeautySalonJoiSchema = Joi.object({
  '@type': Joi.string().valid('BeautySalon').required(),
 ...
 * schema:BeautySalon - Validation schema (w/ JOI)
 *
 * This differs from BeautySalonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BeautySalonOrSubClassJoiSchema = Joi.alternatives().try([
  BeautySalonJoiSchema,
]);

/**
 * Runtime validator for schema:BeautySalon.
 *
 * If some data has a structure which matches a schema:BeautySalon, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBeautySalon = BeautySalon.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBeautySalon instanceof OaValidationError) {
 *   // From this point on, `maybeBeautySalon` will have type `OaValidationError`
 *   const error = maybeBeautySalon;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBeautySalon` will have type `BeautySalon`
 * const beautySalon = maybeBeautySalon;
 * ```
 */
export function validateBeautySalon(maybeBeautySalon: unknown): BeautySalon | OaValidationError {
  const { value, error } = BeautySalonJoiSchema.validate(maybeBeautySalon);
  if (error) {
    return new OaValidationError('BeautySalon', maybeBeautySalon, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BeautySalon;
}
