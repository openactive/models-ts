 ...
 * schema:BedAndBreakfast
 *
 * This differs from BedAndBreakfast because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BedAndBreakfastOrSubClass =
  | BedAndBreakfast
  ;

/**
 * schema:BedAndBreakfast - Validation schema (w/ JOI)
 */
export const BedAndBreakfastJoiSchema = Joi.object({
  '@type': Joi.string().valid('BedAndBreakfast').required(),
 ...
 * schema:BedAndBreakfast - Validation schema (w/ JOI)
 *
 * This differs from BedAndBreakfastJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BedAndBreakfastOrSubClassJoiSchema = Joi.alternatives().try([
  BedAndBreakfastJoiSchema,
]);

/**
 * Runtime validator for schema:BedAndBreakfast.
 *
 * If some data has a structure which matches a schema:BedAndBreakfast, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBedAndBreakfast = validateBedAndBreakfast(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBedAndBreakfast instanceof OaValidationError) {
 *   // From this point on, `maybeBedAndBreakfast` will have type `OaValidationError`
 *   const error = maybeBedAndBreakfast;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBedAndBreakfast` will have type `BedAndBreakfast`
 * const bedAndBreakfast = maybeBedAndBreakfast;
 * ```
 */
export function validateBedAndBreakfast(maybeBedAndBreakfast: unknown): BedAndBreakfast | OaValidationError {
  const { value, error } = BedAndBreakfastJoiSchema.validate(maybeBedAndBreakfast);
  if (error) {
    return new OaValidationError('BedAndBreakfast', maybeBedAndBreakfast, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BedAndBreakfast;
}
