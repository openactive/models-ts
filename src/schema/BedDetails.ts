 ...
 * etc.
 */
export type BedDetailsOrSubClass =
  | BedDetails
  ;
 ...
 */
export const BedDetailsJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BedDetailsOrSubClassJoiSchema = Joi.alternatives().try([
  BedDetailsJoiSchema,
]);
 ...
 * ```
 */
export function validateBedDetails(maybeBedDetails: unknown): BedDetails | OaValidationError {
  const { value, error } = BedDetailsJoiSchema.validate(maybeBedDetails);
  if (error) {
    return new OaValidationError('BedDetails', maybeBedDetails, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BedDetails;
}
