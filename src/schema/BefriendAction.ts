 ...
 * schema:BefriendAction
 *
 * This differs from BefriendAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BefriendActionOrSubClass =
  | BefriendAction
  ;
 ...
 */
export const BefriendActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BefriendActionOrSubClassJoiSchema = Joi.alternatives().try([
  BefriendActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeBefriendAction = BefriendAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBefriendAction(maybeBefriendAction: unknown): BefriendAction | OaValidationError {
  const { value, error } = BefriendActionJoiSchema.validate(maybeBefriendAction);
  if (error) {
    return new OaValidationError('BefriendAction', maybeBefriendAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BefriendAction;
}
