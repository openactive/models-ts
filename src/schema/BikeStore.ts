 ...
 * schema:BikeStore
 *
 * This differs from BikeStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BikeStoreOrSubClass =
  | BikeStore
  ;

/**
 * schema:BikeStore - Validation schema (w/ JOI)
 */
export const BikeStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('BikeStore').required(),
 ...
 * schema:BikeStore - Validation schema (w/ JOI)
 *
 * This differs from BikeStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BikeStoreOrSubClassJoiSchema = Joi.alternatives().try([
  BikeStoreJoiSchema,
]);

/**
 * Runtime validator for schema:BikeStore.
 *
 * If some data has a structure which matches a schema:BikeStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBikeStore = BikeStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBikeStore instanceof OaValidationError) {
 *   // From this point on, `maybeBikeStore` will have type `OaValidationError`
 *   const error = maybeBikeStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBikeStore` will have type `BikeStore`
 * const bikeStore = maybeBikeStore;
 * ```
 */
export function validateBikeStore(maybeBikeStore: unknown): BikeStore | OaValidationError {
  const { value, error } = BikeStoreJoiSchema.validate(maybeBikeStore);
  if (error) {
    return new OaValidationError('BikeStore', maybeBikeStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BikeStore;
}
