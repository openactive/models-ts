 ...
 * schema:BioChemEntity
 *
 * This differs from BioChemEntity because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BioChemEntityOrSubClass =
  | BioChemEntity
  | schema.MolecularEntityOrSubClass
  | schema.ProteinOrSubClass
  | schema.ChemicalSubstanceOrSubClass
  | schema.GeneOrSubClass
  ;

/**
 * schema:BioChemEntity - Validation schema (w/ JOI)
 */
export const BioChemEntityJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BioChemEntityOrSubClassJoiSchema = Joi.alternatives().try([
  BioChemEntityJoiSchema,
 ...
 * ```
 */
export function validateBioChemEntity(maybeBioChemEntity: unknown): BioChemEntity | OaValidationError {
  const { value, error } = BioChemEntityJoiSchema.validate(maybeBioChemEntity);
  if (error) {
    return new OaValidationError('BioChemEntity', maybeBioChemEntity, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BioChemEntity;
}
