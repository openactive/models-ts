...
export type Blog = {
   ...
   */
  blogPosts?: schema.BlogPostingOrSubClass | string;
  /**
   * A posting that is part of this blog.
   */
  blogPost?: schema.BlogPostingOrSubClass | string;
  /**
 ...
 * schema:Blog
 *
 * This differs from Blog because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BlogOrSubClass =
  | Blog
  ;

/**
 * schema:Blog - Validation schema (w/ JOI)
 */
export const BlogJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  issn: Joi.string(),
  blogPosts: Joi.alternatives().try(Joi.lazy(() => schema.BlogPostingOrSubClassJoiSchema), Joi.string().uri()),
  blogPost: Joi.alternatives().try(Joi.lazy(() => schema.BlogPostingOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:Blog - Validation schema (w/ JOI)
 *
 * This differs from BlogJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BlogOrSubClassJoiSchema = Joi.alternatives().try([
  BlogJoiSchema,
]);

/**
 * Runtime validator for schema:Blog.
 *
 * If some data has a structure which matches a schema:Blog, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBlog = Blog.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBlog instanceof OaValidationError) {
 *   // From this point on, `maybeBlog` will have type `OaValidationError`
 *   const error = maybeBlog;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBlog` will have type `Blog`
 * const blog = maybeBlog;
 * ```
 */
export function validateBlog(maybeBlog: unknown): Blog | OaValidationError {
  const { value, error } = BlogJoiSchema.validate(maybeBlog);
  if (error) {
    return new OaValidationError('Blog', maybeBlog, error);
  }
