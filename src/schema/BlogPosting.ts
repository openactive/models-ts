 ...
 * schema:BlogPosting
 *
 * This differs from BlogPosting because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BlogPostingOrSubClass =
  | BlogPosting
  | schema.LiveBlogPostingOrSubClass
  ;

/**
 * schema:BlogPosting - Validation schema (w/ JOI)
 */
export const BlogPostingJoiSchema = Joi.object({
  '@type': Joi.string().valid('BlogPosting').required(),
 ...
 * schema:BlogPosting - Validation schema (w/ JOI)
 *
 * This differs from BlogPostingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BlogPostingOrSubClassJoiSchema = Joi.alternatives().try([
  BlogPostingJoiSchema,
  Joi.lazy(() => schema.LiveBlogPostingOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:BlogPosting.
 *
 * If some data has a structure which matches a schema:BlogPosting, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBlogPosting = BlogPosting.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBlogPosting instanceof OaValidationError) {
 *   // From this point on, `maybeBlogPosting` will have type `OaValidationError`
 *   const error = maybeBlogPosting;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBlogPosting` will have type `BlogPosting`
 * const blogPosting = maybeBlogPosting;
 * ```
 */
export function validateBlogPosting(maybeBlogPosting: unknown): BlogPosting | OaValidationError {
  const { value, error } = BlogPostingJoiSchema.validate(maybeBlogPosting);
  if (error) {
    return new OaValidationError('BlogPosting', maybeBlogPosting, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BlogPosting;
}
