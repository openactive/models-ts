 ...
 * schema:BloodTest
 *
 * This differs from BloodTest because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BloodTestOrSubClass =
  | BloodTest
  ;
 ...
 */
export const BloodTestJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BloodTestOrSubClassJoiSchema = Joi.alternatives().try([
  BloodTestJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeBloodTest = BloodTest.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBloodTest(maybeBloodTest: unknown): BloodTest | OaValidationError {
  const { value, error } = BloodTestJoiSchema.validate(maybeBloodTest);
  if (error) {
    return new OaValidationError('BloodTest', maybeBloodTest, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BloodTest;
}
