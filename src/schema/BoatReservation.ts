 ...
 * schema:BoatReservation
 *
 * This differs from BoatReservation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BoatReservationOrSubClass =
  | BoatReservation
  ;
 ...
 */
export const BoatReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BoatReservationOrSubClassJoiSchema = Joi.alternatives().try([
  BoatReservationJoiSchema,
]);
 ...
 * ```
 */
export function validateBoatReservation(maybeBoatReservation: unknown): BoatReservation | OaValidationError {
  const { value, error } = BoatReservationJoiSchema.validate(maybeBoatReservation);
  if (error) {
    return new OaValidationError('BoatReservation', maybeBoatReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BoatReservation;
}
