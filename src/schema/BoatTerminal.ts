 ...
 * schema:BoatTerminal
 *
 * This differs from BoatTerminal because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BoatTerminalOrSubClass =
  | BoatTerminal
  ;

/**
 * schema:BoatTerminal - Validation schema (w/ JOI)
 */
export const BoatTerminalJoiSchema = Joi.object({
  '@type': Joi.string().valid('BoatTerminal').required(),
 ...
 * schema:BoatTerminal - Validation schema (w/ JOI)
 *
 * This differs from BoatTerminalJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BoatTerminalOrSubClassJoiSchema = Joi.alternatives().try([
  BoatTerminalJoiSchema,
]);

/**
 * Runtime validator for schema:BoatTerminal.
 *
 * If some data has a structure which matches a schema:BoatTerminal, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBoatTerminal = BoatTerminal.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBoatTerminal instanceof OaValidationError) {
 *   // From this point on, `maybeBoatTerminal` will have type `OaValidationError`
 *   const error = maybeBoatTerminal;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBoatTerminal` will have type `BoatTerminal`
 * const boatTerminal = maybeBoatTerminal;
 * ```
 */
export function validateBoatTerminal(maybeBoatTerminal: unknown): BoatTerminal | OaValidationError {
  const { value, error } = BoatTerminalJoiSchema.validate(maybeBoatTerminal);
  if (error) {
    return new OaValidationError('BoatTerminal', maybeBoatTerminal, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BoatTerminal;
}
