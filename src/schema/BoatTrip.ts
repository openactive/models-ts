 ...
 * etc.
 */
export type BoatTripOrSubClass =
  | BoatTrip
  ;
 ...
 */
export const BoatTripJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BoatTripOrSubClassJoiSchema = Joi.alternatives().try([
  BoatTripJoiSchema,
]);
 ...
 * ```
 */
export function validateBoatTrip(maybeBoatTrip: unknown): BoatTrip | OaValidationError {
  const { value, error } = BoatTripJoiSchema.validate(maybeBoatTrip);
  if (error) {
    return new OaValidationError('BoatTrip', maybeBoatTrip, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BoatTrip;
}
