 ...
 * schema:BodyOfWater
 *
 * This differs from BodyOfWater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BodyOfWaterOrSubClass =
  | BodyOfWater
  | schema.OceanBodyOfWaterOrSubClass
  | schema.PondOrSubClass
  | schema.RiverBodyOfWaterOrSubClass
  | schema.ReservoirOrSubClass
  | schema.SeaBodyOfWaterOrSubClass
  | schema.CanalOrSubClass
  | schema.LakeBodyOfWaterOrSubClass
  | schema.WaterfallOrSubClass
  ;

/**
 * schema:BodyOfWater - Validation schema (w/ JOI)
 */
export const BodyOfWaterJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BodyOfWaterOrSubClassJoiSchema = Joi.alternatives().try([
  BodyOfWaterJoiSchema,
 ...
 *
 * ```ts
 * const maybeBodyOfWater = BodyOfWater.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBodyOfWater(maybeBodyOfWater: unknown): BodyOfWater | OaValidationError {
  const { value, error } = BodyOfWaterJoiSchema.validate(maybeBodyOfWater);
  if (error) {
    return new OaValidationError('BodyOfWater', maybeBodyOfWater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BodyOfWater;
}
