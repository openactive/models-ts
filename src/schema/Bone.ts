 ...
 * schema:Bone
 *
 * This differs from Bone because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BoneOrSubClass =
  | Bone
  ;

/**
 * schema:Bone - Validation schema (w/ JOI)
 */
export const BoneJoiSchema = Joi.object({
  '@type': Joi.string().valid('Bone').required(),
 ...
 * schema:Bone - Validation schema (w/ JOI)
 *
 * This differs from BoneJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BoneOrSubClassJoiSchema = Joi.alternatives().try([
  BoneJoiSchema,
]);

/**
 * Runtime validator for schema:Bone.
 *
 * If some data has a structure which matches a schema:Bone, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeBone` will have type `Bone`
 * const bone = maybeBone;
 * ```
 */
export function validateBone(maybeBone: unknown): Bone | OaValidationError {
  const { value, error } = BoneJoiSchema.validate(maybeBone);
  if (error) {
    return new OaValidationError('Bone', maybeBone, error);
  }
