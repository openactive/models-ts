 ...
 * schema:Book
 *
 * This differs from Book because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BookOrSubClass =
  | Book
  | schema.AudiobookOrSubClass
  ;

/**
 * schema:Book - Validation schema (w/ JOI)
 */
 ...
 * schema:Book - Validation schema (w/ JOI)
 *
 * This differs from BookJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BookOrSubClassJoiSchema = Joi.alternatives().try([
  BookJoiSchema,
  Joi.lazy(() => schema.AudiobookOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Book.
 *
 * If some data has a structure which matches a schema:Book, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBook = Book.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBook instanceof OaValidationError) {
 *   // From this point on, `maybeBook` will have type `OaValidationError`
 *   const error = maybeBook;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBook` will have type `Book`
 * const book = maybeBook;
 * ```
 */
export function validateBook(maybeBook: unknown): Book | OaValidationError {
  const { value, error } = BookJoiSchema.validate(maybeBook);
  if (error) {
    return new OaValidationError('Book', maybeBook, error);
  }
