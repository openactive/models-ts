 ...
 * schema:BookSeries
 *
 * This differs from BookSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BookSeriesOrSubClass =
  | BookSeries
  ;

/**
 * schema:BookSeries - Validation schema (w/ JOI)
 */
export const BookSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('BookSeries').required(),
 ...
 * schema:BookSeries - Validation schema (w/ JOI)
 *
 * This differs from BookSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BookSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  BookSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:BookSeries.
 *
 * If some data has a structure which matches a schema:BookSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBookSeries = BookSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBookSeries instanceof OaValidationError) {
 *   // From this point on, `maybeBookSeries` will have type `OaValidationError`
 *   const error = maybeBookSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBookSeries` will have type `BookSeries`
 * const bookSeries = maybeBookSeries;
 * ```
 */
export function validateBookSeries(maybeBookSeries: unknown): BookSeries | OaValidationError {
  const { value, error } = BookSeriesJoiSchema.validate(maybeBookSeries);
  if (error) {
    return new OaValidationError('BookSeries', maybeBookSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BookSeries;
}
