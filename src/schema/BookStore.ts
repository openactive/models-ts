 ...
 * schema:BookStore
 *
 * This differs from BookStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BookStoreOrSubClass =
  | BookStore
  ;

/**
 * schema:BookStore - Validation schema (w/ JOI)
 */
export const BookStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('BookStore').required(),
 ...
 * schema:BookStore - Validation schema (w/ JOI)
 *
 * This differs from BookStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BookStoreOrSubClassJoiSchema = Joi.alternatives().try([
  BookStoreJoiSchema,
]);

/**
 * Runtime validator for schema:BookStore.
 *
 * If some data has a structure which matches a schema:BookStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBookStore = BookStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBookStore instanceof OaValidationError) {
 *   // From this point on, `maybeBookStore` will have type `OaValidationError`
 *   const error = maybeBookStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBookStore` will have type `BookStore`
 * const bookStore = maybeBookStore;
 * ```
 */
export function validateBookStore(maybeBookStore: unknown): BookStore | OaValidationError {
  const { value, error } = BookStoreJoiSchema.validate(maybeBookStore);
  if (error) {
    return new OaValidationError('BookStore', maybeBookStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BookStore;
}
