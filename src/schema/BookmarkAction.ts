 ...
 * schema:BookmarkAction
 *
 * This differs from BookmarkAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BookmarkActionOrSubClass =
  | BookmarkAction
  ;

/**
 * schema:BookmarkAction - Validation schema (w/ JOI)
 */
export const BookmarkActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('BookmarkAction').required(),
 ...
 * schema:BookmarkAction - Validation schema (w/ JOI)
 *
 * This differs from BookmarkActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BookmarkActionOrSubClassJoiSchema = Joi.alternatives().try([
  BookmarkActionJoiSchema,
]);

/**
 * Runtime validator for schema:BookmarkAction.
 *
 * If some data has a structure which matches a schema:BookmarkAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeBookmarkAction` will have type `BookmarkAction`
 * const bookmarkAction = maybeBookmarkAction;
 * ```
 */
export function validateBookmarkAction(maybeBookmarkAction: unknown): BookmarkAction | OaValidationError {
  const { value, error } = BookmarkActionJoiSchema.validate(maybeBookmarkAction);
  if (error) {
    return new OaValidationError('BookmarkAction', maybeBookmarkAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BookmarkAction;
}
