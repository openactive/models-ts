...
export type BorrowAction = {
   ...
   */
  lender?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
 ...
 * schema:BorrowAction
 *
 * This differs from BorrowAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BorrowActionOrSubClass =
  | BorrowAction
  ;
 ...
 */
export const BorrowActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BorrowActionOrSubClassJoiSchema = Joi.alternatives().try([
  BorrowActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeBorrowAction = BorrowAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBorrowAction(maybeBorrowAction: unknown): BorrowAction | OaValidationError {
  const { value, error } = BorrowActionJoiSchema.validate(maybeBorrowAction);
  if (error) {
    return new OaValidationError('BorrowAction', maybeBorrowAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BorrowAction;
}
