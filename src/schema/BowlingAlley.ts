 ...
 * schema:BowlingAlley
 *
 * This differs from BowlingAlley because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BowlingAlleyOrSubClass =
  | BowlingAlley
  ;

/**
 * schema:BowlingAlley - Validation schema (w/ JOI)
 */
export const BowlingAlleyJoiSchema = Joi.object({
  '@type': Joi.string().valid('BowlingAlley').required(),
 ...
 * schema:BowlingAlley - Validation schema (w/ JOI)
 *
 * This differs from BowlingAlleyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BowlingAlleyOrSubClassJoiSchema = Joi.alternatives().try([
  BowlingAlleyJoiSchema,
]);

/**
 * Runtime validator for schema:BowlingAlley.
 *
 * If some data has a structure which matches a schema:BowlingAlley, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBowlingAlley = BowlingAlley.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBowlingAlley instanceof OaValidationError) {
 *   // From this point on, `maybeBowlingAlley` will have type `OaValidationError`
 *   const error = maybeBowlingAlley;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBowlingAlley` will have type `BowlingAlley`
 * const bowlingAlley = maybeBowlingAlley;
 * ```
 */
export function validateBowlingAlley(maybeBowlingAlley: unknown): BowlingAlley | OaValidationError {
  const { value, error } = BowlingAlleyJoiSchema.validate(maybeBowlingAlley);
  if (error) {
    return new OaValidationError('BowlingAlley', maybeBowlingAlley, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BowlingAlley;
}
