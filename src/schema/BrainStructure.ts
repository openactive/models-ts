 ...
 * etc.
 */
export type BrainStructureOrSubClass =
  | BrainStructure
  ;
 ...
 */
export const BrainStructureJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BrainStructureOrSubClassJoiSchema = Joi.alternatives().try([
  BrainStructureJoiSchema,
]);
 ...
 * ```
 */
export function validateBrainStructure(maybeBrainStructure: unknown): BrainStructure | OaValidationError {
  const { value, error } = BrainStructureJoiSchema.validate(maybeBrainStructure);
  if (error) {
    return new OaValidationError('BrainStructure', maybeBrainStructure, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BrainStructure;
}
