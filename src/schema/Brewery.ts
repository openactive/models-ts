 ...
 * schema:Brewery
 *
 * This differs from Brewery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BreweryOrSubClass =
  | Brewery
  ;

/**
 * schema:Brewery - Validation schema (w/ JOI)
 */
export const BreweryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Brewery').required(),
 ...
 * schema:Brewery - Validation schema (w/ JOI)
 *
 * This differs from BreweryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BreweryOrSubClassJoiSchema = Joi.alternatives().try([
  BreweryJoiSchema,
]);

/**
 * Runtime validator for schema:Brewery.
 *
 * If some data has a structure which matches a schema:Brewery, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBrewery = Brewery.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBrewery instanceof OaValidationError) {
 *   // From this point on, `maybeBrewery` will have type `OaValidationError`
 *   const error = maybeBrewery;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBrewery` will have type `Brewery`
 * const brewery = maybeBrewery;
 * ```
 */
export function validateBrewery(maybeBrewery: unknown): Brewery | OaValidationError {
  const { value, error } = BreweryJoiSchema.validate(maybeBrewery);
  if (error) {
    return new OaValidationError('Brewery', maybeBrewery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Brewery;
}
