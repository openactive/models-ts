 ...
 * schema:Bridge
 *
 * This differs from Bridge because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BridgeOrSubClass =
  | Bridge
  ;

/**
 * schema:Bridge - Validation schema (w/ JOI)
 */
export const BridgeJoiSchema = Joi.object({
 ...
 * schema:Bridge - Validation schema (w/ JOI)
 *
 * This differs from BridgeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BridgeOrSubClassJoiSchema = Joi.alternatives().try([
  BridgeJoiSchema,
]);

/**
 * Runtime validator for schema:Bridge.
 *
 * If some data has a structure which matches a schema:Bridge, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBridge = Bridge.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBridge instanceof OaValidationError) {
 *   // From this point on, `maybeBridge` will have type `OaValidationError`
 *   const error = maybeBridge;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBridge` will have type `Bridge`
 * const bridge = maybeBridge;
 * ```
 */
export function validateBridge(maybeBridge: unknown): Bridge | OaValidationError {
  const { value, error } = BridgeJoiSchema.validate(maybeBridge);
  if (error) {
    return new OaValidationError('Bridge', maybeBridge, error);
  }
