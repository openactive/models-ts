 ...
 * ```
 */
export function validateBroadcastChannel(maybeBroadcastChannel: unknown): BroadcastChannel | OaValidationError {
  const { value, error } = BroadcastChannelJoiSchema.validate(maybeBroadcastChannel);
  if (error) {
    return new OaValidationError('BroadcastChannel', maybeBroadcastChannel, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BroadcastChannel;
}
