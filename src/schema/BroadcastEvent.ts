...
export type BroadcastEvent = {
   ...
   */
  broadcastOfEvent?: schema.Event_OrSubClass | string;
  /**
 ...
 * schema:BroadcastEvent
 *
 * This differs from BroadcastEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BroadcastEventOrSubClass =
  | BroadcastEvent
  ;
 ...
 */
export const BroadcastEventJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BroadcastEventOrSubClassJoiSchema = Joi.alternatives().try([
  BroadcastEventJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeBroadcastEvent = BroadcastEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBroadcastEvent(maybeBroadcastEvent: unknown): BroadcastEvent | OaValidationError {
  const { value, error } = BroadcastEventJoiSchema.validate(maybeBroadcastEvent);
  if (error) {
    return new OaValidationError('BroadcastEvent', maybeBroadcastEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BroadcastEvent;
}
