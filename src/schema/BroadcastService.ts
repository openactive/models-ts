 ...
 * schema:BroadcastService
 *
 * This differs from BroadcastService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BroadcastServiceOrSubClass =
  | BroadcastService
  | schema.RadioBroadcastServiceOrSubClass
  ;

/**
 * schema:BroadcastService - Validation schema (w/ JOI)
 */
export const BroadcastServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BroadcastServiceOrSubClassJoiSchema = Joi.alternatives().try([
  BroadcastServiceJoiSchema,
 ...
 * ```
 */
export function validateBroadcastService(maybeBroadcastService: unknown): BroadcastService | OaValidationError {
  const { value, error } = BroadcastServiceJoiSchema.validate(maybeBroadcastService);
  if (error) {
    return new OaValidationError('BroadcastService', maybeBroadcastService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BroadcastService;
}
