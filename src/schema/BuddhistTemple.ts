 ...
 * schema:BuddhistTemple
 *
 * This differs from BuddhistTemple because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BuddhistTempleOrSubClass =
  | BuddhistTemple
  ;

/**
 * schema:BuddhistTemple - Validation schema (w/ JOI)
 */
export const BuddhistTempleJoiSchema = Joi.object({
  '@type': Joi.string().valid('BuddhistTemple').required(),
 ...
 * schema:BuddhistTemple - Validation schema (w/ JOI)
 *
 * This differs from BuddhistTempleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BuddhistTempleOrSubClassJoiSchema = Joi.alternatives().try([
  BuddhistTempleJoiSchema,
]);

/**
 * Runtime validator for schema:BuddhistTemple.
 *
 * If some data has a structure which matches a schema:BuddhistTemple, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBuddhistTemple = BuddhistTemple.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBuddhistTemple instanceof OaValidationError) {
 *   // From this point on, `maybeBuddhistTemple` will have type `OaValidationError`
 *   const error = maybeBuddhistTemple;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBuddhistTemple` will have type `BuddhistTemple`
 * const buddhistTemple = maybeBuddhistTemple;
 * ```
 */
export function validateBuddhistTemple(maybeBuddhistTemple: unknown): BuddhistTemple | OaValidationError {
  const { value, error } = BuddhistTempleJoiSchema.validate(maybeBuddhistTemple);
  if (error) {
    return new OaValidationError('BuddhistTemple', maybeBuddhistTemple, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BuddhistTemple;
}
