...
export type BusOrCoach = {
   ...
   */
  roofLoad?: schema.QuantitativeValueOrSubClass | string;
  /**
 ...
 * schema:BusOrCoach
 *
 * This differs from BusOrCoach because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BusOrCoachOrSubClass =
  | BusOrCoach
  ;

/**
 * schema:BusOrCoach - Validation schema (w/ JOI)
 */
export const BusOrCoachJoiSchema = Joi.object({
  '@type': Joi.string().valid('BusOrCoach').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  roofLoad: Joi.alternatives().try(Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
  acrissCode: Joi.string(),
 ...
 * schema:BusOrCoach - Validation schema (w/ JOI)
 *
 * This differs from BusOrCoachJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BusOrCoachOrSubClassJoiSchema = Joi.alternatives().try([
  BusOrCoachJoiSchema,
]);

/**
 * Runtime validator for schema:BusOrCoach.
 *
 * If some data has a structure which matches a schema:BusOrCoach, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBusOrCoach = BusOrCoach.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBusOrCoach instanceof OaValidationError) {
 *   // From this point on, `maybeBusOrCoach` will have type `OaValidationError`
 *   const error = maybeBusOrCoach;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBusOrCoach` will have type `BusOrCoach`
 * const busOrCoach = maybeBusOrCoach;
 * ```
 */
export function validateBusOrCoach(maybeBusOrCoach: unknown): BusOrCoach | OaValidationError {
  const { value, error } = BusOrCoachJoiSchema.validate(maybeBusOrCoach);
  if (error) {
    return new OaValidationError('BusOrCoach', maybeBusOrCoach, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BusOrCoach;
}
