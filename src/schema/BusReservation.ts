 ...
 * schema:BusReservation
 *
 * This differs from BusReservation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BusReservationOrSubClass =
  | BusReservation
  ;
 ...
 */
export const BusReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BusReservationOrSubClassJoiSchema = Joi.alternatives().try([
  BusReservationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeBusReservation = validateBusReservation(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateBusReservation(maybeBusReservation: unknown): BusReservation | OaValidationError {
  const { value, error } = BusReservationJoiSchema.validate(maybeBusReservation);
  if (error) {
    return new OaValidationError('BusReservation', maybeBusReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BusReservation;
}
