 ...
 * schema:BusStop
 *
 * This differs from BusStop because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BusStopOrSubClass =
  | BusStop
  ;

/**
 * schema:BusStop - Validation schema (w/ JOI)
 */
export const BusStopJoiSchema = Joi.object({
  '@type': Joi.string().valid('BusStop').required(),
 ...
 * schema:BusStop - Validation schema (w/ JOI)
 *
 * This differs from BusStopJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BusStopOrSubClassJoiSchema = Joi.alternatives().try([
  BusStopJoiSchema,
]);

/**
 * Runtime validator for schema:BusStop.
 *
 * If some data has a structure which matches a schema:BusStop, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBusStop = BusStop.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBusStop instanceof OaValidationError) {
 *   // From this point on, `maybeBusStop` will have type `OaValidationError`
 *   const error = maybeBusStop;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBusStop` will have type `BusStop`
 * const busStop = maybeBusStop;
 * ```
 */
export function validateBusStop(maybeBusStop: unknown): BusStop | OaValidationError {
  const { value, error } = BusStopJoiSchema.validate(maybeBusStop);
  if (error) {
    return new OaValidationError('BusStop', maybeBusStop, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BusStop;
}
