 ...
 * etc.
 */
export type BusinessAudienceOrSubClass =
  | BusinessAudience
  ;
 ...
 */
export const BusinessAudienceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BusinessAudienceOrSubClassJoiSchema = Joi.alternatives().try([
  BusinessAudienceJoiSchema,
]);
 ...
 * ```
 */
export function validateBusinessAudience(maybeBusinessAudience: unknown): BusinessAudience | OaValidationError {
  const { value, error } = BusinessAudienceJoiSchema.validate(maybeBusinessAudience);
  if (error) {
    return new OaValidationError('BusinessAudience', maybeBusinessAudience, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BusinessAudience;
}
