 ...
 * 	  
 */
export const BusinessEntityTypeJoiSchema = Joi.string().valid(
);
 ...
 * ```
 */
export function validateBusinessEntityType(maybeBusinessEntityType: unknown): BusinessEntityType | OaValidationError {
  const { value, error } = BusinessEntityTypeJoiSchema.validate(maybeBusinessEntityType);
  if (error) {
    return new OaValidationError('BusinessEntityType', maybeBusinessEntityType, error);
  }
