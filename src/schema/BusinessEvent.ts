 ...
 * schema:BusinessEvent
 *
 * This differs from BusinessEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BusinessEventOrSubClass =
  | BusinessEvent
  ;
 ...
 */
export const BusinessEventJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const BusinessEventOrSubClassJoiSchema = Joi.alternatives().try([
  BusinessEventJoiSchema,
]);
 ...
 * ```
 */
export function validateBusinessEvent(maybeBusinessEvent: unknown): BusinessEvent | OaValidationError {
  const { value, error } = BusinessEventJoiSchema.validate(maybeBusinessEvent);
  if (error) {
    return new OaValidationError('BusinessEvent', maybeBusinessEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BusinessEvent;
}
