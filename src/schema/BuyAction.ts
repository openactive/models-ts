...
export type BuyAction = {
   ...
   */
  vendor?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
   * The warranty promise(s) included in the offer.
   */
  warrantyPromise?: schema.WarrantyPromiseOrSubClass | string;
  /**
 ...
 * schema:BuyAction
 *
 * This differs from BuyAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type BuyActionOrSubClass =
  | BuyAction
  ;

/**
 * schema:BuyAction - Validation schema (w/ JOI)
 */
export const BuyActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('BuyAction').required(),
  ...
  vendor: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:BuyAction - Validation schema (w/ JOI)
 *
 * This differs from BuyActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const BuyActionOrSubClassJoiSchema = Joi.alternatives().try([
  BuyActionJoiSchema,
]);

/**
 * Runtime validator for schema:BuyAction.
 *
 * If some data has a structure which matches a schema:BuyAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeBuyAction = BuyAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeBuyAction instanceof OaValidationError) {
 *   // From this point on, `maybeBuyAction` will have type `OaValidationError`
 *   const error = maybeBuyAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeBuyAction` will have type `BuyAction`
 * const buyAction = maybeBuyAction;
 * ```
 */
export function validateBuyAction(maybeBuyAction: unknown): BuyAction | OaValidationError {
  const { value, error } = BuyActionJoiSchema.validate(maybeBuyAction);
  if (error) {
    return new OaValidationError('BuyAction', maybeBuyAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as BuyAction;
}
