...
export type BuyAction = {
   ...
   */
  vendor?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
...
export const BuyActionJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  vendor: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 *
 * ```ts
 * const maybeBuyAction = validateBuyAction(req.body); // `req.body` will have type `any` or `unknown`
