 ...
 */
export type CableOrSatelliteService = {
 ...
 * schema:CableOrSatelliteService
 *
 * This differs from CableOrSatelliteService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CableOrSatelliteServiceOrSubClass =
 ...
 */
export const CableOrSatelliteServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CableOrSatelliteServiceOrSubClassJoiSchema = Joi.alternatives().try([
  CableOrSatelliteServiceJoiSchema,
]);
 ...
 * ```
 */
export function validateCableOrSatelliteService(maybeCableOrSatelliteService: unknown): CableOrSatelliteService | OaValidationError {
  const { value, error } = CableOrSatelliteServiceJoiSchema.validate(maybeCableOrSatelliteService);
  if (error) {
    return new OaValidationError('CableOrSatelliteService', maybeCableOrSatelliteService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CableOrSatelliteService;
}
