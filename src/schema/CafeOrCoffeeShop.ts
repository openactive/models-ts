 ...
 * schema:CafeOrCoffeeShop
 *
 * This differs from CafeOrCoffeeShop because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CafeOrCoffeeShopOrSubClass =
  | CafeOrCoffeeShop
  ;

/**
 * schema:CafeOrCoffeeShop - Validation schema (w/ JOI)
 */
export const CafeOrCoffeeShopJoiSchema = Joi.object({
  '@type': Joi.string().valid('CafeOrCoffeeShop').required(),
 ...
 * schema:CafeOrCoffeeShop - Validation schema (w/ JOI)
 *
 * This differs from CafeOrCoffeeShopJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CafeOrCoffeeShopOrSubClassJoiSchema = Joi.alternatives().try([
  CafeOrCoffeeShopJoiSchema,
]);

/**
 * Runtime validator for schema:CafeOrCoffeeShop.
 *
 * If some data has a structure which matches a schema:CafeOrCoffeeShop, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCafeOrCoffeeShop = CafeOrCoffeeShop.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCafeOrCoffeeShop instanceof OaValidationError) {
 *   // From this point on, `maybeCafeOrCoffeeShop` will have type `OaValidationError`
 *   const error = maybeCafeOrCoffeeShop;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCafeOrCoffeeShop` will have type `CafeOrCoffeeShop`
 * const cafeOrCoffeeShop = maybeCafeOrCoffeeShop;
 * ```
 */
export function validateCafeOrCoffeeShop(maybeCafeOrCoffeeShop: unknown): CafeOrCoffeeShop | OaValidationError {
  const { value, error } = CafeOrCoffeeShopJoiSchema.validate(maybeCafeOrCoffeeShop);
  if (error) {
    return new OaValidationError('CafeOrCoffeeShop', maybeCafeOrCoffeeShop, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CafeOrCoffeeShop;
}
