 ...
 * schema:Campground
 *
 * This differs from Campground because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CampgroundOrSubClass =
  | Campground
  ;

/**
 * schema:Campground - Validation schema (w/ JOI)
 */
export const CampgroundJoiSchema = Joi.object({
  '@type': Joi.string().valid('Campground').required(),
 ...
 * schema:Campground - Validation schema (w/ JOI)
 *
 * This differs from CampgroundJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CampgroundOrSubClassJoiSchema = Joi.alternatives().try([
  CampgroundJoiSchema,
]);

/**
 * Runtime validator for schema:Campground.
 *
 * If some data has a structure which matches a schema:Campground, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCampground = validateCampground(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCampground instanceof OaValidationError) {
 *   // From this point on, `maybeCampground` will have type `OaValidationError`
 *   const error = maybeCampground;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCampground` will have type `Campground`
 * const campground = maybeCampground;
 * ```
 */
export function validateCampground(maybeCampground: unknown): Campground | OaValidationError {
  const { value, error } = CampgroundJoiSchema.validate(maybeCampground);
  if (error) {
    return new OaValidationError('Campground', maybeCampground, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Campground;
}
