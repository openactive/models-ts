 ...
 * schema:CampingPitch
 *
 * This differs from CampingPitch because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CampingPitchOrSubClass =
  | CampingPitch
  ;

/**
 * schema:CampingPitch - Validation schema (w/ JOI)
 */
export const CampingPitchJoiSchema = Joi.object({
  '@type': Joi.string().valid('CampingPitch').required(),
 ...
 * schema:CampingPitch - Validation schema (w/ JOI)
 *
 * This differs from CampingPitchJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CampingPitchOrSubClassJoiSchema = Joi.alternatives().try([
  CampingPitchJoiSchema,
]);

/**
 * Runtime validator for schema:CampingPitch.
 *
 * If some data has a structure which matches a schema:CampingPitch, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCampingPitch` will have type `CampingPitch`
 * const campingPitch = maybeCampingPitch;
 * ```
 */
export function validateCampingPitch(maybeCampingPitch: unknown): CampingPitch | OaValidationError {
  const { value, error } = CampingPitchJoiSchema.validate(maybeCampingPitch);
  if (error) {
    return new OaValidationError('CampingPitch', maybeCampingPitch, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CampingPitch;
}
