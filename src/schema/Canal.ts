 ...
 * schema:Canal
 *
 * This differs from Canal because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CanalOrSubClass =
  | Canal
  ;

/**
 * schema:Canal - Validation schema (w/ JOI)
 */
export const CanalJoiSchema = Joi.object({
  '@type': Joi.string().valid('Canal').required(),
 ...
 * schema:Canal - Validation schema (w/ JOI)
 *
 * This differs from CanalJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CanalOrSubClassJoiSchema = Joi.alternatives().try([
  CanalJoiSchema,
]);

/**
 * Runtime validator for schema:Canal.
 *
 * If some data has a structure which matches a schema:Canal, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCanal = Canal.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCanal instanceof OaValidationError) {
 *   // From this point on, `maybeCanal` will have type `OaValidationError`
 *   const error = maybeCanal;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCanal` will have type `Canal`
 * const canal = maybeCanal;
 * ```
 */
export function validateCanal(maybeCanal: unknown): Canal | OaValidationError {
  const { value, error } = CanalJoiSchema.validate(maybeCanal);
  if (error) {
    return new OaValidationError('Canal', maybeCanal, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Canal;
}
