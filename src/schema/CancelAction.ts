 ...
 * schema:CancelAction
 *
 * This differs from CancelAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CancelActionOrSubClass =
  | CancelAction
  ;

/**
 * schema:CancelAction - Validation schema (w/ JOI)
 */
export const CancelActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('CancelAction').required(),
 ...
 * schema:CancelAction - Validation schema (w/ JOI)
 *
 * This differs from CancelActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CancelActionOrSubClassJoiSchema = Joi.alternatives().try([
  CancelActionJoiSchema,
]);

/**
 * Runtime validator for schema:CancelAction.
 *
 * If some data has a structure which matches a schema:CancelAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCancelAction = CancelAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCancelAction instanceof OaValidationError) {
 *   // From this point on, `maybeCancelAction` will have type `OaValidationError`
 *   const error = maybeCancelAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCancelAction` will have type `CancelAction`
 * const cancelAction = maybeCancelAction;
 * ```
 */
export function validateCancelAction(maybeCancelAction: unknown): CancelAction | OaValidationError {
  const { value, error } = CancelActionJoiSchema.validate(maybeCancelAction);
  if (error) {
    return new OaValidationError('CancelAction', maybeCancelAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CancelAction;
}
