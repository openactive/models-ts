 ...
 * schema:Car
 *
 * This differs from Car because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CarOrSubClass =
  | Car
  ;

/**
 * schema:Car - Validation schema (w/ JOI)
 */
export const CarJoiSchema = Joi.object({
  '@type': Joi.string().valid('Car').required(),
 ...
 * schema:Car - Validation schema (w/ JOI)
 *
 * This differs from CarJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CarOrSubClassJoiSchema = Joi.alternatives().try([
  CarJoiSchema,
]);

/**
 * Runtime validator for schema:Car.
 *
 * If some data has a structure which matches a schema:Car, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCar = Car.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCar instanceof OaValidationError) {
 *   // From this point on, `maybeCar` will have type `OaValidationError`
 *   const error = maybeCar;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCar` will have type `Car`
 * const car = maybeCar;
 * ```
 */
export function validateCar(maybeCar: unknown): Car | OaValidationError {
  const { value, error } = CarJoiSchema.validate(maybeCar);
  if (error) {
    return new OaValidationError('Car', maybeCar, error);
  }
