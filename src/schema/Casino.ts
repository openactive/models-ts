 ...
 * schema:Casino
 *
 * This differs from Casino because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CasinoOrSubClass =
  | Casino
  ;

/**
 * schema:Casino - Validation schema (w/ JOI)
 */
export const CasinoJoiSchema = Joi.object({
  '@type': Joi.string().valid('Casino').required(),
 ...
 * schema:Casino - Validation schema (w/ JOI)
 *
 * This differs from CasinoJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CasinoOrSubClassJoiSchema = Joi.alternatives().try([
  CasinoJoiSchema,
]);

/**
 * Runtime validator for schema:Casino.
 *
 * If some data has a structure which matches a schema:Casino, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCasino = Casino.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCasino instanceof OaValidationError) {
 *   // From this point on, `maybeCasino` will have type `OaValidationError`
 *   const error = maybeCasino;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCasino` will have type `Casino`
 * const casino = maybeCasino;
 * ```
 */
export function validateCasino(maybeCasino: unknown): Casino | OaValidationError {
  const { value, error } = CasinoJoiSchema.validate(maybeCasino);
  if (error) {
    return new OaValidationError('Casino', maybeCasino, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Casino;
}
