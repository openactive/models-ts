...
export type CategoryCode = {
   ...
   */
  inDefinedTermSet?: string | schema.DefinedTermSetOrSubClass;
  /**
 ...
 * schema:CategoryCode
 *
 * This differs from CategoryCode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CategoryCodeOrSubClass =
  | CategoryCode
 ...
 */
export const CategoryCodeJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CategoryCodeOrSubClassJoiSchema = Joi.alternatives().try([
  CategoryCodeJoiSchema,
 ...
 * ```
 */
export function validateCategoryCode(maybeCategoryCode: unknown): CategoryCode | OaValidationError {
  const { value, error } = CategoryCodeJoiSchema.validate(maybeCategoryCode);
  if (error) {
    return new OaValidationError('CategoryCode', maybeCategoryCode, error);
  }
