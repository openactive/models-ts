...
export type CategoryCodeSet = {
   ...
   */
  hasCategoryCode?: schema.CategoryCodeOrSubClass | string;
  /**
 ...
 * schema:CategoryCodeSet
 *
 * This differs from CategoryCodeSet because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CategoryCodeSetOrSubClass =
  | CategoryCodeSet
  ;

/**
 * schema:CategoryCodeSet - Validation schema (w/ JOI)
 */
export const CategoryCodeSetJoiSchema = Joi.object({
  '@type': Joi.string().valid('CategoryCodeSet').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  hasCategoryCode: Joi.alternatives().try(Joi.lazy(() => schema.CategoryCodeOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:CategoryCodeSet - Validation schema (w/ JOI)
 *
 * This differs from CategoryCodeSetJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CategoryCodeSetOrSubClassJoiSchema = Joi.alternatives().try([
  CategoryCodeSetJoiSchema,
]);

/**
 * Runtime validator for schema:CategoryCodeSet.
 *
 * If some data has a structure which matches a schema:CategoryCodeSet, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCategoryCodeSet = CategoryCodeSet.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCategoryCodeSet instanceof OaValidationError) {
 *   // From this point on, `maybeCategoryCodeSet` will have type `OaValidationError`
 *   const error = maybeCategoryCodeSet;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCategoryCodeSet` will have type `CategoryCodeSet`
 * const categoryCodeSet = maybeCategoryCodeSet;
 * ```
 */
export function validateCategoryCodeSet(maybeCategoryCodeSet: unknown): CategoryCodeSet | OaValidationError {
  const { value, error } = CategoryCodeSetJoiSchema.validate(maybeCategoryCodeSet);
  if (error) {
    return new OaValidationError('CategoryCodeSet', maybeCategoryCodeSet, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CategoryCodeSet;
}
