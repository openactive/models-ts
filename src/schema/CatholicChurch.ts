 ...
 * schema:CatholicChurch
 *
 * This differs from CatholicChurch because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CatholicChurchOrSubClass =
  | CatholicChurch
  ;

/**
 * schema:CatholicChurch - Validation schema (w/ JOI)
 */
export const CatholicChurchJoiSchema = Joi.object({
  '@type': Joi.string().valid('CatholicChurch').required(),
 ...
 * schema:CatholicChurch - Validation schema (w/ JOI)
 *
 * This differs from CatholicChurchJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CatholicChurchOrSubClassJoiSchema = Joi.alternatives().try([
  CatholicChurchJoiSchema,
]);

/**
 * Runtime validator for schema:CatholicChurch.
 *
 * If some data has a structure which matches a schema:CatholicChurch, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCatholicChurch` will have type `CatholicChurch`
 * const catholicChurch = maybeCatholicChurch;
 * ```
 */
export function validateCatholicChurch(maybeCatholicChurch: unknown): CatholicChurch | OaValidationError {
  const { value, error } = CatholicChurchJoiSchema.validate(maybeCatholicChurch);
  if (error) {
    return new OaValidationError('CatholicChurch', maybeCatholicChurch, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CatholicChurch;
}
