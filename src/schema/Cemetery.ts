 ...
 * schema:Cemetery
 *
 * This differs from Cemetery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CemeteryOrSubClass =
  | Cemetery
  ;

/**
 * schema:Cemetery - Validation schema (w/ JOI)
 */
export const CemeteryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Cemetery').required(),
 ...
 * schema:Cemetery - Validation schema (w/ JOI)
 *
 * This differs from CemeteryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CemeteryOrSubClassJoiSchema = Joi.alternatives().try([
  CemeteryJoiSchema,
]);

/**
 * Runtime validator for schema:Cemetery.
 *
 * If some data has a structure which matches a schema:Cemetery, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCemetery` will have type `Cemetery`
 * const cemetery = maybeCemetery;
 * ```
 */
export function validateCemetery(maybeCemetery: unknown): Cemetery | OaValidationError {
  const { value, error } = CemeteryJoiSchema.validate(maybeCemetery);
  if (error) {
    return new OaValidationError('Cemetery', maybeCemetery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Cemetery;
}
