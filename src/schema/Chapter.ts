 ...
 * schema:Chapter
 *
 * This differs from Chapter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChapterOrSubClass =
  | Chapter
  ;

/**
 * schema:Chapter - Validation schema (w/ JOI)
 */
export const ChapterJoiSchema = Joi.object({
  '@type': Joi.string().valid('Chapter').required(),
 ...
 * schema:Chapter - Validation schema (w/ JOI)
 *
 * This differs from ChapterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ChapterOrSubClassJoiSchema = Joi.alternatives().try([
  ChapterJoiSchema,
]);

/**
 * Runtime validator for schema:Chapter.
 *
 * If some data has a structure which matches a schema:Chapter, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeChapter = Chapter.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeChapter instanceof OaValidationError) {
 *   // From this point on, `maybeChapter` will have type `OaValidationError`
 *   const error = maybeChapter;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeChapter` will have type `Chapter`
 * const chapter = maybeChapter;
 * ```
 */
export function validateChapter(maybeChapter: unknown): Chapter | OaValidationError {
  const { value, error } = ChapterJoiSchema.validate(maybeChapter);
  if (error) {
    return new OaValidationError('Chapter', maybeChapter, error);
  }
