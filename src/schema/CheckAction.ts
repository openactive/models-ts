 ...
 * schema:CheckAction
 *
 * This differs from CheckAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CheckActionOrSubClass =
  | CheckAction
  ;

/**
 * schema:CheckAction - Validation schema (w/ JOI)
 */
export const CheckActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('CheckAction').required(),
 ...
 * schema:CheckAction - Validation schema (w/ JOI)
 *
 * This differs from CheckActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CheckActionOrSubClassJoiSchema = Joi.alternatives().try([
  CheckActionJoiSchema,
]);

/**
 * Runtime validator for schema:CheckAction.
 *
 * If some data has a structure which matches a schema:CheckAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCheckAction = CheckAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCheckAction instanceof OaValidationError) {
 *   // From this point on, `maybeCheckAction` will have type `OaValidationError`
 *   const error = maybeCheckAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCheckAction` will have type `CheckAction`
 * const checkAction = maybeCheckAction;
 * ```
 */
export function validateCheckAction(maybeCheckAction: unknown): CheckAction | OaValidationError {
  const { value, error } = CheckActionJoiSchema.validate(maybeCheckAction);
  if (error) {
    return new OaValidationError('CheckAction', maybeCheckAction, error);
  }
