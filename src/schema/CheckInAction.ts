 ...
 * etc.
 */
export type CheckInActionOrSubClass =
  | CheckInAction
  ;
 ...
 */
export const CheckInActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CheckInActionOrSubClassJoiSchema = Joi.alternatives().try([
  CheckInActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeCheckInAction = CheckInAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateCheckInAction(maybeCheckInAction: unknown): CheckInAction | OaValidationError {
  const { value, error } = CheckInActionJoiSchema.validate(maybeCheckInAction);
  if (error) {
    return new OaValidationError('CheckInAction', maybeCheckInAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CheckInAction;
}
