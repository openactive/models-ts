 ...
 * schema:ChildCare
 *
 * This differs from ChildCare because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChildCareOrSubClass =
  | ChildCare
  ;

/**
 * schema:ChildCare - Validation schema (w/ JOI)
 */
export const ChildCareJoiSchema = Joi.object({
  '@type': Joi.string().valid('ChildCare').required(),
 ...
 * schema:ChildCare - Validation schema (w/ JOI)
 *
 * This differs from ChildCareJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ChildCareOrSubClassJoiSchema = Joi.alternatives().try([
  ChildCareJoiSchema,
]);

/**
 * Runtime validator for schema:ChildCare.
 *
 * If some data has a structure which matches a schema:ChildCare, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeChildCare = ChildCare.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeChildCare instanceof OaValidationError) {
 *   // From this point on, `maybeChildCare` will have type `OaValidationError`
 *   const error = maybeChildCare;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeChildCare` will have type `ChildCare`
 * const childCare = maybeChildCare;
 * ```
 */
export function validateChildCare(maybeChildCare: unknown): ChildCare | OaValidationError {
  const { value, error } = ChildCareJoiSchema.validate(maybeChildCare);
  if (error) {
    return new OaValidationError('ChildCare', maybeChildCare, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ChildCare;
}
