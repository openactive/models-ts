 ...
 * schema:ChildrensEvent
 *
 * This differs from ChildrensEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChildrensEventOrSubClass =
  | ChildrensEvent
  ;

/**
 * schema:ChildrensEvent - Validation schema (w/ JOI)
 */
export const ChildrensEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('ChildrensEvent').required(),
 ...
 * schema:ChildrensEvent - Validation schema (w/ JOI)
 *
 * This differs from ChildrensEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ChildrensEventOrSubClassJoiSchema = Joi.alternatives().try([
  ChildrensEventJoiSchema,
]);

/**
 * Runtime validator for schema:ChildrensEvent.
 *
 * If some data has a structure which matches a schema:ChildrensEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeChildrensEvent = ChildrensEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeChildrensEvent instanceof OaValidationError) {
 *   // From this point on, `maybeChildrensEvent` will have type `OaValidationError`
 *   const error = maybeChildrensEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeChildrensEvent` will have type `ChildrensEvent`
 * const childrensEvent = maybeChildrensEvent;
 * ```
 */
export function validateChildrensEvent(maybeChildrensEvent: unknown): ChildrensEvent | OaValidationError {
  const { value, error } = ChildrensEventJoiSchema.validate(maybeChildrensEvent);
  if (error) {
    return new OaValidationError('ChildrensEvent', maybeChildrensEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ChildrensEvent;
}
