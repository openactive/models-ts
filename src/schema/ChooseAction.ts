...
export type ChooseAction = {
   ...
   */
  option?: schema.ThingOrSubClass | string;
  /**
   * A sub property of object. The options subject to this action.
   */
 ...
 * schema:ChooseAction
 *
 * This differs from ChooseAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChooseActionOrSubClass =
  | ChooseAction
  | schema.VoteActionOrSubClass
  ;

/**
 * schema:ChooseAction - Validation schema (w/ JOI)
 */
export const ChooseActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ChooseActionOrSubClassJoiSchema = Joi.alternatives().try([
  ChooseActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeChooseAction = ChooseAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateChooseAction(maybeChooseAction: unknown): ChooseAction | OaValidationError {
  const { value, error } = ChooseActionJoiSchema.validate(maybeChooseAction);
  if (error) {
    return new OaValidationError('ChooseAction', maybeChooseAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ChooseAction;
}
