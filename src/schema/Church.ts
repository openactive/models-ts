 ...
 * schema:Church
 *
 * This differs from Church because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ChurchOrSubClass =
  | Church
  | schema.CatholicChurchOrSubClass
  ;

/**
 * schema:Church - Validation schema (w/ JOI)
 */
export const ChurchJoiSchema = Joi.object({
  '@type': Joi.string().valid('Church').required(),
 ...
 * schema:Church - Validation schema (w/ JOI)
 *
 * This differs from ChurchJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ChurchOrSubClassJoiSchema = Joi.alternatives().try([
  ChurchJoiSchema,
  Joi.lazy(() => schema.CatholicChurchOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Church.
 *
 * If some data has a structure which matches a schema:Church, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeChurch = Church.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeChurch instanceof OaValidationError) {
 *   // From this point on, `maybeChurch` will have type `OaValidationError`
 *   const error = maybeChurch;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeChurch` will have type `Church`
 * const church = maybeChurch;
 * ```
 */
export function validateChurch(maybeChurch: unknown): Church | OaValidationError {
  const { value, error } = ChurchJoiSchema.validate(maybeChurch);
  if (error) {
    return new OaValidationError('Church', maybeChurch, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Church;
}
