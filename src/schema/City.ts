 ...
 * schema:City
 *
 * This differs from City because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CityOrSubClass =
  | City
  ;

/**
 * schema:City - Validation schema (w/ JOI)
 */
export const CityJoiSchema = Joi.object({
 ...
 * schema:City - Validation schema (w/ JOI)
 *
 * This differs from CityJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CityOrSubClassJoiSchema = Joi.alternatives().try([
  CityJoiSchema,
]);

/**
 * Runtime validator for schema:City.
 *
 * If some data has a structure which matches a schema:City, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCity` will have type `City`
 * const city = maybeCity;
 * ```
 */
export function validateCity(maybeCity: unknown): City | OaValidationError {
  const { value, error } = CityJoiSchema.validate(maybeCity);
  if (error) {
    return new OaValidationError('City', maybeCity, error);
  }
