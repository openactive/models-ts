 ...
 * schema:CityHall
 *
 * This differs from CityHall because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CityHallOrSubClass =
  | CityHall
  ;

/**
 * schema:CityHall - Validation schema (w/ JOI)
 */
export const CityHallJoiSchema = Joi.object({
  '@type': Joi.string().valid('CityHall').required(),
 ...
 * schema:CityHall - Validation schema (w/ JOI)
 *
 * This differs from CityHallJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CityHallOrSubClassJoiSchema = Joi.alternatives().try([
  CityHallJoiSchema,
]);

/**
 * Runtime validator for schema:CityHall.
 *
 * If some data has a structure which matches a schema:CityHall, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCityHall = CityHall.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCityHall instanceof OaValidationError) {
 *   // From this point on, `maybeCityHall` will have type `OaValidationError`
 *   const error = maybeCityHall;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCityHall` will have type `CityHall`
 * const cityHall = maybeCityHall;
 * ```
 */
export function validateCityHall(maybeCityHall: unknown): CityHall | OaValidationError {
  const { value, error } = CityHallJoiSchema.validate(maybeCityHall);
  if (error) {
    return new OaValidationError('CityHall', maybeCityHall, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CityHall;
}
