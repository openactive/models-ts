 ...
 * schema:CivicStructure
 *
 * This differs from CivicStructure because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CivicStructureOrSubClass =
  | CivicStructure
  | schema.AquariumOrSubClass
  | schema.EducationalOrganizationOrSubClass
  | schema.MovieTheaterOrSubClass
  | schema.HospitalOrSubClass
  | schema.TrainStationOrSubClass
  | schema.CampgroundOrSubClass
  | schema.PoliceStationOrSubClass
  | schema.BridgeOrSubClass
  | schema.ZooOrSubClass
  | schema.BeachOrSubClass
  | schema.EventVenueOrSubClass
  | schema.PlaygroundOrSubClass
  | schema.SubwayStationOrSubClass
  | schema.AirportOrSubClass
  | schema.BusStopOrSubClass
  | schema.CemeteryOrSubClass
  | schema.FireStationOrSubClass
  | schema.BusStationOrSubClass
  | schema.MuseumOrSubClass
  | schema.ParkOrSubClass
  | schema.TaxiStandOrSubClass
  | schema.PlaceOfWorshipOrSubClass
  | schema.MusicVenueOrSubClass
  | schema.ParkingFacilityOrSubClass
  | schema.StadiumOrArenaOrSubClass
  | schema.CrematoriumOrSubClass
  | schema.BoatTerminalOrSubClass
  | schema.PublicToiletOrSubClass
  | schema.GovernmentBuildingOrSubClass
  | schema.RVParkOrSubClass
  | schema.PerformingArtsTheaterOrSubClass
  ;

/**
 * schema:CivicStructure - Validation schema (w/ JOI)
 */
export const CivicStructureJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CivicStructureOrSubClassJoiSchema = Joi.alternatives().try([
  CivicStructureJoiSchema,
 ...
 *
 * ```ts
 * const maybeCivicStructure = CivicStructure.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateCivicStructure(maybeCivicStructure: unknown): CivicStructure | OaValidationError {
  const { value, error } = CivicStructureJoiSchema.validate(maybeCivicStructure);
  if (error) {
    return new OaValidationError('CivicStructure', maybeCivicStructure, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CivicStructure;
}
