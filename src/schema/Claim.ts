...
export type Claim = {
   ...
   */
  firstAppearance?: schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  claimInterpreter?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
   * Indicates an occurence of a [[Claim]] in some [[CreativeWork]].
   */
  appearance?: schema.CreativeWorkOrSubClass | string;
  /**
 ...
 * schema:Claim
 *
 * This differs from Claim because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ClaimOrSubClass =
  | Claim
  ;

/**
 * schema:Claim - Validation schema (w/ JOI)
 */
export const ClaimJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  firstAppearance: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
  claimInterpreter: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
  appearance: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:Claim - Validation schema (w/ JOI)
 *
 * This differs from ClaimJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ClaimOrSubClassJoiSchema = Joi.alternatives().try([
  ClaimJoiSchema,
]);

/**
 * Runtime validator for schema:Claim.
 *
 * If some data has a structure which matches a schema:Claim, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeClaim` will have type `Claim`
 * const claim = maybeClaim;
 * ```
 */
export function validateClaim(maybeClaim: unknown): Claim | OaValidationError {
  const { value, error } = ClaimJoiSchema.validate(maybeClaim);
  if (error) {
    return new OaValidationError('Claim', maybeClaim, error);
  }
