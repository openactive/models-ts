 ...
 * schema:ClaimReview
 *
 * This differs from ClaimReview because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ClaimReviewOrSubClass =
  | ClaimReview
  ;

/**
 * schema:ClaimReview - Validation schema (w/ JOI)
 */
export const ClaimReviewJoiSchema = Joi.object({
  '@type': Joi.string().valid('ClaimReview').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  claimReviewed: Joi.string(),
 ...
 * schema:ClaimReview - Validation schema (w/ JOI)
 *
 * This differs from ClaimReviewJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ClaimReviewOrSubClassJoiSchema = Joi.alternatives().try([
  ClaimReviewJoiSchema,
]);

/**
 * Runtime validator for schema:ClaimReview.
 *
 * If some data has a structure which matches a schema:ClaimReview, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeClaimReview = validateClaimReview(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeClaimReview instanceof OaValidationError) {
 *   // From this point on, `maybeClaimReview` will have type `OaValidationError`
 *   const error = maybeClaimReview;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeClaimReview` will have type `ClaimReview`
 * const claimReview = maybeClaimReview;
 * ```
 */
export function validateClaimReview(maybeClaimReview: unknown): ClaimReview | OaValidationError {
  const { value, error } = ClaimReviewJoiSchema.validate(maybeClaimReview);
  if (error) {
    return new OaValidationError('ClaimReview', maybeClaimReview, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ClaimReview;
}
