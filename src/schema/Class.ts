...
export type Class = {
   ...
   */
  supersededBy?: schema.ClassOrSubClass | schema.EnumerationOrSubClass | oa.PropertyEnumeration | string;
  /**
 ...
 * schema:Class
 *
 * This differs from Class because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ClassOrSubClass =
  | Class
  ;
 ...
 */
export const ClassJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ClassOrSubClassJoiSchema = Joi.alternatives().try([
  ClassJoiSchema,
]);
 ...
 * ```
 */
export function validateClass(maybeClass: unknown): Class | OaValidationError {
  const { value, error } = ClassJoiSchema.validate(maybeClass);
  if (error) {
    return new OaValidationError('Class', maybeClass, error);
  }
