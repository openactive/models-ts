 ...
 * schema:Clip
 *
 * This differs from Clip because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ClipOrSubClass =
  | Clip
  | schema.RadioClipOrSubClass
  | schema.VideoGameClipOrSubClass
  | schema.TVClipOrSubClass
  | schema.MovieClipOrSubClass
  ;

/**
 * schema:Clip - Validation schema (w/ JOI)
 */
export const ClipJoiSchema = Joi.object({
  '@type': Joi.string().valid('Clip').required(),
 ...
 * schema:Clip - Validation schema (w/ JOI)
 *
 * This differs from ClipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ClipOrSubClassJoiSchema = Joi.alternatives().try([
  ClipJoiSchema,
  Joi.lazy(() => schema.RadioClipOrSubClassJoiSchema),
  Joi.lazy(() => schema.VideoGameClipOrSubClassJoiSchema),
  Joi.lazy(() => schema.TVClipOrSubClassJoiSchema),
  Joi.lazy(() => schema.MovieClipOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Clip.
 *
 * If some data has a structure which matches a schema:Clip, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeClip` will have type `Clip`
 * const clip = maybeClip;
 * ```
 */
export function validateClip(maybeClip: unknown): Clip | OaValidationError {
  const { value, error } = ClipJoiSchema.validate(maybeClip);
  if (error) {
    return new OaValidationError('Clip', maybeClip, error);
  }
