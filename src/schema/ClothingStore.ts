 ...
 * schema:ClothingStore
 *
 * This differs from ClothingStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ClothingStoreOrSubClass =
  | ClothingStore
  ;

/**
 * schema:ClothingStore - Validation schema (w/ JOI)
 */
export const ClothingStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('ClothingStore').required(),
 ...
 * schema:ClothingStore - Validation schema (w/ JOI)
 *
 * This differs from ClothingStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ClothingStoreOrSubClassJoiSchema = Joi.alternatives().try([
  ClothingStoreJoiSchema,
]);

/**
 * Runtime validator for schema:ClothingStore.
 *
 * If some data has a structure which matches a schema:ClothingStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeClothingStore = ClothingStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeClothingStore instanceof OaValidationError) {
 *   // From this point on, `maybeClothingStore` will have type `OaValidationError`
 *   const error = maybeClothingStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeClothingStore` will have type `ClothingStore`
 * const clothingStore = maybeClothingStore;
 * ```
 */
export function validateClothingStore(maybeClothingStore: unknown): ClothingStore | OaValidationError {
  const { value, error } = ClothingStoreJoiSchema.validate(maybeClothingStore);
  if (error) {
    return new OaValidationError('ClothingStore', maybeClothingStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ClothingStore;
}
