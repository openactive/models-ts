 ...
 * schema:Code
 *
 * This differs from Code because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CodeOrSubClass =
  | Code
  ;

/**
 * schema:Code - Validation schema (w/ JOI)
 */
export const CodeJoiSchema = Joi.object({
 ...
 * schema:Code - Validation schema (w/ JOI)
 *
 * This differs from CodeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CodeOrSubClassJoiSchema = Joi.alternatives().try([
  CodeJoiSchema,
]);

/**
 * Runtime validator for schema:Code.
 *
 * If some data has a structure which matches a schema:Code, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCode = Code.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCode instanceof OaValidationError) {
 *   // From this point on, `maybeCode` will have type `OaValidationError`
 *   const error = maybeCode;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCode` will have type `Code`
 * const code = maybeCode;
 * ```
 */
export function validateCode(maybeCode: unknown): Code | OaValidationError {
  const { value, error } = CodeJoiSchema.validate(maybeCode);
  if (error) {
    return new OaValidationError('Code', maybeCode, error);
  }
