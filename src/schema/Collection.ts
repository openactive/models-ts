 ...
 * schema:Collection
 *
 * This differs from Collection because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CollectionOrSubClass =
  | Collection
  | schema.ProductCollectionOrSubClass
  ;

/**
 * schema:Collection - Validation schema (w/ JOI)
 */
 ...
 * schema:Collection - Validation schema (w/ JOI)
 *
 * This differs from CollectionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CollectionOrSubClassJoiSchema = Joi.alternatives().try([
  CollectionJoiSchema,
  Joi.lazy(() => schema.ProductCollectionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Collection.
 *
 * If some data has a structure which matches a schema:Collection, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCollection` will have type `Collection`
 * const collection = maybeCollection;
 * ```
 */
export function validateCollection(maybeCollection: unknown): Collection | OaValidationError {
  const { value, error } = CollectionJoiSchema.validate(maybeCollection);
  if (error) {
    return new OaValidationError('Collection', maybeCollection, error);
  }
