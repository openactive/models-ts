 ...
 * schema:CollectionPage
 *
 * This differs from CollectionPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CollectionPageOrSubClass =
  | CollectionPage
  | schema.MediaGalleryOrSubClass
  ;

/**
 * schema:CollectionPage - Validation schema (w/ JOI)
 */
export const CollectionPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('CollectionPage').required(),
 ...
 * schema:CollectionPage - Validation schema (w/ JOI)
 *
 * This differs from CollectionPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CollectionPageOrSubClassJoiSchema = Joi.alternatives().try([
  CollectionPageJoiSchema,
  Joi.lazy(() => schema.MediaGalleryOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:CollectionPage.
 *
 * If some data has a structure which matches a schema:CollectionPage, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCollectionPage = CollectionPage.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCollectionPage instanceof OaValidationError) {
 *   // From this point on, `maybeCollectionPage` will have type `OaValidationError`
 *   const error = maybeCollectionPage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCollectionPage` will have type `CollectionPage`
 * const collectionPage = maybeCollectionPage;
 * ```
 */
export function validateCollectionPage(maybeCollectionPage: unknown): CollectionPage | OaValidationError {
  const { value, error } = CollectionPageJoiSchema.validate(maybeCollectionPage);
  if (error) {
    return new OaValidationError('CollectionPage', maybeCollectionPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CollectionPage;
}
