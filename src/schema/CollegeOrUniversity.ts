 ...
 */
export type CollegeOrUniversity = {
 ...
 * schema:CollegeOrUniversity
 *
 * This differs from CollegeOrUniversity because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CollegeOrUniversityOrSubClass =
  | CollegeOrUniversity
  ;

/**
 * schema:CollegeOrUniversity - Validation schema (w/ JOI)
 */
export const CollegeOrUniversityJoiSchema = Joi.object({
  '@type': Joi.string().valid('CollegeOrUniversity').required(),
 ...
 * schema:CollegeOrUniversity - Validation schema (w/ JOI)
 *
 * This differs from CollegeOrUniversityJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CollegeOrUniversityOrSubClassJoiSchema = Joi.alternatives().try([
  CollegeOrUniversityJoiSchema,
]);

/**
 * Runtime validator for schema:CollegeOrUniversity.
 *
 * If some data has a structure which matches a schema:CollegeOrUniversity, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCollegeOrUniversity = CollegeOrUniversity.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCollegeOrUniversity instanceof OaValidationError) {
 *   // From this point on, `maybeCollegeOrUniversity` will have type `OaValidationError`
 *   const error = maybeCollegeOrUniversity;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCollegeOrUniversity` will have type `CollegeOrUniversity`
 * const collegeOrUniversity = maybeCollegeOrUniversity;
 * ```
 */
export function validateCollegeOrUniversity(maybeCollegeOrUniversity: unknown): CollegeOrUniversity | OaValidationError {
  const { value, error } = CollegeOrUniversityJoiSchema.validate(maybeCollegeOrUniversity);
  if (error) {
    return new OaValidationError('CollegeOrUniversity', maybeCollegeOrUniversity, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CollegeOrUniversity;
}
