 ...
 * schema:ComedyClub
 *
 * This differs from ComedyClub because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComedyClubOrSubClass =
  | ComedyClub
  ;

/**
 * schema:ComedyClub - Validation schema (w/ JOI)
 */
export const ComedyClubJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComedyClub').required(),
 ...
 * schema:ComedyClub - Validation schema (w/ JOI)
 *
 * This differs from ComedyClubJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComedyClubOrSubClassJoiSchema = Joi.alternatives().try([
  ComedyClubJoiSchema,
]);

/**
 * Runtime validator for schema:ComedyClub.
 *
 * If some data has a structure which matches a schema:ComedyClub, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComedyClub = ComedyClub.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComedyClub instanceof OaValidationError) {
 *   // From this point on, `maybeComedyClub` will have type `OaValidationError`
 *   const error = maybeComedyClub;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComedyClub` will have type `ComedyClub`
 * const comedyClub = maybeComedyClub;
 * ```
 */
export function validateComedyClub(maybeComedyClub: unknown): ComedyClub | OaValidationError {
  const { value, error } = ComedyClubJoiSchema.validate(maybeComedyClub);
  if (error) {
    return new OaValidationError('ComedyClub', maybeComedyClub, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComedyClub;
}
