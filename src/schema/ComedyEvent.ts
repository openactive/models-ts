 ...
 * schema:ComedyEvent
 *
 * This differs from ComedyEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComedyEventOrSubClass =
  | ComedyEvent
  ;

/**
 * schema:ComedyEvent - Validation schema (w/ JOI)
 */
export const ComedyEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComedyEvent').required(),
 ...
 * schema:ComedyEvent - Validation schema (w/ JOI)
 *
 * This differs from ComedyEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComedyEventOrSubClassJoiSchema = Joi.alternatives().try([
  ComedyEventJoiSchema,
]);

/**
 * Runtime validator for schema:ComedyEvent.
 *
 * If some data has a structure which matches a schema:ComedyEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComedyEvent = ComedyEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComedyEvent instanceof OaValidationError) {
 *   // From this point on, `maybeComedyEvent` will have type `OaValidationError`
 *   const error = maybeComedyEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComedyEvent` will have type `ComedyEvent`
 * const comedyEvent = maybeComedyEvent;
 * ```
 */
export function validateComedyEvent(maybeComedyEvent: unknown): ComedyEvent | OaValidationError {
  const { value, error } = ComedyEventJoiSchema.validate(maybeComedyEvent);
  if (error) {
    return new OaValidationError('ComedyEvent', maybeComedyEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComedyEvent;
}
