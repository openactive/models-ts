...
export type ComicCoverArt = {
   ...
   */
  penciler?: schema.PersonOrSubClass | string;
  /**
   * The primary artist for a work
   *     	in a medium other than pencils or digital line art--for example, if the
   *     	primary artwork is done in watercolors or digital paints.
   */
  artist?: schema.PersonOrSubClass | string;
  /**
   * The individual who traces over the pencil drawings in ink after pencils are complete.
   */
  inker?: schema.PersonOrSubClass | string;
  /**
   * The individual who adds lettering, including speech balloons and sound effects, to artwork.
   */
  letterer?: schema.PersonOrSubClass | string;
  /**
   * The individual who adds color to inked drawings.
   */
  colorist?: schema.PersonOrSubClass | string;
  /**
   ...
   */
  artEdition?: string | number;
};

/**
 * schema:ComicCoverArt
 *
 * This differs from ComicCoverArt because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComicCoverArtOrSubClass =
  | ComicCoverArt
  ;

/**
 * schema:ComicCoverArt - Validation schema (w/ JOI)
 */
export const ComicCoverArtJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComicCoverArt').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  penciler: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  artist: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  inker: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  letterer: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  colorist: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
  ...
  artworkSurface: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  artform: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  ...
  artMedium: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  surface: Joi.alternatives().try(Joi.string().uri(), Joi.string()),
  artEdition: Joi.alternatives().try(Joi.string(), Joi.number().integer()),
});

/**
 * schema:ComicCoverArt - Validation schema (w/ JOI)
 *
 * This differs from ComicCoverArtJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComicCoverArtOrSubClassJoiSchema = Joi.alternatives().try([
  ComicCoverArtJoiSchema,
]);

/**
 * Runtime validator for schema:ComicCoverArt.
 *
 * If some data has a structure which matches a schema:ComicCoverArt, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComicCoverArt = ComicCoverArt.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComicCoverArt instanceof OaValidationError) {
 *   // From this point on, `maybeComicCoverArt` will have type `OaValidationError`
 *   const error = maybeComicCoverArt;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComicCoverArt` will have type `ComicCoverArt`
 * const comicCoverArt = maybeComicCoverArt;
 * ```
 */
export function validateComicCoverArt(maybeComicCoverArt: unknown): ComicCoverArt | OaValidationError {
  const { value, error } = ComicCoverArtJoiSchema.validate(maybeComicCoverArt);
  if (error) {
    return new OaValidationError('ComicCoverArt', maybeComicCoverArt, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComicCoverArt;
}
