 ...
 * schema:ComicIssue
 *
 * This differs from ComicIssue because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComicIssueOrSubClass =
  | ComicIssue
  ;

/**
 * schema:ComicIssue - Validation schema (w/ JOI)
 */
export const ComicIssueJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComicIssue').required(),
  ...
  issueNumber: Joi.alternatives().try(Joi.number().integer(), Joi.string()),
 ...
 * schema:ComicIssue - Validation schema (w/ JOI)
 *
 * This differs from ComicIssueJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComicIssueOrSubClassJoiSchema = Joi.alternatives().try([
  ComicIssueJoiSchema,
]);

/**
 * Runtime validator for schema:ComicIssue.
 *
 * If some data has a structure which matches a schema:ComicIssue, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeComicIssue` will have type `ComicIssue`
 * const comicIssue = maybeComicIssue;
 * ```
 */
export function validateComicIssue(maybeComicIssue: unknown): ComicIssue | OaValidationError {
  const { value, error } = ComicIssueJoiSchema.validate(maybeComicIssue);
  if (error) {
    return new OaValidationError('ComicIssue', maybeComicIssue, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComicIssue;
}
