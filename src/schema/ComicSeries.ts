 ...
 * schema:ComicSeries
 *
 * This differs from ComicSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComicSeriesOrSubClass =
  | ComicSeries
  ;

/**
 * schema:ComicSeries - Validation schema (w/ JOI)
 */
export const ComicSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComicSeries').required(),
 ...
 * schema:ComicSeries - Validation schema (w/ JOI)
 *
 * This differs from ComicSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComicSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  ComicSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:ComicSeries.
 *
 * If some data has a structure which matches a schema:ComicSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComicSeries = ComicSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComicSeries instanceof OaValidationError) {
 *   // From this point on, `maybeComicSeries` will have type `OaValidationError`
 *   const error = maybeComicSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComicSeries` will have type `ComicSeries`
 * const comicSeries = maybeComicSeries;
 * ```
 */
export function validateComicSeries(maybeComicSeries: unknown): ComicSeries | OaValidationError {
  const { value, error } = ComicSeriesJoiSchema.validate(maybeComicSeries);
  if (error) {
    return new OaValidationError('ComicSeries', maybeComicSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComicSeries;
}
