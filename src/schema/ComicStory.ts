 ...
 * schema:ComicStory
 *
 * This differs from ComicStory because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComicStoryOrSubClass =
  | ComicStory
  | schema.ComicCoverArtOrSubClass
  ;

/**
 * schema:ComicStory - Validation schema (w/ JOI)
 */
export const ComicStoryJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComicStory').required(),
 ...
 * schema:ComicStory - Validation schema (w/ JOI)
 *
 * This differs from ComicStoryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComicStoryOrSubClassJoiSchema = Joi.alternatives().try([
  ComicStoryJoiSchema,
  Joi.lazy(() => schema.ComicCoverArtOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:ComicStory.
 *
 * If some data has a structure which matches a schema:ComicStory, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComicStory = ComicStory.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComicStory instanceof OaValidationError) {
 *   // From this point on, `maybeComicStory` will have type `OaValidationError`
 *   const error = maybeComicStory;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComicStory` will have type `ComicStory`
 * const comicStory = maybeComicStory;
 * ```
 */
export function validateComicStory(maybeComicStory: unknown): ComicStory | OaValidationError {
  const { value, error } = ComicStoryJoiSchema.validate(maybeComicStory);
  if (error) {
    return new OaValidationError('ComicStory', maybeComicStory, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComicStory;
}
