 ...
 * schema:Comment
 *
 * This differs from Comment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CommentOrSubClass =
  | Comment
  | schema.CorrectionCommentOrSubClass
  | schema.QuestionOrSubClass
  | schema.AnswerOrSubClass
  ;

/**
 * schema:Comment - Validation schema (w/ JOI)
 */
 ...
 * schema:Comment - Validation schema (w/ JOI)
 *
 * This differs from CommentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CommentOrSubClassJoiSchema = Joi.alternatives().try([
  CommentJoiSchema,
  Joi.lazy(() => schema.CorrectionCommentOrSubClassJoiSchema),
  Joi.lazy(() => schema.QuestionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AnswerOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Comment.
 *
 * If some data has a structure which matches a schema:Comment, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComment = Comment.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComment instanceof OaValidationError) {
 *   // From this point on, `maybeComment` will have type `OaValidationError`
 *   const error = maybeComment;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComment` will have type `Comment`
 * const comment = maybeComment;
 * ```
 */
export function validateComment(maybeComment: unknown): Comment | OaValidationError {
  const { value, error } = CommentJoiSchema.validate(maybeComment);
  if (error) {
    return new OaValidationError('Comment', maybeComment, error);
  }
