...
export type CommentAction = {
   ...
   */
  resultComment?: schema.CommentOrSubClass | string;
  /**
 ...
 * schema:CommentAction
 *
 * This differs from CommentAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CommentActionOrSubClass =
  | CommentAction
  ;

/**
 * schema:CommentAction - Validation schema (w/ JOI)
 */
export const CommentActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('CommentAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  resultComment: Joi.alternatives().try(Joi.lazy(() => schema.CommentOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:CommentAction - Validation schema (w/ JOI)
 *
 * This differs from CommentActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CommentActionOrSubClassJoiSchema = Joi.alternatives().try([
  CommentActionJoiSchema,
]);

/**
 * Runtime validator for schema:CommentAction.
 *
 * If some data has a structure which matches a schema:CommentAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCommentAction = CommentAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCommentAction instanceof OaValidationError) {
 *   // From this point on, `maybeCommentAction` will have type `OaValidationError`
 *   const error = maybeCommentAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCommentAction` will have type `CommentAction`
 * const commentAction = maybeCommentAction;
 * ```
 */
export function validateCommentAction(maybeCommentAction: unknown): CommentAction | OaValidationError {
  const { value, error } = CommentActionJoiSchema.validate(maybeCommentAction);
  if (error) {
    return new OaValidationError('CommentAction', maybeCommentAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CommentAction;
}
