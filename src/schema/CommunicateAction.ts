 ...
 * schema:CommunicateAction
 *
 * This differs from CommunicateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CommunicateActionOrSubClass =
  | CommunicateAction
  | schema.AskActionOrSubClass
  | schema.CommentActionOrSubClass
  | schema.InformActionOrSubClass
  | schema.CheckInActionOrSubClass
  | schema.CheckOutActionOrSubClass
  | schema.InviteActionOrSubClass
  | schema.ReplyActionOrSubClass
  | schema.ShareActionOrSubClass
  ;

/**
 * schema:CommunicateAction - Validation schema (w/ JOI)
 */
export const CommunicateActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CommunicateActionOrSubClassJoiSchema = Joi.alternatives().try([
  CommunicateActionJoiSchema,
 ...
 * ```
 */
export function validateCommunicateAction(maybeCommunicateAction: unknown): CommunicateAction | OaValidationError {
  const { value, error } = CommunicateActionJoiSchema.validate(maybeCommunicateAction);
  if (error) {
    return new OaValidationError('CommunicateAction', maybeCommunicateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CommunicateAction;
}
