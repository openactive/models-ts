 ...
 * schema:CompleteDataFeed
 *
 * This differs from CompleteDataFeed because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CompleteDataFeedOrSubClass =
  | CompleteDataFeed
  ;

/**
 * schema:CompleteDataFeed - Validation schema (w/ JOI)
 */
export const CompleteDataFeedJoiSchema = Joi.object({
  '@type': Joi.string().valid('CompleteDataFeed').required(),
 ...
 * schema:CompleteDataFeed - Validation schema (w/ JOI)
 *
 * This differs from CompleteDataFeedJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CompleteDataFeedOrSubClassJoiSchema = Joi.alternatives().try([
  CompleteDataFeedJoiSchema,
]);

/**
 * Runtime validator for schema:CompleteDataFeed.
 *
 * If some data has a structure which matches a schema:CompleteDataFeed, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCompleteDataFeed` will have type `CompleteDataFeed`
 * const completeDataFeed = maybeCompleteDataFeed;
 * ```
 */
export function validateCompleteDataFeed(maybeCompleteDataFeed: unknown): CompleteDataFeed | OaValidationError {
  const { value, error } = CompleteDataFeedJoiSchema.validate(maybeCompleteDataFeed);
  if (error) {
    return new OaValidationError('CompleteDataFeed', maybeCompleteDataFeed, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CompleteDataFeed;
}
