 ...
 * schema:ComputerStore
 *
 * This differs from ComputerStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ComputerStoreOrSubClass =
  | ComputerStore
  ;

/**
 * schema:ComputerStore - Validation schema (w/ JOI)
 */
export const ComputerStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('ComputerStore').required(),
 ...
 * schema:ComputerStore - Validation schema (w/ JOI)
 *
 * This differs from ComputerStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ComputerStoreOrSubClassJoiSchema = Joi.alternatives().try([
  ComputerStoreJoiSchema,
]);

/**
 * Runtime validator for schema:ComputerStore.
 *
 * If some data has a structure which matches a schema:ComputerStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeComputerStore = ComputerStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeComputerStore instanceof OaValidationError) {
 *   // From this point on, `maybeComputerStore` will have type `OaValidationError`
 *   const error = maybeComputerStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeComputerStore` will have type `ComputerStore`
 * const computerStore = maybeComputerStore;
 * ```
 */
export function validateComputerStore(maybeComputerStore: unknown): ComputerStore | OaValidationError {
  const { value, error } = ComputerStoreJoiSchema.validate(maybeComputerStore);
  if (error) {
    return new OaValidationError('ComputerStore', maybeComputerStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ComputerStore;
}
