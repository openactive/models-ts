 ...
 * schema:ConfirmAction
 *
 * This differs from ConfirmAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ConfirmActionOrSubClass =
  | ConfirmAction
  ;

/**
 * schema:ConfirmAction - Validation schema (w/ JOI)
 */
export const ConfirmActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ConfirmAction').required(),
 ...
 * schema:ConfirmAction - Validation schema (w/ JOI)
 *
 * This differs from ConfirmActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ConfirmActionOrSubClassJoiSchema = Joi.alternatives().try([
  ConfirmActionJoiSchema,
]);

/**
 * Runtime validator for schema:ConfirmAction.
 *
 * If some data has a structure which matches a schema:ConfirmAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeConfirmAction = ConfirmAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeConfirmAction instanceof OaValidationError) {
 *   // From this point on, `maybeConfirmAction` will have type `OaValidationError`
 *   const error = maybeConfirmAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeConfirmAction` will have type `ConfirmAction`
 * const confirmAction = maybeConfirmAction;
 * ```
 */
export function validateConfirmAction(maybeConfirmAction: unknown): ConfirmAction | OaValidationError {
  const { value, error } = ConfirmActionJoiSchema.validate(maybeConfirmAction);
  if (error) {
    return new OaValidationError('ConfirmAction', maybeConfirmAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ConfirmAction;
}
