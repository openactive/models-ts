 ...
 * schema:Consortium
 *
 * This differs from Consortium because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ConsortiumOrSubClass =
  | Consortium
  ;

/**
 * schema:Consortium - Validation schema (w/ JOI)
 */
export const ConsortiumJoiSchema = Joi.object({
  '@type': Joi.string().valid('Consortium').required(),
 ...
 * schema:Consortium - Validation schema (w/ JOI)
 *
 * This differs from ConsortiumJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ConsortiumOrSubClassJoiSchema = Joi.alternatives().try([
  ConsortiumJoiSchema,
]);

/**
 * Runtime validator for schema:Consortium.
 *
 * If some data has a structure which matches a schema:Consortium, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeConsortium` will have type `Consortium`
 * const consortium = maybeConsortium;
 * ```
 */
export function validateConsortium(maybeConsortium: unknown): Consortium | OaValidationError {
  const { value, error } = ConsortiumJoiSchema.validate(maybeConsortium);
  if (error) {
    return new OaValidationError('Consortium', maybeConsortium, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Consortium;
}
