 ...
 * schema:ConsumeAction
 *
 * This differs from ConsumeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ConsumeActionOrSubClass =
  | ConsumeAction
  | schema.ListenActionOrSubClass
  | schema.DrinkActionOrSubClass
  | schema.WatchActionOrSubClass
  | schema.ViewActionOrSubClass
  | schema.EatActionOrSubClass
  | schema.ReadActionOrSubClass
  | schema.InstallActionOrSubClass
  | schema.UseActionOrSubClass
  ;

/**
 * schema:ConsumeAction - Validation schema (w/ JOI)
 */
export const ConsumeActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ConsumeActionOrSubClassJoiSchema = Joi.alternatives().try([
  ConsumeActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeConsumeAction = ConsumeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateConsumeAction(maybeConsumeAction: unknown): ConsumeAction | OaValidationError {
  const { value, error } = ConsumeActionJoiSchema.validate(maybeConsumeAction);
  if (error) {
    return new OaValidationError('ConsumeAction', maybeConsumeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ConsumeAction;
}
