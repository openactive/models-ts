 ...
 * schema:ContactPage
 *
 * This differs from ContactPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ContactPageOrSubClass =
  | ContactPage
  ;

/**
 * schema:ContactPage - Validation schema (w/ JOI)
 */
export const ContactPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('ContactPage').required(),
 ...
 * schema:ContactPage - Validation schema (w/ JOI)
 *
 * This differs from ContactPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ContactPageOrSubClassJoiSchema = Joi.alternatives().try([
  ContactPageJoiSchema,
]);

/**
 * Runtime validator for schema:ContactPage.
 *
 * If some data has a structure which matches a schema:ContactPage, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeContactPage = ContactPage.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeContactPage instanceof OaValidationError) {
 *   // From this point on, `maybeContactPage` will have type `OaValidationError`
 *   const error = maybeContactPage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeContactPage` will have type `ContactPage`
 * const contactPage = maybeContactPage;
 * ```
 */
export function validateContactPage(maybeContactPage: unknown): ContactPage | OaValidationError {
  const { value, error } = ContactPageJoiSchema.validate(maybeContactPage);
  if (error) {
    return new OaValidationError('ContactPage', maybeContactPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ContactPage;
}
