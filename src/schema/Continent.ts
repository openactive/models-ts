 ...
 * schema:Continent
 *
 * This differs from Continent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ContinentOrSubClass =
  | Continent
  ;
 ...
 */
export const ContinentJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ContinentOrSubClassJoiSchema = Joi.alternatives().try([
  ContinentJoiSchema,
]);
 ...
 * ```
 */
export function validateContinent(maybeContinent: unknown): Continent | OaValidationError {
  const { value, error } = ContinentJoiSchema.validate(maybeContinent);
  if (error) {
    return new OaValidationError('Continent', maybeContinent, error);
  }
