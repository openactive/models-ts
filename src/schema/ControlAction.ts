 ...
 * schema:ControlAction
 *
 * This differs from ControlAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ControlActionOrSubClass =
  | ControlAction
  | schema.ActivateActionOrSubClass
  | schema.SuspendActionOrSubClass
  | schema.DeactivateActionOrSubClass
  | schema.ResumeActionOrSubClass
  ;

/**
 * schema:ControlAction - Validation schema (w/ JOI)
 */
export const ControlActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ControlAction').required(),
 ...
 * schema:ControlAction - Validation schema (w/ JOI)
 *
 * This differs from ControlActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ControlActionOrSubClassJoiSchema = Joi.alternatives().try([
  ControlActionJoiSchema,
  Joi.lazy(() => schema.ActivateActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.SuspendActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.DeactivateActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.ResumeActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:ControlAction.
 *
 * If some data has a structure which matches a schema:ControlAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeControlAction = ControlAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeControlAction instanceof OaValidationError) {
 *   // From this point on, `maybeControlAction` will have type `OaValidationError`
 *   const error = maybeControlAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeControlAction` will have type `ControlAction`
 * const controlAction = maybeControlAction;
 * ```
 */
export function validateControlAction(maybeControlAction: unknown): ControlAction | OaValidationError {
  const { value, error } = ControlActionJoiSchema.validate(maybeControlAction);
  if (error) {
    return new OaValidationError('ControlAction', maybeControlAction, error);
  }
