 ...
 * schema:ConvenienceStore
 *
 * This differs from ConvenienceStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ConvenienceStoreOrSubClass =
  | ConvenienceStore
  ;

/**
 * schema:ConvenienceStore - Validation schema (w/ JOI)
 */
export const ConvenienceStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('ConvenienceStore').required(),
 ...
 * schema:ConvenienceStore - Validation schema (w/ JOI)
 *
 * This differs from ConvenienceStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ConvenienceStoreOrSubClassJoiSchema = Joi.alternatives().try([
  ConvenienceStoreJoiSchema,
]);

/**
 * Runtime validator for schema:ConvenienceStore.
 *
 * If some data has a structure which matches a schema:ConvenienceStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeConvenienceStore = ConvenienceStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeConvenienceStore instanceof OaValidationError) {
 *   // From this point on, `maybeConvenienceStore` will have type `OaValidationError`
 *   const error = maybeConvenienceStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeConvenienceStore` will have type `ConvenienceStore`
 * const convenienceStore = maybeConvenienceStore;
 * ```
 */
export function validateConvenienceStore(maybeConvenienceStore: unknown): ConvenienceStore | OaValidationError {
  const { value, error } = ConvenienceStoreJoiSchema.validate(maybeConvenienceStore);
  if (error) {
    return new OaValidationError('ConvenienceStore', maybeConvenienceStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ConvenienceStore;
}
