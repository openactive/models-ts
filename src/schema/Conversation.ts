 ...
 * schema:Conversation
 *
 * This differs from Conversation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ConversationOrSubClass =
  | Conversation
  ;

/**
 * schema:Conversation - Validation schema (w/ JOI)
 */
export const ConversationJoiSchema = Joi.object({
  '@type': Joi.string().valid('Conversation').required(),
 ...
 * schema:Conversation - Validation schema (w/ JOI)
 *
 * This differs from ConversationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ConversationOrSubClassJoiSchema = Joi.alternatives().try([
  ConversationJoiSchema,
]);

/**
 * Runtime validator for schema:Conversation.
 *
 * If some data has a structure which matches a schema:Conversation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeConversation = Conversation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeConversation instanceof OaValidationError) {
 *   // From this point on, `maybeConversation` will have type `OaValidationError`
 *   const error = maybeConversation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeConversation` will have type `Conversation`
 * const conversation = maybeConversation;
 * ```
 */
export function validateConversation(maybeConversation: unknown): Conversation | OaValidationError {
  const { value, error } = ConversationJoiSchema.validate(maybeConversation);
  if (error) {
    return new OaValidationError('Conversation', maybeConversation, error);
  }
