...
export type CookAction = {
   ...
   */
  recipe?: schema.RecipeOrSubClass | string;
  /**
 ...
 * schema:CookAction
 *
 * This differs from CookAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CookActionOrSubClass =
  | CookAction
  ;
 ...
 */
export const CookActionJoiSchema = Joi.object({
  ...
  foodEstablishment: Joi.alternatives().try(Joi.lazy(() => schema.FoodEstablishmentOrSubClassJoiSchema), Joi.lazy(() => schema.PlaceOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const CookActionOrSubClassJoiSchema = Joi.alternatives().try([
  CookActionJoiSchema,
]);
 ...
 * ```
 */
export function validateCookAction(maybeCookAction: unknown): CookAction | OaValidationError {
  const { value, error } = CookActionJoiSchema.validate(maybeCookAction);
  if (error) {
    return new OaValidationError('CookAction', maybeCookAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CookAction;
}
