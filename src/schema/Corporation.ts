 ...
 * schema:Corporation
 *
 * This differs from Corporation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CorporationOrSubClass =
  | Corporation
  ;

/**
 * schema:Corporation - Validation schema (w/ JOI)
 */
export const CorporationJoiSchema = Joi.object({
  '@type': Joi.string().valid('Corporation').required(),
 ...
 * schema:Corporation - Validation schema (w/ JOI)
 *
 * This differs from CorporationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CorporationOrSubClassJoiSchema = Joi.alternatives().try([
  CorporationJoiSchema,
]);

/**
 * Runtime validator for schema:Corporation.
 *
 * If some data has a structure which matches a schema:Corporation, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCorporation` will have type `Corporation`
 * const corporation = maybeCorporation;
 * ```
 */
export function validateCorporation(maybeCorporation: unknown): Corporation | OaValidationError {
  const { value, error } = CorporationJoiSchema.validate(maybeCorporation);
  if (error) {
    return new OaValidationError('Corporation', maybeCorporation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Corporation;
}
