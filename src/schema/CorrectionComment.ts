 ...
 * schema:CorrectionComment
 *
 * This differs from CorrectionComment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CorrectionCommentOrSubClass =
  | CorrectionComment
  ;

/**
 * schema:CorrectionComment - Validation schema (w/ JOI)
 */
export const CorrectionCommentJoiSchema = Joi.object({
  '@type': Joi.string().valid('CorrectionComment').required(),
 ...
 * schema:CorrectionComment - Validation schema (w/ JOI)
 *
 * This differs from CorrectionCommentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CorrectionCommentOrSubClassJoiSchema = Joi.alternatives().try([
  CorrectionCommentJoiSchema,
]);

/**
 * Runtime validator for schema:CorrectionComment.
 *
 * If some data has a structure which matches a schema:CorrectionComment, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCorrectionComment = CorrectionComment.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCorrectionComment instanceof OaValidationError) {
 *   // From this point on, `maybeCorrectionComment` will have type `OaValidationError`
 *   const error = maybeCorrectionComment;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCorrectionComment` will have type `CorrectionComment`
 * const correctionComment = maybeCorrectionComment;
 * ```
 */
export function validateCorrectionComment(maybeCorrectionComment: unknown): CorrectionComment | OaValidationError {
  const { value, error } = CorrectionCommentJoiSchema.validate(maybeCorrectionComment);
  if (error) {
    return new OaValidationError('CorrectionComment', maybeCorrectionComment, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CorrectionComment;
}
