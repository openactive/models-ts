 ...
 * schema:Country
 *
 * This differs from Country because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CountryOrSubClass =
  | Country
  ;

/**
 * schema:Country - Validation schema (w/ JOI)
 */
export const CountryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Country').required(),
 ...
 * schema:Country - Validation schema (w/ JOI)
 *
 * This differs from CountryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CountryOrSubClassJoiSchema = Joi.alternatives().try([
  CountryJoiSchema,
]);

/**
 * Runtime validator for schema:Country.
 *
 * If some data has a structure which matches a schema:Country, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCountry = Country.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCountry instanceof OaValidationError) {
 *   // From this point on, `maybeCountry` will have type `OaValidationError`
 *   const error = maybeCountry;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCountry` will have type `Country`
 * const country = maybeCountry;
 * ```
 */
export function validateCountry(maybeCountry: unknown): Country | OaValidationError {
  const { value, error } = CountryJoiSchema.validate(maybeCountry);
  if (error) {
    return new OaValidationError('Country', maybeCountry, error);
  }
