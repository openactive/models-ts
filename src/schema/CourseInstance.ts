...
export type CourseInstance = {
   ...
   */
  instructor?: schema.PersonOrSubClass | string;
  /**
 ...
 * schema:CourseInstance
 *
 * This differs from CourseInstance because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CourseInstanceOrSubClass =
  | CourseInstance
  ;

/**
 * schema:CourseInstance - Validation schema (w/ JOI)
 */
export const CourseInstanceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CourseInstanceOrSubClassJoiSchema = Joi.alternatives().try([
  CourseInstanceJoiSchema,
]);

/**
 * Runtime validator for schema:CourseInstance.
 *
 * If some data has a structure which matches a schema:CourseInstance, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCourseInstance = CourseInstance.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateCourseInstance(maybeCourseInstance: unknown): CourseInstance | OaValidationError {
  const { value, error } = CourseInstanceJoiSchema.validate(maybeCourseInstance);
  if (error) {
    return new OaValidationError('CourseInstance', maybeCourseInstance, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CourseInstance;
}
