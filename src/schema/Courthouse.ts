 ...
 * schema:Courthouse
 *
 * This differs from Courthouse because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CourthouseOrSubClass =
  | Courthouse
  ;
 ...
 */
export const CourthouseJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CourthouseOrSubClassJoiSchema = Joi.alternatives().try([
  CourthouseJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeCourthouse = validateCourthouse(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateCourthouse(maybeCourthouse: unknown): Courthouse | OaValidationError {
  const { value, error } = CourthouseJoiSchema.validate(maybeCourthouse);
  if (error) {
    return new OaValidationError('Courthouse', maybeCourthouse, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Courthouse;
}
