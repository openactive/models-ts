 ...
 * schema:CoverArt
 *
 * This differs from CoverArt because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CoverArtOrSubClass =
  | CoverArt
  | schema.ComicCoverArtOrSubClass
  ;

/**
 * schema:CoverArt - Validation schema (w/ JOI)
 */
export const CoverArtJoiSchema = Joi.object({
  '@type': Joi.string().valid('CoverArt').required(),
 ...
 * schema:CoverArt - Validation schema (w/ JOI)
 *
 * This differs from CoverArtJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CoverArtOrSubClassJoiSchema = Joi.alternatives().try([
  CoverArtJoiSchema,
  Joi.lazy(() => schema.ComicCoverArtOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:CoverArt.
 *
 * If some data has a structure which matches a schema:CoverArt, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCoverArt` will have type `CoverArt`
 * const coverArt = maybeCoverArt;
 * ```
 */
export function validateCoverArt(maybeCoverArt: unknown): CoverArt | OaValidationError {
  const { value, error } = CoverArtJoiSchema.validate(maybeCoverArt);
  if (error) {
    return new OaValidationError('CoverArt', maybeCoverArt, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CoverArt;
}
