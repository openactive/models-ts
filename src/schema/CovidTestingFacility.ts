 ...
 */
export type CovidTestingFacility = {
   ...
   */
  availableService?: schema.MedicalTestOrSubClass | schema.MedicalProcedureOrSubClass | schema.MedicalTherapyOrSubClass | string;
  /**
 ...
 * schema:CovidTestingFacility
 *
 * This differs from CovidTestingFacility because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CovidTestingFacilityOrSubClass =
  | CovidTestingFacility
  ;

/**
 * schema:CovidTestingFacility - Validation schema (w/ JOI)
 */
export const CovidTestingFacilityJoiSchema = Joi.object({
  '@type': Joi.string().valid('CovidTestingFacility').required(),
  ...
  availableService: Joi.alternatives().try(Joi.lazy(() => schema.MedicalTestOrSubClassJoiSchema), Joi.lazy(() => schema.MedicalProcedureOrSubClassJoiSchema), Joi.lazy(() => schema.MedicalTherapyOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:CovidTestingFacility - Validation schema (w/ JOI)
 *
 * This differs from CovidTestingFacilityJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CovidTestingFacilityOrSubClassJoiSchema = Joi.alternatives().try([
  CovidTestingFacilityJoiSchema,
]);

/**
 * Runtime validator for schema:CovidTestingFacility.
 *
 * If some data has a structure which matches a schema:CovidTestingFacility, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCovidTestingFacility = validateCovidTestingFacility(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCovidTestingFacility instanceof OaValidationError) {
 *   // From this point on, `maybeCovidTestingFacility` will have type `OaValidationError`
 *   const error = maybeCovidTestingFacility;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCovidTestingFacility` will have type `CovidTestingFacility`
 * const covidTestingFacility = maybeCovidTestingFacility;
 * ```
 */
export function validateCovidTestingFacility(maybeCovidTestingFacility: unknown): CovidTestingFacility | OaValidationError {
  const { value, error } = CovidTestingFacilityJoiSchema.validate(maybeCovidTestingFacility);
  if (error) {
    return new OaValidationError('CovidTestingFacility', maybeCovidTestingFacility, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CovidTestingFacility;
}
