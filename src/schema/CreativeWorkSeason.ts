 ...
 * schema:CreativeWorkSeason
 *
 * This differs from CreativeWorkSeason because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CreativeWorkSeasonOrSubClass =
  | CreativeWorkSeason
  | schema.RadioSeasonOrSubClass
  | schema.TVSeasonOrSubClass
  | schema.PodcastSeasonOrSubClass
  ;

/**
 * schema:CreativeWorkSeason - Validation schema (w/ JOI)
 */
export const CreativeWorkSeasonJoiSchema = Joi.object({
  '@type': Joi.string().valid('CreativeWorkSeason').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  seasonNumber: Joi.alternatives().try(Joi.number().integer(), Joi.string()),
 ...
 * schema:CreativeWorkSeason - Validation schema (w/ JOI)
 *
 * This differs from CreativeWorkSeasonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CreativeWorkSeasonOrSubClassJoiSchema = Joi.alternatives().try([
  CreativeWorkSeasonJoiSchema,
  Joi.lazy(() => schema.RadioSeasonOrSubClassJoiSchema),
  Joi.lazy(() => schema.TVSeasonOrSubClassJoiSchema),
  Joi.lazy(() => schema.PodcastSeasonOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:CreativeWorkSeason.
 *
 * If some data has a structure which matches a schema:CreativeWorkSeason, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCreativeWorkSeason` will have type `CreativeWorkSeason`
 * const creativeWorkSeason = maybeCreativeWorkSeason;
 * ```
 */
export function validateCreativeWorkSeason(maybeCreativeWorkSeason: unknown): CreativeWorkSeason | OaValidationError {
  const { value, error } = CreativeWorkSeasonJoiSchema.validate(maybeCreativeWorkSeason);
  if (error) {
    return new OaValidationError('CreativeWorkSeason', maybeCreativeWorkSeason, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CreativeWorkSeason;
}
