 ...
 * schema:CreativeWorkSeries
 *
 * This differs from CreativeWorkSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CreativeWorkSeriesOrSubClass =
  | CreativeWorkSeries
  | schema.BookSeriesOrSubClass
  | schema.VideoGameSeriesOrSubClass
  | schema.TVSeriesOrSubClass
  | schema.RadioSeriesOrSubClass
  | schema.PodcastSeriesOrSubClass
  | schema.MovieSeriesOrSubClass
  | schema.PeriodicalOrSubClass
  ;

/**
 * schema:CreativeWorkSeries - Validation schema (w/ JOI)
 */
export const CreativeWorkSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('CreativeWorkSeries').required(),
 ...
 * schema:CreativeWorkSeries - Validation schema (w/ JOI)
 *
 * This differs from CreativeWorkSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CreativeWorkSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  CreativeWorkSeriesJoiSchema,
  Joi.lazy(() => schema.BookSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.VideoGameSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.TVSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.RadioSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.PodcastSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.MovieSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.PeriodicalOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:CreativeWorkSeries.
 *
 * If some data has a structure which matches a schema:CreativeWorkSeries, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCreativeWorkSeries` will have type `CreativeWorkSeries`
 * const creativeWorkSeries = maybeCreativeWorkSeries;
 * ```
 */
export function validateCreativeWorkSeries(maybeCreativeWorkSeries: unknown): CreativeWorkSeries | OaValidationError {
  const { value, error } = CreativeWorkSeriesJoiSchema.validate(maybeCreativeWorkSeries);
  if (error) {
    return new OaValidationError('CreativeWorkSeries', maybeCreativeWorkSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CreativeWorkSeries;
}
