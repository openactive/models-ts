 ...
 * etc.
 */
export type CreditCardOrSubClass =
  | CreditCard
  ;
 ...
 */
export const CreditCardJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CreditCardOrSubClassJoiSchema = Joi.alternatives().try([
  CreditCardJoiSchema,
]);
 ...
 * ```
 */
export function validateCreditCard(maybeCreditCard: unknown): CreditCard | OaValidationError {
  const { value, error } = CreditCardJoiSchema.validate(maybeCreditCard);
  if (error) {
    return new OaValidationError('CreditCard', maybeCreditCard, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CreditCard;
}
