 ...
 * schema:Crematorium
 *
 * This differs from Crematorium because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CrematoriumOrSubClass =
  | Crematorium
  ;

/**
 * schema:Crematorium - Validation schema (w/ JOI)
 */
export const CrematoriumJoiSchema = Joi.object({
  '@type': Joi.string().valid('Crematorium').required(),
 ...
 * schema:Crematorium - Validation schema (w/ JOI)
 *
 * This differs from CrematoriumJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CrematoriumOrSubClassJoiSchema = Joi.alternatives().try([
  CrematoriumJoiSchema,
]);

/**
 * Runtime validator for schema:Crematorium.
 *
 * If some data has a structure which matches a schema:Crematorium, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeCrematorium` will have type `Crematorium`
 * const crematorium = maybeCrematorium;
 * ```
 */
export function validateCrematorium(maybeCrematorium: unknown): Crematorium | OaValidationError {
  const { value, error } = CrematoriumJoiSchema.validate(maybeCrematorium);
  if (error) {
    return new OaValidationError('Crematorium', maybeCrematorium, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Crematorium;
}
