 ...
 * schema:CriticReview
 *
 * This differs from CriticReview because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CriticReviewOrSubClass =
  | CriticReview
  | schema.ReviewNewsArticleOrSubClass
  ;

/**
 * schema:CriticReview - Validation schema (w/ JOI)
 */
export const CriticReviewJoiSchema = Joi.object({
  '@type': Joi.string().valid('CriticReview').required(),
 ...
 * schema:CriticReview - Validation schema (w/ JOI)
 *
 * This differs from CriticReviewJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const CriticReviewOrSubClassJoiSchema = Joi.alternatives().try([
  CriticReviewJoiSchema,
  Joi.lazy(() => schema.ReviewNewsArticleOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:CriticReview.
 *
 * If some data has a structure which matches a schema:CriticReview, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeCriticReview = validateCriticReview(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeCriticReview instanceof OaValidationError) {
 *   // From this point on, `maybeCriticReview` will have type `OaValidationError`
 *   const error = maybeCriticReview;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeCriticReview` will have type `CriticReview`
 * const criticReview = maybeCriticReview;
 * ```
 */
export function validateCriticReview(maybeCriticReview: unknown): CriticReview | OaValidationError {
  const { value, error } = CriticReviewJoiSchema.validate(maybeCriticReview);
  if (error) {
    return new OaValidationError('CriticReview', maybeCriticReview, error);
  }
