 ...
 */
export type CurrencyConversionService = {
 ...
 * schema:CurrencyConversionService
 *
 * This differs from CurrencyConversionService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type CurrencyConversionServiceOrSubClass =
 ...
 */
export const CurrencyConversionServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const CurrencyConversionServiceOrSubClassJoiSchema = Joi.alternatives().try([
  CurrencyConversionServiceJoiSchema,
]);
 ...
 * ```
 */
export function validateCurrencyConversionService(maybeCurrencyConversionService: unknown): CurrencyConversionService | OaValidationError {
  const { value, error } = CurrencyConversionServiceJoiSchema.validate(maybeCurrencyConversionService);
  if (error) {
    return new OaValidationError('CurrencyConversionService', maybeCurrencyConversionService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as CurrencyConversionService;
}
