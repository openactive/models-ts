...
export type DDxElement = {
   ...
   */
  distinguishingSign?: schema.MedicalSignOrSymptomOrSubClass | string;
  /**
 ...
 * schema:DDxElement
 *
 * This differs from DDxElement because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DDxElementOrSubClass =
  | DDxElement
  ;

/**
 * schema:DDxElement - Validation schema (w/ JOI)
 */
export const DDxElementJoiSchema = Joi.object({
  '@type': Joi.string().valid('DDxElement').required(),
  ...
  distinguishingSign: Joi.alternatives().try(Joi.lazy(() => schema.MedicalSignOrSymptomOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:DDxElement - Validation schema (w/ JOI)
 *
 * This differs from DDxElementJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DDxElementOrSubClassJoiSchema = Joi.alternatives().try([
  DDxElementJoiSchema,
]);

/**
 * Runtime validator for schema:DDxElement.
 *
 * If some data has a structure which matches a schema:DDxElement, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDDxElement` will have type `DDxElement`
 * const dDxElement = maybeDDxElement;
 * ```
 */
export function validateDDxElement(maybeDDxElement: unknown): DDxElement | OaValidationError {
  const { value, error } = DDxElementJoiSchema.validate(maybeDDxElement);
  if (error) {
    return new OaValidationError('DDxElement', maybeDDxElement, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DDxElement;
}
