 ...
 * schema:DanceEvent
 *
 * This differs from DanceEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DanceEventOrSubClass =
  | DanceEvent
  ;

/**
 * schema:DanceEvent - Validation schema (w/ JOI)
 */
export const DanceEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('DanceEvent').required(),
 ...
 * schema:DanceEvent - Validation schema (w/ JOI)
 *
 * This differs from DanceEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DanceEventOrSubClassJoiSchema = Joi.alternatives().try([
  DanceEventJoiSchema,
]);

/**
 * Runtime validator for schema:DanceEvent.
 *
 * If some data has a structure which matches a schema:DanceEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDanceEvent = DanceEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDanceEvent instanceof OaValidationError) {
 *   // From this point on, `maybeDanceEvent` will have type `OaValidationError`
 *   const error = maybeDanceEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDanceEvent` will have type `DanceEvent`
 * const danceEvent = maybeDanceEvent;
 * ```
 */
export function validateDanceEvent(maybeDanceEvent: unknown): DanceEvent | OaValidationError {
  const { value, error } = DanceEventJoiSchema.validate(maybeDanceEvent);
  if (error) {
    return new OaValidationError('DanceEvent', maybeDanceEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DanceEvent;
}
