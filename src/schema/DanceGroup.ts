 ...
 * schema:DanceGroup
 *
 * This differs from DanceGroup because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DanceGroupOrSubClass =
  | DanceGroup
  ;

/**
 * schema:DanceGroup - Validation schema (w/ JOI)
 */
export const DanceGroupJoiSchema = Joi.object({
  '@type': Joi.string().valid('DanceGroup').required(),
 ...
 * schema:DanceGroup - Validation schema (w/ JOI)
 *
 * This differs from DanceGroupJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DanceGroupOrSubClassJoiSchema = Joi.alternatives().try([
  DanceGroupJoiSchema,
]);

/**
 * Runtime validator for schema:DanceGroup.
 *
 * If some data has a structure which matches a schema:DanceGroup, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDanceGroup = DanceGroup.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDanceGroup instanceof OaValidationError) {
 *   // From this point on, `maybeDanceGroup` will have type `OaValidationError`
 *   const error = maybeDanceGroup;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDanceGroup` will have type `DanceGroup`
 * const danceGroup = maybeDanceGroup;
 * ```
 */
export function validateDanceGroup(maybeDanceGroup: unknown): DanceGroup | OaValidationError {
  const { value, error } = DanceGroupJoiSchema.validate(maybeDanceGroup);
  if (error) {
    return new OaValidationError('DanceGroup', maybeDanceGroup, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DanceGroup;
}
