...
export type DataCatalog = {
   ...
   */
  dataset?: schema.DatasetOrSubClass | string;
  /**
 ...
 * schema:DataCatalog
 *
 * This differs from DataCatalog because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DataCatalogOrSubClass =
  | DataCatalog
  ;

/**
 * schema:DataCatalog - Validation schema (w/ JOI)
 */
export const DataCatalogJoiSchema = Joi.object({
  '@type': Joi.string().valid('DataCatalog').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  dataset: Joi.alternatives().try(Joi.lazy(() => schema.DatasetOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:DataCatalog - Validation schema (w/ JOI)
 *
 * This differs from DataCatalogJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DataCatalogOrSubClassJoiSchema = Joi.alternatives().try([
  DataCatalogJoiSchema,
]);

/**
 * Runtime validator for schema:DataCatalog.
 *
 * If some data has a structure which matches a schema:DataCatalog, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDataCatalog = DataCatalog.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDataCatalog instanceof OaValidationError) {
 *   // From this point on, `maybeDataCatalog` will have type `OaValidationError`
 *   const error = maybeDataCatalog;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDataCatalog` will have type `DataCatalog`
 * const dataCatalog = maybeDataCatalog;
 * ```
 */
export function validateDataCatalog(maybeDataCatalog: unknown): DataCatalog | OaValidationError {
  const { value, error } = DataCatalogJoiSchema.validate(maybeDataCatalog);
  if (error) {
    return new OaValidationError('DataCatalog', maybeDataCatalog, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DataCatalog;
}
