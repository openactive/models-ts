...
export type DataFeed = {
   ...
   */
  dataFeedElement?: string | schema.ThingOrSubClass | schema.DataFeedItemOrSubClass;
  /**
   * A data catalog which contains this dataset (this property was previously 'catalog', preferred name is now 'includedInDataCatalog').
   */
  includedDataCatalog?: schema.DataCatalogOrSubClass | string;
  /**
   ...
   */
  catalog?: schema.DataCatalogOrSubClass | string;
  /**
   * A downloadable form of this dataset, at a specific location, in a specific format.
   */
  distribution?: schema.DataDownloadOrSubClass | string;
  /**
   * A data catalog which contains this dataset.
   */
  includedInDataCatalog?: schema.DataCatalogOrSubClass | string;
  /**
   ...
   */
  variableMeasured?: schema.PropertyValueOrSubClass | string;
  /**
 ...
 * schema:DataFeed
 *
 * This differs from DataFeed because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DataFeedOrSubClass =
  | DataFeed
  | schema.CompleteDataFeedOrSubClass
  ;

/**
 * schema:DataFeed - Validation schema (w/ JOI)
 */
export const DataFeedJoiSchema = Joi.object({
  '@type': Joi.string().valid('DataFeed').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  dataFeedElement: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.lazy(() => schema.DataFeedItemOrSubClassJoiSchema), Joi.string().uri()),
  ...
  distribution: Joi.alternatives().try(Joi.lazy(() => schema.DataDownloadOrSubClassJoiSchema), Joi.string().uri()),
  includedInDataCatalog: Joi.alternatives().try(Joi.lazy(() => schema.DataCatalogOrSubClassJoiSchema), Joi.string().uri()),
  issn: Joi.string(),
  variableMeasured: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
 ...
 * schema:DataFeed - Validation schema (w/ JOI)
 *
 * This differs from DataFeedJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DataFeedOrSubClassJoiSchema = Joi.alternatives().try([
  DataFeedJoiSchema,
  Joi.lazy(() => schema.CompleteDataFeedOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:DataFeed.
 *
 * If some data has a structure which matches a schema:DataFeed, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDataFeed = validateDataFeed(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDataFeed instanceof OaValidationError) {
 *   // From this point on, `maybeDataFeed` will have type `OaValidationError`
 *   const error = maybeDataFeed;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDataFeed` will have type `DataFeed`
 * const dataFeed = maybeDataFeed;
 * ```
 */
export function validateDataFeed(maybeDataFeed: unknown): DataFeed | OaValidationError {
  const { value, error } = DataFeedJoiSchema.validate(maybeDataFeed);
  if (error) {
    return new OaValidationError('DataFeed', maybeDataFeed, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DataFeed;
}
