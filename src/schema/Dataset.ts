 ...
 * schema:Dataset
 *
 * This differs from Dataset because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DatasetOrSubClass =
  | Dataset
  | schema.DataFeedOrSubClass
  ;

/**
 * schema:Dataset - Validation schema (w/ JOI)
 */
export const DatasetJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DatasetOrSubClassJoiSchema = Joi.alternatives().try([
  DatasetJoiSchema,
  Joi.lazy(() => schema.DataFeedOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Dataset.
 *
 * If some data has a structure which matches a schema:Dataset, it will be returned with the correct type.
 ...
 * ```
 */
export function validateDataset(maybeDataset: unknown): Dataset | OaValidationError {
  const { value, error } = DatasetJoiSchema.validate(maybeDataset);
  if (error) {
    return new OaValidationError('Dataset', maybeDataset, error);
  }
