 ...
 */
export type DatedMoneySpecification = {
 ...
 * schema:DatedMoneySpecification
 *
 * This differs from DatedMoneySpecification because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DatedMoneySpecificationOrSubClass =
 ...
 */
export const DatedMoneySpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DatedMoneySpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  DatedMoneySpecificationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDatedMoneySpecification = DatedMoneySpecification.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDatedMoneySpecification(maybeDatedMoneySpecification: unknown): DatedMoneySpecification | OaValidationError {
  const { value, error } = DatedMoneySpecificationJoiSchema.validate(maybeDatedMoneySpecification);
  if (error) {
    return new OaValidationError('DatedMoneySpecification', maybeDatedMoneySpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DatedMoneySpecification;
}
