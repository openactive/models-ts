 ...
 * schema:DaySpa
 *
 * This differs from DaySpa because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DaySpaOrSubClass =
  | DaySpa
  ;

/**
 * schema:DaySpa - Validation schema (w/ JOI)
 */
export const DaySpaJoiSchema = Joi.object({
  '@type': Joi.string().valid('DaySpa').required(),
 ...
 * schema:DaySpa - Validation schema (w/ JOI)
 *
 * This differs from DaySpaJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DaySpaOrSubClassJoiSchema = Joi.alternatives().try([
  DaySpaJoiSchema,
]);

/**
 * Runtime validator for schema:DaySpa.
 *
 * If some data has a structure which matches a schema:DaySpa, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDaySpa` will have type `DaySpa`
 * const daySpa = maybeDaySpa;
 * ```
 */
export function validateDaySpa(maybeDaySpa: unknown): DaySpa | OaValidationError {
  const { value, error } = DaySpaJoiSchema.validate(maybeDaySpa);
  if (error) {
    return new OaValidationError('DaySpa', maybeDaySpa, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DaySpa;
}
