 ...
 * schema:DeactivateAction
 *
 * This differs from DeactivateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DeactivateActionOrSubClass =
  | DeactivateAction
  ;

/**
 * schema:DeactivateAction - Validation schema (w/ JOI)
 */
export const DeactivateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DeactivateAction').required(),
 ...
 * schema:DeactivateAction - Validation schema (w/ JOI)
 *
 * This differs from DeactivateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DeactivateActionOrSubClassJoiSchema = Joi.alternatives().try([
  DeactivateActionJoiSchema,
]);

/**
 * Runtime validator for schema:DeactivateAction.
 *
 * If some data has a structure which matches a schema:DeactivateAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDeactivateAction = DeactivateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDeactivateAction instanceof OaValidationError) {
 *   // From this point on, `maybeDeactivateAction` will have type `OaValidationError`
 *   const error = maybeDeactivateAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDeactivateAction` will have type `DeactivateAction`
 * const deactivateAction = maybeDeactivateAction;
 * ```
 */
export function validateDeactivateAction(maybeDeactivateAction: unknown): DeactivateAction | OaValidationError {
  const { value, error } = DeactivateActionJoiSchema.validate(maybeDeactivateAction);
  if (error) {
    return new OaValidationError('DeactivateAction', maybeDeactivateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DeactivateAction;
}
