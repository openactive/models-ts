 ...
 */
export type DefenceEstablishment = {
 ...
 * schema:DefenceEstablishment
 *
 * This differs from DefenceEstablishment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DefenceEstablishmentOrSubClass =
  | DefenceEstablishment
  ;

/**
 * schema:DefenceEstablishment - Validation schema (w/ JOI)
 */
export const DefenceEstablishmentJoiSchema = Joi.object({
  '@type': Joi.string().valid('DefenceEstablishment').required(),
 ...
 * schema:DefenceEstablishment - Validation schema (w/ JOI)
 *
 * This differs from DefenceEstablishmentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DefenceEstablishmentOrSubClassJoiSchema = Joi.alternatives().try([
  DefenceEstablishmentJoiSchema,
]);

/**
 * Runtime validator for schema:DefenceEstablishment.
 *
 * If some data has a structure which matches a schema:DefenceEstablishment, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDefenceEstablishment` will have type `DefenceEstablishment`
 * const defenceEstablishment = maybeDefenceEstablishment;
 * ```
 */
export function validateDefenceEstablishment(maybeDefenceEstablishment: unknown): DefenceEstablishment | OaValidationError {
  const { value, error } = DefenceEstablishmentJoiSchema.validate(maybeDefenceEstablishment);
  if (error) {
    return new OaValidationError('DefenceEstablishment', maybeDefenceEstablishment, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DefenceEstablishment;
}
