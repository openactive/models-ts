...
export type DefinedRegion = {
   ...
   */
  postalCodeRange?: schema.PostalCodeRangeSpecificationOrSubClass | string;
  /**
 ...
 * schema:DefinedRegion
 *
 * This differs from DefinedRegion because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DefinedRegionOrSubClass =
  | DefinedRegion
  ;
 ...
 */
export const DefinedRegionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DefinedRegionOrSubClassJoiSchema = Joi.alternatives().try([
  DefinedRegionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDefinedRegion = DefinedRegion.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDefinedRegion(maybeDefinedRegion: unknown): DefinedRegion | OaValidationError {
  const { value, error } = DefinedRegionJoiSchema.validate(maybeDefinedRegion);
  if (error) {
    return new OaValidationError('DefinedRegion', maybeDefinedRegion, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DefinedRegion;
}
