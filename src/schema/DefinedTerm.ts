 ...
 * etc.
 */
export type DefinedTermOrSubClass =
  | DefinedTerm
  | schema.CategoryCodeOrSubClass
  ;

/**
 * schema:DefinedTerm - Validation schema (w/ JOI)
 */
export const DefinedTermJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DefinedTermOrSubClassJoiSchema = Joi.alternatives().try([
  DefinedTermJoiSchema,
 ...
 * ```
 */
export function validateDefinedTerm(maybeDefinedTerm: unknown): DefinedTerm | OaValidationError {
  const { value, error } = DefinedTermJoiSchema.validate(maybeDefinedTerm);
  if (error) {
    return new OaValidationError('DefinedTerm', maybeDefinedTerm, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DefinedTerm;
}
