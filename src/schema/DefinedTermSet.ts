 ...
 * schema:DefinedTermSet
 *
 * This differs from DefinedTermSet because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DefinedTermSetOrSubClass =
  | DefinedTermSet
  | schema.CategoryCodeSetOrSubClass
  ;

/**
 * schema:DefinedTermSet - Validation schema (w/ JOI)
 */
export const DefinedTermSetJoiSchema = Joi.object({
  '@type': Joi.string().valid('DefinedTermSet').required(),
 ...
 * schema:DefinedTermSet - Validation schema (w/ JOI)
 *
 * This differs from DefinedTermSetJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DefinedTermSetOrSubClassJoiSchema = Joi.alternatives().try([
  DefinedTermSetJoiSchema,
  Joi.lazy(() => schema.CategoryCodeSetOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:DefinedTermSet.
 *
 * If some data has a structure which matches a schema:DefinedTermSet, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDefinedTermSet = DefinedTermSet.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDefinedTermSet instanceof OaValidationError) {
 *   // From this point on, `maybeDefinedTermSet` will have type `OaValidationError`
 *   const error = maybeDefinedTermSet;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDefinedTermSet` will have type `DefinedTermSet`
 * const definedTermSet = maybeDefinedTermSet;
 * ```
 */
export function validateDefinedTermSet(maybeDefinedTermSet: unknown): DefinedTermSet | OaValidationError {
  const { value, error } = DefinedTermSetJoiSchema.validate(maybeDefinedTermSet);
  if (error) {
    return new OaValidationError('DefinedTermSet', maybeDefinedTermSet, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DefinedTermSet;
}
