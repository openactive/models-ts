 ...
 * schema:DeleteAction
 *
 * This differs from DeleteAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DeleteActionOrSubClass =
  | DeleteAction
  ;
 ...
 */
export const DeleteActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DeleteActionOrSubClassJoiSchema = Joi.alternatives().try([
  DeleteActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDeleteAction = DeleteAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDeleteAction(maybeDeleteAction: unknown): DeleteAction | OaValidationError {
  const { value, error } = DeleteActionJoiSchema.validate(maybeDeleteAction);
  if (error) {
    return new OaValidationError('DeleteAction', maybeDeleteAction, error);
  }
