 ...
 * schema:DeliveryEvent
 *
 * This differs from DeliveryEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DeliveryEventOrSubClass =
  | DeliveryEvent
  ;
 ...
 */
export const DeliveryEventJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DeliveryEventOrSubClassJoiSchema = Joi.alternatives().try([
  DeliveryEventJoiSchema,
]);
 ...
 * ```
 */
export function validateDeliveryEvent(maybeDeliveryEvent: unknown): DeliveryEvent | OaValidationError {
  const { value, error } = DeliveryEventJoiSchema.validate(maybeDeliveryEvent);
  if (error) {
    return new OaValidationError('DeliveryEvent', maybeDeliveryEvent, error);
  }
