 ...
 * schema:Dentist
 *
 * This differs from Dentist because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DentistOrSubClass =
  | Dentist
  ;

/**
 * schema:Dentist - Validation schema (w/ JOI)
 */
export const DentistJoiSchema = Joi.object({
  '@type': Joi.string().valid('Dentist').required(),
 ...
 * schema:Dentist - Validation schema (w/ JOI)
 *
 * This differs from DentistJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DentistOrSubClassJoiSchema = Joi.alternatives().try([
  DentistJoiSchema,
]);

/**
 * Runtime validator for schema:Dentist.
 *
 * If some data has a structure which matches a schema:Dentist, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDentist = Dentist.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDentist instanceof OaValidationError) {
 *   // From this point on, `maybeDentist` will have type `OaValidationError`
 *   const error = maybeDentist;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDentist` will have type `Dentist`
 * const dentist = maybeDentist;
 * ```
 */
export function validateDentist(maybeDentist: unknown): Dentist | OaValidationError {
  const { value, error } = DentistJoiSchema.validate(maybeDentist);
  if (error) {
    return new OaValidationError('Dentist', maybeDentist, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Dentist;
}
