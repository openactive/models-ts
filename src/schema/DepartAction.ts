 ...
 * schema:DepartAction
 *
 * This differs from DepartAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DepartActionOrSubClass =
  | DepartAction
  ;

/**
 * schema:DepartAction - Validation schema (w/ JOI)
 */
export const DepartActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DepartAction').required(),
 ...
 * schema:DepartAction - Validation schema (w/ JOI)
 *
 * This differs from DepartActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DepartActionOrSubClassJoiSchema = Joi.alternatives().try([
  DepartActionJoiSchema,
]);

/**
 * Runtime validator for schema:DepartAction.
 *
 * If some data has a structure which matches a schema:DepartAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDepartAction = DepartAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDepartAction instanceof OaValidationError) {
 *   // From this point on, `maybeDepartAction` will have type `OaValidationError`
 *   const error = maybeDepartAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDepartAction` will have type `DepartAction`
 * const departAction = maybeDepartAction;
 * ```
 */
export function validateDepartAction(maybeDepartAction: unknown): DepartAction | OaValidationError {
  const { value, error } = DepartActionJoiSchema.validate(maybeDepartAction);
  if (error) {
    return new OaValidationError('DepartAction', maybeDepartAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DepartAction;
}
