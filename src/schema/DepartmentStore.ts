 ...
 * schema:DepartmentStore
 *
 * This differs from DepartmentStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DepartmentStoreOrSubClass =
  | DepartmentStore
  ;

/**
 * schema:DepartmentStore - Validation schema (w/ JOI)
 */
export const DepartmentStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('DepartmentStore').required(),
 ...
 * schema:DepartmentStore - Validation schema (w/ JOI)
 *
 * This differs from DepartmentStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DepartmentStoreOrSubClassJoiSchema = Joi.alternatives().try([
  DepartmentStoreJoiSchema,
]);

/**
 * Runtime validator for schema:DepartmentStore.
 *
 * If some data has a structure which matches a schema:DepartmentStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDepartmentStore = DepartmentStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDepartmentStore instanceof OaValidationError) {
 *   // From this point on, `maybeDepartmentStore` will have type `OaValidationError`
 *   const error = maybeDepartmentStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDepartmentStore` will have type `DepartmentStore`
 * const departmentStore = maybeDepartmentStore;
 * ```
 */
export function validateDepartmentStore(maybeDepartmentStore: unknown): DepartmentStore | OaValidationError {
  const { value, error } = DepartmentStoreJoiSchema.validate(maybeDepartmentStore);
  if (error) {
    return new OaValidationError('DepartmentStore', maybeDepartmentStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DepartmentStore;
}
