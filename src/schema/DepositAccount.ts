 ...
 * schema:DepositAccount
 *
 * This differs from DepositAccount because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DepositAccountOrSubClass =
  | DepositAccount
  ;
 ...
 */
export const DepositAccountJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DepositAccountOrSubClassJoiSchema = Joi.alternatives().try([
  DepositAccountJoiSchema,
]);
 ...
 * ```
 */
export function validateDepositAccount(maybeDepositAccount: unknown): DepositAccount | OaValidationError {
  const { value, error } = DepositAccountJoiSchema.validate(maybeDepositAccount);
  if (error) {
    return new OaValidationError('DepositAccount', maybeDepositAccount, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DepositAccount;
}
