 ...
 * etc.
 */
export type DiagnosticLabOrSubClass =
  | DiagnosticLab
  ;
 ...
 */
export const DiagnosticLabJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DiagnosticLabOrSubClassJoiSchema = Joi.alternatives().try([
  DiagnosticLabJoiSchema,
]);
 ...
 * ```
 */
export function validateDiagnosticLab(maybeDiagnosticLab: unknown): DiagnosticLab | OaValidationError {
  const { value, error } = DiagnosticLabJoiSchema.validate(maybeDiagnosticLab);
  if (error) {
    return new OaValidationError('DiagnosticLab', maybeDiagnosticLab, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DiagnosticLab;
}
