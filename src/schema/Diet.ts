...
export type Diet = {
   ...
   */
  endorsers?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
 ...
 * schema:Diet
 *
 * This differs from Diet because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DietOrSubClass =
  | Diet
  ;

/**
 * schema:Diet - Validation schema (w/ JOI)
 */
export const DietJoiSchema = Joi.object({
  '@type': Joi.string().valid('Diet').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  physiologicalBenefits: Joi.string(),
  risks: Joi.string(),
  expertConsiderations: Joi.string(),
  endorsers: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  dietFeatures: Joi.string(),
 ...
 * schema:Diet - Validation schema (w/ JOI)
 *
 * This differs from DietJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DietOrSubClassJoiSchema = Joi.alternatives().try([
  DietJoiSchema,
]);

/**
 * Runtime validator for schema:Diet.
 *
 * If some data has a structure which matches a schema:Diet, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDiet = Diet.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDiet instanceof OaValidationError) {
 *   // From this point on, `maybeDiet` will have type `OaValidationError`
 *   const error = maybeDiet;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDiet` will have type `Diet`
 * const diet = maybeDiet;
 * ```
 */
export function validateDiet(maybeDiet: unknown): Diet | OaValidationError {
  const { value, error } = DietJoiSchema.validate(maybeDiet);
  if (error) {
    return new OaValidationError('Diet', maybeDiet, error);
  }
