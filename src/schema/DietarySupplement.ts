 ...
 * schema:DietarySupplement
 *
 * This differs from DietarySupplement because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DietarySupplementOrSubClass =
  | DietarySupplement
  ;
 ...
 * `SessionSeries`, etc.
 */
export const DietarySupplementOrSubClassJoiSchema = Joi.alternatives().try([
  DietarySupplementJoiSchema,
]);
 ...
 * ```
 */
export function validateDietarySupplement(maybeDietarySupplement: unknown): DietarySupplement | OaValidationError {
  const { value, error } = DietarySupplementJoiSchema.validate(maybeDietarySupplement);
  if (error) {
    return new OaValidationError('DietarySupplement', maybeDietarySupplement, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DietarySupplement;
}
