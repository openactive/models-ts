...
export type DigitalDocument = {
   ...
   */
  hasDigitalDocumentPermission?: schema.DigitalDocumentPermissionOrSubClass | string;
  /**
 ...
 * schema:DigitalDocument
 *
 * This differs from DigitalDocument because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DigitalDocumentOrSubClass =
  | DigitalDocument
  | oa.TermsOrSubClass
  | schema.SpreadsheetDigitalDocumentOrSubClass
  | schema.NoteDigitalDocumentOrSubClass
  | schema.TextDigitalDocumentOrSubClass
  | schema.PresentationDigitalDocumentOrSubClass
  ;

/**
 * schema:DigitalDocument - Validation schema (w/ JOI)
 */
export const DigitalDocumentJoiSchema = Joi.object({
  '@type': Joi.string().valid('DigitalDocument').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  hasDigitalDocumentPermission: Joi.alternatives().try(Joi.lazy(() => schema.DigitalDocumentPermissionOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:DigitalDocument - Validation schema (w/ JOI)
 *
 * This differs from DigitalDocumentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DigitalDocumentOrSubClassJoiSchema = Joi.alternatives().try([
  DigitalDocumentJoiSchema,
  Joi.lazy(() => oa.TermsOrSubClassJoiSchema),
  Joi.lazy(() => schema.SpreadsheetDigitalDocumentOrSubClassJoiSchema),
  Joi.lazy(() => schema.NoteDigitalDocumentOrSubClassJoiSchema),
  Joi.lazy(() => schema.TextDigitalDocumentOrSubClassJoiSchema),
  Joi.lazy(() => schema.PresentationDigitalDocumentOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:DigitalDocument.
 *
 * If some data has a structure which matches a schema:DigitalDocument, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDigitalDocument = DigitalDocument.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDigitalDocument instanceof OaValidationError) {
 *   // From this point on, `maybeDigitalDocument` will have type `OaValidationError`
 *   const error = maybeDigitalDocument;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDigitalDocument` will have type `DigitalDocument`
 * const digitalDocument = maybeDigitalDocument;
 * ```
 */
export function validateDigitalDocument(maybeDigitalDocument: unknown): DigitalDocument | OaValidationError {
  const { value, error } = DigitalDocumentJoiSchema.validate(maybeDigitalDocument);
  if (error) {
    return new OaValidationError('DigitalDocument', maybeDigitalDocument, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DigitalDocument;
}
