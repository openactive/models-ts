 ...
 * A type of permission which can be granted for accessing a digital document.
 */
export const DigitalDocumentPermissionTypeJoiSchema = Joi.string().valid(
  'https://schema.org/WritePermission',
  'https://schema.org/ReadPermission',
  'https://schema.org/CommentPermission',
);
 ...
 *
 * ```ts
 * const maybeDigitalDocumentPermissionType = DigitalDocumentPermissionType.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDigitalDocumentPermissionType(maybeDigitalDocumentPermissionType: unknown): DigitalDocumentPermissionType | OaValidationError {
  const { value, error } = DigitalDocumentPermissionTypeJoiSchema.validate(maybeDigitalDocumentPermissionType);
  if (error) {
    return new OaValidationError('DigitalDocumentPermissionType', maybeDigitalDocumentPermissionType, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DigitalDocumentPermissionType;
}
