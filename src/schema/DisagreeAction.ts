 ...
 * schema:DisagreeAction
 *
 * This differs from DisagreeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DisagreeActionOrSubClass =
  | DisagreeAction
  ;

/**
 * schema:DisagreeAction - Validation schema (w/ JOI)
 */
export const DisagreeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DisagreeAction').required(),
 ...
 * schema:DisagreeAction - Validation schema (w/ JOI)
 *
 * This differs from DisagreeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DisagreeActionOrSubClassJoiSchema = Joi.alternatives().try([
  DisagreeActionJoiSchema,
]);

/**
 * Runtime validator for schema:DisagreeAction.
 *
 * If some data has a structure which matches a schema:DisagreeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDisagreeAction = DisagreeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDisagreeAction instanceof OaValidationError) {
 *   // From this point on, `maybeDisagreeAction` will have type `OaValidationError`
 *   const error = maybeDisagreeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDisagreeAction` will have type `DisagreeAction`
 * const disagreeAction = maybeDisagreeAction;
 * ```
 */
export function validateDisagreeAction(maybeDisagreeAction: unknown): DisagreeAction | OaValidationError {
  const { value, error } = DisagreeActionJoiSchema.validate(maybeDisagreeAction);
  if (error) {
    return new OaValidationError('DisagreeAction', maybeDisagreeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DisagreeAction;
}
