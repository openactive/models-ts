 ...
 * schema:DiscoverAction
 *
 * This differs from DiscoverAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DiscoverActionOrSubClass =
  | DiscoverAction
  ;
 ...
 */
export const DiscoverActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DiscoverActionOrSubClassJoiSchema = Joi.alternatives().try([
  DiscoverActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDiscoverAction = validateDiscoverAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDiscoverAction(maybeDiscoverAction: unknown): DiscoverAction | OaValidationError {
  const { value, error } = DiscoverActionJoiSchema.validate(maybeDiscoverAction);
  if (error) {
    return new OaValidationError('DiscoverAction', maybeDiscoverAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DiscoverAction;
}
