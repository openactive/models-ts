 ...
 * schema:DiscussionForumPosting
 *
 * This differs from DiscussionForumPosting because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DiscussionForumPostingOrSubClass =
  | DiscussionForumPosting
  ;

/**
 * schema:DiscussionForumPosting - Validation schema (w/ JOI)
 */
export const DiscussionForumPostingJoiSchema = Joi.object({
  '@type': Joi.string().valid('DiscussionForumPosting').required(),
 ...
 * schema:DiscussionForumPosting - Validation schema (w/ JOI)
 *
 * This differs from DiscussionForumPostingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DiscussionForumPostingOrSubClassJoiSchema = Joi.alternatives().try([
  DiscussionForumPostingJoiSchema,
]);

/**
 * Runtime validator for schema:DiscussionForumPosting.
 *
 * If some data has a structure which matches a schema:DiscussionForumPosting, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDiscussionForumPosting` will have type `DiscussionForumPosting`
 * const discussionForumPosting = maybeDiscussionForumPosting;
 * ```
 */
export function validateDiscussionForumPosting(maybeDiscussionForumPosting: unknown): DiscussionForumPosting | OaValidationError {
  const { value, error } = DiscussionForumPostingJoiSchema.validate(maybeDiscussionForumPosting);
  if (error) {
    return new OaValidationError('DiscussionForumPosting', maybeDiscussionForumPosting, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DiscussionForumPosting;
}
