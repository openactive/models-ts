 ...
 * schema:DislikeAction
 *
 * This differs from DislikeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DislikeActionOrSubClass =
  | DislikeAction
  ;

/**
 * schema:DislikeAction - Validation schema (w/ JOI)
 */
export const DislikeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DislikeAction').required(),
 ...
 * schema:DislikeAction - Validation schema (w/ JOI)
 *
 * This differs from DislikeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DislikeActionOrSubClassJoiSchema = Joi.alternatives().try([
  DislikeActionJoiSchema,
]);

/**
 * Runtime validator for schema:DislikeAction.
 *
 * If some data has a structure which matches a schema:DislikeAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDislikeAction` will have type `DislikeAction`
 * const dislikeAction = maybeDislikeAction;
 * ```
 */
export function validateDislikeAction(maybeDislikeAction: unknown): DislikeAction | OaValidationError {
  const { value, error } = DislikeActionJoiSchema.validate(maybeDislikeAction);
  if (error) {
    return new OaValidationError('DislikeAction', maybeDislikeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DislikeAction;
}
