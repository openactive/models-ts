 ...
 * schema:Distillery
 *
 * This differs from Distillery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DistilleryOrSubClass =
  | Distillery
  ;
 ...
 */
export const DistilleryJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DistilleryOrSubClassJoiSchema = Joi.alternatives().try([
  DistilleryJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDistillery = Distillery.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDistillery(maybeDistillery: unknown): Distillery | OaValidationError {
  const { value, error } = DistilleryJoiSchema.validate(maybeDistillery);
  if (error) {
    return new OaValidationError('Distillery', maybeDistillery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Distillery;
}
