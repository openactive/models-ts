 ...
 * schema:DonateAction
 *
 * This differs from DonateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DonateActionOrSubClass =
  | DonateAction
  ;

/**
 * schema:DonateAction - Validation schema (w/ JOI)
 */
export const DonateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DonateAction').required(),
 ...
 * schema:DonateAction - Validation schema (w/ JOI)
 *
 * This differs from DonateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DonateActionOrSubClassJoiSchema = Joi.alternatives().try([
  DonateActionJoiSchema,
]);

/**
 * Runtime validator for schema:DonateAction.
 *
 * If some data has a structure which matches a schema:DonateAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDonateAction = DonateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDonateAction instanceof OaValidationError) {
 *   // From this point on, `maybeDonateAction` will have type `OaValidationError`
 *   const error = maybeDonateAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDonateAction` will have type `DonateAction`
 * const donateAction = maybeDonateAction;
 * ```
 */
export function validateDonateAction(maybeDonateAction: unknown): DonateAction | OaValidationError {
  const { value, error } = DonateActionJoiSchema.validate(maybeDonateAction);
  if (error) {
    return new OaValidationError('DonateAction', maybeDonateAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DonateAction;
}
