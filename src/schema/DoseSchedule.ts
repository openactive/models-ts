 ...
 * etc.
 */
export type DoseScheduleOrSubClass =
  | DoseSchedule
  | schema.MaximumDoseScheduleOrSubClass
  | schema.RecommendedDoseScheduleOrSubClass
  | schema.ReportedDoseScheduleOrSubClass
  ;

/**
 * schema:DoseSchedule - Validation schema (w/ JOI)
 */
export const DoseScheduleJoiSchema = Joi.object({
  '@type': Joi.string().valid('DoseSchedule').required(),
 ...
 * `SessionSeries`, etc.
 */
export const DoseScheduleOrSubClassJoiSchema = Joi.alternatives().try([
  DoseScheduleJoiSchema,
  Joi.lazy(() => schema.MaximumDoseScheduleOrSubClassJoiSchema),
  Joi.lazy(() => schema.RecommendedDoseScheduleOrSubClassJoiSchema),
  Joi.lazy(() => schema.ReportedDoseScheduleOrSubClassJoiSchema),
]);
 ...
 *
 * ```ts
 * const maybeDoseSchedule = DoseSchedule.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDoseSchedule(maybeDoseSchedule: unknown): DoseSchedule | OaValidationError {
  const { value, error } = DoseScheduleJoiSchema.validate(maybeDoseSchedule);
  if (error) {
    return new OaValidationError('DoseSchedule', maybeDoseSchedule, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DoseSchedule;
}
