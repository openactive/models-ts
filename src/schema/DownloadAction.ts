 ...
 * schema:DownloadAction
 *
 * This differs from DownloadAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DownloadActionOrSubClass =
  | DownloadAction
  ;

/**
 * schema:DownloadAction - Validation schema (w/ JOI)
 */
export const DownloadActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DownloadAction').required(),
 ...
 * schema:DownloadAction - Validation schema (w/ JOI)
 *
 * This differs from DownloadActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DownloadActionOrSubClassJoiSchema = Joi.alternatives().try([
  DownloadActionJoiSchema,
]);

/**
 * Runtime validator for schema:DownloadAction.
 *
 * If some data has a structure which matches a schema:DownloadAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDownloadAction = DownloadAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDownloadAction instanceof OaValidationError) {
 *   // From this point on, `maybeDownloadAction` will have type `OaValidationError`
 *   const error = maybeDownloadAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDownloadAction` will have type `DownloadAction`
 * const downloadAction = maybeDownloadAction;
 * ```
 */
export function validateDownloadAction(maybeDownloadAction: unknown): DownloadAction | OaValidationError {
  const { value, error } = DownloadActionJoiSchema.validate(maybeDownloadAction);
  if (error) {
    return new OaValidationError('DownloadAction', maybeDownloadAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DownloadAction;
}
