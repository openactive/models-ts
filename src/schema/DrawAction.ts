 ...
 * schema:DrawAction
 *
 * This differs from DrawAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DrawActionOrSubClass =
  | DrawAction
  ;

/**
 * schema:DrawAction - Validation schema (w/ JOI)
 */
export const DrawActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('DrawAction').required(),
 ...
 * schema:DrawAction - Validation schema (w/ JOI)
 *
 * This differs from DrawActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DrawActionOrSubClassJoiSchema = Joi.alternatives().try([
  DrawActionJoiSchema,
]);

/**
 * Runtime validator for schema:DrawAction.
 *
 * If some data has a structure which matches a schema:DrawAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeDrawAction = DrawAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeDrawAction instanceof OaValidationError) {
 *   // From this point on, `maybeDrawAction` will have type `OaValidationError`
 *   const error = maybeDrawAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeDrawAction` will have type `DrawAction`
 * const drawAction = maybeDrawAction;
 * ```
 */
export function validateDrawAction(maybeDrawAction: unknown): DrawAction | OaValidationError {
  const { value, error } = DrawActionJoiSchema.validate(maybeDrawAction);
  if (error) {
    return new OaValidationError('DrawAction', maybeDrawAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrawAction;
}
