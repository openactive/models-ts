 ...
 * schema:Drawing
 *
 * This differs from Drawing because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DrawingOrSubClass =
  | Drawing
  ;

/**
 * schema:Drawing - Validation schema (w/ JOI)
 */
export const DrawingJoiSchema = Joi.object({
  '@type': Joi.string().valid('Drawing').required(),
 ...
 * schema:Drawing - Validation schema (w/ JOI)
 *
 * This differs from DrawingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DrawingOrSubClassJoiSchema = Joi.alternatives().try([
  DrawingJoiSchema,
]);

/**
 * Runtime validator for schema:Drawing.
 *
 * If some data has a structure which matches a schema:Drawing, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDrawing` will have type `Drawing`
 * const drawing = maybeDrawing;
 * ```
 */
export function validateDrawing(maybeDrawing: unknown): Drawing | OaValidationError {
  const { value, error } = DrawingJoiSchema.validate(maybeDrawing);
  if (error) {
    return new OaValidationError('Drawing', maybeDrawing, error);
  }
