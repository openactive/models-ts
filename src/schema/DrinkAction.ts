 ...
 * schema:DrinkAction
 *
 * This differs from DrinkAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DrinkActionOrSubClass =
  | DrinkAction
  ;
 ...
 */
export const DrinkActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DrinkActionOrSubClassJoiSchema = Joi.alternatives().try([
  DrinkActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDrinkAction = DrinkAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDrinkAction(maybeDrinkAction: unknown): DrinkAction | OaValidationError {
  const { value, error } = DrinkActionJoiSchema.validate(maybeDrinkAction);
  if (error) {
    return new OaValidationError('DrinkAction', maybeDrinkAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrinkAction;
}
