 ...
 * schema:DrugClass
 *
 * This differs from DrugClass because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DrugClassOrSubClass =
  | DrugClass
  ;
 ...
 */
export const DrugClassJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DrugClassOrSubClassJoiSchema = Joi.alternatives().try([
  DrugClassJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDrugClass = validateDrugClass(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDrugClass(maybeDrugClass: unknown): DrugClass | OaValidationError {
  const { value, error } = DrugClassJoiSchema.validate(maybeDrugClass);
  if (error) {
    return new OaValidationError('DrugClass', maybeDrugClass, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrugClass;
}
