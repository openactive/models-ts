...
export type DrugCost = {
   ...
   */
  costCategory?: schema.DrugCostCategoryOrSubClass | string;
  /**
   ...
   */
  costPerUnit?: number | schema.QualitativeValue | string;
  /**
 ...
 * schema:DrugCost
 *
 * This differs from DrugCost because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DrugCostOrSubClass =
  | DrugCost
  ;
 ...
 */
export const DrugCostJoiSchema = Joi.object({
  ...
  costPerUnit: Joi.alternatives().try(Joi.number(), Joi.lazy(() => schema.QualitativeValueJoiSchema), Joi.string()),
 ...
 * `SessionSeries`, etc.
 */
export const DrugCostOrSubClassJoiSchema = Joi.alternatives().try([
  DrugCostJoiSchema,
]);
 ...
 * ```
 */
export function validateDrugCost(maybeDrugCost: unknown): DrugCost | OaValidationError {
  const { value, error } = DrugCostJoiSchema.validate(maybeDrugCost);
  if (error) {
    return new OaValidationError('DrugCost', maybeDrugCost, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrugCost;
}
