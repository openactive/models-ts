...
export type DrugLegalStatus = {
   ...
   */
  applicableLocation?: schema.AdministrativeAreaOrSubClass | string;
  /**
 ...
 * schema:DrugLegalStatus
 *
 * This differs from DrugLegalStatus because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DrugLegalStatusOrSubClass =
  | DrugLegalStatus
  ;
 ...
 */
export const DrugLegalStatusJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DrugLegalStatusOrSubClassJoiSchema = Joi.alternatives().try([
  DrugLegalStatusJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDrugLegalStatus = validateDrugLegalStatus(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDrugLegalStatus(maybeDrugLegalStatus: unknown): DrugLegalStatus | OaValidationError {
  const { value, error } = DrugLegalStatusJoiSchema.validate(maybeDrugLegalStatus);
  if (error) {
    return new OaValidationError('DrugLegalStatus', maybeDrugLegalStatus, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrugLegalStatus;
}
