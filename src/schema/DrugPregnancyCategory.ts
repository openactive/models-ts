 ...
 * etc.
 */
export type DrugPregnancyCategoryOrSubClass =
 ...
 */
export const DrugPregnancyCategoryJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DrugPregnancyCategoryOrSubClassJoiSchema = Joi.alternatives().try([
  DrugPregnancyCategoryJoiSchema,
]);
 ...
 * ```
 */
export function validateDrugPregnancyCategory(maybeDrugPregnancyCategory: unknown): DrugPregnancyCategory | OaValidationError {
  const { value, error } = DrugPregnancyCategoryJoiSchema.validate(maybeDrugPregnancyCategory);
  if (error) {
    return new OaValidationError('DrugPregnancyCategory', maybeDrugPregnancyCategory, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrugPregnancyCategory;
}
