 ...
 * etc.
 */
export type DrugStrengthOrSubClass =
  | DrugStrength
  ;
 ...
 */
export const DrugStrengthJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DrugStrengthOrSubClassJoiSchema = Joi.alternatives().try([
  DrugStrengthJoiSchema,
]);
 ...
 * ```
 */
export function validateDrugStrength(maybeDrugStrength: unknown): DrugStrength | OaValidationError {
  const { value, error } = DrugStrengthJoiSchema.validate(maybeDrugStrength);
  if (error) {
    return new OaValidationError('DrugStrength', maybeDrugStrength, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DrugStrength;
}
