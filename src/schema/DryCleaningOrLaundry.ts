 ...
 */
export type DryCleaningOrLaundry = {
 ...
 * schema:DryCleaningOrLaundry
 *
 * This differs from DryCleaningOrLaundry because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DryCleaningOrLaundryOrSubClass =
  | DryCleaningOrLaundry
  ;

/**
 * schema:DryCleaningOrLaundry - Validation schema (w/ JOI)
 */
export const DryCleaningOrLaundryJoiSchema = Joi.object({
  '@type': Joi.string().valid('DryCleaningOrLaundry').required(),
 ...
 * schema:DryCleaningOrLaundry - Validation schema (w/ JOI)
 *
 * This differs from DryCleaningOrLaundryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const DryCleaningOrLaundryOrSubClassJoiSchema = Joi.alternatives().try([
  DryCleaningOrLaundryJoiSchema,
]);

/**
 * Runtime validator for schema:DryCleaningOrLaundry.
 *
 * If some data has a structure which matches a schema:DryCleaningOrLaundry, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeDryCleaningOrLaundry` will have type `DryCleaningOrLaundry`
 * const dryCleaningOrLaundry = maybeDryCleaningOrLaundry;
 * ```
 */
export function validateDryCleaningOrLaundry(maybeDryCleaningOrLaundry: unknown): DryCleaningOrLaundry | OaValidationError {
  const { value, error } = DryCleaningOrLaundryJoiSchema.validate(maybeDryCleaningOrLaundry);
  if (error) {
    return new OaValidationError('DryCleaningOrLaundry', maybeDryCleaningOrLaundry, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as DryCleaningOrLaundry;
}
