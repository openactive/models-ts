 ...
 * schema:Duration
 *
 * This differs from Duration because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type DurationOrSubClass =
  | Duration
  ;
 ...
 */
export const DurationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const DurationOrSubClassJoiSchema = Joi.alternatives().try([
  DurationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeDuration = Duration.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateDuration(maybeDuration: unknown): Duration | OaValidationError {
  const { value, error } = DurationJoiSchema.validate(maybeDuration);
  if (error) {
    return new OaValidationError('Duration', maybeDuration, error);
  }
