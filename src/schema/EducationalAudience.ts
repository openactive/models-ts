 ...
 */
export type EducationalAudience = {
 ...
 * schema:EducationalAudience
 *
 * This differs from EducationalAudience because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EducationalAudienceOrSubClass =
  | EducationalAudience
  ;
 ...
 */
export const EducationalAudienceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const EducationalAudienceOrSubClassJoiSchema = Joi.alternatives().try([
  EducationalAudienceJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeEducationalAudience = validateEducationalAudience(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateEducationalAudience(maybeEducationalAudience: unknown): EducationalAudience | OaValidationError {
  const { value, error } = EducationalAudienceJoiSchema.validate(maybeEducationalAudience);
  if (error) {
    return new OaValidationError('EducationalAudience', maybeEducationalAudience, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EducationalAudience;
}
