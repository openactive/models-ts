...
export type EducationalOccupationalCredential = {
   ...
   */
  credentialCategory?: schema.DefinedTermOrSubClass | string;
  /**
   ...
   */
  recognizedBy?: schema.OrganizationOrSubClass | string;
  /**
 ...
 * schema:EducationalOccupationalCredential
 *
 * This differs from EducationalOccupationalCredential because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EducationalOccupationalCredentialOrSubClass =
  | EducationalOccupationalCredential
  ;

/**
 * schema:EducationalOccupationalCredential - Validation schema (w/ JOI)
 */
export const EducationalOccupationalCredentialJoiSchema = Joi.object({
  '@type': Joi.string().valid('EducationalOccupationalCredential').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  credentialCategory: Joi.alternatives().try(Joi.lazy(() => schema.DefinedTermOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  ...
  recognizedBy: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:EducationalOccupationalCredential - Validation schema (w/ JOI)
 *
 * This differs from EducationalOccupationalCredentialJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EducationalOccupationalCredentialOrSubClassJoiSchema = Joi.alternatives().try([
  EducationalOccupationalCredentialJoiSchema,
]);

/**
 * Runtime validator for schema:EducationalOccupationalCredential.
 *
 * If some data has a structure which matches a schema:EducationalOccupationalCredential, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEducationalOccupationalCredential = EducationalOccupationalCredential.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEducationalOccupationalCredential instanceof OaValidationError) {
 *   // From this point on, `maybeEducationalOccupationalCredential` will have type `OaValidationError`
 *   const error = maybeEducationalOccupationalCredential;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEducationalOccupationalCredential` will have type `EducationalOccupationalCredential`
 * const educationalOccupationalCredential = maybeEducationalOccupationalCredential;
 * ```
 */
export function validateEducationalOccupationalCredential(maybeEducationalOccupationalCredential: unknown): EducationalOccupationalCredential | OaValidationError {
  const { value, error } = EducationalOccupationalCredentialJoiSchema.validate(maybeEducationalOccupationalCredential);
  if (error) {
    return new OaValidationError('EducationalOccupationalCredential', maybeEducationalOccupationalCredential, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EducationalOccupationalCredential;
}
