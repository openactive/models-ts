 ...
 * schema:EducationalOrganization
 *
 * This differs from EducationalOrganization because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EducationalOrganizationOrSubClass =
  | EducationalOrganization
  | schema.CollegeOrUniversityOrSubClass
  | schema.SchoolOrSubClass
  | schema.HighSchoolOrSubClass
  | schema.ElementarySchoolOrSubClass
  | schema.PreschoolOrSubClass
  | schema.MiddleSchoolOrSubClass
  ;

/**
 * schema:EducationalOrganization - Validation schema (w/ JOI)
 */
export const EducationalOrganizationJoiSchema = Joi.object({
  '@type': Joi.string().valid('EducationalOrganization').required(),
 ...
 * schema:EducationalOrganization - Validation schema (w/ JOI)
 *
 * This differs from EducationalOrganizationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EducationalOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
  EducationalOrganizationJoiSchema,
  Joi.lazy(() => schema.CollegeOrUniversityOrSubClassJoiSchema),
  Joi.lazy(() => schema.SchoolOrSubClassJoiSchema),
  Joi.lazy(() => schema.HighSchoolOrSubClassJoiSchema),
  Joi.lazy(() => schema.ElementarySchoolOrSubClassJoiSchema),
  Joi.lazy(() => schema.PreschoolOrSubClassJoiSchema),
  Joi.lazy(() => schema.MiddleSchoolOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:EducationalOrganization.
 *
 * If some data has a structure which matches a schema:EducationalOrganization, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEducationalOrganization = EducationalOrganization.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEducationalOrganization instanceof OaValidationError) {
 *   // From this point on, `maybeEducationalOrganization` will have type `OaValidationError`
 *   const error = maybeEducationalOrganization;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEducationalOrganization` will have type `EducationalOrganization`
 * const educationalOrganization = maybeEducationalOrganization;
 * ```
 */
export function validateEducationalOrganization(maybeEducationalOrganization: unknown): EducationalOrganization | OaValidationError {
  const { value, error } = EducationalOrganizationJoiSchema.validate(maybeEducationalOrganization);
  if (error) {
    return new OaValidationError('EducationalOrganization', maybeEducationalOrganization, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EducationalOrganization;
}
