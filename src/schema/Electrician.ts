 ...
 * schema:Electrician
 *
 * This differs from Electrician because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ElectricianOrSubClass =
  | Electrician
  ;

/**
 * schema:Electrician - Validation schema (w/ JOI)
 */
export const ElectricianJoiSchema = Joi.object({
  '@type': Joi.string().valid('Electrician').required(),
 ...
 * schema:Electrician - Validation schema (w/ JOI)
 *
 * This differs from ElectricianJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ElectricianOrSubClassJoiSchema = Joi.alternatives().try([
  ElectricianJoiSchema,
]);

/**
 * Runtime validator for schema:Electrician.
 *
 * If some data has a structure which matches a schema:Electrician, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeElectrician = Electrician.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeElectrician instanceof OaValidationError) {
 *   // From this point on, `maybeElectrician` will have type `OaValidationError`
 *   const error = maybeElectrician;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeElectrician` will have type `Electrician`
 * const electrician = maybeElectrician;
 * ```
 */
export function validateElectrician(maybeElectrician: unknown): Electrician | OaValidationError {
  const { value, error } = ElectricianJoiSchema.validate(maybeElectrician);
  if (error) {
    return new OaValidationError('Electrician', maybeElectrician, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Electrician;
}
