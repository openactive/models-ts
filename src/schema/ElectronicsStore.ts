 ...
 * schema:ElectronicsStore
 *
 * This differs from ElectronicsStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ElectronicsStoreOrSubClass =
  | ElectronicsStore
  ;

/**
 * schema:ElectronicsStore - Validation schema (w/ JOI)
 */
export const ElectronicsStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('ElectronicsStore').required(),
 ...
 * schema:ElectronicsStore - Validation schema (w/ JOI)
 *
 * This differs from ElectronicsStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ElectronicsStoreOrSubClassJoiSchema = Joi.alternatives().try([
  ElectronicsStoreJoiSchema,
]);

/**
 * Runtime validator for schema:ElectronicsStore.
 *
 * If some data has a structure which matches a schema:ElectronicsStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeElectronicsStore = ElectronicsStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeElectronicsStore instanceof OaValidationError) {
 *   // From this point on, `maybeElectronicsStore` will have type `OaValidationError`
 *   const error = maybeElectronicsStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeElectronicsStore` will have type `ElectronicsStore`
 * const electronicsStore = maybeElectronicsStore;
 * ```
 */
export function validateElectronicsStore(maybeElectronicsStore: unknown): ElectronicsStore | OaValidationError {
  const { value, error } = ElectronicsStoreJoiSchema.validate(maybeElectronicsStore);
  if (error) {
    return new OaValidationError('ElectronicsStore', maybeElectronicsStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ElectronicsStore;
}
