 ...
 * schema:ElementarySchool
 *
 * This differs from ElementarySchool because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ElementarySchoolOrSubClass =
  | ElementarySchool
  ;

/**
 * schema:ElementarySchool - Validation schema (w/ JOI)
 */
export const ElementarySchoolJoiSchema = Joi.object({
  '@type': Joi.string().valid('ElementarySchool').required(),
 ...
 * schema:ElementarySchool - Validation schema (w/ JOI)
 *
 * This differs from ElementarySchoolJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ElementarySchoolOrSubClassJoiSchema = Joi.alternatives().try([
  ElementarySchoolJoiSchema,
]);

/**
 * Runtime validator for schema:ElementarySchool.
 *
 * If some data has a structure which matches a schema:ElementarySchool, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeElementarySchool = ElementarySchool.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeElementarySchool instanceof OaValidationError) {
 *   // From this point on, `maybeElementarySchool` will have type `OaValidationError`
 *   const error = maybeElementarySchool;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeElementarySchool` will have type `ElementarySchool`
 * const elementarySchool = maybeElementarySchool;
 * ```
 */
export function validateElementarySchool(maybeElementarySchool: unknown): ElementarySchool | OaValidationError {
  const { value, error } = ElementarySchoolJoiSchema.validate(maybeElementarySchool);
  if (error) {
    return new OaValidationError('ElementarySchool', maybeElementarySchool, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ElementarySchool;
}
