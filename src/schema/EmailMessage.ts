...
export type EmailMessage = {
   ...
   */
  sender?: schema.PersonOrSubClass | schema.AudienceOrSubClass | schema.OrganizationOrSubClass | string;
  /**
   ...
   */
  toRecipient?: schema.ContactPointOrSubClass | schema.AudienceOrSubClass | schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
   * A CreativeWork attached to the message.
   */
  messageAttachment?: schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  ccRecipient?: schema.ContactPointOrSubClass | schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
   ...
   */
  bccRecipient?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | schema.ContactPointOrSubClass | string;
  /**
 ...
 * schema:EmailMessage
 *
 * This differs from EmailMessage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EmailMessageOrSubClass =
  | EmailMessage
  ;

/**
 * schema:EmailMessage - Validation schema (w/ JOI)
 */
export const EmailMessageJoiSchema = Joi.object({
  '@type': Joi.string().valid('EmailMessage').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  sender: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.AudienceOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
  dateRead: Joi.string().isoDate(),
  toRecipient: Joi.alternatives().try(Joi.lazy(() => schema.ContactPointOrSubClassJoiSchema), Joi.lazy(() => schema.AudienceOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
  messageAttachment: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
  ...
  ccRecipient: Joi.alternatives().try(Joi.lazy(() => schema.ContactPointOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
  dateSent: Joi.string().isoDate(),
  dateReceived: Joi.string().isoDate(),
  bccRecipient: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.ContactPointOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:EmailMessage - Validation schema (w/ JOI)
 *
 * This differs from EmailMessageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EmailMessageOrSubClassJoiSchema = Joi.alternatives().try([
  EmailMessageJoiSchema,
]);

/**
 * Runtime validator for schema:EmailMessage.
 *
 * If some data has a structure which matches a schema:EmailMessage, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEmailMessage = EmailMessage.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEmailMessage instanceof OaValidationError) {
 *   // From this point on, `maybeEmailMessage` will have type `OaValidationError`
 *   const error = maybeEmailMessage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEmailMessage` will have type `EmailMessage`
 * const emailMessage = maybeEmailMessage;
 * ```
 */
export function validateEmailMessage(maybeEmailMessage: unknown): EmailMessage | OaValidationError {
  const { value, error } = EmailMessageJoiSchema.validate(maybeEmailMessage);
  if (error) {
    return new OaValidationError('EmailMessage', maybeEmailMessage, error);
  }
