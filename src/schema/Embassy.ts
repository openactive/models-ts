 ...
 * schema:Embassy
 *
 * This differs from Embassy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EmbassyOrSubClass =
  | Embassy
  ;

/**
 * schema:Embassy - Validation schema (w/ JOI)
 */
export const EmbassyJoiSchema = Joi.object({
  '@type': Joi.string().valid('Embassy').required(),
 ...
 * schema:Embassy - Validation schema (w/ JOI)
 *
 * This differs from EmbassyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EmbassyOrSubClassJoiSchema = Joi.alternatives().try([
  EmbassyJoiSchema,
]);

/**
 * Runtime validator for schema:Embassy.
 *
 * If some data has a structure which matches a schema:Embassy, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEmbassy = Embassy.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEmbassy instanceof OaValidationError) {
 *   // From this point on, `maybeEmbassy` will have type `OaValidationError`
 *   const error = maybeEmbassy;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEmbassy` will have type `Embassy`
 * const embassy = maybeEmbassy;
 * ```
 */
export function validateEmbassy(maybeEmbassy: unknown): Embassy | OaValidationError {
  const { value, error } = EmbassyJoiSchema.validate(maybeEmbassy);
  if (error) {
    return new OaValidationError('Embassy', maybeEmbassy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Embassy;
}
