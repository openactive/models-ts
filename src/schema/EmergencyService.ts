 ...
 * schema:EmergencyService
 *
 * This differs from EmergencyService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EmergencyServiceOrSubClass =
  | EmergencyService
...
/**
 * schema:EmergencyService - Validation schema (w/ JOI)
 */
export const EmergencyServiceJoiSchema = Joi.object({
  '@type': Joi.string().valid('EmergencyService').required(),
 ...
 * schema:EmergencyService - Validation schema (w/ JOI)
 *
 * This differs from EmergencyServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EmergencyServiceOrSubClassJoiSchema = Joi.alternatives().try([
  EmergencyServiceJoiSchema,
...
/**
 * Runtime validator for schema:EmergencyService.
 *
 * If some data has a structure which matches a schema:EmergencyService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEmergencyService = EmergencyService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEmergencyService instanceof OaValidationError) {
 *   // From this point on, `maybeEmergencyService` will have type `OaValidationError`
 *   const error = maybeEmergencyService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEmergencyService` will have type `EmergencyService`
 * const emergencyService = maybeEmergencyService;
 * ```
 */
export function validateEmergencyService(maybeEmergencyService: unknown): EmergencyService | OaValidationError {
  const { value, error } = EmergencyServiceJoiSchema.validate(maybeEmergencyService);
  if (error) {
    return new OaValidationError('EmergencyService', maybeEmergencyService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EmergencyService;
}
