 ...
 * schema:EmployerReview
 *
 * This differs from EmployerReview because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EmployerReviewOrSubClass =
  | EmployerReview
  ;

/**
 * schema:EmployerReview - Validation schema (w/ JOI)
 */
export const EmployerReviewJoiSchema = Joi.object({
  '@type': Joi.string().valid('EmployerReview').required(),
 ...
 * schema:EmployerReview - Validation schema (w/ JOI)
 *
 * This differs from EmployerReviewJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EmployerReviewOrSubClassJoiSchema = Joi.alternatives().try([
  EmployerReviewJoiSchema,
]);

/**
 * Runtime validator for schema:EmployerReview.
 *
 * If some data has a structure which matches a schema:EmployerReview, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEmployerReview = validateEmployerReview(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEmployerReview instanceof OaValidationError) {
 *   // From this point on, `maybeEmployerReview` will have type `OaValidationError`
 *   const error = maybeEmployerReview;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEmployerReview` will have type `EmployerReview`
 * const employerReview = maybeEmployerReview;
 * ```
 */
export function validateEmployerReview(maybeEmployerReview: unknown): EmployerReview | OaValidationError {
  const { value, error } = EmployerReviewJoiSchema.validate(maybeEmployerReview);
  if (error) {
    return new OaValidationError('EmployerReview', maybeEmployerReview, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EmployerReview;
}
