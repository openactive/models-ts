 ...
 * schema:EmploymentAgency
 *
 * This differs from EmploymentAgency because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EmploymentAgencyOrSubClass =
  | EmploymentAgency
  ;

/**
 * schema:EmploymentAgency - Validation schema (w/ JOI)
 */
export const EmploymentAgencyJoiSchema = Joi.object({
  '@type': Joi.string().valid('EmploymentAgency').required(),
 ...
 * schema:EmploymentAgency - Validation schema (w/ JOI)
 *
 * This differs from EmploymentAgencyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EmploymentAgencyOrSubClassJoiSchema = Joi.alternatives().try([
  EmploymentAgencyJoiSchema,
]);

/**
 * Runtime validator for schema:EmploymentAgency.
 *
 * If some data has a structure which matches a schema:EmploymentAgency, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEmploymentAgency = EmploymentAgency.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEmploymentAgency instanceof OaValidationError) {
 *   // From this point on, `maybeEmploymentAgency` will have type `OaValidationError`
 *   const error = maybeEmploymentAgency;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEmploymentAgency` will have type `EmploymentAgency`
 * const employmentAgency = maybeEmploymentAgency;
 * ```
 */
export function validateEmploymentAgency(maybeEmploymentAgency: unknown): EmploymentAgency | OaValidationError {
  const { value, error } = EmploymentAgencyJoiSchema.validate(maybeEmploymentAgency);
  if (error) {
    return new OaValidationError('EmploymentAgency', maybeEmploymentAgency, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EmploymentAgency;
}
