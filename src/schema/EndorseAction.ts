...
export type EndorseAction = {
   ...
   */
  endorsee?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
 ...
 * schema:EndorseAction
 *
 * This differs from EndorseAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EndorseActionOrSubClass =
  | EndorseAction
  ;

/**
 * schema:EndorseAction - Validation schema (w/ JOI)
 */
export const EndorseActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('EndorseAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  endorsee: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:EndorseAction - Validation schema (w/ JOI)
 *
 * This differs from EndorseActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EndorseActionOrSubClassJoiSchema = Joi.alternatives().try([
  EndorseActionJoiSchema,
]);

/**
 * Runtime validator for schema:EndorseAction.
 *
 * If some data has a structure which matches a schema:EndorseAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeEndorseAction` will have type `EndorseAction`
 * const endorseAction = maybeEndorseAction;
 * ```
 */
export function validateEndorseAction(maybeEndorseAction: unknown): EndorseAction | OaValidationError {
  const { value, error } = EndorseActionJoiSchema.validate(maybeEndorseAction);
  if (error) {
    return new OaValidationError('EndorseAction', maybeEndorseAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EndorseAction;
}
