 ...
 * etc.
 */
export type EndorsementRatingOrSubClass =
  | EndorsementRating
  ;
 ...
 */
export const EndorsementRatingJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const EndorsementRatingOrSubClassJoiSchema = Joi.alternatives().try([
  EndorsementRatingJoiSchema,
]);
 ...
 * ```
 */
export function validateEndorsementRating(maybeEndorsementRating: unknown): EndorsementRating | OaValidationError {
  const { value, error } = EndorsementRatingJoiSchema.validate(maybeEndorsementRating);
  if (error) {
    return new OaValidationError('EndorsementRating', maybeEndorsementRating, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EndorsementRating;
}
