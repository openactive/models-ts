 ...
 * schema:Energy
 *
 * This differs from Energy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EnergyOrSubClass =
  | Energy
  ;
 ...
 */
export const EnergyJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const EnergyOrSubClassJoiSchema = Joi.alternatives().try([
  EnergyJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeEnergy = Energy.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateEnergy(maybeEnergy: unknown): Energy | OaValidationError {
  const { value, error } = EnergyJoiSchema.validate(maybeEnergy);
  if (error) {
    return new OaValidationError('Energy', maybeEnergy, error);
  }
