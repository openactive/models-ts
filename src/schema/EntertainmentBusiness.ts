 ...
 * schema:EntertainmentBusiness
 *
 * This differs from EntertainmentBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EntertainmentBusinessOrSubClass =
  | EntertainmentBusiness
  | schema.MovieTheaterOrSubClass
  | schema.ArtGalleryOrSubClass
  | schema.AmusementParkOrSubClass
  | schema.AdultEntertainmentOrSubClass
  | schema.ComedyClubOrSubClass
  | schema.CasinoOrSubClass
  | schema.NightClubOrSubClass
  ;

/**
 * schema:EntertainmentBusiness - Validation schema (w/ JOI)
 */
export const EntertainmentBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('EntertainmentBusiness').required(),
 ...
 * schema:EntertainmentBusiness - Validation schema (w/ JOI)
 *
 * This differs from EntertainmentBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EntertainmentBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  EntertainmentBusinessJoiSchema,
  Joi.lazy(() => schema.MovieTheaterOrSubClassJoiSchema),
  Joi.lazy(() => schema.ArtGalleryOrSubClassJoiSchema),
  Joi.lazy(() => schema.AmusementParkOrSubClassJoiSchema),
  Joi.lazy(() => schema.AdultEntertainmentOrSubClassJoiSchema),
  Joi.lazy(() => schema.ComedyClubOrSubClassJoiSchema),
  Joi.lazy(() => schema.CasinoOrSubClassJoiSchema),
  Joi.lazy(() => schema.NightClubOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:EntertainmentBusiness.
 *
 * If some data has a structure which matches a schema:EntertainmentBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEntertainmentBusiness = EntertainmentBusiness.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEntertainmentBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeEntertainmentBusiness` will have type `OaValidationError`
 *   const error = maybeEntertainmentBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEntertainmentBusiness` will have type `EntertainmentBusiness`
 * const entertainmentBusiness = maybeEntertainmentBusiness;
 * ```
 */
export function validateEntertainmentBusiness(maybeEntertainmentBusiness: unknown): EntertainmentBusiness | OaValidationError {
  const { value, error } = EntertainmentBusinessJoiSchema.validate(maybeEntertainmentBusiness);
  if (error) {
    return new OaValidationError('EntertainmentBusiness', maybeEntertainmentBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EntertainmentBusiness;
}
