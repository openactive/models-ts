 ...
 * etc.
 */
export type EnumerationOrSubClass =
  | Enumeration
  ;
 ...
 */
export const EnumerationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const EnumerationOrSubClassJoiSchema = Joi.alternatives().try([
  EnumerationJoiSchema,
]);
 ...
 * ```
 */
export function validateEnumeration(maybeEnumeration: unknown): Enumeration | OaValidationError {
  const { value, error } = EnumerationJoiSchema.validate(maybeEnumeration);
  if (error) {
    return new OaValidationError('Enumeration', maybeEnumeration, error);
  }
