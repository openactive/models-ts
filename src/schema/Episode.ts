 ...
 * schema:Episode
 *
 * This differs from Episode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EpisodeOrSubClass =
  | Episode
  | schema.PodcastEpisodeOrSubClass
  | schema.RadioEpisodeOrSubClass
  | schema.TVEpisodeOrSubClass
  ;

/**
 * schema:Episode - Validation schema (w/ JOI)
 */
export const EpisodeJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const EpisodeOrSubClassJoiSchema = Joi.alternatives().try([
  EpisodeJoiSchema,
 ...
 *
 * ```ts
 * const maybeEpisode = Episode.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateEpisode(maybeEpisode: unknown): Episode | OaValidationError {
  const { value, error } = EpisodeJoiSchema.validate(maybeEpisode);
  if (error) {
    return new OaValidationError('Episode', maybeEpisode, error);
  }
