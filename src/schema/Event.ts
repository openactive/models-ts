 ...
 * schema:Event
 *
 * This differs from Event_ because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type Event_OrSubClass =
  | Event_
  | schema.SaleEventOrSubClass
  | schema.FestivalOrSubClass
  | schema.MusicEventOrSubClass
  | schema.ExhibitionEventOrSubClass
  | schema.HackathonOrSubClass
  | schema.ComedyEventOrSubClass
  | schema.EventSeriesOrSubClass
  | schema.ChildrensEventOrSubClass
  | schema.UserInteractionOrSubClass
  | schema.PublicationEventOrSubClass
  | schema.CourseInstanceOrSubClass
  | schema.ScreeningEventOrSubClass
  | schema.DeliveryEventOrSubClass
  | schema.LiteraryEventOrSubClass
  | schema.SportsEventOrSubClass
  | schema.BusinessEventOrSubClass
  | schema.VisualArtsEventOrSubClass
  | schema.SocialEventOrSubClass
  | schema.TheaterEventOrSubClass
  | schema.DanceEventOrSubClass
  | schema.EducationEventOrSubClass
  | schema.FoodEventOrSubClass
  ;

/**
 * schema:Event - Validation schema (w/ JOI)
 */
export const Event_JoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const Event_OrSubClassJoiSchema = Joi.alternatives().try([
  Event_JoiSchema,
 ...
 * ```
 */
export function validateEvent_(maybeEvent_: unknown): Event_ | OaValidationError {
  const { value, error } = Event_JoiSchema.validate(maybeEvent_);
  if (error) {
    return new OaValidationError('Event_', maybeEvent_, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Event_;
}
