 ...
 * etc.
 */
export type EventReservationOrSubClass =
  | EventReservation
  ;
 ...
 */
export const EventReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const EventReservationOrSubClassJoiSchema = Joi.alternatives().try([
  EventReservationJoiSchema,
]);
 ...
 * ```
 */
export function validateEventReservation(maybeEventReservation: unknown): EventReservation | OaValidationError {
  const { value, error } = EventReservationJoiSchema.validate(maybeEventReservation);
  if (error) {
    return new OaValidationError('EventReservation', maybeEventReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EventReservation;
}
