 ...
 * schema:EventSeries
 *
 * This differs from EventSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EventSeriesOrSubClass =
  | EventSeries
  ;

/**
 * schema:EventSeries - Validation schema (w/ JOI)
 */
export const EventSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('EventSeries').required(),
 ...
 * schema:EventSeries - Validation schema (w/ JOI)
 *
 * This differs from EventSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EventSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  EventSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:EventSeries.
 *
 * If some data has a structure which matches a schema:EventSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEventSeries = EventSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEventSeries instanceof OaValidationError) {
 *   // From this point on, `maybeEventSeries` will have type `OaValidationError`
 *   const error = maybeEventSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEventSeries` will have type `EventSeries`
 * const eventSeries = maybeEventSeries;
 * ```
 */
export function validateEventSeries(maybeEventSeries: unknown): EventSeries | OaValidationError {
  const { value, error } = EventSeriesJoiSchema.validate(maybeEventSeries);
  if (error) {
    return new OaValidationError('EventSeries', maybeEventSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EventSeries;
}
