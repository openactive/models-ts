 ...
 * schema:EventVenue
 *
 * This differs from EventVenue because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type EventVenueOrSubClass =
  | EventVenue
  ;

/**
 * schema:EventVenue - Validation schema (w/ JOI)
 */
export const EventVenueJoiSchema = Joi.object({
  '@type': Joi.string().valid('EventVenue').required(),
 ...
 * schema:EventVenue - Validation schema (w/ JOI)
 *
 * This differs from EventVenueJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const EventVenueOrSubClassJoiSchema = Joi.alternatives().try([
  EventVenueJoiSchema,
]);

/**
 * Runtime validator for schema:EventVenue.
 *
 * If some data has a structure which matches a schema:EventVenue, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeEventVenue = EventVenue.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeEventVenue instanceof OaValidationError) {
 *   // From this point on, `maybeEventVenue` will have type `OaValidationError`
 *   const error = maybeEventVenue;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeEventVenue` will have type `EventVenue`
 * const eventVenue = maybeEventVenue;
 * ```
 */
export function validateEventVenue(maybeEventVenue: unknown): EventVenue | OaValidationError {
  const { value, error } = EventVenueJoiSchema.validate(maybeEventVenue);
  if (error) {
    return new OaValidationError('EventVenue', maybeEventVenue, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as EventVenue;
}
