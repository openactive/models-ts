 ...
 * etc.
 */
export type ExerciseActionOrSubClass =
  | ExerciseAction
  ;
 ...
 */
export const ExerciseActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ExerciseActionOrSubClassJoiSchema = Joi.alternatives().try([
  ExerciseActionJoiSchema,
]);
 ...
 * ```
 */
export function validateExerciseAction(maybeExerciseAction: unknown): ExerciseAction | OaValidationError {
  const { value, error } = ExerciseActionJoiSchema.validate(maybeExerciseAction);
  if (error) {
    return new OaValidationError('ExerciseAction', maybeExerciseAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ExerciseAction;
}
