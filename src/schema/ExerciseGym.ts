 ...
 * schema:ExerciseGym
 *
 * This differs from ExerciseGym because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ExerciseGymOrSubClass =
  | ExerciseGym
  ;

/**
 * schema:ExerciseGym - Validation schema (w/ JOI)
 */
export const ExerciseGymJoiSchema = Joi.object({
  '@type': Joi.string().valid('ExerciseGym').required(),
 ...
 * schema:ExerciseGym - Validation schema (w/ JOI)
 *
 * This differs from ExerciseGymJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ExerciseGymOrSubClassJoiSchema = Joi.alternatives().try([
  ExerciseGymJoiSchema,
]);

/**
 * Runtime validator for schema:ExerciseGym.
 *
 * If some data has a structure which matches a schema:ExerciseGym, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeExerciseGym = ExerciseGym.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeExerciseGym instanceof OaValidationError) {
 *   // From this point on, `maybeExerciseGym` will have type `OaValidationError`
 *   const error = maybeExerciseGym;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeExerciseGym` will have type `ExerciseGym`
 * const exerciseGym = maybeExerciseGym;
 * ```
 */
export function validateExerciseGym(maybeExerciseGym: unknown): ExerciseGym | OaValidationError {
  const { value, error } = ExerciseGymJoiSchema.validate(maybeExerciseGym);
  if (error) {
    return new OaValidationError('ExerciseGym', maybeExerciseGym, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ExerciseGym;
}
