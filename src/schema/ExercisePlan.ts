...
export type ExercisePlan = {
   ...
   */
  workload?: schema.EnergyOrSubClass | schema.QuantitativeValueOrSubClass | string;
  /**
   * How often one should engage in the activity.
   */
  activityFrequency?: string | schema.QuantitativeValueOrSubClass;
  /**
   * Quantitative measure gauging the degree of force involved in the exercise, for example, heartbeats per minute. May include the velocity of the movement.
   */
  intensity?: schema.QuantitativeValueOrSubClass | string;
  /**
   * Number of times one should repeat the activity.
   */
  repetitions?: number | schema.QuantitativeValueOrSubClass | string;
  /**
   * Length of time to engage in the activity.
   */
  activityDuration?: schema.QuantitativeValueOrSubClass | string;
  /**
   * How often one should break from the activity.
   */
  restPeriods?: string | schema.QuantitativeValueOrSubClass;
  /**
 ...
 * schema:ExercisePlan
 *
 * This differs from ExercisePlan because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ExercisePlanOrSubClass =
  | ExercisePlan
  ;

/**
 * schema:ExercisePlan - Validation schema (w/ JOI)
 */
export const ExercisePlanJoiSchema = Joi.object({
  '@type': Joi.string().valid('ExercisePlan').required(),
  ...
  additionalVariable: Joi.string(),
  workload: Joi.alternatives().try(Joi.lazy(() => schema.EnergyOrSubClassJoiSchema), Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
  activityFrequency: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
  intensity: Joi.alternatives().try(Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  repetitions: Joi.alternatives().try(Joi.number(), Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
  activityDuration: Joi.alternatives().try(Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  restPeriods: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ExercisePlan - Validation schema (w/ JOI)
 *
 * This differs from ExercisePlanJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ExercisePlanOrSubClassJoiSchema = Joi.alternatives().try([
  ExercisePlanJoiSchema,
]);

/**
 * Runtime validator for schema:ExercisePlan.
 *
 * If some data has a structure which matches a schema:ExercisePlan, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeExercisePlan = ExercisePlan.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeExercisePlan instanceof OaValidationError) {
 *   // From this point on, `maybeExercisePlan` will have type `OaValidationError`
 *   const error = maybeExercisePlan;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeExercisePlan` will have type `ExercisePlan`
 * const exercisePlan = maybeExercisePlan;
 * ```
 */
export function validateExercisePlan(maybeExercisePlan: unknown): ExercisePlan | OaValidationError {
  const { value, error } = ExercisePlanJoiSchema.validate(maybeExercisePlan);
  if (error) {
    return new OaValidationError('ExercisePlan', maybeExercisePlan, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ExercisePlan;
}
