 ...
 * schema:ExhibitionEvent
 *
 * This differs from ExhibitionEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ExhibitionEventOrSubClass =
  | ExhibitionEvent
  ;

/**
 * schema:ExhibitionEvent - Validation schema (w/ JOI)
 */
export const ExhibitionEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('ExhibitionEvent').required(),
 ...
 * schema:ExhibitionEvent - Validation schema (w/ JOI)
 *
 * This differs from ExhibitionEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ExhibitionEventOrSubClassJoiSchema = Joi.alternatives().try([
  ExhibitionEventJoiSchema,
]);

/**
 * Runtime validator for schema:ExhibitionEvent.
 *
 * If some data has a structure which matches a schema:ExhibitionEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeExhibitionEvent = ExhibitionEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeExhibitionEvent instanceof OaValidationError) {
 *   // From this point on, `maybeExhibitionEvent` will have type `OaValidationError`
 *   const error = maybeExhibitionEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeExhibitionEvent` will have type `ExhibitionEvent`
 * const exhibitionEvent = maybeExhibitionEvent;
 * ```
 */
export function validateExhibitionEvent(maybeExhibitionEvent: unknown): ExhibitionEvent | OaValidationError {
  const { value, error } = ExhibitionEventJoiSchema.validate(maybeExhibitionEvent);
  if (error) {
    return new OaValidationError('ExhibitionEvent', maybeExhibitionEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ExhibitionEvent;
}
