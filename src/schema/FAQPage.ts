 ...
 * schema:FAQPage
 *
 * This differs from FAQPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FAQPageOrSubClass =
  | FAQPage
  ;

/**
 * schema:FAQPage - Validation schema (w/ JOI)
 */
export const FAQPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('FAQPage').required(),
 ...
 * schema:FAQPage - Validation schema (w/ JOI)
 *
 * This differs from FAQPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FAQPageOrSubClassJoiSchema = Joi.alternatives().try([
  FAQPageJoiSchema,
]);

/**
 * Runtime validator for schema:FAQPage.
 *
 * If some data has a structure which matches a schema:FAQPage, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFAQPage = FAQPage.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFAQPage instanceof OaValidationError) {
 *   // From this point on, `maybeFAQPage` will have type `OaValidationError`
 *   const error = maybeFAQPage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFAQPage` will have type `FAQPage`
 * const fAQPage = maybeFAQPage;
 * ```
 */
export function validateFAQPage(maybeFAQPage: unknown): FAQPage | OaValidationError {
  const { value, error } = FAQPageJoiSchema.validate(maybeFAQPage);
  if (error) {
    return new OaValidationError('FAQPage', maybeFAQPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FAQPage;
}
