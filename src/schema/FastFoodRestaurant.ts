 ...
 * schema:FastFoodRestaurant
 *
 * This differs from FastFoodRestaurant because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FastFoodRestaurantOrSubClass =
  | FastFoodRestaurant
  ;

/**
 * schema:FastFoodRestaurant - Validation schema (w/ JOI)
 */
export const FastFoodRestaurantJoiSchema = Joi.object({
  '@type': Joi.string().valid('FastFoodRestaurant').required(),
 ...
 * schema:FastFoodRestaurant - Validation schema (w/ JOI)
 *
 * This differs from FastFoodRestaurantJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FastFoodRestaurantOrSubClassJoiSchema = Joi.alternatives().try([
  FastFoodRestaurantJoiSchema,
]);

/**
 * Runtime validator for schema:FastFoodRestaurant.
 *
 * If some data has a structure which matches a schema:FastFoodRestaurant, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFastFoodRestaurant = FastFoodRestaurant.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFastFoodRestaurant instanceof OaValidationError) {
 *   // From this point on, `maybeFastFoodRestaurant` will have type `OaValidationError`
 *   const error = maybeFastFoodRestaurant;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFastFoodRestaurant` will have type `FastFoodRestaurant`
 * const fastFoodRestaurant = maybeFastFoodRestaurant;
 * ```
 */
export function validateFastFoodRestaurant(maybeFastFoodRestaurant: unknown): FastFoodRestaurant | OaValidationError {
  const { value, error } = FastFoodRestaurantJoiSchema.validate(maybeFastFoodRestaurant);
  if (error) {
    return new OaValidationError('FastFoodRestaurant', maybeFastFoodRestaurant, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FastFoodRestaurant;
}
