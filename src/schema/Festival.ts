 ...
 * schema:Festival
 *
 * This differs from Festival because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FestivalOrSubClass =
  | Festival
  ;

/**
 * schema:Festival - Validation schema (w/ JOI)
 */
export const FestivalJoiSchema = Joi.object({
  '@type': Joi.string().valid('Festival').required(),
 ...
 * schema:Festival - Validation schema (w/ JOI)
 *
 * This differs from FestivalJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FestivalOrSubClassJoiSchema = Joi.alternatives().try([
  FestivalJoiSchema,
]);

/**
 * Runtime validator for schema:Festival.
 *
 * If some data has a structure which matches a schema:Festival, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFestival = Festival.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFestival instanceof OaValidationError) {
 *   // From this point on, `maybeFestival` will have type `OaValidationError`
 *   const error = maybeFestival;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFestival` will have type `Festival`
 * const festival = maybeFestival;
 * ```
 */
export function validateFestival(maybeFestival: unknown): Festival | OaValidationError {
  const { value, error } = FestivalJoiSchema.validate(maybeFestival);
  if (error) {
    return new OaValidationError('Festival', maybeFestival, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Festival;
}
