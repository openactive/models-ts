 ...
 * schema:FilmAction
 *
 * This differs from FilmAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FilmActionOrSubClass =
  | FilmAction
  ;

/**
 * schema:FilmAction - Validation schema (w/ JOI)
 */
export const FilmActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('FilmAction').required(),
 ...
 * schema:FilmAction - Validation schema (w/ JOI)
 *
 * This differs from FilmActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FilmActionOrSubClassJoiSchema = Joi.alternatives().try([
  FilmActionJoiSchema,
]);

/**
 * Runtime validator for schema:FilmAction.
 *
 * If some data has a structure which matches a schema:FilmAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFilmAction = FilmAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFilmAction instanceof OaValidationError) {
 *   // From this point on, `maybeFilmAction` will have type `OaValidationError`
 *   const error = maybeFilmAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFilmAction` will have type `FilmAction`
 * const filmAction = maybeFilmAction;
 * ```
 */
export function validateFilmAction(maybeFilmAction: unknown): FilmAction | OaValidationError {
  const { value, error } = FilmActionJoiSchema.validate(maybeFilmAction);
  if (error) {
    return new OaValidationError('FilmAction', maybeFilmAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FilmAction;
}
