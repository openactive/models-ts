 ...
 * schema:FinancialProduct
 *
 * This differs from FinancialProduct because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FinancialProductOrSubClass =
  | FinancialProduct
  | schema.PaymentCardOrSubClass
  | schema.CurrencyConversionServiceOrSubClass
  | schema.PaymentServiceOrSubClass
  | schema.BankAccountOrSubClass
  | schema.LoanOrCreditOrSubClass
  | schema.InvestmentOrDepositOrSubClass
  ;

/**
 * schema:FinancialProduct - Validation schema (w/ JOI)
 */
export const FinancialProductJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FinancialProductOrSubClassJoiSchema = Joi.alternatives().try([
  FinancialProductJoiSchema,
 ...
 * ```
 */
export function validateFinancialProduct(maybeFinancialProduct: unknown): FinancialProduct | OaValidationError {
  const { value, error } = FinancialProductJoiSchema.validate(maybeFinancialProduct);
  if (error) {
    return new OaValidationError('FinancialProduct', maybeFinancialProduct, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FinancialProduct;
}
