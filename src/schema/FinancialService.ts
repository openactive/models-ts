 ...
 * schema:FinancialService
 *
 * This differs from FinancialService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FinancialServiceOrSubClass =
  | FinancialService
  | schema.AutomatedTellerOrSubClass
  | schema.InsuranceAgencyOrSubClass
  | schema.BankOrCreditUnionOrSubClass
  | schema.AccountingServiceOrSubClass
  ;

/**
 * schema:FinancialService - Validation schema (w/ JOI)
 */
export const FinancialServiceJoiSchema = Joi.object({
  '@type': Joi.string().valid('FinancialService').required(),
 ...
 * schema:FinancialService - Validation schema (w/ JOI)
 *
 * This differs from FinancialServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FinancialServiceOrSubClassJoiSchema = Joi.alternatives().try([
  FinancialServiceJoiSchema,
  Joi.lazy(() => schema.AutomatedTellerOrSubClassJoiSchema),
  Joi.lazy(() => schema.InsuranceAgencyOrSubClassJoiSchema),
  Joi.lazy(() => schema.BankOrCreditUnionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AccountingServiceOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:FinancialService.
 *
 * If some data has a structure which matches a schema:FinancialService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFinancialService = FinancialService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFinancialService instanceof OaValidationError) {
 *   // From this point on, `maybeFinancialService` will have type `OaValidationError`
 *   const error = maybeFinancialService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFinancialService` will have type `FinancialService`
 * const financialService = maybeFinancialService;
 * ```
 */
export function validateFinancialService(maybeFinancialService: unknown): FinancialService | OaValidationError {
  const { value, error } = FinancialServiceJoiSchema.validate(maybeFinancialService);
  if (error) {
    return new OaValidationError('FinancialService', maybeFinancialService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FinancialService;
}
