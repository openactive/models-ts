 ...
 * schema:FindAction
 *
 * This differs from FindAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FindActionOrSubClass =
  | FindAction
  | schema.TrackActionOrSubClass
  | schema.CheckActionOrSubClass
  | schema.DiscoverActionOrSubClass
  ;

/**
 * schema:FindAction - Validation schema (w/ JOI)
 */
export const FindActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FindActionOrSubClassJoiSchema = Joi.alternatives().try([
  FindActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeFindAction = FindAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateFindAction(maybeFindAction: unknown): FindAction | OaValidationError {
  const { value, error } = FindActionJoiSchema.validate(maybeFindAction);
  if (error) {
    return new OaValidationError('FindAction', maybeFindAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FindAction;
}
