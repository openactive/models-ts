 ...
 * schema:FireStation
 *
 * This differs from FireStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FireStationOrSubClass =
  | FireStation
  ;

/**
 * schema:FireStation - Validation schema (w/ JOI)
 */
export const FireStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('FireStation').required(),
 ...
 * schema:FireStation - Validation schema (w/ JOI)
 *
 * This differs from FireStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FireStationOrSubClassJoiSchema = Joi.alternatives().try([
  FireStationJoiSchema,
]);

/**
 * Runtime validator for schema:FireStation.
 *
 * If some data has a structure which matches a schema:FireStation, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeFireStation` will have type `FireStation`
 * const fireStation = maybeFireStation;
 * ```
 */
export function validateFireStation(maybeFireStation: unknown): FireStation | OaValidationError {
  const { value, error } = FireStationJoiSchema.validate(maybeFireStation);
  if (error) {
    return new OaValidationError('FireStation', maybeFireStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FireStation;
}
