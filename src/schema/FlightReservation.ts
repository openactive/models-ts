...
export type FlightReservation = {
   ...
   */
  passengerPriorityStatus?: schema.QualitativeValue | string;
  /**
 ...
 * schema:FlightReservation
 *
 * This differs from FlightReservation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FlightReservationOrSubClass =
  | FlightReservation
  ;
 ...
 */
export const FlightReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FlightReservationOrSubClassJoiSchema = Joi.alternatives().try([
  FlightReservationJoiSchema,
]);
 ...
 * ```
 */
export function validateFlightReservation(maybeFlightReservation: unknown): FlightReservation | OaValidationError {
  const { value, error } = FlightReservationJoiSchema.validate(maybeFlightReservation);
  if (error) {
    return new OaValidationError('FlightReservation', maybeFlightReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FlightReservation;
}
