 ...
 * schema:Florist
 *
 * This differs from Florist because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FloristOrSubClass =
  | Florist
  ;

/**
 * schema:Florist - Validation schema (w/ JOI)
 */
export const FloristJoiSchema = Joi.object({
  '@type': Joi.string().valid('Florist').required(),
 ...
 * schema:Florist - Validation schema (w/ JOI)
 *
 * This differs from FloristJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FloristOrSubClassJoiSchema = Joi.alternatives().try([
  FloristJoiSchema,
]);

/**
 * Runtime validator for schema:Florist.
 *
 * If some data has a structure which matches a schema:Florist, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFlorist = Florist.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFlorist instanceof OaValidationError) {
 *   // From this point on, `maybeFlorist` will have type `OaValidationError`
 *   const error = maybeFlorist;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFlorist` will have type `Florist`
 * const florist = maybeFlorist;
 * ```
 */
export function validateFlorist(maybeFlorist: unknown): Florist | OaValidationError {
  const { value, error } = FloristJoiSchema.validate(maybeFlorist);
  if (error) {
    return new OaValidationError('Florist', maybeFlorist, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Florist;
}
