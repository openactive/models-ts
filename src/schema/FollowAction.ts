...
export type FollowAction = {
   ...
   */
  followee?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
 ...
 * schema:FollowAction
 *
 * This differs from FollowAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FollowActionOrSubClass =
  | FollowAction
  ;

/**
 * schema:FollowAction - Validation schema (w/ JOI)
 */
export const FollowActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('FollowAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  followee: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:FollowAction - Validation schema (w/ JOI)
 *
 * This differs from FollowActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FollowActionOrSubClassJoiSchema = Joi.alternatives().try([
  FollowActionJoiSchema,
]);

/**
 * Runtime validator for schema:FollowAction.
 *
 * If some data has a structure which matches a schema:FollowAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFollowAction = FollowAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFollowAction instanceof OaValidationError) {
 *   // From this point on, `maybeFollowAction` will have type `OaValidationError`
 *   const error = maybeFollowAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFollowAction` will have type `FollowAction`
 * const followAction = maybeFollowAction;
 * ```
 */
export function validateFollowAction(maybeFollowAction: unknown): FollowAction | OaValidationError {
  const { value, error } = FollowActionJoiSchema.validate(maybeFollowAction);
  if (error) {
    return new OaValidationError('FollowAction', maybeFollowAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FollowAction;
}
