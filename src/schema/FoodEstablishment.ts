...
export type FoodEstablishment = {
   ...
   */
  menu?: string | schema.MenuOrSubClass;
  /**
   ...
   * Either the actual menu as a structured representation, as text, or a URL of the menu.
   */
  hasMenu?: string | schema.MenuOrSubClass;
  /**
 ...
 * schema:FoodEstablishment
 *
 * This differs from FoodEstablishment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FoodEstablishmentOrSubClass =
  | FoodEstablishment
  | schema.WineryOrSubClass
  | schema.BreweryOrSubClass
  | schema.DistilleryOrSubClass
  | schema.BarOrPubOrSubClass
  | schema.FastFoodRestaurantOrSubClass
  | schema.IceCreamShopOrSubClass
  | schema.CafeOrCoffeeShopOrSubClass
  | schema.BakeryOrSubClass
  | schema.RestaurantOrSubClass
  ;

/**
 * schema:FoodEstablishment - Validation schema (w/ JOI)
 */
export const FoodEstablishmentJoiSchema = Joi.object({
  '@type': Joi.string().valid('FoodEstablishment').required(),
 ...
 * schema:FoodEstablishment - Validation schema (w/ JOI)
 *
 * This differs from FoodEstablishmentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FoodEstablishmentOrSubClassJoiSchema = Joi.alternatives().try([
  FoodEstablishmentJoiSchema,
  Joi.lazy(() => schema.WineryOrSubClassJoiSchema),
  Joi.lazy(() => schema.BreweryOrSubClassJoiSchema),
  Joi.lazy(() => schema.DistilleryOrSubClassJoiSchema),
  Joi.lazy(() => schema.BarOrPubOrSubClassJoiSchema),
  Joi.lazy(() => schema.FastFoodRestaurantOrSubClassJoiSchema),
  Joi.lazy(() => schema.IceCreamShopOrSubClassJoiSchema),
  Joi.lazy(() => schema.CafeOrCoffeeShopOrSubClassJoiSchema),
  Joi.lazy(() => schema.BakeryOrSubClassJoiSchema),
  Joi.lazy(() => schema.RestaurantOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:FoodEstablishment.
 *
 * If some data has a structure which matches a schema:FoodEstablishment, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFoodEstablishment = FoodEstablishment.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFoodEstablishment instanceof OaValidationError) {
 *   // From this point on, `maybeFoodEstablishment` will have type `OaValidationError`
 *   const error = maybeFoodEstablishment;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFoodEstablishment` will have type `FoodEstablishment`
 * const foodEstablishment = maybeFoodEstablishment;
 * ```
 */
export function validateFoodEstablishment(maybeFoodEstablishment: unknown): FoodEstablishment | OaValidationError {
  const { value, error } = FoodEstablishmentJoiSchema.validate(maybeFoodEstablishment);
  if (error) {
    return new OaValidationError('FoodEstablishment', maybeFoodEstablishment, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FoodEstablishment;
}
