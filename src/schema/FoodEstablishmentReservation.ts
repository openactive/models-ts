 ...
 */
export type FoodEstablishmentReservation = {
 ...
 * etc.
 */
export type FoodEstablishmentReservationOrSubClass =
 ...
 */
export const FoodEstablishmentReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FoodEstablishmentReservationOrSubClassJoiSchema = Joi.alternatives().try([
  FoodEstablishmentReservationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeFoodEstablishmentReservation = FoodEstablishmentReservation.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateFoodEstablishmentReservation(maybeFoodEstablishmentReservation: unknown): FoodEstablishmentReservation | OaValidationError {
  const { value, error } = FoodEstablishmentReservationJoiSchema.validate(maybeFoodEstablishmentReservation);
  if (error) {
    return new OaValidationError('FoodEstablishmentReservation', maybeFoodEstablishmentReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FoodEstablishmentReservation;
}
