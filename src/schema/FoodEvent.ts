 ...
 * schema:FoodEvent
 *
 * This differs from FoodEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FoodEventOrSubClass =
  | FoodEvent
  ;

/**
 * schema:FoodEvent - Validation schema (w/ JOI)
 */
export const FoodEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('FoodEvent').required(),
 ...
 * schema:FoodEvent - Validation schema (w/ JOI)
 *
 * This differs from FoodEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FoodEventOrSubClassJoiSchema = Joi.alternatives().try([
  FoodEventJoiSchema,
]);

/**
 * Runtime validator for schema:FoodEvent.
 *
 * If some data has a structure which matches a schema:FoodEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFoodEvent = FoodEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFoodEvent instanceof OaValidationError) {
 *   // From this point on, `maybeFoodEvent` will have type `OaValidationError`
 *   const error = maybeFoodEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFoodEvent` will have type `FoodEvent`
 * const foodEvent = maybeFoodEvent;
 * ```
 */
export function validateFoodEvent(maybeFoodEvent: unknown): FoodEvent | OaValidationError {
  const { value, error } = FoodEventJoiSchema.validate(maybeFoodEvent);
  if (error) {
    return new OaValidationError('FoodEvent', maybeFoodEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FoodEvent;
}
