 ...
 * schema:FoodService
 *
 * This differs from FoodService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FoodServiceOrSubClass =
  | FoodService
  ;
 ...
 */
export const FoodServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const FoodServiceOrSubClassJoiSchema = Joi.alternatives().try([
  FoodServiceJoiSchema,
]);
 ...
 * ```
 */
export function validateFoodService(maybeFoodService: unknown): FoodService | OaValidationError {
  const { value, error } = FoodServiceJoiSchema.validate(maybeFoodService);
  if (error) {
    return new OaValidationError('FoodService', maybeFoodService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FoodService;
}
