 ...
 * schema:FundingAgency
 *
 * This differs from FundingAgency because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FundingAgencyOrSubClass =
  | FundingAgency
  ;

/**
 * schema:FundingAgency - Validation schema (w/ JOI)
 */
export const FundingAgencyJoiSchema = Joi.object({
  '@type': Joi.string().valid('FundingAgency').required(),
 ...
 * schema:FundingAgency - Validation schema (w/ JOI)
 *
 * This differs from FundingAgencyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FundingAgencyOrSubClassJoiSchema = Joi.alternatives().try([
  FundingAgencyJoiSchema,
]);

/**
 * Runtime validator for schema:FundingAgency.
 *
 * If some data has a structure which matches a schema:FundingAgency, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFundingAgency = FundingAgency.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFundingAgency instanceof OaValidationError) {
 *   // From this point on, `maybeFundingAgency` will have type `OaValidationError`
 *   const error = maybeFundingAgency;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFundingAgency` will have type `FundingAgency`
 * const fundingAgency = maybeFundingAgency;
 * ```
 */
export function validateFundingAgency(maybeFundingAgency: unknown): FundingAgency | OaValidationError {
  const { value, error } = FundingAgencyJoiSchema.validate(maybeFundingAgency);
  if (error) {
    return new OaValidationError('FundingAgency', maybeFundingAgency, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FundingAgency;
}
