 ...
 * schema:FundingScheme
 *
 * This differs from FundingScheme because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FundingSchemeOrSubClass =
  | FundingScheme
  ;

/**
 * schema:FundingScheme - Validation schema (w/ JOI)
 */
export const FundingSchemeJoiSchema = Joi.object({
  '@type': Joi.string().valid('FundingScheme').required(),
 ...
 * schema:FundingScheme - Validation schema (w/ JOI)
 *
 * This differs from FundingSchemeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FundingSchemeOrSubClassJoiSchema = Joi.alternatives().try([
  FundingSchemeJoiSchema,
]);

/**
 * Runtime validator for schema:FundingScheme.
 *
 * If some data has a structure which matches a schema:FundingScheme, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeFundingScheme` will have type `FundingScheme`
 * const fundingScheme = maybeFundingScheme;
 * ```
 */
export function validateFundingScheme(maybeFundingScheme: unknown): FundingScheme | OaValidationError {
  const { value, error } = FundingSchemeJoiSchema.validate(maybeFundingScheme);
  if (error) {
    return new OaValidationError('FundingScheme', maybeFundingScheme, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FundingScheme;
}
