 ...
 * schema:FurnitureStore
 *
 * This differs from FurnitureStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type FurnitureStoreOrSubClass =
  | FurnitureStore
  ;

/**
 * schema:FurnitureStore - Validation schema (w/ JOI)
 */
export const FurnitureStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('FurnitureStore').required(),
 ...
 * schema:FurnitureStore - Validation schema (w/ JOI)
 *
 * This differs from FurnitureStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const FurnitureStoreOrSubClassJoiSchema = Joi.alternatives().try([
  FurnitureStoreJoiSchema,
]);

/**
 * Runtime validator for schema:FurnitureStore.
 *
 * If some data has a structure which matches a schema:FurnitureStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeFurnitureStore = FurnitureStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeFurnitureStore instanceof OaValidationError) {
 *   // From this point on, `maybeFurnitureStore` will have type `OaValidationError`
 *   const error = maybeFurnitureStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeFurnitureStore` will have type `FurnitureStore`
 * const furnitureStore = maybeFurnitureStore;
 * ```
 */
export function validateFurnitureStore(maybeFurnitureStore: unknown): FurnitureStore | OaValidationError {
  const { value, error } = FurnitureStoreJoiSchema.validate(maybeFurnitureStore);
  if (error) {
    return new OaValidationError('FurnitureStore', maybeFurnitureStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as FurnitureStore;
}
