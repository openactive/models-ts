 ...
 * schema:Game
 *
 * This differs from Game because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GameOrSubClass =
  | Game
  | schema.VideoGameOrSubClass
  ;

/**
 * schema:Game - Validation schema (w/ JOI)
 */
export const GameJoiSchema = Joi.object({
  '@type': Joi.string().valid('Game').required(),
 ...
 * schema:Game - Validation schema (w/ JOI)
 *
 * This differs from GameJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GameOrSubClassJoiSchema = Joi.alternatives().try([
  GameJoiSchema,
  Joi.lazy(() => schema.VideoGameOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Game.
 *
 * If some data has a structure which matches a schema:Game, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGame = Game.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGame instanceof OaValidationError) {
 *   // From this point on, `maybeGame` will have type `OaValidationError`
 *   const error = maybeGame;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGame` will have type `Game`
 * const game = maybeGame;
 * ```
 */
export function validateGame(maybeGame: unknown): Game | OaValidationError {
  const { value, error } = GameJoiSchema.validate(maybeGame);
  if (error) {
    return new OaValidationError('Game', maybeGame, error);
  }
