...
export type GameServer = {
   ...
   */
  serverStatus?: schema.GameServerStatusOrSubClass | string;
  /**
   * Video game which is played on this server.
   */
  game?: schema.VideoGameOrSubClass | string;
  /**
 ...
 * schema:GameServer
 *
 * This differs from GameServer because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GameServerOrSubClass =
  | GameServer
  ;
 ...
 */
export const GameServerJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const GameServerOrSubClassJoiSchema = Joi.alternatives().try([
  GameServerJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeGameServer = validateGameServer(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateGameServer(maybeGameServer: unknown): GameServer | OaValidationError {
  const { value, error } = GameServerJoiSchema.validate(maybeGameServer);
  if (error) {
    return new OaValidationError('GameServer', maybeGameServer, error);
  }
