 ...
 * schema:GardenStore
 *
 * This differs from GardenStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GardenStoreOrSubClass =
  | GardenStore
  ;

/**
 * schema:GardenStore - Validation schema (w/ JOI)
 */
export const GardenStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('GardenStore').required(),
 ...
 * schema:GardenStore - Validation schema (w/ JOI)
 *
 * This differs from GardenStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GardenStoreOrSubClassJoiSchema = Joi.alternatives().try([
  GardenStoreJoiSchema,
]);

/**
 * Runtime validator for schema:GardenStore.
 *
 * If some data has a structure which matches a schema:GardenStore, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeGardenStore` will have type `GardenStore`
 * const gardenStore = maybeGardenStore;
 * ```
 */
export function validateGardenStore(maybeGardenStore: unknown): GardenStore | OaValidationError {
  const { value, error } = GardenStoreJoiSchema.validate(maybeGardenStore);
  if (error) {
    return new OaValidationError('GardenStore', maybeGardenStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GardenStore;
}
