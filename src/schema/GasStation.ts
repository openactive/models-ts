 ...
 * schema:GasStation
 *
 * This differs from GasStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GasStationOrSubClass =
  | GasStation
  ;

/**
 * schema:GasStation - Validation schema (w/ JOI)
 */
export const GasStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('GasStation').required(),
 ...
 * schema:GasStation - Validation schema (w/ JOI)
 *
 * This differs from GasStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GasStationOrSubClassJoiSchema = Joi.alternatives().try([
  GasStationJoiSchema,
]);

/**
 * Runtime validator for schema:GasStation.
 *
 * If some data has a structure which matches a schema:GasStation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGasStation = GasStation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGasStation instanceof OaValidationError) {
 *   // From this point on, `maybeGasStation` will have type `OaValidationError`
 *   const error = maybeGasStation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGasStation` will have type `GasStation`
 * const gasStation = maybeGasStation;
 * ```
 */
export function validateGasStation(maybeGasStation: unknown): GasStation | OaValidationError {
  const { value, error } = GasStationJoiSchema.validate(maybeGasStation);
  if (error) {
    return new OaValidationError('GasStation', maybeGasStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GasStation;
}
