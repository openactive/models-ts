 ...
 */
export type GatedResidenceCommunity = {
 ...
 * schema:GatedResidenceCommunity
 *
 * This differs from GatedResidenceCommunity because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GatedResidenceCommunityOrSubClass =
  | GatedResidenceCommunity
  ;

/**
 * schema:GatedResidenceCommunity - Validation schema (w/ JOI)
 */
export const GatedResidenceCommunityJoiSchema = Joi.object({
  '@type': Joi.string().valid('GatedResidenceCommunity').required(),
 ...
 * schema:GatedResidenceCommunity - Validation schema (w/ JOI)
 *
 * This differs from GatedResidenceCommunityJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GatedResidenceCommunityOrSubClassJoiSchema = Joi.alternatives().try([
  GatedResidenceCommunityJoiSchema,
]);

/**
 * Runtime validator for schema:GatedResidenceCommunity.
 *
 * If some data has a structure which matches a schema:GatedResidenceCommunity, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeGatedResidenceCommunity` will have type `GatedResidenceCommunity`
 * const gatedResidenceCommunity = maybeGatedResidenceCommunity;
 * ```
 */
export function validateGatedResidenceCommunity(maybeGatedResidenceCommunity: unknown): GatedResidenceCommunity | OaValidationError {
  const { value, error } = GatedResidenceCommunityJoiSchema.validate(maybeGatedResidenceCommunity);
  if (error) {
    return new OaValidationError('GatedResidenceCommunity', maybeGatedResidenceCommunity, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GatedResidenceCommunity;
}
