...
export type Gene = {
   ...
   */
  encodesBioChemEntity?: schema.BioChemEntityOrSubClass | string;
  /**
   * Another gene which is a variation of this one.
   */
  alternativeOf?: schema.GeneOrSubClass | string;
  /**
   * Tissue, organ, biological sample, etc in which activity of this gene has been observed experimentally. For example brain, digestive system.
   */
  expressedIn?: schema.DefinedTermOrSubClass | schema.AnatomicalSystemOrSubClass | schema.AnatomicalStructureOrSubClass | schema.BioChemEntityOrSubClass | string;
  /**
 ...
 * schema:Gene
 *
 * This differs from Gene because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GeneOrSubClass =
  | Gene
  ;

/**
 * schema:Gene - Validation schema (w/ JOI)
 */
export const GeneJoiSchema = Joi.object({
  '@type': Joi.string().valid('Gene').required(),
  ...
  encodesBioChemEntity: Joi.alternatives().try(Joi.lazy(() => schema.BioChemEntityOrSubClassJoiSchema), Joi.string().uri()),
  alternativeOf: Joi.alternatives().try(Joi.lazy(() => schema.GeneOrSubClassJoiSchema), Joi.string().uri()),
  expressedIn: Joi.alternatives().try(Joi.lazy(() => schema.DefinedTermOrSubClassJoiSchema), Joi.lazy(() => schema.AnatomicalSystemOrSubClassJoiSchema), Joi.lazy(() => schema.AnatomicalStructureOrSubClassJoiSchema), Joi.lazy(() => schema.BioChemEntityOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:Gene - Validation schema (w/ JOI)
 *
 * This differs from GeneJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GeneOrSubClassJoiSchema = Joi.alternatives().try([
  GeneJoiSchema,
]);

/**
 * Runtime validator for schema:Gene.
 *
 * If some data has a structure which matches a schema:Gene, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGene = validateGene(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGene instanceof OaValidationError) {
 *   // From this point on, `maybeGene` will have type `OaValidationError`
 *   const error = maybeGene;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGene` will have type `Gene`
 * const gene = maybeGene;
 * ```
 */
export function validateGene(maybeGene: unknown): Gene | OaValidationError {
  const { value, error } = GeneJoiSchema.validate(maybeGene);
  if (error) {
    return new OaValidationError('Gene', maybeGene, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Gene;
}
