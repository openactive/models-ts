 ...
 * schema:GeneralContractor
 *
 * This differs from GeneralContractor because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GeneralContractorOrSubClass =
  | GeneralContractor
  ;

/**
 * schema:GeneralContractor - Validation schema (w/ JOI)
 */
export const GeneralContractorJoiSchema = Joi.object({
  '@type': Joi.string().valid('GeneralContractor').required(),
 ...
 * schema:GeneralContractor - Validation schema (w/ JOI)
 *
 * This differs from GeneralContractorJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GeneralContractorOrSubClassJoiSchema = Joi.alternatives().try([
  GeneralContractorJoiSchema,
]);

/**
 * Runtime validator for schema:GeneralContractor.
 *
 * If some data has a structure which matches a schema:GeneralContractor, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGeneralContractor = GeneralContractor.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGeneralContractor instanceof OaValidationError) {
 *   // From this point on, `maybeGeneralContractor` will have type `OaValidationError`
 *   const error = maybeGeneralContractor;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGeneralContractor` will have type `GeneralContractor`
 * const generalContractor = maybeGeneralContractor;
 * ```
 */
export function validateGeneralContractor(maybeGeneralContractor: unknown): GeneralContractor | OaValidationError {
  const { value, error } = GeneralContractorJoiSchema.validate(maybeGeneralContractor);
  if (error) {
    return new OaValidationError('GeneralContractor', maybeGeneralContractor, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GeneralContractor;
}
