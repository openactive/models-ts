 ...
 * schema:GiveAction
 *
 * This differs from GiveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GiveActionOrSubClass =
  | GiveAction
  ;

/**
 * schema:GiveAction - Validation schema (w/ JOI)
 */
export const GiveActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('GiveAction').required(),
 ...
 * schema:GiveAction - Validation schema (w/ JOI)
 *
 * This differs from GiveActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GiveActionOrSubClassJoiSchema = Joi.alternatives().try([
  GiveActionJoiSchema,
]);

/**
 * Runtime validator for schema:GiveAction.
 *
 * If some data has a structure which matches a schema:GiveAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGiveAction = GiveAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGiveAction instanceof OaValidationError) {
 *   // From this point on, `maybeGiveAction` will have type `OaValidationError`
 *   const error = maybeGiveAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGiveAction` will have type `GiveAction`
 * const giveAction = maybeGiveAction;
 * ```
 */
export function validateGiveAction(maybeGiveAction: unknown): GiveAction | OaValidationError {
  const { value, error } = GiveActionJoiSchema.validate(maybeGiveAction);
  if (error) {
    return new OaValidationError('GiveAction', maybeGiveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GiveAction;
}
