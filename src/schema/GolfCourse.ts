 ...
 * schema:GolfCourse
 *
 * This differs from GolfCourse because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GolfCourseOrSubClass =
  | GolfCourse
  ;

/**
 * schema:GolfCourse - Validation schema (w/ JOI)
 */
export const GolfCourseJoiSchema = Joi.object({
  '@type': Joi.string().valid('GolfCourse').required(),
 ...
 * schema:GolfCourse - Validation schema (w/ JOI)
 *
 * This differs from GolfCourseJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GolfCourseOrSubClassJoiSchema = Joi.alternatives().try([
  GolfCourseJoiSchema,
]);

/**
 * Runtime validator for schema:GolfCourse.
 *
 * If some data has a structure which matches a schema:GolfCourse, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGolfCourse = GolfCourse.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGolfCourse instanceof OaValidationError) {
 *   // From this point on, `maybeGolfCourse` will have type `OaValidationError`
 *   const error = maybeGolfCourse;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGolfCourse` will have type `GolfCourse`
 * const golfCourse = maybeGolfCourse;
 * ```
 */
export function validateGolfCourse(maybeGolfCourse: unknown): GolfCourse | OaValidationError {
  const { value, error } = GolfCourseJoiSchema.validate(maybeGolfCourse);
  if (error) {
    return new OaValidationError('GolfCourse', maybeGolfCourse, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GolfCourse;
}
