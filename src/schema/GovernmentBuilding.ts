 ...
 * schema:GovernmentBuilding
 *
 * This differs from GovernmentBuilding because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GovernmentBuildingOrSubClass =
  | GovernmentBuilding
  | schema.DefenceEstablishmentOrSubClass
  | schema.EmbassyOrSubClass
  | schema.LegislativeBuildingOrSubClass
  | schema.CityHallOrSubClass
  | schema.CourthouseOrSubClass
  ;

/**
 * schema:GovernmentBuilding - Validation schema (w/ JOI)
 */
export const GovernmentBuildingJoiSchema = Joi.object({
  '@type': Joi.string().valid('GovernmentBuilding').required(),
 ...
 * schema:GovernmentBuilding - Validation schema (w/ JOI)
 *
 * This differs from GovernmentBuildingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GovernmentBuildingOrSubClassJoiSchema = Joi.alternatives().try([
  GovernmentBuildingJoiSchema,
  Joi.lazy(() => schema.DefenceEstablishmentOrSubClassJoiSchema),
  Joi.lazy(() => schema.EmbassyOrSubClassJoiSchema),
  Joi.lazy(() => schema.LegislativeBuildingOrSubClassJoiSchema),
  Joi.lazy(() => schema.CityHallOrSubClassJoiSchema),
  Joi.lazy(() => schema.CourthouseOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:GovernmentBuilding.
 *
 * If some data has a structure which matches a schema:GovernmentBuilding, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeGovernmentBuilding` will have type `GovernmentBuilding`
 * const governmentBuilding = maybeGovernmentBuilding;
 * ```
 */
export function validateGovernmentBuilding(maybeGovernmentBuilding: unknown): GovernmentBuilding | OaValidationError {
  const { value, error } = GovernmentBuildingJoiSchema.validate(maybeGovernmentBuilding);
  if (error) {
    return new OaValidationError('GovernmentBuilding', maybeGovernmentBuilding, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GovernmentBuilding;
}
