 ...
 * schema:GovernmentOffice
 *
 * This differs from GovernmentOffice because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GovernmentOfficeOrSubClass =
  | GovernmentOffice
  | schema.PostOfficeOrSubClass
  ;

/**
 * schema:GovernmentOffice - Validation schema (w/ JOI)
 */
export const GovernmentOfficeJoiSchema = Joi.object({
  '@type': Joi.string().valid('GovernmentOffice').required(),
 ...
 * schema:GovernmentOffice - Validation schema (w/ JOI)
 *
 * This differs from GovernmentOfficeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GovernmentOfficeOrSubClassJoiSchema = Joi.alternatives().try([
  GovernmentOfficeJoiSchema,
  Joi.lazy(() => schema.PostOfficeOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:GovernmentOffice.
 *
 * If some data has a structure which matches a schema:GovernmentOffice, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGovernmentOffice = GovernmentOffice.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGovernmentOffice instanceof OaValidationError) {
 *   // From this point on, `maybeGovernmentOffice` will have type `OaValidationError`
 *   const error = maybeGovernmentOffice;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGovernmentOffice` will have type `GovernmentOffice`
 * const governmentOffice = maybeGovernmentOffice;
 * ```
 */
export function validateGovernmentOffice(maybeGovernmentOffice: unknown): GovernmentOffice | OaValidationError {
  const { value, error } = GovernmentOfficeJoiSchema.validate(maybeGovernmentOffice);
  if (error) {
    return new OaValidationError('GovernmentOffice', maybeGovernmentOffice, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GovernmentOffice;
}
