 ...
 * schema:GovernmentOrganization
 *
 * This differs from GovernmentOrganization because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GovernmentOrganizationOrSubClass =
  | GovernmentOrganization
  ;

/**
 * schema:GovernmentOrganization - Validation schema (w/ JOI)
 */
export const GovernmentOrganizationJoiSchema = Joi.object({
  '@type': Joi.string().valid('GovernmentOrganization').required(),
 ...
 * schema:GovernmentOrganization - Validation schema (w/ JOI)
 *
 * This differs from GovernmentOrganizationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GovernmentOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
  GovernmentOrganizationJoiSchema,
]);

/**
 * Runtime validator for schema:GovernmentOrganization.
 *
 * If some data has a structure which matches a schema:GovernmentOrganization, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGovernmentOrganization = GovernmentOrganization.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGovernmentOrganization instanceof OaValidationError) {
 *   // From this point on, `maybeGovernmentOrganization` will have type `OaValidationError`
 *   const error = maybeGovernmentOrganization;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGovernmentOrganization` will have type `GovernmentOrganization`
 * const governmentOrganization = maybeGovernmentOrganization;
 * ```
 */
export function validateGovernmentOrganization(maybeGovernmentOrganization: unknown): GovernmentOrganization | OaValidationError {
  const { value, error } = GovernmentOrganizationJoiSchema.validate(maybeGovernmentOrganization);
  if (error) {
    return new OaValidationError('GovernmentOrganization', maybeGovernmentOrganization, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as GovernmentOrganization;
}
