 ...
 * schema:GroceryStore
 *
 * This differs from GroceryStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GroceryStoreOrSubClass =
  | GroceryStore
  ;

/**
 * schema:GroceryStore - Validation schema (w/ JOI)
 */
export const GroceryStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('GroceryStore').required(),
 ...
 * schema:GroceryStore - Validation schema (w/ JOI)
 *
 * This differs from GroceryStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GroceryStoreOrSubClassJoiSchema = Joi.alternatives().try([
  GroceryStoreJoiSchema,
]);

/**
 * Runtime validator for schema:GroceryStore.
 *
 * If some data has a structure which matches a schema:GroceryStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeGroceryStore = GroceryStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeGroceryStore instanceof OaValidationError) {
 *   // From this point on, `maybeGroceryStore` will have type `OaValidationError`
 *   const error = maybeGroceryStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeGroceryStore` will have type `GroceryStore`
 * const groceryStore = maybeGroceryStore;
 * ```
 */
export function validateGroceryStore(maybeGroceryStore: unknown): GroceryStore | OaValidationError {
  const { value, error } = GroceryStoreJoiSchema.validate(maybeGroceryStore);
  if (error) {
    return new OaValidationError('GroceryStore', maybeGroceryStore, error);
  }
