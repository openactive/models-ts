 ...
 * schema:Guide
 *
 * This differs from Guide because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type GuideOrSubClass =
  | Guide
  ;

/**
 * schema:Guide - Validation schema (w/ JOI)
 */
export const GuideJoiSchema = Joi.object({
  '@type': Joi.string().valid('Guide').required(),
 ...
 * schema:Guide - Validation schema (w/ JOI)
 *
 * This differs from GuideJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const GuideOrSubClassJoiSchema = Joi.alternatives().try([
  GuideJoiSchema,
]);

/**
 * Runtime validator for schema:Guide.
 *
 * If some data has a structure which matches a schema:Guide, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeGuide` will have type `Guide`
 * const guide = maybeGuide;
 * ```
 */
export function validateGuide(maybeGuide: unknown): Guide | OaValidationError {
  const { value, error } = GuideJoiSchema.validate(maybeGuide);
  if (error) {
    return new OaValidationError('Guide', maybeGuide, error);
  }
