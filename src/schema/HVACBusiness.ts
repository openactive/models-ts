 ...
 * schema:HVACBusiness
 *
 * This differs from HVACBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HVACBusinessOrSubClass =
  | HVACBusiness
  ;

/**
 * schema:HVACBusiness - Validation schema (w/ JOI)
 */
export const HVACBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('HVACBusiness').required(),
 ...
 * schema:HVACBusiness - Validation schema (w/ JOI)
 *
 * This differs from HVACBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HVACBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  HVACBusinessJoiSchema,
]);

/**
 * Runtime validator for schema:HVACBusiness.
 *
 * If some data has a structure which matches a schema:HVACBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHVACBusiness = HVACBusiness.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHVACBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeHVACBusiness` will have type `OaValidationError`
 *   const error = maybeHVACBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHVACBusiness` will have type `HVACBusiness`
 * const hVACBusiness = maybeHVACBusiness;
 * ```
 */
export function validateHVACBusiness(maybeHVACBusiness: unknown): HVACBusiness | OaValidationError {
  const { value, error } = HVACBusinessJoiSchema.validate(maybeHVACBusiness);
  if (error) {
    return new OaValidationError('HVACBusiness', maybeHVACBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HVACBusiness;
}
