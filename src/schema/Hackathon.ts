 ...
 * etc.
 */
export type HackathonOrSubClass =
  | Hackathon
  ;
 ...
 */
export const HackathonJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const HackathonOrSubClassJoiSchema = Joi.alternatives().try([
  HackathonJoiSchema,
]);
 ...
 * ```
 */
export function validateHackathon(maybeHackathon: unknown): Hackathon | OaValidationError {
  const { value, error } = HackathonJoiSchema.validate(maybeHackathon);
  if (error) {
    return new OaValidationError('Hackathon', maybeHackathon, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Hackathon;
}
