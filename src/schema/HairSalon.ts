 ...
 * schema:HairSalon
 *
 * This differs from HairSalon because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HairSalonOrSubClass =
  | HairSalon
  ;

/**
 * schema:HairSalon - Validation schema (w/ JOI)
 */
export const HairSalonJoiSchema = Joi.object({
  '@type': Joi.string().valid('HairSalon').required(),
 ...
 * schema:HairSalon - Validation schema (w/ JOI)
 *
 * This differs from HairSalonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HairSalonOrSubClassJoiSchema = Joi.alternatives().try([
  HairSalonJoiSchema,
]);

/**
 * Runtime validator for schema:HairSalon.
 *
 * If some data has a structure which matches a schema:HairSalon, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHairSalon = HairSalon.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHairSalon instanceof OaValidationError) {
 *   // From this point on, `maybeHairSalon` will have type `OaValidationError`
 *   const error = maybeHairSalon;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHairSalon` will have type `HairSalon`
 * const hairSalon = maybeHairSalon;
 * ```
 */
export function validateHairSalon(maybeHairSalon: unknown): HairSalon | OaValidationError {
  const { value, error } = HairSalonJoiSchema.validate(maybeHairSalon);
  if (error) {
    return new OaValidationError('HairSalon', maybeHairSalon, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HairSalon;
}
