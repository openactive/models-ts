 ...
 * schema:HardwareStore
 *
 * This differs from HardwareStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HardwareStoreOrSubClass =
  | HardwareStore
  ;

/**
 * schema:HardwareStore - Validation schema (w/ JOI)
 */
export const HardwareStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('HardwareStore').required(),
 ...
 * schema:HardwareStore - Validation schema (w/ JOI)
 *
 * This differs from HardwareStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HardwareStoreOrSubClassJoiSchema = Joi.alternatives().try([
  HardwareStoreJoiSchema,
]);

/**
 * Runtime validator for schema:HardwareStore.
 *
 * If some data has a structure which matches a schema:HardwareStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHardwareStore = HardwareStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHardwareStore instanceof OaValidationError) {
 *   // From this point on, `maybeHardwareStore` will have type `OaValidationError`
 *   const error = maybeHardwareStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHardwareStore` will have type `HardwareStore`
 * const hardwareStore = maybeHardwareStore;
 * ```
 */
export function validateHardwareStore(maybeHardwareStore: unknown): HardwareStore | OaValidationError {
  const { value, error } = HardwareStoreJoiSchema.validate(maybeHardwareStore);
  if (error) {
    return new OaValidationError('HardwareStore', maybeHardwareStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HardwareStore;
}
