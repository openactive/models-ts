 ...
 * schema:HealthAndBeautyBusiness
 *
 * This differs from HealthAndBeautyBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HealthAndBeautyBusinessOrSubClass =
  | HealthAndBeautyBusiness
  | schema.BeautySalonOrSubClass
  | schema.DaySpaOrSubClass
  | schema.HealthClubOrSubClass
  | schema.HairSalonOrSubClass
  | schema.NailSalonOrSubClass
  | schema.TattooParlorOrSubClass
  ;

/**
 * schema:HealthAndBeautyBusiness - Validation schema (w/ JOI)
 */
export const HealthAndBeautyBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('HealthAndBeautyBusiness').required(),
 ...
 * schema:HealthAndBeautyBusiness - Validation schema (w/ JOI)
 *
 * This differs from HealthAndBeautyBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HealthAndBeautyBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  HealthAndBeautyBusinessJoiSchema,
  Joi.lazy(() => schema.BeautySalonOrSubClassJoiSchema),
  Joi.lazy(() => schema.DaySpaOrSubClassJoiSchema),
  Joi.lazy(() => schema.HealthClubOrSubClassJoiSchema),
  Joi.lazy(() => schema.HairSalonOrSubClassJoiSchema),
  Joi.lazy(() => schema.NailSalonOrSubClassJoiSchema),
  Joi.lazy(() => schema.TattooParlorOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:HealthAndBeautyBusiness.
 *
 * If some data has a structure which matches a schema:HealthAndBeautyBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHealthAndBeautyBusiness = HealthAndBeautyBusiness.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHealthAndBeautyBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeHealthAndBeautyBusiness` will have type `OaValidationError`
 *   const error = maybeHealthAndBeautyBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHealthAndBeautyBusiness` will have type `HealthAndBeautyBusiness`
 * const healthAndBeautyBusiness = maybeHealthAndBeautyBusiness;
 * ```
 */
export function validateHealthAndBeautyBusiness(maybeHealthAndBeautyBusiness: unknown): HealthAndBeautyBusiness | OaValidationError {
  const { value, error } = HealthAndBeautyBusinessJoiSchema.validate(maybeHealthAndBeautyBusiness);
  if (error) {
    return new OaValidationError('HealthAndBeautyBusiness', maybeHealthAndBeautyBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HealthAndBeautyBusiness;
}
