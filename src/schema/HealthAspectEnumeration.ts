 ...
 * HealthAspectEnumeration enumerates several aspects of health content online, each of which might be described using [[hasHealthAspect]] and [[HealthTopicContent]].
 */
export const HealthAspectEnumerationJoiSchema = Joi.string().valid(
  'https://schema.org/BenefitsHealthAspect',
  'https://schema.org/StagesHealthAspect',
  'https://schema.org/SelfCareHealthAspect',
  'https://schema.org/SideEffectsHealthAspect',
 ...
 * ```
 */
export function validateHealthAspectEnumeration(maybeHealthAspectEnumeration: unknown): HealthAspectEnumeration | OaValidationError {
  const { value, error } = HealthAspectEnumerationJoiSchema.validate(maybeHealthAspectEnumeration);
  if (error) {
    return new OaValidationError('HealthAspectEnumeration', maybeHealthAspectEnumeration, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HealthAspectEnumeration;
}
