 ...
 * schema:HealthClub
 *
 * This differs from HealthClub because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HealthClubOrSubClass =
  | HealthClub
  ;

/**
 * schema:HealthClub - Validation schema (w/ JOI)
 */
export const HealthClubJoiSchema = Joi.object({
  '@type': Joi.string().valid('HealthClub').required(),
 ...
 * schema:HealthClub - Validation schema (w/ JOI)
 *
 * This differs from HealthClubJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HealthClubOrSubClassJoiSchema = Joi.alternatives().try([
  HealthClubJoiSchema,
]);

/**
 * Runtime validator for schema:HealthClub.
 *
 * If some data has a structure which matches a schema:HealthClub, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHealthClub = HealthClub.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHealthClub instanceof OaValidationError) {
 *   // From this point on, `maybeHealthClub` will have type `OaValidationError`
 *   const error = maybeHealthClub;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHealthClub` will have type `HealthClub`
 * const healthClub = maybeHealthClub;
 * ```
 */
export function validateHealthClub(maybeHealthClub: unknown): HealthClub | OaValidationError {
  const { value, error } = HealthClubJoiSchema.validate(maybeHealthClub);
  if (error) {
    return new OaValidationError('HealthClub', maybeHealthClub, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HealthClub;
}
