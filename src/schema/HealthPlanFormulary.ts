 ...
 */
export type HealthPlanFormulary = {
 ...
 * etc.
 */
export type HealthPlanFormularyOrSubClass =
  | HealthPlanFormulary
  ;
 ...
 */
export const HealthPlanFormularyJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const HealthPlanFormularyOrSubClassJoiSchema = Joi.alternatives().try([
  HealthPlanFormularyJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeHealthPlanFormulary = HealthPlanFormulary.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateHealthPlanFormulary(maybeHealthPlanFormulary: unknown): HealthPlanFormulary | OaValidationError {
  const { value, error } = HealthPlanFormularyJoiSchema.validate(maybeHealthPlanFormulary);
  if (error) {
    return new OaValidationError('HealthPlanFormulary', maybeHealthPlanFormulary, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HealthPlanFormulary;
}
