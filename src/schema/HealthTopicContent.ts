...
export type HealthTopicContent = {
   ...
   */
  hasHealthAspect?: schema.HealthAspectEnumeration;
  /**
 ...
 * schema:HealthTopicContent
 *
 * This differs from HealthTopicContent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HealthTopicContentOrSubClass =
  | HealthTopicContent
  ;

/**
 * schema:HealthTopicContent - Validation schema (w/ JOI)
 */
export const HealthTopicContentJoiSchema = Joi.object({
  '@type': Joi.string().valid('HealthTopicContent').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  hasHealthAspect: Joi.lazy(() => schema.HealthAspectEnumerationJoiSchema),
  text: Joi.string(),
 ...
 * schema:HealthTopicContent - Validation schema (w/ JOI)
 *
 * This differs from HealthTopicContentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HealthTopicContentOrSubClassJoiSchema = Joi.alternatives().try([
  HealthTopicContentJoiSchema,
]);

/**
 * Runtime validator for schema:HealthTopicContent.
 *
 * If some data has a structure which matches a schema:HealthTopicContent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHealthTopicContent = HealthTopicContent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHealthTopicContent instanceof OaValidationError) {
 *   // From this point on, `maybeHealthTopicContent` will have type `OaValidationError`
 *   const error = maybeHealthTopicContent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHealthTopicContent` will have type `HealthTopicContent`
 * const healthTopicContent = maybeHealthTopicContent;
 * ```
 */
export function validateHealthTopicContent(maybeHealthTopicContent: unknown): HealthTopicContent | OaValidationError {
  const { value, error } = HealthTopicContentJoiSchema.validate(maybeHealthTopicContent);
  if (error) {
    return new OaValidationError('HealthTopicContent', maybeHealthTopicContent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HealthTopicContent;
}
