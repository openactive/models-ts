 ...
 * schema:HighSchool
 *
 * This differs from HighSchool because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HighSchoolOrSubClass =
  | HighSchool
  ;

/**
 * schema:HighSchool - Validation schema (w/ JOI)
 */
export const HighSchoolJoiSchema = Joi.object({
  '@type': Joi.string().valid('HighSchool').required(),
 ...
 * schema:HighSchool - Validation schema (w/ JOI)
 *
 * This differs from HighSchoolJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HighSchoolOrSubClassJoiSchema = Joi.alternatives().try([
  HighSchoolJoiSchema,
]);

/**
 * Runtime validator for schema:HighSchool.
 *
 * If some data has a structure which matches a schema:HighSchool, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHighSchool = HighSchool.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHighSchool instanceof OaValidationError) {
 *   // From this point on, `maybeHighSchool` will have type `OaValidationError`
 *   const error = maybeHighSchool;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHighSchool` will have type `HighSchool`
 * const highSchool = maybeHighSchool;
 * ```
 */
export function validateHighSchool(maybeHighSchool: unknown): HighSchool | OaValidationError {
  const { value, error } = HighSchoolJoiSchema.validate(maybeHighSchool);
  if (error) {
    return new OaValidationError('HighSchool', maybeHighSchool, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HighSchool;
}
