 ...
 * schema:HinduTemple
 *
 * This differs from HinduTemple because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HinduTempleOrSubClass =
  | HinduTemple
  ;

/**
 * schema:HinduTemple - Validation schema (w/ JOI)
 */
export const HinduTempleJoiSchema = Joi.object({
  '@type': Joi.string().valid('HinduTemple').required(),
 ...
 * schema:HinduTemple - Validation schema (w/ JOI)
 *
 * This differs from HinduTempleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HinduTempleOrSubClassJoiSchema = Joi.alternatives().try([
  HinduTempleJoiSchema,
]);

/**
 * Runtime validator for schema:HinduTemple.
 *
 * If some data has a structure which matches a schema:HinduTemple, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHinduTemple = HinduTemple.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHinduTemple instanceof OaValidationError) {
 *   // From this point on, `maybeHinduTemple` will have type `OaValidationError`
 *   const error = maybeHinduTemple;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHinduTemple` will have type `HinduTemple`
 * const hinduTemple = maybeHinduTemple;
 * ```
 */
export function validateHinduTemple(maybeHinduTemple: unknown): HinduTemple | OaValidationError {
  const { value, error } = HinduTempleJoiSchema.validate(maybeHinduTemple);
  if (error) {
    return new OaValidationError('HinduTemple', maybeHinduTemple, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HinduTemple;
}
