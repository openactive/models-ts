 ...
 * schema:HobbyShop
 *
 * This differs from HobbyShop because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HobbyShopOrSubClass =
  | HobbyShop
  ;

/**
 * schema:HobbyShop - Validation schema (w/ JOI)
 */
export const HobbyShopJoiSchema = Joi.object({
  '@type': Joi.string().valid('HobbyShop').required(),
 ...
 * schema:HobbyShop - Validation schema (w/ JOI)
 *
 * This differs from HobbyShopJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HobbyShopOrSubClassJoiSchema = Joi.alternatives().try([
  HobbyShopJoiSchema,
]);

/**
 * Runtime validator for schema:HobbyShop.
 *
 * If some data has a structure which matches a schema:HobbyShop, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHobbyShop = HobbyShop.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHobbyShop instanceof OaValidationError) {
 *   // From this point on, `maybeHobbyShop` will have type `OaValidationError`
 *   const error = maybeHobbyShop;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHobbyShop` will have type `HobbyShop`
 * const hobbyShop = maybeHobbyShop;
 * ```
 */
export function validateHobbyShop(maybeHobbyShop: unknown): HobbyShop | OaValidationError {
  const { value, error } = HobbyShopJoiSchema.validate(maybeHobbyShop);
  if (error) {
    return new OaValidationError('HobbyShop', maybeHobbyShop, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HobbyShop;
}
