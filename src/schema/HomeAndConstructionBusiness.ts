 ...
 * schema:HomeAndConstructionBusiness
 *
 * This differs from HomeAndConstructionBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HomeAndConstructionBusinessOrSubClass =
  | HomeAndConstructionBusiness
  | schema.HVACBusinessOrSubClass
  | schema.MovingCompanyOrSubClass
  | schema.GeneralContractorOrSubClass
  | schema.RoofingContractorOrSubClass
  | schema.ElectricianOrSubClass
  | schema.PlumberOrSubClass
  | schema.LocksmithOrSubClass
  | schema.HousePainterOrSubClass
  ;

/**
 * schema:HomeAndConstructionBusiness - Validation schema (w/ JOI)
 */
export const HomeAndConstructionBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('HomeAndConstructionBusiness').required(),
 ...
 * schema:HomeAndConstructionBusiness - Validation schema (w/ JOI)
 *
 * This differs from HomeAndConstructionBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HomeAndConstructionBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  HomeAndConstructionBusinessJoiSchema,
  Joi.lazy(() => schema.HVACBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.MovingCompanyOrSubClassJoiSchema),
  Joi.lazy(() => schema.GeneralContractorOrSubClassJoiSchema),
  Joi.lazy(() => schema.RoofingContractorOrSubClassJoiSchema),
  Joi.lazy(() => schema.ElectricianOrSubClassJoiSchema),
  Joi.lazy(() => schema.PlumberOrSubClassJoiSchema),
  Joi.lazy(() => schema.LocksmithOrSubClassJoiSchema),
  Joi.lazy(() => schema.HousePainterOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:HomeAndConstructionBusiness.
 *
 * If some data has a structure which matches a schema:HomeAndConstructionBusiness, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeHomeAndConstructionBusiness` will have type `HomeAndConstructionBusiness`
 * const homeAndConstructionBusiness = maybeHomeAndConstructionBusiness;
 * ```
 */
export function validateHomeAndConstructionBusiness(maybeHomeAndConstructionBusiness: unknown): HomeAndConstructionBusiness | OaValidationError {
  const { value, error } = HomeAndConstructionBusinessJoiSchema.validate(maybeHomeAndConstructionBusiness);
  if (error) {
    return new OaValidationError('HomeAndConstructionBusiness', maybeHomeAndConstructionBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HomeAndConstructionBusiness;
}
