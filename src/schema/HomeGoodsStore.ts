 ...
 * schema:HomeGoodsStore
 *
 * This differs from HomeGoodsStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HomeGoodsStoreOrSubClass =
  | HomeGoodsStore
  ;

/**
 * schema:HomeGoodsStore - Validation schema (w/ JOI)
 */
export const HomeGoodsStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('HomeGoodsStore').required(),
 ...
 * schema:HomeGoodsStore - Validation schema (w/ JOI)
 *
 * This differs from HomeGoodsStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HomeGoodsStoreOrSubClassJoiSchema = Joi.alternatives().try([
  HomeGoodsStoreJoiSchema,
]);

/**
 * Runtime validator for schema:HomeGoodsStore.
 *
 * If some data has a structure which matches a schema:HomeGoodsStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHomeGoodsStore = HomeGoodsStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHomeGoodsStore instanceof OaValidationError) {
 *   // From this point on, `maybeHomeGoodsStore` will have type `OaValidationError`
 *   const error = maybeHomeGoodsStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHomeGoodsStore` will have type `HomeGoodsStore`
 * const homeGoodsStore = maybeHomeGoodsStore;
 * ```
 */
export function validateHomeGoodsStore(maybeHomeGoodsStore: unknown): HomeGoodsStore | OaValidationError {
  const { value, error } = HomeGoodsStoreJoiSchema.validate(maybeHomeGoodsStore);
  if (error) {
    return new OaValidationError('HomeGoodsStore', maybeHomeGoodsStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HomeGoodsStore;
}
