...
export type Hospital = {
   ...
   */
  healthcareReportingData?: schema.DatasetOrSubClass | schema.CDCPMDRecordOrSubClass | string;
  /**
 ...
 * schema:Hospital
 *
 * This differs from Hospital because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HospitalOrSubClass =
  | Hospital
  ;
 ...
 */
export const HospitalJoiSchema = Joi.object({
  ...
  healthcareReportingData: Joi.alternatives().try(Joi.lazy(() => schema.DatasetOrSubClassJoiSchema), Joi.lazy(() => schema.CDCPMDRecordOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const HospitalOrSubClassJoiSchema = Joi.alternatives().try([
  HospitalJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeHospital = validateHospital(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateHospital(maybeHospital: unknown): Hospital | OaValidationError {
  const { value, error } = HospitalJoiSchema.validate(maybeHospital);
  if (error) {
    return new OaValidationError('Hospital', maybeHospital, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Hospital;
}
