 ...
 * schema:Hostel
 *
 * This differs from Hostel because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HostelOrSubClass =
  | Hostel
  ;

/**
 * schema:Hostel - Validation schema (w/ JOI)
 */
export const HostelJoiSchema = Joi.object({
  '@type': Joi.string().valid('Hostel').required(),
 ...
 * schema:Hostel - Validation schema (w/ JOI)
 *
 * This differs from HostelJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HostelOrSubClassJoiSchema = Joi.alternatives().try([
  HostelJoiSchema,
]);

/**
 * Runtime validator for schema:Hostel.
 *
 * If some data has a structure which matches a schema:Hostel, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHostel = validateHostel(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHostel instanceof OaValidationError) {
 *   // From this point on, `maybeHostel` will have type `OaValidationError`
 *   const error = maybeHostel;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHostel` will have type `Hostel`
 * const hostel = maybeHostel;
 * ```
 */
export function validateHostel(maybeHostel: unknown): Hostel | OaValidationError {
  const { value, error } = HostelJoiSchema.validate(maybeHostel);
  if (error) {
    return new OaValidationError('Hostel', maybeHostel, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Hostel;
}
