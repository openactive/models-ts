 ...
 * schema:Hotel
 *
 * This differs from Hotel because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HotelOrSubClass =
  | Hotel
  ;

/**
 * schema:Hotel - Validation schema (w/ JOI)
 */
export const HotelJoiSchema = Joi.object({
  '@type': Joi.string().valid('Hotel').required(),
 ...
 * schema:Hotel - Validation schema (w/ JOI)
 *
 * This differs from HotelJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HotelOrSubClassJoiSchema = Joi.alternatives().try([
  HotelJoiSchema,
]);

/**
 * Runtime validator for schema:Hotel.
 *
 * If some data has a structure which matches a schema:Hotel, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHotel = validateHotel(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHotel instanceof OaValidationError) {
 *   // From this point on, `maybeHotel` will have type `OaValidationError`
 *   const error = maybeHotel;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHotel` will have type `Hotel`
 * const hotel = maybeHotel;
 * ```
 */
export function validateHotel(maybeHotel: unknown): Hotel | OaValidationError {
  const { value, error } = HotelJoiSchema.validate(maybeHotel);
  if (error) {
    return new OaValidationError('Hotel', maybeHotel, error);
  }
