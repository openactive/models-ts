...
export type HotelRoom = {
   ...
   */
  bed?: schema.BedDetailsOrSubClass | string | schema.BedTypeOrSubClass;
  /**
   * The allowed total occupancy for the accommodation in persons (including infants etc). For individual accommodations, this is not necessarily the legal maximum but defines the permitted usage as per the contractual agreement (e.g. a double room used by a single person).
   * Typical unit code(s): C62 for person
   */
  occupancy?: schema.QuantitativeValueOrSubClass | string;
  /**
 ...
 * schema:HotelRoom
 *
 * This differs from HotelRoom because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HotelRoomOrSubClass =
  | HotelRoom
  ;

/**
 * schema:HotelRoom - Validation schema (w/ JOI)
 */
export const HotelRoomJoiSchema = Joi.object({
  '@type': Joi.string().valid('HotelRoom').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  bed: Joi.alternatives().try(Joi.lazy(() => schema.BedDetailsOrSubClassJoiSchema), Joi.string(), Joi.lazy(() => schema.BedTypeOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:HotelRoom - Validation schema (w/ JOI)
 *
 * This differs from HotelRoomJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HotelRoomOrSubClassJoiSchema = Joi.alternatives().try([
  HotelRoomJoiSchema,
]);

/**
 * Runtime validator for schema:HotelRoom.
 *
 * If some data has a structure which matches a schema:HotelRoom, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHotelRoom = HotelRoom.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHotelRoom instanceof OaValidationError) {
 *   // From this point on, `maybeHotelRoom` will have type `OaValidationError`
 *   const error = maybeHotelRoom;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHotelRoom` will have type `HotelRoom`
 * const hotelRoom = maybeHotelRoom;
 * ```
 */
export function validateHotelRoom(maybeHotelRoom: unknown): HotelRoom | OaValidationError {
  const { value, error } = HotelRoomJoiSchema.validate(maybeHotelRoom);
  if (error) {
    return new OaValidationError('HotelRoom', maybeHotelRoom, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HotelRoom;
}
