 ...
 * schema:HousePainter
 *
 * This differs from HousePainter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HousePainterOrSubClass =
  | HousePainter
  ;

/**
 * schema:HousePainter - Validation schema (w/ JOI)
 */
export const HousePainterJoiSchema = Joi.object({
  '@type': Joi.string().valid('HousePainter').required(),
 ...
 * schema:HousePainter - Validation schema (w/ JOI)
 *
 * This differs from HousePainterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HousePainterOrSubClassJoiSchema = Joi.alternatives().try([
  HousePainterJoiSchema,
]);

/**
 * Runtime validator for schema:HousePainter.
 *
 * If some data has a structure which matches a schema:HousePainter, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHousePainter = HousePainter.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHousePainter instanceof OaValidationError) {
 *   // From this point on, `maybeHousePainter` will have type `OaValidationError`
 *   const error = maybeHousePainter;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHousePainter` will have type `HousePainter`
 * const housePainter = maybeHousePainter;
 * ```
 */
export function validateHousePainter(maybeHousePainter: unknown): HousePainter | OaValidationError {
  const { value, error } = HousePainterJoiSchema.validate(maybeHousePainter);
  if (error) {
    return new OaValidationError('HousePainter', maybeHousePainter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HousePainter;
}
