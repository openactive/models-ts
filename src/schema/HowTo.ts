 ...
 * schema:HowTo
 *
 * This differs from HowTo because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HowToOrSubClass =
  | HowTo
  | schema.RecipeOrSubClass
  ;

/**
 * schema:HowTo - Validation schema (w/ JOI)
 */
export const HowToJoiSchema = Joi.object({
  '@type': Joi.string().valid('HowTo').required(),
 ...
 * schema:HowTo - Validation schema (w/ JOI)
 *
 * This differs from HowToJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HowToOrSubClassJoiSchema = Joi.alternatives().try([
  HowToJoiSchema,
  Joi.lazy(() => schema.RecipeOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:HowTo.
 *
 * If some data has a structure which matches a schema:HowTo, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHowTo = HowTo.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHowTo instanceof OaValidationError) {
 *   // From this point on, `maybeHowTo` will have type `OaValidationError`
 *   const error = maybeHowTo;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHowTo` will have type `HowTo`
 * const howTo = maybeHowTo;
 * ```
 */
export function validateHowTo(maybeHowTo: unknown): HowTo | OaValidationError {
  const { value, error } = HowToJoiSchema.validate(maybeHowTo);
  if (error) {
    return new OaValidationError('HowTo', maybeHowTo, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HowTo;
}
