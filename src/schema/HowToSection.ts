...
export type HowToSection = {
   ...
   */
  itemListElement?: string | schema.ThingOrSubClass | schema.ListItemOrSubClass;
  /**
 ...
 * schema:HowToSection
 *
 * This differs from HowToSection because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HowToSectionOrSubClass =
  | HowToSection
  ;

/**
 * schema:HowToSection - Validation schema (w/ JOI)
 */
export const HowToSectionJoiSchema = Joi.object({
  '@type': Joi.string().valid('HowToSection').required(),
 ...
 * schema:HowToSection - Validation schema (w/ JOI)
 *
 * This differs from HowToSectionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HowToSectionOrSubClassJoiSchema = Joi.alternatives().try([
  HowToSectionJoiSchema,
]);

/**
 * Runtime validator for schema:HowToSection.
 *
 * If some data has a structure which matches a schema:HowToSection, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeHowToSection` will have type `HowToSection`
 * const howToSection = maybeHowToSection;
 * ```
 */
export function validateHowToSection(maybeHowToSection: unknown): HowToSection | OaValidationError {
  const { value, error } = HowToSectionJoiSchema.validate(maybeHowToSection);
  if (error) {
    return new OaValidationError('HowToSection', maybeHowToSection, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HowToSection;
}
