 ...
 * schema:HowToStep
 *
 * This differs from HowToStep because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HowToStepOrSubClass =
  | HowToStep
  ;

/**
 * schema:HowToStep - Validation schema (w/ JOI)
 */
export const HowToStepJoiSchema = Joi.object({
  '@type': Joi.string().valid('HowToStep').required(),
 ...
 * schema:HowToStep - Validation schema (w/ JOI)
 *
 * This differs from HowToStepJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HowToStepOrSubClassJoiSchema = Joi.alternatives().try([
  HowToStepJoiSchema,
]);

/**
 * Runtime validator for schema:HowToStep.
 *
 * If some data has a structure which matches a schema:HowToStep, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeHowToStep` will have type `HowToStep`
 * const howToStep = maybeHowToStep;
 * ```
 */
export function validateHowToStep(maybeHowToStep: unknown): HowToStep | OaValidationError {
  const { value, error } = HowToStepJoiSchema.validate(maybeHowToStep);
  if (error) {
    return new OaValidationError('HowToStep', maybeHowToStep, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HowToStep;
}
