 ...
 * schema:HowToTip
 *
 * This differs from HowToTip because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HowToTipOrSubClass =
  | HowToTip
  ;

/**
 * schema:HowToTip - Validation schema (w/ JOI)
 */
export const HowToTipJoiSchema = Joi.object({
  '@type': Joi.string().valid('HowToTip').required(),
 ...
 * schema:HowToTip - Validation schema (w/ JOI)
 *
 * This differs from HowToTipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HowToTipOrSubClassJoiSchema = Joi.alternatives().try([
  HowToTipJoiSchema,
]);

/**
 * Runtime validator for schema:HowToTip.
 *
 * If some data has a structure which matches a schema:HowToTip, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHowToTip = HowToTip.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHowToTip instanceof OaValidationError) {
 *   // From this point on, `maybeHowToTip` will have type `OaValidationError`
 *   const error = maybeHowToTip;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHowToTip` will have type `HowToTip`
 * const howToTip = maybeHowToTip;
 * ```
 */
export function validateHowToTip(maybeHowToTip: unknown): HowToTip | OaValidationError {
  const { value, error } = HowToTipJoiSchema.validate(maybeHowToTip);
  if (error) {
    return new OaValidationError('HowToTip', maybeHowToTip, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HowToTip;
}
