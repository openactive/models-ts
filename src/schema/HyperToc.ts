...
export type HyperToc = {
   ...
   */
  tocEntry?: schema.HyperTocEntryOrSubClass | string;
  /**
 ...
 * schema:HyperToc
 *
 * This differs from HyperToc because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HyperTocOrSubClass =
  | HyperToc
  ;

/**
 * schema:HyperToc - Validation schema (w/ JOI)
 */
export const HyperTocJoiSchema = Joi.object({
  '@type': Joi.string().valid('HyperToc').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  tocEntry: Joi.alternatives().try(Joi.lazy(() => schema.HyperTocEntryOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:HyperToc - Validation schema (w/ JOI)
 *
 * This differs from HyperTocJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HyperTocOrSubClassJoiSchema = Joi.alternatives().try([
  HyperTocJoiSchema,
]);

/**
 * Runtime validator for schema:HyperToc.
 *
 * If some data has a structure which matches a schema:HyperToc, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeHyperToc` will have type `HyperToc`
 * const hyperToc = maybeHyperToc;
 * ```
 */
export function validateHyperToc(maybeHyperToc: unknown): HyperToc | OaValidationError {
  const { value, error } = HyperTocJoiSchema.validate(maybeHyperToc);
  if (error) {
    return new OaValidationError('HyperToc', maybeHyperToc, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HyperToc;
}
