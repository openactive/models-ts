...
export type HyperTocEntry = {
   ...
   */
  tocContinuation?: schema.HyperTocEntryOrSubClass | string;
  /**
 ...
 * schema:HyperTocEntry
 *
 * This differs from HyperTocEntry because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type HyperTocEntryOrSubClass =
  | HyperTocEntry
  ;

/**
 * schema:HyperTocEntry - Validation schema (w/ JOI)
 */
export const HyperTocEntryJoiSchema = Joi.object({
  '@type': Joi.string().valid('HyperTocEntry').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  tocContinuation: Joi.alternatives().try(Joi.lazy(() => schema.HyperTocEntryOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:HyperTocEntry - Validation schema (w/ JOI)
 *
 * This differs from HyperTocEntryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const HyperTocEntryOrSubClassJoiSchema = Joi.alternatives().try([
  HyperTocEntryJoiSchema,
]);

/**
 * Runtime validator for schema:HyperTocEntry.
 *
 * If some data has a structure which matches a schema:HyperTocEntry, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeHyperTocEntry = HyperTocEntry.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeHyperTocEntry instanceof OaValidationError) {
 *   // From this point on, `maybeHyperTocEntry` will have type `OaValidationError`
 *   const error = maybeHyperTocEntry;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeHyperTocEntry` will have type `HyperTocEntry`
 * const hyperTocEntry = maybeHyperTocEntry;
 * ```
 */
export function validateHyperTocEntry(maybeHyperTocEntry: unknown): HyperTocEntry | OaValidationError {
  const { value, error } = HyperTocEntryJoiSchema.validate(maybeHyperTocEntry);
  if (error) {
    return new OaValidationError('HyperTocEntry', maybeHyperTocEntry, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as HyperTocEntry;
}
