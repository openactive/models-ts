 ...
 * schema:IgnoreAction
 *
 * This differs from IgnoreAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type IgnoreActionOrSubClass =
  | IgnoreAction
  ;

/**
 * schema:IgnoreAction - Validation schema (w/ JOI)
 */
export const IgnoreActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('IgnoreAction').required(),
 ...
 * schema:IgnoreAction - Validation schema (w/ JOI)
 *
 * This differs from IgnoreActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const IgnoreActionOrSubClassJoiSchema = Joi.alternatives().try([
  IgnoreActionJoiSchema,
]);

/**
 * Runtime validator for schema:IgnoreAction.
 *
 * If some data has a structure which matches a schema:IgnoreAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeIgnoreAction = IgnoreAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeIgnoreAction instanceof OaValidationError) {
 *   // From this point on, `maybeIgnoreAction` will have type `OaValidationError`
 *   const error = maybeIgnoreAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeIgnoreAction` will have type `IgnoreAction`
 * const ignoreAction = maybeIgnoreAction;
 * ```
 */
export function validateIgnoreAction(maybeIgnoreAction: unknown): IgnoreAction | OaValidationError {
  const { value, error } = IgnoreActionJoiSchema.validate(maybeIgnoreAction);
  if (error) {
    return new OaValidationError('IgnoreAction', maybeIgnoreAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as IgnoreAction;
}
