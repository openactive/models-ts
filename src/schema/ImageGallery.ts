 ...
 * schema:ImageGallery
 *
 * This differs from ImageGallery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ImageGalleryOrSubClass =
  | ImageGallery
  ;

/**
 * schema:ImageGallery - Validation schema (w/ JOI)
 */
export const ImageGalleryJoiSchema = Joi.object({
  '@type': Joi.string().valid('ImageGallery').required(),
 ...
 * schema:ImageGallery - Validation schema (w/ JOI)
 *
 * This differs from ImageGalleryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ImageGalleryOrSubClassJoiSchema = Joi.alternatives().try([
  ImageGalleryJoiSchema,
]);

/**
 * Runtime validator for schema:ImageGallery.
 *
 * If some data has a structure which matches a schema:ImageGallery, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeImageGallery` will have type `ImageGallery`
 * const imageGallery = maybeImageGallery;
 * ```
 */
export function validateImageGallery(maybeImageGallery: unknown): ImageGallery | OaValidationError {
  const { value, error } = ImageGalleryJoiSchema.validate(maybeImageGallery);
  if (error) {
    return new OaValidationError('ImageGallery', maybeImageGallery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ImageGallery;
}
