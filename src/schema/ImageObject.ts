 ...
 * schema:ImageObject
 *
 * This differs from ImageObject because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ImageObjectOrSubClass =
  | ImageObject
  | schema.BarcodeOrSubClass
  | schema.ImageObjectSnapshotOrSubClass
  ;

/**
 * schema:ImageObject - Validation schema (w/ JOI)
 */
export const ImageObjectJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ImageObjectOrSubClassJoiSchema = Joi.alternatives().try([
  ImageObjectJoiSchema,
  Joi.lazy(() => schema.BarcodeOrSubClassJoiSchema),
  Joi.lazy(() => schema.ImageObjectSnapshotOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:ImageObject.
 *
 * If some data has a structure which matches a schema:ImageObject, it will be returned with the correct type.
 ...
 * ```
 */
export function validateImageObject(maybeImageObject: unknown): ImageObject | OaValidationError {
  const { value, error } = ImageObjectJoiSchema.validate(maybeImageObject);
  if (error) {
    return new OaValidationError('ImageObject', maybeImageObject, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ImageObject;
}
