 ...
 */
export type ImageObjectSnapshot = {
   ...
   */
  thumbnail?: schema.ImageObjectOrSubClass | string;
  /**
   ...
   */
  exifData?: schema.PropertyValueOrSubClass | string;
  /**
   * The caption for this object. For downloadable machine formats (closed caption, subtitles etc.) use MediaObject and indicate the [[encodingFormat]].
   */
  caption?: schema.MediaObjectOrSubClass | string;
  /**
 ...
 * schema:ImageObjectSnapshot
 *
 * This differs from ImageObjectSnapshot because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ImageObjectSnapshotOrSubClass =
  | ImageObjectSnapshot
  ;

/**
 * schema:ImageObjectSnapshot - Validation schema (w/ JOI)
 */
export const ImageObjectSnapshotJoiSchema = Joi.object({
  '@type': Joi.string().valid('ImageObjectSnapshot').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  thumbnail: Joi.alternatives().try(Joi.lazy(() => schema.ImageObjectOrSubClassJoiSchema), Joi.string().uri()),
  embeddedTextCaption: Joi.string(),
  representativeOfPage: Joi.boolean(),
  exifData: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  caption: Joi.alternatives().try(Joi.lazy(() => schema.MediaObjectOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  bitrate: Joi.string(),
 ...
 * schema:ImageObjectSnapshot - Validation schema (w/ JOI)
 *
 * This differs from ImageObjectSnapshotJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ImageObjectSnapshotOrSubClassJoiSchema = Joi.alternatives().try([
  ImageObjectSnapshotJoiSchema,
]);

/**
 * Runtime validator for schema:ImageObjectSnapshot.
 *
 * If some data has a structure which matches a schema:ImageObjectSnapshot, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeImageObjectSnapshot = validateImageObjectSnapshot(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeImageObjectSnapshot instanceof OaValidationError) {
 *   // From this point on, `maybeImageObjectSnapshot` will have type `OaValidationError`
 *   const error = maybeImageObjectSnapshot;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeImageObjectSnapshot` will have type `ImageObjectSnapshot`
 * const imageObjectSnapshot = maybeImageObjectSnapshot;
 * ```
 */
export function validateImageObjectSnapshot(maybeImageObjectSnapshot: unknown): ImageObjectSnapshot | OaValidationError {
  const { value, error } = ImageObjectSnapshotJoiSchema.validate(maybeImageObjectSnapshot);
  if (error) {
    return new OaValidationError('ImageObjectSnapshot', maybeImageObjectSnapshot, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ImageObjectSnapshot;
}
