 ...
 * etc.
 */
export type IndividualProductOrSubClass =
  | IndividualProduct
  ;
 ...
 */
export const IndividualProductJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const IndividualProductOrSubClassJoiSchema = Joi.alternatives().try([
  IndividualProductJoiSchema,
]);
 ...
 * ```
 */
export function validateIndividualProduct(maybeIndividualProduct: unknown): IndividualProduct | OaValidationError {
  const { value, error } = IndividualProductJoiSchema.validate(maybeIndividualProduct);
  if (error) {
    return new OaValidationError('IndividualProduct', maybeIndividualProduct, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as IndividualProduct;
}
