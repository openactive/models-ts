 ...
 * schema:InformAction
 *
 * This differs from InformAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InformActionOrSubClass =
  | InformAction
  | schema.ConfirmActionOrSubClass
  | schema.RsvpActionOrSubClass
  ;

/**
 * schema:InformAction - Validation schema (w/ JOI)
 */
export const InformActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('InformAction').required(),
 ...
 * schema:InformAction - Validation schema (w/ JOI)
 *
 * This differs from InformActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const InformActionOrSubClassJoiSchema = Joi.alternatives().try([
  InformActionJoiSchema,
  Joi.lazy(() => schema.ConfirmActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.RsvpActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:InformAction.
 *
 * If some data has a structure which matches a schema:InformAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeInformAction = InformAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeInformAction instanceof OaValidationError) {
 *   // From this point on, `maybeInformAction` will have type `OaValidationError`
 *   const error = maybeInformAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeInformAction` will have type `InformAction`
 * const informAction = maybeInformAction;
 * ```
 */
export function validateInformAction(maybeInformAction: unknown): InformAction | OaValidationError {
  const { value, error } = InformActionJoiSchema.validate(maybeInformAction);
  if (error) {
    return new OaValidationError('InformAction', maybeInformAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InformAction;
}
