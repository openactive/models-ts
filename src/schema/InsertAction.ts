 ...
 * schema:InsertAction
 *
 * This differs from InsertAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InsertActionOrSubClass =
  | InsertAction
  | schema.AppendActionOrSubClass
  | schema.PrependActionOrSubClass
  ;

/**
 * schema:InsertAction - Validation schema (w/ JOI)
 */
export const InsertActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InsertActionOrSubClassJoiSchema = Joi.alternatives().try([
  InsertActionJoiSchema,
 ...
 * ```
 */
export function validateInsertAction(maybeInsertAction: unknown): InsertAction | OaValidationError {
  const { value, error } = InsertActionJoiSchema.validate(maybeInsertAction);
  if (error) {
    return new OaValidationError('InsertAction', maybeInsertAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InsertAction;
}
