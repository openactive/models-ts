 ...
 * schema:InstallAction
 *
 * This differs from InstallAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InstallActionOrSubClass =
  | InstallAction
  ;
 ...
 */
export const InstallActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InstallActionOrSubClassJoiSchema = Joi.alternatives().try([
  InstallActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeInstallAction = validateInstallAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateInstallAction(maybeInstallAction: unknown): InstallAction | OaValidationError {
  const { value, error } = InstallActionJoiSchema.validate(maybeInstallAction);
  if (error) {
    return new OaValidationError('InstallAction', maybeInstallAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InstallAction;
}
