 ...
 * schema:InsuranceAgency
 *
 * This differs from InsuranceAgency because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InsuranceAgencyOrSubClass =
  | InsuranceAgency
  ;

/**
 * schema:InsuranceAgency - Validation schema (w/ JOI)
 */
export const InsuranceAgencyJoiSchema = Joi.object({
  '@type': Joi.string().valid('InsuranceAgency').required(),
 ...
 * schema:InsuranceAgency - Validation schema (w/ JOI)
 *
 * This differs from InsuranceAgencyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const InsuranceAgencyOrSubClassJoiSchema = Joi.alternatives().try([
  InsuranceAgencyJoiSchema,
]);

/**
 * Runtime validator for schema:InsuranceAgency.
 *
 * If some data has a structure which matches a schema:InsuranceAgency, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeInsuranceAgency = InsuranceAgency.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeInsuranceAgency instanceof OaValidationError) {
 *   // From this point on, `maybeInsuranceAgency` will have type `OaValidationError`
 *   const error = maybeInsuranceAgency;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeInsuranceAgency` will have type `InsuranceAgency`
 * const insuranceAgency = maybeInsuranceAgency;
 * ```
 */
export function validateInsuranceAgency(maybeInsuranceAgency: unknown): InsuranceAgency | OaValidationError {
  const { value, error } = InsuranceAgencyJoiSchema.validate(maybeInsuranceAgency);
  if (error) {
    return new OaValidationError('InsuranceAgency', maybeInsuranceAgency, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InsuranceAgency;
}
