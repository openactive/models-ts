...
export type InteractionCounter = {
   ...
   */
  interactionService?: schema.WebSiteOrSubClass | schema.SoftwareApplicationOrSubClass | string;
  /**
 ...
 * etc.
 */
export type InteractionCounterOrSubClass =
  | InteractionCounter
  ;
 ...
 */
export const InteractionCounterJoiSchema = Joi.object({
  ...
  interactionService: Joi.alternatives().try(Joi.lazy(() => schema.WebSiteOrSubClassJoiSchema), Joi.lazy(() => schema.SoftwareApplicationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const InteractionCounterOrSubClassJoiSchema = Joi.alternatives().try([
  InteractionCounterJoiSchema,
]);
 ...
 * ```
 */
export function validateInteractionCounter(maybeInteractionCounter: unknown): InteractionCounter | OaValidationError {
  const { value, error } = InteractionCounterJoiSchema.validate(maybeInteractionCounter);
  if (error) {
    return new OaValidationError('InteractionCounter', maybeInteractionCounter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InteractionCounter;
}
