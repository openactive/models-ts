 ...
 * schema:InternetCafe
 *
 * This differs from InternetCafe because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InternetCafeOrSubClass =
  | InternetCafe
  ;

/**
 * schema:InternetCafe - Validation schema (w/ JOI)
 */
export const InternetCafeJoiSchema = Joi.object({
  '@type': Joi.string().valid('InternetCafe').required(),
 ...
 * schema:InternetCafe - Validation schema (w/ JOI)
 *
 * This differs from InternetCafeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const InternetCafeOrSubClassJoiSchema = Joi.alternatives().try([
  InternetCafeJoiSchema,
]);

/**
 * Runtime validator for schema:InternetCafe.
 *
 * If some data has a structure which matches a schema:InternetCafe, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeInternetCafe` will have type `InternetCafe`
 * const internetCafe = maybeInternetCafe;
 * ```
 */
export function validateInternetCafe(maybeInternetCafe: unknown): InternetCafe | OaValidationError {
  const { value, error } = InternetCafeJoiSchema.validate(maybeInternetCafe);
  if (error) {
    return new OaValidationError('InternetCafe', maybeInternetCafe, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InternetCafe;
}
