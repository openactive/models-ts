 ...
 * etc.
 */
export type InvestmentFundOrSubClass =
  | InvestmentFund
  ;
 ...
 */
export const InvestmentFundJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InvestmentFundOrSubClassJoiSchema = Joi.alternatives().try([
  InvestmentFundJoiSchema,
]);
 ...
 * ```
 */
export function validateInvestmentFund(maybeInvestmentFund: unknown): InvestmentFund | OaValidationError {
  const { value, error } = InvestmentFundJoiSchema.validate(maybeInvestmentFund);
  if (error) {
    return new OaValidationError('InvestmentFund', maybeInvestmentFund, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InvestmentFund;
}
