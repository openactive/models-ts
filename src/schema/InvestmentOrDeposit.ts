 ...
 * schema:InvestmentOrDeposit
 *
 * This differs from InvestmentOrDeposit because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InvestmentOrDepositOrSubClass =
  | InvestmentOrDeposit
  | schema.DepositAccountOrSubClass
  | schema.InvestmentFundOrSubClass
  | schema.BrokerageAccountOrSubClass
  ;

/**
 * schema:InvestmentOrDeposit - Validation schema (w/ JOI)
 */
export const InvestmentOrDepositJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const InvestmentOrDepositOrSubClassJoiSchema = Joi.alternatives().try([
  InvestmentOrDepositJoiSchema,
 ...
 * ```
 */
export function validateInvestmentOrDeposit(maybeInvestmentOrDeposit: unknown): InvestmentOrDeposit | OaValidationError {
  const { value, error } = InvestmentOrDepositJoiSchema.validate(maybeInvestmentOrDeposit);
  if (error) {
    return new OaValidationError('InvestmentOrDeposit', maybeInvestmentOrDeposit, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InvestmentOrDeposit;
}
