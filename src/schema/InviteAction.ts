 ...
 * schema:InviteAction
 *
 * This differs from InviteAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type InviteActionOrSubClass =
  | InviteAction
  ;

/**
 * schema:InviteAction - Validation schema (w/ JOI)
 */
export const InviteActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('InviteAction').required(),
 ...
 * schema:InviteAction - Validation schema (w/ JOI)
 *
 * This differs from InviteActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const InviteActionOrSubClassJoiSchema = Joi.alternatives().try([
  InviteActionJoiSchema,
]);

/**
 * Runtime validator for schema:InviteAction.
 *
 * If some data has a structure which matches a schema:InviteAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeInviteAction = InviteAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeInviteAction instanceof OaValidationError) {
 *   // From this point on, `maybeInviteAction` will have type `OaValidationError`
 *   const error = maybeInviteAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeInviteAction` will have type `InviteAction`
 * const inviteAction = maybeInviteAction;
 * ```
 */
export function validateInviteAction(maybeInviteAction: unknown): InviteAction | OaValidationError {
  const { value, error } = InviteActionJoiSchema.validate(maybeInviteAction);
  if (error) {
    return new OaValidationError('InviteAction', maybeInviteAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as InviteAction;
}
