 ...
 * schema:ItemList
 *
 * This differs from ItemList because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ItemListOrSubClass =
  | ItemList
  | schema.BreadcrumbListOrSubClass
  | schema.HowToSectionOrSubClass
  | schema.OfferCatalogOrSubClass
 ...
 */
export const ItemListJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ItemListOrSubClassJoiSchema = Joi.alternatives().try([
  ItemListJoiSchema,
 ...
 *
 * ```ts
 * const maybeItemList = validateItemList(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateItemList(maybeItemList: unknown): ItemList | OaValidationError {
  const { value, error } = ItemListJoiSchema.validate(maybeItemList);
  if (error) {
    return new OaValidationError('ItemList', maybeItemList, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ItemList;
}
