 ...
 * schema:ItemPage
 *
 * This differs from ItemPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ItemPageOrSubClass =
  | ItemPage
  ;

/**
 * schema:ItemPage - Validation schema (w/ JOI)
 */
export const ItemPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('ItemPage').required(),
 ...
 * schema:ItemPage - Validation schema (w/ JOI)
 *
 * This differs from ItemPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ItemPageOrSubClassJoiSchema = Joi.alternatives().try([
  ItemPageJoiSchema,
]);

/**
 * Runtime validator for schema:ItemPage.
 *
 * If some data has a structure which matches a schema:ItemPage, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeItemPage` will have type `ItemPage`
 * const itemPage = maybeItemPage;
 * ```
 */
export function validateItemPage(maybeItemPage: unknown): ItemPage | OaValidationError {
  const { value, error } = ItemPageJoiSchema.validate(maybeItemPage);
  if (error) {
    return new OaValidationError('ItemPage', maybeItemPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ItemPage;
}
