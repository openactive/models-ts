 ...
 * schema:JewelryStore
 *
 * This differs from JewelryStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type JewelryStoreOrSubClass =
  | JewelryStore
  ;

/**
 * schema:JewelryStore - Validation schema (w/ JOI)
 */
export const JewelryStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('JewelryStore').required(),
 ...
 * schema:JewelryStore - Validation schema (w/ JOI)
 *
 * This differs from JewelryStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const JewelryStoreOrSubClassJoiSchema = Joi.alternatives().try([
  JewelryStoreJoiSchema,
]);

/**
 * Runtime validator for schema:JewelryStore.
 *
 * If some data has a structure which matches a schema:JewelryStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeJewelryStore = JewelryStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeJewelryStore instanceof OaValidationError) {
 *   // From this point on, `maybeJewelryStore` will have type `OaValidationError`
 *   const error = maybeJewelryStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeJewelryStore` will have type `JewelryStore`
 * const jewelryStore = maybeJewelryStore;
 * ```
 */
export function validateJewelryStore(maybeJewelryStore: unknown): JewelryStore | OaValidationError {
  const { value, error } = JewelryStoreJoiSchema.validate(maybeJewelryStore);
  if (error) {
    return new OaValidationError('JewelryStore', maybeJewelryStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as JewelryStore;
}
