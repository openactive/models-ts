 ...
 * schema:JoinAction
 *
 * This differs from JoinAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type JoinActionOrSubClass =
  | JoinAction
  ;

/**
 * schema:JoinAction - Validation schema (w/ JOI)
 */
export const JoinActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('JoinAction').required(),
 ...
 * schema:JoinAction - Validation schema (w/ JOI)
 *
 * This differs from JoinActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const JoinActionOrSubClassJoiSchema = Joi.alternatives().try([
  JoinActionJoiSchema,
]);

/**
 * Runtime validator for schema:JoinAction.
 *
 * If some data has a structure which matches a schema:JoinAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeJoinAction = JoinAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeJoinAction instanceof OaValidationError) {
 *   // From this point on, `maybeJoinAction` will have type `OaValidationError`
 *   const error = maybeJoinAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeJoinAction` will have type `JoinAction`
 * const joinAction = maybeJoinAction;
 * ```
 */
export function validateJoinAction(maybeJoinAction: unknown): JoinAction | OaValidationError {
  const { value, error } = JoinActionJoiSchema.validate(maybeJoinAction);
  if (error) {
    return new OaValidationError('JoinAction', maybeJoinAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as JoinAction;
}
