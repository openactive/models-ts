...
export const JointJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  functionalClass: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.MedicalEntityOrSubClassJoiSchema), Joi.string().uri()),
 ...
 *
 * ```ts
 * const maybeJoint = validateJoint(req.body); // `req.body` will have type `any` or `unknown`
