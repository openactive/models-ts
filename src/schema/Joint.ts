 ...
 * schema:Joint
 *
 * This differs from Joint because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type JointOrSubClass =
  | Joint
  ;

/**
 * schema:Joint - Validation schema (w/ JOI)
 */
export const JointJoiSchema = Joi.object({
  '@type': Joi.string().valid('Joint').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  functionalClass: Joi.alternatives().try(Joi.lazy(() => schema.MedicalEntityOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  structuralClass: Joi.string(),
  biomechnicalClass: Joi.string(),
 ...
 * schema:Joint - Validation schema (w/ JOI)
 *
 * This differs from JointJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const JointOrSubClassJoiSchema = Joi.alternatives().try([
  JointJoiSchema,
]);

/**
 * Runtime validator for schema:Joint.
 *
 * If some data has a structure which matches a schema:Joint, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeJoint = Joint.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeJoint instanceof OaValidationError) {
 *   // From this point on, `maybeJoint` will have type `OaValidationError`
 *   const error = maybeJoint;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeJoint` will have type `Joint`
 * const joint = maybeJoint;
 * ```
 */
export function validateJoint(maybeJoint: unknown): Joint | OaValidationError {
  const { value, error } = JointJoiSchema.validate(maybeJoint);
  if (error) {
    return new OaValidationError('Joint', maybeJoint, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Joint;
}
