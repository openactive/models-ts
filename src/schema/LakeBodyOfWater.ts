 ...
 * schema:LakeBodyOfWater
 *
 * This differs from LakeBodyOfWater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LakeBodyOfWaterOrSubClass =
  | LakeBodyOfWater
  ;
 ...
 */
export const LakeBodyOfWaterJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LakeBodyOfWaterOrSubClassJoiSchema = Joi.alternatives().try([
  LakeBodyOfWaterJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeLakeBodyOfWater = LakeBodyOfWater.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateLakeBodyOfWater(maybeLakeBodyOfWater: unknown): LakeBodyOfWater | OaValidationError {
  const { value, error } = LakeBodyOfWaterJoiSchema.validate(maybeLakeBodyOfWater);
  if (error) {
    return new OaValidationError('LakeBodyOfWater', maybeLakeBodyOfWater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LakeBodyOfWater;
}
