 ...
 * schema:Landform
 *
 * This differs from Landform because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LandformOrSubClass =
  | Landform
  | schema.BodyOfWaterOrSubClass
  | schema.ContinentOrSubClass
  | schema.MountainOrSubClass
  | schema.VolcanoOrSubClass
  ;

/**
 * schema:Landform - Validation schema (w/ JOI)
 */
export const LandformJoiSchema = Joi.object({
  '@type': Joi.string().valid('Landform').required(),
 ...
 * schema:Landform - Validation schema (w/ JOI)
 *
 * This differs from LandformJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LandformOrSubClassJoiSchema = Joi.alternatives().try([
  LandformJoiSchema,
  Joi.lazy(() => schema.BodyOfWaterOrSubClassJoiSchema),
  Joi.lazy(() => schema.ContinentOrSubClassJoiSchema),
  Joi.lazy(() => schema.MountainOrSubClassJoiSchema),
  Joi.lazy(() => schema.VolcanoOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Landform.
 *
 * If some data has a structure which matches a schema:Landform, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeLandform` will have type `Landform`
 * const landform = maybeLandform;
 * ```
 */
export function validateLandform(maybeLandform: unknown): Landform | OaValidationError {
  const { value, error } = LandformJoiSchema.validate(maybeLandform);
  if (error) {
    return new OaValidationError('Landform', maybeLandform, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Landform;
}
