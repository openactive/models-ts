 ...
 * schema:LandmarksOrHistoricalBuildings
 *
 * This differs from LandmarksOrHistoricalBuildings because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LandmarksOrHistoricalBuildingsOrSubClass =
  | LandmarksOrHistoricalBuildings
  ;

/**
 * schema:LandmarksOrHistoricalBuildings - Validation schema (w/ JOI)
 */
export const LandmarksOrHistoricalBuildingsJoiSchema = Joi.object({
  '@type': Joi.string().valid('LandmarksOrHistoricalBuildings').required(),
 ...
 * schema:LandmarksOrHistoricalBuildings - Validation schema (w/ JOI)
 *
 * This differs from LandmarksOrHistoricalBuildingsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LandmarksOrHistoricalBuildingsOrSubClassJoiSchema = Joi.alternatives().try([
  LandmarksOrHistoricalBuildingsJoiSchema,
]);

/**
 * Runtime validator for schema:LandmarksOrHistoricalBuildings.
 *
 * If some data has a structure which matches a schema:LandmarksOrHistoricalBuildings, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLandmarksOrHistoricalBuildings = LandmarksOrHistoricalBuildings.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLandmarksOrHistoricalBuildings instanceof OaValidationError) {
 *   // From this point on, `maybeLandmarksOrHistoricalBuildings` will have type `OaValidationError`
 *   const error = maybeLandmarksOrHistoricalBuildings;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLandmarksOrHistoricalBuildings` will have type `LandmarksOrHistoricalBuildings`
 * const landmarksOrHistoricalBuildings = maybeLandmarksOrHistoricalBuildings;
 * ```
 */
export function validateLandmarksOrHistoricalBuildings(maybeLandmarksOrHistoricalBuildings: unknown): LandmarksOrHistoricalBuildings | OaValidationError {
  const { value, error } = LandmarksOrHistoricalBuildingsJoiSchema.validate(maybeLandmarksOrHistoricalBuildings);
  if (error) {
    return new OaValidationError('LandmarksOrHistoricalBuildings', maybeLandmarksOrHistoricalBuildings, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LandmarksOrHistoricalBuildings;
}
