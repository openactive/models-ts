 ...
 * schema:Language
 *
 * This differs from Language because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LanguageOrSubClass =
  | Language
  ;
 ...
 * `SessionSeries`, etc.
 */
export const LanguageOrSubClassJoiSchema = Joi.alternatives().try([
  LanguageJoiSchema,
]);
 ...
 * ```
 */
export function validateLanguage(maybeLanguage: unknown): Language | OaValidationError {
  const { value, error } = LanguageJoiSchema.validate(maybeLanguage);
  if (error) {
    return new OaValidationError('Language', maybeLanguage, error);
  }
