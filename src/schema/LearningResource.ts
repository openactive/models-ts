 ...
 * schema:LearningResource
 *
 * This differs from LearningResource because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LearningResourceOrSubClass =
  | LearningResource
  | schema.CourseOrSubClass
  | schema.QuizOrSubClass
  ;

/**
 * schema:LearningResource - Validation schema (w/ JOI)
 */
export const LearningResourceJoiSchema = Joi.object({
  '@type': Joi.string().valid('LearningResource').required(),
 ...
 * schema:LearningResource - Validation schema (w/ JOI)
 *
 * This differs from LearningResourceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LearningResourceOrSubClassJoiSchema = Joi.alternatives().try([
  LearningResourceJoiSchema,
  Joi.lazy(() => schema.CourseOrSubClassJoiSchema),
  Joi.lazy(() => schema.QuizOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:LearningResource.
 *
 * If some data has a structure which matches a schema:LearningResource, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeLearningResource` will have type `LearningResource`
 * const learningResource = maybeLearningResource;
 * ```
 */
export function validateLearningResource(maybeLearningResource: unknown): LearningResource | OaValidationError {
  const { value, error } = LearningResourceJoiSchema.validate(maybeLearningResource);
  if (error) {
    return new OaValidationError('LearningResource', maybeLearningResource, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LearningResource;
}
