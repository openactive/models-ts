 ...
 * schema:LeaveAction
 *
 * This differs from LeaveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LeaveActionOrSubClass =
  | LeaveAction
  ;
 ...
 */
export const LeaveActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LeaveActionOrSubClassJoiSchema = Joi.alternatives().try([
  LeaveActionJoiSchema,
]);
 ...
 * ```
 */
export function validateLeaveAction(maybeLeaveAction: unknown): LeaveAction | OaValidationError {
  const { value, error } = LeaveActionJoiSchema.validate(maybeLeaveAction);
  if (error) {
    return new OaValidationError('LeaveAction', maybeLeaveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LeaveAction;
}
