 ...
 * etc.
 */
export type LegalForceStatusOrSubClass =
  | LegalForceStatus
  ;
 ...
 */
export const LegalForceStatusJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LegalForceStatusOrSubClassJoiSchema = Joi.alternatives().try([
  LegalForceStatusJoiSchema,
]);
 ...
 * ```
 */
export function validateLegalForceStatus(maybeLegalForceStatus: unknown): LegalForceStatus | OaValidationError {
  const { value, error } = LegalForceStatusJoiSchema.validate(maybeLegalForceStatus);
  if (error) {
    return new OaValidationError('LegalForceStatus', maybeLegalForceStatus, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LegalForceStatus;
}
