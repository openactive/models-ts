 ...
 * schema:LegalService
 *
 * This differs from LegalService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LegalServiceOrSubClass =
  | LegalService
  | schema.AttorneyOrSubClass
  | schema.NotaryOrSubClass
  ;

/**
 * schema:LegalService - Validation schema (w/ JOI)
 */
export const LegalServiceJoiSchema = Joi.object({
  '@type': Joi.string().valid('LegalService').required(),
 ...
 * schema:LegalService - Validation schema (w/ JOI)
 *
 * This differs from LegalServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LegalServiceOrSubClassJoiSchema = Joi.alternatives().try([
  LegalServiceJoiSchema,
  Joi.lazy(() => schema.AttorneyOrSubClassJoiSchema),
  Joi.lazy(() => schema.NotaryOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:LegalService.
 *
 * If some data has a structure which matches a schema:LegalService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLegalService = LegalService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLegalService instanceof OaValidationError) {
 *   // From this point on, `maybeLegalService` will have type `OaValidationError`
 *   const error = maybeLegalService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLegalService` will have type `LegalService`
 * const legalService = maybeLegalService;
 * ```
 */
export function validateLegalService(maybeLegalService: unknown): LegalService | OaValidationError {
  const { value, error } = LegalServiceJoiSchema.validate(maybeLegalService);
  if (error) {
    return new OaValidationError('LegalService', maybeLegalService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LegalService;
}
