 ...
 * schema:Legislation
 *
 * This differs from Legislation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LegislationOrSubClass =
  | Legislation
  | schema.LegislationObjectOrSubClass
  ;

/**
 * schema:Legislation - Validation schema (w/ JOI)
 */
export const LegislationJoiSchema = Joi.object({
  '@type': Joi.string().valid('Legislation').required(),
 ...
 * schema:Legislation - Validation schema (w/ JOI)
 *
 * This differs from LegislationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LegislationOrSubClassJoiSchema = Joi.alternatives().try([
  LegislationJoiSchema,
  Joi.lazy(() => schema.LegislationObjectOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Legislation.
 *
 * If some data has a structure which matches a schema:Legislation, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeLegislation` will have type `Legislation`
 * const legislation = maybeLegislation;
 * ```
 */
export function validateLegislation(maybeLegislation: unknown): Legislation | OaValidationError {
  const { value, error } = LegislationJoiSchema.validate(maybeLegislation);
  if (error) {
    return new OaValidationError('Legislation', maybeLegislation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Legislation;
}
