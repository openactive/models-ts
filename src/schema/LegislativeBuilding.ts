 ...
 */
export type LegislativeBuilding = {
 ...
 * schema:LegislativeBuilding
 *
 * This differs from LegislativeBuilding because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LegislativeBuildingOrSubClass =
  | LegislativeBuilding
  ;

/**
 * schema:LegislativeBuilding - Validation schema (w/ JOI)
 */
export const LegislativeBuildingJoiSchema = Joi.object({
  '@type': Joi.string().valid('LegislativeBuilding').required(),
 ...
 * schema:LegislativeBuilding - Validation schema (w/ JOI)
 *
 * This differs from LegislativeBuildingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LegislativeBuildingOrSubClassJoiSchema = Joi.alternatives().try([
  LegislativeBuildingJoiSchema,
]);

/**
 * Runtime validator for schema:LegislativeBuilding.
 *
 * If some data has a structure which matches a schema:LegislativeBuilding, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLegislativeBuilding = LegislativeBuilding.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLegislativeBuilding instanceof OaValidationError) {
 *   // From this point on, `maybeLegislativeBuilding` will have type `OaValidationError`
 *   const error = maybeLegislativeBuilding;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLegislativeBuilding` will have type `LegislativeBuilding`
 * const legislativeBuilding = maybeLegislativeBuilding;
 * ```
 */
export function validateLegislativeBuilding(maybeLegislativeBuilding: unknown): LegislativeBuilding | OaValidationError {
  const { value, error } = LegislativeBuildingJoiSchema.validate(maybeLegislativeBuilding);
  if (error) {
    return new OaValidationError('LegislativeBuilding', maybeLegislativeBuilding, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LegislativeBuilding;
}
