...
export type LendAction = {
   ...
   */
  borrower?: schema.PersonOrSubClass | string;
  /**
 ...
 * schema:LendAction
 *
 * This differs from LendAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LendActionOrSubClass =
  | LendAction
  ;

/**
 * schema:LendAction - Validation schema (w/ JOI)
 */
export const LendActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('LendAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  borrower: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:LendAction - Validation schema (w/ JOI)
 *
 * This differs from LendActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LendActionOrSubClassJoiSchema = Joi.alternatives().try([
  LendActionJoiSchema,
]);

/**
 * Runtime validator for schema:LendAction.
 *
 * If some data has a structure which matches a schema:LendAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLendAction = LendAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLendAction instanceof OaValidationError) {
 *   // From this point on, `maybeLendAction` will have type `OaValidationError`
 *   const error = maybeLendAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLendAction` will have type `LendAction`
 * const lendAction = maybeLendAction;
 * ```
 */
export function validateLendAction(maybeLendAction: unknown): LendAction | OaValidationError {
  const { value, error } = LendActionJoiSchema.validate(maybeLendAction);
  if (error) {
    return new OaValidationError('LendAction', maybeLendAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LendAction;
}
