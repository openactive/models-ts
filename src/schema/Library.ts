 ...
 * schema:Library
 *
 * This differs from Library because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LibraryOrSubClass =
  | Library
  ;

/**
 * schema:Library - Validation schema (w/ JOI)
 */
export const LibraryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Library').required(),
 ...
 * schema:Library - Validation schema (w/ JOI)
 *
 * This differs from LibraryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LibraryOrSubClassJoiSchema = Joi.alternatives().try([
  LibraryJoiSchema,
]);

/**
 * Runtime validator for schema:Library.
 *
 * If some data has a structure which matches a schema:Library, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLibrary = Library.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLibrary instanceof OaValidationError) {
 *   // From this point on, `maybeLibrary` will have type `OaValidationError`
 *   const error = maybeLibrary;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLibrary` will have type `Library`
 * const library = maybeLibrary;
 * ```
 */
export function validateLibrary(maybeLibrary: unknown): Library | OaValidationError {
  const { value, error } = LibraryJoiSchema.validate(maybeLibrary);
  if (error) {
    return new OaValidationError('Library', maybeLibrary, error);
  }
