 ...
 * schema:LibrarySystem
 *
 * This differs from LibrarySystem because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LibrarySystemOrSubClass =
  | LibrarySystem
  ;

/**
 * schema:LibrarySystem - Validation schema (w/ JOI)
 */
export const LibrarySystemJoiSchema = Joi.object({
  '@type': Joi.string().valid('LibrarySystem').required(),
 ...
 * schema:LibrarySystem - Validation schema (w/ JOI)
 *
 * This differs from LibrarySystemJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LibrarySystemOrSubClassJoiSchema = Joi.alternatives().try([
  LibrarySystemJoiSchema,
]);

/**
 * Runtime validator for schema:LibrarySystem.
 *
 * If some data has a structure which matches a schema:LibrarySystem, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeLibrarySystem` will have type `LibrarySystem`
 * const librarySystem = maybeLibrarySystem;
 * ```
 */
export function validateLibrarySystem(maybeLibrarySystem: unknown): LibrarySystem | OaValidationError {
  const { value, error } = LibrarySystemJoiSchema.validate(maybeLibrarySystem);
  if (error) {
    return new OaValidationError('LibrarySystem', maybeLibrarySystem, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LibrarySystem;
}
