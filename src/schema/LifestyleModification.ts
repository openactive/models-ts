 ...
 * schema:LifestyleModification
 *
 * This differs from LifestyleModification because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LifestyleModificationOrSubClass =
  | LifestyleModification
  | schema.PhysicalActivityOrSubClass
 ...
 */
export const LifestyleModificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LifestyleModificationOrSubClassJoiSchema = Joi.alternatives().try([
  LifestyleModificationJoiSchema,
 ...
 *
 * ```ts
 * const maybeLifestyleModification = LifestyleModification.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateLifestyleModification(maybeLifestyleModification: unknown): LifestyleModification | OaValidationError {
  const { value, error } = LifestyleModificationJoiSchema.validate(maybeLifestyleModification);
  if (error) {
    return new OaValidationError('LifestyleModification', maybeLifestyleModification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LifestyleModification;
}
