 ...
 * etc.
 */
export type LigamentOrSubClass =
  | Ligament
  ;
 ...
 */
export const LigamentJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LigamentOrSubClassJoiSchema = Joi.alternatives().try([
  LigamentJoiSchema,
]);
 ...
 * ```
 */
export function validateLigament(maybeLigament: unknown): Ligament | OaValidationError {
  const { value, error } = LigamentJoiSchema.validate(maybeLigament);
  if (error) {
    return new OaValidationError('Ligament', maybeLigament, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Ligament;
}
