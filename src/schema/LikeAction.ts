 ...
 * schema:LikeAction
 *
 * This differs from LikeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LikeActionOrSubClass =
  | LikeAction
  ;

/**
 * schema:LikeAction - Validation schema (w/ JOI)
 */
export const LikeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('LikeAction').required(),
 ...
 * schema:LikeAction - Validation schema (w/ JOI)
 *
 * This differs from LikeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LikeActionOrSubClassJoiSchema = Joi.alternatives().try([
  LikeActionJoiSchema,
]);

/**
 * Runtime validator for schema:LikeAction.
 *
 * If some data has a structure which matches a schema:LikeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLikeAction = LikeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLikeAction instanceof OaValidationError) {
 *   // From this point on, `maybeLikeAction` will have type `OaValidationError`
 *   const error = maybeLikeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLikeAction` will have type `LikeAction`
 * const likeAction = maybeLikeAction;
 * ```
 */
export function validateLikeAction(maybeLikeAction: unknown): LikeAction | OaValidationError {
  const { value, error } = LikeActionJoiSchema.validate(maybeLikeAction);
  if (error) {
    return new OaValidationError('LikeAction', maybeLikeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LikeAction;
}
