 ...
 * etc.
 */
export type LinkRoleOrSubClass =
  | LinkRole
  ;
 ...
 */
export const LinkRoleJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LinkRoleOrSubClassJoiSchema = Joi.alternatives().try([
  LinkRoleJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeLinkRole = LinkRole.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateLinkRole(maybeLinkRole: unknown): LinkRole | OaValidationError {
  const { value, error } = LinkRoleJoiSchema.validate(maybeLinkRole);
  if (error) {
    return new OaValidationError('LinkRole', maybeLinkRole, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LinkRole;
}
