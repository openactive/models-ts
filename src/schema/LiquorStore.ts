 ...
 * schema:LiquorStore
 *
 * This differs from LiquorStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LiquorStoreOrSubClass =
  | LiquorStore
  ;

/**
 * schema:LiquorStore - Validation schema (w/ JOI)
 */
export const LiquorStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('LiquorStore').required(),
 ...
 * schema:LiquorStore - Validation schema (w/ JOI)
 *
 * This differs from LiquorStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LiquorStoreOrSubClassJoiSchema = Joi.alternatives().try([
  LiquorStoreJoiSchema,
]);

/**
 * Runtime validator for schema:LiquorStore.
 *
 * If some data has a structure which matches a schema:LiquorStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLiquorStore = LiquorStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLiquorStore instanceof OaValidationError) {
 *   // From this point on, `maybeLiquorStore` will have type `OaValidationError`
 *   const error = maybeLiquorStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLiquorStore` will have type `LiquorStore`
 * const liquorStore = maybeLiquorStore;
 * ```
 */
export function validateLiquorStore(maybeLiquorStore: unknown): LiquorStore | OaValidationError {
  const { value, error } = LiquorStoreJoiSchema.validate(maybeLiquorStore);
  if (error) {
    return new OaValidationError('LiquorStore', maybeLiquorStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LiquorStore;
}
