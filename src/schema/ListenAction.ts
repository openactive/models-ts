 ...
 * schema:ListenAction
 *
 * This differs from ListenAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ListenActionOrSubClass =
  | ListenAction
  ;

/**
 * schema:ListenAction - Validation schema (w/ JOI)
 */
export const ListenActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ListenAction').required(),
 ...
 * schema:ListenAction - Validation schema (w/ JOI)
 *
 * This differs from ListenActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ListenActionOrSubClassJoiSchema = Joi.alternatives().try([
  ListenActionJoiSchema,
]);

/**
 * Runtime validator for schema:ListenAction.
 *
 * If some data has a structure which matches a schema:ListenAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeListenAction = ListenAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeListenAction instanceof OaValidationError) {
 *   // From this point on, `maybeListenAction` will have type `OaValidationError`
 *   const error = maybeListenAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeListenAction` will have type `ListenAction`
 * const listenAction = maybeListenAction;
 * ```
 */
export function validateListenAction(maybeListenAction: unknown): ListenAction | OaValidationError {
  const { value, error } = ListenActionJoiSchema.validate(maybeListenAction);
  if (error) {
    return new OaValidationError('ListenAction', maybeListenAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ListenAction;
}
