 ...
 * schema:LiteraryEvent
 *
 * This differs from LiteraryEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LiteraryEventOrSubClass =
  | LiteraryEvent
  ;

/**
 * schema:LiteraryEvent - Validation schema (w/ JOI)
 */
export const LiteraryEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('LiteraryEvent').required(),
 ...
 * schema:LiteraryEvent - Validation schema (w/ JOI)
 *
 * This differs from LiteraryEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LiteraryEventOrSubClassJoiSchema = Joi.alternatives().try([
  LiteraryEventJoiSchema,
]);

/**
 * Runtime validator for schema:LiteraryEvent.
 *
 * If some data has a structure which matches a schema:LiteraryEvent, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeLiteraryEvent` will have type `LiteraryEvent`
 * const literaryEvent = maybeLiteraryEvent;
 * ```
 */
export function validateLiteraryEvent(maybeLiteraryEvent: unknown): LiteraryEvent | OaValidationError {
  const { value, error } = LiteraryEventJoiSchema.validate(maybeLiteraryEvent);
  if (error) {
    return new OaValidationError('LiteraryEvent', maybeLiteraryEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LiteraryEvent;
}
