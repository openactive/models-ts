...
export type LiveBlogPosting = {
   ...
   */
  liveBlogUpdate?: schema.BlogPostingOrSubClass | string;
  /**
   * A CreativeWork such as an image, video, or audio clip shared as part of this posting.
   */
  sharedContent?: schema.CreativeWorkOrSubClass | string;
  /**
 ...
 * schema:LiveBlogPosting
 *
 * This differs from LiveBlogPosting because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LiveBlogPostingOrSubClass =
  | LiveBlogPosting
  ;

/**
 * schema:LiveBlogPosting - Validation schema (w/ JOI)
 */
export const LiveBlogPostingJoiSchema = Joi.object({
  '@type': Joi.string().valid('LiveBlogPosting').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  coverageEndTime: Joi.string().isoDate(),
  coverageStartTime: Joi.string().isoDate(),
  liveBlogUpdate: Joi.alternatives().try(Joi.lazy(() => schema.BlogPostingOrSubClassJoiSchema), Joi.string().uri()),
  sharedContent: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:LiveBlogPosting - Validation schema (w/ JOI)
 *
 * This differs from LiveBlogPostingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LiveBlogPostingOrSubClassJoiSchema = Joi.alternatives().try([
  LiveBlogPostingJoiSchema,
]);

/**
 * Runtime validator for schema:LiveBlogPosting.
 *
 * If some data has a structure which matches a schema:LiveBlogPosting, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLiveBlogPosting = LiveBlogPosting.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLiveBlogPosting instanceof OaValidationError) {
 *   // From this point on, `maybeLiveBlogPosting` will have type `OaValidationError`
 *   const error = maybeLiveBlogPosting;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLiveBlogPosting` will have type `LiveBlogPosting`
 * const liveBlogPosting = maybeLiveBlogPosting;
 * ```
 */
export function validateLiveBlogPosting(maybeLiveBlogPosting: unknown): LiveBlogPosting | OaValidationError {
  const { value, error } = LiveBlogPostingJoiSchema.validate(maybeLiveBlogPosting);
  if (error) {
    return new OaValidationError('LiveBlogPosting', maybeLiveBlogPosting, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LiveBlogPosting;
}
