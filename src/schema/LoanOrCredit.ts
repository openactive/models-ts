 ...
 * etc.
 */
export type LoanOrCreditOrSubClass =
  | LoanOrCredit
  | schema.CreditCardOrSubClass
  | schema.MortgageLoanOrSubClass
  ;

/**
 * schema:LoanOrCredit - Validation schema (w/ JOI)
 */
export const LoanOrCreditJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LoanOrCreditOrSubClassJoiSchema = Joi.alternatives().try([
  LoanOrCreditJoiSchema,
 ...
 * ```
 */
export function validateLoanOrCredit(maybeLoanOrCredit: unknown): LoanOrCredit | OaValidationError {
  const { value, error } = LoanOrCreditJoiSchema.validate(maybeLoanOrCredit);
  if (error) {
    return new OaValidationError('LoanOrCredit', maybeLoanOrCredit, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LoanOrCredit;
}
