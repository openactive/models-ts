 ...
 * schema:LocalBusiness
 *
 * This differs from LocalBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LocalBusinessOrSubClass =
  | LocalBusiness
  | schema.EmploymentAgencyOrSubClass
  | schema.DryCleaningOrLaundryOrSubClass
  | schema.ProfessionalServiceOrSubClass
  | schema.SelfStorageOrSubClass
  | schema.EmergencyServiceOrSubClass
  | schema.MedicalBusinessOrSubClass
  | schema.RadioStationOrSubClass
  | schema.TravelAgencyOrSubClass
  | schema.RecyclingCenterOrSubClass
  | schema.AnimalShelterOrSubClass
  | schema.HomeAndConstructionBusinessOrSubClass
  | schema.FoodEstablishmentOrSubClass
  | schema.LibraryOrSubClass
  | schema.ChildCareOrSubClass
  | schema.DentistOrSubClass
  | schema.InternetCafeOrSubClass
  | schema.FinancialServiceOrSubClass
  | schema.AutomotiveBusinessOrSubClass
  | schema.GovernmentOfficeOrSubClass
  | schema.ShoppingCenterOrSubClass
  | schema.StoreOrSubClass
  | schema.TouristInformationCenterOrSubClass
  | schema.EntertainmentBusinessOrSubClass
  | schema.LegalServiceOrSubClass
  | schema.HealthAndBeautyBusinessOrSubClass
  | schema.LodgingBusinessOrSubClass
  | schema.RealEstateAgentOrSubClass
  | schema.TelevisionStationOrSubClass
  | schema.SportsActivityLocationOrSubClass
  | schema.ArchiveOrganizationOrSubClass
  ;

/**
 * schema:LocalBusiness - Validation schema (w/ JOI)
 */
export const LocalBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('LocalBusiness').required(),
 ...
 * schema:LocalBusiness - Validation schema (w/ JOI)
 *
 * This differs from LocalBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LocalBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  LocalBusinessJoiSchema,
  Joi.lazy(() => schema.EmploymentAgencyOrSubClassJoiSchema),
  Joi.lazy(() => schema.DryCleaningOrLaundryOrSubClassJoiSchema),
  Joi.lazy(() => schema.ProfessionalServiceOrSubClassJoiSchema),
  Joi.lazy(() => schema.SelfStorageOrSubClassJoiSchema),
  Joi.lazy(() => schema.EmergencyServiceOrSubClassJoiSchema),
  Joi.lazy(() => schema.MedicalBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.RadioStationOrSubClassJoiSchema),
  Joi.lazy(() => schema.TravelAgencyOrSubClassJoiSchema),
  Joi.lazy(() => schema.RecyclingCenterOrSubClassJoiSchema),
  Joi.lazy(() => schema.AnimalShelterOrSubClassJoiSchema),
  Joi.lazy(() => schema.HomeAndConstructionBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.FoodEstablishmentOrSubClassJoiSchema),
  Joi.lazy(() => schema.LibraryOrSubClassJoiSchema),
  Joi.lazy(() => schema.ChildCareOrSubClassJoiSchema),
  Joi.lazy(() => schema.DentistOrSubClassJoiSchema),
  Joi.lazy(() => schema.InternetCafeOrSubClassJoiSchema),
  Joi.lazy(() => schema.FinancialServiceOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutomotiveBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.GovernmentOfficeOrSubClassJoiSchema),
  Joi.lazy(() => schema.ShoppingCenterOrSubClassJoiSchema),
  Joi.lazy(() => schema.StoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.TouristInformationCenterOrSubClassJoiSchema),
  Joi.lazy(() => schema.EntertainmentBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.LegalServiceOrSubClassJoiSchema),
  Joi.lazy(() => schema.HealthAndBeautyBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.LodgingBusinessOrSubClassJoiSchema),
  Joi.lazy(() => schema.RealEstateAgentOrSubClassJoiSchema),
  Joi.lazy(() => schema.TelevisionStationOrSubClassJoiSchema),
  Joi.lazy(() => schema.SportsActivityLocationOrSubClassJoiSchema),
  Joi.lazy(() => schema.ArchiveOrganizationOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:LocalBusiness.
 *
 * If some data has a structure which matches a schema:LocalBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLocalBusiness = LocalBusiness.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLocalBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeLocalBusiness` will have type `OaValidationError`
 *   const error = maybeLocalBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLocalBusiness` will have type `LocalBusiness`
 * const localBusiness = maybeLocalBusiness;
 * ```
 */
export function validateLocalBusiness(maybeLocalBusiness: unknown): LocalBusiness | OaValidationError {
  const { value, error } = LocalBusinessJoiSchema.validate(maybeLocalBusiness);
  if (error) {
    return new OaValidationError('LocalBusiness', maybeLocalBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LocalBusiness;
}
