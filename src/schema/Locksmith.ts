 ...
 * schema:Locksmith
 *
 * This differs from Locksmith because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LocksmithOrSubClass =
  | Locksmith
  ;

/**
 * schema:Locksmith - Validation schema (w/ JOI)
 */
export const LocksmithJoiSchema = Joi.object({
  '@type': Joi.string().valid('Locksmith').required(),
 ...
 * schema:Locksmith - Validation schema (w/ JOI)
 *
 * This differs from LocksmithJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LocksmithOrSubClassJoiSchema = Joi.alternatives().try([
  LocksmithJoiSchema,
]);

/**
 * Runtime validator for schema:Locksmith.
 *
 * If some data has a structure which matches a schema:Locksmith, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLocksmith = Locksmith.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLocksmith instanceof OaValidationError) {
 *   // From this point on, `maybeLocksmith` will have type `OaValidationError`
 *   const error = maybeLocksmith;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLocksmith` will have type `Locksmith`
 * const locksmith = maybeLocksmith;
 * ```
 */
export function validateLocksmith(maybeLocksmith: unknown): Locksmith | OaValidationError {
  const { value, error } = LocksmithJoiSchema.validate(maybeLocksmith);
  if (error) {
    return new OaValidationError('Locksmith', maybeLocksmith, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Locksmith;
}
