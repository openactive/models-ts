...
export type LodgingBusiness = {
   ...
   */
  numberOfRooms?: schema.QuantitativeValueOrSubClass | number | string;
  /**
 ...
 * schema:LodgingBusiness
 *
 * This differs from LodgingBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LodgingBusinessOrSubClass =
  | LodgingBusiness
  | schema.BedAndBreakfastOrSubClass
  | schema.CampgroundOrSubClass
  | schema.HostelOrSubClass
  | schema.ResortOrSubClass
  | schema.MotelOrSubClass
  | schema.HotelOrSubClass
  ;

/**
 * schema:LodgingBusiness - Validation schema (w/ JOI)
 */
export const LodgingBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('LodgingBusiness').required(),
 ...
 * schema:LodgingBusiness - Validation schema (w/ JOI)
 *
 * This differs from LodgingBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const LodgingBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  LodgingBusinessJoiSchema,
  Joi.lazy(() => schema.BedAndBreakfastOrSubClassJoiSchema),
  Joi.lazy(() => schema.CampgroundOrSubClassJoiSchema),
  Joi.lazy(() => schema.HostelOrSubClassJoiSchema),
  Joi.lazy(() => schema.ResortOrSubClassJoiSchema),
  Joi.lazy(() => schema.MotelOrSubClassJoiSchema),
  Joi.lazy(() => schema.HotelOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:LodgingBusiness.
 *
 * If some data has a structure which matches a schema:LodgingBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeLodgingBusiness = validateLodgingBusiness(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeLodgingBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeLodgingBusiness` will have type `OaValidationError`
 *   const error = maybeLodgingBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeLodgingBusiness` will have type `LodgingBusiness`
 * const lodgingBusiness = maybeLodgingBusiness;
 * ```
 */
export function validateLodgingBusiness(maybeLodgingBusiness: unknown): LodgingBusiness | OaValidationError {
  const { value, error } = LodgingBusinessJoiSchema.validate(maybeLodgingBusiness);
  if (error) {
    return new OaValidationError('LodgingBusiness', maybeLodgingBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LodgingBusiness;
}
