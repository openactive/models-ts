...
export type LoseAction = {
   ...
   */
  winner?: schema.PersonOrSubClass | string;
  /**
 ...
 * schema:LoseAction
 *
 * This differs from LoseAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type LoseActionOrSubClass =
  | LoseAction
  ;
 ...
 */
export const LoseActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const LoseActionOrSubClassJoiSchema = Joi.alternatives().try([
  LoseActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeLoseAction = LoseAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateLoseAction(maybeLoseAction: unknown): LoseAction | OaValidationError {
  const { value, error } = LoseActionJoiSchema.validate(maybeLoseAction);
  if (error) {
    return new OaValidationError('LoseAction', maybeLoseAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as LoseAction;
}
