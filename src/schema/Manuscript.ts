 ...
 * schema:Manuscript
 *
 * This differs from Manuscript because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ManuscriptOrSubClass =
  | Manuscript
  ;
 ...
 */
export const ManuscriptJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ManuscriptOrSubClassJoiSchema = Joi.alternatives().try([
  ManuscriptJoiSchema,
]);
 ...
 * ```
 */
export function validateManuscript(maybeManuscript: unknown): Manuscript | OaValidationError {
  const { value, error } = ManuscriptJoiSchema.validate(maybeManuscript);
  if (error) {
    return new OaValidationError('Manuscript', maybeManuscript, error);
  }
