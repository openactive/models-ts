 ...
 * schema:Map
 *
 * This differs from Map because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MapOrSubClass =
  | Map
  ;

/**
 * schema:Map - Validation schema (w/ JOI)
 */
export const MapJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  mapType: Joi.lazy(() => schema.MapCategoryTypeJoiSchema),
  text: Joi.string(),
 ...
 * schema:Map - Validation schema (w/ JOI)
 *
 * This differs from MapJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MapOrSubClassJoiSchema = Joi.alternatives().try([
  MapJoiSchema,
]);

/**
 * Runtime validator for schema:Map.
 *
 * If some data has a structure which matches a schema:Map, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMap = Map.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMap instanceof OaValidationError) {
 *   // From this point on, `maybeMap` will have type `OaValidationError`
 *   const error = maybeMap;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMap` will have type `Map`
 * const map = maybeMap;
 * ```
 */
export function validateMap(maybeMap: unknown): Map | OaValidationError {
  const { value, error } = MapJoiSchema.validate(maybeMap);
  if (error) {
    return new OaValidationError('Map', maybeMap, error);
  }
