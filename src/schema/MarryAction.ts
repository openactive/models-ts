 ...
 * schema:MarryAction
 *
 * This differs from MarryAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MarryActionOrSubClass =
  | MarryAction
  ;

/**
 * schema:MarryAction - Validation schema (w/ JOI)
 */
export const MarryActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('MarryAction').required(),
 ...
 * schema:MarryAction - Validation schema (w/ JOI)
 *
 * This differs from MarryActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MarryActionOrSubClassJoiSchema = Joi.alternatives().try([
  MarryActionJoiSchema,
]);

/**
 * Runtime validator for schema:MarryAction.
 *
 * If some data has a structure which matches a schema:MarryAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMarryAction` will have type `MarryAction`
 * const marryAction = maybeMarryAction;
 * ```
 */
export function validateMarryAction(maybeMarryAction: unknown): MarryAction | OaValidationError {
  const { value, error } = MarryActionJoiSchema.validate(maybeMarryAction);
  if (error) {
    return new OaValidationError('MarryAction', maybeMarryAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MarryAction;
}
