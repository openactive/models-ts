 ...
 * schema:Mass
 *
 * This differs from Mass because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MassOrSubClass =
  | Mass
  ;
 ...
 */
export const MassJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MassOrSubClassJoiSchema = Joi.alternatives().try([
  MassJoiSchema,
]);
 ...
 * ```
 */
export function validateMass(maybeMass: unknown): Mass | OaValidationError {
  const { value, error } = MassJoiSchema.validate(maybeMass);
  if (error) {
    return new OaValidationError('Mass', maybeMass, error);
  }
