...
export type MathSolver = {
   ...
   */
  mathExpression?: string | schema.SolveMathActionOrSubClass;
  /**
 ...
 * schema:MathSolver
 *
 * This differs from MathSolver because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MathSolverOrSubClass =
  | MathSolver
  ;

/**
 * schema:MathSolver - Validation schema (w/ JOI)
 */
export const MathSolverJoiSchema = Joi.object({
  '@type': Joi.string().valid('MathSolver').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  mathExpression: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.SolveMathActionOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:MathSolver - Validation schema (w/ JOI)
 *
 * This differs from MathSolverJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MathSolverOrSubClassJoiSchema = Joi.alternatives().try([
  MathSolverJoiSchema,
]);

/**
 * Runtime validator for schema:MathSolver.
 *
 * If some data has a structure which matches a schema:MathSolver, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMathSolver` will have type `MathSolver`
 * const mathSolver = maybeMathSolver;
 * ```
 */
export function validateMathSolver(maybeMathSolver: unknown): MathSolver | OaValidationError {
  const { value, error } = MathSolverJoiSchema.validate(maybeMathSolver);
  if (error) {
    return new OaValidationError('MathSolver', maybeMathSolver, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MathSolver;
}
