 ...
 * Enumeration of common measurement types (or dimensions), for example "chest" for a person, "inseam" for pants, "gauge" for screws, or "wheel" for bicycles.
 */
export const MeasurementTypeEnumerationJoiSchema = Joi.string().valid(
);
 ...
 * ```
 */
export function validateMeasurementTypeEnumeration(maybeMeasurementTypeEnumeration: unknown): MeasurementTypeEnumeration | OaValidationError {
  const { value, error } = MeasurementTypeEnumerationJoiSchema.validate(maybeMeasurementTypeEnumeration);
  if (error) {
    return new OaValidationError('MeasurementTypeEnumeration', maybeMeasurementTypeEnumeration, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MeasurementTypeEnumeration;
}
