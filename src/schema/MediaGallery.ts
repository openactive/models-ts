 ...
 * schema:MediaGallery
 *
 * This differs from MediaGallery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MediaGalleryOrSubClass =
  | MediaGallery
  | schema.ImageGalleryOrSubClass
  | schema.VideoGalleryOrSubClass
  ;

/**
 * schema:MediaGallery - Validation schema (w/ JOI)
 */
export const MediaGalleryJoiSchema = Joi.object({
  '@type': Joi.string().valid('MediaGallery').required(),
 ...
 * schema:MediaGallery - Validation schema (w/ JOI)
 *
 * This differs from MediaGalleryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MediaGalleryOrSubClassJoiSchema = Joi.alternatives().try([
  MediaGalleryJoiSchema,
  Joi.lazy(() => schema.ImageGalleryOrSubClassJoiSchema),
  Joi.lazy(() => schema.VideoGalleryOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MediaGallery.
 *
 * If some data has a structure which matches a schema:MediaGallery, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMediaGallery` will have type `MediaGallery`
 * const mediaGallery = maybeMediaGallery;
 * ```
 */
export function validateMediaGallery(maybeMediaGallery: unknown): MediaGallery | OaValidationError {
  const { value, error } = MediaGalleryJoiSchema.validate(maybeMediaGallery);
  if (error) {
    return new OaValidationError('MediaGallery', maybeMediaGallery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MediaGallery;
}
