 ...
 *  Codes for use with the [[mediaAuthenticityCategory]] property, indicating the authenticity of a media object (in the context of how it was published or shared). In general these codes are not mutually exclusive, although some combinations (such as 'original' versus 'transformed', 'edited' and 'staged') would be contradictory if applied in the same [[MediaReview]]. Note that the application of these codes is with regard to a piece of media shared or published in a particular context.
 */
export const MediaManipulationRatingEnumerationJoiSchema = Joi.string().valid(
  'https://schema.org/OriginalMediaContent',
  'https://schema.org/TransformedContent',
  'https://schema.org/StagedContent',
  'https://schema.org/EditedOrCroppedContent',
 ...
 * ```
 */
export function validateMediaManipulationRatingEnumeration(maybeMediaManipulationRatingEnumeration: unknown): MediaManipulationRatingEnumeration | OaValidationError {
  const { value, error } = MediaManipulationRatingEnumerationJoiSchema.validate(maybeMediaManipulationRatingEnumeration);
  if (error) {
    return new OaValidationError('MediaManipulationRatingEnumeration', maybeMediaManipulationRatingEnumeration, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MediaManipulationRatingEnumeration;
}
