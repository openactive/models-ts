 ...
 * schema:MediaObject
 *
 * This differs from MediaObject because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MediaObjectOrSubClass =
  | MediaObject
  | schema.DataDownloadOrSubClass
  | schema.ImageObjectOrSubClass
  | schema.VideoObjectOrSubClass
  | schema.AudioObjectOrSubClass
  | schema.LegislationObjectOrSubClass
  | schema.MusicVideoObjectOrSubClass
  | schema.ThreeDModelOrSubClass
  ;

/**
 * schema:MediaObject - Validation schema (w/ JOI)
 */
export const MediaObjectJoiSchema = Joi.object({
  '@type': Joi.string().valid('MediaObject').required(),
 ...
 * schema:MediaObject - Validation schema (w/ JOI)
 *
 * This differs from MediaObjectJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MediaObjectOrSubClassJoiSchema = Joi.alternatives().try([
  MediaObjectJoiSchema,
  Joi.lazy(() => schema.DataDownloadOrSubClassJoiSchema),
  Joi.lazy(() => schema.ImageObjectOrSubClassJoiSchema),
  Joi.lazy(() => schema.VideoObjectOrSubClassJoiSchema),
  Joi.lazy(() => schema.AudioObjectOrSubClassJoiSchema),
  Joi.lazy(() => schema.LegislationObjectOrSubClassJoiSchema),
  Joi.lazy(() => schema.MusicVideoObjectOrSubClassJoiSchema),
  Joi.lazy(() => schema.ThreeDModelOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MediaObject.
 *
 * If some data has a structure which matches a schema:MediaObject, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMediaObject = MediaObject.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMediaObject instanceof OaValidationError) {
 *   // From this point on, `maybeMediaObject` will have type `OaValidationError`
 *   const error = maybeMediaObject;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMediaObject` will have type `MediaObject`
 * const mediaObject = maybeMediaObject;
 * ```
 */
export function validateMediaObject(maybeMediaObject: unknown): MediaObject | OaValidationError {
  const { value, error } = MediaObjectJoiSchema.validate(maybeMediaObject);
  if (error) {
    return new OaValidationError('MediaObject', maybeMediaObject, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MediaObject;
}
