...
export type MediaReviewItem = {
   ...
   */
  mediaItemAppearance?: schema.MediaObjectOrSubClass | string;
  /**
 ...
 * schema:MediaReviewItem
 *
 * This differs from MediaReviewItem because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MediaReviewItemOrSubClass =
  | MediaReviewItem
  ;
 ...
 */
export const MediaReviewItemJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MediaReviewItemOrSubClassJoiSchema = Joi.alternatives().try([
  MediaReviewItemJoiSchema,
]);
 ...
 * ```
 */
export function validateMediaReviewItem(maybeMediaReviewItem: unknown): MediaReviewItem | OaValidationError {
  const { value, error } = MediaReviewItemJoiSchema.validate(maybeMediaReviewItem);
  if (error) {
    return new OaValidationError('MediaReviewItem', maybeMediaReviewItem, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MediaReviewItem;
}
