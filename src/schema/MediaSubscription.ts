...
export type MediaSubscription = {
   ...
   */
  authenticator?: schema.OrganizationOrSubClass | string;
  /**
 ...
 * schema:MediaSubscription
 *
 * This differs from MediaSubscription because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MediaSubscriptionOrSubClass =
  | MediaSubscription
  ;
 ...
 */
export const MediaSubscriptionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MediaSubscriptionOrSubClassJoiSchema = Joi.alternatives().try([
  MediaSubscriptionJoiSchema,
]);
 ...
 * ```
 */
export function validateMediaSubscription(maybeMediaSubscription: unknown): MediaSubscription | OaValidationError {
  const { value, error } = MediaSubscriptionJoiSchema.validate(maybeMediaSubscription);
  if (error) {
    return new OaValidationError('MediaSubscription', maybeMediaSubscription, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MediaSubscription;
}
