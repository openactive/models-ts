 ...
 * schema:MedicalBusiness
 *
 * This differs from MedicalBusiness because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalBusinessOrSubClass =
  | MedicalBusiness
  | schema.PharmacyOrSubClass
  | schema.OpticianOrSubClass
...
/**
 * schema:MedicalBusiness - Validation schema (w/ JOI)
 */
export const MedicalBusinessJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalBusiness').required(),
 ...
 * schema:MedicalBusiness - Validation schema (w/ JOI)
 *
 * This differs from MedicalBusinessJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalBusinessOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalBusinessJoiSchema,
  Joi.lazy(() => schema.PharmacyOrSubClassJoiSchema),
  Joi.lazy(() => schema.OpticianOrSubClassJoiSchema),
...
/**
 * Runtime validator for schema:MedicalBusiness.
 *
 * If some data has a structure which matches a schema:MedicalBusiness, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalBusiness = MedicalBusiness.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalBusiness instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalBusiness` will have type `OaValidationError`
 *   const error = maybeMedicalBusiness;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalBusiness` will have type `MedicalBusiness`
 * const medicalBusiness = maybeMedicalBusiness;
 * ```
 */
export function validateMedicalBusiness(maybeMedicalBusiness: unknown): MedicalBusiness | OaValidationError {
  const { value, error } = MedicalBusinessJoiSchema.validate(maybeMedicalBusiness);
  if (error) {
    return new OaValidationError('MedicalBusiness', maybeMedicalBusiness, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalBusiness;
}
