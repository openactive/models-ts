...
export type MedicalCause = {
   ...
   */
  causeOf?: schema.MedicalEntityOrSubClass | string;
  /**
 ...
 * schema:MedicalCause
 *
 * This differs from MedicalCause because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalCauseOrSubClass =
  | MedicalCause
  ;
 ...
 */
export const MedicalCauseJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalCauseOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalCauseJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMedicalCause = MedicalCause.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalCause(maybeMedicalCause: unknown): MedicalCause | OaValidationError {
  const { value, error } = MedicalCauseJoiSchema.validate(maybeMedicalCause);
  if (error) {
    return new OaValidationError('MedicalCause', maybeMedicalCause, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalCause;
}
