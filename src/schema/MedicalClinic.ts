 ...
 * schema:MedicalClinic
 *
 * This differs from MedicalClinic because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalClinicOrSubClass =
  | MedicalClinic
  | schema.CovidTestingFacilityOrSubClass
  ;

/**
 * schema:MedicalClinic - Validation schema (w/ JOI)
 */
export const MedicalClinicJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalClinic').required(),
 ...
 * schema:MedicalClinic - Validation schema (w/ JOI)
 *
 * This differs from MedicalClinicJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalClinicOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalClinicJoiSchema,
  Joi.lazy(() => schema.CovidTestingFacilityOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MedicalClinic.
 *
 * If some data has a structure which matches a schema:MedicalClinic, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalClinic = validateMedicalClinic(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalClinic instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalClinic` will have type `OaValidationError`
 *   const error = maybeMedicalClinic;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalClinic` will have type `MedicalClinic`
 * const medicalClinic = maybeMedicalClinic;
 * ```
 */
export function validateMedicalClinic(maybeMedicalClinic: unknown): MedicalClinic | OaValidationError {
  const { value, error } = MedicalClinicJoiSchema.validate(maybeMedicalClinic);
  if (error) {
    return new OaValidationError('MedicalClinic', maybeMedicalClinic, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalClinic;
}
