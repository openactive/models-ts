...
export type MedicalCode = {
   ...
   */
  inCodeSet?: string | schema.CategoryCodeSetOrSubClass;
  /**
 ...
 * schema:MedicalCode
 *
 * This differs from MedicalCode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalCodeOrSubClass =
  | MedicalCode
  ;
 ...
 */
export const MedicalCodeJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalCodeOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalCodeJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMedicalCode = MedicalCode.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalCode(maybeMedicalCode: unknown): MedicalCode | OaValidationError {
  const { value, error } = MedicalCodeJoiSchema.validate(maybeMedicalCode);
  if (error) {
    return new OaValidationError('MedicalCode', maybeMedicalCode, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalCode;
}
