 ...
 * schema:MedicalCondition
 *
 * This differs from MedicalCondition because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalConditionOrSubClass =
  | MedicalCondition
  | schema.InfectiousDiseaseOrSubClass
  | schema.MedicalSignOrSymptomOrSubClass
  ;

/**
 * schema:MedicalCondition - Validation schema (w/ JOI)
 */
export const MedicalConditionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalConditionOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalConditionJoiSchema,
 ...
 *
 * ```ts
 * const maybeMedicalCondition = validateMedicalCondition(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalCondition(maybeMedicalCondition: unknown): MedicalCondition | OaValidationError {
  const { value, error } = MedicalConditionJoiSchema.validate(maybeMedicalCondition);
  if (error) {
    return new OaValidationError('MedicalCondition', maybeMedicalCondition, error);
  }
