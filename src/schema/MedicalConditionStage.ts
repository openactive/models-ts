 ...
 */
export type MedicalConditionStage = {
 ...
 * schema:MedicalConditionStage
 *
 * This differs from MedicalConditionStage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalConditionStageOrSubClass =
 ...
 */
export const MedicalConditionStageJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalConditionStageOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalConditionStageJoiSchema,
]);
 ...
 * ```
 */
export function validateMedicalConditionStage(maybeMedicalConditionStage: unknown): MedicalConditionStage | OaValidationError {
  const { value, error } = MedicalConditionStageJoiSchema.validate(maybeMedicalConditionStage);
  if (error) {
    return new OaValidationError('MedicalConditionStage', maybeMedicalConditionStage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalConditionStage;
}
