 ...
 * schema:MedicalDevice
 *
 * This differs from MedicalDevice because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalDeviceOrSubClass =
  | MedicalDevice
  ;
 ...
 */
export const MedicalDeviceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalDeviceOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalDeviceJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMedicalDevice = MedicalDevice.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalDevice(maybeMedicalDevice: unknown): MedicalDevice | OaValidationError {
  const { value, error } = MedicalDeviceJoiSchema.validate(maybeMedicalDevice);
  if (error) {
    return new OaValidationError('MedicalDevice', maybeMedicalDevice, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalDevice;
}
