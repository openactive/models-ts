 ...
 * schema:MedicalEntity
 *
 * This differs from MedicalEntity because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalEntityOrSubClass =
  | MedicalEntity
  | schema.MedicalContraindicationOrSubClass
  | schema.MedicalIntangibleOrSubClass
  | schema.AnatomicalStructureOrSubClass
  | schema.MedicalIndicationOrSubClass
  | schema.MedicalProcedureOrSubClass
  | schema.DrugClassOrSubClass
  | schema.MedicalRiskFactorOrSubClass
  | schema.MedicalCauseOrSubClass
  | schema.SubstanceOrSubClass
  | schema.SuperficialAnatomyOrSubClass
  | schema.MedicalRiskEstimatorOrSubClass
  | schema.MedicalConditionOrSubClass
  | schema.AnatomicalSystemOrSubClass
  | schema.MedicalStudyOrSubClass
  | schema.DrugCostOrSubClass
  | schema.MedicalTestOrSubClass
  | schema.MedicalGuidelineOrSubClass
  | schema.LifestyleModificationOrSubClass
  | schema.MedicalDeviceOrSubClass
  ;

/**
 * schema:MedicalEntity - Validation schema (w/ JOI)
 */
export const MedicalEntityJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalEntityOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalEntityJoiSchema,
 ...
 *
 * ```ts
 * const maybeMedicalEntity = MedicalEntity.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalEntity(maybeMedicalEntity: unknown): MedicalEntity | OaValidationError {
  const { value, error } = MedicalEntityJoiSchema.validate(maybeMedicalEntity);
  if (error) {
    return new OaValidationError('MedicalEntity', maybeMedicalEntity, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalEntity;
}
