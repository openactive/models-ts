 ...
 * schema:MedicalGuideline
 *
 * This differs from MedicalGuideline because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalGuidelineOrSubClass =
  | MedicalGuideline
  | schema.MedicalGuidelineRecommendationOrSubClass
  | schema.MedicalGuidelineContraindicationOrSubClass
  ;

/**
 * schema:MedicalGuideline - Validation schema (w/ JOI)
 */
export const MedicalGuidelineJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalGuideline').required(),
 ...
 * schema:MedicalGuideline - Validation schema (w/ JOI)
 *
 * This differs from MedicalGuidelineJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalGuidelineOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalGuidelineJoiSchema,
  Joi.lazy(() => schema.MedicalGuidelineRecommendationOrSubClassJoiSchema),
  Joi.lazy(() => schema.MedicalGuidelineContraindicationOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MedicalGuideline.
 *
 * If some data has a structure which matches a schema:MedicalGuideline, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalGuideline = MedicalGuideline.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalGuideline instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalGuideline` will have type `OaValidationError`
 *   const error = maybeMedicalGuideline;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalGuideline` will have type `MedicalGuideline`
 * const medicalGuideline = maybeMedicalGuideline;
 * ```
 */
export function validateMedicalGuideline(maybeMedicalGuideline: unknown): MedicalGuideline | OaValidationError {
  const { value, error } = MedicalGuidelineJoiSchema.validate(maybeMedicalGuideline);
  if (error) {
    return new OaValidationError('MedicalGuideline', maybeMedicalGuideline, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalGuideline;
}
