 ...
 * schema:MedicalOrganization
 *
 * This differs from MedicalOrganization because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalOrganizationOrSubClass =
  | MedicalOrganization
  | schema.PharmacyOrSubClass
  | schema.HospitalOrSubClass
  | schema.DiagnosticLabOrSubClass
  | schema.DentistOrSubClass
  | schema.MedicalClinicOrSubClass
  | schema.VeterinaryCareOrSubClass
  | schema.PhysicianOrSubClass
  ;

/**
 * schema:MedicalOrganization - Validation schema (w/ JOI)
 */
export const MedicalOrganizationJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalOrganization').required(),
 ...
 * schema:MedicalOrganization - Validation schema (w/ JOI)
 *
 * This differs from MedicalOrganizationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalOrganizationJoiSchema,
  Joi.lazy(() => schema.PharmacyOrSubClassJoiSchema),
  Joi.lazy(() => schema.HospitalOrSubClassJoiSchema),
  Joi.lazy(() => schema.DiagnosticLabOrSubClassJoiSchema),
  Joi.lazy(() => schema.DentistOrSubClassJoiSchema),
  Joi.lazy(() => schema.MedicalClinicOrSubClassJoiSchema),
  Joi.lazy(() => schema.VeterinaryCareOrSubClassJoiSchema),
  Joi.lazy(() => schema.PhysicianOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MedicalOrganization.
 *
 * If some data has a structure which matches a schema:MedicalOrganization, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalOrganization = MedicalOrganization.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalOrganization instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalOrganization` will have type `OaValidationError`
 *   const error = maybeMedicalOrganization;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalOrganization` will have type `MedicalOrganization`
 * const medicalOrganization = maybeMedicalOrganization;
 * ```
 */
export function validateMedicalOrganization(maybeMedicalOrganization: unknown): MedicalOrganization | OaValidationError {
  const { value, error } = MedicalOrganizationJoiSchema.validate(maybeMedicalOrganization);
  if (error) {
    return new OaValidationError('MedicalOrganization', maybeMedicalOrganization, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalOrganization;
}
