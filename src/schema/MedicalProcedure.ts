 ...
 * schema:MedicalProcedure
 *
 * This differs from MedicalProcedure because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalProcedureOrSubClass =
  | MedicalProcedure
  | schema.PhysicalExamOrSubClass
  | schema.TherapeuticProcedureOrSubClass
  | schema.SurgicalProcedureOrSubClass
  | schema.DiagnosticProcedureOrSubClass
 ...
 */
export const MedicalProcedureJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalProcedureOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalProcedureJoiSchema,
 ...
 *
 * ```ts
 * const maybeMedicalProcedure = MedicalProcedure.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalProcedure(maybeMedicalProcedure: unknown): MedicalProcedure | OaValidationError {
  const { value, error } = MedicalProcedureJoiSchema.validate(maybeMedicalProcedure);
  if (error) {
    return new OaValidationError('MedicalProcedure', maybeMedicalProcedure, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalProcedure;
}
