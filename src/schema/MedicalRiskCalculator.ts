 ...
 */
export type MedicalRiskCalculator = {
 ...
 * etc.
 */
export type MedicalRiskCalculatorOrSubClass =
 ...
 */
export const MedicalRiskCalculatorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalRiskCalculatorOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalRiskCalculatorJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMedicalRiskCalculator = MedicalRiskCalculator.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalRiskCalculator(maybeMedicalRiskCalculator: unknown): MedicalRiskCalculator | OaValidationError {
  const { value, error } = MedicalRiskCalculatorJoiSchema.validate(maybeMedicalRiskCalculator);
  if (error) {
    return new OaValidationError('MedicalRiskCalculator', maybeMedicalRiskCalculator, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalRiskCalculator;
}
