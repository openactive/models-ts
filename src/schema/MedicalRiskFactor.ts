 ...
 * etc.
 */
export type MedicalRiskFactorOrSubClass =
  | MedicalRiskFactor
  ;
 ...
 */
export const MedicalRiskFactorJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalRiskFactorOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalRiskFactorJoiSchema,
]);
 ...
 * ```
 */
export function validateMedicalRiskFactor(maybeMedicalRiskFactor: unknown): MedicalRiskFactor | OaValidationError {
  const { value, error } = MedicalRiskFactorJoiSchema.validate(maybeMedicalRiskFactor);
  if (error) {
    return new OaValidationError('MedicalRiskFactor', maybeMedicalRiskFactor, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalRiskFactor;
}
