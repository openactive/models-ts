 ...
 * schema:MedicalRiskScore
 *
 * This differs from MedicalRiskScore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalRiskScoreOrSubClass =
  | MedicalRiskScore
  ;

/**
 * schema:MedicalRiskScore - Validation schema (w/ JOI)
 */
export const MedicalRiskScoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalRiskScore').required(),
 ...
 * schema:MedicalRiskScore - Validation schema (w/ JOI)
 *
 * This differs from MedicalRiskScoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalRiskScoreOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalRiskScoreJoiSchema,
]);

/**
 * Runtime validator for schema:MedicalRiskScore.
 *
 * If some data has a structure which matches a schema:MedicalRiskScore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalRiskScore = MedicalRiskScore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalRiskScore instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalRiskScore` will have type `OaValidationError`
 *   const error = maybeMedicalRiskScore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalRiskScore` will have type `MedicalRiskScore`
 * const medicalRiskScore = maybeMedicalRiskScore;
 * ```
 */
export function validateMedicalRiskScore(maybeMedicalRiskScore: unknown): MedicalRiskScore | OaValidationError {
  const { value, error } = MedicalRiskScoreJoiSchema.validate(maybeMedicalRiskScore);
  if (error) {
    return new OaValidationError('MedicalRiskScore', maybeMedicalRiskScore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalRiskScore;
}
