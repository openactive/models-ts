 ...
 */
export type MedicalScholarlyArticle = {
 ...
 * schema:MedicalScholarlyArticle
 *
 * This differs from MedicalScholarlyArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalScholarlyArticleOrSubClass =
  | MedicalScholarlyArticle
  ;

/**
 * schema:MedicalScholarlyArticle - Validation schema (w/ JOI)
 */
export const MedicalScholarlyArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalScholarlyArticle').required(),
 ...
 * schema:MedicalScholarlyArticle - Validation schema (w/ JOI)
 *
 * This differs from MedicalScholarlyArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalScholarlyArticleOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalScholarlyArticleJoiSchema,
]);

/**
 * Runtime validator for schema:MedicalScholarlyArticle.
 *
 * If some data has a structure which matches a schema:MedicalScholarlyArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalScholarlyArticle = MedicalScholarlyArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalScholarlyArticle instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalScholarlyArticle` will have type `OaValidationError`
 *   const error = maybeMedicalScholarlyArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalScholarlyArticle` will have type `MedicalScholarlyArticle`
 * const medicalScholarlyArticle = maybeMedicalScholarlyArticle;
 * ```
 */
export function validateMedicalScholarlyArticle(maybeMedicalScholarlyArticle: unknown): MedicalScholarlyArticle | OaValidationError {
  const { value, error } = MedicalScholarlyArticleJoiSchema.validate(maybeMedicalScholarlyArticle);
  if (error) {
    return new OaValidationError('MedicalScholarlyArticle', maybeMedicalScholarlyArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalScholarlyArticle;
}
