 ...
 * schema:MedicalSign
 *
 * This differs from MedicalSign because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalSignOrSubClass =
  | MedicalSign
  | schema.VitalSignOrSubClass
  ;

/**
 * schema:MedicalSign - Validation schema (w/ JOI)
 */
export const MedicalSignJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalSign').required(),
 ...
 * schema:MedicalSign - Validation schema (w/ JOI)
 *
 * This differs from MedicalSignJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalSignOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalSignJoiSchema,
  Joi.lazy(() => schema.VitalSignOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MedicalSign.
 *
 * If some data has a structure which matches a schema:MedicalSign, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalSign = MedicalSign.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalSign instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalSign` will have type `OaValidationError`
 *   const error = maybeMedicalSign;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalSign` will have type `MedicalSign`
 * const medicalSign = maybeMedicalSign;
 * ```
 */
export function validateMedicalSign(maybeMedicalSign: unknown): MedicalSign | OaValidationError {
  const { value, error } = MedicalSignJoiSchema.validate(maybeMedicalSign);
  if (error) {
    return new OaValidationError('MedicalSign', maybeMedicalSign, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalSign;
}
