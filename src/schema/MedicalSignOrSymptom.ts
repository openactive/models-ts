 ...
 * etc.
 */
export type MedicalSignOrSymptomOrSubClass =
  | MedicalSignOrSymptom
  | schema.MedicalSymptomOrSubClass
  | schema.MedicalSignOrSubClass
  ;

/**
 * schema:MedicalSignOrSymptom - Validation schema (w/ JOI)
 */
export const MedicalSignOrSymptomJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalSignOrSymptomOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalSignOrSymptomJoiSchema,
 ...
 * ```
 */
export function validateMedicalSignOrSymptom(maybeMedicalSignOrSymptom: unknown): MedicalSignOrSymptom | OaValidationError {
  const { value, error } = MedicalSignOrSymptomJoiSchema.validate(maybeMedicalSignOrSymptom);
  if (error) {
    return new OaValidationError('MedicalSignOrSymptom', maybeMedicalSignOrSymptom, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalSignOrSymptom;
}
