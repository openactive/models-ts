 ...
 * schema:MedicalStudy
 *
 * This differs from MedicalStudy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalStudyOrSubClass =
  | MedicalStudy
  | schema.MedicalObservationalStudyOrSubClass
  | schema.MedicalTrialOrSubClass
  ;

/**
 * schema:MedicalStudy - Validation schema (w/ JOI)
 */
export const MedicalStudyJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalStudyOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalStudyJoiSchema,
 ...
 *
 * ```ts
 * const maybeMedicalStudy = MedicalStudy.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalStudy(maybeMedicalStudy: unknown): MedicalStudy | OaValidationError {
  const { value, error } = MedicalStudyJoiSchema.validate(maybeMedicalStudy);
  if (error) {
    return new OaValidationError('MedicalStudy', maybeMedicalStudy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalStudy;
}
