 ...
 * schema:MedicalSymptom
 *
 * This differs from MedicalSymptom because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalSymptomOrSubClass =
  | MedicalSymptom
  ;

/**
 * schema:MedicalSymptom - Validation schema (w/ JOI)
 */
export const MedicalSymptomJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalSymptom').required(),
 ...
 * schema:MedicalSymptom - Validation schema (w/ JOI)
 *
 * This differs from MedicalSymptomJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalSymptomOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalSymptomJoiSchema,
]);

/**
 * Runtime validator for schema:MedicalSymptom.
 *
 * If some data has a structure which matches a schema:MedicalSymptom, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalSymptom = MedicalSymptom.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalSymptom instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalSymptom` will have type `OaValidationError`
 *   const error = maybeMedicalSymptom;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalSymptom` will have type `MedicalSymptom`
 * const medicalSymptom = maybeMedicalSymptom;
 * ```
 */
export function validateMedicalSymptom(maybeMedicalSymptom: unknown): MedicalSymptom | OaValidationError {
  const { value, error } = MedicalSymptomJoiSchema.validate(maybeMedicalSymptom);
  if (error) {
    return new OaValidationError('MedicalSymptom', maybeMedicalSymptom, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalSymptom;
}
