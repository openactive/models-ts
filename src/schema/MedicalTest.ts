 ...
 * schema:MedicalTest
 *
 * This differs from MedicalTest because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalTestOrSubClass =
  | MedicalTest
  | schema.PathologyTestOrSubClass
  | schema.MedicalTestPanelOrSubClass
  | schema.BloodTestOrSubClass
  | schema.ImagingTestOrSubClass
  ;

/**
 * schema:MedicalTest - Validation schema (w/ JOI)
 */
export const MedicalTestJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalTest').required(),
 ...
 * schema:MedicalTest - Validation schema (w/ JOI)
 *
 * This differs from MedicalTestJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalTestOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalTestJoiSchema,
  Joi.lazy(() => schema.PathologyTestOrSubClassJoiSchema),
  Joi.lazy(() => schema.MedicalTestPanelOrSubClassJoiSchema),
  Joi.lazy(() => schema.BloodTestOrSubClassJoiSchema),
  Joi.lazy(() => schema.ImagingTestOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MedicalTest.
 *
 * If some data has a structure which matches a schema:MedicalTest, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMedicalTest = MedicalTest.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMedicalTest instanceof OaValidationError) {
 *   // From this point on, `maybeMedicalTest` will have type `OaValidationError`
 *   const error = maybeMedicalTest;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMedicalTest` will have type `MedicalTest`
 * const medicalTest = maybeMedicalTest;
 * ```
 */
export function validateMedicalTest(maybeMedicalTest: unknown): MedicalTest | OaValidationError {
  const { value, error } = MedicalTestJoiSchema.validate(maybeMedicalTest);
  if (error) {
    return new OaValidationError('MedicalTest', maybeMedicalTest, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalTest;
}
