...
export type MedicalTrial = {
   ...
   */
  trialDesign?: schema.MedicalTrialDesignOrSubClass | string;
  /**
 ...
 * schema:MedicalTrial
 *
 * This differs from MedicalTrial because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalTrialOrSubClass =
  | MedicalTrial
  ;
 ...
 */
export const MedicalTrialJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MedicalTrialOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalTrialJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMedicalTrial = MedicalTrial.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMedicalTrial(maybeMedicalTrial: unknown): MedicalTrial | OaValidationError {
  const { value, error } = MedicalTrialJoiSchema.validate(maybeMedicalTrial);
  if (error) {
    return new OaValidationError('MedicalTrial', maybeMedicalTrial, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalTrial;
}
