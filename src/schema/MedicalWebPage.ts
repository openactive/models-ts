...
export type MedicalWebPage = {
   ...
   */
  medicalAudience?: schema.MedicalAudienceTypeOrSubClass | schema.MedicalAudienceOrSubClass | string;
  /**
 ...
 * schema:MedicalWebPage
 *
 * This differs from MedicalWebPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MedicalWebPageOrSubClass =
  | MedicalWebPage
  ;

/**
 * schema:MedicalWebPage - Validation schema (w/ JOI)
 */
export const MedicalWebPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('MedicalWebPage').required(),
  ...
  '@id': Joi.string().uri(),
  aspect: Joi.string(),
  medicalAudience: Joi.alternatives().try(Joi.lazy(() => schema.MedicalAudienceTypeOrSubClassJoiSchema), Joi.lazy(() => schema.MedicalAudienceOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:MedicalWebPage - Validation schema (w/ JOI)
 *
 * This differs from MedicalWebPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MedicalWebPageOrSubClassJoiSchema = Joi.alternatives().try([
  MedicalWebPageJoiSchema,
]);

/**
 * Runtime validator for schema:MedicalWebPage.
 *
 * If some data has a structure which matches a schema:MedicalWebPage, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMedicalWebPage` will have type `MedicalWebPage`
 * const medicalWebPage = maybeMedicalWebPage;
 * ```
 */
export function validateMedicalWebPage(maybeMedicalWebPage: unknown): MedicalWebPage | OaValidationError {
  const { value, error } = MedicalWebPageJoiSchema.validate(maybeMedicalWebPage);
  if (error) {
    return new OaValidationError('MedicalWebPage', maybeMedicalWebPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MedicalWebPage;
}
