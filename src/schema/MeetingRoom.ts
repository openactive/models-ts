 ...
 * schema:MeetingRoom
 *
 * This differs from MeetingRoom because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MeetingRoomOrSubClass =
  | MeetingRoom
  ;

/**
 * schema:MeetingRoom - Validation schema (w/ JOI)
 */
export const MeetingRoomJoiSchema = Joi.object({
  '@type': Joi.string().valid('MeetingRoom').required(),
 ...
 * schema:MeetingRoom - Validation schema (w/ JOI)
 *
 * This differs from MeetingRoomJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MeetingRoomOrSubClassJoiSchema = Joi.alternatives().try([
  MeetingRoomJoiSchema,
]);

/**
 * Runtime validator for schema:MeetingRoom.
 *
 * If some data has a structure which matches a schema:MeetingRoom, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMeetingRoom = validateMeetingRoom(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMeetingRoom instanceof OaValidationError) {
 *   // From this point on, `maybeMeetingRoom` will have type `OaValidationError`
 *   const error = maybeMeetingRoom;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMeetingRoom` will have type `MeetingRoom`
 * const meetingRoom = maybeMeetingRoom;
 * ```
 */
export function validateMeetingRoom(maybeMeetingRoom: unknown): MeetingRoom | OaValidationError {
  const { value, error } = MeetingRoomJoiSchema.validate(maybeMeetingRoom);
  if (error) {
    return new OaValidationError('MeetingRoom', maybeMeetingRoom, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MeetingRoom;
}
