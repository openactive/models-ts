 ...
 * schema:MensClothingStore
 *
 * This differs from MensClothingStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MensClothingStoreOrSubClass =
  | MensClothingStore
  ;

/**
 * schema:MensClothingStore - Validation schema (w/ JOI)
 */
export const MensClothingStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('MensClothingStore').required(),
 ...
 * schema:MensClothingStore - Validation schema (w/ JOI)
 *
 * This differs from MensClothingStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MensClothingStoreOrSubClassJoiSchema = Joi.alternatives().try([
  MensClothingStoreJoiSchema,
]);

/**
 * Runtime validator for schema:MensClothingStore.
 *
 * If some data has a structure which matches a schema:MensClothingStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMensClothingStore = MensClothingStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMensClothingStore instanceof OaValidationError) {
 *   // From this point on, `maybeMensClothingStore` will have type `OaValidationError`
 *   const error = maybeMensClothingStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMensClothingStore` will have type `MensClothingStore`
 * const mensClothingStore = maybeMensClothingStore;
 * ```
 */
export function validateMensClothingStore(maybeMensClothingStore: unknown): MensClothingStore | OaValidationError {
  const { value, error } = MensClothingStoreJoiSchema.validate(maybeMensClothingStore);
  if (error) {
    return new OaValidationError('MensClothingStore', maybeMensClothingStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MensClothingStore;
}
