...
export type Menu = {
   ...
   */
  hasMenuItem?: schema.MenuItemOrSubClass | string;
  /**
   * A subgrouping of the menu (by dishes, course, serving time period, etc.).
   */
  hasMenuSection?: schema.MenuSectionOrSubClass | string;
  /**
 ...
 * schema:Menu
 *
 * This differs from Menu because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MenuOrSubClass =
  | Menu
  ;

/**
 * schema:Menu - Validation schema (w/ JOI)
 */
export const MenuJoiSchema = Joi.object({
  '@type': Joi.string().valid('Menu').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  hasMenuItem: Joi.alternatives().try(Joi.lazy(() => schema.MenuItemOrSubClassJoiSchema), Joi.string().uri()),
  hasMenuSection: Joi.alternatives().try(Joi.lazy(() => schema.MenuSectionOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:Menu - Validation schema (w/ JOI)
 *
 * This differs from MenuJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MenuOrSubClassJoiSchema = Joi.alternatives().try([
  MenuJoiSchema,
]);

/**
 * Runtime validator for schema:Menu.
 *
 * If some data has a structure which matches a schema:Menu, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMenu = Menu.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMenu instanceof OaValidationError) {
 *   // From this point on, `maybeMenu` will have type `OaValidationError`
 *   const error = maybeMenu;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMenu` will have type `Menu`
 * const menu = maybeMenu;
 * ```
 */
export function validateMenu(maybeMenu: unknown): Menu | OaValidationError {
  const { value, error } = MenuJoiSchema.validate(maybeMenu);
  if (error) {
    return new OaValidationError('Menu', maybeMenu, error);
  }
