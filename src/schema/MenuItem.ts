 ...
 * etc.
 */
export type MenuItemOrSubClass =
  | MenuItem
  ;
 ...
 */
export const MenuItemJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MenuItemOrSubClassJoiSchema = Joi.alternatives().try([
  MenuItemJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMenuItem = MenuItem.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMenuItem(maybeMenuItem: unknown): MenuItem | OaValidationError {
  const { value, error } = MenuItemJoiSchema.validate(maybeMenuItem);
  if (error) {
    return new OaValidationError('MenuItem', maybeMenuItem, error);
  }
