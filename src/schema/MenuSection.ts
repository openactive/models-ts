 ...
 * schema:MenuSection
 *
 * This differs from MenuSection because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MenuSectionOrSubClass =
  | MenuSection
  ;

/**
 * schema:MenuSection - Validation schema (w/ JOI)
 */
export const MenuSectionJoiSchema = Joi.object({
  '@type': Joi.string().valid('MenuSection').required(),
 ...
 * schema:MenuSection - Validation schema (w/ JOI)
 *
 * This differs from MenuSectionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MenuSectionOrSubClassJoiSchema = Joi.alternatives().try([
  MenuSectionJoiSchema,
]);

/**
 * Runtime validator for schema:MenuSection.
 *
 * If some data has a structure which matches a schema:MenuSection, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMenuSection` will have type `MenuSection`
 * const menuSection = maybeMenuSection;
 * ```
 */
export function validateMenuSection(maybeMenuSection: unknown): MenuSection | OaValidationError {
  const { value, error } = MenuSectionJoiSchema.validate(maybeMenuSection);
  if (error) {
    return new OaValidationError('MenuSection', maybeMenuSection, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MenuSection;
}
