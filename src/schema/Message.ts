 ...
 * schema:Message
 *
 * This differs from Message because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MessageOrSubClass =
  | Message
  | schema.EmailMessageOrSubClass
  ;

/**
 * schema:Message - Validation schema (w/ JOI)
 */
 ...
 * schema:Message - Validation schema (w/ JOI)
 *
 * This differs from MessageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MessageOrSubClassJoiSchema = Joi.alternatives().try([
  MessageJoiSchema,
  Joi.lazy(() => schema.EmailMessageOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Message.
 *
 * If some data has a structure which matches a schema:Message, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMessage = Message.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMessage instanceof OaValidationError) {
 *   // From this point on, `maybeMessage` will have type `OaValidationError`
 *   const error = maybeMessage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMessage` will have type `Message`
 * const message = maybeMessage;
 * ```
 */
export function validateMessage(maybeMessage: unknown): Message | OaValidationError {
  const { value, error } = MessageJoiSchema.validate(maybeMessage);
  if (error) {
    return new OaValidationError('Message', maybeMessage, error);
  }
