 ...
 * schema:MiddleSchool
 *
 * This differs from MiddleSchool because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MiddleSchoolOrSubClass =
  | MiddleSchool
  ;
 ...
 */
export const MiddleSchoolJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MiddleSchoolOrSubClassJoiSchema = Joi.alternatives().try([
  MiddleSchoolJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMiddleSchool = MiddleSchool.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMiddleSchool(maybeMiddleSchool: unknown): MiddleSchool | OaValidationError {
  const { value, error } = MiddleSchoolJoiSchema.validate(maybeMiddleSchool);
  if (error) {
    return new OaValidationError('MiddleSchool', maybeMiddleSchool, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MiddleSchool;
}
