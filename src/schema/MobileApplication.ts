 ...
 * schema:MobileApplication
 *
 * This differs from MobileApplication because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MobileApplicationOrSubClass =
  | MobileApplication
  ;

/**
 * schema:MobileApplication - Validation schema (w/ JOI)
 */
export const MobileApplicationJoiSchema = Joi.object({
  '@type': Joi.string().valid('MobileApplication').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  carrierRequirements: Joi.string(),
 ...
 * schema:MobileApplication - Validation schema (w/ JOI)
 *
 * This differs from MobileApplicationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MobileApplicationOrSubClassJoiSchema = Joi.alternatives().try([
  MobileApplicationJoiSchema,
]);

/**
 * Runtime validator for schema:MobileApplication.
 *
 * If some data has a structure which matches a schema:MobileApplication, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMobileApplication = validateMobileApplication(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMobileApplication instanceof OaValidationError) {
 *   // From this point on, `maybeMobileApplication` will have type `OaValidationError`
 *   const error = maybeMobileApplication;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMobileApplication` will have type `MobileApplication`
 * const mobileApplication = maybeMobileApplication;
 * ```
 */
export function validateMobileApplication(maybeMobileApplication: unknown): MobileApplication | OaValidationError {
  const { value, error } = MobileApplicationJoiSchema.validate(maybeMobileApplication);
  if (error) {
    return new OaValidationError('MobileApplication', maybeMobileApplication, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MobileApplication;
}
