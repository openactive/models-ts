 ...
 * schema:MobilePhoneStore
 *
 * This differs from MobilePhoneStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MobilePhoneStoreOrSubClass =
  | MobilePhoneStore
  ;

/**
 * schema:MobilePhoneStore - Validation schema (w/ JOI)
 */
export const MobilePhoneStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('MobilePhoneStore').required(),
 ...
 * schema:MobilePhoneStore - Validation schema (w/ JOI)
 *
 * This differs from MobilePhoneStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MobilePhoneStoreOrSubClassJoiSchema = Joi.alternatives().try([
  MobilePhoneStoreJoiSchema,
]);

/**
 * Runtime validator for schema:MobilePhoneStore.
 *
 * If some data has a structure which matches a schema:MobilePhoneStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMobilePhoneStore = MobilePhoneStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMobilePhoneStore instanceof OaValidationError) {
 *   // From this point on, `maybeMobilePhoneStore` will have type `OaValidationError`
 *   const error = maybeMobilePhoneStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMobilePhoneStore` will have type `MobilePhoneStore`
 * const mobilePhoneStore = maybeMobilePhoneStore;
 * ```
 */
export function validateMobilePhoneStore(maybeMobilePhoneStore: unknown): MobilePhoneStore | OaValidationError {
  const { value, error } = MobilePhoneStoreJoiSchema.validate(maybeMobilePhoneStore);
  if (error) {
    return new OaValidationError('MobilePhoneStore', maybeMobilePhoneStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MobilePhoneStore;
}
