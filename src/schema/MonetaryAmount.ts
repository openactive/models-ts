 ...
 * schema:MonetaryAmount
 *
 * This differs from MonetaryAmount because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MonetaryAmountOrSubClass =
  | MonetaryAmount
  ;
 ...
 */
export const MonetaryAmountJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MonetaryAmountOrSubClassJoiSchema = Joi.alternatives().try([
  MonetaryAmountJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMonetaryAmount = MonetaryAmount.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMonetaryAmount(maybeMonetaryAmount: unknown): MonetaryAmount | OaValidationError {
  const { value, error } = MonetaryAmountJoiSchema.validate(maybeMonetaryAmount);
  if (error) {
    return new OaValidationError('MonetaryAmount', maybeMonetaryAmount, error);
  }
