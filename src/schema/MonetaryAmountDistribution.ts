 ...
 */
export type MonetaryAmountDistribution = {
 ...
 * schema:MonetaryAmountDistribution
 *
 * This differs from MonetaryAmountDistribution because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MonetaryAmountDistributionOrSubClass =
 ...
 */
export const MonetaryAmountDistributionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MonetaryAmountDistributionOrSubClassJoiSchema = Joi.alternatives().try([
  MonetaryAmountDistributionJoiSchema,
]);
 ...
 * ```
 */
export function validateMonetaryAmountDistribution(maybeMonetaryAmountDistribution: unknown): MonetaryAmountDistribution | OaValidationError {
  const { value, error } = MonetaryAmountDistributionJoiSchema.validate(maybeMonetaryAmountDistribution);
  if (error) {
    return new OaValidationError('MonetaryAmountDistribution', maybeMonetaryAmountDistribution, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MonetaryAmountDistribution;
}
