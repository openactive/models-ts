 ...
 * schema:MonetaryGrant
 *
 * This differs from MonetaryGrant because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MonetaryGrantOrSubClass =
  | MonetaryGrant
  ;
 ...
 */
export const MonetaryGrantJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MonetaryGrantOrSubClassJoiSchema = Joi.alternatives().try([
  MonetaryGrantJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMonetaryGrant = MonetaryGrant.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMonetaryGrant(maybeMonetaryGrant: unknown): MonetaryGrant | OaValidationError {
  const { value, error } = MonetaryGrantJoiSchema.validate(maybeMonetaryGrant);
  if (error) {
    return new OaValidationError('MonetaryGrant', maybeMonetaryGrant, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MonetaryGrant;
}
