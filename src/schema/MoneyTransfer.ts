...
export type MoneyTransfer = {
   ...
   */
  beneficiaryBank?: schema.BankOrCreditUnionOrSubClass | string;
  /**
...
export const MoneyTransferJoiSchema = Joi.object({
  ...
  beneficiaryBank: Joi.alternatives().try(Joi.lazy(() => schema.BankOrCreditUnionOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
 ...
 *
 * ```ts
 * const maybeMoneyTransfer = validateMoneyTransfer(req.body); // `req.body` will have type `any` or `unknown`
