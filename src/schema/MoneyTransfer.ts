...
export type MoneyTransfer = {
   ...
   */
  beneficiaryBank?: string | schema.BankOrCreditUnionOrSubClass;
  /**
   * The amount of money.
   */
 ...
 * schema:MoneyTransfer
 *
 * This differs from MoneyTransfer because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MoneyTransferOrSubClass =
  | MoneyTransfer
  ;

/**
 * schema:MoneyTransfer - Validation schema (w/ JOI)
 */
export const MoneyTransferJoiSchema = Joi.object({
  '@type': Joi.string().valid('MoneyTransfer').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  beneficiaryBank: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.BankOrCreditUnionOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:MoneyTransfer - Validation schema (w/ JOI)
 *
 * This differs from MoneyTransferJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MoneyTransferOrSubClassJoiSchema = Joi.alternatives().try([
  MoneyTransferJoiSchema,
]);

/**
 * Runtime validator for schema:MoneyTransfer.
 *
 * If some data has a structure which matches a schema:MoneyTransfer, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMoneyTransfer = MoneyTransfer.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMoneyTransfer instanceof OaValidationError) {
 *   // From this point on, `maybeMoneyTransfer` will have type `OaValidationError`
 *   const error = maybeMoneyTransfer;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMoneyTransfer` will have type `MoneyTransfer`
 * const moneyTransfer = maybeMoneyTransfer;
 * ```
 */
export function validateMoneyTransfer(maybeMoneyTransfer: unknown): MoneyTransfer | OaValidationError {
  const { value, error } = MoneyTransferJoiSchema.validate(maybeMoneyTransfer);
  if (error) {
    return new OaValidationError('MoneyTransfer', maybeMoneyTransfer, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MoneyTransfer;
}
