 ...
 * schema:Mosque
 *
 * This differs from Mosque because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MosqueOrSubClass =
  | Mosque
  ;

/**
 * schema:Mosque - Validation schema (w/ JOI)
 */
export const MosqueJoiSchema = Joi.object({
  '@type': Joi.string().valid('Mosque').required(),
 ...
 * schema:Mosque - Validation schema (w/ JOI)
 *
 * This differs from MosqueJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MosqueOrSubClassJoiSchema = Joi.alternatives().try([
  MosqueJoiSchema,
]);

/**
 * Runtime validator for schema:Mosque.
 *
 * If some data has a structure which matches a schema:Mosque, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMosque = Mosque.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMosque instanceof OaValidationError) {
 *   // From this point on, `maybeMosque` will have type `OaValidationError`
 *   const error = maybeMosque;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMosque` will have type `Mosque`
 * const mosque = maybeMosque;
 * ```
 */
export function validateMosque(maybeMosque: unknown): Mosque | OaValidationError {
  const { value, error } = MosqueJoiSchema.validate(maybeMosque);
  if (error) {
    return new OaValidationError('Mosque', maybeMosque, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Mosque;
}
