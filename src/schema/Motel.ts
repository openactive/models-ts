 ...
 * schema:Motel
 *
 * This differs from Motel because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MotelOrSubClass =
  | Motel
  ;

/**
 * schema:Motel - Validation schema (w/ JOI)
 */
export const MotelJoiSchema = Joi.object({
  '@type': Joi.string().valid('Motel').required(),
 ...
 * schema:Motel - Validation schema (w/ JOI)
 *
 * This differs from MotelJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MotelOrSubClassJoiSchema = Joi.alternatives().try([
  MotelJoiSchema,
]);

/**
 * Runtime validator for schema:Motel.
 *
 * If some data has a structure which matches a schema:Motel, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMotel = validateMotel(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMotel instanceof OaValidationError) {
 *   // From this point on, `maybeMotel` will have type `OaValidationError`
 *   const error = maybeMotel;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMotel` will have type `Motel`
 * const motel = maybeMotel;
 * ```
 */
export function validateMotel(maybeMotel: unknown): Motel | OaValidationError {
  const { value, error } = MotelJoiSchema.validate(maybeMotel);
  if (error) {
    return new OaValidationError('Motel', maybeMotel, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Motel;
}
