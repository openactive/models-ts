 ...
 * schema:Motorcycle
 *
 * This differs from Motorcycle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MotorcycleOrSubClass =
  | Motorcycle
  ;
 ...
 */
export const MotorcycleJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MotorcycleOrSubClassJoiSchema = Joi.alternatives().try([
  MotorcycleJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeMotorcycle = Motorcycle.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateMotorcycle(maybeMotorcycle: unknown): Motorcycle | OaValidationError {
  const { value, error } = MotorcycleJoiSchema.validate(maybeMotorcycle);
  if (error) {
    return new OaValidationError('Motorcycle', maybeMotorcycle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Motorcycle;
}
