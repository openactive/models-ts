 ...
 * schema:MotorcycleDealer
 *
 * This differs from MotorcycleDealer because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MotorcycleDealerOrSubClass =
  | MotorcycleDealer
  ;

/**
 * schema:MotorcycleDealer - Validation schema (w/ JOI)
 */
export const MotorcycleDealerJoiSchema = Joi.object({
  '@type': Joi.string().valid('MotorcycleDealer').required(),
 ...
 * schema:MotorcycleDealer - Validation schema (w/ JOI)
 *
 * This differs from MotorcycleDealerJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MotorcycleDealerOrSubClassJoiSchema = Joi.alternatives().try([
  MotorcycleDealerJoiSchema,
]);

/**
 * Runtime validator for schema:MotorcycleDealer.
 *
 * If some data has a structure which matches a schema:MotorcycleDealer, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMotorcycleDealer = MotorcycleDealer.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMotorcycleDealer instanceof OaValidationError) {
 *   // From this point on, `maybeMotorcycleDealer` will have type `OaValidationError`
 *   const error = maybeMotorcycleDealer;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMotorcycleDealer` will have type `MotorcycleDealer`
 * const motorcycleDealer = maybeMotorcycleDealer;
 * ```
 */
export function validateMotorcycleDealer(maybeMotorcycleDealer: unknown): MotorcycleDealer | OaValidationError {
  const { value, error } = MotorcycleDealerJoiSchema.validate(maybeMotorcycleDealer);
  if (error) {
    return new OaValidationError('MotorcycleDealer', maybeMotorcycleDealer, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MotorcycleDealer;
}
