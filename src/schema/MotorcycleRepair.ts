 ...
 * schema:MotorcycleRepair
 *
 * This differs from MotorcycleRepair because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MotorcycleRepairOrSubClass =
  | MotorcycleRepair
  ;

/**
 * schema:MotorcycleRepair - Validation schema (w/ JOI)
 */
export const MotorcycleRepairJoiSchema = Joi.object({
  '@type': Joi.string().valid('MotorcycleRepair').required(),
 ...
 * schema:MotorcycleRepair - Validation schema (w/ JOI)
 *
 * This differs from MotorcycleRepairJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MotorcycleRepairOrSubClassJoiSchema = Joi.alternatives().try([
  MotorcycleRepairJoiSchema,
]);

/**
 * Runtime validator for schema:MotorcycleRepair.
 *
 * If some data has a structure which matches a schema:MotorcycleRepair, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMotorcycleRepair = MotorcycleRepair.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMotorcycleRepair instanceof OaValidationError) {
 *   // From this point on, `maybeMotorcycleRepair` will have type `OaValidationError`
 *   const error = maybeMotorcycleRepair;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMotorcycleRepair` will have type `MotorcycleRepair`
 * const motorcycleRepair = maybeMotorcycleRepair;
 * ```
 */
export function validateMotorcycleRepair(maybeMotorcycleRepair: unknown): MotorcycleRepair | OaValidationError {
  const { value, error } = MotorcycleRepairJoiSchema.validate(maybeMotorcycleRepair);
  if (error) {
    return new OaValidationError('MotorcycleRepair', maybeMotorcycleRepair, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MotorcycleRepair;
}
