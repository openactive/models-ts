 ...
 * schema:Mountain
 *
 * This differs from Mountain because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MountainOrSubClass =
  | Mountain
  ;

/**
 * schema:Mountain - Validation schema (w/ JOI)
 */
export const MountainJoiSchema = Joi.object({
  '@type': Joi.string().valid('Mountain').required(),
 ...
 * schema:Mountain - Validation schema (w/ JOI)
 *
 * This differs from MountainJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MountainOrSubClassJoiSchema = Joi.alternatives().try([
  MountainJoiSchema,
]);

/**
 * Runtime validator for schema:Mountain.
 *
 * If some data has a structure which matches a schema:Mountain, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMountain` will have type `Mountain`
 * const mountain = maybeMountain;
 * ```
 */
export function validateMountain(maybeMountain: unknown): Mountain | OaValidationError {
  const { value, error } = MountainJoiSchema.validate(maybeMountain);
  if (error) {
    return new OaValidationError('Mountain', maybeMountain, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Mountain;
}
