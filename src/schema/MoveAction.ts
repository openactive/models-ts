 ...
 * schema:MoveAction
 *
 * This differs from MoveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MoveActionOrSubClass =
  | MoveAction
  | schema.DepartActionOrSubClass
  | schema.ArriveActionOrSubClass
  | schema.TravelActionOrSubClass
  ;

/**
 * schema:MoveAction - Validation schema (w/ JOI)
 */
export const MoveActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('MoveAction').required(),
 ...
 * schema:MoveAction - Validation schema (w/ JOI)
 *
 * This differs from MoveActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MoveActionOrSubClassJoiSchema = Joi.alternatives().try([
  MoveActionJoiSchema,
  Joi.lazy(() => schema.DepartActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.ArriveActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.TravelActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MoveAction.
 *
 * If some data has a structure which matches a schema:MoveAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMoveAction = MoveAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMoveAction instanceof OaValidationError) {
 *   // From this point on, `maybeMoveAction` will have type `OaValidationError`
 *   const error = maybeMoveAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMoveAction` will have type `MoveAction`
 * const moveAction = maybeMoveAction;
 * ```
 */
export function validateMoveAction(maybeMoveAction: unknown): MoveAction | OaValidationError {
  const { value, error } = MoveActionJoiSchema.validate(maybeMoveAction);
  if (error) {
    return new OaValidationError('MoveAction', maybeMoveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MoveAction;
}
