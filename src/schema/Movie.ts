 ...
 * schema:Movie
 *
 * This differs from Movie because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MovieOrSubClass =
  | Movie
  ;

/**
 * schema:Movie - Validation schema (w/ JOI)
 */
 ...
 * schema:Movie - Validation schema (w/ JOI)
 *
 * This differs from MovieJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MovieOrSubClassJoiSchema = Joi.alternatives().try([
  MovieJoiSchema,
]);

/**
 * Runtime validator for schema:Movie.
 *
 * If some data has a structure which matches a schema:Movie, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMovie = Movie.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMovie instanceof OaValidationError) {
 *   // From this point on, `maybeMovie` will have type `OaValidationError`
 *   const error = maybeMovie;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMovie` will have type `Movie`
 * const movie = maybeMovie;
 * ```
 */
export function validateMovie(maybeMovie: unknown): Movie | OaValidationError {
  const { value, error } = MovieJoiSchema.validate(maybeMovie);
  if (error) {
    return new OaValidationError('Movie', maybeMovie, error);
  }
