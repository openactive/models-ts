 ...
 * schema:MovieClip
 *
 * This differs from MovieClip because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MovieClipOrSubClass =
  | MovieClip
  ;

/**
 * schema:MovieClip - Validation schema (w/ JOI)
 */
export const MovieClipJoiSchema = Joi.object({
  '@type': Joi.string().valid('MovieClip').required(),
 ...
 * schema:MovieClip - Validation schema (w/ JOI)
 *
 * This differs from MovieClipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MovieClipOrSubClassJoiSchema = Joi.alternatives().try([
  MovieClipJoiSchema,
]);

/**
 * Runtime validator for schema:MovieClip.
 *
 * If some data has a structure which matches a schema:MovieClip, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMovieClip = MovieClip.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMovieClip instanceof OaValidationError) {
 *   // From this point on, `maybeMovieClip` will have type `OaValidationError`
 *   const error = maybeMovieClip;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMovieClip` will have type `MovieClip`
 * const movieClip = maybeMovieClip;
 * ```
 */
export function validateMovieClip(maybeMovieClip: unknown): MovieClip | OaValidationError {
  const { value, error } = MovieClipJoiSchema.validate(maybeMovieClip);
  if (error) {
    return new OaValidationError('MovieClip', maybeMovieClip, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MovieClip;
}
