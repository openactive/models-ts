 ...
 * schema:MovieRentalStore
 *
 * This differs from MovieRentalStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MovieRentalStoreOrSubClass =
  | MovieRentalStore
  ;

/**
 * schema:MovieRentalStore - Validation schema (w/ JOI)
 */
export const MovieRentalStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('MovieRentalStore').required(),
 ...
 * schema:MovieRentalStore - Validation schema (w/ JOI)
 *
 * This differs from MovieRentalStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MovieRentalStoreOrSubClassJoiSchema = Joi.alternatives().try([
  MovieRentalStoreJoiSchema,
]);

/**
 * Runtime validator for schema:MovieRentalStore.
 *
 * If some data has a structure which matches a schema:MovieRentalStore, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMovieRentalStore` will have type `MovieRentalStore`
 * const movieRentalStore = maybeMovieRentalStore;
 * ```
 */
export function validateMovieRentalStore(maybeMovieRentalStore: unknown): MovieRentalStore | OaValidationError {
  const { value, error } = MovieRentalStoreJoiSchema.validate(maybeMovieRentalStore);
  if (error) {
    return new OaValidationError('MovieRentalStore', maybeMovieRentalStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MovieRentalStore;
}
