 ...
 * schema:MovieSeries
 *
 * This differs from MovieSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MovieSeriesOrSubClass =
  | MovieSeries
  ;

/**
 * schema:MovieSeries - Validation schema (w/ JOI)
 */
export const MovieSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('MovieSeries').required(),
 ...
 * schema:MovieSeries - Validation schema (w/ JOI)
 *
 * This differs from MovieSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MovieSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  MovieSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:MovieSeries.
 *
 * If some data has a structure which matches a schema:MovieSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMovieSeries = MovieSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMovieSeries instanceof OaValidationError) {
 *   // From this point on, `maybeMovieSeries` will have type `OaValidationError`
 *   const error = maybeMovieSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMovieSeries` will have type `MovieSeries`
 * const movieSeries = maybeMovieSeries;
 * ```
 */
export function validateMovieSeries(maybeMovieSeries: unknown): MovieSeries | OaValidationError {
  const { value, error } = MovieSeriesJoiSchema.validate(maybeMovieSeries);
  if (error) {
    return new OaValidationError('MovieSeries', maybeMovieSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MovieSeries;
}
