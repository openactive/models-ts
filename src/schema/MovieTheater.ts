 ...
 * schema:MovieTheater
 *
 * This differs from MovieTheater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MovieTheaterOrSubClass =
  | MovieTheater
  ;

/**
 * schema:MovieTheater - Validation schema (w/ JOI)
 */
export const MovieTheaterJoiSchema = Joi.object({
  '@type': Joi.string().valid('MovieTheater').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  screenCount: Joi.number(),
 ...
 * schema:MovieTheater - Validation schema (w/ JOI)
 *
 * This differs from MovieTheaterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MovieTheaterOrSubClassJoiSchema = Joi.alternatives().try([
  MovieTheaterJoiSchema,
]);

/**
 * Runtime validator for schema:MovieTheater.
 *
 * If some data has a structure which matches a schema:MovieTheater, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMovieTheater = MovieTheater.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMovieTheater instanceof OaValidationError) {
 *   // From this point on, `maybeMovieTheater` will have type `OaValidationError`
 *   const error = maybeMovieTheater;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMovieTheater` will have type `MovieTheater`
 * const movieTheater = maybeMovieTheater;
 * ```
 */
export function validateMovieTheater(maybeMovieTheater: unknown): MovieTheater | OaValidationError {
  const { value, error } = MovieTheaterJoiSchema.validate(maybeMovieTheater);
  if (error) {
    return new OaValidationError('MovieTheater', maybeMovieTheater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MovieTheater;
}
