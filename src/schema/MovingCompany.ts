 ...
 * schema:MovingCompany
 *
 * This differs from MovingCompany because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MovingCompanyOrSubClass =
  | MovingCompany
  ;

/**
 * schema:MovingCompany - Validation schema (w/ JOI)
 */
export const MovingCompanyJoiSchema = Joi.object({
  '@type': Joi.string().valid('MovingCompany').required(),
 ...
 * schema:MovingCompany - Validation schema (w/ JOI)
 *
 * This differs from MovingCompanyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MovingCompanyOrSubClassJoiSchema = Joi.alternatives().try([
  MovingCompanyJoiSchema,
]);

/**
 * Runtime validator for schema:MovingCompany.
 *
 * If some data has a structure which matches a schema:MovingCompany, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMovingCompany = MovingCompany.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMovingCompany instanceof OaValidationError) {
 *   // From this point on, `maybeMovingCompany` will have type `OaValidationError`
 *   const error = maybeMovingCompany;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMovingCompany` will have type `MovingCompany`
 * const movingCompany = maybeMovingCompany;
 * ```
 */
export function validateMovingCompany(maybeMovingCompany: unknown): MovingCompany | OaValidationError {
  const { value, error } = MovingCompanyJoiSchema.validate(maybeMovingCompany);
  if (error) {
    return new OaValidationError('MovingCompany', maybeMovingCompany, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MovingCompany;
}
