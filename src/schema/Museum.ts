 ...
 * schema:Museum
 *
 * This differs from Museum because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MuseumOrSubClass =
  | Museum
  ;

/**
 * schema:Museum - Validation schema (w/ JOI)
 */
export const MuseumJoiSchema = Joi.object({
  '@type': Joi.string().valid('Museum').required(),
 ...
 * schema:Museum - Validation schema (w/ JOI)
 *
 * This differs from MuseumJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MuseumOrSubClassJoiSchema = Joi.alternatives().try([
  MuseumJoiSchema,
]);

/**
 * Runtime validator for schema:Museum.
 *
 * If some data has a structure which matches a schema:Museum, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMuseum = Museum.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMuseum instanceof OaValidationError) {
 *   // From this point on, `maybeMuseum` will have type `OaValidationError`
 *   const error = maybeMuseum;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMuseum` will have type `Museum`
 * const museum = maybeMuseum;
 * ```
 */
export function validateMuseum(maybeMuseum: unknown): Museum | OaValidationError {
  const { value, error } = MuseumJoiSchema.validate(maybeMuseum);
  if (error) {
    return new OaValidationError('Museum', maybeMuseum, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Museum;
}
