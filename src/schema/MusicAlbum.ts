...
export type MusicAlbum = {
   ...
   */
  albumRelease?: schema.MusicReleaseOrSubClass | string;
  /**
   * The artist that performed this album or recording.
   */
  byArtist?: schema.MusicGroupOrSubClass | schema.PersonOrSubClass | string;
  /**
   ...
   */
  track?: schema.ItemListOrSubClass | schema.MusicRecordingOrSubClass | string;
  /**
   * A music recording (track)&#x2014;usually a single song.
   */
  tracks?: schema.MusicRecordingOrSubClass | string;
  /**
 ...
 * schema:MusicAlbum
 *
 * This differs from MusicAlbum because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicAlbumOrSubClass =
  | MusicAlbum
  ;

/**
 * schema:MusicAlbum - Validation schema (w/ JOI)
 */
export const MusicAlbumJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicAlbum').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  albumProductionType: Joi.lazy(() => schema.MusicAlbumProductionTypeJoiSchema),
  albumRelease: Joi.alternatives().try(Joi.lazy(() => schema.MusicReleaseOrSubClassJoiSchema), Joi.string().uri()),
  byArtist: Joi.alternatives().try(Joi.lazy(() => schema.MusicGroupOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  albumReleaseType: Joi.lazy(() => schema.MusicAlbumReleaseTypeJoiSchema),
  track: Joi.alternatives().try(Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.lazy(() => schema.MusicRecordingOrSubClassJoiSchema), Joi.string().uri()),
  tracks: Joi.alternatives().try(Joi.lazy(() => schema.MusicRecordingOrSubClassJoiSchema), Joi.string().uri()),
  numTracks: Joi.number().integer(),
  text: Joi.string(),
 ...
 * schema:MusicAlbum - Validation schema (w/ JOI)
 *
 * This differs from MusicAlbumJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicAlbumOrSubClassJoiSchema = Joi.alternatives().try([
  MusicAlbumJoiSchema,
]);

/**
 * Runtime validator for schema:MusicAlbum.
 *
 * If some data has a structure which matches a schema:MusicAlbum, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicAlbum = MusicAlbum.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicAlbum instanceof OaValidationError) {
 *   // From this point on, `maybeMusicAlbum` will have type `OaValidationError`
 *   const error = maybeMusicAlbum;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicAlbum` will have type `MusicAlbum`
 * const musicAlbum = maybeMusicAlbum;
 * ```
 */
export function validateMusicAlbum(maybeMusicAlbum: unknown): MusicAlbum | OaValidationError {
  const { value, error } = MusicAlbumJoiSchema.validate(maybeMusicAlbum);
  if (error) {
    return new OaValidationError('MusicAlbum', maybeMusicAlbum, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicAlbum;
}
