 ...
 * etc.
 */
export type MusicCompositionOrSubClass =
  | MusicComposition
  ;
 ...
 */
export const MusicCompositionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const MusicCompositionOrSubClassJoiSchema = Joi.alternatives().try([
  MusicCompositionJoiSchema,
]);
 ...
 * ```
 */
export function validateMusicComposition(maybeMusicComposition: unknown): MusicComposition | OaValidationError {
  const { value, error } = MusicCompositionJoiSchema.validate(maybeMusicComposition);
  if (error) {
    return new OaValidationError('MusicComposition', maybeMusicComposition, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicComposition;
}
