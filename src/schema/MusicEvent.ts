 ...
 * schema:MusicEvent
 *
 * This differs from MusicEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicEventOrSubClass =
  | MusicEvent
  ;

/**
 * schema:MusicEvent - Validation schema (w/ JOI)
 */
export const MusicEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicEvent').required(),
 ...
 * schema:MusicEvent - Validation schema (w/ JOI)
 *
 * This differs from MusicEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicEventOrSubClassJoiSchema = Joi.alternatives().try([
  MusicEventJoiSchema,
]);

/**
 * Runtime validator for schema:MusicEvent.
 *
 * If some data has a structure which matches a schema:MusicEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicEvent = MusicEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicEvent instanceof OaValidationError) {
 *   // From this point on, `maybeMusicEvent` will have type `OaValidationError`
 *   const error = maybeMusicEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicEvent` will have type `MusicEvent`
 * const musicEvent = maybeMusicEvent;
 * ```
 */
export function validateMusicEvent(maybeMusicEvent: unknown): MusicEvent | OaValidationError {
  const { value, error } = MusicEventJoiSchema.validate(maybeMusicEvent);
  if (error) {
    return new OaValidationError('MusicEvent', maybeMusicEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicEvent;
}
