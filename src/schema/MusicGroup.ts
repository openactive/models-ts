...
export type MusicGroup = {
   ...
   */
  musicGroupMember?: schema.PersonOrSubClass | string;
  /**
   ...
   * A music album.
   */
  album?: schema.MusicAlbumOrSubClass | string;
  /**
   ...
   */
  albums?: schema.MusicAlbumOrSubClass | string;
  /**
 ...
 * schema:MusicGroup
 *
 * This differs from MusicGroup because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicGroupOrSubClass =
  | MusicGroup
  ;

/**
 * schema:MusicGroup - Validation schema (w/ JOI)
 */
export const MusicGroupJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicGroup').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  musicGroupMember: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  ...
  album: Joi.alternatives().try(Joi.lazy(() => schema.MusicAlbumOrSubClassJoiSchema), Joi.string().uri()),
  tracks: Joi.alternatives().try(Joi.lazy(() => schema.MusicRecordingOrSubClassJoiSchema), Joi.string().uri()),
  albums: Joi.alternatives().try(Joi.lazy(() => schema.MusicAlbumOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:MusicGroup - Validation schema (w/ JOI)
 *
 * This differs from MusicGroupJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicGroupOrSubClassJoiSchema = Joi.alternatives().try([
  MusicGroupJoiSchema,
]);

/**
 * Runtime validator for schema:MusicGroup.
 *
 * If some data has a structure which matches a schema:MusicGroup, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicGroup = MusicGroup.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicGroup instanceof OaValidationError) {
 *   // From this point on, `maybeMusicGroup` will have type `OaValidationError`
 *   const error = maybeMusicGroup;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicGroup` will have type `MusicGroup`
 * const musicGroup = maybeMusicGroup;
 * ```
 */
export function validateMusicGroup(maybeMusicGroup: unknown): MusicGroup | OaValidationError {
  const { value, error } = MusicGroupJoiSchema.validate(maybeMusicGroup);
  if (error) {
    return new OaValidationError('MusicGroup', maybeMusicGroup, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicGroup;
}
