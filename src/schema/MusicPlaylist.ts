 ...
 * schema:MusicPlaylist
 *
 * This differs from MusicPlaylist because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicPlaylistOrSubClass =
  | MusicPlaylist
  | schema.MusicAlbumOrSubClass
  | schema.MusicReleaseOrSubClass
  ;

/**
 * schema:MusicPlaylist - Validation schema (w/ JOI)
 */
export const MusicPlaylistJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicPlaylist').required(),
 ...
 * schema:MusicPlaylist - Validation schema (w/ JOI)
 *
 * This differs from MusicPlaylistJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicPlaylistOrSubClassJoiSchema = Joi.alternatives().try([
  MusicPlaylistJoiSchema,
  Joi.lazy(() => schema.MusicAlbumOrSubClassJoiSchema),
  Joi.lazy(() => schema.MusicReleaseOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:MusicPlaylist.
 *
 * If some data has a structure which matches a schema:MusicPlaylist, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicPlaylist = MusicPlaylist.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicPlaylist instanceof OaValidationError) {
 *   // From this point on, `maybeMusicPlaylist` will have type `OaValidationError`
 *   const error = maybeMusicPlaylist;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicPlaylist` will have type `MusicPlaylist`
 * const musicPlaylist = maybeMusicPlaylist;
 * ```
 */
export function validateMusicPlaylist(maybeMusicPlaylist: unknown): MusicPlaylist | OaValidationError {
  const { value, error } = MusicPlaylistJoiSchema.validate(maybeMusicPlaylist);
  if (error) {
    return new OaValidationError('MusicPlaylist', maybeMusicPlaylist, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicPlaylist;
}
