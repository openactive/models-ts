...
export type MusicRecording = {
   ...
   */
  recordingOf?: schema.MusicCompositionOrSubClass | string;
  /**
   ...
   */
  inPlaylist?: schema.MusicPlaylistOrSubClass | string;
  /**
   ...
   */
  inAlbum?: schema.MusicAlbumOrSubClass | string;
  /**
 ...
 * schema:MusicRecording
 *
 * This differs from MusicRecording because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicRecordingOrSubClass =
  | MusicRecording
  ;

/**
 * schema:MusicRecording - Validation schema (w/ JOI)
 */
export const MusicRecordingJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicRecording').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  recordingOf: Joi.alternatives().try(Joi.lazy(() => schema.MusicCompositionOrSubClassJoiSchema), Joi.string().uri()),
  ...
  inPlaylist: Joi.alternatives().try(Joi.lazy(() => schema.MusicPlaylistOrSubClassJoiSchema), Joi.string().uri()),
  duration: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  isrcCode: Joi.string(),
  inAlbum: Joi.alternatives().try(Joi.lazy(() => schema.MusicAlbumOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:MusicRecording - Validation schema (w/ JOI)
 *
 * This differs from MusicRecordingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicRecordingOrSubClassJoiSchema = Joi.alternatives().try([
  MusicRecordingJoiSchema,
]);

/**
 * Runtime validator for schema:MusicRecording.
 *
 * If some data has a structure which matches a schema:MusicRecording, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicRecording = MusicRecording.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicRecording instanceof OaValidationError) {
 *   // From this point on, `maybeMusicRecording` will have type `OaValidationError`
 *   const error = maybeMusicRecording;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicRecording` will have type `MusicRecording`
 * const musicRecording = maybeMusicRecording;
 * ```
 */
export function validateMusicRecording(maybeMusicRecording: unknown): MusicRecording | OaValidationError {
  const { value, error } = MusicRecordingJoiSchema.validate(maybeMusicRecording);
  if (error) {
    return new OaValidationError('MusicRecording', maybeMusicRecording, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicRecording;
}
