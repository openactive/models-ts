...
export type MusicRelease = {
   ...
   */
  releaseOf?: schema.MusicAlbumOrSubClass | string;
  /**
   * The label that issued the release.
   */
  recordLabel?: schema.OrganizationOrSubClass | string;
  /**
   * The group the release is credited to if different than the byArtist. For example, Red and Blue is credited to "Stefani Germanotta Band", but by Lady Gaga.
   */
  creditedTo?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
 ...
 * schema:MusicRelease
 *
 * This differs from MusicRelease because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicReleaseOrSubClass =
  | MusicRelease
  ;

/**
 * schema:MusicRelease - Validation schema (w/ JOI)
 */
export const MusicReleaseJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicRelease').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  catalogNumber: Joi.string(),
  releaseOf: Joi.alternatives().try(Joi.lazy(() => schema.MusicAlbumOrSubClassJoiSchema), Joi.string().uri()),
  recordLabel: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
  creditedTo: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
  musicReleaseFormat: Joi.lazy(() => schema.MusicReleaseFormatTypeJoiSchema),
 ...
 * schema:MusicRelease - Validation schema (w/ JOI)
 *
 * This differs from MusicReleaseJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicReleaseOrSubClassJoiSchema = Joi.alternatives().try([
  MusicReleaseJoiSchema,
]);

/**
 * Runtime validator for schema:MusicRelease.
 *
 * If some data has a structure which matches a schema:MusicRelease, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeMusicRelease` will have type `MusicRelease`
 * const musicRelease = maybeMusicRelease;
 * ```
 */
export function validateMusicRelease(maybeMusicRelease: unknown): MusicRelease | OaValidationError {
  const { value, error } = MusicReleaseJoiSchema.validate(maybeMusicRelease);
  if (error) {
    return new OaValidationError('MusicRelease', maybeMusicRelease, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicRelease;
}
