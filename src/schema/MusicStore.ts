 ...
 * schema:MusicStore
 *
 * This differs from MusicStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicStoreOrSubClass =
  | MusicStore
  ;

/**
 * schema:MusicStore - Validation schema (w/ JOI)
 */
export const MusicStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicStore').required(),
 ...
 * schema:MusicStore - Validation schema (w/ JOI)
 *
 * This differs from MusicStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicStoreOrSubClassJoiSchema = Joi.alternatives().try([
  MusicStoreJoiSchema,
]);

/**
 * Runtime validator for schema:MusicStore.
 *
 * If some data has a structure which matches a schema:MusicStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicStore = MusicStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicStore instanceof OaValidationError) {
 *   // From this point on, `maybeMusicStore` will have type `OaValidationError`
 *   const error = maybeMusicStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicStore` will have type `MusicStore`
 * const musicStore = maybeMusicStore;
 * ```
 */
export function validateMusicStore(maybeMusicStore: unknown): MusicStore | OaValidationError {
  const { value, error } = MusicStoreJoiSchema.validate(maybeMusicStore);
  if (error) {
    return new OaValidationError('MusicStore', maybeMusicStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicStore;
}
