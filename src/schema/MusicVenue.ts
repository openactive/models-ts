 ...
 * schema:MusicVenue
 *
 * This differs from MusicVenue because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicVenueOrSubClass =
  | MusicVenue
  ;

/**
 * schema:MusicVenue - Validation schema (w/ JOI)
 */
export const MusicVenueJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicVenue').required(),
 ...
 * schema:MusicVenue - Validation schema (w/ JOI)
 *
 * This differs from MusicVenueJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicVenueOrSubClassJoiSchema = Joi.alternatives().try([
  MusicVenueJoiSchema,
]);

/**
 * Runtime validator for schema:MusicVenue.
 *
 * If some data has a structure which matches a schema:MusicVenue, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicVenue = MusicVenue.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicVenue instanceof OaValidationError) {
 *   // From this point on, `maybeMusicVenue` will have type `OaValidationError`
 *   const error = maybeMusicVenue;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicVenue` will have type `MusicVenue`
 * const musicVenue = maybeMusicVenue;
 * ```
 */
export function validateMusicVenue(maybeMusicVenue: unknown): MusicVenue | OaValidationError {
  const { value, error } = MusicVenueJoiSchema.validate(maybeMusicVenue);
  if (error) {
    return new OaValidationError('MusicVenue', maybeMusicVenue, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicVenue;
}
