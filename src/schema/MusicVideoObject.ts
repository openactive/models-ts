 ...
 * schema:MusicVideoObject
 *
 * This differs from MusicVideoObject because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type MusicVideoObjectOrSubClass =
  | MusicVideoObject
  ;

/**
 * schema:MusicVideoObject - Validation schema (w/ JOI)
 */
export const MusicVideoObjectJoiSchema = Joi.object({
  '@type': Joi.string().valid('MusicVideoObject').required(),
 ...
 * schema:MusicVideoObject - Validation schema (w/ JOI)
 *
 * This differs from MusicVideoObjectJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const MusicVideoObjectOrSubClassJoiSchema = Joi.alternatives().try([
  MusicVideoObjectJoiSchema,
]);

/**
 * Runtime validator for schema:MusicVideoObject.
 *
 * If some data has a structure which matches a schema:MusicVideoObject, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeMusicVideoObject = MusicVideoObject.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeMusicVideoObject instanceof OaValidationError) {
 *   // From this point on, `maybeMusicVideoObject` will have type `OaValidationError`
 *   const error = maybeMusicVideoObject;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeMusicVideoObject` will have type `MusicVideoObject`
 * const musicVideoObject = maybeMusicVideoObject;
 * ```
 */
export function validateMusicVideoObject(maybeMusicVideoObject: unknown): MusicVideoObject | OaValidationError {
  const { value, error } = MusicVideoObjectJoiSchema.validate(maybeMusicVideoObject);
  if (error) {
    return new OaValidationError('MusicVideoObject', maybeMusicVideoObject, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as MusicVideoObject;
}
