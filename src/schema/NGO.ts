 ...
 * schema:NGO
 *
 * This differs from NGO because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NGOOrSubClass =
  | NGO
  ;
 ...
 */
export const NGOJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const NGOOrSubClassJoiSchema = Joi.alternatives().try([
  NGOJoiSchema,
]);
 ...
 * ```
 */
export function validateNGO(maybeNGO: unknown): NGO | OaValidationError {
  const { value, error } = NGOJoiSchema.validate(maybeNGO);
  if (error) {
    return new OaValidationError('NGO', maybeNGO, error);
  }
