 ...
 * etc.
 */
export type NLNonprofitTypeOrSubClass =
  | NLNonprofitType
  ;
 ...
 */
export const NLNonprofitTypeJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const NLNonprofitTypeOrSubClassJoiSchema = Joi.alternatives().try([
  NLNonprofitTypeJoiSchema,
]);
 ...
 * ```
 */
export function validateNLNonprofitType(maybeNLNonprofitType: unknown): NLNonprofitType | OaValidationError {
  const { value, error } = NLNonprofitTypeJoiSchema.validate(maybeNLNonprofitType);
  if (error) {
    return new OaValidationError('NLNonprofitType', maybeNLNonprofitType, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as NLNonprofitType;
}
