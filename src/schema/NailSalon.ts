 ...
 * schema:NailSalon
 *
 * This differs from NailSalon because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NailSalonOrSubClass =
  | NailSalon
  ;

/**
 * schema:NailSalon - Validation schema (w/ JOI)
 */
export const NailSalonJoiSchema = Joi.object({
  '@type': Joi.string().valid('NailSalon').required(),
 ...
 * schema:NailSalon - Validation schema (w/ JOI)
 *
 * This differs from NailSalonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const NailSalonOrSubClassJoiSchema = Joi.alternatives().try([
  NailSalonJoiSchema,
]);

/**
 * Runtime validator for schema:NailSalon.
 *
 * If some data has a structure which matches a schema:NailSalon, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeNailSalon = NailSalon.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeNailSalon instanceof OaValidationError) {
 *   // From this point on, `maybeNailSalon` will have type `OaValidationError`
 *   const error = maybeNailSalon;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeNailSalon` will have type `NailSalon`
 * const nailSalon = maybeNailSalon;
 * ```
 */
export function validateNailSalon(maybeNailSalon: unknown): NailSalon | OaValidationError {
  const { value, error } = NailSalonJoiSchema.validate(maybeNailSalon);
  if (error) {
    return new OaValidationError('NailSalon', maybeNailSalon, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as NailSalon;
}
