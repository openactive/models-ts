...
export type Nerve = {
   ...
   */
  sourcedFrom?: schema.BrainStructureOrSubClass | string;
  /**
   * The neurological pathway extension that involves muscle control.
   */
  nerveMotor?: schema.MuscleOrSubClass | string;
  /**
   * The branches that delineate from the nerve bundle. Not to be confused with [[branchOf]].
   */
  branch?: schema.AnatomicalStructureOrSubClass | string;
  /**
   * The neurological pathway extension that inputs and sends information to the brain or spinal cord.
   */
  sensoryUnit?: schema.SuperficialAnatomyOrSubClass | schema.AnatomicalStructureOrSubClass | string;
  /**
 ...
 * schema:Nerve
 *
 * This differs from Nerve because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NerveOrSubClass =
  | Nerve
  ;
 ...
 */
export const NerveJoiSchema = Joi.object({
  ...
  sensoryUnit: Joi.alternatives().try(Joi.lazy(() => schema.SuperficialAnatomyOrSubClassJoiSchema), Joi.lazy(() => schema.AnatomicalStructureOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const NerveOrSubClassJoiSchema = Joi.alternatives().try([
  NerveJoiSchema,
]);
 ...
 * ```
 */
export function validateNerve(maybeNerve: unknown): Nerve | OaValidationError {
  const { value, error } = NerveJoiSchema.validate(maybeNerve);
  if (error) {
    return new OaValidationError('Nerve', maybeNerve, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Nerve;
}
