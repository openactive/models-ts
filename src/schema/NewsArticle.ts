...
export type NewsArticle = {
   ...
   *          
   */
  speakable?: schema.SpeakableSpecificationOrSubClass | string;
  /**
 ...
 * schema:NewsArticle
 *
 * This differs from NewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NewsArticleOrSubClass =
  | NewsArticle
  | schema.ReportageNewsArticleOrSubClass
  | schema.OpinionNewsArticleOrSubClass
  | schema.BackgroundNewsArticleOrSubClass
  | schema.AnalysisNewsArticleOrSubClass
  | schema.AskPublicNewsArticleOrSubClass
  | schema.ReviewNewsArticleOrSubClass
  ;

/**
 * schema:NewsArticle - Validation schema (w/ JOI)
 */
export const NewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('NewsArticle').required(),
  ...
  speakable: Joi.alternatives().try(Joi.lazy(() => schema.SpeakableSpecificationOrSubClassJoiSchema), Joi.string().uri()),
  ...
  pageStart: Joi.alternatives().try(Joi.string(), Joi.number().integer()),
  pageEnd: Joi.alternatives().try(Joi.string(), Joi.number().integer()),
 ...
 * schema:NewsArticle - Validation schema (w/ JOI)
 *
 * This differs from NewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const NewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  NewsArticleJoiSchema,
  Joi.lazy(() => schema.ReportageNewsArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.OpinionNewsArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.BackgroundNewsArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.AnalysisNewsArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.AskPublicNewsArticleOrSubClassJoiSchema),
  Joi.lazy(() => schema.ReviewNewsArticleOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:NewsArticle.
 *
 * If some data has a structure which matches a schema:NewsArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeNewsArticle = NewsArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeNewsArticle instanceof OaValidationError) {
 *   // From this point on, `maybeNewsArticle` will have type `OaValidationError`
 *   const error = maybeNewsArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeNewsArticle` will have type `NewsArticle`
 * const newsArticle = maybeNewsArticle;
 * ```
 */
export function validateNewsArticle(maybeNewsArticle: unknown): NewsArticle | OaValidationError {
  const { value, error } = NewsArticleJoiSchema.validate(maybeNewsArticle);
  if (error) {
    return new OaValidationError('NewsArticle', maybeNewsArticle, error);
  }
