 ...
 */
export type NewsMediaOrganization = {
   ...
   */
  missionCoveragePrioritiesPolicy?: schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  noBylinesPolicy?: string | schema.CreativeWorkOrSubClass;
  /**
   * Disclosure about verification and fact-checking processes for a [[NewsMediaOrganization]] or other fact-checking [[Organization]].
   */
  verificationFactCheckingPolicy?: schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  masthead?: string | schema.CreativeWorkOrSubClass;
  /**
 ...
 * schema:NewsMediaOrganization
 *
 * This differs from NewsMediaOrganization because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NewsMediaOrganizationOrSubClass =
 ...
 */
export const NewsMediaOrganizationJoiSchema = Joi.object({
  ...
  missionCoveragePrioritiesPolicy: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
  ...
  noBylinesPolicy: Joi.alternatives().try(Joi.string().uri(), Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema)),
  ...
  masthead: Joi.alternatives().try(Joi.string().uri(), Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema)),
 ...
 * `SessionSeries`, etc.
 */
export const NewsMediaOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
  NewsMediaOrganizationJoiSchema,
]);
 ...
 * ```
 */
export function validateNewsMediaOrganization(maybeNewsMediaOrganization: unknown): NewsMediaOrganization | OaValidationError {
  const { value, error } = NewsMediaOrganizationJoiSchema.validate(maybeNewsMediaOrganization);
  if (error) {
    return new OaValidationError('NewsMediaOrganization', maybeNewsMediaOrganization, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as NewsMediaOrganization;
}
