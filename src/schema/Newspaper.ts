 ...
 * schema:Newspaper
 *
 * This differs from Newspaper because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NewspaperOrSubClass =
  | Newspaper
  ;

/**
 * schema:Newspaper - Validation schema (w/ JOI)
 */
export const NewspaperJoiSchema = Joi.object({
  '@type': Joi.string().valid('Newspaper').required(),
 ...
 * schema:Newspaper - Validation schema (w/ JOI)
 *
 * This differs from NewspaperJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const NewspaperOrSubClassJoiSchema = Joi.alternatives().try([
  NewspaperJoiSchema,
]);

/**
 * Runtime validator for schema:Newspaper.
 *
 * If some data has a structure which matches a schema:Newspaper, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeNewspaper = Newspaper.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeNewspaper instanceof OaValidationError) {
 *   // From this point on, `maybeNewspaper` will have type `OaValidationError`
 *   const error = maybeNewspaper;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeNewspaper` will have type `Newspaper`
 * const newspaper = maybeNewspaper;
 * ```
 */
export function validateNewspaper(maybeNewspaper: unknown): Newspaper | OaValidationError {
  const { value, error } = NewspaperJoiSchema.validate(maybeNewspaper);
  if (error) {
    return new OaValidationError('Newspaper', maybeNewspaper, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Newspaper;
}
