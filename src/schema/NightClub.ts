 ...
 * schema:NightClub
 *
 * This differs from NightClub because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NightClubOrSubClass =
  | NightClub
  ;

/**
 * schema:NightClub - Validation schema (w/ JOI)
 */
export const NightClubJoiSchema = Joi.object({
  '@type': Joi.string().valid('NightClub').required(),
 ...
 * schema:NightClub - Validation schema (w/ JOI)
 *
 * This differs from NightClubJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const NightClubOrSubClassJoiSchema = Joi.alternatives().try([
  NightClubJoiSchema,
]);

/**
 * Runtime validator for schema:NightClub.
 *
 * If some data has a structure which matches a schema:NightClub, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeNightClub = NightClub.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeNightClub instanceof OaValidationError) {
 *   // From this point on, `maybeNightClub` will have type `OaValidationError`
 *   const error = maybeNightClub;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeNightClub` will have type `NightClub`
 * const nightClub = maybeNightClub;
 * ```
 */
export function validateNightClub(maybeNightClub: unknown): NightClub | OaValidationError {
  const { value, error } = NightClubJoiSchema.validate(maybeNightClub);
  if (error) {
    return new OaValidationError('NightClub', maybeNightClub, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as NightClub;
}
