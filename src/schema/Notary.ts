 ...
 * schema:Notary
 *
 * This differs from Notary because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NotaryOrSubClass =
  | Notary
  ;

/**
 * schema:Notary - Validation schema (w/ JOI)
 */
export const NotaryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Notary').required(),
 ...
 * schema:Notary - Validation schema (w/ JOI)
 *
 * This differs from NotaryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const NotaryOrSubClassJoiSchema = Joi.alternatives().try([
  NotaryJoiSchema,
]);

/**
 * Runtime validator for schema:Notary.
 *
 * If some data has a structure which matches a schema:Notary, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeNotary` will have type `Notary`
 * const notary = maybeNotary;
 * ```
 */
export function validateNotary(maybeNotary: unknown): Notary | OaValidationError {
  const { value, error } = NotaryJoiSchema.validate(maybeNotary);
  if (error) {
    return new OaValidationError('Notary', maybeNotary, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Notary;
}
