 ...
 */
export type NoteDigitalDocument = {
 ...
 * schema:NoteDigitalDocument
 *
 * This differs from NoteDigitalDocument because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type NoteDigitalDocumentOrSubClass =
  | NoteDigitalDocument
  ;

/**
 * schema:NoteDigitalDocument - Validation schema (w/ JOI)
 */
export const NoteDigitalDocumentJoiSchema = Joi.object({
  '@type': Joi.string().valid('NoteDigitalDocument').required(),
 ...
 * schema:NoteDigitalDocument - Validation schema (w/ JOI)
 *
 * This differs from NoteDigitalDocumentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const NoteDigitalDocumentOrSubClassJoiSchema = Joi.alternatives().try([
  NoteDigitalDocumentJoiSchema,
]);

/**
 * Runtime validator for schema:NoteDigitalDocument.
 *
 * If some data has a structure which matches a schema:NoteDigitalDocument, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeNoteDigitalDocument = NoteDigitalDocument.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeNoteDigitalDocument instanceof OaValidationError) {
 *   // From this point on, `maybeNoteDigitalDocument` will have type `OaValidationError`
 *   const error = maybeNoteDigitalDocument;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeNoteDigitalDocument` will have type `NoteDigitalDocument`
 * const noteDigitalDocument = maybeNoteDigitalDocument;
 * ```
 */
export function validateNoteDigitalDocument(maybeNoteDigitalDocument: unknown): NoteDigitalDocument | OaValidationError {
  const { value, error } = NoteDigitalDocumentJoiSchema.validate(maybeNoteDigitalDocument);
  if (error) {
    return new OaValidationError('NoteDigitalDocument', maybeNoteDigitalDocument, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as NoteDigitalDocument;
}
