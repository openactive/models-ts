 ...
 * etc.
 */
export type OccupationOrSubClass =
  | Occupation
  ;
 ...
 */
export const OccupationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OccupationOrSubClassJoiSchema = Joi.alternatives().try([
  OccupationJoiSchema,
]);
 ...
 * ```
 */
export function validateOccupation(maybeOccupation: unknown): Occupation | OaValidationError {
  const { value, error } = OccupationJoiSchema.validate(maybeOccupation);
  if (error) {
    return new OaValidationError('Occupation', maybeOccupation, error);
  }
