 ...
 */
export type OccupationalTherapy = {
 ...
 * schema:OccupationalTherapy
 *
 * This differs from OccupationalTherapy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OccupationalTherapyOrSubClass =
  | OccupationalTherapy
  ;
 ...
 */
export const OccupationalTherapyJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OccupationalTherapyOrSubClassJoiSchema = Joi.alternatives().try([
  OccupationalTherapyJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeOccupationalTherapy = OccupationalTherapy.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOccupationalTherapy(maybeOccupationalTherapy: unknown): OccupationalTherapy | OaValidationError {
  const { value, error } = OccupationalTherapyJoiSchema.validate(maybeOccupationalTherapy);
  if (error) {
    return new OaValidationError('OccupationalTherapy', maybeOccupationalTherapy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OccupationalTherapy;
}
