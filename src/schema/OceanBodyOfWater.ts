 ...
 * schema:OceanBodyOfWater
 *
 * This differs from OceanBodyOfWater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OceanBodyOfWaterOrSubClass =
  | OceanBodyOfWater
  ;

/**
 * schema:OceanBodyOfWater - Validation schema (w/ JOI)
 */
export const OceanBodyOfWaterJoiSchema = Joi.object({
  '@type': Joi.string().valid('OceanBodyOfWater').required(),
 ...
 * schema:OceanBodyOfWater - Validation schema (w/ JOI)
 *
 * This differs from OceanBodyOfWaterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OceanBodyOfWaterOrSubClassJoiSchema = Joi.alternatives().try([
  OceanBodyOfWaterJoiSchema,
]);

/**
 * Runtime validator for schema:OceanBodyOfWater.
 *
 * If some data has a structure which matches a schema:OceanBodyOfWater, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOceanBodyOfWater = OceanBodyOfWater.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeOceanBodyOfWater instanceof OaValidationError) {
 *   // From this point on, `maybeOceanBodyOfWater` will have type `OaValidationError`
 *   const error = maybeOceanBodyOfWater;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeOceanBodyOfWater` will have type `OceanBodyOfWater`
 * const oceanBodyOfWater = maybeOceanBodyOfWater;
 * ```
 */
export function validateOceanBodyOfWater(maybeOceanBodyOfWater: unknown): OceanBodyOfWater | OaValidationError {
  const { value, error } = OceanBodyOfWaterJoiSchema.validate(maybeOceanBodyOfWater);
  if (error) {
    return new OaValidationError('OceanBodyOfWater', maybeOceanBodyOfWater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OceanBodyOfWater;
}
