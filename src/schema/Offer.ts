 ...
 * etc.
 */
export type OfferOrSubClass =
  | Offer
  | schema.OfferForPurchaseOrSubClass
  | schema.OfferForLeaseOrSubClass
  | schema.AggregateOfferOrSubClass
  ;

/**
 * schema:Offer - Validation schema (w/ JOI)
 */
export const OfferJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OfferOrSubClassJoiSchema = Joi.alternatives().try([
  OfferJoiSchema,
  Joi.lazy(() => schema.OfferForPurchaseOrSubClassJoiSchema),
  Joi.lazy(() => schema.OfferForLeaseOrSubClassJoiSchema),
  Joi.lazy(() => schema.AggregateOfferOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Offer.
 *
 * If some data has a structure which matches a schema:Offer, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOffer = Offer.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOffer(maybeOffer: unknown): Offer | OaValidationError {
  const { value, error } = OfferJoiSchema.validate(maybeOffer);
  if (error) {
    return new OaValidationError('Offer', maybeOffer, error);
  }
