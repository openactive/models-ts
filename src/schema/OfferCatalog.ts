 ...
 * etc.
 */
export type OfferCatalogOrSubClass =
  | OfferCatalog
  ;
 ...
 */
export const OfferCatalogJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OfferCatalogOrSubClassJoiSchema = Joi.alternatives().try([
  OfferCatalogJoiSchema,
]);
 ...
 * ```
 */
export function validateOfferCatalog(maybeOfferCatalog: unknown): OfferCatalog | OaValidationError {
  const { value, error } = OfferCatalogJoiSchema.validate(maybeOfferCatalog);
  if (error) {
    return new OaValidationError('OfferCatalog', maybeOfferCatalog, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OfferCatalog;
}
