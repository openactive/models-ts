 ...
 * etc.
 */
export type OfferForLeaseOrSubClass =
  | OfferForLease
  ;
 ...
 */
export const OfferForLeaseJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OfferForLeaseOrSubClassJoiSchema = Joi.alternatives().try([
  OfferForLeaseJoiSchema,
]);
 ...
 * ```
 */
export function validateOfferForLease(maybeOfferForLease: unknown): OfferForLease | OaValidationError {
  const { value, error } = OfferForLeaseJoiSchema.validate(maybeOfferForLease);
  if (error) {
    return new OaValidationError('OfferForLease', maybeOfferForLease, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OfferForLease;
}
