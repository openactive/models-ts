 ...
 * schema:OfferForPurchase
 *
 * This differs from OfferForPurchase because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OfferForPurchaseOrSubClass =
  | OfferForPurchase
  ;

/**
 * schema:OfferForPurchase - Validation schema (w/ JOI)
 */
export const OfferForPurchaseJoiSchema = Joi.object({
  '@type': Joi.string().valid('OfferForPurchase').required(),
 ...
 * schema:OfferForPurchase - Validation schema (w/ JOI)
 *
 * This differs from OfferForPurchaseJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OfferForPurchaseOrSubClassJoiSchema = Joi.alternatives().try([
  OfferForPurchaseJoiSchema,
]);

/**
 * Runtime validator for schema:OfferForPurchase.
 *
 * If some data has a structure which matches a schema:OfferForPurchase, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeOfferForPurchase` will have type `OfferForPurchase`
 * const offerForPurchase = maybeOfferForPurchase;
 * ```
 */
export function validateOfferForPurchase(maybeOfferForPurchase: unknown): OfferForPurchase | OaValidationError {
  const { value, error } = OfferForPurchaseJoiSchema.validate(maybeOfferForPurchase);
  if (error) {
    return new OaValidationError('OfferForPurchase', maybeOfferForPurchase, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OfferForPurchase;
}
