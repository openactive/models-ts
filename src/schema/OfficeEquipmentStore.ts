 ...
 */
export type OfficeEquipmentStore = {
 ...
 * schema:OfficeEquipmentStore
 *
 * This differs from OfficeEquipmentStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OfficeEquipmentStoreOrSubClass =
  | OfficeEquipmentStore
  ;

/**
 * schema:OfficeEquipmentStore - Validation schema (w/ JOI)
 */
export const OfficeEquipmentStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('OfficeEquipmentStore').required(),
 ...
 * schema:OfficeEquipmentStore - Validation schema (w/ JOI)
 *
 * This differs from OfficeEquipmentStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OfficeEquipmentStoreOrSubClassJoiSchema = Joi.alternatives().try([
  OfficeEquipmentStoreJoiSchema,
]);

/**
 * Runtime validator for schema:OfficeEquipmentStore.
 *
 * If some data has a structure which matches a schema:OfficeEquipmentStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOfficeEquipmentStore = OfficeEquipmentStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeOfficeEquipmentStore instanceof OaValidationError) {
 *   // From this point on, `maybeOfficeEquipmentStore` will have type `OaValidationError`
 *   const error = maybeOfficeEquipmentStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeOfficeEquipmentStore` will have type `OfficeEquipmentStore`
 * const officeEquipmentStore = maybeOfficeEquipmentStore;
 * ```
 */
export function validateOfficeEquipmentStore(maybeOfficeEquipmentStore: unknown): OfficeEquipmentStore | OaValidationError {
  const { value, error } = OfficeEquipmentStoreJoiSchema.validate(maybeOfficeEquipmentStore);
  if (error) {
    return new OaValidationError('OfficeEquipmentStore', maybeOfficeEquipmentStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OfficeEquipmentStore;
}
