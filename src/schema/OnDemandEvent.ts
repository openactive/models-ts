 ...
 * schema:OnDemandEvent
 *
 * This differs from OnDemandEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OnDemandEventOrSubClass =
  | OnDemandEvent
  ;

/**
 * schema:OnDemandEvent - Validation schema (w/ JOI)
 */
export const OnDemandEventJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OnDemandEventOrSubClassJoiSchema = Joi.alternatives().try([
  OnDemandEventJoiSchema,
]);

/**
 * Runtime validator for schema:OnDemandEvent.
 *
 * If some data has a structure which matches a schema:OnDemandEvent, it will be returned with the correct type.
 ...
 * ```
 */
export function validateOnDemandEvent(maybeOnDemandEvent: unknown): OnDemandEvent | OaValidationError {
  const { value, error } = OnDemandEventJoiSchema.validate(maybeOnDemandEvent);
  if (error) {
    return new OaValidationError('OnDemandEvent', maybeOnDemandEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OnDemandEvent;
}
