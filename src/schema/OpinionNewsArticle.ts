 ...
 */
export type OpinionNewsArticle = {
 ...
 * schema:OpinionNewsArticle
 *
 * This differs from OpinionNewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OpinionNewsArticleOrSubClass =
  | OpinionNewsArticle
  ;

/**
 * schema:OpinionNewsArticle - Validation schema (w/ JOI)
 */
export const OpinionNewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('OpinionNewsArticle').required(),
 ...
 * schema:OpinionNewsArticle - Validation schema (w/ JOI)
 *
 * This differs from OpinionNewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OpinionNewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  OpinionNewsArticleJoiSchema,
]);

/**
 * Runtime validator for schema:OpinionNewsArticle.
 *
 * If some data has a structure which matches a schema:OpinionNewsArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOpinionNewsArticle = OpinionNewsArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeOpinionNewsArticle instanceof OaValidationError) {
 *   // From this point on, `maybeOpinionNewsArticle` will have type `OaValidationError`
 *   const error = maybeOpinionNewsArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeOpinionNewsArticle` will have type `OpinionNewsArticle`
 * const opinionNewsArticle = maybeOpinionNewsArticle;
 * ```
 */
export function validateOpinionNewsArticle(maybeOpinionNewsArticle: unknown): OpinionNewsArticle | OaValidationError {
  const { value, error } = OpinionNewsArticleJoiSchema.validate(maybeOpinionNewsArticle);
  if (error) {
    return new OaValidationError('OpinionNewsArticle', maybeOpinionNewsArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OpinionNewsArticle;
}
