 ...
 * schema:Optician
 *
 * This differs from Optician because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OpticianOrSubClass =
  | Optician
  ;

/**
 * schema:Optician - Validation schema (w/ JOI)
 */
export const OpticianJoiSchema = Joi.object({
  '@type': Joi.string().valid('Optician').required(),
 ...
 * schema:Optician - Validation schema (w/ JOI)
 *
 * This differs from OpticianJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OpticianOrSubClassJoiSchema = Joi.alternatives().try([
  OpticianJoiSchema,
]);

/**
 * Runtime validator for schema:Optician.
 *
 * If some data has a structure which matches a schema:Optician, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOptician = Optician.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeOptician instanceof OaValidationError) {
 *   // From this point on, `maybeOptician` will have type `OaValidationError`
 *   const error = maybeOptician;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeOptician` will have type `Optician`
 * const optician = maybeOptician;
 * ```
 */
export function validateOptician(maybeOptician: unknown): Optician | OaValidationError {
  const { value, error } = OpticianJoiSchema.validate(maybeOptician);
  if (error) {
    return new OaValidationError('Optician', maybeOptician, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Optician;
}
