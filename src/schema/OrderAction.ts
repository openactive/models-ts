 ...
 * schema:OrderAction
 *
 * This differs from OrderAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OrderActionOrSubClass =
  | OrderAction
  ;
 ...
 */
export const OrderActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const OrderActionOrSubClassJoiSchema = Joi.alternatives().try([
  OrderActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeOrderAction = OrderAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateOrderAction(maybeOrderAction: unknown): OrderAction | OaValidationError {
  const { value, error } = OrderActionJoiSchema.validate(maybeOrderAction);
  if (error) {
    return new OaValidationError('OrderAction', maybeOrderAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OrderAction;
}
