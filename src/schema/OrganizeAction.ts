 ...
 * schema:OrganizeAction
 *
 * This differs from OrganizeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OrganizeActionOrSubClass =
  | OrganizeAction
  | schema.PlanActionOrSubClass
  | schema.BookmarkActionOrSubClass
  | schema.AllocateActionOrSubClass
  | schema.ApplyActionOrSubClass
  ;

/**
 * schema:OrganizeAction - Validation schema (w/ JOI)
 */
export const OrganizeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('OrganizeAction').required(),
 ...
 * schema:OrganizeAction - Validation schema (w/ JOI)
 *
 * This differs from OrganizeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OrganizeActionOrSubClassJoiSchema = Joi.alternatives().try([
  OrganizeActionJoiSchema,
  Joi.lazy(() => schema.PlanActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.BookmarkActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AllocateActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.ApplyActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:OrganizeAction.
 *
 * If some data has a structure which matches a schema:OrganizeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOrganizeAction = OrganizeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeOrganizeAction instanceof OaValidationError) {
 *   // From this point on, `maybeOrganizeAction` will have type `OaValidationError`
 *   const error = maybeOrganizeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeOrganizeAction` will have type `OrganizeAction`
 * const organizeAction = maybeOrganizeAction;
 * ```
 */
export function validateOrganizeAction(maybeOrganizeAction: unknown): OrganizeAction | OaValidationError {
  const { value, error } = OrganizeActionJoiSchema.validate(maybeOrganizeAction);
  if (error) {
    return new OaValidationError('OrganizeAction', maybeOrganizeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OrganizeAction;
}
