 ...
 * schema:OutletStore
 *
 * This differs from OutletStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OutletStoreOrSubClass =
  | OutletStore
  ;

/**
 * schema:OutletStore - Validation schema (w/ JOI)
 */
export const OutletStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('OutletStore').required(),
 ...
 * schema:OutletStore - Validation schema (w/ JOI)
 *
 * This differs from OutletStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const OutletStoreOrSubClassJoiSchema = Joi.alternatives().try([
  OutletStoreJoiSchema,
]);

/**
 * Runtime validator for schema:OutletStore.
 *
 * If some data has a structure which matches a schema:OutletStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeOutletStore = OutletStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeOutletStore instanceof OaValidationError) {
 *   // From this point on, `maybeOutletStore` will have type `OaValidationError`
 *   const error = maybeOutletStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeOutletStore` will have type `OutletStore`
 * const outletStore = maybeOutletStore;
 * ```
 */
export function validateOutletStore(maybeOutletStore: unknown): OutletStore | OaValidationError {
  const { value, error } = OutletStoreJoiSchema.validate(maybeOutletStore);
  if (error) {
    return new OaValidationError('OutletStore', maybeOutletStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OutletStore;
}
