...
export type OwnershipInfo = {
   ...
   */
  acquiredFrom?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
 ...
 * schema:OwnershipInfo
 *
 * This differs from OwnershipInfo because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type OwnershipInfoOrSubClass =
  | OwnershipInfo
  ;
 ...
 */
export const OwnershipInfoJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  acquiredFrom: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const OwnershipInfoOrSubClassJoiSchema = Joi.alternatives().try([
  OwnershipInfoJoiSchema,
]);
 ...
 * ```
 */
export function validateOwnershipInfo(maybeOwnershipInfo: unknown): OwnershipInfo | OaValidationError {
  const { value, error } = OwnershipInfoJoiSchema.validate(maybeOwnershipInfo);
  if (error) {
    return new OaValidationError('OwnershipInfo', maybeOwnershipInfo, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as OwnershipInfo;
}
