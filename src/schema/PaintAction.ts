 ...
 * schema:PaintAction
 *
 * This differs from PaintAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PaintActionOrSubClass =
  | PaintAction
  ;

/**
 * schema:PaintAction - Validation schema (w/ JOI)
 */
export const PaintActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('PaintAction').required(),
 ...
 * schema:PaintAction - Validation schema (w/ JOI)
 *
 * This differs from PaintActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PaintActionOrSubClassJoiSchema = Joi.alternatives().try([
  PaintActionJoiSchema,
]);

/**
 * Runtime validator for schema:PaintAction.
 *
 * If some data has a structure which matches a schema:PaintAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePaintAction` will have type `PaintAction`
 * const paintAction = maybePaintAction;
 * ```
 */
export function validatePaintAction(maybePaintAction: unknown): PaintAction | OaValidationError {
  const { value, error } = PaintActionJoiSchema.validate(maybePaintAction);
  if (error) {
    return new OaValidationError('PaintAction', maybePaintAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PaintAction;
}
