 ...
 * schema:Painting
 *
 * This differs from Painting because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PaintingOrSubClass =
  | Painting
  ;

/**
 * schema:Painting - Validation schema (w/ JOI)
 */
export const PaintingJoiSchema = Joi.object({
  '@type': Joi.string().valid('Painting').required(),
 ...
 * schema:Painting - Validation schema (w/ JOI)
 *
 * This differs from PaintingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PaintingOrSubClassJoiSchema = Joi.alternatives().try([
  PaintingJoiSchema,
]);

/**
 * Runtime validator for schema:Painting.
 *
 * If some data has a structure which matches a schema:Painting, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePainting = Painting.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePainting instanceof OaValidationError) {
 *   // From this point on, `maybePainting` will have type `OaValidationError`
 *   const error = maybePainting;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePainting` will have type `Painting`
 * const painting = maybePainting;
 * ```
 */
export function validatePainting(maybePainting: unknown): Painting | OaValidationError {
  const { value, error } = PaintingJoiSchema.validate(maybePainting);
  if (error) {
    return new OaValidationError('Painting', maybePainting, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Painting;
}
