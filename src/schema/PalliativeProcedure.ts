 ...
 * etc.
 */
export type PalliativeProcedureOrSubClass =
  | PalliativeProcedure
  ;
 ...
 */
export const PalliativeProcedureJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PalliativeProcedureOrSubClassJoiSchema = Joi.alternatives().try([
  PalliativeProcedureJoiSchema,
]);
 ...
 * ```
 */
export function validatePalliativeProcedure(maybePalliativeProcedure: unknown): PalliativeProcedure | OaValidationError {
  const { value, error } = PalliativeProcedureJoiSchema.validate(maybePalliativeProcedure);
  if (error) {
    return new OaValidationError('PalliativeProcedure', maybePalliativeProcedure, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PalliativeProcedure;
}
