 ...
 * schema:Park
 *
 * This differs from Park because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ParkOrSubClass =
  | Park
  ;

/**
 * schema:Park - Validation schema (w/ JOI)
 */
export const ParkJoiSchema = Joi.object({
  '@type': Joi.string().valid('Park').required(),
 ...
 * schema:Park - Validation schema (w/ JOI)
 *
 * This differs from ParkJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ParkOrSubClassJoiSchema = Joi.alternatives().try([
  ParkJoiSchema,
]);

/**
 * Runtime validator for schema:Park.
 *
 * If some data has a structure which matches a schema:Park, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePark = Park.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePark instanceof OaValidationError) {
 *   // From this point on, `maybePark` will have type `OaValidationError`
 *   const error = maybePark;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePark` will have type `Park`
 * const park = maybePark;
 * ```
 */
export function validatePark(maybePark: unknown): Park | OaValidationError {
  const { value, error } = ParkJoiSchema.validate(maybePark);
  if (error) {
    return new OaValidationError('Park', maybePark, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Park;
}
