 ...
 * schema:ParkingFacility
 *
 * This differs from ParkingFacility because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ParkingFacilityOrSubClass =
  | ParkingFacility
  ;

/**
 * schema:ParkingFacility - Validation schema (w/ JOI)
 */
export const ParkingFacilityJoiSchema = Joi.object({
  '@type': Joi.string().valid('ParkingFacility').required(),
 ...
 * schema:ParkingFacility - Validation schema (w/ JOI)
 *
 * This differs from ParkingFacilityJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ParkingFacilityOrSubClassJoiSchema = Joi.alternatives().try([
  ParkingFacilityJoiSchema,
]);

/**
 * Runtime validator for schema:ParkingFacility.
 *
 * If some data has a structure which matches a schema:ParkingFacility, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeParkingFacility = ParkingFacility.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeParkingFacility instanceof OaValidationError) {
 *   // From this point on, `maybeParkingFacility` will have type `OaValidationError`
 *   const error = maybeParkingFacility;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeParkingFacility` will have type `ParkingFacility`
 * const parkingFacility = maybeParkingFacility;
 * ```
 */
export function validateParkingFacility(maybeParkingFacility: unknown): ParkingFacility | OaValidationError {
  const { value, error } = ParkingFacilityJoiSchema.validate(maybeParkingFacility);
  if (error) {
    return new OaValidationError('ParkingFacility', maybeParkingFacility, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ParkingFacility;
}
