 ...
 * schema:PathologyTest
 *
 * This differs from PathologyTest because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PathologyTestOrSubClass =
  | PathologyTest
  ;
 ...
 */
export const PathologyTestJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PathologyTestOrSubClassJoiSchema = Joi.alternatives().try([
  PathologyTestJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybePathologyTest = PathologyTest.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validatePathologyTest(maybePathologyTest: unknown): PathologyTest | OaValidationError {
  const { value, error } = PathologyTestJoiSchema.validate(maybePathologyTest);
  if (error) {
    return new OaValidationError('PathologyTest', maybePathologyTest, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PathologyTest;
}
