 ...
 * schema:PawnShop
 *
 * This differs from PawnShop because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PawnShopOrSubClass =
  | PawnShop
  ;

/**
 * schema:PawnShop - Validation schema (w/ JOI)
 */
export const PawnShopJoiSchema = Joi.object({
  '@type': Joi.string().valid('PawnShop').required(),
 ...
 * schema:PawnShop - Validation schema (w/ JOI)
 *
 * This differs from PawnShopJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PawnShopOrSubClassJoiSchema = Joi.alternatives().try([
  PawnShopJoiSchema,
]);

/**
 * Runtime validator for schema:PawnShop.
 *
 * If some data has a structure which matches a schema:PawnShop, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePawnShop = PawnShop.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePawnShop instanceof OaValidationError) {
 *   // From this point on, `maybePawnShop` will have type `OaValidationError`
 *   const error = maybePawnShop;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePawnShop` will have type `PawnShop`
 * const pawnShop = maybePawnShop;
 * ```
 */
export function validatePawnShop(maybePawnShop: unknown): PawnShop | OaValidationError {
  const { value, error } = PawnShopJoiSchema.validate(maybePawnShop);
  if (error) {
    return new OaValidationError('PawnShop', maybePawnShop, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PawnShop;
}
