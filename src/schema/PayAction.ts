 ...
 * schema:PayAction
 *
 * This differs from PayAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PayActionOrSubClass =
  | PayAction
  ;

/**
 * schema:PayAction - Validation schema (w/ JOI)
 */
export const PayActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('PayAction').required(),
 ...
 * schema:PayAction - Validation schema (w/ JOI)
 *
 * This differs from PayActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PayActionOrSubClassJoiSchema = Joi.alternatives().try([
  PayActionJoiSchema,
]);

/**
 * Runtime validator for schema:PayAction.
 *
 * If some data has a structure which matches a schema:PayAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePayAction = PayAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePayAction instanceof OaValidationError) {
 *   // From this point on, `maybePayAction` will have type `OaValidationError`
 *   const error = maybePayAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePayAction` will have type `PayAction`
 * const payAction = maybePayAction;
 * ```
 */
export function validatePayAction(maybePayAction: unknown): PayAction | OaValidationError {
  const { value, error } = PayActionJoiSchema.validate(maybePayAction);
  if (error) {
    return new OaValidationError('PayAction', maybePayAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PayAction;
}
