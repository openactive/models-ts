 ...
 * schema:PaymentCard
 *
 * This differs from PaymentCard because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PaymentCardOrSubClass =
  | PaymentCard
 ...
 */
export const PaymentCardJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PaymentCardOrSubClassJoiSchema = Joi.alternatives().try([
  PaymentCardJoiSchema,
 ...
 * ```
 */
export function validatePaymentCard(maybePaymentCard: unknown): PaymentCard | OaValidationError {
  const { value, error } = PaymentCardJoiSchema.validate(maybePaymentCard);
  if (error) {
    return new OaValidationError('PaymentCard', maybePaymentCard, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PaymentCard;
}
