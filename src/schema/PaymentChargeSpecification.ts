 ...
 */
export type PaymentChargeSpecification = {
 ...
 * schema:PaymentChargeSpecification
 *
 * This differs from PaymentChargeSpecification because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PaymentChargeSpecificationOrSubClass =
 ...
 */
export const PaymentChargeSpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PaymentChargeSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  PaymentChargeSpecificationJoiSchema,
]);
 ...
 * ```
 */
export function validatePaymentChargeSpecification(maybePaymentChargeSpecification: unknown): PaymentChargeSpecification | OaValidationError {
  const { value, error } = PaymentChargeSpecificationJoiSchema.validate(maybePaymentChargeSpecification);
  if (error) {
    return new OaValidationError('PaymentChargeSpecification', maybePaymentChargeSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PaymentChargeSpecification;
}
