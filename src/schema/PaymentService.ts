 ...
 * schema:PaymentService
 *
 * This differs from PaymentService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PaymentServiceOrSubClass =
  | PaymentService
  ;
 ...
 */
export const PaymentServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PaymentServiceOrSubClassJoiSchema = Joi.alternatives().try([
  PaymentServiceJoiSchema,
]);
 ...
 * ```
 */
export function validatePaymentService(maybePaymentService: unknown): PaymentService | OaValidationError {
  const { value, error } = PaymentServiceJoiSchema.validate(maybePaymentService);
  if (error) {
    return new OaValidationError('PaymentService', maybePaymentService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PaymentService;
}
