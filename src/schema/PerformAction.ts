...
export type PerformAction = {
   ...
   */
  entertainmentBusiness?: schema.EntertainmentBusinessOrSubClass | string;
  /**
 ...
 * schema:PerformAction
 *
 * This differs from PerformAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PerformActionOrSubClass =
  | PerformAction
  ;

/**
 * schema:PerformAction - Validation schema (w/ JOI)
 */
export const PerformActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('PerformAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  entertainmentBusiness: Joi.alternatives().try(Joi.lazy(() => schema.EntertainmentBusinessOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:PerformAction - Validation schema (w/ JOI)
 *
 * This differs from PerformActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PerformActionOrSubClassJoiSchema = Joi.alternatives().try([
  PerformActionJoiSchema,
]);

/**
 * Runtime validator for schema:PerformAction.
 *
 * If some data has a structure which matches a schema:PerformAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePerformAction = PerformAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePerformAction instanceof OaValidationError) {
 *   // From this point on, `maybePerformAction` will have type `OaValidationError`
 *   const error = maybePerformAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePerformAction` will have type `PerformAction`
 * const performAction = maybePerformAction;
 * ```
 */
export function validatePerformAction(maybePerformAction: unknown): PerformAction | OaValidationError {
  const { value, error } = PerformActionJoiSchema.validate(maybePerformAction);
  if (error) {
    return new OaValidationError('PerformAction', maybePerformAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PerformAction;
}
