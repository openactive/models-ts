 ...
 * etc.
 */
export type PerformanceRoleOrSubClass =
  | PerformanceRole
  ;
 ...
 */
export const PerformanceRoleJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PerformanceRoleOrSubClassJoiSchema = Joi.alternatives().try([
  PerformanceRoleJoiSchema,
]);
 ...
 * ```
 */
export function validatePerformanceRole(maybePerformanceRole: unknown): PerformanceRole | OaValidationError {
  const { value, error } = PerformanceRoleJoiSchema.validate(maybePerformanceRole);
  if (error) {
    return new OaValidationError('PerformanceRole', maybePerformanceRole, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PerformanceRole;
}
