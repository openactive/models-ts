 ...
 */
export type PerformingArtsTheater = {
 ...
 * schema:PerformingArtsTheater
 *
 * This differs from PerformingArtsTheater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PerformingArtsTheaterOrSubClass =
  | PerformingArtsTheater
  ;

/**
 * schema:PerformingArtsTheater - Validation schema (w/ JOI)
 */
export const PerformingArtsTheaterJoiSchema = Joi.object({
  '@type': Joi.string().valid('PerformingArtsTheater').required(),
 ...
 * schema:PerformingArtsTheater - Validation schema (w/ JOI)
 *
 * This differs from PerformingArtsTheaterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PerformingArtsTheaterOrSubClassJoiSchema = Joi.alternatives().try([
  PerformingArtsTheaterJoiSchema,
]);

/**
 * Runtime validator for schema:PerformingArtsTheater.
 *
 * If some data has a structure which matches a schema:PerformingArtsTheater, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePerformingArtsTheater = PerformingArtsTheater.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePerformingArtsTheater instanceof OaValidationError) {
 *   // From this point on, `maybePerformingArtsTheater` will have type `OaValidationError`
 *   const error = maybePerformingArtsTheater;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePerformingArtsTheater` will have type `PerformingArtsTheater`
 * const performingArtsTheater = maybePerformingArtsTheater;
 * ```
 */
export function validatePerformingArtsTheater(maybePerformingArtsTheater: unknown): PerformingArtsTheater | OaValidationError {
  const { value, error } = PerformingArtsTheaterJoiSchema.validate(maybePerformingArtsTheater);
  if (error) {
    return new OaValidationError('PerformingArtsTheater', maybePerformingArtsTheater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PerformingArtsTheater;
}
