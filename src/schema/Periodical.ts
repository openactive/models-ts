 ...
 * schema:Periodical
 *
 * This differs from Periodical because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PeriodicalOrSubClass =
  | Periodical
  | schema.ComicSeriesOrSubClass
  | schema.NewspaperOrSubClass
  ;

/**
 * schema:Periodical - Validation schema (w/ JOI)
 */
export const PeriodicalJoiSchema = Joi.object({
  '@type': Joi.string().valid('Periodical').required(),
 ...
 * schema:Periodical - Validation schema (w/ JOI)
 *
 * This differs from PeriodicalJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PeriodicalOrSubClassJoiSchema = Joi.alternatives().try([
  PeriodicalJoiSchema,
  Joi.lazy(() => schema.ComicSeriesOrSubClassJoiSchema),
  Joi.lazy(() => schema.NewspaperOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Periodical.
 *
 * If some data has a structure which matches a schema:Periodical, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePeriodical` will have type `Periodical`
 * const periodical = maybePeriodical;
 * ```
 */
export function validatePeriodical(maybePeriodical: unknown): Periodical | OaValidationError {
  const { value, error } = PeriodicalJoiSchema.validate(maybePeriodical);
  if (error) {
    return new OaValidationError('Periodical', maybePeriodical, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Periodical;
}
