 ...
 * schema:PetStore
 *
 * This differs from PetStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PetStoreOrSubClass =
  | PetStore
  ;

/**
 * schema:PetStore - Validation schema (w/ JOI)
 */
export const PetStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('PetStore').required(),
 ...
 * schema:PetStore - Validation schema (w/ JOI)
 *
 * This differs from PetStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PetStoreOrSubClassJoiSchema = Joi.alternatives().try([
  PetStoreJoiSchema,
]);

/**
 * Runtime validator for schema:PetStore.
 *
 * If some data has a structure which matches a schema:PetStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePetStore = PetStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePetStore instanceof OaValidationError) {
 *   // From this point on, `maybePetStore` will have type `OaValidationError`
 *   const error = maybePetStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePetStore` will have type `PetStore`
 * const petStore = maybePetStore;
 * ```
 */
export function validatePetStore(maybePetStore: unknown): PetStore | OaValidationError {
  const { value, error } = PetStoreJoiSchema.validate(maybePetStore);
  if (error) {
    return new OaValidationError('PetStore', maybePetStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PetStore;
}
