 ...
 * schema:Pharmacy
 *
 * This differs from Pharmacy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PharmacyOrSubClass =
  | Pharmacy
  ;

/**
 * schema:Pharmacy - Validation schema (w/ JOI)
 */
export const PharmacyJoiSchema = Joi.object({
  '@type': Joi.string().valid('Pharmacy').required(),
 ...
 * schema:Pharmacy - Validation schema (w/ JOI)
 *
 * This differs from PharmacyJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PharmacyOrSubClassJoiSchema = Joi.alternatives().try([
  PharmacyJoiSchema,
]);

/**
 * Runtime validator for schema:Pharmacy.
 *
 * If some data has a structure which matches a schema:Pharmacy, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePharmacy = Pharmacy.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePharmacy instanceof OaValidationError) {
 *   // From this point on, `maybePharmacy` will have type `OaValidationError`
 *   const error = maybePharmacy;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePharmacy` will have type `Pharmacy`
 * const pharmacy = maybePharmacy;
 * ```
 */
export function validatePharmacy(maybePharmacy: unknown): Pharmacy | OaValidationError {
  const { value, error } = PharmacyJoiSchema.validate(maybePharmacy);
  if (error) {
    return new OaValidationError('Pharmacy', maybePharmacy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Pharmacy;
}
