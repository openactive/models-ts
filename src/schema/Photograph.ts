 ...
 * schema:Photograph
 *
 * This differs from Photograph because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PhotographOrSubClass =
  | Photograph
  ;

/**
 * schema:Photograph - Validation schema (w/ JOI)
 */
export const PhotographJoiSchema = Joi.object({
  '@type': Joi.string().valid('Photograph').required(),
 ...
 * schema:Photograph - Validation schema (w/ JOI)
 *
 * This differs from PhotographJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PhotographOrSubClassJoiSchema = Joi.alternatives().try([
  PhotographJoiSchema,
]);

/**
 * Runtime validator for schema:Photograph.
 *
 * If some data has a structure which matches a schema:Photograph, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePhotograph = Photograph.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePhotograph instanceof OaValidationError) {
 *   // From this point on, `maybePhotograph` will have type `OaValidationError`
 *   const error = maybePhotograph;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePhotograph` will have type `Photograph`
 * const photograph = maybePhotograph;
 * ```
 */
export function validatePhotograph(maybePhotograph: unknown): Photograph | OaValidationError {
  const { value, error } = PhotographJoiSchema.validate(maybePhotograph);
  if (error) {
    return new OaValidationError('Photograph', maybePhotograph, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Photograph;
}
