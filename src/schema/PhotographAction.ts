 ...
 * schema:PhotographAction
 *
 * This differs from PhotographAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PhotographActionOrSubClass =
  | PhotographAction
  ;

/**
 * schema:PhotographAction - Validation schema (w/ JOI)
 */
export const PhotographActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('PhotographAction').required(),
 ...
 * schema:PhotographAction - Validation schema (w/ JOI)
 *
 * This differs from PhotographActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PhotographActionOrSubClassJoiSchema = Joi.alternatives().try([
  PhotographActionJoiSchema,
]);

/**
 * Runtime validator for schema:PhotographAction.
 *
 * If some data has a structure which matches a schema:PhotographAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePhotographAction = PhotographAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePhotographAction instanceof OaValidationError) {
 *   // From this point on, `maybePhotographAction` will have type `OaValidationError`
 *   const error = maybePhotographAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePhotographAction` will have type `PhotographAction`
 * const photographAction = maybePhotographAction;
 * ```
 */
export function validatePhotographAction(maybePhotographAction: unknown): PhotographAction | OaValidationError {
  const { value, error } = PhotographActionJoiSchema.validate(maybePhotographAction);
  if (error) {
    return new OaValidationError('PhotographAction', maybePhotographAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PhotographAction;
}
