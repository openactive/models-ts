 ...
 * etc.
 */
export type PhysicalActivityOrSubClass =
  | PhysicalActivity
 ...
 */
export const PhysicalActivityJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PhysicalActivityOrSubClassJoiSchema = Joi.alternatives().try([
  PhysicalActivityJoiSchema,
 ...
 *
 * ```ts
 * const maybePhysicalActivity = PhysicalActivity.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validatePhysicalActivity(maybePhysicalActivity: unknown): PhysicalActivity | OaValidationError {
  const { value, error } = PhysicalActivityJoiSchema.validate(maybePhysicalActivity);
  if (error) {
    return new OaValidationError('PhysicalActivity', maybePhysicalActivity, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PhysicalActivity;
}
