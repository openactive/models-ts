 ...
 * schema:PhysicalExam
 *
 * This differs from PhysicalExam because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PhysicalExamOrSubClass =
  | PhysicalExam
  ;
 ...
 */
export const PhysicalExamJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PhysicalExamOrSubClassJoiSchema = Joi.alternatives().try([
  PhysicalExamJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybePhysicalExam = PhysicalExam.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validatePhysicalExam(maybePhysicalExam: unknown): PhysicalExam | OaValidationError {
  const { value, error } = PhysicalExamJoiSchema.validate(maybePhysicalExam);
  if (error) {
    return new OaValidationError('PhysicalExam', maybePhysicalExam, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PhysicalExam;
}
