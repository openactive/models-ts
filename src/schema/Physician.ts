...
export type Physician = {
   ...
   */
  hospitalAffiliation?: schema.HospitalOrSubClass | string;
  /**
 ...
 * schema:Physician
 *
 * This differs from Physician because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PhysicianOrSubClass =
  | Physician
  ;

/**
 * schema:Physician - Validation schema (w/ JOI)
 */
export const PhysicianJoiSchema = Joi.object({
  '@type': Joi.string().valid('Physician').required(),
  ...
  hospitalAffiliation: Joi.alternatives().try(Joi.lazy(() => schema.HospitalOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:Physician - Validation schema (w/ JOI)
 *
 * This differs from PhysicianJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PhysicianOrSubClassJoiSchema = Joi.alternatives().try([
  PhysicianJoiSchema,
]);

/**
 * Runtime validator for schema:Physician.
 *
 * If some data has a structure which matches a schema:Physician, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePhysician = validatePhysician(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePhysician instanceof OaValidationError) {
 *   // From this point on, `maybePhysician` will have type `OaValidationError`
 *   const error = maybePhysician;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePhysician` will have type `Physician`
 * const physician = maybePhysician;
 * ```
 */
export function validatePhysician(maybePhysician: unknown): Physician | OaValidationError {
  const { value, error } = PhysicianJoiSchema.validate(maybePhysician);
  if (error) {
    return new OaValidationError('Physician', maybePhysician, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Physician;
}
