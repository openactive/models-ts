 ...
 * etc.
 */
export type PlaceOrSubClass =
  | Place
  | schema.ResidenceOrSubClass
  | schema.AccommodationOrSubClass
  | schema.LocalBusinessOrSubClass
  | schema.AdministrativeAreaOrSubClass
  | schema.CivicStructureOrSubClass
  | schema.TouristAttractionOrSubClass
  | schema.TouristDestinationOrSubClass
  | schema.LandmarksOrHistoricalBuildingsOrSubClass
  | schema.LandformOrSubClass
  ;

/**
 * schema:Place - Validation schema (w/ JOI)
 */
export const PlaceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PlaceOrSubClassJoiSchema = Joi.alternatives().try([
  PlaceJoiSchema,
 ...
 * ```
 */
export function validatePlace(maybePlace: unknown): Place | OaValidationError {
  const { value, error } = PlaceJoiSchema.validate(maybePlace);
  if (error) {
    return new OaValidationError('Place', maybePlace, error);
  }
