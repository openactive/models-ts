 ...
 * schema:PlaceOfWorship
 *
 * This differs from PlaceOfWorship because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PlaceOfWorshipOrSubClass =
  | PlaceOfWorship
  | schema.SynagogueOrSubClass
  | schema.HinduTempleOrSubClass
  | schema.BuddhistTempleOrSubClass
  | schema.MosqueOrSubClass
  | schema.ChurchOrSubClass
  ;

/**
 * schema:PlaceOfWorship - Validation schema (w/ JOI)
 */
export const PlaceOfWorshipJoiSchema = Joi.object({
  '@type': Joi.string().valid('PlaceOfWorship').required(),
 ...
 * schema:PlaceOfWorship - Validation schema (w/ JOI)
 *
 * This differs from PlaceOfWorshipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PlaceOfWorshipOrSubClassJoiSchema = Joi.alternatives().try([
  PlaceOfWorshipJoiSchema,
  Joi.lazy(() => schema.SynagogueOrSubClassJoiSchema),
  Joi.lazy(() => schema.HinduTempleOrSubClassJoiSchema),
  Joi.lazy(() => schema.BuddhistTempleOrSubClassJoiSchema),
  Joi.lazy(() => schema.MosqueOrSubClassJoiSchema),
  Joi.lazy(() => schema.ChurchOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:PlaceOfWorship.
 *
 * If some data has a structure which matches a schema:PlaceOfWorship, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePlaceOfWorship` will have type `PlaceOfWorship`
 * const placeOfWorship = maybePlaceOfWorship;
 * ```
 */
export function validatePlaceOfWorship(maybePlaceOfWorship: unknown): PlaceOfWorship | OaValidationError {
  const { value, error } = PlaceOfWorshipJoiSchema.validate(maybePlaceOfWorship);
  if (error) {
    return new OaValidationError('PlaceOfWorship', maybePlaceOfWorship, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PlaceOfWorship;
}
