 ...
 * schema:PlanAction
 *
 * This differs from PlanAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PlanActionOrSubClass =
  | PlanAction
  | schema.ReserveActionOrSubClass
  | schema.CancelActionOrSubClass
  | schema.ScheduleActionOrSubClass
  ;

/**
 * schema:PlanAction - Validation schema (w/ JOI)
 */
export const PlanActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('PlanAction').required(),
 ...
 * schema:PlanAction - Validation schema (w/ JOI)
 *
 * This differs from PlanActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PlanActionOrSubClassJoiSchema = Joi.alternatives().try([
  PlanActionJoiSchema,
  Joi.lazy(() => schema.ReserveActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.CancelActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.ScheduleActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:PlanAction.
 *
 * If some data has a structure which matches a schema:PlanAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePlanAction = PlanAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePlanAction instanceof OaValidationError) {
 *   // From this point on, `maybePlanAction` will have type `OaValidationError`
 *   const error = maybePlanAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePlanAction` will have type `PlanAction`
 * const planAction = maybePlanAction;
 * ```
 */
export function validatePlanAction(maybePlanAction: unknown): PlanAction | OaValidationError {
  const { value, error } = PlanActionJoiSchema.validate(maybePlanAction);
  if (error) {
    return new OaValidationError('PlanAction', maybePlanAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PlanAction;
}
