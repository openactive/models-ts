 ...
 * schema:Play
 *
 * This differs from Play because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PlayOrSubClass =
  | Play
  ;

/**
 * schema:Play - Validation schema (w/ JOI)
 */
export const PlayJoiSchema = Joi.object({
  '@type': Joi.string().valid('Play').required(),
 ...
 * schema:Play - Validation schema (w/ JOI)
 *
 * This differs from PlayJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PlayOrSubClassJoiSchema = Joi.alternatives().try([
  PlayJoiSchema,
]);

/**
 * Runtime validator for schema:Play.
 *
 * If some data has a structure which matches a schema:Play, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePlay = Play.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePlay instanceof OaValidationError) {
 *   // From this point on, `maybePlay` will have type `OaValidationError`
 *   const error = maybePlay;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePlay` will have type `Play`
 * const play = maybePlay;
 * ```
 */
export function validatePlay(maybePlay: unknown): Play | OaValidationError {
  const { value, error } = PlayJoiSchema.validate(maybePlay);
  if (error) {
    return new OaValidationError('Play', maybePlay, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Play;
}
