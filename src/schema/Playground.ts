 ...
 * schema:Playground
 *
 * This differs from Playground because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PlaygroundOrSubClass =
  | Playground
  ;

/**
 * schema:Playground - Validation schema (w/ JOI)
 */
export const PlaygroundJoiSchema = Joi.object({
  '@type': Joi.string().valid('Playground').required(),
 ...
 * schema:Playground - Validation schema (w/ JOI)
 *
 * This differs from PlaygroundJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PlaygroundOrSubClassJoiSchema = Joi.alternatives().try([
  PlaygroundJoiSchema,
]);

/**
 * Runtime validator for schema:Playground.
 *
 * If some data has a structure which matches a schema:Playground, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePlayground` will have type `Playground`
 * const playground = maybePlayground;
 * ```
 */
export function validatePlayground(maybePlayground: unknown): Playground | OaValidationError {
  const { value, error } = PlaygroundJoiSchema.validate(maybePlayground);
  if (error) {
    return new OaValidationError('Playground', maybePlayground, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Playground;
}
