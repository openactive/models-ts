 ...
 * schema:Plumber
 *
 * This differs from Plumber because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PlumberOrSubClass =
  | Plumber
  ;

/**
 * schema:Plumber - Validation schema (w/ JOI)
 */
export const PlumberJoiSchema = Joi.object({
  '@type': Joi.string().valid('Plumber').required(),
 ...
 * schema:Plumber - Validation schema (w/ JOI)
 *
 * This differs from PlumberJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PlumberOrSubClassJoiSchema = Joi.alternatives().try([
  PlumberJoiSchema,
]);

/**
 * Runtime validator for schema:Plumber.
 *
 * If some data has a structure which matches a schema:Plumber, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePlumber = Plumber.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePlumber instanceof OaValidationError) {
 *   // From this point on, `maybePlumber` will have type `OaValidationError`
 *   const error = maybePlumber;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePlumber` will have type `Plumber`
 * const plumber = maybePlumber;
 * ```
 */
export function validatePlumber(maybePlumber: unknown): Plumber | OaValidationError {
  const { value, error } = PlumberJoiSchema.validate(maybePlumber);
  if (error) {
    return new OaValidationError('Plumber', maybePlumber, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Plumber;
}
