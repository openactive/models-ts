 ...
 * schema:PodcastEpisode
 *
 * This differs from PodcastEpisode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PodcastEpisodeOrSubClass =
  | PodcastEpisode
  ;

/**
 * schema:PodcastEpisode - Validation schema (w/ JOI)
 */
export const PodcastEpisodeJoiSchema = Joi.object({
  '@type': Joi.string().valid('PodcastEpisode').required(),
 ...
 * schema:PodcastEpisode - Validation schema (w/ JOI)
 *
 * This differs from PodcastEpisodeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PodcastEpisodeOrSubClassJoiSchema = Joi.alternatives().try([
  PodcastEpisodeJoiSchema,
]);

/**
 * Runtime validator for schema:PodcastEpisode.
 *
 * If some data has a structure which matches a schema:PodcastEpisode, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePodcastEpisode = PodcastEpisode.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePodcastEpisode instanceof OaValidationError) {
 *   // From this point on, `maybePodcastEpisode` will have type `OaValidationError`
 *   const error = maybePodcastEpisode;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePodcastEpisode` will have type `PodcastEpisode`
 * const podcastEpisode = maybePodcastEpisode;
 * ```
 */
export function validatePodcastEpisode(maybePodcastEpisode: unknown): PodcastEpisode | OaValidationError {
  const { value, error } = PodcastEpisodeJoiSchema.validate(maybePodcastEpisode);
  if (error) {
    return new OaValidationError('PodcastEpisode', maybePodcastEpisode, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PodcastEpisode;
}
