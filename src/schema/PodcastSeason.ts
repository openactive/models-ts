 ...
 * schema:PodcastSeason
 *
 * This differs from PodcastSeason because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PodcastSeasonOrSubClass =
  | PodcastSeason
  ;

/**
 * schema:PodcastSeason - Validation schema (w/ JOI)
 */
export const PodcastSeasonJoiSchema = Joi.object({
  '@type': Joi.string().valid('PodcastSeason').required(),
 ...
 * schema:PodcastSeason - Validation schema (w/ JOI)
 *
 * This differs from PodcastSeasonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PodcastSeasonOrSubClassJoiSchema = Joi.alternatives().try([
  PodcastSeasonJoiSchema,
]);

/**
 * Runtime validator for schema:PodcastSeason.
 *
 * If some data has a structure which matches a schema:PodcastSeason, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePodcastSeason = PodcastSeason.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePodcastSeason instanceof OaValidationError) {
 *   // From this point on, `maybePodcastSeason` will have type `OaValidationError`
 *   const error = maybePodcastSeason;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePodcastSeason` will have type `PodcastSeason`
 * const podcastSeason = maybePodcastSeason;
 * ```
 */
export function validatePodcastSeason(maybePodcastSeason: unknown): PodcastSeason | OaValidationError {
  const { value, error } = PodcastSeasonJoiSchema.validate(maybePodcastSeason);
  if (error) {
    return new OaValidationError('PodcastSeason', maybePodcastSeason, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PodcastSeason;
}
