 ...
 * schema:PodcastSeries
 *
 * This differs from PodcastSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PodcastSeriesOrSubClass =
  | PodcastSeries
  ;

/**
 * schema:PodcastSeries - Validation schema (w/ JOI)
 */
export const PodcastSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('PodcastSeries').required(),
 ...
 * schema:PodcastSeries - Validation schema (w/ JOI)
 *
 * This differs from PodcastSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PodcastSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  PodcastSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:PodcastSeries.
 *
 * If some data has a structure which matches a schema:PodcastSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePodcastSeries = PodcastSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePodcastSeries instanceof OaValidationError) {
 *   // From this point on, `maybePodcastSeries` will have type `OaValidationError`
 *   const error = maybePodcastSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePodcastSeries` will have type `PodcastSeries`
 * const podcastSeries = maybePodcastSeries;
 * ```
 */
export function validatePodcastSeries(maybePodcastSeries: unknown): PodcastSeries | OaValidationError {
  const { value, error } = PodcastSeriesJoiSchema.validate(maybePodcastSeries);
  if (error) {
    return new OaValidationError('PodcastSeries', maybePodcastSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PodcastSeries;
}
