 ...
 * schema:PoliceStation
 *
 * This differs from PoliceStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PoliceStationOrSubClass =
  | PoliceStation
  ;

/**
 * schema:PoliceStation - Validation schema (w/ JOI)
 */
export const PoliceStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('PoliceStation').required(),
 ...
 * schema:PoliceStation - Validation schema (w/ JOI)
 *
 * This differs from PoliceStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PoliceStationOrSubClassJoiSchema = Joi.alternatives().try([
  PoliceStationJoiSchema,
]);

/**
 * Runtime validator for schema:PoliceStation.
 *
 * If some data has a structure which matches a schema:PoliceStation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePoliceStation = PoliceStation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePoliceStation instanceof OaValidationError) {
 *   // From this point on, `maybePoliceStation` will have type `OaValidationError`
 *   const error = maybePoliceStation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePoliceStation` will have type `PoliceStation`
 * const policeStation = maybePoliceStation;
 * ```
 */
export function validatePoliceStation(maybePoliceStation: unknown): PoliceStation | OaValidationError {
  const { value, error } = PoliceStationJoiSchema.validate(maybePoliceStation);
  if (error) {
    return new OaValidationError('PoliceStation', maybePoliceStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PoliceStation;
}
