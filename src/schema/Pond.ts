 ...
 * schema:Pond
 *
 * This differs from Pond because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PondOrSubClass =
  | Pond
  ;

/**
 * schema:Pond - Validation schema (w/ JOI)
 */
export const PondJoiSchema = Joi.object({
  '@type': Joi.string().valid('Pond').required(),
 ...
 * schema:Pond - Validation schema (w/ JOI)
 *
 * This differs from PondJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PondOrSubClassJoiSchema = Joi.alternatives().try([
  PondJoiSchema,
]);

/**
 * Runtime validator for schema:Pond.
 *
 * If some data has a structure which matches a schema:Pond, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePond = Pond.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePond instanceof OaValidationError) {
 *   // From this point on, `maybePond` will have type `OaValidationError`
 *   const error = maybePond;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePond` will have type `Pond`
 * const pond = maybePond;
 * ```
 */
export function validatePond(maybePond: unknown): Pond | OaValidationError {
  const { value, error } = PondJoiSchema.validate(maybePond);
  if (error) {
    return new OaValidationError('Pond', maybePond, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Pond;
}
