 ...
 * schema:PostOffice
 *
 * This differs from PostOffice because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PostOfficeOrSubClass =
  | PostOffice
  ;

/**
 * schema:PostOffice - Validation schema (w/ JOI)
 */
export const PostOfficeJoiSchema = Joi.object({
  '@type': Joi.string().valid('PostOffice').required(),
 ...
 * schema:PostOffice - Validation schema (w/ JOI)
 *
 * This differs from PostOfficeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PostOfficeOrSubClassJoiSchema = Joi.alternatives().try([
  PostOfficeJoiSchema,
]);

/**
 * Runtime validator for schema:PostOffice.
 *
 * If some data has a structure which matches a schema:PostOffice, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePostOffice` will have type `PostOffice`
 * const postOffice = maybePostOffice;
 * ```
 */
export function validatePostOffice(maybePostOffice: unknown): PostOffice | OaValidationError {
  const { value, error } = PostOfficeJoiSchema.validate(maybePostOffice);
  if (error) {
    return new OaValidationError('PostOffice', maybePostOffice, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PostOffice;
}
