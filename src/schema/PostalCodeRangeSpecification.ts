 ...
 * schema:PostalCodeRangeSpecification
 *
 * This differs from PostalCodeRangeSpecification because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PostalCodeRangeSpecificationOrSubClass =
 ...
 */
export const PostalCodeRangeSpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PostalCodeRangeSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  PostalCodeRangeSpecificationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybePostalCodeRangeSpecification = validatePostalCodeRangeSpecification(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validatePostalCodeRangeSpecification(maybePostalCodeRangeSpecification: unknown): PostalCodeRangeSpecification | OaValidationError {
  const { value, error } = PostalCodeRangeSpecificationJoiSchema.validate(maybePostalCodeRangeSpecification);
  if (error) {
    return new OaValidationError('PostalCodeRangeSpecification', maybePostalCodeRangeSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PostalCodeRangeSpecification;
}
