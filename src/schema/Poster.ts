 ...
 * schema:Poster
 *
 * This differs from Poster because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PosterOrSubClass =
  | Poster
  ;
 ...
 */
export const PosterJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PosterOrSubClassJoiSchema = Joi.alternatives().try([
  PosterJoiSchema,
]);
 ...
 * ```
 */
export function validatePoster(maybePoster: unknown): Poster | OaValidationError {
  const { value, error } = PosterJoiSchema.validate(maybePoster);
  if (error) {
    return new OaValidationError('Poster', maybePoster, error);
  }
