 ...
 * etc.
 */
export type PreOrderActionOrSubClass =
  | PreOrderAction
  ;
 ...
 */
export const PreOrderActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PreOrderActionOrSubClassJoiSchema = Joi.alternatives().try([
  PreOrderActionJoiSchema,
]);
 ...
 * ```
 */
export function validatePreOrderAction(maybePreOrderAction: unknown): PreOrderAction | OaValidationError {
  const { value, error } = PreOrderActionJoiSchema.validate(maybePreOrderAction);
  if (error) {
    return new OaValidationError('PreOrderAction', maybePreOrderAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PreOrderAction;
}
