 ...
 * schema:PrependAction
 *
 * This differs from PrependAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PrependActionOrSubClass =
  | PrependAction
  ;

/**
 * schema:PrependAction - Validation schema (w/ JOI)
 */
export const PrependActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('PrependAction').required(),
 ...
 * schema:PrependAction - Validation schema (w/ JOI)
 *
 * This differs from PrependActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PrependActionOrSubClassJoiSchema = Joi.alternatives().try([
  PrependActionJoiSchema,
]);

/**
 * Runtime validator for schema:PrependAction.
 *
 * If some data has a structure which matches a schema:PrependAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePrependAction = PrependAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePrependAction instanceof OaValidationError) {
 *   // From this point on, `maybePrependAction` will have type `OaValidationError`
 *   const error = maybePrependAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePrependAction` will have type `PrependAction`
 * const prependAction = maybePrependAction;
 * ```
 */
export function validatePrependAction(maybePrependAction: unknown): PrependAction | OaValidationError {
  const { value, error } = PrependActionJoiSchema.validate(maybePrependAction);
  if (error) {
    return new OaValidationError('PrependAction', maybePrependAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PrependAction;
}
