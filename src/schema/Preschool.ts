 ...
 * schema:Preschool
 *
 * This differs from Preschool because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PreschoolOrSubClass =
  | Preschool
  ;

/**
 * schema:Preschool - Validation schema (w/ JOI)
 */
export const PreschoolJoiSchema = Joi.object({
  '@type': Joi.string().valid('Preschool').required(),
 ...
 * schema:Preschool - Validation schema (w/ JOI)
 *
 * This differs from PreschoolJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PreschoolOrSubClassJoiSchema = Joi.alternatives().try([
  PreschoolJoiSchema,
]);

/**
 * Runtime validator for schema:Preschool.
 *
 * If some data has a structure which matches a schema:Preschool, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePreschool` will have type `Preschool`
 * const preschool = maybePreschool;
 * ```
 */
export function validatePreschool(maybePreschool: unknown): Preschool | OaValidationError {
  const { value, error } = PreschoolJoiSchema.validate(maybePreschool);
  if (error) {
    return new OaValidationError('Preschool', maybePreschool, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Preschool;
}
