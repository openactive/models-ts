 ...
 */
export type PresentationDigitalDocument = {
 ...
 * schema:PresentationDigitalDocument
 *
 * This differs from PresentationDigitalDocument because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PresentationDigitalDocumentOrSubClass =
  | PresentationDigitalDocument
  ;

/**
 * schema:PresentationDigitalDocument - Validation schema (w/ JOI)
 */
export const PresentationDigitalDocumentJoiSchema = Joi.object({
  '@type': Joi.string().valid('PresentationDigitalDocument').required(),
 ...
 * schema:PresentationDigitalDocument - Validation schema (w/ JOI)
 *
 * This differs from PresentationDigitalDocumentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PresentationDigitalDocumentOrSubClassJoiSchema = Joi.alternatives().try([
  PresentationDigitalDocumentJoiSchema,
]);

/**
 * Runtime validator for schema:PresentationDigitalDocument.
 *
 * If some data has a structure which matches a schema:PresentationDigitalDocument, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePresentationDigitalDocument` will have type `PresentationDigitalDocument`
 * const presentationDigitalDocument = maybePresentationDigitalDocument;
 * ```
 */
export function validatePresentationDigitalDocument(maybePresentationDigitalDocument: unknown): PresentationDigitalDocument | OaValidationError {
  const { value, error } = PresentationDigitalDocumentJoiSchema.validate(maybePresentationDigitalDocument);
  if (error) {
    return new OaValidationError('PresentationDigitalDocument', maybePresentationDigitalDocument, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PresentationDigitalDocument;
}
