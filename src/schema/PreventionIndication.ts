 ...
 */
export type PreventionIndication = {
 ...
 * schema:PreventionIndication
 *
 * This differs from PreventionIndication because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PreventionIndicationOrSubClass =
 ...
 */
export const PreventionIndicationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const PreventionIndicationOrSubClassJoiSchema = Joi.alternatives().try([
  PreventionIndicationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybePreventionIndication = PreventionIndication.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validatePreventionIndication(maybePreventionIndication: unknown): PreventionIndication | OaValidationError {
  const { value, error } = PreventionIndicationJoiSchema.validate(maybePreventionIndication);
  if (error) {
    return new OaValidationError('PreventionIndication', maybePreventionIndication, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PreventionIndication;
}
