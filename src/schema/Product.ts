 ...
 * schema:Product
 *
 * This differs from Product because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ProductOrSubClass =
  | Product
  | oa.FacilityUseOrSubClass
  | schema.IndividualProductOrSubClass
  | schema.SomeProductsOrSubClass
  | schema.VehicleOrSubClass
  | schema.ProductCollectionOrSubClass
  | schema.ProductModelOrSubClass
  | schema.ProductGroupOrSubClass
  ;

/**
 * schema:Product - Validation schema (w/ JOI)
 */
export const ProductJoiSchema = Joi.object({
  '@type': Joi.string().valid('Product').required(),
 ...
 * schema:Product - Validation schema (w/ JOI)
 *
 * This differs from ProductJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ProductOrSubClassJoiSchema = Joi.alternatives().try([
  ProductJoiSchema,
  Joi.lazy(() => oa.FacilityUseOrSubClassJoiSchema),
  Joi.lazy(() => schema.IndividualProductOrSubClassJoiSchema),
  Joi.lazy(() => schema.SomeProductsOrSubClassJoiSchema),
  Joi.lazy(() => schema.VehicleOrSubClassJoiSchema),
  Joi.lazy(() => schema.ProductCollectionOrSubClassJoiSchema),
  Joi.lazy(() => schema.ProductModelOrSubClassJoiSchema),
  Joi.lazy(() => schema.ProductGroupOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Product.
 *
 * If some data has a structure which matches a schema:Product, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeProduct = Product.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeProduct instanceof OaValidationError) {
 *   // From this point on, `maybeProduct` will have type `OaValidationError`
 *   const error = maybeProduct;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeProduct` will have type `Product`
 * const product = maybeProduct;
 * ```
 */
export function validateProduct(maybeProduct: unknown): Product | OaValidationError {
  const { value, error } = ProductJoiSchema.validate(maybeProduct);
  if (error) {
    return new OaValidationError('Product', maybeProduct, error);
  }
