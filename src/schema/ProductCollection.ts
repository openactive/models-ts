 ...
 * schema:ProductCollection
 *
 * This differs from ProductCollection because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ProductCollectionOrSubClass =
  | ProductCollection
  ;
 ...
 */
export const ProductCollectionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ProductCollectionOrSubClassJoiSchema = Joi.alternatives().try([
  ProductCollectionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeProductCollection = ProductCollection.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateProductCollection(maybeProductCollection: unknown): ProductCollection | OaValidationError {
  const { value, error } = ProductCollectionJoiSchema.validate(maybeProductCollection);
  if (error) {
    return new OaValidationError('ProductCollection', maybeProductCollection, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ProductCollection;
}
