...
export type ProductGroup = {
   ...
   */
  hasVariant?: schema.ProductOrSubClass | string;
  /**
   ...
   */
  variesBy?: string | schema.DefinedTermOrSubClass;
  /**
 ...
 * schema:ProductGroup
 *
 * This differs from ProductGroup because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ProductGroupOrSubClass =
  | ProductGroup
  ;

/**
 * schema:ProductGroup - Validation schema (w/ JOI)
 */
export const ProductGroupJoiSchema = Joi.object({
  '@type': Joi.string().valid('ProductGroup').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  hasVariant: Joi.alternatives().try(Joi.lazy(() => schema.ProductOrSubClassJoiSchema), Joi.string().uri()),
  productGroupID: Joi.string(),
  variesBy: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.DefinedTermOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ProductGroup - Validation schema (w/ JOI)
 *
 * This differs from ProductGroupJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ProductGroupOrSubClassJoiSchema = Joi.alternatives().try([
  ProductGroupJoiSchema,
]);

/**
 * Runtime validator for schema:ProductGroup.
 *
 * If some data has a structure which matches a schema:ProductGroup, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeProductGroup = ProductGroup.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeProductGroup instanceof OaValidationError) {
 *   // From this point on, `maybeProductGroup` will have type `OaValidationError`
 *   const error = maybeProductGroup;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeProductGroup` will have type `ProductGroup`
 * const productGroup = maybeProductGroup;
 * ```
 */
export function validateProductGroup(maybeProductGroup: unknown): ProductGroup | OaValidationError {
  const { value, error } = ProductGroupJoiSchema.validate(maybeProductGroup);
  if (error) {
    return new OaValidationError('ProductGroup', maybeProductGroup, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ProductGroup;
}
