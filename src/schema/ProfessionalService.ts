 ...
 * schema:ProfessionalService
 *
 * This differs from ProfessionalService because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ProfessionalServiceOrSubClass =
  | ProfessionalService
  ;

/**
 * schema:ProfessionalService - Validation schema (w/ JOI)
 */
export const ProfessionalServiceJoiSchema = Joi.object({
  '@type': Joi.string().valid('ProfessionalService').required(),
 ...
 * schema:ProfessionalService - Validation schema (w/ JOI)
 *
 * This differs from ProfessionalServiceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ProfessionalServiceOrSubClassJoiSchema = Joi.alternatives().try([
  ProfessionalServiceJoiSchema,
]);

/**
 * Runtime validator for schema:ProfessionalService.
 *
 * If some data has a structure which matches a schema:ProfessionalService, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeProfessionalService = ProfessionalService.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeProfessionalService instanceof OaValidationError) {
 *   // From this point on, `maybeProfessionalService` will have type `OaValidationError`
 *   const error = maybeProfessionalService;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeProfessionalService` will have type `ProfessionalService`
 * const professionalService = maybeProfessionalService;
 * ```
 */
export function validateProfessionalService(maybeProfessionalService: unknown): ProfessionalService | OaValidationError {
  const { value, error } = ProfessionalServiceJoiSchema.validate(maybeProfessionalService);
  if (error) {
    return new OaValidationError('ProfessionalService', maybeProfessionalService, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ProfessionalService;
}
