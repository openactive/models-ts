 ...
 * schema:ProfilePage
 *
 * This differs from ProfilePage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ProfilePageOrSubClass =
  | ProfilePage
  ;

/**
 * schema:ProfilePage - Validation schema (w/ JOI)
 */
export const ProfilePageJoiSchema = Joi.object({
  '@type': Joi.string().valid('ProfilePage').required(),
 ...
 * schema:ProfilePage - Validation schema (w/ JOI)
 *
 * This differs from ProfilePageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ProfilePageOrSubClassJoiSchema = Joi.alternatives().try([
  ProfilePageJoiSchema,
]);

/**
 * Runtime validator for schema:ProfilePage.
 *
 * If some data has a structure which matches a schema:ProfilePage, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeProfilePage = ProfilePage.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeProfilePage instanceof OaValidationError) {
 *   // From this point on, `maybeProfilePage` will have type `OaValidationError`
 *   const error = maybeProfilePage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeProfilePage` will have type `ProfilePage`
 * const profilePage = maybeProfilePage;
 * ```
 */
export function validateProfilePage(maybeProfilePage: unknown): ProfilePage | OaValidationError {
  const { value, error } = ProfilePageJoiSchema.validate(maybeProfilePage);
  if (error) {
    return new OaValidationError('ProfilePage', maybeProfilePage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ProfilePage;
}
