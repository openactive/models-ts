 ...
 * schema:Project
 *
 * This differs from Project because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ProjectOrSubClass =
  | Project
  | schema.ResearchProjectOrSubClass
  | schema.FundingAgencyOrSubClass
  ;

/**
 * schema:Project - Validation schema (w/ JOI)
 */
 ...
 * schema:Project - Validation schema (w/ JOI)
 *
 * This differs from ProjectJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ProjectOrSubClassJoiSchema = Joi.alternatives().try([
  ProjectJoiSchema,
  Joi.lazy(() => schema.ResearchProjectOrSubClassJoiSchema),
  Joi.lazy(() => schema.FundingAgencyOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Project.
 *
 * If some data has a structure which matches a schema:Project, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeProject = Project.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeProject instanceof OaValidationError) {
 *   // From this point on, `maybeProject` will have type `OaValidationError`
 *   const error = maybeProject;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeProject` will have type `Project`
 * const project = maybeProject;
 * ```
 */
export function validateProject(maybeProject: unknown): Project | OaValidationError {
  const { value, error } = ProjectJoiSchema.validate(maybeProject);
  if (error) {
    return new OaValidationError('Project', maybeProject, error);
  }
