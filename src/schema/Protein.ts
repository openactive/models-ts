 ...
 */
export const ProteinJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ProteinOrSubClassJoiSchema = Joi.alternatives().try([
  ProteinJoiSchema,
]);
 ...
 * ```
 */
export function validateProtein(maybeProtein: unknown): Protein | OaValidationError {
  const { value, error } = ProteinJoiSchema.validate(maybeProtein);
  if (error) {
    return new OaValidationError('Protein', maybeProtein, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Protein;
}
