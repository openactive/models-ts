 ...
 */
export type PsychologicalTreatment = {
 ...
 * schema:PsychologicalTreatment
 *
 * This differs from PsychologicalTreatment because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PsychologicalTreatmentOrSubClass =
  | PsychologicalTreatment
  ;

/**
 * schema:PsychologicalTreatment - Validation schema (w/ JOI)
 */
export const PsychologicalTreatmentJoiSchema = Joi.object({
  '@type': Joi.string().valid('PsychologicalTreatment').required(),
 ...
 * schema:PsychologicalTreatment - Validation schema (w/ JOI)
 *
 * This differs from PsychologicalTreatmentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PsychologicalTreatmentOrSubClassJoiSchema = Joi.alternatives().try([
  PsychologicalTreatmentJoiSchema,
]);

/**
 * Runtime validator for schema:PsychologicalTreatment.
 *
 * If some data has a structure which matches a schema:PsychologicalTreatment, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePsychologicalTreatment = PsychologicalTreatment.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePsychologicalTreatment instanceof OaValidationError) {
 *   // From this point on, `maybePsychologicalTreatment` will have type `OaValidationError`
 *   const error = maybePsychologicalTreatment;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePsychologicalTreatment` will have type `PsychologicalTreatment`
 * const psychologicalTreatment = maybePsychologicalTreatment;
 * ```
 */
export function validatePsychologicalTreatment(maybePsychologicalTreatment: unknown): PsychologicalTreatment | OaValidationError {
  const { value, error } = PsychologicalTreatmentJoiSchema.validate(maybePsychologicalTreatment);
  if (error) {
    return new OaValidationError('PsychologicalTreatment', maybePsychologicalTreatment, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PsychologicalTreatment;
}
