 ...
 */
export type PublicSwimmingPool = {
 ...
 * schema:PublicSwimmingPool
 *
 * This differs from PublicSwimmingPool because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PublicSwimmingPoolOrSubClass =
  | PublicSwimmingPool
  ;

/**
 * schema:PublicSwimmingPool - Validation schema (w/ JOI)
 */
export const PublicSwimmingPoolJoiSchema = Joi.object({
  '@type': Joi.string().valid('PublicSwimmingPool').required(),
 ...
 * schema:PublicSwimmingPool - Validation schema (w/ JOI)
 *
 * This differs from PublicSwimmingPoolJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PublicSwimmingPoolOrSubClassJoiSchema = Joi.alternatives().try([
  PublicSwimmingPoolJoiSchema,
]);

/**
 * Runtime validator for schema:PublicSwimmingPool.
 *
 * If some data has a structure which matches a schema:PublicSwimmingPool, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePublicSwimmingPool` will have type `PublicSwimmingPool`
 * const publicSwimmingPool = maybePublicSwimmingPool;
 * ```
 */
export function validatePublicSwimmingPool(maybePublicSwimmingPool: unknown): PublicSwimmingPool | OaValidationError {
  const { value, error } = PublicSwimmingPoolJoiSchema.validate(maybePublicSwimmingPool);
  if (error) {
    return new OaValidationError('PublicSwimmingPool', maybePublicSwimmingPool, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PublicSwimmingPool;
}
