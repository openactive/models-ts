 ...
 * schema:PublicToilet
 *
 * This differs from PublicToilet because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PublicToiletOrSubClass =
  | PublicToilet
  ;

/**
 * schema:PublicToilet - Validation schema (w/ JOI)
 */
export const PublicToiletJoiSchema = Joi.object({
  '@type': Joi.string().valid('PublicToilet').required(),
 ...
 * schema:PublicToilet - Validation schema (w/ JOI)
 *
 * This differs from PublicToiletJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PublicToiletOrSubClassJoiSchema = Joi.alternatives().try([
  PublicToiletJoiSchema,
]);

/**
 * Runtime validator for schema:PublicToilet.
 *
 * If some data has a structure which matches a schema:PublicToilet, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePublicToilet = PublicToilet.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePublicToilet instanceof OaValidationError) {
 *   // From this point on, `maybePublicToilet` will have type `OaValidationError`
 *   const error = maybePublicToilet;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePublicToilet` will have type `PublicToilet`
 * const publicToilet = maybePublicToilet;
 * ```
 */
export function validatePublicToilet(maybePublicToilet: unknown): PublicToilet | OaValidationError {
  const { value, error } = PublicToiletJoiSchema.validate(maybePublicToilet);
  if (error) {
    return new OaValidationError('PublicToilet', maybePublicToilet, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PublicToilet;
}
