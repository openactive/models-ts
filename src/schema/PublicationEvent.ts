...
export type PublicationEvent = {
   ...
   */
  publishedOn?: schema.BroadcastServiceOrSubClass | string;
  /**
   * An agent associated with the publication event.
   */
  publishedBy?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
 ...
 * schema:PublicationEvent
 *
 * This differs from PublicationEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PublicationEventOrSubClass =
  | PublicationEvent
  | schema.OnDemandEventOrSubClass
  | schema.BroadcastEventOrSubClass
  ;

/**
 * schema:PublicationEvent - Validation schema (w/ JOI)
 */
export const PublicationEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('PublicationEvent').required(),
  ...
  publishedBy: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:PublicationEvent - Validation schema (w/ JOI)
 *
 * This differs from PublicationEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PublicationEventOrSubClassJoiSchema = Joi.alternatives().try([
  PublicationEventJoiSchema,
  Joi.lazy(() => schema.OnDemandEventOrSubClassJoiSchema),
  Joi.lazy(() => schema.BroadcastEventOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:PublicationEvent.
 *
 * If some data has a structure which matches a schema:PublicationEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePublicationEvent = PublicationEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePublicationEvent instanceof OaValidationError) {
 *   // From this point on, `maybePublicationEvent` will have type `OaValidationError`
 *   const error = maybePublicationEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePublicationEvent` will have type `PublicationEvent`
 * const publicationEvent = maybePublicationEvent;
 * ```
 */
export function validatePublicationEvent(maybePublicationEvent: unknown): PublicationEvent | OaValidationError {
  const { value, error } = PublicationEventJoiSchema.validate(maybePublicationEvent);
  if (error) {
    return new OaValidationError('PublicationEvent', maybePublicationEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PublicationEvent;
}
