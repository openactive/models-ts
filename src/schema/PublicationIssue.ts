 ...
 * schema:PublicationIssue
 *
 * This differs from PublicationIssue because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PublicationIssueOrSubClass =
  | PublicationIssue
  | schema.ComicIssueOrSubClass
  ;

/**
 * schema:PublicationIssue - Validation schema (w/ JOI)
 */
export const PublicationIssueJoiSchema = Joi.object({
  '@type': Joi.string().valid('PublicationIssue').required(),
 ...
 * schema:PublicationIssue - Validation schema (w/ JOI)
 *
 * This differs from PublicationIssueJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PublicationIssueOrSubClassJoiSchema = Joi.alternatives().try([
  PublicationIssueJoiSchema,
  Joi.lazy(() => schema.ComicIssueOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:PublicationIssue.
 *
 * If some data has a structure which matches a schema:PublicationIssue, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybePublicationIssue = PublicationIssue.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybePublicationIssue instanceof OaValidationError) {
 *   // From this point on, `maybePublicationIssue` will have type `OaValidationError`
 *   const error = maybePublicationIssue;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybePublicationIssue` will have type `PublicationIssue`
 * const publicationIssue = maybePublicationIssue;
 * ```
 */
export function validatePublicationIssue(maybePublicationIssue: unknown): PublicationIssue | OaValidationError {
  const { value, error } = PublicationIssueJoiSchema.validate(maybePublicationIssue);
  if (error) {
    return new OaValidationError('PublicationIssue', maybePublicationIssue, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PublicationIssue;
}
