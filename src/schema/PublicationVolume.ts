 ...
 * schema:PublicationVolume
 *
 * This differs from PublicationVolume because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type PublicationVolumeOrSubClass =
  | PublicationVolume
  ;

/**
 * schema:PublicationVolume - Validation schema (w/ JOI)
 */
export const PublicationVolumeJoiSchema = Joi.object({
  '@type': Joi.string().valid('PublicationVolume').required(),
  ...
  volumeNumber: Joi.alternatives().try(Joi.string(), Joi.number().integer()),
 ...
 * schema:PublicationVolume - Validation schema (w/ JOI)
 *
 * This differs from PublicationVolumeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const PublicationVolumeOrSubClassJoiSchema = Joi.alternatives().try([
  PublicationVolumeJoiSchema,
]);

/**
 * Runtime validator for schema:PublicationVolume.
 *
 * If some data has a structure which matches a schema:PublicationVolume, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybePublicationVolume` will have type `PublicationVolume`
 * const publicationVolume = maybePublicationVolume;
 * ```
 */
export function validatePublicationVolume(maybePublicationVolume: unknown): PublicationVolume | OaValidationError {
  const { value, error } = PublicationVolumeJoiSchema.validate(maybePublicationVolume);
  if (error) {
    return new OaValidationError('PublicationVolume', maybePublicationVolume, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as PublicationVolume;
}
