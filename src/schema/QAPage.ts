 ...
 * schema:QAPage
 *
 * This differs from QAPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type QAPageOrSubClass =
  | QAPage
  ;

/**
 * schema:QAPage - Validation schema (w/ JOI)
 */
export const QAPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('QAPage').required(),
 ...
 * schema:QAPage - Validation schema (w/ JOI)
 *
 * This differs from QAPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const QAPageOrSubClassJoiSchema = Joi.alternatives().try([
  QAPageJoiSchema,
]);

/**
 * Runtime validator for schema:QAPage.
 *
 * If some data has a structure which matches a schema:QAPage, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeQAPage` will have type `QAPage`
 * const qAPage = maybeQAPage;
 * ```
 */
export function validateQAPage(maybeQAPage: unknown): QAPage | OaValidationError {
  const { value, error } = QAPageJoiSchema.validate(maybeQAPage);
  if (error) {
    return new OaValidationError('QAPage', maybeQAPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as QAPage;
}
