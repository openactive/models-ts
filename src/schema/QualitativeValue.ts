 ...
 * A predefined value for a product characteristic, e.g. the power cord plug type 'US' or the garment sizes 'S', 'M', 'L', and 'XL'.
 */
export const QualitativeValueJoiSchema = Joi.string().valid(
);
 ...
 * ```
 */
export function validateQualitativeValue(maybeQualitativeValue: unknown): QualitativeValue | OaValidationError {
  const { value, error } = QualitativeValueJoiSchema.validate(maybeQualitativeValue);
  if (error) {
    return new OaValidationError('QualitativeValue', maybeQualitativeValue, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as QualitativeValue;
}
