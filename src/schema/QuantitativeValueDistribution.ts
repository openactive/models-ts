 ...
 * etc.
 */
export type QuantitativeValueDistributionOrSubClass =
  | QuantitativeValueDistribution
  | schema.MonetaryAmountDistributionOrSubClass
  ;

/**
 * schema:QuantitativeValueDistribution - Validation schema (w/ JOI)
 */
export const QuantitativeValueDistributionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const QuantitativeValueDistributionOrSubClassJoiSchema = Joi.alternatives().try([
  QuantitativeValueDistributionJoiSchema,
 ...
 * ```
 */
export function validateQuantitativeValueDistribution(maybeQuantitativeValueDistribution: unknown): QuantitativeValueDistribution | OaValidationError {
  const { value, error } = QuantitativeValueDistributionJoiSchema.validate(maybeQuantitativeValueDistribution);
  if (error) {
    return new OaValidationError('QuantitativeValueDistribution', maybeQuantitativeValueDistribution, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as QuantitativeValueDistribution;
}
