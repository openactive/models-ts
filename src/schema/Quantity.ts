 ...
 * schema:Quantity
 *
 * This differs from Quantity because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type QuantityOrSubClass =
  | Quantity
  | schema.EnergyOrSubClass
  | schema.MassOrSubClass
  | schema.DurationOrSubClass
  | schema.DistanceOrSubClass
  ;

/**
 * schema:Quantity - Validation schema (w/ JOI)
 */
export const QuantityJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const QuantityOrSubClassJoiSchema = Joi.alternatives().try([
  QuantityJoiSchema,
 ...
 *
 * ```ts
 * const maybeQuantity = validateQuantity(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateQuantity(maybeQuantity: unknown): Quantity | OaValidationError {
  const { value, error } = QuantityJoiSchema.validate(maybeQuantity);
  if (error) {
    return new OaValidationError('Quantity', maybeQuantity, error);
  }
