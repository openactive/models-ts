...
export type Question = {
   ...
   */
  suggestedAnswer?: schema.AnswerOrSubClass | schema.ItemListOrSubClass | string;
  /**
   ...
   */
  acceptedAnswer?: schema.AnswerOrSubClass | schema.ItemListOrSubClass | string;
  /**
   ...
   */
  parentItem?: schema.CommentOrSubClass | string;
  /**
 ...
 * schema:Question
 *
 * This differs from Question because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type QuestionOrSubClass =
  | Question
  ;

/**
 * schema:Question - Validation schema (w/ JOI)
 */
export const QuestionJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  suggestedAnswer: Joi.alternatives().try(Joi.lazy(() => schema.AnswerOrSubClassJoiSchema), Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.string().uri()),
  answerCount: Joi.number().integer(),
  acceptedAnswer: Joi.alternatives().try(Joi.lazy(() => schema.AnswerOrSubClassJoiSchema), Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.string().uri()),
  eduQuestionType: Joi.string(),
  parentItem: Joi.alternatives().try(Joi.lazy(() => schema.CommentOrSubClassJoiSchema), Joi.string().uri()),
  downvoteCount: Joi.number().integer(),
  upvoteCount: Joi.number().integer(),
  text: Joi.string(),
 ...
 * schema:Question - Validation schema (w/ JOI)
 *
 * This differs from QuestionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const QuestionOrSubClassJoiSchema = Joi.alternatives().try([
  QuestionJoiSchema,
]);

/**
 * Runtime validator for schema:Question.
 *
 * If some data has a structure which matches a schema:Question, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeQuestion = Question.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeQuestion instanceof OaValidationError) {
 *   // From this point on, `maybeQuestion` will have type `OaValidationError`
 *   const error = maybeQuestion;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeQuestion` will have type `Question`
 * const question = maybeQuestion;
 * ```
 */
export function validateQuestion(maybeQuestion: unknown): Question | OaValidationError {
  const { value, error } = QuestionJoiSchema.validate(maybeQuestion);
  if (error) {
    return new OaValidationError('Question', maybeQuestion, error);
  }
