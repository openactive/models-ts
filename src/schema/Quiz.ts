 ...
 * schema:Quiz
 *
 * This differs from Quiz because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type QuizOrSubClass =
  | Quiz
  ;

/**
 * schema:Quiz - Validation schema (w/ JOI)
 */
export const QuizJoiSchema = Joi.object({
 ...
 * schema:Quiz - Validation schema (w/ JOI)
 *
 * This differs from QuizJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const QuizOrSubClassJoiSchema = Joi.alternatives().try([
  QuizJoiSchema,
]);

/**
 * Runtime validator for schema:Quiz.
 *
 * If some data has a structure which matches a schema:Quiz, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeQuiz = Quiz.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeQuiz instanceof OaValidationError) {
 *   // From this point on, `maybeQuiz` will have type `OaValidationError`
 *   const error = maybeQuiz;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeQuiz` will have type `Quiz`
 * const quiz = maybeQuiz;
 * ```
 */
export function validateQuiz(maybeQuiz: unknown): Quiz | OaValidationError {
  const { value, error } = QuizJoiSchema.validate(maybeQuiz);
  if (error) {
    return new OaValidationError('Quiz', maybeQuiz, error);
  }
