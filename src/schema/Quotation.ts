...
export type Quotation = {
   ...
   */
  spokenByCharacter?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
 ...
 * schema:Quotation
 *
 * This differs from Quotation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type QuotationOrSubClass =
  | Quotation
  ;

/**
 * schema:Quotation - Validation schema (w/ JOI)
 */
export const QuotationJoiSchema = Joi.object({
  '@type': Joi.string().valid('Quotation').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  spokenByCharacter: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:Quotation - Validation schema (w/ JOI)
 *
 * This differs from QuotationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const QuotationOrSubClassJoiSchema = Joi.alternatives().try([
  QuotationJoiSchema,
]);

/**
 * Runtime validator for schema:Quotation.
 *
 * If some data has a structure which matches a schema:Quotation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeQuotation = Quotation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeQuotation instanceof OaValidationError) {
 *   // From this point on, `maybeQuotation` will have type `OaValidationError`
 *   const error = maybeQuotation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeQuotation` will have type `Quotation`
 * const quotation = maybeQuotation;
 * ```
 */
export function validateQuotation(maybeQuotation: unknown): Quotation | OaValidationError {
  const { value, error } = QuotationJoiSchema.validate(maybeQuotation);
  if (error) {
    return new OaValidationError('Quotation', maybeQuotation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Quotation;
}
