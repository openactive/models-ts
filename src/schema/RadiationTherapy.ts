 ...
 * schema:RadiationTherapy
 *
 * This differs from RadiationTherapy because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RadiationTherapyOrSubClass =
  | RadiationTherapy
  ;
 ...
 */
export const RadiationTherapyJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const RadiationTherapyOrSubClassJoiSchema = Joi.alternatives().try([
  RadiationTherapyJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeRadiationTherapy = RadiationTherapy.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateRadiationTherapy(maybeRadiationTherapy: unknown): RadiationTherapy | OaValidationError {
  const { value, error } = RadiationTherapyJoiSchema.validate(maybeRadiationTherapy);
  if (error) {
    return new OaValidationError('RadiationTherapy', maybeRadiationTherapy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadiationTherapy;
}
