 ...
 * etc.
 */
export type RadioChannelOrSubClass =
  | RadioChannel
  | schema.FMRadioChannelOrSubClass
  | schema.AMRadioChannelOrSubClass
  ;

/**
 * schema:RadioChannel - Validation schema (w/ JOI)
 */
export const RadioChannelJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const RadioChannelOrSubClassJoiSchema = Joi.alternatives().try([
  RadioChannelJoiSchema,
 ...
 * ```
 */
export function validateRadioChannel(maybeRadioChannel: unknown): RadioChannel | OaValidationError {
  const { value, error } = RadioChannelJoiSchema.validate(maybeRadioChannel);
  if (error) {
    return new OaValidationError('RadioChannel', maybeRadioChannel, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadioChannel;
}
