 ...
 * schema:RadioClip
 *
 * This differs from RadioClip because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RadioClipOrSubClass =
  | RadioClip
  ;

/**
 * schema:RadioClip - Validation schema (w/ JOI)
 */
export const RadioClipJoiSchema = Joi.object({
  '@type': Joi.string().valid('RadioClip').required(),
 ...
 * schema:RadioClip - Validation schema (w/ JOI)
 *
 * This differs from RadioClipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RadioClipOrSubClassJoiSchema = Joi.alternatives().try([
  RadioClipJoiSchema,
]);

/**
 * Runtime validator for schema:RadioClip.
 *
 * If some data has a structure which matches a schema:RadioClip, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRadioClip = RadioClip.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRadioClip instanceof OaValidationError) {
 *   // From this point on, `maybeRadioClip` will have type `OaValidationError`
 *   const error = maybeRadioClip;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRadioClip` will have type `RadioClip`
 * const radioClip = maybeRadioClip;
 * ```
 */
export function validateRadioClip(maybeRadioClip: unknown): RadioClip | OaValidationError {
  const { value, error } = RadioClipJoiSchema.validate(maybeRadioClip);
  if (error) {
    return new OaValidationError('RadioClip', maybeRadioClip, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadioClip;
}
