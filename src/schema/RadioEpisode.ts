 ...
 * etc.
 */
export type RadioEpisodeOrSubClass =
  | RadioEpisode
  ;
 ...
 */
export const RadioEpisodeJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const RadioEpisodeOrSubClassJoiSchema = Joi.alternatives().try([
  RadioEpisodeJoiSchema,
]);
 ...
 * ```
 */
export function validateRadioEpisode(maybeRadioEpisode: unknown): RadioEpisode | OaValidationError {
  const { value, error } = RadioEpisodeJoiSchema.validate(maybeRadioEpisode);
  if (error) {
    return new OaValidationError('RadioEpisode', maybeRadioEpisode, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadioEpisode;
}
