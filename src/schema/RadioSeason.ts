 ...
 * schema:RadioSeason
 *
 * This differs from RadioSeason because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RadioSeasonOrSubClass =
  | RadioSeason
  ;

/**
 * schema:RadioSeason - Validation schema (w/ JOI)
 */
export const RadioSeasonJoiSchema = Joi.object({
  '@type': Joi.string().valid('RadioSeason').required(),
 ...
 * schema:RadioSeason - Validation schema (w/ JOI)
 *
 * This differs from RadioSeasonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RadioSeasonOrSubClassJoiSchema = Joi.alternatives().try([
  RadioSeasonJoiSchema,
]);

/**
 * Runtime validator for schema:RadioSeason.
 *
 * If some data has a structure which matches a schema:RadioSeason, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRadioSeason = RadioSeason.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRadioSeason instanceof OaValidationError) {
 *   // From this point on, `maybeRadioSeason` will have type `OaValidationError`
 *   const error = maybeRadioSeason;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRadioSeason` will have type `RadioSeason`
 * const radioSeason = maybeRadioSeason;
 * ```
 */
export function validateRadioSeason(maybeRadioSeason: unknown): RadioSeason | OaValidationError {
  const { value, error } = RadioSeasonJoiSchema.validate(maybeRadioSeason);
  if (error) {
    return new OaValidationError('RadioSeason', maybeRadioSeason, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadioSeason;
}
