 ...
 * schema:RadioSeries
 *
 * This differs from RadioSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RadioSeriesOrSubClass =
  | RadioSeries
  ;

/**
 * schema:RadioSeries - Validation schema (w/ JOI)
 */
export const RadioSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('RadioSeries').required(),
 ...
 * schema:RadioSeries - Validation schema (w/ JOI)
 *
 * This differs from RadioSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RadioSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  RadioSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:RadioSeries.
 *
 * If some data has a structure which matches a schema:RadioSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRadioSeries = RadioSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRadioSeries instanceof OaValidationError) {
 *   // From this point on, `maybeRadioSeries` will have type `OaValidationError`
 *   const error = maybeRadioSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRadioSeries` will have type `RadioSeries`
 * const radioSeries = maybeRadioSeries;
 * ```
 */
export function validateRadioSeries(maybeRadioSeries: unknown): RadioSeries | OaValidationError {
  const { value, error } = RadioSeriesJoiSchema.validate(maybeRadioSeries);
  if (error) {
    return new OaValidationError('RadioSeries', maybeRadioSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadioSeries;
}
