 ...
 * schema:RadioStation
 *
 * This differs from RadioStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RadioStationOrSubClass =
  | RadioStation
  ;

/**
 * schema:RadioStation - Validation schema (w/ JOI)
 */
export const RadioStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('RadioStation').required(),
 ...
 * schema:RadioStation - Validation schema (w/ JOI)
 *
 * This differs from RadioStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RadioStationOrSubClassJoiSchema = Joi.alternatives().try([
  RadioStationJoiSchema,
]);

/**
 * Runtime validator for schema:RadioStation.
 *
 * If some data has a structure which matches a schema:RadioStation, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeRadioStation` will have type `RadioStation`
 * const radioStation = maybeRadioStation;
 * ```
 */
export function validateRadioStation(maybeRadioStation: unknown): RadioStation | OaValidationError {
  const { value, error } = RadioStationJoiSchema.validate(maybeRadioStation);
  if (error) {
    return new OaValidationError('RadioStation', maybeRadioStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RadioStation;
}
