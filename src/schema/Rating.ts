 ...
 * etc.
 */
export type RatingOrSubClass =
  | Rating
  | schema.EndorsementRatingOrSubClass
  | schema.AggregateRatingOrSubClass
  ;
 ...
 * `SessionSeries`, etc.
 */
export const RatingOrSubClassJoiSchema = Joi.alternatives().try([
  RatingJoiSchema,
 ...
 * ```
 */
export function validateRating(maybeRating: unknown): Rating | OaValidationError {
  const { value, error } = RatingJoiSchema.validate(maybeRating);
  if (error) {
    return new OaValidationError('Rating', maybeRating, error);
  }
