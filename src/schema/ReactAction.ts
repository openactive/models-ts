 ...
 * schema:ReactAction
 *
 * This differs from ReactAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReactActionOrSubClass =
  | ReactAction
  | schema.EndorseActionOrSubClass
  | schema.WantActionOrSubClass
  | schema.DisagreeActionOrSubClass
  | schema.LikeActionOrSubClass
  | schema.AgreeActionOrSubClass
  | schema.DislikeActionOrSubClass
  ;

/**
 * schema:ReactAction - Validation schema (w/ JOI)
 */
export const ReactActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ReactActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReactActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeReactAction = ReactAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateReactAction(maybeReactAction: unknown): ReactAction | OaValidationError {
  const { value, error } = ReactActionJoiSchema.validate(maybeReactAction);
  if (error) {
    return new OaValidationError('ReactAction', maybeReactAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReactAction;
}
