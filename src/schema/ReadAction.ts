 ...
 * schema:ReadAction
 *
 * This differs from ReadAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReadActionOrSubClass =
  | ReadAction
  ;
 ...
 */
export const ReadActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ReadActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReadActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeReadAction = ReadAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateReadAction(maybeReadAction: unknown): ReadAction | OaValidationError {
  const { value, error } = ReadActionJoiSchema.validate(maybeReadAction);
  if (error) {
    return new OaValidationError('ReadAction', maybeReadAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReadAction;
}
