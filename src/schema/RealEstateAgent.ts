 ...
 * schema:RealEstateAgent
 *
 * This differs from RealEstateAgent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RealEstateAgentOrSubClass =
  | RealEstateAgent
  ;

/**
 * schema:RealEstateAgent - Validation schema (w/ JOI)
 */
export const RealEstateAgentJoiSchema = Joi.object({
  '@type': Joi.string().valid('RealEstateAgent').required(),
 ...
 * schema:RealEstateAgent - Validation schema (w/ JOI)
 *
 * This differs from RealEstateAgentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RealEstateAgentOrSubClassJoiSchema = Joi.alternatives().try([
  RealEstateAgentJoiSchema,
]);

/**
 * Runtime validator for schema:RealEstateAgent.
 *
 * If some data has a structure which matches a schema:RealEstateAgent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRealEstateAgent = RealEstateAgent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRealEstateAgent instanceof OaValidationError) {
 *   // From this point on, `maybeRealEstateAgent` will have type `OaValidationError`
 *   const error = maybeRealEstateAgent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRealEstateAgent` will have type `RealEstateAgent`
 * const realEstateAgent = maybeRealEstateAgent;
 * ```
 */
export function validateRealEstateAgent(maybeRealEstateAgent: unknown): RealEstateAgent | OaValidationError {
  const { value, error } = RealEstateAgentJoiSchema.validate(maybeRealEstateAgent);
  if (error) {
    return new OaValidationError('RealEstateAgent', maybeRealEstateAgent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RealEstateAgent;
}
