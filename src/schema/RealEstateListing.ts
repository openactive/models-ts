 ...
 * schema:RealEstateListing
 *
 * This differs from RealEstateListing because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RealEstateListingOrSubClass =
  | RealEstateListing
  ;

/**
 * schema:RealEstateListing - Validation schema (w/ JOI)
 */
export const RealEstateListingJoiSchema = Joi.object({
  '@type': Joi.string().valid('RealEstateListing').required(),
 ...
 * schema:RealEstateListing - Validation schema (w/ JOI)
 *
 * This differs from RealEstateListingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RealEstateListingOrSubClassJoiSchema = Joi.alternatives().try([
  RealEstateListingJoiSchema,
]);

/**
 * Runtime validator for schema:RealEstateListing.
 *
 * If some data has a structure which matches a schema:RealEstateListing, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeRealEstateListing` will have type `RealEstateListing`
 * const realEstateListing = maybeRealEstateListing;
 * ```
 */
export function validateRealEstateListing(maybeRealEstateListing: unknown): RealEstateListing | OaValidationError {
  const { value, error } = RealEstateListingJoiSchema.validate(maybeRealEstateListing);
  if (error) {
    return new OaValidationError('RealEstateListing', maybeRealEstateListing, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RealEstateListing;
}
