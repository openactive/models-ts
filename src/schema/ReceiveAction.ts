 ...
 * schema:ReceiveAction
 *
 * This differs from ReceiveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReceiveActionOrSubClass =
  | ReceiveAction
  ;

/**
 * schema:ReceiveAction - Validation schema (w/ JOI)
 */
export const ReceiveActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReceiveAction').required(),
 ...
 * schema:ReceiveAction - Validation schema (w/ JOI)
 *
 * This differs from ReceiveActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReceiveActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReceiveActionJoiSchema,
]);

/**
 * Runtime validator for schema:ReceiveAction.
 *
 * If some data has a structure which matches a schema:ReceiveAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReceiveAction = ReceiveAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReceiveAction instanceof OaValidationError) {
 *   // From this point on, `maybeReceiveAction` will have type `OaValidationError`
 *   const error = maybeReceiveAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReceiveAction` will have type `ReceiveAction`
 * const receiveAction = maybeReceiveAction;
 * ```
 */
export function validateReceiveAction(maybeReceiveAction: unknown): ReceiveAction | OaValidationError {
  const { value, error } = ReceiveActionJoiSchema.validate(maybeReceiveAction);
  if (error) {
    return new OaValidationError('ReceiveAction', maybeReceiveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReceiveAction;
}
