...
export type Recipe = {
   ...
   */
  recipeYield?: schema.QuantitativeValueOrSubClass | string;
  /**
   ...
   */
  recipeInstructions?: schema.ItemListOrSubClass | schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  supply?: schema.HowToSupplyOrSubClass | string;
  /**
   * A single step item (as HowToStep, text, document, video, etc.) or a HowToSection.
   */
  step?: schema.HowToStepOrSubClass | string | schema.CreativeWorkOrSubClass | schema.HowToSectionOrSubClass;
  /**
   ...
   */
  yield?: string | schema.QuantitativeValueOrSubClass;
  /**
   ...
   */
  steps?: schema.CreativeWorkOrSubClass | schema.ItemListOrSubClass | string;
  /**
   * A sub property of instrument. An object used (but not consumed) when performing instructions or a direction.
   */
  tool?: string | schema.HowToToolOrSubClass;
  /**
 ...
 * schema:Recipe
 *
 * This differs from Recipe because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RecipeOrSubClass =
  | Recipe
  ;

/**
 * schema:Recipe - Validation schema (w/ JOI)
 */
export const RecipeJoiSchema = Joi.object({
  ...
  recipeCuisine: Joi.string(),
  recipeIngredient: Joi.string(),
  recipeYield: Joi.alternatives().try(Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  suitableForDiet: Joi.lazy(() => schema.RestrictedDietJoiSchema),
  cookTime: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  recipeCategory: Joi.string(),
  recipeInstructions: Joi.alternatives().try(Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  nutrition: Joi.alternatives().try(Joi.lazy(() => schema.NutritionInformationOrSubClassJoiSchema), Joi.string().uri()),
  ingredients: Joi.string(),
  supply: Joi.alternatives().try(Joi.lazy(() => schema.HowToSupplyOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  step: Joi.alternatives().try(Joi.lazy(() => schema.HowToStepOrSubClassJoiSchema), Joi.string(), Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.lazy(() => schema.HowToSectionOrSubClassJoiSchema), Joi.string().uri()),
  performTime: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  estimatedCost: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.MonetaryAmountOrSubClassJoiSchema), Joi.string().uri()),
  yield: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
  totalTime: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  steps: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  tool: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.HowToToolOrSubClassJoiSchema), Joi.string().uri()),
  prepTime: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:Recipe - Validation schema (w/ JOI)
 *
 * This differs from RecipeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RecipeOrSubClassJoiSchema = Joi.alternatives().try([
  RecipeJoiSchema,
]);

/**
 * Runtime validator for schema:Recipe.
 *
 * If some data has a structure which matches a schema:Recipe, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRecipe = Recipe.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRecipe instanceof OaValidationError) {
 *   // From this point on, `maybeRecipe` will have type `OaValidationError`
 *   const error = maybeRecipe;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRecipe` will have type `Recipe`
 * const recipe = maybeRecipe;
 * ```
 */
export function validateRecipe(maybeRecipe: unknown): Recipe | OaValidationError {
  const { value, error } = RecipeJoiSchema.validate(maybeRecipe);
  if (error) {
    return new OaValidationError('Recipe', maybeRecipe, error);
  }
