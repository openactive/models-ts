 ...
 * schema:Recommendation
 *
 * This differs from Recommendation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RecommendationOrSubClass =
  | Recommendation
  ;

/**
 * schema:Recommendation - Validation schema (w/ JOI)
 */
export const RecommendationJoiSchema = Joi.object({
  '@type': Joi.string().valid('Recommendation').required(),
 ...
 * schema:Recommendation - Validation schema (w/ JOI)
 *
 * This differs from RecommendationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RecommendationOrSubClassJoiSchema = Joi.alternatives().try([
  RecommendationJoiSchema,
]);

/**
 * Runtime validator for schema:Recommendation.
 *
 * If some data has a structure which matches a schema:Recommendation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRecommendation = validateRecommendation(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRecommendation instanceof OaValidationError) {
 *   // From this point on, `maybeRecommendation` will have type `OaValidationError`
 *   const error = maybeRecommendation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRecommendation` will have type `Recommendation`
 * const recommendation = maybeRecommendation;
 * ```
 */
export function validateRecommendation(maybeRecommendation: unknown): Recommendation | OaValidationError {
  const { value, error } = RecommendationJoiSchema.validate(maybeRecommendation);
  if (error) {
    return new OaValidationError('Recommendation', maybeRecommendation, error);
  }
