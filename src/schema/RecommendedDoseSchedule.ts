 ...
 */
export type RecommendedDoseSchedule = {
 ...
 * schema:RecommendedDoseSchedule
 *
 * This differs from RecommendedDoseSchedule because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RecommendedDoseScheduleOrSubClass =
 ...
 */
export const RecommendedDoseScheduleJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const RecommendedDoseScheduleOrSubClassJoiSchema = Joi.alternatives().try([
  RecommendedDoseScheduleJoiSchema,
]);
 ...
 * ```
 */
export function validateRecommendedDoseSchedule(maybeRecommendedDoseSchedule: unknown): RecommendedDoseSchedule | OaValidationError {
  const { value, error } = RecommendedDoseScheduleJoiSchema.validate(maybeRecommendedDoseSchedule);
  if (error) {
    return new OaValidationError('RecommendedDoseSchedule', maybeRecommendedDoseSchedule, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RecommendedDoseSchedule;
}
