 ...
 * schema:RecyclingCenter
 *
 * This differs from RecyclingCenter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RecyclingCenterOrSubClass =
  | RecyclingCenter
  ;

/**
 * schema:RecyclingCenter - Validation schema (w/ JOI)
 */
export const RecyclingCenterJoiSchema = Joi.object({
  '@type': Joi.string().valid('RecyclingCenter').required(),
 ...
 * schema:RecyclingCenter - Validation schema (w/ JOI)
 *
 * This differs from RecyclingCenterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RecyclingCenterOrSubClassJoiSchema = Joi.alternatives().try([
  RecyclingCenterJoiSchema,
]);

/**
 * Runtime validator for schema:RecyclingCenter.
 *
 * If some data has a structure which matches a schema:RecyclingCenter, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRecyclingCenter = RecyclingCenter.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRecyclingCenter instanceof OaValidationError) {
 *   // From this point on, `maybeRecyclingCenter` will have type `OaValidationError`
 *   const error = maybeRecyclingCenter;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRecyclingCenter` will have type `RecyclingCenter`
 * const recyclingCenter = maybeRecyclingCenter;
 * ```
 */
export function validateRecyclingCenter(maybeRecyclingCenter: unknown): RecyclingCenter | OaValidationError {
  const { value, error } = RecyclingCenterJoiSchema.validate(maybeRecyclingCenter);
  if (error) {
    return new OaValidationError('RecyclingCenter', maybeRecyclingCenter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RecyclingCenter;
}
