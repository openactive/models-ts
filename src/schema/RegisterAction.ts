 ...
 * schema:RegisterAction
 *
 * This differs from RegisterAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RegisterActionOrSubClass =
  | RegisterAction
  ;

/**
 * schema:RegisterAction - Validation schema (w/ JOI)
 */
export const RegisterActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('RegisterAction').required(),
 ...
 * schema:RegisterAction - Validation schema (w/ JOI)
 *
 * This differs from RegisterActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RegisterActionOrSubClassJoiSchema = Joi.alternatives().try([
  RegisterActionJoiSchema,
]);

/**
 * Runtime validator for schema:RegisterAction.
 *
 * If some data has a structure which matches a schema:RegisterAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeRegisterAction` will have type `RegisterAction`
 * const registerAction = maybeRegisterAction;
 * ```
 */
export function validateRegisterAction(maybeRegisterAction: unknown): RegisterAction | OaValidationError {
  const { value, error } = RegisterActionJoiSchema.validate(maybeRegisterAction);
  if (error) {
    return new OaValidationError('RegisterAction', maybeRegisterAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RegisterAction;
}
