 ...
 * schema:RejectAction
 *
 * This differs from RejectAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RejectActionOrSubClass =
  | RejectAction
  ;

/**
 * schema:RejectAction - Validation schema (w/ JOI)
 */
export const RejectActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('RejectAction').required(),
 ...
 * schema:RejectAction - Validation schema (w/ JOI)
 *
 * This differs from RejectActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RejectActionOrSubClassJoiSchema = Joi.alternatives().try([
  RejectActionJoiSchema,
]);

/**
 * Runtime validator for schema:RejectAction.
 *
 * If some data has a structure which matches a schema:RejectAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRejectAction = RejectAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRejectAction instanceof OaValidationError) {
 *   // From this point on, `maybeRejectAction` will have type `OaValidationError`
 *   const error = maybeRejectAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRejectAction` will have type `RejectAction`
 * const rejectAction = maybeRejectAction;
 * ```
 */
export function validateRejectAction(maybeRejectAction: unknown): RejectAction | OaValidationError {
  const { value, error } = RejectActionJoiSchema.validate(maybeRejectAction);
  if (error) {
    return new OaValidationError('RejectAction', maybeRejectAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RejectAction;
}
