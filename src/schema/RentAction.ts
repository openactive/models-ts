...
export type RentAction = {
   ...
   */
  landlord?: schema.OrganizationOrSubClass | schema.PersonOrSubClass | string;
  /**
   * A sub property of participant. The real estate agent involved in the action.
   */
  realEstateAgent?: schema.RealEstateAgentOrSubClass | string;
  /**
 ...
 * schema:RentAction
 *
 * This differs from RentAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RentActionOrSubClass =
  | RentAction
  ;
 ...
 */
export const RentActionJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  landlord: Joi.alternatives().try(Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const RentActionOrSubClassJoiSchema = Joi.alternatives().try([
  RentActionJoiSchema,
]);
 ...
 * ```
 */
export function validateRentAction(maybeRentAction: unknown): RentAction | OaValidationError {
  const { value, error } = RentActionJoiSchema.validate(maybeRentAction);
  if (error) {
    return new OaValidationError('RentAction', maybeRentAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RentAction;
}
