 ...
 */
export type RentalCarReservation = {
   ...
   */
  dropoffLocation?: schema.PlaceOrSubClass | string;
  /**
   ...
   */
  pickupLocation?: schema.PlaceOrSubClass | string;
  /**
 ...
 * schema:RentalCarReservation
 *
 * This differs from RentalCarReservation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RentalCarReservationOrSubClass =
 ...
 */
export const RentalCarReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const RentalCarReservationOrSubClassJoiSchema = Joi.alternatives().try([
  RentalCarReservationJoiSchema,
]);
 ...
 * ```
 */
export function validateRentalCarReservation(maybeRentalCarReservation: unknown): RentalCarReservation | OaValidationError {
  const { value, error } = RentalCarReservationJoiSchema.validate(maybeRentalCarReservation);
  if (error) {
    return new OaValidationError('RentalCarReservation', maybeRentalCarReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RentalCarReservation;
}
