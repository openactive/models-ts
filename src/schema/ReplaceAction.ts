...
export type ReplaceAction = {
   ...
   */
  replacee?: schema.ThingOrSubClass | string;
  /**
   * A sub property of object. The object that replaces.
   */
  replacer?: schema.ThingOrSubClass | string;
  /**
 ...
 * schema:ReplaceAction
 *
 * This differs from ReplaceAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReplaceActionOrSubClass =
  | ReplaceAction
  ;

/**
 * schema:ReplaceAction - Validation schema (w/ JOI)
 */
export const ReplaceActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReplaceAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  replacee: Joi.alternatives().try(Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri()),
  replacer: Joi.alternatives().try(Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ReplaceAction - Validation schema (w/ JOI)
 *
 * This differs from ReplaceActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReplaceActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReplaceActionJoiSchema,
]);

/**
 * Runtime validator for schema:ReplaceAction.
 *
 * If some data has a structure which matches a schema:ReplaceAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeReplaceAction` will have type `ReplaceAction`
 * const replaceAction = maybeReplaceAction;
 * ```
 */
export function validateReplaceAction(maybeReplaceAction: unknown): ReplaceAction | OaValidationError {
  const { value, error } = ReplaceActionJoiSchema.validate(maybeReplaceAction);
  if (error) {
    return new OaValidationError('ReplaceAction', maybeReplaceAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReplaceAction;
}
