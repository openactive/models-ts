 ...
 * schema:ReplyAction
 *
 * This differs from ReplyAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReplyActionOrSubClass =
  | ReplyAction
  ;

/**
 * schema:ReplyAction - Validation schema (w/ JOI)
 */
export const ReplyActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReplyAction').required(),
 ...
 * schema:ReplyAction - Validation schema (w/ JOI)
 *
 * This differs from ReplyActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReplyActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReplyActionJoiSchema,
]);

/**
 * Runtime validator for schema:ReplyAction.
 *
 * If some data has a structure which matches a schema:ReplyAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeReplyAction` will have type `ReplyAction`
 * const replyAction = maybeReplyAction;
 * ```
 */
export function validateReplyAction(maybeReplyAction: unknown): ReplyAction | OaValidationError {
  const { value, error } = ReplyActionJoiSchema.validate(maybeReplyAction);
  if (error) {
    return new OaValidationError('ReplyAction', maybeReplyAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReplyAction;
}
