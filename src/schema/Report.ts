 ...
 * schema:Report
 *
 * This differs from Report because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReportOrSubClass =
  | Report
  ;

/**
 * schema:Report - Validation schema (w/ JOI)
 */
 ...
 * schema:Report - Validation schema (w/ JOI)
 *
 * This differs from ReportJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReportOrSubClassJoiSchema = Joi.alternatives().try([
  ReportJoiSchema,
]);

/**
 * Runtime validator for schema:Report.
 *
 * If some data has a structure which matches a schema:Report, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReport = Report.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReport instanceof OaValidationError) {
 *   // From this point on, `maybeReport` will have type `OaValidationError`
 *   const error = maybeReport;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReport` will have type `Report`
 * const report = maybeReport;
 * ```
 */
export function validateReport(maybeReport: unknown): Report | OaValidationError {
  const { value, error } = ReportJoiSchema.validate(maybeReport);
  if (error) {
    return new OaValidationError('Report', maybeReport, error);
  }
