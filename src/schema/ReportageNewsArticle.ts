 ...
 */
export type ReportageNewsArticle = {
 ...
 * schema:ReportageNewsArticle
 *
 * This differs from ReportageNewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReportageNewsArticleOrSubClass =
  | ReportageNewsArticle
  ;

/**
 * schema:ReportageNewsArticle - Validation schema (w/ JOI)
 */
export const ReportageNewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReportageNewsArticle').required(),
 ...
 * schema:ReportageNewsArticle - Validation schema (w/ JOI)
 *
 * This differs from ReportageNewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReportageNewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  ReportageNewsArticleJoiSchema,
]);

/**
 * Runtime validator for schema:ReportageNewsArticle.
 *
 * If some data has a structure which matches a schema:ReportageNewsArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReportageNewsArticle = ReportageNewsArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReportageNewsArticle instanceof OaValidationError) {
 *   // From this point on, `maybeReportageNewsArticle` will have type `OaValidationError`
 *   const error = maybeReportageNewsArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReportageNewsArticle` will have type `ReportageNewsArticle`
 * const reportageNewsArticle = maybeReportageNewsArticle;
 * ```
 */
export function validateReportageNewsArticle(maybeReportageNewsArticle: unknown): ReportageNewsArticle | OaValidationError {
  const { value, error } = ReportageNewsArticleJoiSchema.validate(maybeReportageNewsArticle);
  if (error) {
    return new OaValidationError('ReportageNewsArticle', maybeReportageNewsArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReportageNewsArticle;
}
