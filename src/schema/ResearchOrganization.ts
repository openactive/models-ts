 ...
 * schema:ResearchOrganization
 *
 * This differs from ResearchOrganization because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ResearchOrganizationOrSubClass =
  | ResearchOrganization
  ;

/**
 * schema:ResearchOrganization - Validation schema (w/ JOI)
 */
export const ResearchOrganizationJoiSchema = Joi.object({
  '@type': Joi.string().valid('ResearchOrganization').required(),
 ...
 * schema:ResearchOrganization - Validation schema (w/ JOI)
 *
 * This differs from ResearchOrganizationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ResearchOrganizationOrSubClassJoiSchema = Joi.alternatives().try([
  ResearchOrganizationJoiSchema,
]);

/**
 * Runtime validator for schema:ResearchOrganization.
 *
 * If some data has a structure which matches a schema:ResearchOrganization, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeResearchOrganization = validateResearchOrganization(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeResearchOrganization instanceof OaValidationError) {
 *   // From this point on, `maybeResearchOrganization` will have type `OaValidationError`
 *   const error = maybeResearchOrganization;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeResearchOrganization` will have type `ResearchOrganization`
 * const researchOrganization = maybeResearchOrganization;
 * ```
 */
export function validateResearchOrganization(maybeResearchOrganization: unknown): ResearchOrganization | OaValidationError {
  const { value, error } = ResearchOrganizationJoiSchema.validate(maybeResearchOrganization);
  if (error) {
    return new OaValidationError('ResearchOrganization', maybeResearchOrganization, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ResearchOrganization;
}
