 ...
 * schema:Reservation
 *
 * This differs from Reservation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReservationOrSubClass =
  | Reservation
  | schema.RentalCarReservationOrSubClass
  | schema.TaxiReservationOrSubClass
  | schema.ReservationPackageOrSubClass
  | schema.BoatReservationOrSubClass
  | schema.FoodEstablishmentReservationOrSubClass
  | schema.EventReservationOrSubClass
  | schema.FlightReservationOrSubClass
  | schema.BusReservationOrSubClass
  | schema.TrainReservationOrSubClass
  | schema.LodgingReservationOrSubClass
  ;

/**
 * schema:Reservation - Validation schema (w/ JOI)
 */
export const ReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ReservationOrSubClassJoiSchema = Joi.alternatives().try([
  ReservationJoiSchema,
 ...
 *
 * ```ts
 * const maybeReservation = Reservation.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateReservation(maybeReservation: unknown): Reservation | OaValidationError {
  const { value, error } = ReservationJoiSchema.validate(maybeReservation);
  if (error) {
    return new OaValidationError('Reservation', maybeReservation, error);
  }
