 ...
 * schema:ReserveAction
 *
 * This differs from ReserveAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReserveActionOrSubClass =
  | ReserveAction
  ;
 ...
 */
export const ReserveActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ReserveActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReserveActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeReserveAction = ReserveAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateReserveAction(maybeReserveAction: unknown): ReserveAction | OaValidationError {
  const { value, error } = ReserveActionJoiSchema.validate(maybeReserveAction);
  if (error) {
    return new OaValidationError('ReserveAction', maybeReserveAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReserveAction;
}
