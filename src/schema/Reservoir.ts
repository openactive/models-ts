 ...
 * schema:Reservoir
 *
 * This differs from Reservoir because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReservoirOrSubClass =
  | Reservoir
  ;

/**
 * schema:Reservoir - Validation schema (w/ JOI)
 */
export const ReservoirJoiSchema = Joi.object({
  '@type': Joi.string().valid('Reservoir').required(),
 ...
 * schema:Reservoir - Validation schema (w/ JOI)
 *
 * This differs from ReservoirJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReservoirOrSubClassJoiSchema = Joi.alternatives().try([
  ReservoirJoiSchema,
]);

/**
 * Runtime validator for schema:Reservoir.
 *
 * If some data has a structure which matches a schema:Reservoir, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeReservoir` will have type `Reservoir`
 * const reservoir = maybeReservoir;
 * ```
 */
export function validateReservoir(maybeReservoir: unknown): Reservoir | OaValidationError {
  const { value, error } = ReservoirJoiSchema.validate(maybeReservoir);
  if (error) {
    return new OaValidationError('Reservoir', maybeReservoir, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Reservoir;
}
