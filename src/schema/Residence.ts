 ...
 * schema:Residence
 *
 * This differs from Residence because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ResidenceOrSubClass =
  | Residence
  | schema.ApartmentComplexOrSubClass
  | schema.GatedResidenceCommunityOrSubClass
  ;

/**
 * schema:Residence - Validation schema (w/ JOI)
 */
export const ResidenceJoiSchema = Joi.object({
  '@type': Joi.string().valid('Residence').required(),
 ...
 * schema:Residence - Validation schema (w/ JOI)
 *
 * This differs from ResidenceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ResidenceOrSubClassJoiSchema = Joi.alternatives().try([
  ResidenceJoiSchema,
  Joi.lazy(() => schema.ApartmentComplexOrSubClassJoiSchema),
  Joi.lazy(() => schema.GatedResidenceCommunityOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Residence.
 *
 * If some data has a structure which matches a schema:Residence, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeResidence = Residence.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeResidence instanceof OaValidationError) {
 *   // From this point on, `maybeResidence` will have type `OaValidationError`
 *   const error = maybeResidence;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeResidence` will have type `Residence`
 * const residence = maybeResidence;
 * ```
 */
export function validateResidence(maybeResidence: unknown): Residence | OaValidationError {
  const { value, error } = ResidenceJoiSchema.validate(maybeResidence);
  if (error) {
    return new OaValidationError('Residence', maybeResidence, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Residence;
}
