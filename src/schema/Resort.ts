 ...
 * schema:Resort
 *
 * This differs from Resort because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ResortOrSubClass =
  | Resort
 ...
 */
export const ResortJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ResortOrSubClassJoiSchema = Joi.alternatives().try([
  ResortJoiSchema,
 ...
 *
 * ```ts
 * const maybeResort = Resort.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateResort(maybeResort: unknown): Resort | OaValidationError {
  const { value, error } = ResortJoiSchema.validate(maybeResort);
  if (error) {
    return new OaValidationError('Resort', maybeResort, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Resort;
}
