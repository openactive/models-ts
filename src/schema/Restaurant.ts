 ...
 * schema:Restaurant
 *
 * This differs from Restaurant because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RestaurantOrSubClass =
  | Restaurant
  ;

/**
 * schema:Restaurant - Validation schema (w/ JOI)
 */
export const RestaurantJoiSchema = Joi.object({
  '@type': Joi.string().valid('Restaurant').required(),
 ...
 * schema:Restaurant - Validation schema (w/ JOI)
 *
 * This differs from RestaurantJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RestaurantOrSubClassJoiSchema = Joi.alternatives().try([
  RestaurantJoiSchema,
]);

/**
 * Runtime validator for schema:Restaurant.
 *
 * If some data has a structure which matches a schema:Restaurant, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRestaurant = Restaurant.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRestaurant instanceof OaValidationError) {
 *   // From this point on, `maybeRestaurant` will have type `OaValidationError`
 *   const error = maybeRestaurant;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRestaurant` will have type `Restaurant`
 * const restaurant = maybeRestaurant;
 * ```
 */
export function validateRestaurant(maybeRestaurant: unknown): Restaurant | OaValidationError {
  const { value, error } = RestaurantJoiSchema.validate(maybeRestaurant);
  if (error) {
    return new OaValidationError('Restaurant', maybeRestaurant, error);
  }
