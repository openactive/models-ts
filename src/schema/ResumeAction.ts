 ...
 * schema:ResumeAction
 *
 * This differs from ResumeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ResumeActionOrSubClass =
  | ResumeAction
  ;

/**
 * schema:ResumeAction - Validation schema (w/ JOI)
 */
export const ResumeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ResumeAction').required(),
 ...
 * schema:ResumeAction - Validation schema (w/ JOI)
 *
 * This differs from ResumeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ResumeActionOrSubClassJoiSchema = Joi.alternatives().try([
  ResumeActionJoiSchema,
]);

/**
 * Runtime validator for schema:ResumeAction.
 *
 * If some data has a structure which matches a schema:ResumeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeResumeAction = ResumeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeResumeAction instanceof OaValidationError) {
 *   // From this point on, `maybeResumeAction` will have type `OaValidationError`
 *   const error = maybeResumeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeResumeAction` will have type `ResumeAction`
 * const resumeAction = maybeResumeAction;
 * ```
 */
export function validateResumeAction(maybeResumeAction: unknown): ResumeAction | OaValidationError {
  const { value, error } = ResumeActionJoiSchema.validate(maybeResumeAction);
  if (error) {
    return new OaValidationError('ResumeAction', maybeResumeAction, error);
  }
