 ...
 * schema:ReturnAction
 *
 * This differs from ReturnAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReturnActionOrSubClass =
  | ReturnAction
  ;

/**
 * schema:ReturnAction - Validation schema (w/ JOI)
 */
export const ReturnActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReturnAction').required(),
 ...
 * schema:ReturnAction - Validation schema (w/ JOI)
 *
 * This differs from ReturnActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReturnActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReturnActionJoiSchema,
]);

/**
 * Runtime validator for schema:ReturnAction.
 *
 * If some data has a structure which matches a schema:ReturnAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReturnAction = ReturnAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReturnAction instanceof OaValidationError) {
 *   // From this point on, `maybeReturnAction` will have type `OaValidationError`
 *   const error = maybeReturnAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReturnAction` will have type `ReturnAction`
 * const returnAction = maybeReturnAction;
 * ```
 */
export function validateReturnAction(maybeReturnAction: unknown): ReturnAction | OaValidationError {
  const { value, error } = ReturnActionJoiSchema.validate(maybeReturnAction);
  if (error) {
    return new OaValidationError('ReturnAction', maybeReturnAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReturnAction;
}
