 ...
 * ```
 */
export function validateReturnFeesEnumeration(maybeReturnFeesEnumeration: unknown): ReturnFeesEnumeration | OaValidationError {
  const { value, error } = ReturnFeesEnumerationJoiSchema.validate(maybeReturnFeesEnumeration);
  if (error) {
    return new OaValidationError('ReturnFeesEnumeration', maybeReturnFeesEnumeration, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReturnFeesEnumeration;
}
