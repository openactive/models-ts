 ...
 * schema:Review
 *
 * This differs from Review because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReviewOrSubClass =
  | Review
  | schema.MediaReviewOrSubClass
  | schema.UserReviewOrSubClass
  | schema.RecommendationOrSubClass
  | schema.EmployerReviewOrSubClass
  | schema.ClaimReviewOrSubClass
  | schema.CriticReviewOrSubClass
  ;

/**
 * schema:Review - Validation schema (w/ JOI)
 */
export const ReviewJoiSchema = Joi.object({
  '@type': Joi.string().valid('Review').required(),
 ...
 * schema:Review - Validation schema (w/ JOI)
 *
 * This differs from ReviewJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReviewOrSubClassJoiSchema = Joi.alternatives().try([
  ReviewJoiSchema,
  Joi.lazy(() => schema.MediaReviewOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserReviewOrSubClassJoiSchema),
  Joi.lazy(() => schema.RecommendationOrSubClassJoiSchema),
  Joi.lazy(() => schema.EmployerReviewOrSubClassJoiSchema),
  Joi.lazy(() => schema.ClaimReviewOrSubClassJoiSchema),
  Joi.lazy(() => schema.CriticReviewOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Review.
 *
 * If some data has a structure which matches a schema:Review, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReview = validateReview(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReview instanceof OaValidationError) {
 *   // From this point on, `maybeReview` will have type `OaValidationError`
 *   const error = maybeReview;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReview` will have type `Review`
 * const review = maybeReview;
 * ```
 */
export function validateReview(maybeReview: unknown): Review | OaValidationError {
  const { value, error } = ReviewJoiSchema.validate(maybeReview);
  if (error) {
    return new OaValidationError('Review', maybeReview, error);
  }
