...
export type ReviewAction = {
   ...
   */
  resultReview?: schema.ReviewOrSubClass | string;
  /**
 ...
 * schema:ReviewAction
 *
 * This differs from ReviewAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReviewActionOrSubClass =
  | ReviewAction
  ;

/**
 * schema:ReviewAction - Validation schema (w/ JOI)
 */
export const ReviewActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReviewAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  resultReview: Joi.alternatives().try(Joi.lazy(() => schema.ReviewOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ReviewAction - Validation schema (w/ JOI)
 *
 * This differs from ReviewActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReviewActionOrSubClassJoiSchema = Joi.alternatives().try([
  ReviewActionJoiSchema,
]);

/**
 * Runtime validator for schema:ReviewAction.
 *
 * If some data has a structure which matches a schema:ReviewAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReviewAction = ReviewAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReviewAction instanceof OaValidationError) {
 *   // From this point on, `maybeReviewAction` will have type `OaValidationError`
 *   const error = maybeReviewAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReviewAction` will have type `ReviewAction`
 * const reviewAction = maybeReviewAction;
 * ```
 */
export function validateReviewAction(maybeReviewAction: unknown): ReviewAction | OaValidationError {
  const { value, error } = ReviewActionJoiSchema.validate(maybeReviewAction);
  if (error) {
    return new OaValidationError('ReviewAction', maybeReviewAction, error);
  }
