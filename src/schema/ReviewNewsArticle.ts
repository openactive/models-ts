...
export type ReviewNewsArticle = {
   ...
   */
  backstory?: schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  associatedMediaReview?: schema.ReviewOrSubClass | string;
  /**
   ...
   */
  positiveNotes?: schema.WebContentOrSubClass | string | schema.ListItemOrSubClass | schema.ItemListOrSubClass;
  /**
   ...
   */
  associatedClaimReview?: schema.ReviewOrSubClass | string;
  /**
   * An associated [[Review]].
   */
  associatedReview?: schema.ReviewOrSubClass | string;
  /**
   ...
   */
  negativeNotes?: schema.ListItemOrSubClass | string | schema.ItemListOrSubClass | schema.WebContentOrSubClass;
  /**
 ...
 * schema:ReviewNewsArticle
 *
 * This differs from ReviewNewsArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ReviewNewsArticleOrSubClass =
  | ReviewNewsArticle
  ;

/**
 * schema:ReviewNewsArticle - Validation schema (w/ JOI)
 */
export const ReviewNewsArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('ReviewNewsArticle').required(),
  ...
  '@id': Joi.string().uri(),
  dateline: Joi.string(),
  printSection: Joi.string(),
  printPage: Joi.string(),
  printColumn: Joi.string(),
  printEdition: Joi.string(),
  backstory: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  ...
  pageStart: Joi.alternatives().try(Joi.number().integer(), Joi.string()),
  pagination: Joi.string(),
  pageEnd: Joi.alternatives().try(Joi.number().integer(), Joi.string()),
  ...
  associatedMediaReview: Joi.alternatives().try(Joi.lazy(() => schema.ReviewOrSubClassJoiSchema), Joi.string().uri()),
  positiveNotes: Joi.alternatives().try(Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string(), Joi.lazy(() => schema.ListItemOrSubClassJoiSchema), Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.string().uri()),
  reviewAspect: Joi.string(),
  reviewRating: Joi.alternatives().try(Joi.lazy(() => schema.RatingOrSubClassJoiSchema), Joi.string().uri()),
  associatedClaimReview: Joi.alternatives().try(Joi.lazy(() => schema.ReviewOrSubClassJoiSchema), Joi.string().uri()),
  associatedReview: Joi.alternatives().try(Joi.lazy(() => schema.ReviewOrSubClassJoiSchema), Joi.string().uri()),
  reviewBody: Joi.string(),
  negativeNotes: Joi.alternatives().try(Joi.lazy(() => schema.ListItemOrSubClassJoiSchema), Joi.string(), Joi.lazy(() => schema.ItemListOrSubClassJoiSchema), Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ReviewNewsArticle - Validation schema (w/ JOI)
 *
 * This differs from ReviewNewsArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ReviewNewsArticleOrSubClassJoiSchema = Joi.alternatives().try([
  ReviewNewsArticleJoiSchema,
]);

/**
 * Runtime validator for schema:ReviewNewsArticle.
 *
 * If some data has a structure which matches a schema:ReviewNewsArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeReviewNewsArticle = validateReviewNewsArticle(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeReviewNewsArticle instanceof OaValidationError) {
 *   // From this point on, `maybeReviewNewsArticle` will have type `OaValidationError`
 *   const error = maybeReviewNewsArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeReviewNewsArticle` will have type `ReviewNewsArticle`
 * const reviewNewsArticle = maybeReviewNewsArticle;
 * ```
 */
export function validateReviewNewsArticle(maybeReviewNewsArticle: unknown): ReviewNewsArticle | OaValidationError {
  const { value, error } = ReviewNewsArticleJoiSchema.validate(maybeReviewNewsArticle);
  if (error) {
    return new OaValidationError('ReviewNewsArticle', maybeReviewNewsArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ReviewNewsArticle;
}
