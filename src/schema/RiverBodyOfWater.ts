 ...
 * schema:RiverBodyOfWater
 *
 * This differs from RiverBodyOfWater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RiverBodyOfWaterOrSubClass =
  | RiverBodyOfWater
  ;

/**
 * schema:RiverBodyOfWater - Validation schema (w/ JOI)
 */
export const RiverBodyOfWaterJoiSchema = Joi.object({
  '@type': Joi.string().valid('RiverBodyOfWater').required(),
 ...
 * schema:RiverBodyOfWater - Validation schema (w/ JOI)
 *
 * This differs from RiverBodyOfWaterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RiverBodyOfWaterOrSubClassJoiSchema = Joi.alternatives().try([
  RiverBodyOfWaterJoiSchema,
]);

/**
 * Runtime validator for schema:RiverBodyOfWater.
 *
 * If some data has a structure which matches a schema:RiverBodyOfWater, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeRiverBodyOfWater` will have type `RiverBodyOfWater`
 * const riverBodyOfWater = maybeRiverBodyOfWater;
 * ```
 */
export function validateRiverBodyOfWater(maybeRiverBodyOfWater: unknown): RiverBodyOfWater | OaValidationError {
  const { value, error } = RiverBodyOfWaterJoiSchema.validate(maybeRiverBodyOfWater);
  if (error) {
    return new OaValidationError('RiverBodyOfWater', maybeRiverBodyOfWater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RiverBodyOfWater;
}
