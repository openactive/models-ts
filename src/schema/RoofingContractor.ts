 ...
 * schema:RoofingContractor
 *
 * This differs from RoofingContractor because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RoofingContractorOrSubClass =
  | RoofingContractor
  ;

/**
 * schema:RoofingContractor - Validation schema (w/ JOI)
 */
export const RoofingContractorJoiSchema = Joi.object({
  '@type': Joi.string().valid('RoofingContractor').required(),
 ...
 * schema:RoofingContractor - Validation schema (w/ JOI)
 *
 * This differs from RoofingContractorJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RoofingContractorOrSubClassJoiSchema = Joi.alternatives().try([
  RoofingContractorJoiSchema,
]);

/**
 * Runtime validator for schema:RoofingContractor.
 *
 * If some data has a structure which matches a schema:RoofingContractor, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRoofingContractor = RoofingContractor.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRoofingContractor instanceof OaValidationError) {
 *   // From this point on, `maybeRoofingContractor` will have type `OaValidationError`
 *   const error = maybeRoofingContractor;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRoofingContractor` will have type `RoofingContractor`
 * const roofingContractor = maybeRoofingContractor;
 * ```
 */
export function validateRoofingContractor(maybeRoofingContractor: unknown): RoofingContractor | OaValidationError {
  const { value, error } = RoofingContractorJoiSchema.validate(maybeRoofingContractor);
  if (error) {
    return new OaValidationError('RoofingContractor', maybeRoofingContractor, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RoofingContractor;
}
