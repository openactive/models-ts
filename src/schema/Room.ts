 ...
 * schema:Room
 *
 * This differs from Room because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RoomOrSubClass =
  | Room
  | schema.MeetingRoomOrSubClass
  | schema.HotelRoomOrSubClass
  ;

/**
 * schema:Room - Validation schema (w/ JOI)
 */
export const RoomJoiSchema = Joi.object({
  '@type': Joi.string().valid('Room').required(),
 ...
 * schema:Room - Validation schema (w/ JOI)
 *
 * This differs from RoomJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const RoomOrSubClassJoiSchema = Joi.alternatives().try([
  RoomJoiSchema,
  Joi.lazy(() => schema.MeetingRoomOrSubClassJoiSchema),
  Joi.lazy(() => schema.HotelRoomOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Room.
 *
 * If some data has a structure which matches a schema:Room, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeRoom = validateRoom(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeRoom instanceof OaValidationError) {
 *   // From this point on, `maybeRoom` will have type `OaValidationError`
 *   const error = maybeRoom;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeRoom` will have type `Room`
 * const room = maybeRoom;
 * ```
 */
export function validateRoom(maybeRoom: unknown): Room | OaValidationError {
  const { value, error } = RoomJoiSchema.validate(maybeRoom);
  if (error) {
    return new OaValidationError('Room', maybeRoom, error);
  }
