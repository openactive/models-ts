 ...
 * schema:RsvpAction
 *
 * This differs from RsvpAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type RsvpActionOrSubClass =
  | RsvpAction
  ;
 ...
 */
export const RsvpActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const RsvpActionOrSubClassJoiSchema = Joi.alternatives().try([
  RsvpActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeRsvpAction = RsvpAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateRsvpAction(maybeRsvpAction: unknown): RsvpAction | OaValidationError {
  const { value, error } = RsvpActionJoiSchema.validate(maybeRsvpAction);
  if (error) {
    return new OaValidationError('RsvpAction', maybeRsvpAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as RsvpAction;
}
