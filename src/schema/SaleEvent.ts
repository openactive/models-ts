 ...
 * schema:SaleEvent
 *
 * This differs from SaleEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SaleEventOrSubClass =
  | SaleEvent
  ;

/**
 * schema:SaleEvent - Validation schema (w/ JOI)
 */
export const SaleEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('SaleEvent').required(),
 ...
 * schema:SaleEvent - Validation schema (w/ JOI)
 *
 * This differs from SaleEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SaleEventOrSubClassJoiSchema = Joi.alternatives().try([
  SaleEventJoiSchema,
]);

/**
 * Runtime validator for schema:SaleEvent.
 *
 * If some data has a structure which matches a schema:SaleEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSaleEvent = SaleEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSaleEvent instanceof OaValidationError) {
 *   // From this point on, `maybeSaleEvent` will have type `OaValidationError`
 *   const error = maybeSaleEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSaleEvent` will have type `SaleEvent`
 * const saleEvent = maybeSaleEvent;
 * ```
 */
export function validateSaleEvent(maybeSaleEvent: unknown): SaleEvent | OaValidationError {
  const { value, error } = SaleEventJoiSchema.validate(maybeSaleEvent);
  if (error) {
    return new OaValidationError('SaleEvent', maybeSaleEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SaleEvent;
}
