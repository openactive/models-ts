 ...
 * schema:SatiricalArticle
 *
 * This differs from SatiricalArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SatiricalArticleOrSubClass =
  | SatiricalArticle
  ;

/**
 * schema:SatiricalArticle - Validation schema (w/ JOI)
 */
export const SatiricalArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('SatiricalArticle').required(),
 ...
 * schema:SatiricalArticle - Validation schema (w/ JOI)
 *
 * This differs from SatiricalArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SatiricalArticleOrSubClassJoiSchema = Joi.alternatives().try([
  SatiricalArticleJoiSchema,
]);

/**
 * Runtime validator for schema:SatiricalArticle.
 *
 * If some data has a structure which matches a schema:SatiricalArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSatiricalArticle = SatiricalArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSatiricalArticle instanceof OaValidationError) {
 *   // From this point on, `maybeSatiricalArticle` will have type `OaValidationError`
 *   const error = maybeSatiricalArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSatiricalArticle` will have type `SatiricalArticle`
 * const satiricalArticle = maybeSatiricalArticle;
 * ```
 */
export function validateSatiricalArticle(maybeSatiricalArticle: unknown): SatiricalArticle | OaValidationError {
  const { value, error } = SatiricalArticleJoiSchema.validate(maybeSatiricalArticle);
  if (error) {
    return new OaValidationError('SatiricalArticle', maybeSatiricalArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SatiricalArticle;
}
