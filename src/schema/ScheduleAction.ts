 ...
 * schema:ScheduleAction
 *
 * This differs from ScheduleAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ScheduleActionOrSubClass =
  | ScheduleAction
  ;
 ...
 */
export const ScheduleActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ScheduleActionOrSubClassJoiSchema = Joi.alternatives().try([
  ScheduleActionJoiSchema,
]);
 ...
 * ```
 */
export function validateScheduleAction(maybeScheduleAction: unknown): ScheduleAction | OaValidationError {
  const { value, error } = ScheduleActionJoiSchema.validate(maybeScheduleAction);
  if (error) {
    return new OaValidationError('ScheduleAction', maybeScheduleAction, error);
  }
