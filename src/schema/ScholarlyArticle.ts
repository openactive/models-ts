 ...
 * schema:ScholarlyArticle
 *
 * This differs from ScholarlyArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ScholarlyArticleOrSubClass =
  | ScholarlyArticle
  | schema.MedicalScholarlyArticleOrSubClass
  ;

/**
 * schema:ScholarlyArticle - Validation schema (w/ JOI)
 */
export const ScholarlyArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('ScholarlyArticle').required(),
 ...
 * schema:ScholarlyArticle - Validation schema (w/ JOI)
 *
 * This differs from ScholarlyArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ScholarlyArticleOrSubClassJoiSchema = Joi.alternatives().try([
  ScholarlyArticleJoiSchema,
  Joi.lazy(() => schema.MedicalScholarlyArticleOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:ScholarlyArticle.
 *
 * If some data has a structure which matches a schema:ScholarlyArticle, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeScholarlyArticle` will have type `ScholarlyArticle`
 * const scholarlyArticle = maybeScholarlyArticle;
 * ```
 */
export function validateScholarlyArticle(maybeScholarlyArticle: unknown): ScholarlyArticle | OaValidationError {
  const { value, error } = ScholarlyArticleJoiSchema.validate(maybeScholarlyArticle);
  if (error) {
    return new OaValidationError('ScholarlyArticle', maybeScholarlyArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ScholarlyArticle;
}
