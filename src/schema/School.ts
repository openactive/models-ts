 ...
 * `SessionSeries`, etc.
 */
export const SchoolOrSubClassJoiSchema = Joi.alternatives().try([
  SchoolJoiSchema,
]);
 ...
 * ```
 */
export function validateSchool(maybeSchool: unknown): School | OaValidationError {
  const { value, error } = SchoolJoiSchema.validate(maybeSchool);
  if (error) {
    return new OaValidationError('School', maybeSchool, error);
  }
