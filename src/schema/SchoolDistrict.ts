 ...
 * schema:SchoolDistrict
 *
 * This differs from SchoolDistrict because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SchoolDistrictOrSubClass =
  | SchoolDistrict
  ;

/**
 * schema:SchoolDistrict - Validation schema (w/ JOI)
 */
export const SchoolDistrictJoiSchema = Joi.object({
  '@type': Joi.string().valid('SchoolDistrict').required(),
 ...
 * schema:SchoolDistrict - Validation schema (w/ JOI)
 *
 * This differs from SchoolDistrictJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SchoolDistrictOrSubClassJoiSchema = Joi.alternatives().try([
  SchoolDistrictJoiSchema,
]);

/**
 * Runtime validator for schema:SchoolDistrict.
 *
 * If some data has a structure which matches a schema:SchoolDistrict, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSchoolDistrict` will have type `SchoolDistrict`
 * const schoolDistrict = maybeSchoolDistrict;
 * ```
 */
export function validateSchoolDistrict(maybeSchoolDistrict: unknown): SchoolDistrict | OaValidationError {
  const { value, error } = SchoolDistrictJoiSchema.validate(maybeSchoolDistrict);
  if (error) {
    return new OaValidationError('SchoolDistrict', maybeSchoolDistrict, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SchoolDistrict;
}
