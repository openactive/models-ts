...
export type ScreeningEvent = {
   ...
   */
  workPresented?: schema.MovieOrSubClass | string;
  /**
 ...
 * schema:ScreeningEvent
 *
 * This differs from ScreeningEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ScreeningEventOrSubClass =
  | ScreeningEvent
  ;

/**
 * schema:ScreeningEvent - Validation schema (w/ JOI)
 */
export const ScreeningEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('ScreeningEvent').required(),
  ...
  workPresented: Joi.alternatives().try(Joi.lazy(() => schema.MovieOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:ScreeningEvent - Validation schema (w/ JOI)
 *
 * This differs from ScreeningEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ScreeningEventOrSubClassJoiSchema = Joi.alternatives().try([
  ScreeningEventJoiSchema,
]);

/**
 * Runtime validator for schema:ScreeningEvent.
 *
 * If some data has a structure which matches a schema:ScreeningEvent, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeScreeningEvent` will have type `ScreeningEvent`
 * const screeningEvent = maybeScreeningEvent;
 * ```
 */
export function validateScreeningEvent(maybeScreeningEvent: unknown): ScreeningEvent | OaValidationError {
  const { value, error } = ScreeningEventJoiSchema.validate(maybeScreeningEvent);
  if (error) {
    return new OaValidationError('ScreeningEvent', maybeScreeningEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ScreeningEvent;
}
