 ...
 * schema:Sculpture
 *
 * This differs from Sculpture because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SculptureOrSubClass =
  | Sculpture
  ;

/**
 * schema:Sculpture - Validation schema (w/ JOI)
 */
export const SculptureJoiSchema = Joi.object({
  '@type': Joi.string().valid('Sculpture').required(),
 ...
 * schema:Sculpture - Validation schema (w/ JOI)
 *
 * This differs from SculptureJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SculptureOrSubClassJoiSchema = Joi.alternatives().try([
  SculptureJoiSchema,
]);

/**
 * Runtime validator for schema:Sculpture.
 *
 * If some data has a structure which matches a schema:Sculpture, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSculpture = Sculpture.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSculpture instanceof OaValidationError) {
 *   // From this point on, `maybeSculpture` will have type `OaValidationError`
 *   const error = maybeSculpture;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSculpture` will have type `Sculpture`
 * const sculpture = maybeSculpture;
 * ```
 */
export function validateSculpture(maybeSculpture: unknown): Sculpture | OaValidationError {
  const { value, error } = SculptureJoiSchema.validate(maybeSculpture);
  if (error) {
    return new OaValidationError('Sculpture', maybeSculpture, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Sculpture;
}
