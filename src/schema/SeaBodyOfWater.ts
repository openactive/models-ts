 ...
 * schema:SeaBodyOfWater
 *
 * This differs from SeaBodyOfWater because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SeaBodyOfWaterOrSubClass =
  | SeaBodyOfWater
  ;

/**
 * schema:SeaBodyOfWater - Validation schema (w/ JOI)
 */
export const SeaBodyOfWaterJoiSchema = Joi.object({
  '@type': Joi.string().valid('SeaBodyOfWater').required(),
 ...
 * schema:SeaBodyOfWater - Validation schema (w/ JOI)
 *
 * This differs from SeaBodyOfWaterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SeaBodyOfWaterOrSubClassJoiSchema = Joi.alternatives().try([
  SeaBodyOfWaterJoiSchema,
]);

/**
 * Runtime validator for schema:SeaBodyOfWater.
 *
 * If some data has a structure which matches a schema:SeaBodyOfWater, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSeaBodyOfWater` will have type `SeaBodyOfWater`
 * const seaBodyOfWater = maybeSeaBodyOfWater;
 * ```
 */
export function validateSeaBodyOfWater(maybeSeaBodyOfWater: unknown): SeaBodyOfWater | OaValidationError {
  const { value, error } = SeaBodyOfWaterJoiSchema.validate(maybeSeaBodyOfWater);
  if (error) {
    return new OaValidationError('SeaBodyOfWater', maybeSeaBodyOfWater, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SeaBodyOfWater;
}
