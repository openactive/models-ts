 ...
 * schema:SearchAction
 *
 * This differs from SearchAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SearchActionOrSubClass =
  | SearchAction
  ;

/**
 * schema:SearchAction - Validation schema (w/ JOI)
 */
export const SearchActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('SearchAction').required(),
 ...
 * schema:SearchAction - Validation schema (w/ JOI)
 *
 * This differs from SearchActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SearchActionOrSubClassJoiSchema = Joi.alternatives().try([
  SearchActionJoiSchema,
]);

/**
 * Runtime validator for schema:SearchAction.
 *
 * If some data has a structure which matches a schema:SearchAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSearchAction = SearchAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSearchAction instanceof OaValidationError) {
 *   // From this point on, `maybeSearchAction` will have type `OaValidationError`
 *   const error = maybeSearchAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSearchAction` will have type `SearchAction`
 * const searchAction = maybeSearchAction;
 * ```
 */
export function validateSearchAction(maybeSearchAction: unknown): SearchAction | OaValidationError {
  const { value, error } = SearchActionJoiSchema.validate(maybeSearchAction);
  if (error) {
    return new OaValidationError('SearchAction', maybeSearchAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SearchAction;
}
