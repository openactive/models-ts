 ...
 * schema:SearchResultsPage
 *
 * This differs from SearchResultsPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SearchResultsPageOrSubClass =
  | SearchResultsPage
  ;

/**
 * schema:SearchResultsPage - Validation schema (w/ JOI)
 */
export const SearchResultsPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('SearchResultsPage').required(),
 ...
 * schema:SearchResultsPage - Validation schema (w/ JOI)
 *
 * This differs from SearchResultsPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SearchResultsPageOrSubClassJoiSchema = Joi.alternatives().try([
  SearchResultsPageJoiSchema,
]);

/**
 * Runtime validator for schema:SearchResultsPage.
 *
 * If some data has a structure which matches a schema:SearchResultsPage, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSearchResultsPage` will have type `SearchResultsPage`
 * const searchResultsPage = maybeSearchResultsPage;
 * ```
 */
export function validateSearchResultsPage(maybeSearchResultsPage: unknown): SearchResultsPage | OaValidationError {
  const { value, error } = SearchResultsPageJoiSchema.validate(maybeSearchResultsPage);
  if (error) {
    return new OaValidationError('SearchResultsPage', maybeSearchResultsPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SearchResultsPage;
}
