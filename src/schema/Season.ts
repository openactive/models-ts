 ...
 * schema:Season
 *
 * This differs from Season because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SeasonOrSubClass =
  | Season
  ;

/**
 * schema:Season - Validation schema (w/ JOI)
 */
export const SeasonJoiSchema = Joi.object({
  '@type': Joi.string().valid('Season').required(),
 ...
 * schema:Season - Validation schema (w/ JOI)
 *
 * This differs from SeasonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SeasonOrSubClassJoiSchema = Joi.alternatives().try([
  SeasonJoiSchema,
]);

/**
 * Runtime validator for schema:Season.
 *
 * If some data has a structure which matches a schema:Season, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSeason = Season.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSeason instanceof OaValidationError) {
 *   // From this point on, `maybeSeason` will have type `OaValidationError`
 *   const error = maybeSeason;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSeason` will have type `Season`
 * const season = maybeSeason;
 * ```
 */
export function validateSeason(maybeSeason: unknown): Season | OaValidationError {
  const { value, error } = SeasonJoiSchema.validate(maybeSeason);
  if (error) {
    return new OaValidationError('Season', maybeSeason, error);
  }
