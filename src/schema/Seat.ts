...
export type Seat = {
   ...
   */
  seatingType?: string | schema.QualitativeValue;
  /**
 ...
 * schema:Seat
 *
 * This differs from Seat because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SeatOrSubClass =
  | Seat
  ;
 ...
 */
export const SeatJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SeatOrSubClassJoiSchema = Joi.alternatives().try([
  SeatJoiSchema,
]);
 ...
 * ```
 */
export function validateSeat(maybeSeat: unknown): Seat | OaValidationError {
  const { value, error } = SeatJoiSchema.validate(maybeSeat);
  if (error) {
    return new OaValidationError('Seat', maybeSeat, error);
  }
