 ...
 * schema:SeekToAction
 *
 * This differs from SeekToAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SeekToActionOrSubClass =
  | SeekToAction
  ;

/**
 * schema:SeekToAction - Validation schema (w/ JOI)
 */
export const SeekToActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('SeekToAction').required(),
 ...
 * schema:SeekToAction - Validation schema (w/ JOI)
 *
 * This differs from SeekToActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SeekToActionOrSubClassJoiSchema = Joi.alternatives().try([
  SeekToActionJoiSchema,
]);

/**
 * Runtime validator for schema:SeekToAction.
 *
 * If some data has a structure which matches a schema:SeekToAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSeekToAction = validateSeekToAction(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSeekToAction instanceof OaValidationError) {
 *   // From this point on, `maybeSeekToAction` will have type `OaValidationError`
 *   const error = maybeSeekToAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSeekToAction` will have type `SeekToAction`
 * const seekToAction = maybeSeekToAction;
 * ```
 */
export function validateSeekToAction(maybeSeekToAction: unknown): SeekToAction | OaValidationError {
  const { value, error } = SeekToActionJoiSchema.validate(maybeSeekToAction);
  if (error) {
    return new OaValidationError('SeekToAction', maybeSeekToAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SeekToAction;
}
