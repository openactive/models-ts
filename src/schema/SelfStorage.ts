 ...
 * schema:SelfStorage
 *
 * This differs from SelfStorage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SelfStorageOrSubClass =
  | SelfStorage
  ;

/**
 * schema:SelfStorage - Validation schema (w/ JOI)
 */
export const SelfStorageJoiSchema = Joi.object({
  '@type': Joi.string().valid('SelfStorage').required(),
 ...
 * schema:SelfStorage - Validation schema (w/ JOI)
 *
 * This differs from SelfStorageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SelfStorageOrSubClassJoiSchema = Joi.alternatives().try([
  SelfStorageJoiSchema,
]);

/**
 * Runtime validator for schema:SelfStorage.
 *
 * If some data has a structure which matches a schema:SelfStorage, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSelfStorage` will have type `SelfStorage`
 * const selfStorage = maybeSelfStorage;
 * ```
 */
export function validateSelfStorage(maybeSelfStorage: unknown): SelfStorage | OaValidationError {
  const { value, error } = SelfStorageJoiSchema.validate(maybeSelfStorage);
  if (error) {
    return new OaValidationError('SelfStorage', maybeSelfStorage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SelfStorage;
}
