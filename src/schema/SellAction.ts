...
export type SellAction = {
   ...
   */
  buyer?: schema.PersonOrSubClass | schema.OrganizationOrSubClass | string;
  /**
 ...
 * schema:SellAction
 *
 * This differs from SellAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SellActionOrSubClass =
  | SellAction
  ;
 ...
 */
export const SellActionJoiSchema = Joi.object({
  ...
  '@id': Joi.string().uri(),
  buyer: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.OrganizationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const SellActionOrSubClassJoiSchema = Joi.alternatives().try([
  SellActionJoiSchema,
]);
 ...
 * ```
 */
export function validateSellAction(maybeSellAction: unknown): SellAction | OaValidationError {
  const { value, error } = SellActionJoiSchema.validate(maybeSellAction);
  if (error) {
    return new OaValidationError('SellAction', maybeSellAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SellAction;
}
