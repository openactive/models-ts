 ...
 * schema:Service
 *
 * This differs from Service because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ServiceOrSubClass =
  | Service
  | oa.BookingServiceOrSubClass
  | schema.FinancialProductOrSubClass
  | schema.FoodServiceOrSubClass
  | schema.CableOrSatelliteServiceOrSubClass
  | schema.BroadcastServiceOrSubClass
  | schema.TaxiOrSubClass
  | schema.GovernmentServiceOrSubClass
  | schema.WebAPIOrSubClass
  | schema.TaxiServiceOrSubClass
  ;

/**
 * schema:Service - Validation schema (w/ JOI)
 */
export const ServiceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const ServiceOrSubClassJoiSchema = Joi.alternatives().try([
  ServiceJoiSchema,
 ...
 * ```
 */
export function validateService(maybeService: unknown): Service | OaValidationError {
  const { value, error } = ServiceJoiSchema.validate(maybeService);
  if (error) {
    return new OaValidationError('Service', maybeService, error);
  }
