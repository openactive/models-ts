 ...
 * schema:ShareAction
 *
 * This differs from ShareAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ShareActionOrSubClass =
  | ShareAction
  ;

/**
 * schema:ShareAction - Validation schema (w/ JOI)
 */
export const ShareActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ShareAction').required(),
 ...
 * schema:ShareAction - Validation schema (w/ JOI)
 *
 * This differs from ShareActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ShareActionOrSubClassJoiSchema = Joi.alternatives().try([
  ShareActionJoiSchema,
]);

/**
 * Runtime validator for schema:ShareAction.
 *
 * If some data has a structure which matches a schema:ShareAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeShareAction` will have type `ShareAction`
 * const shareAction = maybeShareAction;
 * ```
 */
export function validateShareAction(maybeShareAction: unknown): ShareAction | OaValidationError {
  const { value, error } = ShareActionJoiSchema.validate(maybeShareAction);
  if (error) {
    return new OaValidationError('ShareAction', maybeShareAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ShareAction;
}
