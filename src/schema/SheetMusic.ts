 ...
 * schema:SheetMusic
 *
 * This differs from SheetMusic because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SheetMusicOrSubClass =
  | SheetMusic
  ;

/**
 * schema:SheetMusic - Validation schema (w/ JOI)
 */
export const SheetMusicJoiSchema = Joi.object({
  '@type': Joi.string().valid('SheetMusic').required(),
 ...
 * schema:SheetMusic - Validation schema (w/ JOI)
 *
 * This differs from SheetMusicJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SheetMusicOrSubClassJoiSchema = Joi.alternatives().try([
  SheetMusicJoiSchema,
]);

/**
 * Runtime validator for schema:SheetMusic.
 *
 * If some data has a structure which matches a schema:SheetMusic, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSheetMusic = SheetMusic.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSheetMusic instanceof OaValidationError) {
 *   // From this point on, `maybeSheetMusic` will have type `OaValidationError`
 *   const error = maybeSheetMusic;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSheetMusic` will have type `SheetMusic`
 * const sheetMusic = maybeSheetMusic;
 * ```
 */
export function validateSheetMusic(maybeSheetMusic: unknown): SheetMusic | OaValidationError {
  const { value, error } = SheetMusicJoiSchema.validate(maybeSheetMusic);
  if (error) {
    return new OaValidationError('SheetMusic', maybeSheetMusic, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SheetMusic;
}
