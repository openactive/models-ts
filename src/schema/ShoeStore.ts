 ...
 * schema:ShoeStore
 *
 * This differs from ShoeStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ShoeStoreOrSubClass =
  | ShoeStore
  ;

/**
 * schema:ShoeStore - Validation schema (w/ JOI)
 */
export const ShoeStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('ShoeStore').required(),
 ...
 * schema:ShoeStore - Validation schema (w/ JOI)
 *
 * This differs from ShoeStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ShoeStoreOrSubClassJoiSchema = Joi.alternatives().try([
  ShoeStoreJoiSchema,
]);

/**
 * Runtime validator for schema:ShoeStore.
 *
 * If some data has a structure which matches a schema:ShoeStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeShoeStore = ShoeStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeShoeStore instanceof OaValidationError) {
 *   // From this point on, `maybeShoeStore` will have type `OaValidationError`
 *   const error = maybeShoeStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeShoeStore` will have type `ShoeStore`
 * const shoeStore = maybeShoeStore;
 * ```
 */
export function validateShoeStore(maybeShoeStore: unknown): ShoeStore | OaValidationError {
  const { value, error } = ShoeStoreJoiSchema.validate(maybeShoeStore);
  if (error) {
    return new OaValidationError('ShoeStore', maybeShoeStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ShoeStore;
}
