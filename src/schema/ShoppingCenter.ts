 ...
 * schema:ShoppingCenter
 *
 * This differs from ShoppingCenter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ShoppingCenterOrSubClass =
  | ShoppingCenter
  ;

/**
 * schema:ShoppingCenter - Validation schema (w/ JOI)
 */
export const ShoppingCenterJoiSchema = Joi.object({
  '@type': Joi.string().valid('ShoppingCenter').required(),
 ...
 * schema:ShoppingCenter - Validation schema (w/ JOI)
 *
 * This differs from ShoppingCenterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ShoppingCenterOrSubClassJoiSchema = Joi.alternatives().try([
  ShoppingCenterJoiSchema,
]);

/**
 * Runtime validator for schema:ShoppingCenter.
 *
 * If some data has a structure which matches a schema:ShoppingCenter, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeShoppingCenter = ShoppingCenter.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeShoppingCenter instanceof OaValidationError) {
 *   // From this point on, `maybeShoppingCenter` will have type `OaValidationError`
 *   const error = maybeShoppingCenter;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeShoppingCenter` will have type `ShoppingCenter`
 * const shoppingCenter = maybeShoppingCenter;
 * ```
 */
export function validateShoppingCenter(maybeShoppingCenter: unknown): ShoppingCenter | OaValidationError {
  const { value, error } = ShoppingCenterJoiSchema.validate(maybeShoppingCenter);
  if (error) {
    return new OaValidationError('ShoppingCenter', maybeShoppingCenter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ShoppingCenter;
}
