 ...
 * schema:ShortStory
 *
 * This differs from ShortStory because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ShortStoryOrSubClass =
  | ShortStory
  ;

/**
 * schema:ShortStory - Validation schema (w/ JOI)
 */
export const ShortStoryJoiSchema = Joi.object({
  '@type': Joi.string().valid('ShortStory').required(),
 ...
 * schema:ShortStory - Validation schema (w/ JOI)
 *
 * This differs from ShortStoryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ShortStoryOrSubClassJoiSchema = Joi.alternatives().try([
  ShortStoryJoiSchema,
]);

/**
 * Runtime validator for schema:ShortStory.
 *
 * If some data has a structure which matches a schema:ShortStory, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeShortStory = ShortStory.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeShortStory instanceof OaValidationError) {
 *   // From this point on, `maybeShortStory` will have type `OaValidationError`
 *   const error = maybeShortStory;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeShortStory` will have type `ShortStory`
 * const shortStory = maybeShortStory;
 * ```
 */
export function validateShortStory(maybeShortStory: unknown): ShortStory | OaValidationError {
  const { value, error } = ShortStoryJoiSchema.validate(maybeShortStory);
  if (error) {
    return new OaValidationError('ShortStory', maybeShortStory, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ShortStory;
}
