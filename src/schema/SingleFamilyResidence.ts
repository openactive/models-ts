 ...
 */
export type SingleFamilyResidence = {
 ...
 * schema:SingleFamilyResidence
 *
 * This differs from SingleFamilyResidence because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SingleFamilyResidenceOrSubClass =
 ...
 */
export const SingleFamilyResidenceJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SingleFamilyResidenceOrSubClassJoiSchema = Joi.alternatives().try([
  SingleFamilyResidenceJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeSingleFamilyResidence = SingleFamilyResidence.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateSingleFamilyResidence(maybeSingleFamilyResidence: unknown): SingleFamilyResidence | OaValidationError {
  const { value, error } = SingleFamilyResidenceJoiSchema.validate(maybeSingleFamilyResidence);
  if (error) {
    return new OaValidationError('SingleFamilyResidence', maybeSingleFamilyResidence, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SingleFamilyResidence;
}
