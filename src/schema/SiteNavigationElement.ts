 ...
 */
export type SiteNavigationElement = {
 ...
 * schema:SiteNavigationElement
 *
 * This differs from SiteNavigationElement because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SiteNavigationElementOrSubClass =
  | SiteNavigationElement
  ;

/**
 * schema:SiteNavigationElement - Validation schema (w/ JOI)
 */
export const SiteNavigationElementJoiSchema = Joi.object({
  '@type': Joi.string().valid('SiteNavigationElement').required(),
 ...
 * schema:SiteNavigationElement - Validation schema (w/ JOI)
 *
 * This differs from SiteNavigationElementJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SiteNavigationElementOrSubClassJoiSchema = Joi.alternatives().try([
  SiteNavigationElementJoiSchema,
]);

/**
 * Runtime validator for schema:SiteNavigationElement.
 *
 * If some data has a structure which matches a schema:SiteNavigationElement, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSiteNavigationElement` will have type `SiteNavigationElement`
 * const siteNavigationElement = maybeSiteNavigationElement;
 * ```
 */
export function validateSiteNavigationElement(maybeSiteNavigationElement: unknown): SiteNavigationElement | OaValidationError {
  const { value, error } = SiteNavigationElementJoiSchema.validate(maybeSiteNavigationElement);
  if (error) {
    return new OaValidationError('SiteNavigationElement', maybeSiteNavigationElement, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SiteNavigationElement;
}
