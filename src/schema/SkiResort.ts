 ...
 * schema:SkiResort
 *
 * This differs from SkiResort because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SkiResortOrSubClass =
  | SkiResort
  ;

/**
 * schema:SkiResort - Validation schema (w/ JOI)
 */
export const SkiResortJoiSchema = Joi.object({
  '@type': Joi.string().valid('SkiResort').required(),
 ...
 * schema:SkiResort - Validation schema (w/ JOI)
 *
 * This differs from SkiResortJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SkiResortOrSubClassJoiSchema = Joi.alternatives().try([
  SkiResortJoiSchema,
]);

/**
 * Runtime validator for schema:SkiResort.
 *
 * If some data has a structure which matches a schema:SkiResort, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSkiResort = validateSkiResort(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSkiResort instanceof OaValidationError) {
 *   // From this point on, `maybeSkiResort` will have type `OaValidationError`
 *   const error = maybeSkiResort;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSkiResort` will have type `SkiResort`
 * const skiResort = maybeSkiResort;
 * ```
 */
export function validateSkiResort(maybeSkiResort: unknown): SkiResort | OaValidationError {
  const { value, error } = SkiResortJoiSchema.validate(maybeSkiResort);
  if (error) {
    return new OaValidationError('SkiResort', maybeSkiResort, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SkiResort;
}
