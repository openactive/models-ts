 ...
 * schema:SocialEvent
 *
 * This differs from SocialEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SocialEventOrSubClass =
  | SocialEvent
  ;
 ...
 */
export const SocialEventJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SocialEventOrSubClassJoiSchema = Joi.alternatives().try([
  SocialEventJoiSchema,
]);
 ...
 * ```
 */
export function validateSocialEvent(maybeSocialEvent: unknown): SocialEvent | OaValidationError {
  const { value, error } = SocialEventJoiSchema.validate(maybeSocialEvent);
  if (error) {
    return new OaValidationError('SocialEvent', maybeSocialEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SocialEvent;
}
