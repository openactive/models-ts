 ...
 * schema:SocialMediaPosting
 *
 * This differs from SocialMediaPosting because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SocialMediaPostingOrSubClass =
  | SocialMediaPosting
  | schema.BlogPostingOrSubClass
  | schema.DiscussionForumPostingOrSubClass
  ;

/**
 * schema:SocialMediaPosting - Validation schema (w/ JOI)
 */
export const SocialMediaPostingJoiSchema = Joi.object({
  '@type': Joi.string().valid('SocialMediaPosting').required(),
 ...
 * schema:SocialMediaPosting - Validation schema (w/ JOI)
 *
 * This differs from SocialMediaPostingJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SocialMediaPostingOrSubClassJoiSchema = Joi.alternatives().try([
  SocialMediaPostingJoiSchema,
  Joi.lazy(() => schema.BlogPostingOrSubClassJoiSchema),
  Joi.lazy(() => schema.DiscussionForumPostingOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:SocialMediaPosting.
 *
 * If some data has a structure which matches a schema:SocialMediaPosting, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSocialMediaPosting` will have type `SocialMediaPosting`
 * const socialMediaPosting = maybeSocialMediaPosting;
 * ```
 */
export function validateSocialMediaPosting(maybeSocialMediaPosting: unknown): SocialMediaPosting | OaValidationError {
  const { value, error } = SocialMediaPostingJoiSchema.validate(maybeSocialMediaPosting);
  if (error) {
    return new OaValidationError('SocialMediaPosting', maybeSocialMediaPosting, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SocialMediaPosting;
}
