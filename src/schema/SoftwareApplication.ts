 ...
 * schema:SoftwareApplication
 *
 * This differs from SoftwareApplication because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SoftwareApplicationOrSubClass =
  | SoftwareApplication
  | schema.MobileApplicationOrSubClass
  | schema.WebApplicationOrSubClass
 ...
 */
export const SoftwareApplicationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SoftwareApplicationOrSubClassJoiSchema = Joi.alternatives().try([
  SoftwareApplicationJoiSchema,
 ...
 *
 * ```ts
 * const maybeSoftwareApplication = validateSoftwareApplication(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateSoftwareApplication(maybeSoftwareApplication: unknown): SoftwareApplication | OaValidationError {
  const { value, error } = SoftwareApplicationJoiSchema.validate(maybeSoftwareApplication);
  if (error) {
    return new OaValidationError('SoftwareApplication', maybeSoftwareApplication, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SoftwareApplication;
}
