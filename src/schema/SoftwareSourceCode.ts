...
export type SoftwareSourceCode = {
   ...
   */
  programmingLanguage?: string | schema.ComputerLanguageOrSubClass;
  /**
   ...
   */
  targetProduct?: schema.SoftwareApplicationOrSubClass | string;
  /**
   * What type of code sample: full (compile ready) solution, code snippet, inline code, scripts, template.
   */
  sampleType?: string;
 ...
 * schema:SoftwareSourceCode
 *
 * This differs from SoftwareSourceCode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SoftwareSourceCodeOrSubClass =
  | SoftwareSourceCode
  ;

/**
 * schema:SoftwareSourceCode - Validation schema (w/ JOI)
 */
export const SoftwareSourceCodeJoiSchema = Joi.object({
  '@type': Joi.string().valid('SoftwareSourceCode').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  runtimePlatform: Joi.string(),
  programmingLanguage: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.ComputerLanguageOrSubClassJoiSchema), Joi.string().uri()),
  codeSampleType: Joi.string(),
  codeRepository: Joi.string().uri(),
  runtime: Joi.string(),
  targetProduct: Joi.alternatives().try(Joi.lazy(() => schema.SoftwareApplicationOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:SoftwareSourceCode - Validation schema (w/ JOI)
 *
 * This differs from SoftwareSourceCodeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SoftwareSourceCodeOrSubClassJoiSchema = Joi.alternatives().try([
  SoftwareSourceCodeJoiSchema,
]);

/**
 * Runtime validator for schema:SoftwareSourceCode.
 *
 * If some data has a structure which matches a schema:SoftwareSourceCode, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSoftwareSourceCode = SoftwareSourceCode.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSoftwareSourceCode instanceof OaValidationError) {
 *   // From this point on, `maybeSoftwareSourceCode` will have type `OaValidationError`
 *   const error = maybeSoftwareSourceCode;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSoftwareSourceCode` will have type `SoftwareSourceCode`
 * const softwareSourceCode = maybeSoftwareSourceCode;
 * ```
 */
export function validateSoftwareSourceCode(maybeSoftwareSourceCode: unknown): SoftwareSourceCode | OaValidationError {
  const { value, error } = SoftwareSourceCodeJoiSchema.validate(maybeSoftwareSourceCode);
  if (error) {
    return new OaValidationError('SoftwareSourceCode', maybeSoftwareSourceCode, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SoftwareSourceCode;
}
