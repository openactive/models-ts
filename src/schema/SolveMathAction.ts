 ...
 * schema:SolveMathAction
 *
 * This differs from SolveMathAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SolveMathActionOrSubClass =
  | SolveMathAction
  ;
 ...
 */
export const SolveMathActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SolveMathActionOrSubClassJoiSchema = Joi.alternatives().try([
  SolveMathActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeSolveMathAction = validateSolveMathAction(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateSolveMathAction(maybeSolveMathAction: unknown): SolveMathAction | OaValidationError {
  const { value, error } = SolveMathActionJoiSchema.validate(maybeSolveMathAction);
  if (error) {
    return new OaValidationError('SolveMathAction', maybeSolveMathAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SolveMathAction;
}
