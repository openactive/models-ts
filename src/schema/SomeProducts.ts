 ...
 * schema:SomeProducts
 *
 * This differs from SomeProducts because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SomeProductsOrSubClass =
  | SomeProducts
  ;

/**
 * schema:SomeProducts - Validation schema (w/ JOI)
 */
export const SomeProductsJoiSchema = Joi.object({
  '@type': Joi.string().valid('SomeProducts').required(),
 ...
 * schema:SomeProducts - Validation schema (w/ JOI)
 *
 * This differs from SomeProductsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SomeProductsOrSubClassJoiSchema = Joi.alternatives().try([
  SomeProductsJoiSchema,
]);

/**
 * Runtime validator for schema:SomeProducts.
 *
 * If some data has a structure which matches a schema:SomeProducts, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSomeProducts = SomeProducts.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSomeProducts instanceof OaValidationError) {
 *   // From this point on, `maybeSomeProducts` will have type `OaValidationError`
 *   const error = maybeSomeProducts;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSomeProducts` will have type `SomeProducts`
 * const someProducts = maybeSomeProducts;
 * ```
 */
export function validateSomeProducts(maybeSomeProducts: unknown): SomeProducts | OaValidationError {
  const { value, error } = SomeProductsJoiSchema.validate(maybeSomeProducts);
  if (error) {
    return new OaValidationError('SomeProducts', maybeSomeProducts, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SomeProducts;
}
