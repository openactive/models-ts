 ...
 * schema:SpeakableSpecification
 *
 * This differs from SpeakableSpecification because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SpeakableSpecificationOrSubClass =
 ...
 */
export const SpeakableSpecificationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SpeakableSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  SpeakableSpecificationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeSpeakableSpecification = SpeakableSpecification.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateSpeakableSpecification(maybeSpeakableSpecification: unknown): SpeakableSpecification | OaValidationError {
  const { value, error } = SpeakableSpecificationJoiSchema.validate(maybeSpeakableSpecification);
  if (error) {
    return new OaValidationError('SpeakableSpecification', maybeSpeakableSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SpeakableSpecification;
}
