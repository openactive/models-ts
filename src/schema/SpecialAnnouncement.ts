...
export type SpecialAnnouncement = {
   ...
   */
  gettingTestedInfo?: schema.WebContentOrSubClass | string;
  /**
   * Information about school closures.
   */
  schoolClosuresInfo?: string | schema.WebContentOrSubClass;
  /**
   * Statistical information about the spread of a disease, either as [[WebContent]], or
   *   described directly as a [[Dataset]], or the specific [[Observation]]s in the dataset. When a [[WebContent]] URL is
   *   provided, the page indicated might also contain more such markup.
   */
  diseaseSpreadStatistics?: schema.DatasetOrSubClass | schema.WebContentOrSubClass | schema.ObservationOrSubClass | string;
  /**
   * Information about public transport closures.
   */
  publicTransportClosuresInfo?: string | schema.WebContentOrSubClass;
  /**
   * Information about travel bans, e.g. in the context of a pandemic.
   */
  travelBans?: schema.WebContentOrSubClass | string;
  /**
   * The URL for a feed, e.g. associated with a podcast series, blog, or series of date-stamped updates. This is usually RSS or Atom.
   */
  webFeed?: string | schema.DataFeedOrSubClass;
  /**
   * Indicates a page with news updates and guidelines. This could often be (but is not required to be) the main page containing [[SpecialAnnouncement]] markup on a site.
   */
  newsUpdatesAndGuidelines?: schema.WebContentOrSubClass | string;
  /**
   ...
   */
  diseasePreventionInfo?: string | schema.WebContentOrSubClass;
  /**
   * Indicates a specific [[CivicStructure]] or [[LocalBusiness]] associated with the SpecialAnnouncement. For example, a specific testing facility or business with special opening hours. For a larger geographic region like a quarantine of an entire region, use [[spatialCoverage]].
   */
  announcementLocation?: schema.CivicStructureOrSubClass | schema.LocalBusinessOrSubClass | string;
  /**
   * Guidelines about quarantine rules, e.g. in the context of a pandemic.
   */
  quarantineGuidelines?: schema.WebContentOrSubClass | string;
  /**
   * governmentBenefitsInfo provides information about government benefits associated with a SpecialAnnouncement.
   */
  governmentBenefitsInfo?: schema.GovernmentServiceOrSubClass | string;
  /**
 ...
 * schema:SpecialAnnouncement
 *
 * This differs from SpecialAnnouncement because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SpecialAnnouncementOrSubClass =
  | SpecialAnnouncement
  ;

/**
 * schema:SpecialAnnouncement - Validation schema (w/ JOI)
 */
export const SpecialAnnouncementJoiSchema = Joi.object({
  '@type': Joi.string().valid('SpecialAnnouncement').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  gettingTestedInfo: Joi.alternatives().try(Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string().uri()),
  schoolClosuresInfo: Joi.alternatives().try(Joi.string().uri(), Joi.lazy(() => schema.WebContentOrSubClassJoiSchema)),
  diseaseSpreadStatistics: Joi.alternatives().try(Joi.lazy(() => schema.DatasetOrSubClassJoiSchema), Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.lazy(() => schema.ObservationOrSubClassJoiSchema), Joi.string().uri()),
  publicTransportClosuresInfo: Joi.alternatives().try(Joi.string().uri(), Joi.lazy(() => schema.WebContentOrSubClassJoiSchema)),
  travelBans: Joi.alternatives().try(Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string().uri()),
  webFeed: Joi.alternatives().try(Joi.string().uri(), Joi.lazy(() => schema.DataFeedOrSubClassJoiSchema)),
  newsUpdatesAndGuidelines: Joi.alternatives().try(Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string().uri()),
  ...
  diseasePreventionInfo: Joi.alternatives().try(Joi.string().uri(), Joi.lazy(() => schema.WebContentOrSubClassJoiSchema)),
  announcementLocation: Joi.alternatives().try(Joi.lazy(() => schema.CivicStructureOrSubClassJoiSchema), Joi.lazy(() => schema.LocalBusinessOrSubClassJoiSchema), Joi.string().uri()),
  quarantineGuidelines: Joi.alternatives().try(Joi.lazy(() => schema.WebContentOrSubClassJoiSchema), Joi.string().uri()),
  governmentBenefitsInfo: Joi.alternatives().try(Joi.lazy(() => schema.GovernmentServiceOrSubClassJoiSchema), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:SpecialAnnouncement - Validation schema (w/ JOI)
 *
 * This differs from SpecialAnnouncementJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SpecialAnnouncementOrSubClassJoiSchema = Joi.alternatives().try([
  SpecialAnnouncementJoiSchema,
]);

/**
 * Runtime validator for schema:SpecialAnnouncement.
 *
 * If some data has a structure which matches a schema:SpecialAnnouncement, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSpecialAnnouncement` will have type `SpecialAnnouncement`
 * const specialAnnouncement = maybeSpecialAnnouncement;
 * ```
 */
export function validateSpecialAnnouncement(maybeSpecialAnnouncement: unknown): SpecialAnnouncement | OaValidationError {
  const { value, error } = SpecialAnnouncementJoiSchema.validate(maybeSpecialAnnouncement);
  if (error) {
    return new OaValidationError('SpecialAnnouncement', maybeSpecialAnnouncement, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SpecialAnnouncement;
}
