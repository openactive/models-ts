 ...
 */
export type SportingGoodsStore = {
 ...
 * schema:SportingGoodsStore
 *
 * This differs from SportingGoodsStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SportingGoodsStoreOrSubClass =
  | SportingGoodsStore
  ;

/**
 * schema:SportingGoodsStore - Validation schema (w/ JOI)
 */
export const SportingGoodsStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('SportingGoodsStore').required(),
 ...
 * schema:SportingGoodsStore - Validation schema (w/ JOI)
 *
 * This differs from SportingGoodsStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SportingGoodsStoreOrSubClassJoiSchema = Joi.alternatives().try([
  SportingGoodsStoreJoiSchema,
]);

/**
 * Runtime validator for schema:SportingGoodsStore.
 *
 * If some data has a structure which matches a schema:SportingGoodsStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSportingGoodsStore = SportingGoodsStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSportingGoodsStore instanceof OaValidationError) {
 *   // From this point on, `maybeSportingGoodsStore` will have type `OaValidationError`
 *   const error = maybeSportingGoodsStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSportingGoodsStore` will have type `SportingGoodsStore`
 * const sportingGoodsStore = maybeSportingGoodsStore;
 * ```
 */
export function validateSportingGoodsStore(maybeSportingGoodsStore: unknown): SportingGoodsStore | OaValidationError {
  const { value, error } = SportingGoodsStoreJoiSchema.validate(maybeSportingGoodsStore);
  if (error) {
    return new OaValidationError('SportingGoodsStore', maybeSportingGoodsStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SportingGoodsStore;
}
