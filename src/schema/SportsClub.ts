 ...
 * schema:SportsClub
 *
 * This differs from SportsClub because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SportsClubOrSubClass =
  | SportsClub
  ;

/**
 * schema:SportsClub - Validation schema (w/ JOI)
 */
export const SportsClubJoiSchema = Joi.object({
  '@type': Joi.string().valid('SportsClub').required(),
 ...
 * schema:SportsClub - Validation schema (w/ JOI)
 *
 * This differs from SportsClubJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SportsClubOrSubClassJoiSchema = Joi.alternatives().try([
  SportsClubJoiSchema,
]);

/**
 * Runtime validator for schema:SportsClub.
 *
 * If some data has a structure which matches a schema:SportsClub, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSportsClub = SportsClub.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSportsClub instanceof OaValidationError) {
 *   // From this point on, `maybeSportsClub` will have type `OaValidationError`
 *   const error = maybeSportsClub;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSportsClub` will have type `SportsClub`
 * const sportsClub = maybeSportsClub;
 * ```
 */
export function validateSportsClub(maybeSportsClub: unknown): SportsClub | OaValidationError {
  const { value, error } = SportsClubJoiSchema.validate(maybeSportsClub);
  if (error) {
    return new OaValidationError('SportsClub', maybeSportsClub, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SportsClub;
}
