...
export type SportsEvent = {
   ...
   */
  competitor?: schema.PersonOrSubClass | schema.SportsTeamOrSubClass | string;
  /**
   * The away team in a sports event.
   */
  awayTeam?: schema.PersonOrSubClass | schema.SportsTeamOrSubClass | string;
  /**
   * The home team in a sports event.
   */
  homeTeam?: schema.SportsTeamOrSubClass | schema.PersonOrSubClass | string;
  /**
 ...
 * schema:SportsEvent
 *
 * This differs from SportsEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SportsEventOrSubClass =
  | SportsEvent
  ;

/**
 * schema:SportsEvent - Validation schema (w/ JOI)
 */
export const SportsEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('SportsEvent').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  competitor: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.SportsTeamOrSubClassJoiSchema), Joi.string().uri()),
  awayTeam: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.lazy(() => schema.SportsTeamOrSubClassJoiSchema), Joi.string().uri()),
  homeTeam: Joi.alternatives().try(Joi.lazy(() => schema.SportsTeamOrSubClassJoiSchema), Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:SportsEvent - Validation schema (w/ JOI)
 *
 * This differs from SportsEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SportsEventOrSubClassJoiSchema = Joi.alternatives().try([
  SportsEventJoiSchema,
]);

/**
 * Runtime validator for schema:SportsEvent.
 *
 * If some data has a structure which matches a schema:SportsEvent, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSportsEvent` will have type `SportsEvent`
 * const sportsEvent = maybeSportsEvent;
 * ```
 */
export function validateSportsEvent(maybeSportsEvent: unknown): SportsEvent | OaValidationError {
  const { value, error } = SportsEventJoiSchema.validate(maybeSportsEvent);
  if (error) {
    return new OaValidationError('SportsEvent', maybeSportsEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SportsEvent;
}
