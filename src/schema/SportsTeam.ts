...
export type SportsTeam = {
   ...
   */
  coach?: schema.PersonOrSubClass | string;
  /**
   ...
   */
  athlete?: schema.PersonOrSubClass | string;
  /**
 ...
 * schema:SportsTeam
 *
 * This differs from SportsTeam because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SportsTeamOrSubClass =
  | SportsTeam
  ;

/**
 * schema:SportsTeam - Validation schema (w/ JOI)
 */
export const SportsTeamJoiSchema = Joi.object({
  '@type': Joi.string().valid('SportsTeam').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  coach: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  gender: Joi.alternatives().try(Joi.lazy(() => schema.GenderTypeJoiSchema), Joi.string()),
  athlete: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:SportsTeam - Validation schema (w/ JOI)
 *
 * This differs from SportsTeamJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SportsTeamOrSubClassJoiSchema = Joi.alternatives().try([
  SportsTeamJoiSchema,
]);

/**
 * Runtime validator for schema:SportsTeam.
 *
 * If some data has a structure which matches a schema:SportsTeam, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSportsTeam = SportsTeam.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSportsTeam instanceof OaValidationError) {
 *   // From this point on, `maybeSportsTeam` will have type `OaValidationError`
 *   const error = maybeSportsTeam;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSportsTeam` will have type `SportsTeam`
 * const sportsTeam = maybeSportsTeam;
 * ```
 */
export function validateSportsTeam(maybeSportsTeam: unknown): SportsTeam | OaValidationError {
  const { value, error } = SportsTeamJoiSchema.validate(maybeSportsTeam);
  if (error) {
    return new OaValidationError('SportsTeam', maybeSportsTeam, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SportsTeam;
}
