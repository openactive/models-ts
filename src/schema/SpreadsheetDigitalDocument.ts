 ...
 */
export type SpreadsheetDigitalDocument = {
 ...
 * schema:SpreadsheetDigitalDocument
 *
 * This differs from SpreadsheetDigitalDocument because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SpreadsheetDigitalDocumentOrSubClass =
  | SpreadsheetDigitalDocument
  ;

/**
 * schema:SpreadsheetDigitalDocument - Validation schema (w/ JOI)
 */
export const SpreadsheetDigitalDocumentJoiSchema = Joi.object({
  '@type': Joi.string().valid('SpreadsheetDigitalDocument').required(),
 ...
 * schema:SpreadsheetDigitalDocument - Validation schema (w/ JOI)
 *
 * This differs from SpreadsheetDigitalDocumentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SpreadsheetDigitalDocumentOrSubClassJoiSchema = Joi.alternatives().try([
  SpreadsheetDigitalDocumentJoiSchema,
]);

/**
 * Runtime validator for schema:SpreadsheetDigitalDocument.
 *
 * If some data has a structure which matches a schema:SpreadsheetDigitalDocument, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSpreadsheetDigitalDocument = SpreadsheetDigitalDocument.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSpreadsheetDigitalDocument instanceof OaValidationError) {
 *   // From this point on, `maybeSpreadsheetDigitalDocument` will have type `OaValidationError`
 *   const error = maybeSpreadsheetDigitalDocument;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSpreadsheetDigitalDocument` will have type `SpreadsheetDigitalDocument`
 * const spreadsheetDigitalDocument = maybeSpreadsheetDigitalDocument;
 * ```
 */
export function validateSpreadsheetDigitalDocument(maybeSpreadsheetDigitalDocument: unknown): SpreadsheetDigitalDocument | OaValidationError {
  const { value, error } = SpreadsheetDigitalDocumentJoiSchema.validate(maybeSpreadsheetDigitalDocument);
  if (error) {
    return new OaValidationError('SpreadsheetDigitalDocument', maybeSpreadsheetDigitalDocument, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SpreadsheetDigitalDocument;
}
