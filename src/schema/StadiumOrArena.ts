 ...
 * schema:StadiumOrArena
 *
 * This differs from StadiumOrArena because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type StadiumOrArenaOrSubClass =
  | StadiumOrArena
  ;

/**
 * schema:StadiumOrArena - Validation schema (w/ JOI)
 */
export const StadiumOrArenaJoiSchema = Joi.object({
  '@type': Joi.string().valid('StadiumOrArena').required(),
 ...
 * schema:StadiumOrArena - Validation schema (w/ JOI)
 *
 * This differs from StadiumOrArenaJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const StadiumOrArenaOrSubClassJoiSchema = Joi.alternatives().try([
  StadiumOrArenaJoiSchema,
]);

/**
 * Runtime validator for schema:StadiumOrArena.
 *
 * If some data has a structure which matches a schema:StadiumOrArena, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeStadiumOrArena = StadiumOrArena.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeStadiumOrArena instanceof OaValidationError) {
 *   // From this point on, `maybeStadiumOrArena` will have type `OaValidationError`
 *   const error = maybeStadiumOrArena;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeStadiumOrArena` will have type `StadiumOrArena`
 * const stadiumOrArena = maybeStadiumOrArena;
 * ```
 */
export function validateStadiumOrArena(maybeStadiumOrArena: unknown): StadiumOrArena | OaValidationError {
  const { value, error } = StadiumOrArenaJoiSchema.validate(maybeStadiumOrArena);
  if (error) {
    return new OaValidationError('StadiumOrArena', maybeStadiumOrArena, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as StadiumOrArena;
}
