 ...
 * schema:State
 *
 * This differs from State because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type StateOrSubClass =
  | State
  ;
 ...
 */
export const StateJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const StateOrSubClassJoiSchema = Joi.alternatives().try([
  StateJoiSchema,
]);
 ...
 * ```
 */
export function validateState(maybeState: unknown): State | OaValidationError {
  const { value, error } = StateJoiSchema.validate(maybeState);
  if (error) {
    return new OaValidationError('State', maybeState, error);
  }
