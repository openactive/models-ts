 ...
 * schema:Statement
 *
 * This differs from Statement because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type StatementOrSubClass =
  | Statement
  ;

/**
 * schema:Statement - Validation schema (w/ JOI)
 */
export const StatementJoiSchema = Joi.object({
  '@type': Joi.string().valid('Statement').required(),
 ...
 * schema:Statement - Validation schema (w/ JOI)
 *
 * This differs from StatementJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const StatementOrSubClassJoiSchema = Joi.alternatives().try([
  StatementJoiSchema,
]);

/**
 * Runtime validator for schema:Statement.
 *
 * If some data has a structure which matches a schema:Statement, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeStatement = validateStatement(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeStatement instanceof OaValidationError) {
 *   // From this point on, `maybeStatement` will have type `OaValidationError`
 *   const error = maybeStatement;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeStatement` will have type `Statement`
 * const statement = maybeStatement;
 * ```
 */
export function validateStatement(maybeStatement: unknown): Statement | OaValidationError {
  const { value, error } = StatementJoiSchema.validate(maybeStatement);
  if (error) {
    return new OaValidationError('Statement', maybeStatement, error);
  }
