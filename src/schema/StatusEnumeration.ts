 ...
 * Lists or enumerations dealing with status types.
 */
export const StatusEnumerationJoiSchema = Joi.string().valid(
);
 ...
 * ```
 */
export function validateStatusEnumeration(maybeStatusEnumeration: unknown): StatusEnumeration | OaValidationError {
  const { value, error } = StatusEnumerationJoiSchema.validate(maybeStatusEnumeration);
  if (error) {
    return new OaValidationError('StatusEnumeration', maybeStatusEnumeration, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as StatusEnumeration;
}
