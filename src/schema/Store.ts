 ...
 * schema:Store
 *
 * This differs from Store because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type StoreOrSubClass =
  | Store
  | schema.WholesaleStoreOrSubClass
  | schema.OfficeEquipmentStoreOrSubClass
  | schema.LiquorStoreOrSubClass
  | schema.PetStoreOrSubClass
  | schema.ToyStoreOrSubClass
  | schema.ConvenienceStoreOrSubClass
  | schema.OutletStoreOrSubClass
  | schema.MusicStoreOrSubClass
  | schema.HobbyShopOrSubClass
  | schema.MovieRentalStoreOrSubClass
  | schema.FloristOrSubClass
  | schema.ComputerStoreOrSubClass
  | schema.AutoPartsStoreOrSubClass
  | schema.PawnShopOrSubClass
  | schema.BookStoreOrSubClass
  | schema.JewelryStoreOrSubClass
  | schema.MobilePhoneStoreOrSubClass
  | schema.ClothingStoreOrSubClass
  | schema.DepartmentStoreOrSubClass
  | schema.ShoeStoreOrSubClass
  | schema.HomeGoodsStoreOrSubClass
  | schema.ElectronicsStoreOrSubClass
  | schema.TireShopOrSubClass
  | schema.SportingGoodsStoreOrSubClass
  | schema.BikeStoreOrSubClass
  | schema.GardenStoreOrSubClass
  | schema.MensClothingStoreOrSubClass
  | schema.HardwareStoreOrSubClass
  | schema.GroceryStoreOrSubClass
  | schema.FurnitureStoreOrSubClass
  ;

/**
 * schema:Store - Validation schema (w/ JOI)
 */
export const StoreJoiSchema = Joi.object({
 ...
 * schema:Store - Validation schema (w/ JOI)
 *
 * This differs from StoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const StoreOrSubClassJoiSchema = Joi.alternatives().try([
  StoreJoiSchema,
  Joi.lazy(() => schema.WholesaleStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.OfficeEquipmentStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.LiquorStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.PetStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.ToyStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.ConvenienceStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.OutletStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.MusicStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.HobbyShopOrSubClassJoiSchema),
  Joi.lazy(() => schema.MovieRentalStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.FloristOrSubClassJoiSchema),
  Joi.lazy(() => schema.ComputerStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.AutoPartsStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.PawnShopOrSubClassJoiSchema),
  Joi.lazy(() => schema.BookStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.JewelryStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.MobilePhoneStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.ClothingStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.DepartmentStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.ShoeStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.HomeGoodsStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.ElectronicsStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.TireShopOrSubClassJoiSchema),
  Joi.lazy(() => schema.SportingGoodsStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.BikeStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.GardenStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.MensClothingStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.HardwareStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.GroceryStoreOrSubClassJoiSchema),
  Joi.lazy(() => schema.FurnitureStoreOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Store.
 *
 * If some data has a structure which matches a schema:Store, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeStore = Store.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeStore instanceof OaValidationError) {
 *   // From this point on, `maybeStore` will have type `OaValidationError`
 *   const error = maybeStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeStore` will have type `Store`
 * const store = maybeStore;
 * ```
 */
export function validateStore(maybeStore: unknown): Store | OaValidationError {
  const { value, error } = StoreJoiSchema.validate(maybeStore);
  if (error) {
    return new OaValidationError('Store', maybeStore, error);
  }
