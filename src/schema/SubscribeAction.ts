 ...
 * schema:SubscribeAction
 *
 * This differs from SubscribeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SubscribeActionOrSubClass =
  | SubscribeAction
  ;
 ...
 */
export const SubscribeActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SubscribeActionOrSubClassJoiSchema = Joi.alternatives().try([
  SubscribeActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeSubscribeAction = SubscribeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateSubscribeAction(maybeSubscribeAction: unknown): SubscribeAction | OaValidationError {
  const { value, error } = SubscribeActionJoiSchema.validate(maybeSubscribeAction);
  if (error) {
    return new OaValidationError('SubscribeAction', maybeSubscribeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SubscribeAction;
}
