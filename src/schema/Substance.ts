 ...
 * schema:Substance
 *
 * This differs from Substance because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SubstanceOrSubClass =
  | Substance
  | schema.DrugOrSubClass
  | schema.DietarySupplementOrSubClass
  ;

/**
 * schema:Substance - Validation schema (w/ JOI)
 */
export const SubstanceJoiSchema = Joi.object({
  '@type': Joi.string().valid('Substance').required(),
 ...
 * schema:Substance - Validation schema (w/ JOI)
 *
 * This differs from SubstanceJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SubstanceOrSubClassJoiSchema = Joi.alternatives().try([
  SubstanceJoiSchema,
  Joi.lazy(() => schema.DrugOrSubClassJoiSchema),
  Joi.lazy(() => schema.DietarySupplementOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:Substance.
 *
 * If some data has a structure which matches a schema:Substance, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSubstance = Substance.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSubstance instanceof OaValidationError) {
 *   // From this point on, `maybeSubstance` will have type `OaValidationError`
 *   const error = maybeSubstance;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSubstance` will have type `Substance`
 * const substance = maybeSubstance;
 * ```
 */
export function validateSubstance(maybeSubstance: unknown): Substance | OaValidationError {
  const { value, error } = SubstanceJoiSchema.validate(maybeSubstance);
  if (error) {
    return new OaValidationError('Substance', maybeSubstance, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Substance;
}
