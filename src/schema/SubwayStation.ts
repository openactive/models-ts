 ...
 * schema:SubwayStation
 *
 * This differs from SubwayStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SubwayStationOrSubClass =
  | SubwayStation
  ;

/**
 * schema:SubwayStation - Validation schema (w/ JOI)
 */
export const SubwayStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('SubwayStation').required(),
 ...
 * schema:SubwayStation - Validation schema (w/ JOI)
 *
 * This differs from SubwayStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SubwayStationOrSubClassJoiSchema = Joi.alternatives().try([
  SubwayStationJoiSchema,
]);

/**
 * Runtime validator for schema:SubwayStation.
 *
 * If some data has a structure which matches a schema:SubwayStation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSubwayStation = SubwayStation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSubwayStation instanceof OaValidationError) {
 *   // From this point on, `maybeSubwayStation` will have type `OaValidationError`
 *   const error = maybeSubwayStation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSubwayStation` will have type `SubwayStation`
 * const subwayStation = maybeSubwayStation;
 * ```
 */
export function validateSubwayStation(maybeSubwayStation: unknown): SubwayStation | OaValidationError {
  const { value, error } = SubwayStationJoiSchema.validate(maybeSubwayStation);
  if (error) {
    return new OaValidationError('SubwayStation', maybeSubwayStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SubwayStation;
}
