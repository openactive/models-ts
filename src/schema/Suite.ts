 ...
 * schema:Suite
 *
 * This differs from Suite because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SuiteOrSubClass =
  | Suite
  ;
 ...
 */
export const SuiteJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const SuiteOrSubClassJoiSchema = Joi.alternatives().try([
  SuiteJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeSuite = Suite.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateSuite(maybeSuite: unknown): Suite | OaValidationError {
  const { value, error } = SuiteJoiSchema.validate(maybeSuite);
  if (error) {
    return new OaValidationError('Suite', maybeSuite, error);
  }
