 ...
 */
export type SuperficialAnatomy = {
 ...
 * etc.
 */
export type SuperficialAnatomyOrSubClass =
  | SuperficialAnatomy
  ;
 ...
 */
export const SuperficialAnatomyJoiSchema = Joi.object({
  ...
  relatedAnatomy: Joi.alternatives().try(Joi.lazy(() => schema.AnatomicalSystemOrSubClassJoiSchema), Joi.lazy(() => schema.AnatomicalStructureOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const SuperficialAnatomyOrSubClassJoiSchema = Joi.alternatives().try([
  SuperficialAnatomyJoiSchema,
]);
 ...
 * ```
 */
export function validateSuperficialAnatomy(maybeSuperficialAnatomy: unknown): SuperficialAnatomy | OaValidationError {
  const { value, error } = SuperficialAnatomyJoiSchema.validate(maybeSuperficialAnatomy);
  if (error) {
    return new OaValidationError('SuperficialAnatomy', maybeSuperficialAnatomy, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SuperficialAnatomy;
}
