 ...
 * schema:SurgicalProcedure
 *
 * This differs from SurgicalProcedure because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SurgicalProcedureOrSubClass =
  | SurgicalProcedure
  ;

/**
 * schema:SurgicalProcedure - Validation schema (w/ JOI)
 */
export const SurgicalProcedureJoiSchema = Joi.object({
  '@type': Joi.string().valid('SurgicalProcedure').required(),
 ...
 * schema:SurgicalProcedure - Validation schema (w/ JOI)
 *
 * This differs from SurgicalProcedureJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SurgicalProcedureOrSubClassJoiSchema = Joi.alternatives().try([
  SurgicalProcedureJoiSchema,
]);

/**
 * Runtime validator for schema:SurgicalProcedure.
 *
 * If some data has a structure which matches a schema:SurgicalProcedure, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSurgicalProcedure = SurgicalProcedure.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSurgicalProcedure instanceof OaValidationError) {
 *   // From this point on, `maybeSurgicalProcedure` will have type `OaValidationError`
 *   const error = maybeSurgicalProcedure;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSurgicalProcedure` will have type `SurgicalProcedure`
 * const surgicalProcedure = maybeSurgicalProcedure;
 * ```
 */
export function validateSurgicalProcedure(maybeSurgicalProcedure: unknown): SurgicalProcedure | OaValidationError {
  const { value, error } = SurgicalProcedureJoiSchema.validate(maybeSurgicalProcedure);
  if (error) {
    return new OaValidationError('SurgicalProcedure', maybeSurgicalProcedure, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SurgicalProcedure;
}
