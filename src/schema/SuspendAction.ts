 ...
 * schema:SuspendAction
 *
 * This differs from SuspendAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SuspendActionOrSubClass =
  | SuspendAction
  ;

/**
 * schema:SuspendAction - Validation schema (w/ JOI)
 */
export const SuspendActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('SuspendAction').required(),
 ...
 * schema:SuspendAction - Validation schema (w/ JOI)
 *
 * This differs from SuspendActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SuspendActionOrSubClassJoiSchema = Joi.alternatives().try([
  SuspendActionJoiSchema,
]);

/**
 * Runtime validator for schema:SuspendAction.
 *
 * If some data has a structure which matches a schema:SuspendAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeSuspendAction` will have type `SuspendAction`
 * const suspendAction = maybeSuspendAction;
 * ```
 */
export function validateSuspendAction(maybeSuspendAction: unknown): SuspendAction | OaValidationError {
  const { value, error } = SuspendActionJoiSchema.validate(maybeSuspendAction);
  if (error) {
    return new OaValidationError('SuspendAction', maybeSuspendAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as SuspendAction;
}
