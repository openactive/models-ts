 ...
 * schema:Synagogue
 *
 * This differs from Synagogue because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type SynagogueOrSubClass =
  | Synagogue
  ;

/**
 * schema:Synagogue - Validation schema (w/ JOI)
 */
export const SynagogueJoiSchema = Joi.object({
  '@type': Joi.string().valid('Synagogue').required(),
 ...
 * schema:Synagogue - Validation schema (w/ JOI)
 *
 * This differs from SynagogueJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const SynagogueOrSubClassJoiSchema = Joi.alternatives().try([
  SynagogueJoiSchema,
]);

/**
 * Runtime validator for schema:Synagogue.
 *
 * If some data has a structure which matches a schema:Synagogue, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeSynagogue = Synagogue.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeSynagogue instanceof OaValidationError) {
 *   // From this point on, `maybeSynagogue` will have type `OaValidationError`
 *   const error = maybeSynagogue;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeSynagogue` will have type `Synagogue`
 * const synagogue = maybeSynagogue;
 * ```
 */
export function validateSynagogue(maybeSynagogue: unknown): Synagogue | OaValidationError {
  const { value, error } = SynagogueJoiSchema.validate(maybeSynagogue);
  if (error) {
    return new OaValidationError('Synagogue', maybeSynagogue, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Synagogue;
}
