...
export type TVClip = {
   ...
   */
  partOfTVSeries?: schema.TVSeriesOrSubClass | string;
  /**
 ...
 * schema:TVClip
 *
 * This differs from TVClip because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TVClipOrSubClass =
  | TVClip
  ;

/**
 * schema:TVClip - Validation schema (w/ JOI)
 */
export const TVClipJoiSchema = Joi.object({
  '@type': Joi.string().valid('TVClip').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  partOfTVSeries: Joi.alternatives().try(Joi.lazy(() => schema.TVSeriesOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:TVClip - Validation schema (w/ JOI)
 *
 * This differs from TVClipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TVClipOrSubClassJoiSchema = Joi.alternatives().try([
  TVClipJoiSchema,
]);

/**
 * Runtime validator for schema:TVClip.
 *
 * If some data has a structure which matches a schema:TVClip, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTVClip = TVClip.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTVClip instanceof OaValidationError) {
 *   // From this point on, `maybeTVClip` will have type `OaValidationError`
 *   const error = maybeTVClip;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTVClip` will have type `TVClip`
 * const tVClip = maybeTVClip;
 * ```
 */
export function validateTVClip(maybeTVClip: unknown): TVClip | OaValidationError {
  const { value, error } = TVClipJoiSchema.validate(maybeTVClip);
  if (error) {
    return new OaValidationError('TVClip', maybeTVClip, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TVClip;
}
