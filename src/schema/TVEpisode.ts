...
export type TVEpisode = {
   ...
   */
  subtitleLanguage?: string | schema.LanguageOrSubClass;
  /**
 ...
 * schema:TVEpisode
 *
 * This differs from TVEpisode because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TVEpisodeOrSubClass =
  | TVEpisode
  ;

/**
 * schema:TVEpisode - Validation schema (w/ JOI)
 */
export const TVEpisodeJoiSchema = Joi.object({
  '@type': Joi.string().valid('TVEpisode').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  subtitleLanguage: Joi.alternatives().try(Joi.string(), Joi.lazy(() => schema.LanguageOrSubClassJoiSchema), Joi.string().uri()),
  titleEIDR: Joi.alternatives().try(Joi.string().uri(), Joi.string()),
 ...
 * schema:TVEpisode - Validation schema (w/ JOI)
 *
 * This differs from TVEpisodeJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TVEpisodeOrSubClassJoiSchema = Joi.alternatives().try([
  TVEpisodeJoiSchema,
]);

/**
 * Runtime validator for schema:TVEpisode.
 *
 * If some data has a structure which matches a schema:TVEpisode, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTVEpisode = TVEpisode.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTVEpisode instanceof OaValidationError) {
 *   // From this point on, `maybeTVEpisode` will have type `OaValidationError`
 *   const error = maybeTVEpisode;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTVEpisode` will have type `TVEpisode`
 * const tVEpisode = maybeTVEpisode;
 * ```
 */
export function validateTVEpisode(maybeTVEpisode: unknown): TVEpisode | OaValidationError {
  const { value, error } = TVEpisodeJoiSchema.validate(maybeTVEpisode);
  if (error) {
    return new OaValidationError('TVEpisode', maybeTVEpisode, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TVEpisode;
}
