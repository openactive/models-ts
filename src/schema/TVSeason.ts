 ...
 * schema:TVSeason
 *
 * This differs from TVSeason because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TVSeasonOrSubClass =
  | TVSeason
  ;

/**
 * schema:TVSeason - Validation schema (w/ JOI)
 */
export const TVSeasonJoiSchema = Joi.object({
  '@type': Joi.string().valid('TVSeason').required(),
 ...
 * schema:TVSeason - Validation schema (w/ JOI)
 *
 * This differs from TVSeasonJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TVSeasonOrSubClassJoiSchema = Joi.alternatives().try([
  TVSeasonJoiSchema,
]);

/**
 * Runtime validator for schema:TVSeason.
 *
 * If some data has a structure which matches a schema:TVSeason, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTVSeason = TVSeason.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTVSeason instanceof OaValidationError) {
 *   // From this point on, `maybeTVSeason` will have type `OaValidationError`
 *   const error = maybeTVSeason;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTVSeason` will have type `TVSeason`
 * const tVSeason = maybeTVSeason;
 * ```
 */
export function validateTVSeason(maybeTVSeason: unknown): TVSeason | OaValidationError {
  const { value, error } = TVSeasonJoiSchema.validate(maybeTVSeason);
  if (error) {
    return new OaValidationError('TVSeason', maybeTVSeason, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TVSeason;
}
