 ...
 * schema:TVSeries
 *
 * This differs from TVSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TVSeriesOrSubClass =
  | TVSeries
  ;

/**
 * schema:TVSeries - Validation schema (w/ JOI)
 */
export const TVSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('TVSeries').required(),
 ...
 * schema:TVSeries - Validation schema (w/ JOI)
 *
 * This differs from TVSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TVSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  TVSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:TVSeries.
 *
 * If some data has a structure which matches a schema:TVSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTVSeries = TVSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTVSeries instanceof OaValidationError) {
 *   // From this point on, `maybeTVSeries` will have type `OaValidationError`
 *   const error = maybeTVSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTVSeries` will have type `TVSeries`
 * const tVSeries = maybeTVSeries;
 * ```
 */
export function validateTVSeries(maybeTVSeries: unknown): TVSeries | OaValidationError {
  const { value, error } = TVSeriesJoiSchema.validate(maybeTVSeries);
  if (error) {
    return new OaValidationError('TVSeries', maybeTVSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TVSeries;
}
