 ...
 * schema:Table
 *
 * This differs from Table because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TableOrSubClass =
  | Table
  ;

/**
 * schema:Table - Validation schema (w/ JOI)
 */
export const TableJoiSchema = Joi.object({
 ...
 * schema:Table - Validation schema (w/ JOI)
 *
 * This differs from TableJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TableOrSubClassJoiSchema = Joi.alternatives().try([
  TableJoiSchema,
]);

/**
 * Runtime validator for schema:Table.
 *
 * If some data has a structure which matches a schema:Table, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeTable` will have type `Table`
 * const table = maybeTable;
 * ```
 */
export function validateTable(maybeTable: unknown): Table | OaValidationError {
  const { value, error } = TableJoiSchema.validate(maybeTable);
  if (error) {
    return new OaValidationError('Table', maybeTable, error);
  }
