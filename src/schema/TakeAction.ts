 ...
 * schema:TakeAction
 *
 * This differs from TakeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TakeActionOrSubClass =
  | TakeAction
  ;

/**
 * schema:TakeAction - Validation schema (w/ JOI)
 */
export const TakeActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('TakeAction').required(),
 ...
 * schema:TakeAction - Validation schema (w/ JOI)
 *
 * This differs from TakeActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TakeActionOrSubClassJoiSchema = Joi.alternatives().try([
  TakeActionJoiSchema,
]);

/**
 * Runtime validator for schema:TakeAction.
 *
 * If some data has a structure which matches a schema:TakeAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTakeAction = TakeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTakeAction instanceof OaValidationError) {
 *   // From this point on, `maybeTakeAction` will have type `OaValidationError`
 *   const error = maybeTakeAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTakeAction` will have type `TakeAction`
 * const takeAction = maybeTakeAction;
 * ```
 */
export function validateTakeAction(maybeTakeAction: unknown): TakeAction | OaValidationError {
  const { value, error } = TakeActionJoiSchema.validate(maybeTakeAction);
  if (error) {
    return new OaValidationError('TakeAction', maybeTakeAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TakeAction;
}
