 ...
 * schema:TattooParlor
 *
 * This differs from TattooParlor because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TattooParlorOrSubClass =
  | TattooParlor
  ;

/**
 * schema:TattooParlor - Validation schema (w/ JOI)
 */
export const TattooParlorJoiSchema = Joi.object({
  '@type': Joi.string().valid('TattooParlor').required(),
 ...
 * schema:TattooParlor - Validation schema (w/ JOI)
 *
 * This differs from TattooParlorJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TattooParlorOrSubClassJoiSchema = Joi.alternatives().try([
  TattooParlorJoiSchema,
]);

/**
 * Runtime validator for schema:TattooParlor.
 *
 * If some data has a structure which matches a schema:TattooParlor, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTattooParlor = TattooParlor.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTattooParlor instanceof OaValidationError) {
 *   // From this point on, `maybeTattooParlor` will have type `OaValidationError`
 *   const error = maybeTattooParlor;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTattooParlor` will have type `TattooParlor`
 * const tattooParlor = maybeTattooParlor;
 * ```
 */
export function validateTattooParlor(maybeTattooParlor: unknown): TattooParlor | OaValidationError {
  const { value, error } = TattooParlorJoiSchema.validate(maybeTattooParlor);
  if (error) {
    return new OaValidationError('TattooParlor', maybeTattooParlor, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TattooParlor;
}
