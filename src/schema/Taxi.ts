 ...
 * schema:Taxi
 *
 * This differs from Taxi because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TaxiOrSubClass =
  | Taxi
  ;
 ...
 */
export const TaxiJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TaxiOrSubClassJoiSchema = Joi.alternatives().try([
  TaxiJoiSchema,
]);
 ...
 * ```
 */
export function validateTaxi(maybeTaxi: unknown): Taxi | OaValidationError {
  const { value, error } = TaxiJoiSchema.validate(maybeTaxi);
  if (error) {
    return new OaValidationError('Taxi', maybeTaxi, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Taxi;
}
