 ...
 * schema:TaxiReservation
 *
 * This differs from TaxiReservation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TaxiReservationOrSubClass =
  | TaxiReservation
  ;
 ...
 */
export const TaxiReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TaxiReservationOrSubClassJoiSchema = Joi.alternatives().try([
  TaxiReservationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeTaxiReservation = TaxiReservation.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTaxiReservation(maybeTaxiReservation: unknown): TaxiReservation | OaValidationError {
  const { value, error } = TaxiReservationJoiSchema.validate(maybeTaxiReservation);
  if (error) {
    return new OaValidationError('TaxiReservation', maybeTaxiReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TaxiReservation;
}
