 ...
 * schema:TaxiStand
 *
 * This differs from TaxiStand because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TaxiStandOrSubClass =
  | TaxiStand
  ;

/**
 * schema:TaxiStand - Validation schema (w/ JOI)
 */
export const TaxiStandJoiSchema = Joi.object({
  '@type': Joi.string().valid('TaxiStand').required(),
 ...
 * schema:TaxiStand - Validation schema (w/ JOI)
 *
 * This differs from TaxiStandJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TaxiStandOrSubClassJoiSchema = Joi.alternatives().try([
  TaxiStandJoiSchema,
]);

/**
 * Runtime validator for schema:TaxiStand.
 *
 * If some data has a structure which matches a schema:TaxiStand, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTaxiStand = TaxiStand.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTaxiStand instanceof OaValidationError) {
 *   // From this point on, `maybeTaxiStand` will have type `OaValidationError`
 *   const error = maybeTaxiStand;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTaxiStand` will have type `TaxiStand`
 * const taxiStand = maybeTaxiStand;
 * ```
 */
export function validateTaxiStand(maybeTaxiStand: unknown): TaxiStand | OaValidationError {
  const { value, error } = TaxiStandJoiSchema.validate(maybeTaxiStand);
  if (error) {
    return new OaValidationError('TaxiStand', maybeTaxiStand, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TaxiStand;
}
