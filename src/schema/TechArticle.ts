 ...
 * schema:TechArticle
 *
 * This differs from TechArticle because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TechArticleOrSubClass =
  | TechArticle
  | schema.APIReferenceOrSubClass
  ;

/**
 * schema:TechArticle - Validation schema (w/ JOI)
 */
export const TechArticleJoiSchema = Joi.object({
  '@type': Joi.string().valid('TechArticle').required(),
 ...
 * schema:TechArticle - Validation schema (w/ JOI)
 *
 * This differs from TechArticleJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TechArticleOrSubClassJoiSchema = Joi.alternatives().try([
  TechArticleJoiSchema,
  Joi.lazy(() => schema.APIReferenceOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:TechArticle.
 *
 * If some data has a structure which matches a schema:TechArticle, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTechArticle = TechArticle.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTechArticle instanceof OaValidationError) {
 *   // From this point on, `maybeTechArticle` will have type `OaValidationError`
 *   const error = maybeTechArticle;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTechArticle` will have type `TechArticle`
 * const techArticle = maybeTechArticle;
 * ```
 */
export function validateTechArticle(maybeTechArticle: unknown): TechArticle | OaValidationError {
  const { value, error } = TechArticleJoiSchema.validate(maybeTechArticle);
  if (error) {
    return new OaValidationError('TechArticle', maybeTechArticle, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TechArticle;
}
