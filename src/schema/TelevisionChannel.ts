 ...
 * etc.
 */
export type TelevisionChannelOrSubClass =
  | TelevisionChannel
  ;
 ...
 */
export const TelevisionChannelJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TelevisionChannelOrSubClassJoiSchema = Joi.alternatives().try([
  TelevisionChannelJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeTelevisionChannel = TelevisionChannel.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTelevisionChannel(maybeTelevisionChannel: unknown): TelevisionChannel | OaValidationError {
  const { value, error } = TelevisionChannelJoiSchema.validate(maybeTelevisionChannel);
  if (error) {
    return new OaValidationError('TelevisionChannel', maybeTelevisionChannel, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TelevisionChannel;
}
