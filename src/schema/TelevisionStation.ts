 ...
 * schema:TelevisionStation
 *
 * This differs from TelevisionStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TelevisionStationOrSubClass =
  | TelevisionStation
  ;

/**
 * schema:TelevisionStation - Validation schema (w/ JOI)
 */
export const TelevisionStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('TelevisionStation').required(),
 ...
 * schema:TelevisionStation - Validation schema (w/ JOI)
 *
 * This differs from TelevisionStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TelevisionStationOrSubClassJoiSchema = Joi.alternatives().try([
  TelevisionStationJoiSchema,
]);

/**
 * Runtime validator for schema:TelevisionStation.
 *
 * If some data has a structure which matches a schema:TelevisionStation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTelevisionStation = TelevisionStation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTelevisionStation instanceof OaValidationError) {
 *   // From this point on, `maybeTelevisionStation` will have type `OaValidationError`
 *   const error = maybeTelevisionStation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTelevisionStation` will have type `TelevisionStation`
 * const televisionStation = maybeTelevisionStation;
 * ```
 */
export function validateTelevisionStation(maybeTelevisionStation: unknown): TelevisionStation | OaValidationError {
  const { value, error } = TelevisionStationJoiSchema.validate(maybeTelevisionStation);
  if (error) {
    return new OaValidationError('TelevisionStation', maybeTelevisionStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TelevisionStation;
}
