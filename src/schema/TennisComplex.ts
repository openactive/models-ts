 ...
 * schema:TennisComplex
 *
 * This differs from TennisComplex because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TennisComplexOrSubClass =
  | TennisComplex
  ;

/**
 * schema:TennisComplex - Validation schema (w/ JOI)
 */
export const TennisComplexJoiSchema = Joi.object({
  '@type': Joi.string().valid('TennisComplex').required(),
 ...
 * schema:TennisComplex - Validation schema (w/ JOI)
 *
 * This differs from TennisComplexJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TennisComplexOrSubClassJoiSchema = Joi.alternatives().try([
  TennisComplexJoiSchema,
]);

/**
 * Runtime validator for schema:TennisComplex.
 *
 * If some data has a structure which matches a schema:TennisComplex, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeTennisComplex` will have type `TennisComplex`
 * const tennisComplex = maybeTennisComplex;
 * ```
 */
export function validateTennisComplex(maybeTennisComplex: unknown): TennisComplex | OaValidationError {
  const { value, error } = TennisComplexJoiSchema.validate(maybeTennisComplex);
  if (error) {
    return new OaValidationError('TennisComplex', maybeTennisComplex, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TennisComplex;
}
