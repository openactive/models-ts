 ...
 */
export type TextDigitalDocument = {
 ...
 * schema:TextDigitalDocument
 *
 * This differs from TextDigitalDocument because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TextDigitalDocumentOrSubClass =
  | TextDigitalDocument
  ;

/**
 * schema:TextDigitalDocument - Validation schema (w/ JOI)
 */
export const TextDigitalDocumentJoiSchema = Joi.object({
  '@type': Joi.string().valid('TextDigitalDocument').required(),
 ...
 * schema:TextDigitalDocument - Validation schema (w/ JOI)
 *
 * This differs from TextDigitalDocumentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TextDigitalDocumentOrSubClassJoiSchema = Joi.alternatives().try([
  TextDigitalDocumentJoiSchema,
]);

/**
 * Runtime validator for schema:TextDigitalDocument.
 *
 * If some data has a structure which matches a schema:TextDigitalDocument, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTextDigitalDocument = TextDigitalDocument.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTextDigitalDocument instanceof OaValidationError) {
 *   // From this point on, `maybeTextDigitalDocument` will have type `OaValidationError`
 *   const error = maybeTextDigitalDocument;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTextDigitalDocument` will have type `TextDigitalDocument`
 * const textDigitalDocument = maybeTextDigitalDocument;
 * ```
 */
export function validateTextDigitalDocument(maybeTextDigitalDocument: unknown): TextDigitalDocument | OaValidationError {
  const { value, error } = TextDigitalDocumentJoiSchema.validate(maybeTextDigitalDocument);
  if (error) {
    return new OaValidationError('TextDigitalDocument', maybeTextDigitalDocument, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TextDigitalDocument;
}
