 ...
 * schema:TheaterEvent
 *
 * This differs from TheaterEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TheaterEventOrSubClass =
  | TheaterEvent
  ;

/**
 * schema:TheaterEvent - Validation schema (w/ JOI)
 */
export const TheaterEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('TheaterEvent').required(),
 ...
 * schema:TheaterEvent - Validation schema (w/ JOI)
 *
 * This differs from TheaterEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TheaterEventOrSubClassJoiSchema = Joi.alternatives().try([
  TheaterEventJoiSchema,
]);

/**
 * Runtime validator for schema:TheaterEvent.
 *
 * If some data has a structure which matches a schema:TheaterEvent, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeTheaterEvent` will have type `TheaterEvent`
 * const theaterEvent = maybeTheaterEvent;
 * ```
 */
export function validateTheaterEvent(maybeTheaterEvent: unknown): TheaterEvent | OaValidationError {
  const { value, error } = TheaterEventJoiSchema.validate(maybeTheaterEvent);
  if (error) {
    return new OaValidationError('TheaterEvent', maybeTheaterEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TheaterEvent;
}
