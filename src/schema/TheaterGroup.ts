 ...
 * schema:TheaterGroup
 *
 * This differs from TheaterGroup because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TheaterGroupOrSubClass =
  | TheaterGroup
  ;

/**
 * schema:TheaterGroup - Validation schema (w/ JOI)
 */
export const TheaterGroupJoiSchema = Joi.object({
  '@type': Joi.string().valid('TheaterGroup').required(),
 ...
 * schema:TheaterGroup - Validation schema (w/ JOI)
 *
 * This differs from TheaterGroupJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TheaterGroupOrSubClassJoiSchema = Joi.alternatives().try([
  TheaterGroupJoiSchema,
]);

/**
 * Runtime validator for schema:TheaterGroup.
 *
 * If some data has a structure which matches a schema:TheaterGroup, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTheaterGroup = TheaterGroup.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTheaterGroup instanceof OaValidationError) {
 *   // From this point on, `maybeTheaterGroup` will have type `OaValidationError`
 *   const error = maybeTheaterGroup;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTheaterGroup` will have type `TheaterGroup`
 * const theaterGroup = maybeTheaterGroup;
 * ```
 */
export function validateTheaterGroup(maybeTheaterGroup: unknown): TheaterGroup | OaValidationError {
  const { value, error } = TheaterGroupJoiSchema.validate(maybeTheaterGroup);
  if (error) {
    return new OaValidationError('TheaterGroup', maybeTheaterGroup, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TheaterGroup;
}
