 ...
 * etc.
 */
export type TherapeuticProcedureOrSubClass =
  | TherapeuticProcedure
  | schema.MedicalTherapyOrSubClass
  | schema.PsychologicalTreatmentOrSubClass
  ;

/**
 * schema:TherapeuticProcedure - Validation schema (w/ JOI)
 */
export const TherapeuticProcedureJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TherapeuticProcedureOrSubClassJoiSchema = Joi.alternatives().try([
  TherapeuticProcedureJoiSchema,
 ...
 *
 * ```ts
 * const maybeTherapeuticProcedure = TherapeuticProcedure.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTherapeuticProcedure(maybeTherapeuticProcedure: unknown): TherapeuticProcedure | OaValidationError {
  const { value, error } = TherapeuticProcedureJoiSchema.validate(maybeTherapeuticProcedure);
  if (error) {
    return new OaValidationError('TherapeuticProcedure', maybeTherapeuticProcedure, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TherapeuticProcedure;
}
