 ...
 * schema:Thesis
 *
 * This differs from Thesis because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ThesisOrSubClass =
  | Thesis
  ;

/**
 * schema:Thesis - Validation schema (w/ JOI)
 */
export const ThesisJoiSchema = Joi.object({
  '@type': Joi.string().valid('Thesis').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  inSupportOf: Joi.string(),
  text: Joi.string(),
 ...
 * schema:Thesis - Validation schema (w/ JOI)
 *
 * This differs from ThesisJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ThesisOrSubClassJoiSchema = Joi.alternatives().try([
  ThesisJoiSchema,
]);

/**
 * Runtime validator for schema:Thesis.
 *
 * If some data has a structure which matches a schema:Thesis, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeThesis = Thesis.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeThesis instanceof OaValidationError) {
 *   // From this point on, `maybeThesis` will have type `OaValidationError`
 *   const error = maybeThesis;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeThesis` will have type `Thesis`
 * const thesis = maybeThesis;
 * ```
 */
export function validateThesis(maybeThesis: unknown): Thesis | OaValidationError {
  const { value, error } = ThesisJoiSchema.validate(maybeThesis);
  if (error) {
    return new OaValidationError('Thesis', maybeThesis, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Thesis;
}
