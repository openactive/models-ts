 ...
 * `SessionSeries`, etc.
 */
export const TicketOrSubClassJoiSchema = Joi.alternatives().try([
  TicketJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeTicket = Ticket.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTicket(maybeTicket: unknown): Ticket | OaValidationError {
  const { value, error } = TicketJoiSchema.validate(maybeTicket);
  if (error) {
    return new OaValidationError('Ticket', maybeTicket, error);
  }
