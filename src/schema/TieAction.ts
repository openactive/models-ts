 ...
 * schema:TieAction
 *
 * This differs from TieAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TieActionOrSubClass =
  | TieAction
  ;

/**
 * schema:TieAction - Validation schema (w/ JOI)
 */
export const TieActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('TieAction').required(),
 ...
 * schema:TieAction - Validation schema (w/ JOI)
 *
 * This differs from TieActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TieActionOrSubClassJoiSchema = Joi.alternatives().try([
  TieActionJoiSchema,
]);

/**
 * Runtime validator for schema:TieAction.
 *
 * If some data has a structure which matches a schema:TieAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTieAction = TieAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTieAction instanceof OaValidationError) {
 *   // From this point on, `maybeTieAction` will have type `OaValidationError`
 *   const error = maybeTieAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTieAction` will have type `TieAction`
 * const tieAction = maybeTieAction;
 * ```
 */
export function validateTieAction(maybeTieAction: unknown): TieAction | OaValidationError {
  const { value, error } = TieActionJoiSchema.validate(maybeTieAction);
  if (error) {
    return new OaValidationError('TieAction', maybeTieAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TieAction;
}
