 ...
 * schema:TipAction
 *
 * This differs from TipAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TipActionOrSubClass =
  | TipAction
  ;

/**
 * schema:TipAction - Validation schema (w/ JOI)
 */
export const TipActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('TipAction').required(),
 ...
 * schema:TipAction - Validation schema (w/ JOI)
 *
 * This differs from TipActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TipActionOrSubClassJoiSchema = Joi.alternatives().try([
  TipActionJoiSchema,
]);

/**
 * Runtime validator for schema:TipAction.
 *
 * If some data has a structure which matches a schema:TipAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTipAction = TipAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTipAction instanceof OaValidationError) {
 *   // From this point on, `maybeTipAction` will have type `OaValidationError`
 *   const error = maybeTipAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTipAction` will have type `TipAction`
 * const tipAction = maybeTipAction;
 * ```
 */
export function validateTipAction(maybeTipAction: unknown): TipAction | OaValidationError {
  const { value, error } = TipActionJoiSchema.validate(maybeTipAction);
  if (error) {
    return new OaValidationError('TipAction', maybeTipAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TipAction;
}
