 ...
 * schema:TireShop
 *
 * This differs from TireShop because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TireShopOrSubClass =
  | TireShop
  ;

/**
 * schema:TireShop - Validation schema (w/ JOI)
 */
export const TireShopJoiSchema = Joi.object({
  '@type': Joi.string().valid('TireShop').required(),
 ...
 * schema:TireShop - Validation schema (w/ JOI)
 *
 * This differs from TireShopJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TireShopOrSubClassJoiSchema = Joi.alternatives().try([
  TireShopJoiSchema,
]);

/**
 * Runtime validator for schema:TireShop.
 *
 * If some data has a structure which matches a schema:TireShop, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTireShop = TireShop.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTireShop instanceof OaValidationError) {
 *   // From this point on, `maybeTireShop` will have type `OaValidationError`
 *   const error = maybeTireShop;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTireShop` will have type `TireShop`
 * const tireShop = maybeTireShop;
 * ```
 */
export function validateTireShop(maybeTireShop: unknown): TireShop | OaValidationError {
  const { value, error } = TireShopJoiSchema.validate(maybeTireShop);
  if (error) {
    return new OaValidationError('TireShop', maybeTireShop, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TireShop;
}
