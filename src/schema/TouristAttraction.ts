 ...
 * schema:TouristAttraction
 *
 * This differs from TouristAttraction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TouristAttractionOrSubClass =
  | TouristAttraction
  ;

/**
 * schema:TouristAttraction - Validation schema (w/ JOI)
 */
export const TouristAttractionJoiSchema = Joi.object({
  '@type': Joi.string().valid('TouristAttraction').required(),
 ...
 * schema:TouristAttraction - Validation schema (w/ JOI)
 *
 * This differs from TouristAttractionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TouristAttractionOrSubClassJoiSchema = Joi.alternatives().try([
  TouristAttractionJoiSchema,
]);

/**
 * Runtime validator for schema:TouristAttraction.
 *
 * If some data has a structure which matches a schema:TouristAttraction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTouristAttraction = TouristAttraction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTouristAttraction instanceof OaValidationError) {
 *   // From this point on, `maybeTouristAttraction` will have type `OaValidationError`
 *   const error = maybeTouristAttraction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTouristAttraction` will have type `TouristAttraction`
 * const touristAttraction = maybeTouristAttraction;
 * ```
 */
export function validateTouristAttraction(maybeTouristAttraction: unknown): TouristAttraction | OaValidationError {
  const { value, error } = TouristAttractionJoiSchema.validate(maybeTouristAttraction);
  if (error) {
    return new OaValidationError('TouristAttraction', maybeTouristAttraction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TouristAttraction;
}
