...
export type TouristDestination = {
   ...
   */
  includesAttraction?: schema.TouristAttractionOrSubClass | string;
  /**
 ...
 * schema:TouristDestination
 *
 * This differs from TouristDestination because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TouristDestinationOrSubClass =
  | TouristDestination
  ;
 ...
 */
export const TouristDestinationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TouristDestinationOrSubClassJoiSchema = Joi.alternatives().try([
  TouristDestinationJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeTouristDestination = TouristDestination.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTouristDestination(maybeTouristDestination: unknown): TouristDestination | OaValidationError {
  const { value, error } = TouristDestinationJoiSchema.validate(maybeTouristDestination);
  if (error) {
    return new OaValidationError('TouristDestination', maybeTouristDestination, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TouristDestination;
}
