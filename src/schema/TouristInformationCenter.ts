 ...
 */
export type TouristInformationCenter = {
 ...
 * schema:TouristInformationCenter
 *
 * This differs from TouristInformationCenter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TouristInformationCenterOrSubClass =
  | TouristInformationCenter
  ;

/**
 * schema:TouristInformationCenter - Validation schema (w/ JOI)
 */
export const TouristInformationCenterJoiSchema = Joi.object({
  '@type': Joi.string().valid('TouristInformationCenter').required(),
 ...
 * schema:TouristInformationCenter - Validation schema (w/ JOI)
 *
 * This differs from TouristInformationCenterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TouristInformationCenterOrSubClassJoiSchema = Joi.alternatives().try([
  TouristInformationCenterJoiSchema,
]);

/**
 * Runtime validator for schema:TouristInformationCenter.
 *
 * If some data has a structure which matches a schema:TouristInformationCenter, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTouristInformationCenter = TouristInformationCenter.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTouristInformationCenter instanceof OaValidationError) {
 *   // From this point on, `maybeTouristInformationCenter` will have type `OaValidationError`
 *   const error = maybeTouristInformationCenter;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTouristInformationCenter` will have type `TouristInformationCenter`
 * const touristInformationCenter = maybeTouristInformationCenter;
 * ```
 */
export function validateTouristInformationCenter(maybeTouristInformationCenter: unknown): TouristInformationCenter | OaValidationError {
  const { value, error } = TouristInformationCenterJoiSchema.validate(maybeTouristInformationCenter);
  if (error) {
    return new OaValidationError('TouristInformationCenter', maybeTouristInformationCenter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TouristInformationCenter;
}
