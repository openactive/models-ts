 ...
 * schema:ToyStore
 *
 * This differs from ToyStore because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ToyStoreOrSubClass =
  | ToyStore
  ;

/**
 * schema:ToyStore - Validation schema (w/ JOI)
 */
export const ToyStoreJoiSchema = Joi.object({
  '@type': Joi.string().valid('ToyStore').required(),
 ...
 * schema:ToyStore - Validation schema (w/ JOI)
 *
 * This differs from ToyStoreJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ToyStoreOrSubClassJoiSchema = Joi.alternatives().try([
  ToyStoreJoiSchema,
]);

/**
 * Runtime validator for schema:ToyStore.
 *
 * If some data has a structure which matches a schema:ToyStore, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeToyStore = ToyStore.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeToyStore instanceof OaValidationError) {
 *   // From this point on, `maybeToyStore` will have type `OaValidationError`
 *   const error = maybeToyStore;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeToyStore` will have type `ToyStore`
 * const toyStore = maybeToyStore;
 * ```
 */
export function validateToyStore(maybeToyStore: unknown): ToyStore | OaValidationError {
  const { value, error } = ToyStoreJoiSchema.validate(maybeToyStore);
  if (error) {
    return new OaValidationError('ToyStore', maybeToyStore, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ToyStore;
}
