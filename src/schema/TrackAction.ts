 ...
 * schema:TrackAction
 *
 * This differs from TrackAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TrackActionOrSubClass =
  | TrackAction
  ;

/**
 * schema:TrackAction - Validation schema (w/ JOI)
 */
export const TrackActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('TrackAction').required(),
 ...
 * schema:TrackAction - Validation schema (w/ JOI)
 *
 * This differs from TrackActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TrackActionOrSubClassJoiSchema = Joi.alternatives().try([
  TrackActionJoiSchema,
]);

/**
 * Runtime validator for schema:TrackAction.
 *
 * If some data has a structure which matches a schema:TrackAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeTrackAction` will have type `TrackAction`
 * const trackAction = maybeTrackAction;
 * ```
 */
export function validateTrackAction(maybeTrackAction: unknown): TrackAction | OaValidationError {
  const { value, error } = TrackActionJoiSchema.validate(maybeTrackAction);
  if (error) {
    return new OaValidationError('TrackAction', maybeTrackAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TrackAction;
}
