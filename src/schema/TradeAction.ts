 ...
 * schema:TradeAction
 *
 * This differs from TradeAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TradeActionOrSubClass =
  | TradeAction
  | schema.PreOrderActionOrSubClass
  | schema.RentActionOrSubClass
  | schema.OrderActionOrSubClass
  | schema.DonateActionOrSubClass
  | schema.BuyActionOrSubClass
  | schema.PayActionOrSubClass
  | schema.TipActionOrSubClass
  | schema.SellActionOrSubClass
  | schema.QuoteActionOrSubClass
  ;

/**
 * schema:TradeAction - Validation schema (w/ JOI)
 */
export const TradeActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TradeActionOrSubClassJoiSchema = Joi.alternatives().try([
  TradeActionJoiSchema,
 ...
 *
 * ```ts
 * const maybeTradeAction = TradeAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTradeAction(maybeTradeAction: unknown): TradeAction | OaValidationError {
  const { value, error } = TradeActionJoiSchema.validate(maybeTradeAction);
  if (error) {
    return new OaValidationError('TradeAction', maybeTradeAction, error);
  }
