 ...
 * etc.
 */
export type TrainReservationOrSubClass =
  | TrainReservation
  ;
 ...
 */
export const TrainReservationJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TrainReservationOrSubClassJoiSchema = Joi.alternatives().try([
  TrainReservationJoiSchema,
]);
 ...
 * ```
 */
export function validateTrainReservation(maybeTrainReservation: unknown): TrainReservation | OaValidationError {
  const { value, error } = TrainReservationJoiSchema.validate(maybeTrainReservation);
  if (error) {
    return new OaValidationError('TrainReservation', maybeTrainReservation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TrainReservation;
}
