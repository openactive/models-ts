 ...
 * schema:TrainStation
 *
 * This differs from TrainStation because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TrainStationOrSubClass =
  | TrainStation
  ;

/**
 * schema:TrainStation - Validation schema (w/ JOI)
 */
export const TrainStationJoiSchema = Joi.object({
  '@type': Joi.string().valid('TrainStation').required(),
 ...
 * schema:TrainStation - Validation schema (w/ JOI)
 *
 * This differs from TrainStationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TrainStationOrSubClassJoiSchema = Joi.alternatives().try([
  TrainStationJoiSchema,
]);

/**
 * Runtime validator for schema:TrainStation.
 *
 * If some data has a structure which matches a schema:TrainStation, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeTrainStation = TrainStation.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeTrainStation instanceof OaValidationError) {
 *   // From this point on, `maybeTrainStation` will have type `OaValidationError`
 *   const error = maybeTrainStation;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeTrainStation` will have type `TrainStation`
 * const trainStation = maybeTrainStation;
 * ```
 */
export function validateTrainStation(maybeTrainStation: unknown): TrainStation | OaValidationError {
  const { value, error } = TrainStationJoiSchema.validate(maybeTrainStation);
  if (error) {
    return new OaValidationError('TrainStation', maybeTrainStation, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TrainStation;
}
