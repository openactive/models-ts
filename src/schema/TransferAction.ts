 ...
 * etc.
 */
export type TransferActionOrSubClass =
  | TransferAction
  | schema.MoneyTransferOrSubClass
  | schema.LendActionOrSubClass
  | schema.GiveActionOrSubClass
  | schema.SendActionOrSubClass
  | schema.TakeActionOrSubClass
  | schema.ReturnActionOrSubClass
  | schema.DownloadActionOrSubClass
  | schema.BorrowActionOrSubClass
  | schema.ReceiveActionOrSubClass
  ;

/**
 * schema:TransferAction - Validation schema (w/ JOI)
 */
export const TransferActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TransferActionOrSubClassJoiSchema = Joi.alternatives().try([
  TransferActionJoiSchema,
 ...
 * ```
 */
export function validateTransferAction(maybeTransferAction: unknown): TransferAction | OaValidationError {
  const { value, error } = TransferActionJoiSchema.validate(maybeTransferAction);
  if (error) {
    return new OaValidationError('TransferAction', maybeTransferAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TransferAction;
}
