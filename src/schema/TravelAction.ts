 ...
 * schema:TravelAction
 *
 * This differs from TravelAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TravelActionOrSubClass =
  | TravelAction
  ;
 ...
 */
export const TravelActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TravelActionOrSubClassJoiSchema = Joi.alternatives().try([
  TravelActionJoiSchema,
]);
 ...
 *
 * ```ts
 * const maybeTravelAction = TravelAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 ...
 * ```
 */
export function validateTravelAction(maybeTravelAction: unknown): TravelAction | OaValidationError {
  const { value, error } = TravelActionJoiSchema.validate(maybeTravelAction);
  if (error) {
    return new OaValidationError('TravelAction', maybeTravelAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TravelAction;
}
