 ...
 * etc.
 */
export type TravelAgencyOrSubClass =
  | TravelAgency
  ;
 ...
 */
export const TravelAgencyJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const TravelAgencyOrSubClassJoiSchema = Joi.alternatives().try([
  TravelAgencyJoiSchema,
]);
 ...
 * ```
 */
export function validateTravelAgency(maybeTravelAgency: unknown): TravelAgency | OaValidationError {
  const { value, error } = TravelAgencyJoiSchema.validate(maybeTravelAgency);
  if (error) {
    return new OaValidationError('TravelAgency', maybeTravelAgency, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TravelAgency;
}
