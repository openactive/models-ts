 ...
 * schema:TreatmentIndication
 *
 * This differs from TreatmentIndication because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type TreatmentIndicationOrSubClass =
  | TreatmentIndication
  ;

/**
 * schema:TreatmentIndication - Validation schema (w/ JOI)
 */
export const TreatmentIndicationJoiSchema = Joi.object({
  '@type': Joi.string().valid('TreatmentIndication').required(),
 ...
 * schema:TreatmentIndication - Validation schema (w/ JOI)
 *
 * This differs from TreatmentIndicationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const TreatmentIndicationOrSubClassJoiSchema = Joi.alternatives().try([
  TreatmentIndicationJoiSchema,
]);

/**
 * Runtime validator for schema:TreatmentIndication.
 *
 * If some data has a structure which matches a schema:TreatmentIndication, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeTreatmentIndication` will have type `TreatmentIndication`
 * const treatmentIndication = maybeTreatmentIndication;
 * ```
 */
export function validateTreatmentIndication(maybeTreatmentIndication: unknown): TreatmentIndication | OaValidationError {
  const { value, error } = TreatmentIndicationJoiSchema.validate(maybeTreatmentIndication);
  if (error) {
    return new OaValidationError('TreatmentIndication', maybeTreatmentIndication, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as TreatmentIndication;
}
