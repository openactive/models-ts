 ...
 */
export const URLJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const URLOrSubClassJoiSchema = Joi.alternatives().try([
  URLJoiSchema,
]);
 ...
 * ```
 */
export function validateURL(maybeURL: unknown): URL | OaValidationError {
  const { value, error } = URLJoiSchema.validate(maybeURL);
  if (error) {
    return new OaValidationError('URL', maybeURL, error);
  }
