 ...
 * schema:UnRegisterAction
 *
 * This differs from UnRegisterAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UnRegisterActionOrSubClass =
  | UnRegisterAction
  ;

/**
 * schema:UnRegisterAction - Validation schema (w/ JOI)
 */
export const UnRegisterActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('UnRegisterAction').required(),
 ...
 * schema:UnRegisterAction - Validation schema (w/ JOI)
 *
 * This differs from UnRegisterActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UnRegisterActionOrSubClassJoiSchema = Joi.alternatives().try([
  UnRegisterActionJoiSchema,
]);

/**
 * Runtime validator for schema:UnRegisterAction.
 *
 * If some data has a structure which matches a schema:UnRegisterAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUnRegisterAction = UnRegisterAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUnRegisterAction instanceof OaValidationError) {
 *   // From this point on, `maybeUnRegisterAction` will have type `OaValidationError`
 *   const error = maybeUnRegisterAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUnRegisterAction` will have type `UnRegisterAction`
 * const unRegisterAction = maybeUnRegisterAction;
 * ```
 */
export function validateUnRegisterAction(maybeUnRegisterAction: unknown): UnRegisterAction | OaValidationError {
  const { value, error } = UnRegisterActionJoiSchema.validate(maybeUnRegisterAction);
  if (error) {
    return new OaValidationError('UnRegisterAction', maybeUnRegisterAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnRegisterAction;
}
