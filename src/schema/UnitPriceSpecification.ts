 ...
 */
export type UnitPriceSpecification = {
   ...
   */
  priceType?: string | schema.PriceTypeEnumeration;
  /**
   * Specifies for how long this price (or price component) will be billed. Can be used, for example, to model the contractual duration of a subscription or payment plan. Type can be either a Duration or a Number (in which case the unit of measurement, for example month, is specified by the unitCode property).
   */
  billingDuration?: schema.QuantitativeValueOrSubClass | string | number;
  /**
 ...
 * etc.
 */
export type UnitPriceSpecificationOrSubClass =
 ...
 */
export const UnitPriceSpecificationJoiSchema = Joi.object({
  ...
  billingDuration: Joi.alternatives().try(Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string(), Joi.number(), Joi.string().uri()),
 ...
 * `SessionSeries`, etc.
 */
export const UnitPriceSpecificationOrSubClassJoiSchema = Joi.alternatives().try([
  UnitPriceSpecificationJoiSchema,
]);
 ...
 * ```
 */
export function validateUnitPriceSpecification(maybeUnitPriceSpecification: unknown): UnitPriceSpecification | OaValidationError {
  const { value, error } = UnitPriceSpecificationJoiSchema.validate(maybeUnitPriceSpecification);
  if (error) {
    return new OaValidationError('UnitPriceSpecification', maybeUnitPriceSpecification, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UnitPriceSpecification;
}
