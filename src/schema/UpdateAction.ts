...
export type UpdateAction = {
   ...
   */
  collection?: schema.ThingOrSubClass | string;
  /**
   * A sub property of object. The collection target of the action.
   */
  targetCollection?: schema.ThingOrSubClass | string;
  /**
 ...
 * schema:UpdateAction
 *
 * This differs from UpdateAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UpdateActionOrSubClass =
  | UpdateAction
  | schema.ReplaceActionOrSubClass
  | schema.AddActionOrSubClass
  | schema.DeleteActionOrSubClass
  ;

/**
 * schema:UpdateAction - Validation schema (w/ JOI)
 */
export const UpdateActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('UpdateAction').required(),
 ...
 * schema:UpdateAction - Validation schema (w/ JOI)
 *
 * This differs from UpdateActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UpdateActionOrSubClassJoiSchema = Joi.alternatives().try([
  UpdateActionJoiSchema,
  Joi.lazy(() => schema.ReplaceActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.AddActionOrSubClassJoiSchema),
  Joi.lazy(() => schema.DeleteActionOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:UpdateAction.
 *
 * If some data has a structure which matches a schema:UpdateAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUpdateAction = UpdateAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUpdateAction instanceof OaValidationError) {
 *   // From this point on, `maybeUpdateAction` will have type `OaValidationError`
 *   const error = maybeUpdateAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUpdateAction` will have type `UpdateAction`
 * const updateAction = maybeUpdateAction;
 * ```
 */
export function validateUpdateAction(maybeUpdateAction: unknown): UpdateAction | OaValidationError {
  const { value, error } = UpdateActionJoiSchema.validate(maybeUpdateAction);
  if (error) {
    return new OaValidationError('UpdateAction', maybeUpdateAction, error);
  }
