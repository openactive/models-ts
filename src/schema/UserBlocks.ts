 ...
 * schema:UserBlocks
 *
 * This differs from UserBlocks because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserBlocksOrSubClass =
  | UserBlocks
  ;

/**
 * schema:UserBlocks - Validation schema (w/ JOI)
 */
export const UserBlocksJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserBlocks').required(),
 ...
 * schema:UserBlocks - Validation schema (w/ JOI)
 *
 * This differs from UserBlocksJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserBlocksOrSubClassJoiSchema = Joi.alternatives().try([
  UserBlocksJoiSchema,
]);

/**
 * Runtime validator for schema:UserBlocks.
 *
 * If some data has a structure which matches a schema:UserBlocks, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeUserBlocks` will have type `UserBlocks`
 * const userBlocks = maybeUserBlocks;
 * ```
 */
export function validateUserBlocks(maybeUserBlocks: unknown): UserBlocks | OaValidationError {
  const { value, error } = UserBlocksJoiSchema.validate(maybeUserBlocks);
  if (error) {
    return new OaValidationError('UserBlocks', maybeUserBlocks, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserBlocks;
}
