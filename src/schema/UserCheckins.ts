 ...
 * schema:UserCheckins
 *
 * This differs from UserCheckins because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserCheckinsOrSubClass =
  | UserCheckins
  ;

/**
 * schema:UserCheckins - Validation schema (w/ JOI)
 */
export const UserCheckinsJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserCheckins').required(),
 ...
 * schema:UserCheckins - Validation schema (w/ JOI)
 *
 * This differs from UserCheckinsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserCheckinsOrSubClassJoiSchema = Joi.alternatives().try([
  UserCheckinsJoiSchema,
]);

/**
 * Runtime validator for schema:UserCheckins.
 *
 * If some data has a structure which matches a schema:UserCheckins, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserCheckins = UserCheckins.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserCheckins instanceof OaValidationError) {
 *   // From this point on, `maybeUserCheckins` will have type `OaValidationError`
 *   const error = maybeUserCheckins;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserCheckins` will have type `UserCheckins`
 * const userCheckins = maybeUserCheckins;
 * ```
 */
export function validateUserCheckins(maybeUserCheckins: unknown): UserCheckins | OaValidationError {
  const { value, error } = UserCheckinsJoiSchema.validate(maybeUserCheckins);
  if (error) {
    return new OaValidationError('UserCheckins', maybeUserCheckins, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserCheckins;
}
