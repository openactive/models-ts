...
export type UserComments = {
   ...
   */
  discusses?: schema.CreativeWorkOrSubClass | string;
  /**
 ...
 * schema:UserComments
 *
 * This differs from UserComments because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserCommentsOrSubClass =
  | UserComments
  ;

/**
 * schema:UserComments - Validation schema (w/ JOI)
 */
export const UserCommentsJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserComments').required(),
  ...
  discusses: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
  replyToUrl: Joi.string().uri(),
  commentTime: Joi.string().isoDate(),
 ...
 * schema:UserComments - Validation schema (w/ JOI)
 *
 * This differs from UserCommentsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserCommentsOrSubClassJoiSchema = Joi.alternatives().try([
  UserCommentsJoiSchema,
]);

/**
 * Runtime validator for schema:UserComments.
 *
 * If some data has a structure which matches a schema:UserComments, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserComments = UserComments.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserComments instanceof OaValidationError) {
 *   // From this point on, `maybeUserComments` will have type `OaValidationError`
 *   const error = maybeUserComments;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserComments` will have type `UserComments`
 * const userComments = maybeUserComments;
 * ```
 */
export function validateUserComments(maybeUserComments: unknown): UserComments | OaValidationError {
  const { value, error } = UserCommentsJoiSchema.validate(maybeUserComments);
  if (error) {
    return new OaValidationError('UserComments', maybeUserComments, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserComments;
}
