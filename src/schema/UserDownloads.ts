 ...
 * schema:UserDownloads
 *
 * This differs from UserDownloads because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserDownloadsOrSubClass =
  | UserDownloads
  ;

/**
 * schema:UserDownloads - Validation schema (w/ JOI)
 */
export const UserDownloadsJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserDownloads').required(),
 ...
 * schema:UserDownloads - Validation schema (w/ JOI)
 *
 * This differs from UserDownloadsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserDownloadsOrSubClassJoiSchema = Joi.alternatives().try([
  UserDownloadsJoiSchema,
]);

/**
 * Runtime validator for schema:UserDownloads.
 *
 * If some data has a structure which matches a schema:UserDownloads, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserDownloads = UserDownloads.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserDownloads instanceof OaValidationError) {
 *   // From this point on, `maybeUserDownloads` will have type `OaValidationError`
 *   const error = maybeUserDownloads;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserDownloads` will have type `UserDownloads`
 * const userDownloads = maybeUserDownloads;
 * ```
 */
export function validateUserDownloads(maybeUserDownloads: unknown): UserDownloads | OaValidationError {
  const { value, error } = UserDownloadsJoiSchema.validate(maybeUserDownloads);
  if (error) {
    return new OaValidationError('UserDownloads', maybeUserDownloads, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserDownloads;
}
