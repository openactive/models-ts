 ...
 * schema:UserInteraction
 *
 * This differs from UserInteraction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserInteractionOrSubClass =
  | UserInteraction
  | schema.UserPlaysOrSubClass
  | schema.UserBlocksOrSubClass
  | schema.UserPageVisitsOrSubClass
  | schema.UserPlusOnesOrSubClass
  | schema.UserCheckinsOrSubClass
  | schema.UserDownloadsOrSubClass
  | schema.UserCommentsOrSubClass
  | schema.UserTweetsOrSubClass
  | schema.UserLikesOrSubClass
  ;

/**
 * schema:UserInteraction - Validation schema (w/ JOI)
 */
export const UserInteractionJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserInteraction').required(),
 ...
 * schema:UserInteraction - Validation schema (w/ JOI)
 *
 * This differs from UserInteractionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserInteractionOrSubClassJoiSchema = Joi.alternatives().try([
  UserInteractionJoiSchema,
  Joi.lazy(() => schema.UserPlaysOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserBlocksOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserPageVisitsOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserPlusOnesOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserCheckinsOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserDownloadsOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserCommentsOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserTweetsOrSubClassJoiSchema),
  Joi.lazy(() => schema.UserLikesOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:UserInteraction.
 *
 * If some data has a structure which matches a schema:UserInteraction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserInteraction = UserInteraction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserInteraction instanceof OaValidationError) {
 *   // From this point on, `maybeUserInteraction` will have type `OaValidationError`
 *   const error = maybeUserInteraction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserInteraction` will have type `UserInteraction`
 * const userInteraction = maybeUserInteraction;
 * ```
 */
export function validateUserInteraction(maybeUserInteraction: unknown): UserInteraction | OaValidationError {
  const { value, error } = UserInteractionJoiSchema.validate(maybeUserInteraction);
  if (error) {
    return new OaValidationError('UserInteraction', maybeUserInteraction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserInteraction;
}
