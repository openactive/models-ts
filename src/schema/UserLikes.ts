 ...
 * schema:UserLikes
 *
 * This differs from UserLikes because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserLikesOrSubClass =
  | UserLikes
  ;

/**
 * schema:UserLikes - Validation schema (w/ JOI)
 */
export const UserLikesJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserLikes').required(),
 ...
 * schema:UserLikes - Validation schema (w/ JOI)
 *
 * This differs from UserLikesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserLikesOrSubClassJoiSchema = Joi.alternatives().try([
  UserLikesJoiSchema,
]);

/**
 * Runtime validator for schema:UserLikes.
 *
 * If some data has a structure which matches a schema:UserLikes, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserLikes = UserLikes.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserLikes instanceof OaValidationError) {
 *   // From this point on, `maybeUserLikes` will have type `OaValidationError`
 *   const error = maybeUserLikes;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserLikes` will have type `UserLikes`
 * const userLikes = maybeUserLikes;
 * ```
 */
export function validateUserLikes(maybeUserLikes: unknown): UserLikes | OaValidationError {
  const { value, error } = UserLikesJoiSchema.validate(maybeUserLikes);
  if (error) {
    return new OaValidationError('UserLikes', maybeUserLikes, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserLikes;
}
