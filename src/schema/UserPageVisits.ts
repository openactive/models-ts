 ...
 * schema:UserPageVisits
 *
 * This differs from UserPageVisits because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserPageVisitsOrSubClass =
  | UserPageVisits
  ;

/**
 * schema:UserPageVisits - Validation schema (w/ JOI)
 */
export const UserPageVisitsJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserPageVisits').required(),
 ...
 * schema:UserPageVisits - Validation schema (w/ JOI)
 *
 * This differs from UserPageVisitsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserPageVisitsOrSubClassJoiSchema = Joi.alternatives().try([
  UserPageVisitsJoiSchema,
]);

/**
 * Runtime validator for schema:UserPageVisits.
 *
 * If some data has a structure which matches a schema:UserPageVisits, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserPageVisits = UserPageVisits.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserPageVisits instanceof OaValidationError) {
 *   // From this point on, `maybeUserPageVisits` will have type `OaValidationError`
 *   const error = maybeUserPageVisits;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserPageVisits` will have type `UserPageVisits`
 * const userPageVisits = maybeUserPageVisits;
 * ```
 */
export function validateUserPageVisits(maybeUserPageVisits: unknown): UserPageVisits | OaValidationError {
  const { value, error } = UserPageVisitsJoiSchema.validate(maybeUserPageVisits);
  if (error) {
    return new OaValidationError('UserPageVisits', maybeUserPageVisits, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserPageVisits;
}
