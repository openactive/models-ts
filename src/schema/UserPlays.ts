 ...
 * etc.
 */
export type UserPlaysOrSubClass =
  | UserPlays
  ;
 ...
 */
export const UserPlaysJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const UserPlaysOrSubClassJoiSchema = Joi.alternatives().try([
  UserPlaysJoiSchema,
]);
 ...
 * ```
 */
export function validateUserPlays(maybeUserPlays: unknown): UserPlays | OaValidationError {
  const { value, error } = UserPlaysJoiSchema.validate(maybeUserPlays);
  if (error) {
    return new OaValidationError('UserPlays', maybeUserPlays, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserPlays;
}
