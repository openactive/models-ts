 ...
 * schema:UserPlusOnes
 *
 * This differs from UserPlusOnes because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserPlusOnesOrSubClass =
  | UserPlusOnes
  ;

/**
 * schema:UserPlusOnes - Validation schema (w/ JOI)
 */
export const UserPlusOnesJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserPlusOnes').required(),
 ...
 * schema:UserPlusOnes - Validation schema (w/ JOI)
 *
 * This differs from UserPlusOnesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserPlusOnesOrSubClassJoiSchema = Joi.alternatives().try([
  UserPlusOnesJoiSchema,
]);

/**
 * Runtime validator for schema:UserPlusOnes.
 *
 * If some data has a structure which matches a schema:UserPlusOnes, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserPlusOnes = UserPlusOnes.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserPlusOnes instanceof OaValidationError) {
 *   // From this point on, `maybeUserPlusOnes` will have type `OaValidationError`
 *   const error = maybeUserPlusOnes;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserPlusOnes` will have type `UserPlusOnes`
 * const userPlusOnes = maybeUserPlusOnes;
 * ```
 */
export function validateUserPlusOnes(maybeUserPlusOnes: unknown): UserPlusOnes | OaValidationError {
  const { value, error } = UserPlusOnesJoiSchema.validate(maybeUserPlusOnes);
  if (error) {
    return new OaValidationError('UserPlusOnes', maybeUserPlusOnes, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserPlusOnes;
}
