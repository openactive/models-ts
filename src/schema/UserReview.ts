 ...
 * schema:UserReview
 *
 * This differs from UserReview because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserReviewOrSubClass =
  | UserReview
  ;

/**
 * schema:UserReview - Validation schema (w/ JOI)
 */
export const UserReviewJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserReview').required(),
 ...
 * schema:UserReview - Validation schema (w/ JOI)
 *
 * This differs from UserReviewJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserReviewOrSubClassJoiSchema = Joi.alternatives().try([
  UserReviewJoiSchema,
]);

/**
 * Runtime validator for schema:UserReview.
 *
 * If some data has a structure which matches a schema:UserReview, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserReview = validateUserReview(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserReview instanceof OaValidationError) {
 *   // From this point on, `maybeUserReview` will have type `OaValidationError`
 *   const error = maybeUserReview;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserReview` will have type `UserReview`
 * const userReview = maybeUserReview;
 * ```
 */
export function validateUserReview(maybeUserReview: unknown): UserReview | OaValidationError {
  const { value, error } = UserReviewJoiSchema.validate(maybeUserReview);
  if (error) {
    return new OaValidationError('UserReview', maybeUserReview, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserReview;
}
