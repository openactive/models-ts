 ...
 * schema:UserTweets
 *
 * This differs from UserTweets because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type UserTweetsOrSubClass =
  | UserTweets
  ;

/**
 * schema:UserTweets - Validation schema (w/ JOI)
 */
export const UserTweetsJoiSchema = Joi.object({
  '@type': Joi.string().valid('UserTweets').required(),
 ...
 * schema:UserTweets - Validation schema (w/ JOI)
 *
 * This differs from UserTweetsJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const UserTweetsOrSubClassJoiSchema = Joi.alternatives().try([
  UserTweetsJoiSchema,
]);

/**
 * Runtime validator for schema:UserTweets.
 *
 * If some data has a structure which matches a schema:UserTweets, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeUserTweets = UserTweets.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeUserTweets instanceof OaValidationError) {
 *   // From this point on, `maybeUserTweets` will have type `OaValidationError`
 *   const error = maybeUserTweets;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeUserTweets` will have type `UserTweets`
 * const userTweets = maybeUserTweets;
 * ```
 */
export function validateUserTweets(maybeUserTweets: unknown): UserTweets | OaValidationError {
  const { value, error } = UserTweetsJoiSchema.validate(maybeUserTweets);
  if (error) {
    return new OaValidationError('UserTweets', maybeUserTweets, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as UserTweets;
}
