...
export type Vein = {
   ...
   */
  tributary?: schema.AnatomicalStructureOrSubClass | string;
  /**
   * The vasculature that the vein drains into.
   */
  drainsTo?: schema.VesselOrSubClass | string;
  /**
 ...
 * schema:Vein
 *
 * This differs from Vein because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VeinOrSubClass =
  | Vein
  ;

/**
 * schema:Vein - Validation schema (w/ JOI)
 */
export const VeinJoiSchema = Joi.object({
  '@type': Joi.string().valid('Vein').required(),
  ...
  tributary: Joi.alternatives().try(Joi.lazy(() => schema.AnatomicalStructureOrSubClassJoiSchema), Joi.string().uri()),
  drainsTo: Joi.alternatives().try(Joi.lazy(() => schema.VesselOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:Vein - Validation schema (w/ JOI)
 *
 * This differs from VeinJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VeinOrSubClassJoiSchema = Joi.alternatives().try([
  VeinJoiSchema,
]);

/**
 * Runtime validator for schema:Vein.
 *
 * If some data has a structure which matches a schema:Vein, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVein = Vein.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVein instanceof OaValidationError) {
 *   // From this point on, `maybeVein` will have type `OaValidationError`
 *   const error = maybeVein;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVein` will have type `Vein`
 * const vein = maybeVein;
 * ```
 */
export function validateVein(maybeVein: unknown): Vein | OaValidationError {
  const { value, error } = VeinJoiSchema.validate(maybeVein);
  if (error) {
    return new OaValidationError('Vein', maybeVein, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Vein;
}
