 ...
 * schema:Vessel
 *
 * This differs from Vessel because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VesselOrSubClass =
  | Vessel
  | schema.ArteryOrSubClass
  | schema.LymphaticVesselOrSubClass
  | schema.VeinOrSubClass
  ;

/**
 * schema:Vessel - Validation schema (w/ JOI)
 */
export const VesselJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const VesselOrSubClassJoiSchema = Joi.alternatives().try([
  VesselJoiSchema,
 ...
 * ```
 */
export function validateVessel(maybeVessel: unknown): Vessel | OaValidationError {
  const { value, error } = VesselJoiSchema.validate(maybeVessel);
  if (error) {
    return new OaValidationError('Vessel', maybeVessel, error);
  }
