 ...
 * schema:VeterinaryCare
 *
 * This differs from VeterinaryCare because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VeterinaryCareOrSubClass =
  | VeterinaryCare
  ;

/**
 * schema:VeterinaryCare - Validation schema (w/ JOI)
 */
export const VeterinaryCareJoiSchema = Joi.object({
  '@type': Joi.string().valid('VeterinaryCare').required(),
 ...
 * schema:VeterinaryCare - Validation schema (w/ JOI)
 *
 * This differs from VeterinaryCareJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VeterinaryCareOrSubClassJoiSchema = Joi.alternatives().try([
  VeterinaryCareJoiSchema,
]);

/**
 * Runtime validator for schema:VeterinaryCare.
 *
 * If some data has a structure which matches a schema:VeterinaryCare, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeVeterinaryCare` will have type `VeterinaryCare`
 * const veterinaryCare = maybeVeterinaryCare;
 * ```
 */
export function validateVeterinaryCare(maybeVeterinaryCare: unknown): VeterinaryCare | OaValidationError {
  const { value, error } = VeterinaryCareJoiSchema.validate(maybeVeterinaryCare);
  if (error) {
    return new OaValidationError('VeterinaryCare', maybeVeterinaryCare, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VeterinaryCare;
}
