 ...
 * schema:VideoGallery
 *
 * This differs from VideoGallery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VideoGalleryOrSubClass =
  | VideoGallery
  ;

/**
 * schema:VideoGallery - Validation schema (w/ JOI)
 */
export const VideoGalleryJoiSchema = Joi.object({
  '@type': Joi.string().valid('VideoGallery').required(),
 ...
 * schema:VideoGallery - Validation schema (w/ JOI)
 *
 * This differs from VideoGalleryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VideoGalleryOrSubClassJoiSchema = Joi.alternatives().try([
  VideoGalleryJoiSchema,
]);

/**
 * Runtime validator for schema:VideoGallery.
 *
 * If some data has a structure which matches a schema:VideoGallery, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVideoGallery = VideoGallery.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVideoGallery instanceof OaValidationError) {
 *   // From this point on, `maybeVideoGallery` will have type `OaValidationError`
 *   const error = maybeVideoGallery;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVideoGallery` will have type `VideoGallery`
 * const videoGallery = maybeVideoGallery;
 * ```
 */
export function validateVideoGallery(maybeVideoGallery: unknown): VideoGallery | OaValidationError {
  const { value, error } = VideoGalleryJoiSchema.validate(maybeVideoGallery);
  if (error) {
    return new OaValidationError('VideoGallery', maybeVideoGallery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VideoGallery;
}
