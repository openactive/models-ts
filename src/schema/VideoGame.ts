...
export type VideoGame = {
   ...
   */
  gameTip?: schema.CreativeWorkOrSubClass | string;
  /**
   * The server on which  it is possible to play the game.
   */
  gameServer?: schema.GameServerOrSubClass | string;
  /**
   ...
   */
  screenshot?: string | schema.ImageObjectOrSubClass;
  /**
   ...
   */
  softwareAddOn?: schema.SoftwareApplicationOrSubClass | string;
  /**
   ...
   */
  softwareHelp?: schema.CreativeWorkOrSubClass | string;
  /**
   ...
   */
  supportingData?: schema.DataFeedOrSubClass | string;
  /**
 ...
 * schema:VideoGame
 *
 * This differs from VideoGame because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VideoGameOrSubClass =
  | VideoGame
  ;

/**
 * schema:VideoGame - Validation schema (w/ JOI)
 */
export const VideoGameJoiSchema = Joi.object({
  '@type': Joi.string().valid('VideoGame').required(),
  ...
  gameTip: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
  gameServer: Joi.alternatives().try(Joi.lazy(() => schema.GameServerOrSubClassJoiSchema), Joi.string().uri()),
  ...
  applicationSubCategory: Joi.alternatives().try(Joi.string().uri(), Joi.string()),
  ...
  softwareRequirements: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
  ...
  fileSize: Joi.string(),
  releaseNotes: Joi.alternatives().try(Joi.string(), Joi.string().uri()),
 ...
 * schema:VideoGame - Validation schema (w/ JOI)
 *
 * This differs from VideoGameJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VideoGameOrSubClassJoiSchema = Joi.alternatives().try([
  VideoGameJoiSchema,
]);

/**
 * Runtime validator for schema:VideoGame.
 *
 * If some data has a structure which matches a schema:VideoGame, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVideoGame = validateVideoGame(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVideoGame instanceof OaValidationError) {
 *   // From this point on, `maybeVideoGame` will have type `OaValidationError`
 *   const error = maybeVideoGame;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVideoGame` will have type `VideoGame`
 * const videoGame = maybeVideoGame;
 * ```
 */
export function validateVideoGame(maybeVideoGame: unknown): VideoGame | OaValidationError {
  const { value, error } = VideoGameJoiSchema.validate(maybeVideoGame);
  if (error) {
    return new OaValidationError('VideoGame', maybeVideoGame, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VideoGame;
}
