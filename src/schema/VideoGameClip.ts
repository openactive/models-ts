...
export type VideoGameClip = {
   ...
   */
  partOfEpisode?: schema.EpisodeOrSubClass | string;
  /**
   ...
   */
  endOffset?: schema.HyperTocEntryOrSubClass | number | string;
  /**
   ...
   */
  startOffset?: schema.HyperTocEntryOrSubClass | number | string;
  /**
 ...
 * schema:VideoGameClip
 *
 * This differs from VideoGameClip because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VideoGameClipOrSubClass =
  | VideoGameClip
  ;

/**
 * schema:VideoGameClip - Validation schema (w/ JOI)
 */
export const VideoGameClipJoiSchema = Joi.object({
  '@type': Joi.string().valid('VideoGameClip').required(),
  ...
  partOfEpisode: Joi.alternatives().try(Joi.lazy(() => schema.EpisodeOrSubClassJoiSchema), Joi.string().uri()),
  clipNumber: Joi.alternatives().try(Joi.number().integer(), Joi.string()),
  ...
  endOffset: Joi.alternatives().try(Joi.lazy(() => schema.HyperTocEntryOrSubClassJoiSchema), Joi.number(), Joi.string().uri()),
  ...
  startOffset: Joi.alternatives().try(Joi.lazy(() => schema.HyperTocEntryOrSubClassJoiSchema), Joi.number(), Joi.string().uri()),
  text: Joi.string(),
 ...
 * schema:VideoGameClip - Validation schema (w/ JOI)
 *
 * This differs from VideoGameClipJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VideoGameClipOrSubClassJoiSchema = Joi.alternatives().try([
  VideoGameClipJoiSchema,
]);

/**
 * Runtime validator for schema:VideoGameClip.
 *
 * If some data has a structure which matches a schema:VideoGameClip, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVideoGameClip = VideoGameClip.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVideoGameClip instanceof OaValidationError) {
 *   // From this point on, `maybeVideoGameClip` will have type `OaValidationError`
 *   const error = maybeVideoGameClip;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVideoGameClip` will have type `VideoGameClip`
 * const videoGameClip = maybeVideoGameClip;
 * ```
 */
export function validateVideoGameClip(maybeVideoGameClip: unknown): VideoGameClip | OaValidationError {
  const { value, error } = VideoGameClipJoiSchema.validate(maybeVideoGameClip);
  if (error) {
    return new OaValidationError('VideoGameClip', maybeVideoGameClip, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VideoGameClip;
}
