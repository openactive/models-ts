...
export type VideoGameSeries = {
   ...
   */
  numberOfPlayers?: schema.QuantitativeValueOrSubClass | string;
  /**
   * A season in a media series.
   */
  seasons?: schema.CreativeWorkSeasonOrSubClass | string;
  /**
   * A season in a media series.
   */
  season?: schema.CreativeWorkSeasonOrSubClass | string;
  /**
   ...
   */
  gameLocation?: schema.PostalAddressOrSubClass | schema.PlaceOrSubClass | string;
  /**
   ...
   */
  episodes?: schema.EpisodeOrSubClass | string;
  /**
   * An item is an object within the game world that can be collected by a player or, occasionally, a non-player character.
   */
  gameItem?: schema.ThingOrSubClass | string;
  /**
   * The electronic systems used to play <a href="http://en.wikipedia.org/wiki/Category:Video_game_platforms">video games</a>.
   */
  gamePlatform?: schema.ThingOrSubClass | string;
  /**
   * A piece of data that represents a particular aspect of a fictional character (skill, power, character points, advantage, disadvantage).
   */
  characterAttribute?: schema.ThingOrSubClass | string;
  /**
   ...
   */
  containsSeason?: schema.CreativeWorkSeasonOrSubClass | string;
  /**
   * The task that a player-controlled character, or group of characters may complete in order to gain a reward.
   */
  quest?: schema.ThingOrSubClass | string;
  /**
   * An episode of a tv, radio or game media within a series or season.
   */
  episode?: schema.EpisodeOrSubClass | string;
  /**
   ...
   */
  cheatCode?: schema.CreativeWorkOrSubClass | string;
  /**
 ...
 * schema:VideoGameSeries
 *
 * This differs from VideoGameSeries because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VideoGameSeriesOrSubClass =
  | VideoGameSeries
  ;

/**
 * schema:VideoGameSeries - Validation schema (w/ JOI)
 */
export const VideoGameSeriesJoiSchema = Joi.object({
  '@type': Joi.string().valid('VideoGameSeries').required(),
  ...
  numberOfPlayers: Joi.alternatives().try(Joi.lazy(() => schema.QuantitativeValueOrSubClassJoiSchema), Joi.string().uri()),
  seasons: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkSeasonOrSubClassJoiSchema), Joi.string().uri()),
  season: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkSeasonOrSubClassJoiSchema), Joi.string().uri()),
  playMode: Joi.lazy(() => schema.GamePlayModeJoiSchema),
  actor: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  numberOfSeasons: Joi.number().integer(),
  gameLocation: Joi.alternatives().try(Joi.lazy(() => schema.PostalAddressOrSubClassJoiSchema), Joi.lazy(() => schema.PlaceOrSubClassJoiSchema), Joi.string().uri()),
  ...
  episodes: Joi.alternatives().try(Joi.lazy(() => schema.EpisodeOrSubClassJoiSchema), Joi.string().uri()),
  gameItem: Joi.alternatives().try(Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri()),
  gamePlatform: Joi.alternatives().try(Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri(), Joi.string()),
  characterAttribute: Joi.alternatives().try(Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri()),
  actors: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
  containsSeason: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkSeasonOrSubClassJoiSchema), Joi.string().uri()),
  quest: Joi.alternatives().try(Joi.lazy(() => schema.ThingOrSubClassJoiSchema), Joi.string().uri()),
  episode: Joi.alternatives().try(Joi.lazy(() => schema.EpisodeOrSubClassJoiSchema), Joi.string().uri()),
  ...
  cheatCode: Joi.alternatives().try(Joi.lazy(() => schema.CreativeWorkOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:VideoGameSeries - Validation schema (w/ JOI)
 *
 * This differs from VideoGameSeriesJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VideoGameSeriesOrSubClassJoiSchema = Joi.alternatives().try([
  VideoGameSeriesJoiSchema,
]);

/**
 * Runtime validator for schema:VideoGameSeries.
 *
 * If some data has a structure which matches a schema:VideoGameSeries, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVideoGameSeries = VideoGameSeries.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVideoGameSeries instanceof OaValidationError) {
 *   // From this point on, `maybeVideoGameSeries` will have type `OaValidationError`
 *   const error = maybeVideoGameSeries;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVideoGameSeries` will have type `VideoGameSeries`
 * const videoGameSeries = maybeVideoGameSeries;
 * ```
 */
export function validateVideoGameSeries(maybeVideoGameSeries: unknown): VideoGameSeries | OaValidationError {
  const { value, error } = VideoGameSeriesJoiSchema.validate(maybeVideoGameSeries);
  if (error) {
    return new OaValidationError('VideoGameSeries', maybeVideoGameSeries, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VideoGameSeries;
}
