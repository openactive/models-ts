 ...
 * schema:VideoObject
 *
 * This differs from VideoObject because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VideoObjectOrSubClass =
  | VideoObject
  | schema.VideoObjectSnapshotOrSubClass
  ;

/**
 * schema:VideoObject - Validation schema (w/ JOI)
 */
export const VideoObjectJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const VideoObjectOrSubClassJoiSchema = Joi.alternatives().try([
  VideoObjectJoiSchema,
  Joi.lazy(() => schema.VideoObjectSnapshotOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:VideoObject.
 *
 * If some data has a structure which matches a schema:VideoObject, it will be returned with the correct type.
 ...
 * ```
 */
export function validateVideoObject(maybeVideoObject: unknown): VideoObject | OaValidationError {
  const { value, error } = VideoObjectJoiSchema.validate(maybeVideoObject);
  if (error) {
    return new OaValidationError('VideoObject', maybeVideoObject, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VideoObject;
}
