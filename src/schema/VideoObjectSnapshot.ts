 ...
 */
export type VideoObjectSnapshot = {
 ...
 * schema:VideoObjectSnapshot
 *
 * This differs from VideoObjectSnapshot because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VideoObjectSnapshotOrSubClass =
  | VideoObjectSnapshot
  ;

/**
 * schema:VideoObjectSnapshot - Validation schema (w/ JOI)
 */
export const VideoObjectSnapshotJoiSchema = Joi.object({
  '@type': Joi.string().valid('VideoObjectSnapshot').required(),
 ...
 * schema:VideoObjectSnapshot - Validation schema (w/ JOI)
 *
 * This differs from VideoObjectSnapshotJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VideoObjectSnapshotOrSubClassJoiSchema = Joi.alternatives().try([
  VideoObjectSnapshotJoiSchema,
]);

/**
 * Runtime validator for schema:VideoObjectSnapshot.
 *
 * If some data has a structure which matches a schema:VideoObjectSnapshot, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVideoObjectSnapshot = validateVideoObjectSnapshot(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVideoObjectSnapshot instanceof OaValidationError) {
 *   // From this point on, `maybeVideoObjectSnapshot` will have type `OaValidationError`
 *   const error = maybeVideoObjectSnapshot;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVideoObjectSnapshot` will have type `VideoObjectSnapshot`
 * const videoObjectSnapshot = maybeVideoObjectSnapshot;
 * ```
 */
export function validateVideoObjectSnapshot(maybeVideoObjectSnapshot: unknown): VideoObjectSnapshot | OaValidationError {
  const { value, error } = VideoObjectSnapshotJoiSchema.validate(maybeVideoObjectSnapshot);
  if (error) {
    return new OaValidationError('VideoObjectSnapshot', maybeVideoObjectSnapshot, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VideoObjectSnapshot;
}
