 ...
 * schema:ViewAction
 *
 * This differs from ViewAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ViewActionOrSubClass =
  | ViewAction
  ;

/**
 * schema:ViewAction - Validation schema (w/ JOI)
 */
export const ViewActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('ViewAction').required(),
 ...
 * schema:ViewAction - Validation schema (w/ JOI)
 *
 * This differs from ViewActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ViewActionOrSubClassJoiSchema = Joi.alternatives().try([
  ViewActionJoiSchema,
]);

/**
 * Runtime validator for schema:ViewAction.
 *
 * If some data has a structure which matches a schema:ViewAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeViewAction = ViewAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeViewAction instanceof OaValidationError) {
 *   // From this point on, `maybeViewAction` will have type `OaValidationError`
 *   const error = maybeViewAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeViewAction` will have type `ViewAction`
 * const viewAction = maybeViewAction;
 * ```
 */
export function validateViewAction(maybeViewAction: unknown): ViewAction | OaValidationError {
  const { value, error } = ViewActionJoiSchema.validate(maybeViewAction);
  if (error) {
    return new OaValidationError('ViewAction', maybeViewAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as ViewAction;
}
