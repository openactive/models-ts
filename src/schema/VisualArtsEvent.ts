 ...
 * schema:VisualArtsEvent
 *
 * This differs from VisualArtsEvent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VisualArtsEventOrSubClass =
  | VisualArtsEvent
  ;

/**
 * schema:VisualArtsEvent - Validation schema (w/ JOI)
 */
export const VisualArtsEventJoiSchema = Joi.object({
  '@type': Joi.string().valid('VisualArtsEvent').required(),
 ...
 * schema:VisualArtsEvent - Validation schema (w/ JOI)
 *
 * This differs from VisualArtsEventJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VisualArtsEventOrSubClassJoiSchema = Joi.alternatives().try([
  VisualArtsEventJoiSchema,
]);

/**
 * Runtime validator for schema:VisualArtsEvent.
 *
 * If some data has a structure which matches a schema:VisualArtsEvent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVisualArtsEvent = VisualArtsEvent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVisualArtsEvent instanceof OaValidationError) {
 *   // From this point on, `maybeVisualArtsEvent` will have type `OaValidationError`
 *   const error = maybeVisualArtsEvent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVisualArtsEvent` will have type `VisualArtsEvent`
 * const visualArtsEvent = maybeVisualArtsEvent;
 * ```
 */
export function validateVisualArtsEvent(maybeVisualArtsEvent: unknown): VisualArtsEvent | OaValidationError {
  const { value, error } = VisualArtsEventJoiSchema.validate(maybeVisualArtsEvent);
  if (error) {
    return new OaValidationError('VisualArtsEvent', maybeVisualArtsEvent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VisualArtsEvent;
}
