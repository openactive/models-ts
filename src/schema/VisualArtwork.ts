 ...
 * schema:VisualArtwork
 *
 * This differs from VisualArtwork because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VisualArtworkOrSubClass =
  | VisualArtwork
  | schema.CoverArtOrSubClass
  ;

/**
 * schema:VisualArtwork - Validation schema (w/ JOI)
 */
export const VisualArtworkJoiSchema = Joi.object({
  '@type': Joi.string().valid('VisualArtwork').required(),
 ...
 * schema:VisualArtwork - Validation schema (w/ JOI)
 *
 * This differs from VisualArtworkJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VisualArtworkOrSubClassJoiSchema = Joi.alternatives().try([
  VisualArtworkJoiSchema,
  Joi.lazy(() => schema.CoverArtOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:VisualArtwork.
 *
 * If some data has a structure which matches a schema:VisualArtwork, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeVisualArtwork` will have type `VisualArtwork`
 * const visualArtwork = maybeVisualArtwork;
 * ```
 */
export function validateVisualArtwork(maybeVisualArtwork: unknown): VisualArtwork | OaValidationError {
  const { value, error } = VisualArtworkJoiSchema.validate(maybeVisualArtwork);
  if (error) {
    return new OaValidationError('VisualArtwork', maybeVisualArtwork, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VisualArtwork;
}
