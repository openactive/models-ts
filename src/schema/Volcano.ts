 ...
 * schema:Volcano
 *
 * This differs from Volcano because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type VolcanoOrSubClass =
  | Volcano
  ;

/**
 * schema:Volcano - Validation schema (w/ JOI)
 */
export const VolcanoJoiSchema = Joi.object({
  '@type': Joi.string().valid('Volcano').required(),
 ...
 * schema:Volcano - Validation schema (w/ JOI)
 *
 * This differs from VolcanoJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const VolcanoOrSubClassJoiSchema = Joi.alternatives().try([
  VolcanoJoiSchema,
]);

/**
 * Runtime validator for schema:Volcano.
 *
 * If some data has a structure which matches a schema:Volcano, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeVolcano = Volcano.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeVolcano instanceof OaValidationError) {
 *   // From this point on, `maybeVolcano` will have type `OaValidationError`
 *   const error = maybeVolcano;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeVolcano` will have type `Volcano`
 * const volcano = maybeVolcano;
 * ```
 */
export function validateVolcano(maybeVolcano: unknown): Volcano | OaValidationError {
  const { value, error } = VolcanoJoiSchema.validate(maybeVolcano);
  if (error) {
    return new OaValidationError('Volcano', maybeVolcano, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Volcano;
}
