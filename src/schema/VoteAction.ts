 ...
 * etc.
 */
export type VoteActionOrSubClass =
  | VoteAction
  ;
 ...
 */
export const VoteActionJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const VoteActionOrSubClassJoiSchema = Joi.alternatives().try([
  VoteActionJoiSchema,
]);
 ...
 * ```
 */
export function validateVoteAction(maybeVoteAction: unknown): VoteAction | OaValidationError {
  const { value, error } = VoteActionJoiSchema.validate(maybeVoteAction);
  if (error) {
    return new OaValidationError('VoteAction', maybeVoteAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as VoteAction;
}
