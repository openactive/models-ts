 ...
 * schema:WPAdBlock
 *
 * This differs from WPAdBlock because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WPAdBlockOrSubClass =
  | WPAdBlock
  ;

/**
 * schema:WPAdBlock - Validation schema (w/ JOI)
 */
export const WPAdBlockJoiSchema = Joi.object({
  '@type': Joi.string().valid('WPAdBlock').required(),
 ...
 * schema:WPAdBlock - Validation schema (w/ JOI)
 *
 * This differs from WPAdBlockJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WPAdBlockOrSubClassJoiSchema = Joi.alternatives().try([
  WPAdBlockJoiSchema,
]);

/**
 * Runtime validator for schema:WPAdBlock.
 *
 * If some data has a structure which matches a schema:WPAdBlock, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeWPAdBlock` will have type `WPAdBlock`
 * const wPAdBlock = maybeWPAdBlock;
 * ```
 */
export function validateWPAdBlock(maybeWPAdBlock: unknown): WPAdBlock | OaValidationError {
  const { value, error } = WPAdBlockJoiSchema.validate(maybeWPAdBlock);
  if (error) {
    return new OaValidationError('WPAdBlock', maybeWPAdBlock, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WPAdBlock;
}
