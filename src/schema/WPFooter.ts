 ...
 * schema:WPFooter
 *
 * This differs from WPFooter because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WPFooterOrSubClass =
  | WPFooter
  ;

/**
 * schema:WPFooter - Validation schema (w/ JOI)
 */
export const WPFooterJoiSchema = Joi.object({
  '@type': Joi.string().valid('WPFooter').required(),
 ...
 * schema:WPFooter - Validation schema (w/ JOI)
 *
 * This differs from WPFooterJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WPFooterOrSubClassJoiSchema = Joi.alternatives().try([
  WPFooterJoiSchema,
]);

/**
 * Runtime validator for schema:WPFooter.
 *
 * If some data has a structure which matches a schema:WPFooter, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeWPFooter` will have type `WPFooter`
 * const wPFooter = maybeWPFooter;
 * ```
 */
export function validateWPFooter(maybeWPFooter: unknown): WPFooter | OaValidationError {
  const { value, error } = WPFooterJoiSchema.validate(maybeWPFooter);
  if (error) {
    return new OaValidationError('WPFooter', maybeWPFooter, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WPFooter;
}
