 ...
 * schema:WPHeader
 *
 * This differs from WPHeader because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WPHeaderOrSubClass =
  | WPHeader
  ;

/**
 * schema:WPHeader - Validation schema (w/ JOI)
 */
export const WPHeaderJoiSchema = Joi.object({
  '@type': Joi.string().valid('WPHeader').required(),
 ...
 * schema:WPHeader - Validation schema (w/ JOI)
 *
 * This differs from WPHeaderJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WPHeaderOrSubClassJoiSchema = Joi.alternatives().try([
  WPHeaderJoiSchema,
]);

/**
 * Runtime validator for schema:WPHeader.
 *
 * If some data has a structure which matches a schema:WPHeader, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeWPHeader` will have type `WPHeader`
 * const wPHeader = maybeWPHeader;
 * ```
 */
export function validateWPHeader(maybeWPHeader: unknown): WPHeader | OaValidationError {
  const { value, error } = WPHeaderJoiSchema.validate(maybeWPHeader);
  if (error) {
    return new OaValidationError('WPHeader', maybeWPHeader, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WPHeader;
}
