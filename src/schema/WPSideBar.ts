 ...
 * schema:WPSideBar
 *
 * This differs from WPSideBar because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WPSideBarOrSubClass =
  | WPSideBar
  ;

/**
 * schema:WPSideBar - Validation schema (w/ JOI)
 */
export const WPSideBarJoiSchema = Joi.object({
  '@type': Joi.string().valid('WPSideBar').required(),
 ...
 * schema:WPSideBar - Validation schema (w/ JOI)
 *
 * This differs from WPSideBarJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WPSideBarOrSubClassJoiSchema = Joi.alternatives().try([
  WPSideBarJoiSchema,
]);

/**
 * Runtime validator for schema:WPSideBar.
 *
 * If some data has a structure which matches a schema:WPSideBar, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWPSideBar = WPSideBar.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWPSideBar instanceof OaValidationError) {
 *   // From this point on, `maybeWPSideBar` will have type `OaValidationError`
 *   const error = maybeWPSideBar;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWPSideBar` will have type `WPSideBar`
 * const wPSideBar = maybeWPSideBar;
 * ```
 */
export function validateWPSideBar(maybeWPSideBar: unknown): WPSideBar | OaValidationError {
  const { value, error } = WPSideBarJoiSchema.validate(maybeWPSideBar);
  if (error) {
    return new OaValidationError('WPSideBar', maybeWPSideBar, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WPSideBar;
}
