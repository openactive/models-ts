 ...
 * schema:WantAction
 *
 * This differs from WantAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WantActionOrSubClass =
  | WantAction
  ;

/**
 * schema:WantAction - Validation schema (w/ JOI)
 */
export const WantActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('WantAction').required(),
 ...
 * schema:WantAction - Validation schema (w/ JOI)
 *
 * This differs from WantActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WantActionOrSubClassJoiSchema = Joi.alternatives().try([
  WantActionJoiSchema,
]);

/**
 * Runtime validator for schema:WantAction.
 *
 * If some data has a structure which matches a schema:WantAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWantAction = WantAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWantAction instanceof OaValidationError) {
 *   // From this point on, `maybeWantAction` will have type `OaValidationError`
 *   const error = maybeWantAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWantAction` will have type `WantAction`
 * const wantAction = maybeWantAction;
 * ```
 */
export function validateWantAction(maybeWantAction: unknown): WantAction | OaValidationError {
  const { value, error } = WantActionJoiSchema.validate(maybeWantAction);
  if (error) {
    return new OaValidationError('WantAction', maybeWantAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WantAction;
}
