 ...
 * etc.
 */
export type WarrantyPromiseOrSubClass =
  | WarrantyPromise
  ;
 ...
 */
export const WarrantyPromiseJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const WarrantyPromiseOrSubClassJoiSchema = Joi.alternatives().try([
  WarrantyPromiseJoiSchema,
]);
 ...
 * ```
 */
export function validateWarrantyPromise(maybeWarrantyPromise: unknown): WarrantyPromise | OaValidationError {
  const { value, error } = WarrantyPromiseJoiSchema.validate(maybeWarrantyPromise);
  if (error) {
    return new OaValidationError('WarrantyPromise', maybeWarrantyPromise, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WarrantyPromise;
}
