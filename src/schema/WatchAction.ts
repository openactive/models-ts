 ...
 * schema:WatchAction
 *
 * This differs from WatchAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WatchActionOrSubClass =
  | WatchAction
  ;

/**
 * schema:WatchAction - Validation schema (w/ JOI)
 */
export const WatchActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('WatchAction').required(),
 ...
 * schema:WatchAction - Validation schema (w/ JOI)
 *
 * This differs from WatchActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WatchActionOrSubClassJoiSchema = Joi.alternatives().try([
  WatchActionJoiSchema,
]);

/**
 * Runtime validator for schema:WatchAction.
 *
 * If some data has a structure which matches a schema:WatchAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWatchAction = WatchAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWatchAction instanceof OaValidationError) {
 *   // From this point on, `maybeWatchAction` will have type `OaValidationError`
 *   const error = maybeWatchAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWatchAction` will have type `WatchAction`
 * const watchAction = maybeWatchAction;
 * ```
 */
export function validateWatchAction(maybeWatchAction: unknown): WatchAction | OaValidationError {
  const { value, error } = WatchActionJoiSchema.validate(maybeWatchAction);
  if (error) {
    return new OaValidationError('WatchAction', maybeWatchAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WatchAction;
}
