 ...
 * schema:Waterfall
 *
 * This differs from Waterfall because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WaterfallOrSubClass =
  | Waterfall
  ;
 ...
 */
export const WaterfallJoiSchema = Joi.object({
 ...
 * `SessionSeries`, etc.
 */
export const WaterfallOrSubClassJoiSchema = Joi.alternatives().try([
  WaterfallJoiSchema,
]);
 ...
 * ```
 */
export function validateWaterfall(maybeWaterfall: unknown): Waterfall | OaValidationError {
  const { value, error } = WaterfallJoiSchema.validate(maybeWaterfall);
  if (error) {
    return new OaValidationError('Waterfall', maybeWaterfall, error);
  }
