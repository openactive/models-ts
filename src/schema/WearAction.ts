 ...
 * schema:WearAction
 *
 * This differs from WearAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WearActionOrSubClass =
  | WearAction
  ;

/**
 * schema:WearAction - Validation schema (w/ JOI)
 */
export const WearActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('WearAction').required(),
 ...
 * schema:WearAction - Validation schema (w/ JOI)
 *
 * This differs from WearActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WearActionOrSubClassJoiSchema = Joi.alternatives().try([
  WearActionJoiSchema,
]);

/**
 * Runtime validator for schema:WearAction.
 *
 * If some data has a structure which matches a schema:WearAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWearAction = WearAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWearAction instanceof OaValidationError) {
 *   // From this point on, `maybeWearAction` will have type `OaValidationError`
 *   const error = maybeWearAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWearAction` will have type `WearAction`
 * const wearAction = maybeWearAction;
 * ```
 */
export function validateWearAction(maybeWearAction: unknown): WearAction | OaValidationError {
  const { value, error } = WearActionJoiSchema.validate(maybeWearAction);
  if (error) {
    return new OaValidationError('WearAction', maybeWearAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WearAction;
}
