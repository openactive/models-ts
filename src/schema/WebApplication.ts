 ...
 * schema:WebApplication
 *
 * This differs from WebApplication because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WebApplicationOrSubClass =
  | WebApplication
  ;

/**
 * schema:WebApplication - Validation schema (w/ JOI)
 */
export const WebApplicationJoiSchema = Joi.object({
  '@type': Joi.string().valid('WebApplication').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  browserRequirements: Joi.string(),
 ...
 * schema:WebApplication - Validation schema (w/ JOI)
 *
 * This differs from WebApplicationJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WebApplicationOrSubClassJoiSchema = Joi.alternatives().try([
  WebApplicationJoiSchema,
]);

/**
 * Runtime validator for schema:WebApplication.
 *
 * If some data has a structure which matches a schema:WebApplication, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeWebApplication` will have type `WebApplication`
 * const webApplication = maybeWebApplication;
 * ```
 */
export function validateWebApplication(maybeWebApplication: unknown): WebApplication | OaValidationError {
  const { value, error } = WebApplicationJoiSchema.validate(maybeWebApplication);
  if (error) {
    return new OaValidationError('WebApplication', maybeWebApplication, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WebApplication;
}
