 ...
 * schema:WebContent
 *
 * This differs from WebContent because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WebContentOrSubClass =
  | WebContent
  | schema.HealthTopicContentOrSubClass
  ;

/**
 * schema:WebContent - Validation schema (w/ JOI)
 */
export const WebContentJoiSchema = Joi.object({
  '@type': Joi.string().valid('WebContent').required(),
 ...
 * schema:WebContent - Validation schema (w/ JOI)
 *
 * This differs from WebContentJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WebContentOrSubClassJoiSchema = Joi.alternatives().try([
  WebContentJoiSchema,
  Joi.lazy(() => schema.HealthTopicContentOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:WebContent.
 *
 * If some data has a structure which matches a schema:WebContent, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWebContent = WebContent.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWebContent instanceof OaValidationError) {
 *   // From this point on, `maybeWebContent` will have type `OaValidationError`
 *   const error = maybeWebContent;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWebContent` will have type `WebContent`
 * const webContent = maybeWebContent;
 * ```
 */
export function validateWebContent(maybeWebContent: unknown): WebContent | OaValidationError {
  const { value, error } = WebContentJoiSchema.validate(maybeWebContent);
  if (error) {
    return new OaValidationError('WebContent', maybeWebContent, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WebContent;
}
