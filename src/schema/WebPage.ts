 ...
 * schema:WebPage
 *
 * This differs from WebPage because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WebPageOrSubClass =
  | WebPage
  | schema.SearchResultsPageOrSubClass
  | schema.QAPageOrSubClass
  | schema.RealEstateListingOrSubClass
  | schema.FAQPageOrSubClass
  | schema.AboutPageOrSubClass
  | schema.CheckoutPageOrSubClass
  | schema.ContactPageOrSubClass
  | schema.ItemPageOrSubClass
  | schema.CollectionPageOrSubClass
  | schema.ProfilePageOrSubClass
  | schema.MedicalWebPageOrSubClass
  ;

/**
 * schema:WebPage - Validation schema (w/ JOI)
 */
export const WebPageJoiSchema = Joi.object({
  '@type': Joi.string().valid('WebPage').required(),
 ...
 * schema:WebPage - Validation schema (w/ JOI)
 *
 * This differs from WebPageJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WebPageOrSubClassJoiSchema = Joi.alternatives().try([
  WebPageJoiSchema,
  Joi.lazy(() => schema.SearchResultsPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.QAPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.RealEstateListingOrSubClassJoiSchema),
  Joi.lazy(() => schema.FAQPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.AboutPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.CheckoutPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.ContactPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.ItemPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.CollectionPageOrSubClassJoiSchema),
  Joi.lazy(() => schema.ProfilePageOrSubClassJoiSchema),
  Joi.lazy(() => schema.MedicalWebPageOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:WebPage.
 *
 * If some data has a structure which matches a schema:WebPage, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWebPage = WebPage.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWebPage instanceof OaValidationError) {
 *   // From this point on, `maybeWebPage` will have type `OaValidationError`
 *   const error = maybeWebPage;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWebPage` will have type `WebPage`
 * const webPage = maybeWebPage;
 * ```
 */
export function validateWebPage(maybeWebPage: unknown): WebPage | OaValidationError {
  const { value, error } = WebPageJoiSchema.validate(maybeWebPage);
  if (error) {
    return new OaValidationError('WebPage', maybeWebPage, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WebPage;
}
