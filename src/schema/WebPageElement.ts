...
export type WebPageElement = {
   ...
   */
  xpath?: schema.XPathTypeOrSubClass;
  /**
   * A CSS selector, e.g. of a [[SpeakableSpecification]] or [[WebPageElement]]. In the latter case, multiple matches within a page can constitute a single conceptual "Web page element".
   */
  cssSelector?: schema.CssSelectorTypeOrSubClass;
  /**
 ...
 * schema:WebPageElement
 *
 * This differs from WebPageElement because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WebPageElementOrSubClass =
  | WebPageElement
  | schema.WPAdBlockOrSubClass
  | schema.TableOrSubClass
  | schema.WPFooterOrSubClass
  | schema.WPSideBarOrSubClass
  | schema.SiteNavigationElementOrSubClass
  | schema.WPHeaderOrSubClass
  ;

/**
 * schema:WebPageElement - Validation schema (w/ JOI)
 */
export const WebPageElementJoiSchema = Joi.object({
  '@type': Joi.string().valid('WebPageElement').required(),
 ...
 * schema:WebPageElement - Validation schema (w/ JOI)
 *
 * This differs from WebPageElementJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WebPageElementOrSubClassJoiSchema = Joi.alternatives().try([
  WebPageElementJoiSchema,
  Joi.lazy(() => schema.WPAdBlockOrSubClassJoiSchema),
  Joi.lazy(() => schema.TableOrSubClassJoiSchema),
  Joi.lazy(() => schema.WPFooterOrSubClassJoiSchema),
  Joi.lazy(() => schema.WPSideBarOrSubClassJoiSchema),
  Joi.lazy(() => schema.SiteNavigationElementOrSubClassJoiSchema),
  Joi.lazy(() => schema.WPHeaderOrSubClassJoiSchema),
]);

/**
 * Runtime validator for schema:WebPageElement.
 *
 * If some data has a structure which matches a schema:WebPageElement, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWebPageElement = WebPageElement.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWebPageElement instanceof OaValidationError) {
 *   // From this point on, `maybeWebPageElement` will have type `OaValidationError`
 *   const error = maybeWebPageElement;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWebPageElement` will have type `WebPageElement`
 * const webPageElement = maybeWebPageElement;
 * ```
 */
export function validateWebPageElement(maybeWebPageElement: unknown): WebPageElement | OaValidationError {
  const { value, error } = WebPageElementJoiSchema.validate(maybeWebPageElement);
  if (error) {
    return new OaValidationError('WebPageElement', maybeWebPageElement, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WebPageElement;
}
