 ...
 * schema:WebSite
 *
 * This differs from WebSite because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WebSiteOrSubClass =
  | WebSite
  ;

/**
 * schema:WebSite - Validation schema (w/ JOI)
 */
export const WebSiteJoiSchema = Joi.object({
 ...
 * schema:WebSite - Validation schema (w/ JOI)
 *
 * This differs from WebSiteJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WebSiteOrSubClassJoiSchema = Joi.alternatives().try([
  WebSiteJoiSchema,
]);

/**
 * Runtime validator for schema:WebSite.
 *
 * If some data has a structure which matches a schema:WebSite, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWebSite = WebSite.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWebSite instanceof OaValidationError) {
 *   // From this point on, `maybeWebSite` will have type `OaValidationError`
 *   const error = maybeWebSite;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWebSite` will have type `WebSite`
 * const webSite = maybeWebSite;
 * ```
 */
export function validateWebSite(maybeWebSite: unknown): WebSite | OaValidationError {
  const { value, error } = WebSiteJoiSchema.validate(maybeWebSite);
  if (error) {
    return new OaValidationError('WebSite', maybeWebSite, error);
  }
