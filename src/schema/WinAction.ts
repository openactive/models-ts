...
export type WinAction = {
   ...
   */
  loser?: schema.PersonOrSubClass | string;
  /**
 ...
 * schema:WinAction
 *
 * This differs from WinAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WinActionOrSubClass =
  | WinAction
  ;

/**
 * schema:WinAction - Validation schema (w/ JOI)
 */
export const WinActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('WinAction').required(),
  '@context': Joi.alternatives().try([Joi.string(), Joi.array().items(Joi.string())]),
  identifier: Joi.alternatives().try(Joi.lazy(() => schema.PropertyValueOrSubClassJoiSchema), Joi.string(), Joi.string().uri()),
  name: Joi.string(),
  description: Joi.string(),
  '@id': Joi.string().uri(),
  loser: Joi.alternatives().try(Joi.lazy(() => schema.PersonOrSubClassJoiSchema), Joi.string().uri()),
 ...
 * schema:WinAction - Validation schema (w/ JOI)
 *
 * This differs from WinActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WinActionOrSubClassJoiSchema = Joi.alternatives().try([
  WinActionJoiSchema,
]);

/**
 * Runtime validator for schema:WinAction.
 *
 * If some data has a structure which matches a schema:WinAction, it will be returned with the correct type.
 ...
 * }
 * // From this point on, `maybeWinAction` will have type `WinAction`
 * const winAction = maybeWinAction;
 * ```
 */
export function validateWinAction(maybeWinAction: unknown): WinAction | OaValidationError {
  const { value, error } = WinActionJoiSchema.validate(maybeWinAction);
  if (error) {
    return new OaValidationError('WinAction', maybeWinAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WinAction;
}
