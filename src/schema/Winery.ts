 ...
 * schema:Winery
 *
 * This differs from Winery because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WineryOrSubClass =
  | Winery
  ;

/**
 * schema:Winery - Validation schema (w/ JOI)
 */
export const WineryJoiSchema = Joi.object({
  '@type': Joi.string().valid('Winery').required(),
 ...
 * schema:Winery - Validation schema (w/ JOI)
 *
 * This differs from WineryJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WineryOrSubClassJoiSchema = Joi.alternatives().try([
  WineryJoiSchema,
]);

/**
 * Runtime validator for schema:Winery.
 *
 * If some data has a structure which matches a schema:Winery, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWinery = Winery.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWinery instanceof OaValidationError) {
 *   // From this point on, `maybeWinery` will have type `OaValidationError`
 *   const error = maybeWinery;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWinery` will have type `Winery`
 * const winery = maybeWinery;
 * ```
 */
export function validateWinery(maybeWinery: unknown): Winery | OaValidationError {
  const { value, error } = WineryJoiSchema.validate(maybeWinery);
  if (error) {
    return new OaValidationError('Winery', maybeWinery, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as Winery;
}
