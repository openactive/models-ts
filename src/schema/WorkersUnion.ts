 ...
 * schema:WorkersUnion
 *
 * This differs from WorkersUnion because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WorkersUnionOrSubClass =
  | WorkersUnion
  ;

/**
 * schema:WorkersUnion - Validation schema (w/ JOI)
 */
export const WorkersUnionJoiSchema = Joi.object({
  '@type': Joi.string().valid('WorkersUnion').required(),
 ...
 * schema:WorkersUnion - Validation schema (w/ JOI)
 *
 * This differs from WorkersUnionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WorkersUnionOrSubClassJoiSchema = Joi.alternatives().try([
  WorkersUnionJoiSchema,
]);

/**
 * Runtime validator for schema:WorkersUnion.
 *
 * If some data has a structure which matches a schema:WorkersUnion, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWorkersUnion = WorkersUnion.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWorkersUnion instanceof OaValidationError) {
 *   // From this point on, `maybeWorkersUnion` will have type `OaValidationError`
 *   const error = maybeWorkersUnion;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWorkersUnion` will have type `WorkersUnion`
 * const workersUnion = maybeWorkersUnion;
 * ```
 */
export function validateWorkersUnion(maybeWorkersUnion: unknown): WorkersUnion | OaValidationError {
  const { value, error } = WorkersUnionJoiSchema.validate(maybeWorkersUnion);
  if (error) {
    return new OaValidationError('WorkersUnion', maybeWorkersUnion, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WorkersUnion;
}
