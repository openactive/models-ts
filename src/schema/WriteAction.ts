 ...
 * schema:WriteAction
 *
 * This differs from WriteAction because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type WriteActionOrSubClass =
  | WriteAction
  ;

/**
 * schema:WriteAction - Validation schema (w/ JOI)
 */
export const WriteActionJoiSchema = Joi.object({
  '@type': Joi.string().valid('WriteAction').required(),
 ...
 * schema:WriteAction - Validation schema (w/ JOI)
 *
 * This differs from WriteActionJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const WriteActionOrSubClassJoiSchema = Joi.alternatives().try([
  WriteActionJoiSchema,
]);

/**
 * Runtime validator for schema:WriteAction.
 *
 * If some data has a structure which matches a schema:WriteAction, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeWriteAction = WriteAction.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeWriteAction instanceof OaValidationError) {
 *   // From this point on, `maybeWriteAction` will have type `OaValidationError`
 *   const error = maybeWriteAction;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeWriteAction` will have type `WriteAction`
 * const writeAction = maybeWriteAction;
 * ```
 */
export function validateWriteAction(maybeWriteAction: unknown): WriteAction | OaValidationError {
  const { value, error } = WriteActionJoiSchema.validate(maybeWriteAction);
  if (error) {
    return new OaValidationError('WriteAction', maybeWriteAction, error);
  }
  /* Joi does not implement TS Type Guards, so TS does not implicitly know that this has now been validated
  to have the right type. Therefore, we just cast it to the right type. */
  return value as WriteAction;
}
