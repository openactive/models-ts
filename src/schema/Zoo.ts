 ...
 * schema:Zoo
 *
 * This differs from Zoo because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event_OrSubClass` allows `Event`s as well as `ScheduledSession`s, `SessionSeries`,
 * etc.
 */
export type ZooOrSubClass =
  | Zoo
  ;

/**
 * schema:Zoo - Validation schema (w/ JOI)
 */
export const ZooJoiSchema = Joi.object({
  '@type': Joi.string().valid('Zoo').required(),
 ...
 * schema:Zoo - Validation schema (w/ JOI)
 *
 * This differs from ZooJoiSchema because it also allows for objects that have the type of a model that
 * sub-classes this model. e.g. `Event.OrSubClassJoiSchema` allows `Event`s as well as `ScheduledSession`s,
 * `SessionSeries`, etc.
 */
export const ZooOrSubClassJoiSchema = Joi.alternatives().try([
  ZooJoiSchema,
]);

/**
 * Runtime validator for schema:Zoo.
 *
 * If some data has a structure which matches a schema:Zoo, it will be returned with the correct type.
 * Otherwise, this function returns an OAValidationError with details about why the data does not match.
 *
 * Use this to e.g. check a JSON object received in an HTTP request. Example usage (for an express request handler):
 *
 * ```ts
 * const maybeZoo = Zoo.validate(req.body); // `req.body` will have type `any` or `unknown`
 * if (maybeZoo instanceof OaValidationError) {
 *   // From this point on, `maybeZoo` will have type `OaValidationError`
 *   const error = maybeZoo;
 *   // Do something with the error. Maybe ignore it? Or log it? Or throw? Up to you.
 * }
 * // From this point on, `maybeZoo` will have type `Zoo`
 * const zoo = maybeZoo;
 * ```
 */
export function validateZoo(maybeZoo: unknown): Zoo | OaValidationError {
  const { value, error } = ZooJoiSchema.validate(maybeZoo);
  if (error) {
    return new OaValidationError('Zoo', maybeZoo, error);
  }
